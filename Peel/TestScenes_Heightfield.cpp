...
HeightfieldHelper::HeightfieldHelper(udword nb_u, udword nb_v)
    : mIndexBuffers(null), mNbU(nb_u), mNbV(nb_v), mScaleU(1.0f), mScaleV(1.0f), mSizeX(0.0f), mSizeZ(0.0f) {}
...
PintActorHandle HeightfieldHelper::Setup(Pint& pint, const PintCaps& caps, const Point& pos) {
    if (!caps.mSupportHeightfields) return null;
    ...
    if (0) {
        ...
        PintHeightfieldData HFData;
        {
            ...
            for (udword i = 0; i < mNbU * mNbV; i++) Heights[i] = V[i].y;
