...
c) yield the processor by informing it that it is waiting for work and requesting it to more efficiently use compute
resources.
*/
struct PxDefaultCpuDispatcherWaitForWorkMode {
    enum Enum { eWAIT_FOR_WORK, eYIELD_THREAD, eYIELD_PROCESSOR };
};
...
\note yieldProcessorCount must be  greater than zero if eYIELD_PROCESSOR is the chosen mode and equal to zero for all
other modes.
...
*/
PxDefaultCpuDispatcher* PxDefaultCpuDispatcherCreate(
        physx::PxU32 numThreads,
        physx::PxU32* affinityMasks = NULL,
        PxDefaultCpuDispatcherWaitForWorkMode::Enum mode = PxDefaultCpuDispatcherWaitForWorkMode::eWAIT_FOR_WORK,
        physx::PxU32 yieldProcessorCount = 0);
