...
void PVDHelper::createPvdConnection() {
    ...
    //	PxVisualDebuggerConnectionFlags theConnectionFlags( PxVisualDebuggerConnectionFlags(
    //PxVisualDebuggerConnectionFlag::Profile|PxVisualDebuggerConnectionFlag::Memory ) ); 	if(!gUseFullPvdConnection)
    ...
    /*	PVD::PvdConnection* theConnection = PxVisualDebuggerExt::createConnection(mPhysics->getPvdConnectionManager(),
       PVD_HOST, 5425, 10, theConnectionFlags ); if(theConnection) theConnection->release();*/
...
const char* PhysX::GetUIName() const { return "PhysX 4.1.2"; }
...
typedef physx::PxPhysics* (*PxFunction_CreateBasePhysics)(physx::PxU32 version,
...
typedef physx::PxCooking* (*PxFunction_CreateCooking)(physx::PxU32 version,
                                                      physx::PxFoundation& foundation,
                                                      const physx::PxCookingParams& params);
typedef physx::PxFoundation* (*PxFunction_CreateFoundation)(physx::PxU32 version,
...
void PhysX::Init(const PINT_WORLD_CREATE& desc) {
    ...
    __asm fstenv FPUEnv HMODULE handle0 = ::LoadLibraryA("PhysX3_x86.dll");
    HMODULE handle1 = ::LoadLibraryA("PhysX3Cooking_x86.dll");
    HMODULE handle2 = ::LoadLibraryA("PhysX3Common_x86.dll");
    __asm fldenv FPUEnv if (!handle0 || !handle1 || !handle2) return;
...
//	nvidia::pvdsdk::NvPvd* PvdSDK = null;
    ...
    // null);
    gDefaultCPUDispatcher =
            physx5::PxDefaultCpuDispatcherCreate(mParams.ThreadIndexToNbThreads(mParams.mNbThreadsIndex), null,
                                                 physx5::PxDefaultCpuDispatcherWaitForWorkMode::eYIELD_THREAD);
    ...
    if (gUsePVD) {
        ...
        if (mPhysics->getPvdConnectionManager()) mPhysics->getPvdConnectionManager()->addHandler(*gPVDHelper);
    }
#endif
...
#ifdef SUPPORT_PVD
    if (gPVDHelper) gPVDHelper->togglePvdConnection();
...
Point PhysX::GetMainColor() { return Point(238.0f / 255.0f, 223.0f / 255.0f, 204.0f / 255.0f); }
