...
struct ShadowMap::Vec4 {
    Vec4() {}
    Vec4(const Point& v, float vw = 1.0f) {
        x = v.x;
        y = v.y;
        z = v.z;
        w = vw;
...
struct ShadowMap::Matrix44 {
    ...
    void zero() {
        for (int i = 0; i < 16; i++) elems[i] = 0.0f;
    }
    void id() {
        zero();
        elems[0] = 1.0f;
...
//----------------------------------------------------------------------------------
ShadowMap::ShadowMap(int w, int h, float fovi, int matOffseti, int resolution, int nb_splits) {
    shadowOff = 1.0f;
    // shadowOff = 2.0f;
    shadowOff2 = 2048.0f;
    // shadowOff2 = 4096.0f;
    fov = fovi;
    cur_num_splits = nb_splits;
    // cur_num_splits = 1;
...
//----------------------------------------------------------------------------------
void ShadowMap::updateFrustumPoints(Frustum& f, const Point& center, const Point& view_dir) {
    Point up(0.0, 1.0, 0.0);
    Point right = view_dir ^ up;

    const Point fc = center + view_dir * f.fard;
    const Point nc = center + view_dir * f.neard;
    ...
    const float near_height = tanf(f.fov / 2.0f) * f.neard;
    const float near_width = near_height * f.ratio;
    const float far_height = tanf(f.fov / 2.0f) * f.fard;
    const float far_width = far_height * f.ratio;
...
float ShadowMap::applyCropMatrix(Frustum& f) {
    ...
    // transf.z = shad_modelview[2] * f.point[0].x + shad_modelview[6] * f.point[0].y + shad_modelview[10] *
    // f.point[0].z + shad_modelview[14];
...
void ShadowMap::makeShadowMap(const Point& cameraPos,
                              const Point& cameraDir,
                              const Point& lightDir,
                              float znear,
                              float zfar,
    ...
    gluLookAt(cameraPos.x, cameraPos.y, cameraPos.z, cameraPos.x - lightDir.x, cameraPos.y - lightDir.y,
              cameraPos.z - lightDir.z, -1.0, 0.0, 0.0);
...
static void cameraInverse(float* dst, const float* src) {
    dst[0] = src[0];
    dst[1] = src[4];
    dst[2] = src[8];
    dst[3] = 0.0;
...
void ShadowMap::prepareForRender(const float* cam_modelview, const float* cam_proj) {
    ...
    const float bias[16] = {0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.5, 0.5f, 0.5, 1.0};
    ...
    for (int i = cur_num_splits; i < MAX_SPLITS; i++) far_bound[i] = 0;
...
void ShadowMap::init() {
    ...
    // SystemTexture* ST = CreateSystemTexture(0, 0, depth_tex_ar, "ShadowMap");

    glBindTexture(GL_TEXTURE_2D_ARRAY_EXT, depth_tex_ar);
    glTexImage3D(GL_TEXTURE_2D_ARRAY_EXT, 0, GL_DEPTH_COMPONENT24, depth_size, depth_size, MAX_SPLITS, 0,
