...
public:
    ...
    virtual void OnClick() {
        if (GetID() == 0) mOwner.CreateCompound();
...
void CompoundCreateWindow::SetCompoundPos(const Point& pos) {
    ...
    if (mEditPos) mEditPos->SetPos(pos);
}

void CompoundCreateWindow::SetCompoundRot(const Quat& rot) {
    mCompoundPR.mRot = rot;
    if (mEditRot) mEditRot->SetEulerAngles(rot);
}

void CompoundCreateWindow::InitFrom(
    ...
    mIndex = index;
    mHandles.Empty();
    for (udword i = 0; i < nb; i++) mHandles.AddPtr(handles[i]);
...

    {
        ...
        if (nb && API) {
            ...
            for (udword i = 0; i < nb; i++) {
                ...
                if (i == 0) CompoundRot = GlobalPose.mRot;
            }
        }
        SetCompoundPos(CompoundPos);
        SetCompoundRot(CompoundRot);

        if (mMassEditBox) mMassEditBox->SetText(_F("%f", Mass));
    }

    if (mActorsLabel) mActorsLabel->SetLabel(_F("Nb actors to merge: %d", nb));
...
void CompoundCreateWindow::CreateCompound() {
    if (mCallback) mCallback->CreateCompound(*this);
}
...
int CompoundCreateWindow::handleEvent(IceGUIEvent* event) { return 0; }
    ...
    virtual void OnListboxEvent(ListBoxEvent event) {
        if (event == LBE_SELECTION_CHANGED) {
            ...
            if (Selected < mOwner.mHandles.GetNbEntries()) {
                ...
                if (mOwner.mButton0) mOwner.mButton0->SetEnabled(true);
                if (mOwner.mButton1) mOwner.mButton1->SetEnabled(true);
            }
        } else if (event == LBE_DBL_CLK) {
            Select(-1);
            mOwner.mSelected = null;
            if (mOwner.mButton0) mOwner.mButton0->SetEnabled(false);
            if (mOwner.mButton1) mOwner.mButton1->SetEnabled(false);
        }
...
CompoundCreateWindow* CreateCompoundCreateGUI(IceWidget* parent, sdword x, sdword y) {
    ...
    sdword YOffset = 4;
    {
        ...
        CCW->mNameEditBox = GUI_CreateEditBox(CCW, 0, OffsetX + NameLabelWidth, YOffset, EditBoxWidth, EditBoxHeight,
                                              "Default name", null, EDITBOX_TEXT, null, null);
        YOffset += EditBoxHeight;

        CCW->mMassLabel = GUI_CreateLabel(CCW, OffsetX, YOffset + 2, NameLabelWidth, 20, "Mass:", null);
        CCW->mMassEditBox = GUI_CreateEditBox(CCW, 0, OffsetX + NameLabelWidth, YOffset, EditBoxWidth, EditBoxHeight,
                                              "-1.0", null, EDITBOX_FLOAT_POSITIVE, null, null);
        YOffset += EditBoxHeight;
...
void CloseCompoundCreateGUI() { DELETESINGLE(gCompoundCreateWindow); }

void HideCompoundCreateWindow() {
    if (!gCompoundCreateWindow) return;
    gCompoundCreateWindow->SetVisible(false);
    gCompoundCreateWindow->InitFrom(null, INVALID_ID, 0, null,
                                    null);  // Make sure we don't keep dangling pointers in there
}
...
CompoundCreateWindow* GetVisibleCompoundCreateWindow() {
    if (!gCompoundCreateWindow || !gCompoundCreateWindow->IsVisible()) return null;
