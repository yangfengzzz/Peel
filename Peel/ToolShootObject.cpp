...
void ToolShootObject::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
    ...
    helper.CreateEditBox(parent, 0, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gObjectSize), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::gEBCallback, gTooltip_ObjectSize);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 100, 20, "Object mass:", &owner);
    helper.CreateEditBox(parent, 1, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gObjectMass), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::gEBCallback, gTooltip_ObjectMass);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 100, 20, "Object velocity:", &owner);
    helper.CreateEditBox(parent, 2, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gObjectVelocity), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::gEBCallback, gTooltip_ObjectVelocity);
    y += YStep;
}
