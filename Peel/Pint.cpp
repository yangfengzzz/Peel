...
PintCaps::PintCaps()
    :
      ...
      // There is no simple definition that easily maps to existing implementations in different engines so this is
      // experimental.
      ...
      // Merging the two bools would either prevent the PEEL 1.1 articulation scenes from running on old PhysX plugins,
      // or it would make them crash when they try to run the scenes designed for the new articulations/interface.
...
static const char* gShapeTypeName[] = {
        "Undefined", "Sphere", "Capsule", "Cylinder", "Box", "Convex", "Mesh", "Mesh2",
};

const char* GetPintShapeName(PintShape type) { return gShapeTypeName[type]; }

static const char* gJointTypeName[] = {
        "Undefined", "Spherical", "Hinge", "Hinge2",          "Prismatic", "Fixed",
        "Distance",  "D6",        "Gear",  "Rack and pinion", "Chain",     "Portal",
};

const char* GetPintJointName(PintJoint type) { return gJointTypeName[type]; }
