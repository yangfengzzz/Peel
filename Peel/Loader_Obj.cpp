...
struct ParseContext {
    ParseContext(const WavefrontLoaderParams& params, WavefrontDatabase& database)
        : mParams(params), mDatabase(database), mCurrentMesh(null), mIndexBase(0), mCurrentID(0) {}

    void FinishMesh() {
        if (0) FixMeshIndices(*mCurrentMesh);
...
static bool gParseCallback(
    ...
    if (pb[0] == "o") {
        if (Ctx->mParams.mMergeMeshes) {
            if (!Ctx->mCurrentMesh) Ctx->mCurrentMesh = ICE_NEW(WavefrontMesh)(null, Ctx->mCurrentID++);
        } else {
            if (Ctx->mCurrentMesh) Ctx->FinishMesh();
...
WavefrontDatabase::~WavefrontDatabase() { Release(); }
