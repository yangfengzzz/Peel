...
inline_ PintVehicleHandle GetNativeVehicleHandle(const BaseData* data) {
    return PintVehicleHandle(GetNativeHandle(data));
}
inline_ PintCharacterHandle GetNativeCharacterHandle(const BaseData* data) {
    return PintCharacterHandle(GetNativeHandle(data));
}
...
struct MergeParams {
    MergeParams(const char* name = null, float mass = -1.0f, const PR* pose = null /*, bool kinematic=false*/)
        : mName(name),
          mMass(mass),
          mPose(pose)  //, mKinematic(kinematic)
    {}
...
public:
    ...
    //		virtual	bool				FindTouchedTriangles(Container& indices, PintSQThreadContext context, PintShapeHandle
    //handle, const PR& pose, const PintSphereOverlapData& sphere)	const	{ return
    //mPint.NotImplemented("Pint_Shape::FindTouchedTriangles");	} 		virtual	bool
    //Refit(PintShapeHandle shape, PintActorHandle actor)	{ return mPint.NotImplemented("Pint_Shape::Refit");
    //}
...
public:
    ...
    /*		virtual	bool				ResetVehicleData(PintVehicleHandle vehicle);
                    virtual	bool				AddActor(PintVehicleHandle vehicle, PintActorHandle
       actor); virtual	bool				AddShape(PintVehicleHandle vehicle, const PINT_SHAPE_CREATE&
       create);*/
};
...
public:
    ...
    //{ NotImplemented("DeleteConvexObject");	return false;	}
    ...
    //NotImplemented("DeleteMeshObject");	return false;	}
    ...
    //		virtual	bool						SetSQFlag(PintActorHandle actor, PintShapeHandle shape, bool
    //flag);
    ...
    //		virtual	void						SetRCArticulatedMotor(PintActorHandle object, const PINT_ARTICULATED_MOTOR_CREATE&
    //motor);
    ...
    PintActorHandle MergeActors(udword nb,
    ...
    inline_ const MaterialData* GetEditorMaterial(udword i) const {
        return (const MaterialData*)mEditorMaterials.GetEntry(i);
    }
    ...
    inline_ const AggregateData* GetEditorAggregate(udword i) const {
        return (const AggregateData*)mEditorAggregates.GetEntry(i);
    }
...
private:
    ...
    const SphereShapeData* RegisterSphereShape(const PINT_SPHERE_CREATE& create,
