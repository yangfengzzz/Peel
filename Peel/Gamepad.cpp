...
bool InitGamepads() {
    ...
    static const char* xInputLibs[xInputLibCount] = {"xinput1_4.dll", "xinput1_3.dll", "xinput1_2.dll",
                                                     "xinput1_1.dll"};
    for (udword i = 0; i < xInputLibCount; i++) {
        gXInputLibrary = LoadLibraryA(xInputLibs[i]);
        if (gXInputLibrary) break;
    }
...
void ProcessGamepads(GamepadInterface& gpint) {
    ...
    if (!gXInputLibrary || !gXInputGetState || !gXInputGetCapabilities) return;
        ...
        if ((--disConnected[p]) == 0) {
            ...
            if (state == ERROR_DEVICE_NOT_CONNECTED) {
...

                {
                    ...
                    // PT: I think we do the 2 last ones separately because they're listed in GamepadControls but not in
                    // buttonMasks...
                    {
                ...
                const int axisData[] = {inputState.Gamepad.sThumbRX, inputState.Gamepad.sThumbRY,
                                        inputState.Gamepad.sThumbLX, inputState.Gamepad.sThumbLY};
                for (udword i = 0; i < MAX_GAMEPAD_AXES; i++) {
                    if (axisData[i] != gLastAxisData[p][i]) {
                        ...
                        /*						if(Value>1.0f)
                                                                        ...
                                                                        if(Value<-1.0f)
                                                                                Value = -1.0f;
                                                                        if(fabsf(Value)>0.2f)	// ### TODO: expose that
                           threshold*/
                        {
                            // if(data)
#ifdef GAMEPAD_LOG
