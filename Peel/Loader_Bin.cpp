...
static bool LoadBIN(const char* filename,
                    SurfaceManager& test,
                    const Point* scale,
                    bool merge_meshes,
                    bool reset_pivots,
                    udword tessellation,
                    TessellationScheme ts) {
    ...
    struct Load {
        static void Vertices(
                IndexedSurface& surface, udword nb_verts, IceFile& file, AABB& bounds, const Point* scale) {
    ...
    if (!merge_meshes) {
        for (udword i = 0; i < NbMeshes; i++) {
            ...
            if (tessellation) Tessellate(IS, tessellation, ts);
    ...
    printf("-LoadBIN: loaded %d tris and %d verts, for a total of %d Kb.\n", TotalNbTris, TotalNbVerts,
           GrandTotal / 1024);
    printf("-LoadBIN: min bounds: %f | %f | %f\n", GlobalBounds.GetMin(0), GlobalBounds.GetMin(1),
           GlobalBounds.GetMin(2));
    printf("-LoadBIN: max bounds: %f | %f | %f\n", GlobalBounds.GetMax(0), GlobalBounds.GetMax(1),
           GlobalBounds.GetMax(2));
    return true;
}

void LoadMeshesFromFile_(SurfaceManager& test,
                         const char* filename,
                         const Point* scale,
                         bool mergeMeshes,
                         udword tessellation,
...
void LoadBinMeshesFromFile(SurfaceManager& test, const char* filename, const BinLoaderSettings& settings) {
    const char* File = FindPEELFile(filename);
    if (!File || !LoadBIN(File, test, &settings.mScale, settings.mMergeMeshes, settings.mResetPivot,
                          settings.mTessellation, settings.mTS))
        OutputConsoleError(_F("Failed to load '%s'\n", filename));
}
