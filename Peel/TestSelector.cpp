...
static int NextCategory(int start_index) {
    ...
    do {
        // Key down
        CurrentTest++;
        if (CurrentTest == NbTests) CurrentTest = 0;
...
int TestSelectionKeyboardCallback(int key, int current_test) {
    ...
    switch (key) {
        case GLUTX_KEY_DOWN: {
            current_test++;
            if (current_test == NbTests) current_test = 0;
            break;
        }
        case GLUTX_KEY_UP: {
            if (current_test)
...
PhysicsTest* RenderTestSelector(
    ...
    for (int i = 0; i < NbToGo; i++) {
        ...
        if (Test) {
            ...
            if (Test->IsPrivate()) {
                texter.print(x + XOffset, y, text_scale,
                             _F("%d: %s - %s (PRIVATE)", RealIndex, CategoryText[TC], Test->GetName()));
            } else {
                texter.print(x + XOffset, y, text_scale,
                             _F("%d: %s - %s", RealIndex, CategoryText[TC], Test->GetName()));
            }
        }

        if (Selected) CandidateTest = Test;
