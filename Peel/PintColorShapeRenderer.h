...
// TODO: consider splitting this class, the color isn't used in the shader when there's a texture. Or maybe we could
// modulate both. Actually maybe this design is wrong anyway, we just need a material class somewhere
class PintColorShapeRenderer : public PintShapeRenderer {
    friend PintColorShapeRenderer* CreatePintColorShapeRenderer(PintShapeRenderer* renderer,
...
public:
    ...
    struct Data {
        inline_ Data(PintShapeRenderer* source_renderer, const RGBAColor& color, const ManagedTexture* texture)
            : mRenderer(source_renderer), mColor(color), mTexture(texture) {}
...
PintColorShapeRenderer* CreatePintColorShapeRenderer(PintShapeRenderer* renderer,
                                                     const RGBAColor& color,
                                                     const ManagedTexture* texture);
