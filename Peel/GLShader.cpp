...
GLShader::Loader::~Loader() { ICE_FREE(mData); }
...
GLShader::Data GLShader::CompileProgram(const char* vsource, const char* fsource, const char* gsource) {
    ...
    if (vsource) data.mVertexShader = ::CompileShader(GL_VERTEX_SHADER, data.mProgram, 1, &vsource);

    if (fsource) data.mFragmentShader = ::CompileShader(GL_FRAGMENT_SHADER, data.mProgram, 1, &fsource);
...
GLShader::Data GLShader::CompileMultiFilesShader(udword nb_sources, const char** sources) {
    ...
    for (udword i = 0; i < nb_sources; i++) lines[i + 1] = sources[i];
