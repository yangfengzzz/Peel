...
ActiveEdgesRenderer::~ActiveEdgesRenderer() { Release(); }
...
PintDLMeshShapeRenderer::PintDLMeshShapeRenderer(const PintSurfaceInterface& surface, udword flags)
    : mData(surface, flags) {
    CreateDisplayList(surface, flags);
}

PintDLMeshShapeRenderer::PintDLMeshShapeRenderer(const MultiSurface& multi_surface, udword flags)
    : mData(PintSurfaceInterface(multi_surface.GetSurfaceInterface()), flags) {
    CreateDisplayList(multi_surface, flags);
}

PintDLMeshShapeRenderer::~PintDLMeshShapeRenderer() {}

void PintDLMeshShapeRenderer::CreateDisplayList(const SurfaceInterface& surface,
                                                udword flags,
                                                const IndexedSurface* UVSurface) {
    ...
    if (active_edges) mActiveEdges.CreateDL(surface);
            ...
            if (1) {
                ...
                for (udword j = 0; j < 3; j++) {
                    const udword VRef = VRefs[j];
                    {
                        ...
                        glNormal3f(-n.x, -n.y,
                                   -n.z);  // ### note the sign because NORMALSCREATE doesn't do what we need
                    }
    ...
    for (udword i = 0; i < Create.NbFaces; i++) {
        ...
        if (0) {
            if (VRefs[0] >= surface.mNbVerts) printf("Invalid vertex index!\n");
            if (VRefs[1] >= surface.mNbVerts) printf("Invalid vertex index!\n");
            if (VRefs[2] >= surface.mNbVerts) printf("Invalid vertex index!\n");
        }
    }
...
void PintDLMeshShapeRenderer::CreateDisplayList(const MultiSurface& multi_surface, udword flags) {
    ...
    if (uses_active_edges) mActiveEdges.CreateDL(surface);
    ...
    if (!uses_vertex_normals) {
        ...
        for (udword i = 0; i < Create.NbFaces; i++) {
            ...
            create.mWMatID[i]	: INVALID_ID;
                            ...
                            if(!Builder.AddFace(FD))	return SetIceError("ConsolidatedSurface::Consolidate:
            MeshBuilder AddFace failed!", null);*/
        }
...
PintDLMeshShapeRenderer2::PintDLMeshShapeRenderer2(const PintSurfaceInterface& surface, udword flags)
    : PintDLMeshShapeRenderer(surface, flags) {}

PintDLMeshShapeRenderer2::PintDLMeshShapeRenderer2(const MultiSurface& multi_surface, udword flags)
    : PintDLMeshShapeRenderer(multi_surface, flags) {}
