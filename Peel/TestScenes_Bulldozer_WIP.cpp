...
static PintJointHandle CreateHinge(Pint& pint,
                                   PintActorHandle obj0,
                                   PintActorHandle obj1,
                                   const Point& local_pivot0,
                                   const Point& local_pivot1,
                                   const Point& local_axis0,
                                   const Point& local_axis1,
                                   float min_limit = MIN_FLOAT,
                                   float max_limit = MAX_FLOAT) {
...
static PintActorHandle CreateGear(Pint& pint,
                                  const PINT_MATERIAL_CREATE* material,
                                  const Point& pos,
                                  float gear_mass,
                                  const CylinderMesh& inside_cylinder,
                                  PintShapeRenderer* inside_cylinder_renderer,
                                  const CylinderMesh& outside_cylinder,
                                  PintShapeRenderer* outside_cylinder_renderer) {
...
static PINT_SHAPE_CREATE* CreateGearHolderShapes(PINT_SHAPE_CREATE*& StartShape,
                                                 PINT_SHAPE_CREATE*& CurrentShape,
                                                 const Point& pos,
                                                 const Point& gear_center,
                                                 udword nb_gears,
                                                 const Point* gear_offsets,
                                                 const CylinderMesh& inside_cylinder,
                                                 const PINT_MATERIAL_CREATE* material,
                                                 const Point& local_offset) {
    for (udword i = 0; i < nb_gears; i++) {
        const Point GearPos = pos + gear_offsets[i];
        {
            ...
            if (!StartShape) StartShape = BoxDesc;

            if (CurrentShape) CurrentShape->SetNext(BoxDesc);
...
static PintActorHandle CreateCaterpillarTrackWIP(Pint& pint,
                                                 ...
                                                 const Point* gear_offsets,
                                                 float gear_mass,
                                                 PintActorHandle gear_holder = null,
                                                 const Point* gear_holder_offset = null) {
    // We tweak the incoming cylinders so that:
    // - they have a large amount of vertices. That way we get a smoother curve.
    // - they take the height of the track into account. That way the system doesn't start jittering when we increase
    // the number of solver iterations.
    const float TrackHeight = 0.1f;

    CylinderMesh TweakedOutsideCylinder;
    TweakedOutsideCylinder.Generate(256, outside_cylinder.mRadius + TrackHeight, outside_cylinder.mHalfHeight,
                                    outside_cylinder.mOrientation);
    ...
    if (!GearHolder) {
        ...
        StartShape = CreateGearHolderShapes(StartShape, CurrentShape, pos, GearCenter, nb_gears, gear_offsets,
                                            inside_cylinder, material, Point(0.0f, 0.0f, 0.0f));
    ...
    for (udword i = 0; i < nb_gears; i++) {
        const Point GearPos = pos + gear_offsets[i];
        PintActorHandle CurrentGear = CreateGear(pint, material, GearPos, gear_mass, inside_cylinder,
                                                 inside_cylinder_renderer, outside_cylinder, outside_cylinder_renderer);
        objects.mGears[i] = CurrentGear;

        PintJointHandle h =
                CreateHinge(pint, CurrentGear, GearHolder, Point(0.0f, 0.0f, 0.0f),
                            gear_holder_offset ? (*gear_holder_offset) + GearPos - GearCenter : GearPos - GearCenter,
...
static const char* gDesc_BulldozerTest =
        "Bulldozer test. This uses regular joints (no articulation). This is an experimental, work-in-progress test. It has not been properly tuned or \
...
static PintActorHandle CreateBoxObject(
        Pint& pint, const Point& extents, const Point& pos, float mass, const Quat* rotation = null) {
    PINT_BOX_CREATE BoxDesc(extents);
    ...
    if (rotation) ObjectDesc.mRotation = *rotation;
    return CreatePintObject(pint, ObjectDesc);
}
...
static void CreatePrismatic(Pint& pint,
    ...
    PintActorHandle BottomPrismaticObject =
            CreateBoxObject(pint, BottomPrismaticExtents, BottomPrismaticPos, PrismaticMass, &Q);
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    const Point LeftSidePos(SideExtents.x - ChassisExtents.x, SideExtents.y + SideDistToGround,
                            -ChassisExtents.z - ChassisGap - SideExtents.z);
    const Point RightSidePos(SideExtents.x - ChassisExtents.x, SideExtents.y + SideDistToGround,
                             ChassisExtents.z + ChassisGap + SideExtents.z);
    ...
    if (1) {
        ...
        if (0) {
            ...
            for (udword i = 0; i < 2; i++) {
                ...
                CreateCaterpillarTrackWIP(pint, CTO, &mHighFrictionMaterial, Pos, mInsideCylinder,
                                          GetRegisteredRenderers()[0], mOutsideCylinder, GetRegisteredRenderers()[1],
            ...
            for (udword i = 0; i < 2; i++) {
...

                {
                    ...
                    StartShape = CreateGearHolderShapes(StartShape, CurrentShape, Pos, GearCenter, NbGears, GearOffsets,
                                                        mInsideCylinder, &mHighFrictionMaterial, GearCenter);
                }
            }
            ...
            if (1) {
                ...
                for (udword i = 0; i < 2; i++) {
                    ...
                    CaterpillarTrackObjects CTO;
                    CreateCaterpillarTrackWIP(pint, CTO, &mHighFrictionMaterial, Pos, mInsideCylinder,
                                              GetRegisteredRenderers()[0], mOutsideCylinder,
    ...
    CreateHinge(pint, ChassisObject, LeftSideObject,
                Point(-ChassisExtents.x, -ChassisExtents.y + SideExtents.y, -ChassisExtents.z - ChassisGap),
                Point(-SideExtents.x, 0.0f, SideExtents.z), HingeAxis, HingeAxis);
    CreateHinge(pint, ChassisObject, RightSideObject,
                Point(-ChassisExtents.x, -ChassisExtents.y + SideExtents.y, ChassisExtents.z + ChassisGap),
                Point(-SideExtents.x, 0.0f, -SideExtents.z), HingeAxis, HingeAxis);
    ...
    if (1) {
        ...
        const Point BladePos(LeftSidePos.x + SideExtents.x + BladeExtents.x, SideDistToGround + BladeExtents.y, 0.0f);
        ...
        CreateHinge(pint, BladeObject, LeftSideObject, Point(-BladeExtents.x, -BladeExtents.y + SideExtents.y, -dz),
                    Point(SideExtents.x, 0.0f, 0.0f), HingeAxis, HingeAxis);
        CreateHinge(pint, BladeObject, RightSideObject, Point(-BladeExtents.x, -BladeExtents.y + SideExtents.y, dz),
                    Point(SideExtents.x, 0.0f, 0.0f), HingeAxis, HingeAxis);
