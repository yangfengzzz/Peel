...
bool GLFontRenderer::init() {
    ...
    if (!m_textureObject) return false;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, OGL_FONT_TEXTURE_WIDTH, OGL_FONT_TEXTURE_HEIGHT, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, P);
...
void GLFontRenderer::print(float x,
                           float y,
                           float fontSize,
                           const char* pString,
                           bool forceMonoSpace,
...
void GLFontRenderer::print_(float x,
                            float y,
                            float fontSize,
                            const char* pString,
                            bool forceMonoSpace,
    ...
    if (m_isInit && num > 0) {
        ...
        for (unsigned int i = 0; i < num; i++) {
            ...
            if (c < OGL_FONT_CHARS_PER_ROW * OGL_FONT_CHARS_PER_COL) {
                ...
                if (forceMonoSpace) glyphWidth = (float)monoSpaceWidth;

                glyphWidth =
                        glyphWidth * (fontSize / (((float)OGL_FONT_TEXTURE_WIDTH) / OGL_FONT_CHARS_PER_ROW)) - 0.01f;
