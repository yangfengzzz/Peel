...
static bool SetupSphereOverlaps(
        TestBase& test, udword nb_x, udword nb_y, float scale_x, float scale_y, float altitude, float sphere_radius) {
...
static bool SetupBoxOverlaps(
        TestBase& test, udword nb_x, udword nb_y, float scale_x, float scale_y, float altitude, float box_radius) {
    BasicRandom Rnd(42);
...
static bool SetupCapsuleOverlaps(TestBase& test,
                                 udword nb_x,
                                 udword nb_y,
                                 float scale_x,
                                 float scale_y,
...
static const char* gDesc_OverlapShapesVsShapes =
        "Overlaps N*N query shapes vs M*M static shapes. Overlap mode is either 'any' or 'all'. \
...
public:
    OverlapShapesVsShapes() : mQueryShapeType(PINT_SHAPE_UNDEFINED) {}
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 0;
        {
            ...
            {
                IceComboBox* CB =
                        CreateComboBox<IceComboBox>(UI, 0, 4 + OffsetX, y, 170, 20, "Overlap query type", null, null);
            ...
            mEditBox_QueryShapeGridSize = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "32", &UIElems,
            ...
            mEditBox_QueryShapeSize = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
            ...
            mComboBox_QueryShapeType =
                    CreateShapeTypeComboBox(UI, 4 + OffsetX, y, true, SSM_UNDEFINED | SSM_SPHERE | SSM_BOX);
            ...
            mEditBox_StaticShapeGridSize = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "64", &UIElems,
                                                                EDITBOX_INTEGER_POSITIVE, null, null);
            mEditBox_StaticShapeGridSize->SetEnabled(true);
            y += YStep;

            /*			helper.CreateLabel(UI, 4, y+LabelOffsetY, LabelWidth, 20, "Static shape size:",
               &UIElems); mEditBox_StaticShapeSize = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "1.0",
               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); mEditBox_StaticShapeSize->SetEnabled(true); y += YStep;*/

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Static shape type:", &UIElems);
            mComboBox_StaticShapeType = CreateShapeTypeComboBox(
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (mQueryShapeType == PINT_SHAPE_SPHERE && !caps.mSupportSphereOverlaps) return false;
        if (mQueryShapeType == PINT_SHAPE_BOX && !caps.mSupportBoxOverlaps) return false;
        if (mQueryShapeType == PINT_SHAPE_CAPSULE && !caps.mSupportCapsuleOverlaps) return false;
        if (mQueryShapeType == PINT_SHAPE_CONVEX && !caps.mSupportConvexOverlaps) return false;

        // We only support these ones for now
        if (mQueryShapeType != PINT_SHAPE_UNDEFINED && mQueryShapeType != PINT_SHAPE_SPHERE &&
            mQueryShapeType != PINT_SHAPE_BOX)
            return false;

        PintShape StaticShapeType = PINT_SHAPE_UNDEFINED;
        if (mComboBox_StaticShapeType) StaticShapeType = PintShape(mComboBox_StaticShapeType->GetSelectedIndex());
        if (StaticShapeType == PINT_SHAPE_UNDEFINED) return true;
...
static const char* gDesc_OverlapObjects_DynamicSphere =
        "OVERLAP_OBJECTS: single dynamic sphere overlap against 128*128 static boxes. \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportSphereOverlaps) return false;
...
virtual udword Update(Pint& pint, float dt) { return DoBatchSphereOverlaps(*this, pint, OVERLAP_OBJECTS); }

END_TEST(OverlapObjects_DynamicSphere)

///////////////////////////////////////////////////////////////////////////////

static const char* gDesc_OverlapObjects_DynamicSpheres =
        "OVERLAP_OBJECTS: 32*32 (dynamic) spheres against 128*128 static boxes. \
...
static const char* gDesc_OverlapAny_SphereVsTessBunny =
        "OVERLAP_ANY: 1 sphere against the tessellated bunny. This is really just to investigate an issue in PhysX.";
...
virtual udword Update(Pint& pint, float dt) { return DoBatchSphereOverlaps(*this, pint, OVERLAP_ANY); }

END_TEST(OverlapAny_SphereVsTessBunny)

///////////////////////////////////////////////////////////////////////////////

static const char* gDesc_OverlapObjects_SphereVsTessBunny =
        "OVERLAP_OBJECTS: 1 sphere against the tessellated bunny. This is really just to investigate an issue in "
        "Havok.";
...
static const char* gDesc_OverlapAny_SpheresVsTessBunny =
        "OVERLAP_ANY: 16*16*16 spheres against the tessellated bunny. \
...
virtual bool CommonSetup() {
    TestBase::CommonSetup();
    LoadMeshesFromFile_(*this, "bunny.bin", null, false, 3);
    for (udword i = 0; i < 16; i++) SetupSphereOverlaps(*this, 16, 16, 10.0f, 10.0f, float(i), 0.5f);
    return true;
}
...
static const char* gDesc_OverlapAny_BoxesVsTessBunny =
        "OVERLAP_ANY: 16*16*16 boxes against the tessellated bunny. \
...
virtual bool CommonSetup() {
    TestBase::CommonSetup();
    LoadMeshesFromFile_(*this, "bunny.bin", null, false, 3);
    for (udword i = 0; i < 16; i++) SetupBoxOverlaps(*this, 16, 16, 10.0f, 10.0f, float(i), 0.5f);
    return true;
}

virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportBoxOverlaps) return false;

    return CreateMeshesFromRegisteredSurfaces(pint, caps);
}

virtual udword Update(Pint& pint, float dt) { return DoBatchBoxOverlaps(*this, pint, OVERLAP_ANY); }

END_TEST(OverlapAny_BoxesVsTessBunny)

///////////////////////////////////////////////////////////////////////////////

static const char* gDesc_OverlapAny_CapsulesVsTessBunny =
        "OVERLAP_ANY: 16*16*16 capsules against the tessellated bunny. \
...
virtual bool CommonSetup() {
    TestBase::CommonSetup();
    LoadMeshesFromFile_(*this, "bunny.bin", null, false, 3);
    for (udword i = 0; i < 16; i++) SetupCapsuleOverlaps(*this, 16, 16, 10.0f, 10.0f, float(i), 0.5f, 10.0f);
    return true;
}

virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportCapsuleOverlaps) return false;

    return CreateMeshesFromRegisteredSurfaces(pint, caps);
}

virtual udword Update(Pint& pint, float dt) { return DoBatchCapsuleOverlaps(*this, pint, OVERLAP_ANY); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportSphereOverlaps || !caps.mSupportMeshes) return false;
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportBoxOverlaps || !caps.mSupportMeshes) return false;
...
static const char* gDesc_OverlapObjects_GiantSphereVsKP =
        "OVERLAP_OBJECTS: one single giant sphere against the KP level. Goal is to retrieve all meshes touched by the "
        "sphere. It's one of these rare tests where Opcode 1.3 still beats everybody, thanks to its 'containment "
...
static const char* gDesc_OverlapObjects_GiantBoxVsKP =
        "OVERLAP_OBJECTS: one single giant box against the KP level. Goal is to retrieve all meshes touched by the "
        "box.";
...
virtual udword Update(Pint& pint, float dt) { return DoBatchBoxOverlaps(*this, pint, OVERLAP_OBJECTS); }
...
public:
    SceneOverlapAnyVsStaticHeightfield(udword scenario)
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (mScenario == 0) {
            ...
            if (!caps.mSupportConvexOverlaps) return false;
...
public:
    ...
    public:
        ...
        public:
            ...
            static const char* gDesc_OverlapAny_ConvexesVsHeightfield =
                    "OVERLAP_ANY: 4096 convexes against a heightfield";
