...
static void gBulletHitCallback(const PintRaycastHit& hit,
...
typedef void (*BulletHitCallback)(const PintRaycastHit& hit,
                                  const Point& start_pos,
                                  const Ray& world_ray,
                                  void* user_data);
...
static void DeleteBullet(InternalBullet* B, bool lost_bullet = false) {
    if (B->mDeleteCallback) (B->mDeleteCallback)(B->mData, B->mUserData, lost_bullet);
...
bool RegisterBullet(const Bullet& bullet,
                    BulletDeleteCallback delete_callback,
                    /*BulletFilterCallback filter_callback,*/ BulletHitCallback hit_callback,
                    BulletRenderCallback render_callback,
                    void* user_data,
                    float render_length) {
    if (!gBullets) gBullets = ICE_NEW(Container);
...
void EvolveBullets(/*RMScene* scene, const TimeInfo& time*/ float elapsed_time) {
    if (gRenderInfo) gRenderInfo->Reset();
        ...
        if (B->mHasTouched || SceneBounds.ContainsPoint(B->mData.mRay.mOrig)) {
            ...
            if (!B->mHasTouched) {
                ...
                if (_Raycast(*pint, Hit, B->mData.mRay.mOrig, B->mData.mRay.mDir, MaxDist)) {
                    ...
                    B->mHasTouched = true;
                    if (B->mHitCallback) (B->mHitCallback)(Hit, B->mData.mStartPos, B->mData.mRay, B->mUserData);
                }
            }
            ...
            if (B->mHasTouched) {
                if (B->mRenderLength == 0.0f) {
                    ...
                    if (d1 > d0) MustDeleteBullet = true;
                }
            }
            ...
            if (B->mRenderCallback && !MustDeleteBullet) {
                if (!gRenderInfo) gRenderInfo = ICE_NEW(Container);
                // Save render info
                ibr = reinterpret_cast<InternalBulletRender*>(
                        gRenderInfo->Reserve(sizeof(InternalBulletRender) / sizeof(udword)));
                ...
                //				if(!B->mFlybyTest)
                //					B->mFlybyTest = FlybyTest(B->mData.mSkeleton, ibr->mOldPos,
                //ibr->mCurPos, B->mData.mRay.mDir);
            }
...
void ToolBullet::RightDownCallback(Pint& pint, udword pint_index) { mIsFiring = true; }

void ToolBullet::RightUpCallback(Pint& pint, udword pint_index) { mIsFiring = false; }

void ToolBullet::PreRenderCallback() { EvolveBullets(1.0f / 60.0f); }
...
void ToolBullet::RenderCallback(PintRender& render, Pint& pint, udword pint_index) {
    ...
    while (B != Last) {
        ...
        if (B->mRenderCallback) (B->mRenderCallback)(B->mOldPos, B->mCurPos, B->mUserData);
        B++;
    }
}
