...
static inline_ void DrawLine(const Point& p0, const Point& p1, Vertices& buffer) { buffer.AddVertex(p0).AddVertex(p1); }
...
static void DrawWireframeBox(const OBB& box, const Point& offset, Vertices& buffer) {
    ...
    for (udword j = 0; j < 8; j++) Pts[j] += offset;
...
void PintSQ::DrawRaycastAnyData(udword nb,
                                const PintRaycastData* data,
                                const PintBooleanHit* hits,
                                const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawRaycastAllData(udword nb,
                                const PintRaycastData* data,
                                const PintMultipleHits* hits,
                                const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawSphereSweepData(udword nb,
                                 const PintSphereSweepData* data,
                                 const PintRaycastHit* hits,
                                 const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawCapsuleSweepData(udword nb,
                                  const PintCapsuleSweepData* data,
                                  const PintRaycastHit* hits,
                                  const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawConvexSweepData(udword nb,
                                 const PintConvexSweepData* data,
                                 const PintRaycastHit* hits,
                                 const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawSphereOverlapAnyData(udword nb,
                                      const PintSphereOverlapData* data,
                                      const PintBooleanHit* hits,
                                      const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawSphereOverlapObjectsData(udword nb,
                                          const PintSphereOverlapData* data,
                                          const PintMultipleHits* hits,
                                          const Point& color) {
    if (!nb || !data || !hits) return;
    ...
    for (udword i = 0; i < nb; i++) {
        ...
        DrawWireframeSphere(data[i].mSphere, Point(0.0f, 0.0f, 0.0f),
                            hits[i].mNbHits ? mBatchedLines_R : mBatchedLines_G);
    }
}

void PintSQ::DrawBoxOverlapAnyData(udword nb,
                                   const PintBoxOverlapData* data,
                                   const PintBooleanHit* hits,
                                   const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawBoxOverlapObjectsData(udword nb,
                                       const PintBoxOverlapData* data,
                                       const PintMultipleHits* hits,
                                       const Point& color) {
    if (!nb || !data || !hits) return;
...
void PintSQ::DrawCapsuleOverlapAnyData(udword nb,
                                       const PintCapsuleOverlapData* data,
                                       const PintBooleanHit* hits,
                                       const Point& color) {
    if (!nb || !data || !hits) return;
    ...
    for (udword i = 0; i < nb; i++) {
        ...
        DrawWireframeCapsule(Capsule.mRadius, M, Pose, Point(0.0f, 0.0f, 0.0f),
                             hits[i].mHit ? mBatchedLines_R : mBatchedLines_G);
    }
}

void PintSQ::DrawCapsuleOverlapObjectsData(udword nb,
                                           const PintCapsuleOverlapData* data,
                                           const PintMultipleHits* hits,
                                           const Point& color) {}

void PintSQ::DrawConvexOverlapAnyData(udword nb,
                                      const PintConvexOverlapData* data,
                                      const PintBooleanHit* hits,
                                      const Point& color) {
    if (!nb || !data || !hits) return;
    ...
    /*	for(udword i=0;i<nb;i++)
            {
                    ...
                    DrawWireframeCapsule(Capsule.mRadius, M, Pose, Point(0.0f, 0.0f, 0.0f), hits[i].mHit ?
       mBatchedLines_R : mBatchedLines_G);
            }*/
...
void PintSQ::DrawConvexOverlapObjectsData(udword nb,
                                          const PintConvexOverlapData* data,
                                          const PintMultipleHits* hits,
                                          const Point& color) {}

void PintSQ::Render(PintRender& renderer, PintRenderPass render_pass, bool paused) {
    if (!(mOwner->GetFlags() & PINT_IS_ACTIVE)) return;

    if (render_pass != PINT_RENDER_PASS_MAIN) return;

    const Point Color = mOwner->GetMainColor();

    // New design: we don't render lines immediately, we batch them for later rendering after the current shader has
    // been deactivated.
    ...
    DrawCapsuleOverlapObjectsData(mCapsuleOverlapObjects.mNbHits, mCapsuleOverlapData, mCapsuleOverlapObjects.mHits,
                                  Color);
