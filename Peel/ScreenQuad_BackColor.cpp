...
ScreenQuadBackColor::ScreenQuadBackColor(const Point& back_color) : mBackColor(back_color) {}
...
void ScreenQuadBackColor::CreateUI(PintGUIHelper& helper, Widgets* widgets, IceWindow* parent) {
    ...
    if (0) {
        ...
        IceButton* B = helper.CreateButton(parent, COLOR_PICKER_BACK_COLOR, x + 80, y2, 40, 20, " ", widgets,
                                           gButtonCallback, "Select: back color" /*tab_control*/);
        const ScreenQuadBackColor* SQBC =
                static_cast<const ScreenQuadBackColor*>(gScreenQuadModes[SCREEN_QUAD_BACK_COLOR]);
        B->EnableBitmap();
        B->SetBitmapColor(ubyte(SQBC->mBackColor.x * 255.0f), ubyte(SQBC->mBackColor.y * 255.0f),
                          ubyte(SQBC->mBackColor.z * 255.0f));
    }
#endif
}
