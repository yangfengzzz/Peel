...
public:
    virtual void* zoneStart(const char* eventName, bool detached, uint64_t contextId) override {
        if (!detached) return Spy::StartScopedZone(eventName);
...
#pragma comment( \
        lib, "../../../../PEEL_Externals/5_1_0/bin/win.x86_64.vc141.mt/profile/PhysXCharacterKinematic_static_64.lib")
// #ifdef USE_NEW_PVD
...
#pragma comment( \
        lib, "../../../../PEEL_Externals/5_1_0/bin/win.x86_64.vc141.mt/debug/PhysXCharacterKinematic_static_64.lib")
...
#pragma comment( \
        lib, "../../../../PEEL_Externals/5_1_0/bin/win.x86_64.vc141.mt/release/PhysXCharacterKinematic_static_64.lib")
#ifdef USE_NEW_PVD
...
const char* PhysX::GetUIName() const { return "PhysX 5.1.0"; }
...
static PxFilterFlags ContactModifySimulationFilterShader(PxFilterObjectAttributes attributes0,
    ...
    //	pairFlags =
    //PxPairFlag::eCONTACT_DEFAULT|PxPairFlag::eNOTIFY_CONTACT_POINTS|PxPairFlag::eNOTIFY_TOUCH_FOUND|PxPairFlag::eNOTIFY_TOUCH_PERSISTS;
...
// typedef physx::PxPhysics*	(*PxFunction_CreateBasePhysics)	(physx::PxU32 version, physx::PxFoundation& foundation,
// const physx::PxTolerancesScale& scale, bool trackOutstandingAllocations, physx::PxProfileZoneManager*
// profileZoneManager); typedef void				(*PxFunction_RegisterPCM)
// (physx::PxPhysics& physics); typedef physx::PxCooking*	(*PxFunction_CreateCooking)
// (physx::PxU32 version, physx::PxFoundation& foundation, const physx::PxCookingParams& params); typedef
// physx::PxFoundation*(*PxFunction_CreateFoundation)	(physx::PxU32 version, physx::PxAllocatorCallback& allocator,
// physx::PxErrorCallback& errorCallback); PX_C_EXPORT bool PX_CALL_CONV PxInitExtensions(physx::PxPhysics& physics);
#endif
...
/*	udword FPUEnv[256];
        ...
        if(!handle0 || !handle1 || !handle2)
                return;
        PxFunction_CreateBasePhysics func0	= (PxFunction_CreateBasePhysics)	GetProcAddress(handle0,
   "PxCreateBasePhysics"); PxFunction_RegisterPCM func1		= (PxFunction_RegisterPCM)
   GetProcAddress(handle0, "PxRegisterPCM"); PxFunction_CreateCooking func2		= (PxFunction_CreateCooking)
   GetProcAddress(handle0, "PxCreateCooking"); PxFunction_CreateFoundation func3	= (PxFunction_CreateFoundation)
   GetProcAddress(handle0, "PxCreateFoundation");
...
//	mFoundation = (func3)(PX_PHYSICS_VERSION, *gDefaultAllocator, *gDefaultErrorCallback);
    ...
    if (!PVDStatus) printf("PVD connection failed\n");
#endif
    ...
    ASSERT(status);
    //	gDefaultCPUDispatcher = PxDefaultCpuDispatcherCreate(mParams.ThreadIndexToNbThreads(mParams.mNbThreadsIndex),
    //null);
    gDefaultCPUDispatcher = PxDefaultCpuDispatcherCreate(mParams.ThreadIndexToNbThreads(mParams.mNbThreadsIndex), null,
                                                         PxDefaultCpuDispatcherWaitForWorkMode::eYIELD_THREAD);
    //	gDefaultCPUDispatcher = PxDefaultCpuDispatcherCreate(16, null);

    //	gDispatcher = PxDefaultCpuDispatcherCreate(inNumThreads, NULL,
    //PxDefaultCpuDispatcherWaitForWorkMode::eYIELD_THREAD); 	gDispatcher = PxDefaultCpuDispatcherCreate(inNumThreads,
    //NULL, PxDefaultCpuDispatcherWaitForWorkMode::eYIELD_PROCESSOR, 8);
...

    {
        ...
        SetupSceneDesc(sceneDesc, desc, mParams, gDefaultCPUDispatcher, &gSimulationEventCallback,
                       &gNewContactModifyCallback);
        ...
        if (mParams.mUseGPU) {
            ...
            /*			sceneDesc.gpuDynamicsConfig.patchStreamCapacity *= 2; //KS - must increase because we can exceed the
            default 4MB buffer with the arena demo! sceneDesc.gpuDynamicsConfig.contactStreamCapacity *= 2; //KS - must
            increase because we can exceed the default 4MB buffer with the arena demo!
            ...
            gCudaContextManager =
                    PxCreateCudaContextManager(*mFoundation, cudaContextManagerDesc, PxGetProfilerCallback());
            ...
            //				sceneDesc.gpuDispatcher = gCudaContextManager->getGpuDispatcher();	//Set the GPU dispatcher, used
            //by GRB to dispatch CUDA kernels.
    ...
    if (gCudaContextManager) {
        ...
        PxPBDParticleSystem* particleSystem =
                initParticles(*mPhysics, *mScene, mParams, 50, 120 * (useLargeFluid ? 5 : 1), 30, FluidPos, 0.1f,
                              maxDiffuseParticles);
...
void PhysX::TestNewFeature() {
...
#ifdef REMOVED
    {
        ...
        PxHeightField* pHeightField =
                mCooking->createHeightField(heightFieldDesc, mPhysics->getPhysicsInsertionCallback());
    ...
    for (udword i = 0; i < NbRows; i++) {
        ...
        for (udword i = 1; i < NbSpheres - 1; i++) {
            ...
            //			Handles[i] = CreateDynamicObject(pint, &SphereDesc, Pos);
            {
                ...
                Handles[i] =
                        CreateArticulationLink(articulation, (PxArticulationLink*)Handles[i - 1], *this, ObjectDesc);
...
void PhysX::UpdateFromUI() { SharedUpdateFromUI(); }
...
udword PhysX_CQS::BatchRaycasts(PintSQThreadContext context,
                                udword nb,
                                PintRaycastHit* dest,
    ...
    if (gSceneQuerySystem) {
        ...
        udword NbHits = 0;
        while (nb--) {
            if (PEEL_RaycastClosest((const PxVec3&)(raycasts->mOrigin), (const PxVec3&)(raycasts->mDir),
                                    raycasts->mMaxDist, hit, sqFlags, fd1, FCB)) {
                NbHits++;
...
udword PhysX_CQS::BatchRaycastAny(PintSQThreadContext context,
                                  udword nb,
                                  PintBooleanHit* dest,
                                  const PintRaycastData* raycasts) {
    if (gSceneQuerySystem) {
        ...
        while (nb--) {
            const bool b =
                    PEEL_RaycastAny(ToPxVec3(raycasts->mOrigin), ToPxVec3(raycasts->mDir), raycasts->mMaxDist, PF, FCB);
...
static udword _BatchShapeOverlapAny(udword nb,
                                    PintBooleanHit* restrict_ dest,
                                    const PintOverlapDataT* restrict_ overlaps,
                                    const PxQueryFilterData& filterData,
                                    PxQueryFilterCallback* filterCB) {
    udword NbHits = 0;
    while (nb--) {
        PxTransform Pose;
...
udword PhysX_CQS::BatchSphereOverlapAny(PintSQThreadContext context,
                                        udword nb,
                                        PintBooleanHit* dest,
                                        const PintSphereOverlapData* overlaps) {
    if (gSceneQuerySystem) {
        struct IceToPx {
            static inline_ PxSphereGeometry ToPxGeometry(PxTransform& pose, const PintSphereOverlapData* overlaps) {
                return ToPxSphereGeometry(pose, overlaps->mSphere);
            }
        };
        return _BatchShapeOverlapAny<PintSphereOverlapData, PxSphereGeometry, IceToPx>(
                nb, dest, overlaps, GetSQFilterData(), GetSQFilterCallback());
    }

    return PhysX::BatchSphereOverlapAny(context, nb, dest, overlaps);
}

udword PhysX_CQS::BatchBoxOverlapAny(PintSQThreadContext context,
                                     udword nb,
                                     PintBooleanHit* dest,
                                     const PintBoxOverlapData* overlaps) {
    if (gSceneQuerySystem) {
        struct IceToPx {
            static inline_ PxBoxGeometry ToPxGeometry(PxTransform& pose, const PintBoxOverlapData* overlaps) {
                return ToPxBoxGeometry(pose, overlaps->mBox);
            }
        };
        return _BatchShapeOverlapAny<PintBoxOverlapData, PxBoxGeometry, IceToPx>(nb, dest, overlaps, GetSQFilterData(),
                                                                                 GetSQFilterCallback());
    }
...
udword PhysX_CQS::BatchCapsuleOverlapAny(PintSQThreadContext context,
                                         udword nb,
                                         PintBooleanHit* dest,
                                         const PintCapsuleOverlapData* overlaps) {
    if (gSceneQuerySystem) {
        struct IceToPx {
            static inline_ PxCapsuleGeometry ToPxGeometry(PxTransform& pose, const PintCapsuleOverlapData* overlaps) {
                return ToPxCapsuleGeometry(pose, overlaps->mCapsule);
            }
        };
        return _BatchShapeOverlapAny<PintCapsuleOverlapData, PxCapsuleGeometry, IceToPx>(
                nb, dest, overlaps, GetSQFilterData(), GetSQFilterCallback());
    }
...
template <class PintOverlapDataT, class PxGeometryT, class AdapterT>
static udword _BatchShapeOverlapObjects(udword nb,
                                        PintMultipleHits* restrict_ dest,
                                        Container& stream,
                                        const PintOverlapDataT* restrict_ overlaps,
                                        const PxQueryFilterData& filterData,
                                        PxQueryFilterCallback* filterCB) {
...
udword PhysX_CQS::BatchSphereOverlapObjects(PintSQThreadContext context,
                                            udword nb,
    ...
    if (gSceneQuerySystem) {
        ...
        return _BatchShapeOverlapObjects<PintSphereOverlapData, PxSphereGeometry, IceToPx>(
                nb, dest, stream, overlaps, GetSQFilterData(), GetSQFilterCallback());
    }

    return PhysX::BatchSphereOverlapObjects(context, nb, dest, stream, overlaps);
}

udword PhysX_CQS::BatchBoxOverlapObjects(PintSQThreadContext context,
                                         udword nb,
                                         PintMultipleHits* dest,
                                         Container& stream,
                                         const PintBoxOverlapData* overlaps) {
    if (gSceneQuerySystem) {
        ...
        return _BatchShapeOverlapObjects<PintBoxOverlapData, PxBoxGeometry, IceToPx>(
                nb, dest, stream, overlaps, GetSQFilterData(), GetSQFilterCallback());
    }
    return PhysX::BatchBoxOverlapObjects(context, nb, dest, stream, overlaps);
}

udword PhysX_CQS::BatchCapsuleOverlapObjects(PintSQThreadContext context,
                                             udword nb,
                                             PintMultipleHits* dest,
                                             Container& stream,
                                             const PintCapsuleOverlapData* overlaps) {
    if (gSceneQuerySystem) {
        ...
        return _BatchShapeOverlapObjects<PintCapsuleOverlapData, PxCapsuleGeometry, IceToPx>(
                nb, dest, stream, overlaps, GetSQFilterData(), GetSQFilterCallback());
    }
...
static udword _BatchSweeps(udword nb,
                           PintRaycastHit* restrict_ dest,
                           const PintSweepDataT* restrict_ sweeps,
                           PxSceneQueryFlags sweepQueryFlags,
                           const PxQueryFilterData& filterData,
                           PxQueryFilterCallback* filterCB) {
    udword NbHits = 0;
    while (nb--) {
        ...
        PxSweepHit Hit;
        if (PEEL_SweepClosest(Geom, Pose, ToPxVec3(sweeps->mDir), sweeps->mMaxDist, Hit, sweepQueryFlags, filterData,
                              filterCB)) {
            NbHits++;
            FillResultStruct(*dest, Hit);
        } else {
...
udword PhysX_CQS::BatchBoxSweeps(PintSQThreadContext context,
                                 udword nb,
                                 PintRaycastHit* dest,
                                 const PintBoxSweepData* sweeps) {
    if (gSceneQuerySystem) {
        struct IceToPx {
            static inline_ PxBoxGeometry ToPxGeometry(PxTransform& pose, const PintBoxSweepData* sweeps) {
                return ToPxBoxGeometry(pose, sweeps->mBox);
            }
        };
        return _BatchSweeps<PintBoxSweepData, PxBoxGeometry, IceToPx>(nb, dest, sweeps, GetSweepQueryFlags(mParams),
                                                                      GetSQFilterData(), GetSQFilterCallback());
    }

    return PhysX::BatchBoxSweeps(context, nb, dest, sweeps);
}

udword PhysX_CQS::BatchSphereSweeps(PintSQThreadContext context,
                                    udword nb,
                                    PintRaycastHit* dest,
                                    const PintSphereSweepData* sweeps) {
    if (gSceneQuerySystem) {
        struct IceToPx {
            static inline_ PxSphereGeometry ToPxGeometry(PxTransform& pose, const PintSphereSweepData* sweeps) {
                return ToPxSphereGeometry(pose, sweeps->mSphere);
            }
        };
        return _BatchSweeps<PintSphereSweepData, PxSphereGeometry, IceToPx>(
                nb, dest, sweeps, GetSweepQueryFlags(mParams), GetSQFilterData(), GetSQFilterCallback());
    }

    return PhysX::BatchSphereSweeps(context, nb, dest, sweeps);
}

udword PhysX_CQS::BatchCapsuleSweeps(PintSQThreadContext context,
                                     udword nb,
                                     PintRaycastHit* dest,
                                     const PintCapsuleSweepData* sweeps) {
    if (gSceneQuerySystem) {
        struct IceToPx {
            static inline_ PxCapsuleGeometry ToPxGeometry(PxTransform& pose, const PintCapsuleSweepData* sweeps) {
                return ToPxCapsuleGeometry(pose, sweeps->mCapsule);
            }
        };
        return _BatchSweeps<PintCapsuleSweepData, PxCapsuleGeometry, IceToPx>(
                nb, dest, sweeps, GetSweepQueryFlags(mParams), GetSQFilterData(), GetSQFilterCallback());
    }
...
udword PhysX_CQS::BatchConvexSweeps(PintSQThreadContext context,
                                    udword nb,
                                    PintRaycastHit* dest,
    ...
    if (gSceneQuerySystem) {
        ...
        while (nb--) {
            ...
            PxSweepHit Hit;
            if (PEEL_SweepClosest(Geom, Pose, ToPxVec3(sweeps->mDir), sweeps->mMaxDist, Hit, sweepQueryFlags, PF,
                                  FCB)) {
                NbHits++;
                FillResultStruct(*dest, Hit);
            } else {
...
udword PhysX_CQS::FindTriangles_MeshMeshOverlap(PintSQThreadContext context,
...
void PhysX_CloseGUI() { PhysX3::CloseSharedGUI(); }
