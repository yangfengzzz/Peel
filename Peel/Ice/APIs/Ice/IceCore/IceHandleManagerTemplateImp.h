 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
TEMPLATED_MANAGER::~TEMPLATED_MANAGER() { SetupLists(); }
...
Handle TEMPLATED_MANAGER::Add(const TEMPLATED_OBJECT& object) {
    ...
    if (mNbFreeIndices) {
        ...
        if (mCurrentNbObjects == mMaxNbObjects) {
            ...
            TEMPLATED_OBJECT* NewList =
                    (TEMPLATED_OBJECT*)ICE_ALLOC(sizeof(TEMPLATED_OBJECT) * mMaxNbObjects);  // New physical list
...
void TEMPLATED_MANAGER::RemovePhysicalIndex(udword physical_index) {
    ...
    {
        //		if(mStamps[VirtualIndex]!=handle>>16)	return;							//
        //Stamp mismatch => index has been recycled

        // Update list so that there's no hole
        mObjects[physical_index] =
                mObjects[--mCurrentNbObjects];  // Move the real object so that the array has no holes.
...
bool TEMPLATED_MANAGER::Remap(const udword* ranks) {
    ...
    // Get some bytes
    TEMPLATED_OBJECT* NewList = (TEMPLATED_OBJECT*)ICE_ALLOC(sizeof(TEMPLATED_OBJECT) * mMaxNbObjects);
    CHECKALLOC(NewList);
    uword* NewOTI = (uword*)ICE_ALLOC(sizeof(uword) * mMaxNbObjects);
    CHECKALLOC(NewOTI);
    uword* NewITO = (uword*)ICE_ALLOC(sizeof(uword) * mMaxNbObjects);
    CHECKALLOC(NewITO);
    ...
    for (udword i = 0; i < mCurrentNbObjects; i++) {
        ...
        const uword Handle =
                mInToOut[NewIndex];  // Catch current handle (which is not modified by the remapping process)
