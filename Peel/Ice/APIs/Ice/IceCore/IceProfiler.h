...
inline_ void StartProfile(udword& val) { val = udword(__rdtsc()); }
inline_ void EndProfile(udword& val) { val = udword(__rdtsc()) - val; }
#else
...
class ProfileEntry {
public:
    inline_ ProfileEntry(const char* label) : mLabel(label){__PROFILE_START(mLabel)} inline_ ~ProfileEntry() {
        __PROFILE_END(mLabel)
    }
