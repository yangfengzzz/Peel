 ...
 *	Typical cases :
 *	- Limit = 0 :	log file is updated each time a log function is called. This mode is useful to debug crashing
 *apps, to check the last log message just before it crashed. (Usually a crashing app still closes the log file, so we
 *get a chance to see what went wrong)
 *	- Limit > 0 :	in that mode, the log file is only dumped to disk from time to time. That way, loops updating
 *the log file won't make the app slow down too much.
 *
