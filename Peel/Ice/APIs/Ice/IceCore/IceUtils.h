...
#define START_RUNONCE                    \
    {                                    \
        static bool __RunOnce__ = false; \
        if (!__RunOnce__) {
#define END_RUNONCE     \
    __RunOnce__ = true; \
    }                   \
...
// single instructions for this, the single instructions are usually microcoded loops that test a bit per cycle; a
// log-time algorithm coded in C is often faster. The following code uses a variable-precision SWAR algorithm to perform
...
inline_ udword ones32(udword x) {
    ...
    */
    ...
    x += (x >> 16);
    return (x & 0x0000003f);
    // "It is worthwhile noting that the SWAR population count algorithm given above can be improved upon for the case
    // of counting the population of multi-word bit sets. How? The last few steps in the reduction are using only a
    // portion of the SWAR width to produce their results; thus, it would be possible to combine these steps across
    // multiple words being reduced. One additional note: the AMD Athlon optimization guidelines suggest a very similar
    // algorithm that replaces the last three lines with return((x * 0x01010101) >> 24);. For the Athlon (which has a
    // very fast integer multiply), I would have expected AMD's code to be faster... but it is actually 6% slower
    // according to my benchmarks using a 1.2GHz Athlon (a Thunderbird). Why? Well, it so happens that GCC doesn't use a
    // multiply instruction - it writes out the equivalent shift and add sequence!"
}
...
inline_ udword tzc(sdword x) { return (ones32((x & -x) - 1)); }
...
inline_ sdword max_(sdword a, sdword b) {
    sdword delta = a - b;
    return a - (delta & (delta >> 31));
}

// "Integer Selection
// A branchless, lookup-free, alternative to code like if (a<b) x=c; else x=d; is ((((a-b) >> (WORDBITS-1)) & (c^d)) ^
// d). This code assumes that the shift is signed, which, of course, C does not promise."
inline_ sdword IntegerSelection(sdword a, sdword b, sdword c, sdword d) { return ((((a - b) >> 31) & (c ^ d)) ^ d); }
...
// A Gray code is any binary coding sequence in which only a single bit position changes as we move from one value to
// the next. There are many such codes, but the traditional one is computed such that the Kth Gray code is K^(K>>1).
//
// The well-known algorithm for conversion from Gray to binary is a linear sequence of XORs that makes it seem each bit
// must be dealt with separately. Fortunately, that is equivalent to a parallel prefix XOR that can be computed using
// SWAR techniques in log time. For 32-bit Gray code values produced as described above, the conversion from Gray code
// back to unsigned binary is:"
...
*/
template <class Type>
inline_ Type FeedbackFilter(Type val, Type& memory, float sharpness) {
    ASSERT(sharpness >= Type(0.0) && sharpness <= Type(1.0) && "Invalid sharpness value in feedback filter");
    if (sharpness < Type(0.0))
        sharpness = Type(0.0);
    else if (sharpness > Type(1.0))
        sharpness = Type(1.0);
    return memory = val * sharpness + memory * (Type(1.0) - sharpness);
}
...
//	#define PREVENT_COPY(curclass)	void operator = (const curclass& object)	{	ASSERT(!"Bad use of
//operator =");	}
// ... actually this is better !
#define PREVENT_COPY(cur_class)         \
private:                                \
    cur_class(const cur_class& object); \
    cur_class& operator=(const cur_class& object);
...
FUNCTION ICECORE_API udword IntegerDivideConstant(udword x, udword divisor);
...
inline_ void FSinCos(float& c, float& s, float f) {
    ...
    float Local = f;
#ifdef WIN32
    _asm fld Local _asm fsincos _asm fstp LocalCos _asm fstp LocalSin
    // TODO: Put this back and make it link
    /*
#elif LINUX
  asm("fld	Local\n\t"
      "fsincos\n\t"
...
#define GET_NEXT_INDICES3(i, j, k) \
    j = (1 << i) & 3;              \
    k = (1 << j) & 3;

inline_ uword ToWord(float value) {
    ASSERT(value >= 0.0f && value <= 65535.0f);
    return uword(value);
}
inline_ uword ToWord(udword value) {
    ASSERT((value & 0xffff) == value);
    return uword(value);
}
inline_ sword ToWord(sdword value) {
    ASSERT((value & 0xffff) == value);
    return sword(value);
}

inline_ ubyte ToByte(uword value) {
    ASSERT((value & 0xff) == value);
    return ubyte(value);
}
inline_ sbyte ToByte(sword value) {
    ASSERT((value & 0xff) == value);
    return sbyte(value);
}

inline_ ubyte ToByte(udword value) {
    ASSERT((value & 0xff) == value);
    return ubyte(value);
}
inline_ sbyte ToByte(sdword value) {
    ASSERT((value & 0xff) == value);
    return sbyte(value);
}

uword ToWord(uword value);
ubyte ToByte(ubyte value);
