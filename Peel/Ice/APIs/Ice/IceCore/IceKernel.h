...
enum KernelFlag {
    ...
    KF_SECURITY =
            (1
             << 2),  //!< Enables high-level security. Kernel is allowed to automatically disable "explosive" features.
    KF_CELL_NOTIF = (1 << 3),  //!< Enables Kernel-to-cells notifications
    KF_CELL_MODIF =
            (1 << 4),  //!< Allows the kernel to automatically fix bad pointers within cells (handle that one with care)
...
//				bool				RegisterAPI		(IceAPI* api);
    ...
    inline_ bool CreateTypedRef(Cell* owner, Cell* ref, RTYPE type) {
        return mTracker.AddRef(ToWord(owner->m__KernelID), ToWord(ref->m__KernelID), type);
    }
     ...
     *	\param		list	[out] list of cells. The container is not reset at first so you can append more and more cells
     *from different types. \param		type	[in] wanted type \return		the number of cells
     *found \see		GetCell(const char* name)
     */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     *	\param		list	[out] list of cells. The container is sorted so that most probable cells are the first
     *ones. \param		name	[in] wanted cell's name \return		number of cells in returned list
     */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    inline_ void InvalidateCache() {
        mCachedKID = INVALID_KID;
        mCachedCell = null;
    }
#endif
    PREVENT_COPY(Kernel)
private:
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /**
     *	Fixes invalid pointers and invalid KIDs within cell-based classes. This is an advanced ICE feature, possibly
     *quite dangerous. The kernel here has full control and can modify all class members, even private ones. Use the
     *appropriate flag if you want to ban this behaviour. \param		owner			[in] the cell to
     *fix \param		bad_ref			[in] the invalid cell pointer (the one we want to fix within the
     *cell) \param		new_ref			[in] the new cell pointer or null (will replace the old one)
     *	\param		allow_modif		[in] true to allow the actual fix, else just look for the bad pointer without fixing
