...
class TEMPLATED_EXPORT TEMPLATED_CONTAINER : public PtrContainer {
public:
    ...
    inline_ void AddCallback(TEMPLATED_CALLBACK callback, void* user_data) {
        AddPtr((void*)callback).AddPtr(user_data);
    }
    // Remove a callback from the container, otherwise preserving the insertion order
    //		inline_			bool				RemoveCallback(TEMPLATED_CALLBACK callback)	{ return
    //DeleteKeepingOrder(udword(callback));	}
    bool RemoveCallback(TEMPLATED_CALLBACK callback);

    // Executes all callbacks in order
    /*		inline_			void				ExecCallbacks(TEMPLATED_PARAM param, IceCallbackParams* context, udword
       user_data)
                                                                                            {
                                                                                                    // Loop through
       callbacks udword NbCallbacks = GetNbEntries(); TEMPLATED_CALLBACK* Callbacks = (TEMPLATED_CALLBACK*)GetEntries();
                                                                                                    while(NbCallbacks--)
                                                                                                    {
                                                                                                            // Get
       current callback TEMPLATED_CALLBACK cb = *Callbacks++;
                                                                                                            // Execute
       callback if(cb)	(cb)(param, context, user_data);
                                                                                                    }
                                                                                            }*/
...
public:
    ...
    /*		// Setup user-defined data
                    inline_			void					SetUserData(udword data)				{ mUserData =
       data;		}
                    // Get user-defined data
                    inline_			udword					GetUserData()					const	{ return
       mUserData;		}
    */
