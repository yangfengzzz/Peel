...
public:                                                                   \
    ...
    virtual bool IsKindOf(const char* name, udword* depth = null) const { \
        ...
        if (strcmp(current_class::GetClassName(), name) == 0)             \
            return true;                                                  \
        else                                                              \
            return base_class::IsKindOf(name, depth);                     \
    }                                                                     \
                                                                          \
...
#define DYNAMIC_CAST(current_class, class_name) \
    (##class_name*)((!current_class || !current_class->IsKindOf(#class_name)) ? null : current_class)
