...
public:
    ...
    inline_ operator float() const {
        float tmp = MAX_FLOAT;
        IsFloat(mText, &tmp);
        return tmp;
    }
    //! Cast operator for sdword = String
    inline_ operator sdword() const {
        sdword tmp = INVALID_NUMBER;
        IsDecimal(mText, &tmp);
        return tmp;
    }
    ...
    inline_ String& operator=(const char* string) {
        Set(string);
        return *this;
    }
    //! Operator for String = char*
    inline_ String& operator=(char* string) {
        Set((const char*)string);
        return *this;
    }
    //! Operator for String = String
    inline_ String& operator=(const String& string) {
        Set(string);
        return *this;
    }
...
ICECORE_API inline_ bool operator==(const String& s1, const char* s2) {
...
ICECORE_API inline_ bool operator==(const String& s1, const String& s2) {
    return s1.Compare(s2) == 0;
}  //!< Operator for String == String
ICECORE_API inline_ bool operator!=(const String& s1, const String& s2) {
    return s1.Compare(s2) != 0;
...
class ICECORE_API Strings : public PtrContainer {
public:
    Strings();
    ~Strings();
...
protected:
    udword SetLength(udword count, bool KeepOldContent = true);  // This method is protected because it transforms the
                                                                 // String object in a temporary "non-valid" String!
