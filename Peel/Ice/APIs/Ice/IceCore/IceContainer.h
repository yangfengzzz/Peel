...
public:
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /**
     *	Initializes the container so that it uses an external memory buffer. The container doesn't own the memory,
     *resizing is disabled. \param		max_entries		[in] max number of entries in the container
     *	\param		entries			[in] external memory buffer
     *	\param		allow_resize	[in] false to prevent resizing the array (forced fixed length), true to enable it. In
     *that last case the initial memory is NOT released, it's up to the caller. That way the initial memory can be on
     *the stack.
     */
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    inline_ void ForceSize(udword size) { mCurNbEntries = size; }
    ...
    inline_ udword GetMaxNbEntries() const {
        return mMaxNbEntries;
    }                                                                //!< Returns max number of entries before resizing.
    ...
    // Growth control
    //				float			GetGrowthFactor()				const;												//!< Returns the growth
    //factor
    //				void			SetGrowthFactor(float growth);														//!< Sets the growth
    //factor
...
#else
#define LocalContainer(x, n) \
    Container0 x;            \
    x.InitSharedBuffers(n, (udword*)StackAlloc(sizeof(udword) * n), true);
#endif
...
public:
    ...
    Container2(const Container2& object) {
        for (udword i = 0; i < object.GetNbEntries(); i++) Add(object.GetEntry(i));
    }
    ...
    inline_ udword* Reserve(udword nb) {
        ...
        /*									// Resize if needed
           ...
           Resize(nb);
                                                                                ...
                                                                                // We expect the user to fill reserved
           memory with 'nb' udwords udword* Reserved = &mEntries[mCurNbEntries];

                                                                                // Meanwhile, we do as if it had been
           filled mCurNbEntries += nb;

                                                                                // This is mainly used to avoid the copy
           when possible return Reserved;*/
    }
    ...
    bool Delete(udword entry) {
        ...
        for (udword i = 0; i < NbEntries; i++) {
            if (Entries[i] == entry) {
                // Entry has been found at index i. The strategy is to copy the last current entry at index i, and
                // decrement the current number of entries.
                DeleteIndex(i);
                return true;
            }
        }
    ...
    // Stats
    udword GetUsedRam() const { return sizeof(Container2) + GetMaxNbEntries() * sizeof(udword); }
    /*
                    // Helpers
...
class ICECORE_API PtrContainer : public Allocateable {
public:
    PtrContainer();
    ~PtrContainer();

    inline_ PtrContainer& AddPtr(void* ptr) {
        // Resize if needed
        if (mCurNbEntries == mMaxNbEntries) Resize();

        // Add new entry
        mEntries[mCurNbEntries++] = ptr;
        return *this;
    }

    inline_ PtrContainer& AddPtr(const void* ptr) { return AddPtr(const_cast<void*>(ptr)); }

    inline_ PtrContainer& AddPtrs(void** entries, udword nb) {
        if (entries && nb) {
            // Resize if needed
            if (mCurNbEntries + nb > mMaxNbEntries) Resize(nb);

            // Add new entries
            CopyMemory(&mEntries[mCurNbEntries], entries, nb * sizeof(void*));
            mCurNbEntries += nb;
        }
        return *this;
    }

    inline_ PtrContainer& AddPtrs(const void** entries, udword nb) { return AddPtrs(const_cast<void**>(entries), nb); }

    inline_ PtrContainer& AddUnique(void* ptr) {
        if (!Contains(ptr)) AddPtr(ptr);
        return *this;
    }

    inline_ PtrContainer& AddUnique(const void* ptr) { return AddUnique(const_cast<void*>(ptr)); }
    ...
    bool SetSize(udword nb);
    bool Contains(void* entry, udword* location = null) const;
    bool Delete(void* entry);

    inline_ bool Delete(const void* ptr) { return Delete(const_cast<void*>(ptr)); }
    ...
    bool DeleteKeepingOrder(void* entry);
    ...
    inline_ void* GetEntry(udword i) const { return mEntries[i]; }  //!< Returns ith entry.
    inline_ void** GetEntries() const { return mEntries; }          //!< Returns the list of entries.

    //! Read-access as an array
    inline_ void* operator[](udword i) const {
        ASSERT(i < mCurNbEntries);
        return mEntries[i];
    }
    //! Write-access as an array
    inline_ void*& operator[](udword i) {
        ASSERT(i < mCurNbEntries);
        return mEntries[i];
    }
