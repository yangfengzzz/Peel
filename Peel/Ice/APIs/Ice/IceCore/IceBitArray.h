...
inline_ udword BitsToBytes(udword nb_bits) { return (nb_bits >> 3) + ((nb_bits & 7) ? 1 : 0); }

inline_ udword BitsToDwords(udword nb_bits) { return (nb_bits >> 5) + ((nb_bits & 31) ? 1 : 0); }
...
public:
    ...
    void Empty();
    void Resize(udword nb_bits);

    inline_ void SetBitChecked(udword bit_number) {
        const udword Index = bit_number >> 5;
        if (Index >= mSize) Resize(bit_number);
        mBits[Index] |= 1 << (bit_number & 31);
    }
    ...
    inline_ BOOL IsSetChecked(udword bit_number) const {
        const udword Index = bit_number >> 5;
        if (Index >= mSize) return 0;
        return mBits[Index] & (1 << (bit_number & 31));
    }
...
public:
    inline_ udword Index(udword x, udword y) const {
        if (x > y) Swap(x, y);
        return x + (y ? ((y - 1) * y) >> 1 : 0);
    }

    inline_ void Set(udword x, udword y) {
        udword i = Index(x, y);
        mBits[i >> 3] |= BitMasks[i & 7];
    }
    inline_ void Clear(udword x, udword y) {
        udword i = Index(x, y);
        mBits[i >> 3] &= NegBitMasks[i & 7];
    }
    inline_ void Toggle(udword x, udword y) {
        udword i = Index(x, y);
        mBits[i >> 3] ^= BitMasks[i & 7];
    }
    inline_ bool IsSet(udword x, udword y) const {
        udword i = Index(x, y);
        return (mBits[i >> 3] & BitMasks[i & 7]) != 0;
    }
