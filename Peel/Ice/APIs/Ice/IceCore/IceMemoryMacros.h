...
//!	\see		CopyMemory
//!	\see		MoveMemory
inline_ void FillMemory(void* dest, size_t size, ubyte val) { memset(dest, val, size); }
...
inline_ void FlexiCopy(const void* src, void* dst, udword nb_elem, size_t elem_size, udword stride) {
    ubyte* d = (ubyte*)dst;
...
//! Clears current object. Laziness is my business! HANDLE WITH CARE. ### Removed, too dangerous, cleared too many
//! v-tables
// #define CLEAROBJECT		{ memset(this, 0, SIZEOFOBJECT);	}

// The two macros below are here for several reasons:
// - sometimes we write "delete x" instead of "delete []x" just because we don't pay attention. Using the macro forces
// you
...
// - deleting null is a valid operation according to the standard, yet some lame memory managers don't like it. In sake
// of
...
#define CHECKALLOC(x) \
    if (!x) return SetIceError("Out of memory.", EC_OUT_OF_MEMORY);
#else
#define CHECKALLOC(x) \
    if (!x) return false;
#endif
...
#define SAFE_ALLOC(ptr, type, count) \
    ...
    ptr = new type[count];           \
    CHECKALLOC(ptr);
#define SAFE_ICE_ALLOC(ptr, type, count) \
    DELETEARRAY(ptr);                    \
    ptr = ICE_NEW(type)[count];          \
    CHECKALLOC(ptr);
...
//	#define ICE_ALLOC(x)			GetAllocator()->mallocDebug(x, __FILE__, __LINE__, #x,
//MEMORY_PERSISTENT)
#define ICE_ALLOC_TMP(x) GetAllocator()->mallocDebug(x, __FILE__, __LINE__, "(undefined)", MEMORY_TEMP, false)
#define ICE_ALLOC(x) GetAllocator()->mallocDebug(x, __FILE__, __LINE__, "(undefined)", MEMORY_PERSISTENT, false)
#define ICE_ALLOC_TMP2(x, y) GetAllocator()->mallocDebug(x, __FILE__, __LINE__, #y, MEMORY_TEMP, false)
#define ICE_ALLOC2(x, y) GetAllocator()->mallocDebug(x, __FILE__, __LINE__, #y, MEMORY_PERSISTENT, false)
#else
