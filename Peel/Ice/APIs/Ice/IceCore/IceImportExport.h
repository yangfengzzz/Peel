...
public:                                                                                                 \
    ...
    virtual udword Export(ExportContext& ec) {                                                          \
        if (!base_class::Export(ec))                                                                    \
            return 0;                                                                                   \
        else                                                                                            \
            return ec.Save(#current_class, this, mClassDescriptor, current_class::GetDescriptorSize()); \
    }                                                                                                   \
                                                                                                        \
    virtual udword Import(ImportContext& ic) {                                                          \
        if (!base_class::Import(ic))                                                                    \
            return 0;                                                                                   \
        else                                                                                            \
            return ic.Load(#current_class, this, mClassDescriptor, current_class::GetDescriptorSize()); \
    }
...
public:                 \
    virtual bool ExecCommand(const char* command, void* user_data = null);
...
#define DECLARE_METHOD_0(method)              \
    if (!command)                             \
        EnumCommand(#method, user_data);      \
    else if (strcmp(command, #method) == 0) { \
        method();                             \
        return true;                          \
    }
#define DECLARE_METHOD_1(method, p0)          \
    if (!command)                             \
        EnumCommand(#method, user_data);      \
    else if (strcmp(command, #method) == 0) { \
        method(p0);                           \
...
#define DECLARE_METHOD_2(method, p0, p1)      \
    if (!command)                             \
        EnumCommand(#method, user_data);      \
    else if (strcmp(command, #method) == 0) { \
        method(p0, p1);                       \
...
#define IMPLEMENT_CLASS(current_class)                                                   \
    ...
    void current_class::DeleteInstance(Cell* instance) {                                 \
        if (instance) delete (current_class*)instance;                                   \
    }                                                                                    \
    bool current_class::RegisterClass() {                                                \
        return GetKernel()->RegisterClass(#current_class, current_class::CreateInstance, \
                                          current_class::DeleteInstance);                \
    }
...
#define LAZY_CREATE(class_name, address) \
    if (!address) address = CreateICEObject(class_name)
