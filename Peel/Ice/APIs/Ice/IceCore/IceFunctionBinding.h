...
/*
        ...
        On NT 4.0 for example, some functions do not exist, hence IceCore fails to initialize. This is painful since
   *all* programs using ICE then fail on NT 4.0, even if they don't actually use the non-existing OS function. By using
   dynamic loading, we only penalize apps that actually use those missing calls. We also give them a chance to run
   anyway, in case the call is not vital.
...
FUNCTION ICECORE_API LONG RegCreateKeyEx(HKEY key,
                                         LPCSTR subkey,
                                         DWORD reserved,
                                         LPSTR lpclass,
                                         DWORD options,
                                         ...
                                         PHKEY result,
                                         LPDWORD disp);
FUNCTION ICECORE_API LONG
RegSetValueEx(HKEY key, LPCSTR valuename, DWORD reserved, DWORD type, CONST BYTE* data, DWORD cbdata);
FUNCTION ICECORE_API LONG RegEnumKeyEx(HKEY key,
                                       DWORD index,
                                       LPSTR name,
                                       LPDWORD cbname,
                                       LPDWORD reserved,
...
FUNCTION ICECORE_API LONG
RegQueryValueEx(HKEY key, LPCSTR valuename, LPDWORD reserved, LPDWORD type, LPBYTE data, LPDWORD cbdata);
FUNCTION ICECORE_API LONG RegCloseKey(HKEY key);

// GDI functions
FUNCTION ICECORE_API int GetDeviceCaps(HDC hdc, int index);
FUNCTION ICECORE_API BOOL
ExtTextOut(HDC hdc, int x, int y, UINT options, CONST RECT* rect, LPCSTR str, UINT count, CONST INT* dx);
...
FUNCTION ICECORE_API int SetDIBitsToDevice(HDC hdc,
                                           int x_dest,
                                           int y_dest,
                                           DWORD width,
                                           DWORD height,
...
FUNCTION ICECORE_API HFONT CreateFont(int height,
                                      int width,
                                      int escapement,
                                      int orientation,
                                      int weight,
...
FUNCTION ICECORE_API HBITMAP
CreateDIBSection(HDC hdc, CONST BITMAPINFO* pbmi, UINT usage, VOID** bit_values, HANDLE section, DWORD offset);
FUNCTION ICECORE_API int StretchDIBits(HDC hdc,
                                       int destx,
                                       int desty,
                                       int dw,
                                       int dh,
...
// Window functions
FUNCTION ICECORE_API HWND CreateWindowEx(DWORD ex_style,
                                         LPCSTR class_name,
                                         LPCSTR window_name,
                                         DWORD style,
                                         int x,
...
FUNCTION ICECORE_API LRESULT
CallWindowProc(IN WNDPROC prevwndfunc, IN HWND hwnd, IN UINT msg, IN WPARAM wparam, IN LPARAM lparam);
...
FUNCTION ICECORE_API int GetWindowTextLength(HWND hwnd);
...
FUNCTION ICECORE_API HDWP
DeferWindowPos(HDWP hwin_pos_info, HWND hwnd, HWND hwnd_insert_after, int x, int y, int cx, int cy, UINT flags);
...
// Dialog functions
FUNCTION ICECORE_API HWND CreateDialog_(HINSTANCE hinstance,
...
FUNCTION ICECORE_API int DialogBox_(
        HINSTANCE hinstance, LPCSTR template_name, HWND hwnd_parent, DLGPROC dialog_func, LPARAM init_param);
...
FUNCTION ICECORE_API BOOL
TrackPopupMenu(HMENU menu, UINT flags, int x, int y, int reserved, HWND hwnd, CONST RECT* rect);
...
FUNCTION ICECORE_API BOOL AppendMenu(HMENU menu, UINT flags, UINT_PTR ID_new_item, LPCSTR new_item);
FUNCTION ICECORE_API BOOL GetMenuItemInfo(HMENU menu, UINT flags, BOOL b, LPMENUITEMINFOA info);
FUNCTION ICECORE_API BOOL ModifyMenu(HMENU menu, UINT position, UINT flags, UINT_PTR ID_new_item, LPCSTR new_item);
...
// AVI player
FUNCTION ICECORE_API HRESULT
AVIStreamOpenFromFile(PAVISTREAM FAR* avi, LPCSTR file, DWORD type, LONG param, UINT mode, CLSID FAR* handler);
...
FUNCTION ICECORE_API BOOL DrawDibDraw(HDRAWDIB hdd,
                                      HDC hdc,
                                      int xDst,
                                      int yDst,
                                      int dxDst,
