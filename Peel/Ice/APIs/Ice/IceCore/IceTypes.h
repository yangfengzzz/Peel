...
// & std types - we should incrementally replace the ICE types with these
...
//	typedef udword				size_t;
//	typedef uqword				size_t;
...
ICE_COMPILE_TIME_ASSERT(sizeof(size_t) == sizeof(void*));

//! TO BE DOCUMENTED
#define DECLARE_ICE_HANDLE(name) \
    struct name##__ {            \
        int unused;              \
    };                           \
...
/* Obsolete stuff - if needed, move to dedicated header and include in few files using this.
        #define		SQR(x)			((x)*(x)) //!< Returns x square #define		CUBE(x)
((x)*(x)*(x))					//!< Returns x cube

        #define		AND		& //!< ... #define		OR		|
//!< ... #define		XOR		^
//!< ...

        #define		QUADRAT(x)		((x)*(x))						//!< Returns x
square

        typedef int					(__stdcall* PROC)();			//!< A standard
procedure call. typedef bool				(*ENUMERATION)(udword value, udword param, udword context);
//!< ICE standard enumeration call
        ...
        __forceinline	double	drand48()			{ return (double) ( ((double)rand()) /
((double)RAND_MAX) );	} #endif
*/
