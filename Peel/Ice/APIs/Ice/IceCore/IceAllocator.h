...
public:
    ...
    virtual void* mallocDebug(
            size_t size, const char* filename, udword line, const char* class_name, MemoryType type, bool from_new) = 0;
...
FUNCTION ICECORE_API Allocator* CreateDefaultAllocator();
...
public:
    ...
    inline_ void* operator new(size_t size, const char* filename, int line, const char* class_name, MemoryType type) {
        return GetAllocator()->mallocDebug(size, filename, line, class_name, type, true);
    }
    inline_ void* operator new[](size_t size, MemoryType type) { return GetAllocator()->malloc(size, type); }
    inline_ void* operator new[](size_t size, const char* filename, int line, const char* class_name, MemoryType type) {
        return GetAllocator()->mallocDebug(size, filename, line, class_name, type, true);
    ...
    inline_ void operator delete[](void* p) { GetAllocator()->free(p, true); }
    inline_ void operator delete[](void* p, MemoryType) { GetAllocator()->free(p, true); }
    inline_ void operator delete[](void* p, const char*, int, const char*, MemoryType) {
