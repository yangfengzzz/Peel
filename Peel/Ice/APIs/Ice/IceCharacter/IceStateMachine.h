...
public:
    ...
    /*		inline_	void				SetTransitionCallback(TransitionCallback cb)	{ mTransitionCB		= cb;
       } inline_	void				SetMotionStartCallback(MotionCallback cb)		{
       mMotionStartCB	= cb;						} inline_	void
       SetMotionCallback(MotionCallback cb)			{ mMotionCB			= cb;
       }
                    inline_	void				SetUserData(void* user_data)					{ mUserData
       = user_data;				}

                    inline_	TransitionCallback	GetTransitionCallback()			const			{ return
       mTransitionCB;							} inline_	MotionCallback
       GetMotionCallback()				const			{ return mMotionCB;
       } inline_	MotionCallback		GetMotionStartCallback()		const			{ return
       mMotionStartCB;						} inline_	void*
       GetUserData()					const			{ return mUserData;
       }
    */
