...
//	MotionCells' children are MotionTransitions. One MotionCell may have multiple MotionTransitions, but there can
//only be a single active transition at a particular time. 	The active transition is of course one of the cell's
...
public:
    // Transition creation
    inline_ MotionTransition* CreateTransition(const MOTIONTRANSITIONCREATE& create) {
        return mTransitions.CreateTransition(create);
    }
    ...
    //} 		inline_	MotionTransition*	GetTransition(udword id)				const	{ return
    //(MotionTransition*)mTransitions.GetEntry(id);	}
    ...
    MotionCell& DispatchEventToTransitions(MotionState& state,
                                           const BasicMotionEvent& event,
                                           float event_time,
                                           float current_time);
    ...
    //		inline_	MotionCell&			ResetLocalTime(udword baseframe=0, float deltatm=0.0f)	{ mLocalTime = deltatm *
    //float(baseframe); return *this; } 				MotionCell&			UpdateLocalTime(float relativetime, float
    //deltatm);

    // Data access
    //		inline_	float				GetLocalTime()							const	{ return mLocalTime;
    //} 		inline_	float				GetInterpolCoeff()
    //const	{ return mt;									} 		inline_	udword
    //GetLocalFrame()							const	{ return mLocalFrame;
    //}
    ...
    //} 		inline_	const String&		GetDataType()
    //const	{ return mDataType;										}
...
//				float				mt;					//!< Interpolation coeff between
//0.0f and 1.0f 				udword				mLocalFrame;		//!< Current local frame
#ifdef SUPPORT_MOTION_MODIFIERS
    ...
    // Strings
    //				String				mDataType;			//!< User-defined generic
    //identifier
