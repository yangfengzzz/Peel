...
#define STATEMACHINE_MAX_NB_EVENTS \
    128  // We keep track of STATEMACHINE_MAX_NB_EVENTS in an event stack ### Make it dynamic + resize methods
         //	#define STATEMACHINE_MAX_NB_EVENTS	32		// We keep track of STATEMACHINE_MAX_NB_EVENTS in an event
//stack ### Make it dynamic + resize methods 	#define STATEMACHINE_MAX_NB_EVENTS	3		// We keep track of
//STATEMACHINE_MAX_NB_EVENTS in an event stack ### Make it dynamic + resize methods
...
public:
    //				MotionCell*			mStartMotion;		//!< Starting motion in the state
    //machine 				MotionCell*			mEndMotion;			//!< Ending motion in the state machine
...
private:
    ...
    /*#ifdef SUPPORT_NEW_DESIGN
                                    ...
                                    udword				SrcLastFrameCheck(udword value, udword index)
    const; udword				DstLastFrameCheck(udword value, udword index) const; #else udword
    SrcLastFrameCheck(udword value) const; udword				DstLastFrameCheck(udword value) const;
            #endif
