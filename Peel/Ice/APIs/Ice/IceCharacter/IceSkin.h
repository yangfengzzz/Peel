...
public:
     ...
     *	- the destination buffer should not directly be a vertex buffer, since you usually don't need to do the skinning
     *on vertices replicated by a consolidation process. Hence you may perform the skinning on the base mesh, then
     *replicate skinned vertices within the vertex buffer. Depends on your app, actually.
     *	- the normals are not computed here anymore, since the final number of vertices may be quite different from the
     *number of vertices in the original skin (notably when it's used as the control net of a subdivision surface)
     *
...
private:
    ...
    udword* mBonesID;  //!< A list of NbBonesID IDs. There can be one or many BonesID for each vertex of the skin. (==>
                       //!< links to underlying bones)
