...
enum SkelFlag {
    ...
    SKELETON_READONLY = SKELETON_CONSTANT_TRANSLAT | SKELETON_IS_BLENDING | SKELETON_TRANS_PENDING |
                        SKELETON_IS_MOVING | SKELETON_INVALID_GLOBAL,
...
//		inline_ const Point&		GetAccumOffset()					const	{ return
//mAccumOffset;							}
//		inline_ void				ResetAccumOffset()							{
//mAccumOffset.Zero();							}
#endif
...
private:
    ...
    Matrix4x4* mGlobalMat;     //!< Current motion expressed as global matrices for each bone
                               //				Bone*				mBoneRoot;							//!< bonne route! ... hum, the root bone, shortcut in
                               //mBones
                               // Time management
