...
inline_ bool RayOBB(const Ray& ray, const OBB& obb) { return Ctc::RayOBB(ray, obb.mCenter, obb.mExtents, obb.mRot); }
...
CONTACT_API int ray_intersect(
        const Point& min, const Point& max, const Point& ro, const Point& rd, float& tnear, float& tfar);
