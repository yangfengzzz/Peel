...
public:
    ...
    inline_ IntervalPoint(const Point& p0, const Point& p1) {
        x = IceInterval(p0.x, p1.x);
        if (x.a > x.b) TSwap(x.a, x.b);
        y = IceInterval(p0.y, p1.y);
        if (y.a > y.b) TSwap(y.a, y.b);
        z = IceInterval(p0.z, p1.z);
        if (z.a > z.b) TSwap(z.a, z.b);
    }
    inline_ ~IntervalPoint() {}
    ...
    inline_ IntervalPoint operator^(const IntervalPoint& p) const {
        return IntervalPoint(y * p.z - z * p.y, z * p.x - x * p.z, x * p.y - y * p.x);
    }
...
public:
    ...
    inline_ IntervalMatrix3x3(IceInterval m00,
                              IceInterval m01,
                              IceInterval m02,
                              IceInterval m10,
                              IceInterval m11,
...
CONTACT_API bool ContinuousOBBOBB(const OBB& world_box0,
                                  const OBB& world_box1,
                                  const Screwing& screw0,
                                  const Screwing& screw1);
CONTACT_API BOOL IsCollidingContact(const Point& world_pt,
                                    const Point& world_normal,
                                    const Screwing& s0,
                                    const Screwing& s1);
CONTACT_API BOOL IsCollidingContact(const Point& world_pt,
                                    const Point& world_normal,
                                    const Point& pivot0,
                                    const Point& velocity0,
                                    const Point& omega0,
                                    const Point& pivot1,
                                    const Point& velocity1,
                                    const Point& omega1);
CONTACT_API void ComputeBounds(AABB& bounds, const Point& world_pt, const IceInterval& it, const Screwing& screwing);
CONTACT_API BOOL
CloseEdgeCollision(const Point& p1, const Point& p2, const Point& p3, const Point& p4, float* dist = null);
...
CONTACT_API float VertexFaceTest_(udword nb,
                                  const Point& pa,
                                  const Point& pb,
                                  const Point& pc,
                                  const Point& pd,
...
CONTACT_API float EdgeEdgeTest_(udword nb,
                                const Point& pa,
                                const Point& pb,
                                const Point& pc,
                                const Point& pd,
                                ...
                                ,
                                const Point& edge_normal0,
                                const Point& edge_normal1
#endif
);
