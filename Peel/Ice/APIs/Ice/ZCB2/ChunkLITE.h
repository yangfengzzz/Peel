...
class ZCB2_API LITEChunk : public PRSChunk {
    ...
    /*		// Data access
                    ...
                    inline_			bool				GetCastShadows()	const	{ return
       (mLITECore.mFlags&ZCB2_LITE_CAST_SHADOWS)!=0;	} inline_			bool
       GetLightUsed()		const	{ return (mLITECore.mFlags&ZCB2_LITE_USED)!=0;			}

                    // Chunk definition
                                                    bool				SetDiffuseColor(const RGBAColor&
       color); bool				SetSpecularColor(const RGBAColor& color); bool
       SetAmbientColor(const RGBAColor& color); bool				SetCastShadows(bool flag); bool
       SetLightUsed(bool flag); private:
                    // Chunk data
