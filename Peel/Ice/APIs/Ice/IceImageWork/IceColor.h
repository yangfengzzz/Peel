...
public:
    ...
    inline_ void SetColor(const RGBAPixel& col) {
        B = col.B;
        G = col.G;
        R = col.R;
        A = col.A;
    }
    inline_ void SetColor(ubyte r, ubyte g, ubyte b, ubyte a = PIXEL_OPAQUE) {
        R = r;
        G = g;
        B = b;
        A = a;
...
public:
    ...
    inline_ operator udword() const {
        ...
        if (SIR(A) >= IEEE_1_0)
            ...
            Color = ((udword)(A * 255.0f)) << 24;

        if (SIR(R) >= IEEE_1_0)
            Color |= 0xFF << 16;
        else if (SIR(R) > 0)
            Color |= ((udword)(R * 255.0f)) << 16;

        if (SIR(G) >= IEEE_1_0)
            Color |= 0xFF << 8;
        else if (SIR(G) > 0)
            Color |= ((udword)(G * 255.0f)) << 8;

        if (SIR(B) >= IEEE_1_0)
            Color |= 0xFF;
        else if (SIR(B) > 0)
            Color |= (udword)(B * 255.0f);
