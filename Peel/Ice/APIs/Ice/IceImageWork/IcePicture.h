...
public:
    ...
    Picture(const Picture& source);
    Picture(uword width, uword height, const RGBAPixel* pixels = null);
    ~Picture();
    ...
    bool CopyRegion(const Picture& src,
                    uword region_width,
                    uword region_height,
                    uword src_x,
                    uword src_y,
    ...
    Picture& CreateAnisotropicTable(float r = 0.0f,
                                    float g = 0.0f,
                                    float b = 0.0f,
                                    float kd = 1.0f,
                                    float ks = 1.0f,
    ...
    bool Shade(const RGBAPixel& start_color,
               const RGBAPixel& end_color,
               udword start_line = INVALID_ID,
               udword end_line = INVALID_ID);
    bool Fill(const RGBAPixel& color);
    bool MakeRasters(const RGBAPixel& start_color,
                     const RGBAPixel& mid_color,
                     const RGBAPixel& end_color,
                     udword mid_line = INVALID_ID);
    Picture& Luminance();
    ...
    Picture& BinaryAlpha2(const RGBPixel& test_value,
                          ubyte alpha_full = PIXEL_OPAQUE,
                          ubyte alpha_null = PIXEL_INVISIBLE,
                          const RGBPixel* replacement_value = null);
