...
enum LightType {
    LIGHT_POINT =
            0x1,  //!< Light is a point source. The light has a position in space and radiates light in all directions.
    LIGHT_SPOT = 0x2,  //!< Light is a spotlight source. This light is like a point light, except that the illumination
                       //!< is limited to a cone. This light type has a direction and several other parameters that
                       //!< determine the shape of the cone it produces.
    LIGHT_DIRECT = 0x3,  //!< Light is a directional source. This is equivalent to using a point light source at an
                         //!< infinite distance.
