...
#ifdef _DEBUG
#define BRS_CHECK(flag, mask)                       \
    mNbBRSCalls++;                                  \
    const bool Current = (mCacheFlags & mask) != 0; \
    if (Current == (flag != 0)) return FALSE;       \
    if (flag)                                       \
        mCacheFlags |= mask;                        \
    else                                            \
        mCacheFlags &= ~mask;                       \
...
#ifdef _DEBUG
#define DRS_CHECK(dword, mask)                     \
    mNbDRSCalls++;                                 \
    if (mCacheDwords[mask] == dword) return FALSE; \
    mCacheDwords[mask] = dword;                    \
    mNbDRSDone++;
#define TS_CHECK(dword, member)                              \
    mNbTSCalls++;                                            \
    if (IR(mTexCaches[i].member) == IR(dword)) return FALSE; \
    mTexCaches[i].member = dword;                            \
    mNbTSDone++;
#else
#define DRS_CHECK(dword, mask)                     \
    if (mCacheDwords[mask] == dword) return FALSE; \
...
#define PL_CHECK(level, mask) \
    if (level <= mPriorityLevels[mask]) return FALSE;
#define TPL_CHECK(level, member) \
    if (level <= mPLTexture[i].member) return FALSE;
#else
#define PL_CHECK(level, mask)
#define TPL_CHECK(level, member)
#endif
...
//		#define RS_PREVENT(cndt, text)		if(mCheckings && (cndt))	SetIceError(text, gEC_InvalidRS,
//mUpdateLogFile); 		#define RS_PREVENT(cndt, text)		if(mCheckings && (cndt))	{ SetIceError(text, 0,
//mUpdateLogFile); return FALSE; }
#define RS_PREVENT(cndt, text) \
    if (mCheckings && (cndt)) return FALSE;
#else
#define RS_PREVENT(cndt, text)
#endif
...
public:
    ...
    // Management methods
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    virtual bool InitDefaultRenderStates(const RenderCaps& caps,
                                         bool reset_all);  // Initialize the default render states
    ...
    inline_ void SetWorldMatrix_CACHED(const Matrix4x4& world) {
        if (&world != mLastTransform) SetWorldMatrix(world);
    }
...
public:
    ...
    inline void FlipFrontFaces() {
        if (mFrontFace == FRONTFACE_CW)
            mFrontFace = FRONTFACE_CCW;
        else if (mFrontFace == FRONTFACE_CCW)
            mFrontFace = FRONTFACE_CW;
    }
    // Generic render state calls
#ifdef RS_PRIORITYLEVELS
    inline_ BOOL SetRenderStateBool(BooleanRenderState id, BOOL flag, PriorityLevel plevel = 128) {
        return (this->*mBRSTable[GetBRSIndex(id)])(flag, plevel);
    }
    inline_ BOOL SetRenderStateDword(DwordRenderState id, udword state, PriorityLevel plevel = 128) {
        return (this->*mDRSTable[id])(state, plevel);
    }
    inline_ BOOL SetRenderStateTexture(TextureRenderState id, udword stage, udword state, PriorityLevel plevel = 128) {
        return (this->*mTSTable[id])(stage, state, plevel);
    }
#else
    inline_ BOOL SetRenderStateBool(BooleanRenderState id, BOOL flag) {
        return (this->*mBRSTable[GetBRSIndex(id)])(flag);
    }
    inline_ BOOL SetRenderStateDword(DwordRenderState id, udword state) { return (this->*mDRSTable[id])(state); }
    inline_ BOOL SetRenderStateTexture(TextureRenderState id, udword stage, udword state) {
        return (this->*mTSTable[id])(stage, state);
    }
#endif
...
#define DECLARE_BRS(name, def) virtual inline_ BOOL Set##name(BOOL flag = def)
    ...
    DECLARE_BRS(TexturePerspective, TRUE) {
        ...
        PL_CHECK(plevel, RS_TEXTUREPERSPECTIVE);
        BRS_CHECK(flag, BRS_TEXTUREPERSPECTIVE);
        return TRUE;
    }

    DECLARE_BRS(ZWrite, TRUE) {
        PL_CHECK(plevel, RS_ZWRITE);
        BRS_CHECK(flag, BRS_ZWRITE);
        return TRUE;
    }

    DECLARE_BRS(AlphaTest, FALSE) {
        PL_CHECK(plevel, RS_ALPHATEST);
        BRS_CHECK(flag, BRS_ALPHATEST);
        return TRUE;
    }

    DECLARE_BRS(Dithering, TRUE) {
        RS_PREVENT(!mCaps.mRasterCaps.mDither, "Dithering not supported");
        PL_CHECK(plevel, RS_DITHERING);
        BRS_CHECK(flag, BRS_DITHERING);
        return TRUE;
    }

    DECLARE_BRS(AlphaBlending, FALSE) {
        PL_CHECK(plevel, RS_ALPHABLENDING);
        BRS_CHECK(flag, BRS_ALPHABLENDING);
        return TRUE;
    }

    DECLARE_BRS(Fog, FALSE) {
        PL_CHECK(plevel, RS_FOG);
        BRS_CHECK(flag, BRS_FOG);
        return TRUE;
    }

    DECLARE_BRS(Specular, FALSE) {
        PL_CHECK(plevel, RS_SPECULAR);
        BRS_CHECK(flag, BRS_SPECULAR);
        return TRUE;
    }

    DECLARE_BRS(StencilTest, FALSE) {
        PL_CHECK(plevel, RS_STENCILTEST);
        BRS_CHECK(flag, BRS_STENCILTEST);
        return TRUE;
    }

    DECLARE_BRS(Lighting, TRUE) {
        PL_CHECK(plevel, RS_LIGHTING);
        BRS_CHECK(flag, BRS_LIGHTING);
        return TRUE;
    }

    DECLARE_BRS(LastPixel, TRUE) {
        PL_CHECK(plevel, RS_LASTPIXEL);
        BRS_CHECK(flag, BRS_LASTPIXEL);
        return TRUE;
    }

    DECLARE_BRS(StippledAlpha, FALSE) {
        PL_CHECK(plevel, RS_STIPPLEDALPHA);
        BRS_CHECK(flag, BRS_STIPPLEDALPHA);
        return TRUE;
    }

    DECLARE_BRS(EdgeAntialias, FALSE) {
        PL_CHECK(plevel, RS_EDGEANTIALIAS);
        BRS_CHECK(flag, BRS_EDGEANTIALIAS);
        return TRUE;
    }

    DECLARE_BRS(ColorKeyMode, FALSE) {
        PL_CHECK(plevel, RS_COLORKEYMODE);
        BRS_CHECK(flag, BRS_COLORKEYMODE);
        return TRUE;
    }

    DECLARE_BRS(ColorKeyBlendMode, FALSE) {
        PL_CHECK(plevel, RS_COLORKEYBLENDMODE);
        BRS_CHECK(flag, BRS_COLORKEYBLENDMODE);
        return TRUE;
    }

    DECLARE_BRS(RangeBasedFog, FALSE) {
        PL_CHECK(plevel, RS_RANGEBASEDFOG);
        BRS_CHECK(flag, BRS_RANGEBASEDFOG);
        return TRUE;
    }

    DECLARE_BRS(Clipping, TRUE) {
        PL_CHECK(plevel, RS_CLIPPING);
        BRS_CHECK(flag, BRS_CLIPPING);
        return TRUE;
    }

    DECLARE_BRS(ScreenExtents, FALSE) {
        PL_CHECK(plevel, RS_SCREENEXTENTS);
        BRS_CHECK(flag, BRS_SCREENEXTENTS);
        return TRUE;
    }

    DECLARE_BRS(ColorVertex, FALSE) {
        PL_CHECK(plevel, RS_COLORVERTEX);
        BRS_CHECK(flag, BRS_COLORVERTEX);
        return TRUE;
    }

    DECLARE_BRS(PerspSpecHigh, TRUE) {
        PL_CHECK(plevel, RS_PERSPSPECHIGH);
        BRS_CHECK(flag, BRS_PERSPSPECHIGH);
        return TRUE;
    }

    DECLARE_BRS(Normalize, FALSE) {
        PL_CHECK(plevel, RS_NORMALIZE);
        BRS_CHECK(flag, BRS_NORMALIZE);
        return TRUE;
    }

    DECLARE_BRS(SoftwareVertexProcessing, FALSE) {
        PL_CHECK(plevel, RS_SOFTWAREVERTEXPROCESSING);
        BRS_CHECK(flag, BRS_SOFTWAREVERTEXPROCESSING);
        return TRUE;
    }

    DECLARE_BRS(PointSprite, FALSE) {
        PL_CHECK(plevel, RS_POINTSPRITE);
        BRS_CHECK(flag, BRS_POINTSPRITE);
        return TRUE;
    }

    DECLARE_BRS(PointScale, FALSE) {
        PL_CHECK(plevel, RS_POINTSCALE);
        BRS_CHECK(flag, BRS_POINTSCALE);
        return TRUE;
    }

    DECLARE_BRS(MultiSampleAntialias, TRUE) {
        PL_CHECK(plevel, RS_MULTISAMPLEANTIALIAS);
        BRS_CHECK(flag, BRS_MULTISAMPLEANTIALIAS);
        return TRUE;
    }

    DECLARE_BRS(IndexedVertexBlend, TRUE) {
        PL_CHECK(plevel, RS_INDEXEDVERTEXBLEND);
        BRS_CHECK(flag, BRS_INDEXEDVERTEXBLEND);
        return TRUE;
    }
...
#define DECLARE_DRS(name, type, def) virtual inline_ BOOL Set##name(type value = def)
    ...
    DECLARE_DRS(AmbientColor, udword, 0) {
        PL_CHECK(plevel, RS_AMBIENTCOLOR);
        DRS_CHECK(value, DRS_AMBIENTCOLOR);
        return TRUE;
    }

    DECLARE_DRS(AntialiasMode, ANTIALIASMODE, ANTIALIAS_NONE) {
        PL_CHECK(plevel, RS_ANTIALIASMODE);
        DRS_CHECK(udword(value), DRS_ANTIALIASMODE);
        return TRUE;
    }

    DECLARE_DRS(ZBufferMode, ZBUFFERMODE, ZB_TRUE) {
        RS_PREVENT(zm == ZB_USEW && !mCaps.mRasterCaps.mWBuffer, "WBuffer not supported");
        PL_CHECK(plevel, RS_ZBUFFERMODE);
        DRS_CHECK(udword(value), DRS_ZBUFFERMODE);
        return TRUE;
    }
    ...
    DECLARE_DRS(ZBufferCmpMode, CMPFUNC, CMP_LESSEQUAL) {
        ...
        if (mCheckings) {
            ...
            RS_PREVENT(cf == CMP_GREATEREQUAL && !mCaps.mZCmpCaps.mGreaterEqual,
                       "ZBuffer: CMP_GREATEREQUAL not supported");
...
#endif
        PL_CHECK(plevel, RS_ZBUFFERCMPMODE);
        DRS_CHECK(udword(value), DRS_ZBUFFERCMPMODE);
        return TRUE;
    }

    DECLARE_DRS(ZBias, udword, 0) {
        RS_PREVENT(!mCaps.mRasterCaps.mZBias, "ZBias not supported");
        PL_CHECK(plevel, RS_ZBIAS);
        DRS_CHECK(value, DRS_ZBIAS);
        return TRUE;
    }

    DECLARE_DRS(LinePattern, udword, 0) {
        PL_CHECK(plevel, RS_LINEPATTERN);
        DRS_CHECK(value, DRS_LINEPATTERN);
        return TRUE;
    }

    DECLARE_DRS(AlphaRef, udword, 0) {
        PL_CHECK(plevel, RS_ALPHAREF);
        DRS_CHECK(value, DRS_ALPHAREF);
        return TRUE;
    }
    ...
    DECLARE_DRS(AlphaFunc, CMPFUNC, CMP_ALWAYS) {
        ...
        if (mCheckings) {
            ...
            RS_PREVENT(af == CMP_GREATEREQUAL && !mCaps.mAlphaCmpCaps.mGreaterEqual,
                       "AlphaFunc: CMP_GREATEREQUAL not supported");
            RS_PREVENT(af == CMP_LESS && !mCaps.mAlphaCmpCaps.mLess, "AlphaFunc: CMP_LESS not supported");
            RS_PREVENT(af == CMP_LESSEQUAL && !mCaps.mAlphaCmpCaps.mLessEqual,
                       "AlphaFunc: CMP_LESSEQUAL not supported");
...
#endif
        PL_CHECK(plevel, RS_ALPHAFUNC);
        DRS_CHECK(udword(value), DRS_ALPHAFUNC);
        return TRUE;
    }
    ...
    DECLARE_DRS(SrcBlend, ALPHABLEND, ABLEND_SRCALPHA) {
        ...
        if (mCheckings) {
            ...
            RS_PREVENT(mode == ABLEND_SRCCOLOR && !mCaps.mSrcBlendCaps.mSrcColor,
                       "SrcBlend: ABLEND_SRCCOLOR not supported");
            RS_PREVENT(mode == ABLEND_INVSRCCOLOR && !mCaps.mSrcBlendCaps.mInvSrcColor,
                       "SrcBlend: ABLEND_INVSRCCOLOR not supported");
            RS_PREVENT(mode == ABLEND_SRCALPHA && !mCaps.mSrcBlendCaps.mSrcAlpha,
                       "SrcBlend: ABLEND_SRCALPHA not supported");
            RS_PREVENT(mode == ABLEND_INVSRCALPHA && !mCaps.mSrcBlendCaps.mInvSrcAlpha,
                       "SrcBlend: ABLEND_INVSRCALPHA not supported");
            RS_PREVENT(mode == ABLEND_DESTALPHA && !mCaps.mSrcBlendCaps.mDestAlpha,
                       "SrcBlend: ABLEND_DESTALPHA not supported");
            RS_PREVENT(mode == ABLEND_INVDESTALPHA && !mCaps.mSrcBlendCaps.mInvDestAlpha,
                       "SrcBlend: ABLEND_INVDESTALPHA not supported");
            RS_PREVENT(mode == ABLEND_DESTCOLOR && !mCaps.mSrcBlendCaps.mDestColor,
                       "SrcBlend: ABLEND_DESTCOLOR not supported");
            RS_PREVENT(mode == ABLEND_INVDESTCOLOR && !mCaps.mSrcBlendCaps.mInvDestColor,
                       "SrcBlend: ABLEND_INVDESTCOLOR not supported");
            RS_PREVENT(mode == ABLEND_SRCALPHASAT && !mCaps.mSrcBlendCaps.mSrcAlphaSat,
                       "SrcBlend: ABLEND_SRCALPHASAT not supported");
            RS_PREVENT(mode == ABLEND_BOTHSRCALPHA && !mCaps.mSrcBlendCaps.mBothSrcAlpha,
                       "SrcBlend: ABLEND_BOTHSRCALPHA not supported");
            RS_PREVENT(mode == ABLEND_BOTHINVSRCALPHA && !mCaps.mSrcBlendCaps.mBothInvSrcAlpha,
                       "SrcBlend: ABLEND_BOTHINVSRCALPHA not supported");
        }
#endif
        PL_CHECK(plevel, RS_SRCBLEND);
        DRS_CHECK(udword(value), DRS_SRCBLEND);
        return TRUE;
    }
    ...
    DECLARE_DRS(DstBlend, ALPHABLEND, ABLEND_INVSRCALPHA) {
        ...
        if (mCheckings) {
            ...
            RS_PREVENT(mode == ABLEND_SRCCOLOR && !mCaps.mDestBlendCaps.mSrcColor,
                       "DestBlend: ABLEND_SRCCOLOR not supported");
            RS_PREVENT(mode == ABLEND_INVSRCCOLOR && !mCaps.mDestBlendCaps.mInvSrcColor,
                       "DestBlend: ABLEND_INVSRCCOLOR not supported");
            RS_PREVENT(mode == ABLEND_SRCALPHA && !mCaps.mDestBlendCaps.mSrcAlpha,
                       "DestBlend: ABLEND_SRCALPHA not supported");
            RS_PREVENT(mode == ABLEND_INVSRCALPHA && !mCaps.mDestBlendCaps.mInvSrcAlpha,
                       "DestBlend: ABLEND_INVSRCALPHA not supported");
            RS_PREVENT(mode == ABLEND_DESTALPHA && !mCaps.mDestBlendCaps.mDestAlpha,
                       "DestBlend: ABLEND_DESTALPHA not supported");
            RS_PREVENT(mode == ABLEND_INVDESTALPHA && !mCaps.mDestBlendCaps.mInvDestAlpha,
                       "DestBlend: ABLEND_INVDESTALPHA not supported");
            RS_PREVENT(mode == ABLEND_DESTCOLOR && !mCaps.mDestBlendCaps.mDestColor,
                       "DestBlend: ABLEND_DESTCOLOR not supported");
            RS_PREVENT(mode == ABLEND_INVDESTCOLOR && !mCaps.mDestBlendCaps.mInvDestColor,
                       "DestBlend: ABLEND_INVDESTCOLOR not supported");
            RS_PREVENT(mode == ABLEND_SRCALPHASAT && !mCaps.mDestBlendCaps.mSrcAlphaSat,
                       "DestBlend: ABLEND_SRCALPHASAT not supported");
            RS_PREVENT(mode == ABLEND_BOTHSRCALPHA && !mCaps.mDestBlendCaps.mBothSrcAlpha,
                       "DestBlend: ABLEND_BOTHSRCALPHA not supported");
            RS_PREVENT(mode == ABLEND_BOTHINVSRCALPHA && !mCaps.mDestBlendCaps.mBothInvSrcAlpha,
                       "DestBlend: ABLEND_BOTHINVSRCALPHA not supported");
        }
#endif
        PL_CHECK(plevel, RS_DESTBLEND);
        DRS_CHECK(udword(value), DRS_DESTBLEND);
        return TRUE;
    }

    DECLARE_DRS(DiffuseMaterialSource, MCS, MCS_COLOR1) {
        PL_CHECK(plevel, RS_DIFFUSEMATSRC);
        DRS_CHECK(udword(value), DRS_DIFFUSEMATSRC);
        return TRUE;
    }

    DECLARE_DRS(SpecularMaterialSource, MCS, MCS_COLOR2) {
        PL_CHECK(plevel, RS_SPECULARMATSRC);
        DRS_CHECK(udword(value), DRS_SPECULARMATSRC);
        return TRUE;
    }

    DECLARE_DRS(AmbientMaterialSource, MCS, MCS_COLOR2) {
        PL_CHECK(plevel, RS_AMBIENTMATSRC);
        DRS_CHECK(udword(value), DRS_AMBIENTMATSRC);
        return TRUE;
    }

    DECLARE_DRS(EmissiveMaterialSource, MCS, MCS_MATERIAL) {
        PL_CHECK(plevel, RS_EMISSIVEMATSRC);
        DRS_CHECK(udword(value), DRS_EMISSIVEMATSRC);
        return TRUE;
    }

    DECLARE_DRS(VertexBlend, VBLEND, VBLEND_DISABLE) {
        PL_CHECK(plevel, RS_VERTEXBLEND);
        DRS_CHECK(udword(value), DRS_VERTEXBLEND);
        return TRUE;
    }

    DECLARE_DRS(TextureFactor, udword, 0) {
        PL_CHECK(plevel, RS_TEXTUREFACTOR);
        DRS_CHECK(value, DRS_TEXTUREFACTOR);
        return TRUE;
    }

    DECLARE_DRS(FillMode, FILLMODE, FILL_SOLID) {
        PL_CHECK(plevel, RS_FILLMODE);
        DRS_CHECK(udword(value), DRS_FILLMODE);
        return TRUE;
    }

    DECLARE_DRS(ShadeMode, SHADEMODE, SHADE_GOURAUD) {
        PL_CHECK(plevel, RS_SHADEMODE);
        DRS_CHECK(udword(value), DRS_SHADEMODE);
        return TRUE;
    }
...
#endif
        PL_CHECK(plevel, RS_CULLMODE);
        DRS_CHECK(udword(value), DRS_CULLMODE);
        return TRUE;
    }

    DECLARE_DRS(StencilRef, udword, 0) {
        PL_CHECK(plevel, RS_STENCILREF);
        DRS_CHECK(value, DRS_STENCILREF);
        return TRUE;
    }
    ...
    DECLARE_DRS(StencilCmpMode, CMPFUNC, CMP_ALWAYS) {
        /*	if(mCheckings)
                {
                        RS_PREVENT(cf==CMP_ALWAYS		&& !mCaps.AlphaCmpCaps_Always,
           "StencilFunc: CMP_ALWAYS not supported"); RS_PREVENT(cf==CMP_EQUAL		&& !mCaps.AlphaCmpCaps_Equal,
           "StencilFunc: CMP_EQUAL not supported"); RS_PREVENT(cf==CMP_GREATER		&& !mCaps.AlphaCmpCaps_Greater,
           "StencilFunc: CMP_GREATER not supported"); RS_PREVENT(cf==CMP_GREATEREQUAL	&&
           !mCaps.AlphaCmpCaps_GreaterEqual,	"StencilFunc: CMP_GREATEREQUAL not supported"); RS_PREVENT(cf==CMP_LESS
           && !mCaps.AlphaCmpCaps_Less,			"StencilFunc: CMP_LESS not supported");
                        RS_PREVENT(cf==CMP_LESSEQUAL	&& !mCaps.AlphaCmpCaps_LessEqual,
           "StencilFunc: CMP_LESSEQUAL not supported"); RS_PREVENT(cf==CMP_NEVER		&&
           !mCaps.AlphaCmpCaps_Never,			"StencilFunc: CMP_NEVER not supported");
                        RS_PREVENT(cf==CMP_NOTEQUAL		&& !mCaps.AlphaCmpCaps_NotEqual,
           "StencilFunc: CMP_NOTEQUAL not supported");
                }*/
        PL_CHECK(plevel, RS_STENCILCMPMODE);
        DRS_CHECK(udword(value), DRS_STENCILCMPMODE);
        return TRUE;
    }

    DECLARE_DRS(StencilCmpMask, udword, 0xffffffff) {
        PL_CHECK(plevel, RS_STENCILCMPMASK);
        DRS_CHECK(udword(value), DRS_STENCILCMPMASK);
        return TRUE;
    }

    DECLARE_DRS(StencilWriteMask, udword, 0xffffffff) {
        PL_CHECK(plevel, RS_STENCILWRITEMASK);
        DRS_CHECK(udword(value), DRS_STENCILWRITEMASK);
        return TRUE;
    }
...
#ifdef RS_SUPPORT_RUNTIME_CHECKINGS
        if (mCheckings) {
            RS_PREVENT(sop == STENCILOP_KEEP && !mCaps.mStencilOp.mKeep,
                       "StencilFailOp: STENCILOP_KEEP not supported!");
            RS_PREVENT(sop == STENCILOP_ZERO && !mCaps.mStencilOp.mZero,
                       "StencilFailOp: STENCILOP_ZERO not supported!");
            RS_PREVENT(sop == STENCILOP_REPLACE && !mCaps.mStencilOp.mReplace,
                       "StencilFailOp: STENCILOP_REPLACE not supported!");
            RS_PREVENT(sop == STENCILOP_INCRSAT && !mCaps.mStencilOp.mIncrSat,
                       "StencilFailOp: STENCILOP_INCRSAT not supported!");
            RS_PREVENT(sop == STENCILOP_DECRSAT && !mCaps.mStencilOp.mDecrSat,
                       "StencilFailOp: STENCILOP_DECRSAT not supported!");
            RS_PREVENT(sop == STENCILOP_INVERT && !mCaps.mStencilOp.mInvert,
                       "StencilFailOp: STENCILOP_INVERT not supported!");
            RS_PREVENT(sop == STENCILOP_INCR && !mCaps.mStencilOp.mIncr,
                       "StencilFailOp: STENCILOP_INCR not supported!");
            RS_PREVENT(sop == STENCILOP_DECR && !mCaps.mStencilOp.mDecr,
                       "StencilFailOp: STENCILOP_DECR not supported!");
        }
#endif
        PL_CHECK(plevel, RS_STENCILFAILOP);
        DRS_CHECK(udword(value), DRS_STENCILFAILOP);
        return TRUE;
    }
    ...
    DECLARE_DRS(StencilZFailOp, STENCILOP, STENCILOP_KEEP) {
        ...
        if (mCheckings) {
            RS_PREVENT(sop == STENCILOP_KEEP && !mCaps.mStencilOp.mKeep,
                       "StencilZFailOp: STENCILOP_KEEP not supported!");
            RS_PREVENT(sop == STENCILOP_ZERO && !mCaps.mStencilOp.mZero,
                       "StencilZFailOp: STENCILOP_ZERO not supported!");
            RS_PREVENT(sop == STENCILOP_REPLACE && !mCaps.mStencilOp.mReplace,
                       "StencilZFailOp: STENCILOP_REPLACE not supported!");
            RS_PREVENT(sop == STENCILOP_INCRSAT && !mCaps.mStencilOp.mIncrSat,
                       "StencilZFailOp: STENCILOP_INCRSAT not supported!");
            RS_PREVENT(sop == STENCILOP_DECRSAT && !mCaps.mStencilOp.mDecrSat,
                       "StencilZFailOp: STENCILOP_DECRSAT not supported!");
            RS_PREVENT(sop == STENCILOP_INVERT && !mCaps.mStencilOp.mInvert,
                       "StencilZFailOp: STENCILOP_INVERT not supported!");
            RS_PREVENT(sop == STENCILOP_INCR && !mCaps.mStencilOp.mIncr,
                       "StencilZFailOp: STENCILOP_INCR not supported!");
            RS_PREVENT(sop == STENCILOP_DECR && !mCaps.mStencilOp.mDecr,
                       "StencilZFailOp: STENCILOP_DECR not supported!");
        }
#endif
        PL_CHECK(plevel, RS_STENCILZFAILOP);
        DRS_CHECK(udword(value), DRS_STENCILZFAILOP);
        return TRUE;
    }
    ...
    DECLARE_DRS(StencilPassOp, STENCILOP, STENCILOP_KEEP) {
        ...
        if (mCheckings) {
            RS_PREVENT(sop == STENCILOP_KEEP && !mCaps.mStencilOp.mKeep,
                       "StencilPassOp: STENCILOP_KEEP not supported!");
            RS_PREVENT(sop == STENCILOP_ZERO && !mCaps.mStencilOp.mZero,
                       "StencilPassOp: STENCILOP_ZERO not supported!");
            RS_PREVENT(sop == STENCILOP_REPLACE && !mCaps.mStencilOp.mReplace,
                       "StencilPassOp: STENCILOP_REPLACE not supported!");
            RS_PREVENT(sop == STENCILOP_INCRSAT && !mCaps.mStencilOp.mIncrSat,
                       "StencilPassOp: STENCILOP_INCRSAT not supported!");
            RS_PREVENT(sop == STENCILOP_DECRSAT && !mCaps.mStencilOp.mDecrSat,
                       "StencilPassOp: STENCILOP_DECRSAT not supported!");
            RS_PREVENT(sop == STENCILOP_INVERT && !mCaps.mStencilOp.mInvert,
                       "StencilPassOp: STENCILOP_INVERT not supported!");
            RS_PREVENT(sop == STENCILOP_INCR && !mCaps.mStencilOp.mIncr,
                       "StencilPassOp: STENCILOP_INCR not supported!");
            RS_PREVENT(sop == STENCILOP_DECR && !mCaps.mStencilOp.mDecr,
                       "StencilPassOp: STENCILOP_DECR not supported!");
        }
#endif
        PL_CHECK(plevel, RS_STENCILPASSOP);
        DRS_CHECK(udword(value), DRS_STENCILPASSOP);
        return TRUE;
    }

    DECLARE_DRS(FogVertexMode, FOGMODE, FOGMODE_NONE) {
        PL_CHECK(plevel, RS_FOGVERTEXMODE);
        DRS_CHECK(udword(value), DRS_FOGVERTEXMODE);
        return TRUE;
    }

    DECLARE_DRS(FogMode, FOGMODE, FOGMODE_NONE) {
        PL_CHECK(plevel, RS_FOGMODE);
        DRS_CHECK(udword(value), DRS_FOGMODE);
        return TRUE;
    }

    DECLARE_DRS(FogDensity, float, 0.0f) {
        PL_CHECK(plevel, RS_FOGDENSITY);
        DRS_CHECK(*(udword*)&value, DRS_FOGDENSITY);
        return TRUE;
    }

    DECLARE_DRS(FogColor, udword, 0) {
        PL_CHECK(plevel, RS_FOGCOLOR);
        DRS_CHECK(value, DRS_FOGCOLOR);
        return TRUE;
    }

    DECLARE_DRS(FogStart, float, 0.0f) {
        PL_CHECK(plevel, RS_FOGSTART);
        DRS_CHECK(*(udword*)&value, DRS_FOGSTART);
        return TRUE;
    }

    DECLARE_DRS(FogEnd, float, 0.0f) {
        PL_CHECK(plevel, RS_FOGEND);
        DRS_CHECK(*(udword*)&value, DRS_FOGEND);
        return TRUE;
    }

    DECLARE_DRS(MaterialProps, const MaterialProps*, null) {
        // WARNING: cache must be invalidated when material properties are dynamic....
        PL_CHECK(plevel, RS_MATERIAL);
        DRS_CHECK(udword(value), DRS_MATERIAL);
        return TRUE;
    }

    DECLARE_DRS(PointSize, float, 1.0f) {
        PL_CHECK(plevel, RS_POINTSIZE);
        DRS_CHECK(*(udword*)&value, DRS_POINTSIZE);
        return TRUE;
    }

    DECLARE_DRS(PointSizeMin, float, 1.0f) {
        PL_CHECK(plevel, RS_POINTSIZEMIN);
        DRS_CHECK(*(udword*)&value, DRS_POINTSIZEMIN);
        return TRUE;
    }

    DECLARE_DRS(PointScaleA, float, 1.0f) {
        PL_CHECK(plevel, RS_POINTSCALEA);
        DRS_CHECK(*(udword*)&value, DRS_POINTSCALEA);
        return TRUE;
    }

    DECLARE_DRS(PointScaleB, float, 0.0f) {
        PL_CHECK(plevel, RS_POINTSCALEB);
        DRS_CHECK(*(udword*)&value, DRS_POINTSCALEB);
        return TRUE;
    }

    DECLARE_DRS(PointScaleC, float, 0.0f) {
        PL_CHECK(plevel, RS_POINTSCALEC);
        DRS_CHECK(*(udword*)&value, DRS_POINTSCALEC);
        return TRUE;
    }

    DECLARE_DRS(MultiSampleMask, udword, 0xffffffff) {
        PL_CHECK(plevel, RS_MULTISAMPLEMASK);
        DRS_CHECK(value, DRS_MULTISAMPLEMASK);
        return TRUE;
    }

    DECLARE_DRS(PatchEdgeStyle, PATCHEDGESTYLE, PES_DISCRETE) {
        PL_CHECK(plevel, RS_PATCHEDGESTYLE);
        DRS_CHECK(udword(value), DRS_PATCHEDGESTYLE);
        return TRUE;
    }

    DECLARE_DRS(PatchSegments, float, 0.0f) {
        PL_CHECK(plevel, RS_PATCHSEGMENTS);
        DRS_CHECK(*(udword*)&value, DRS_PATCHSEGMENTS);
        return TRUE;
    }

    DECLARE_DRS(PointSizeMax, float, 1.0f) {
        PL_CHECK(plevel, RS_POINTSIZEMAX);
        DRS_CHECK(*(udword*)&value, DRS_POINTSIZEMAX);
        return TRUE;
    }

    DECLARE_DRS(ColorWriteEnable, udword, CWE_ALL) {
        RS_PREVENT(!mCaps.mMiscCaps.mColorWriteEnable, "Color-write-enable not supported");
        PL_CHECK(plevel, RS_COLORWRITEENABLE);
        DRS_CHECK(value, DRS_COLORWRITEENABLE);
        return TRUE;
    }

    DECLARE_DRS(TweenFactor, float, 0.0f) {
        PL_CHECK(plevel, RS_TWEENFACTOR);
        DRS_CHECK(*(udword*)&value, DRS_TWEENFACTOR);
        return TRUE;
    }

    DECLARE_DRS(BlendOp, BLENDOP, BLENDOP_ADD) {
        PL_CHECK(plevel, RS_BLENDOP);
        DRS_CHECK(udword(value), DRS_BLENDOP);
        return TRUE;
    }

    DECLARE_DRS(VertexShader, VertexShader*, null) {
        PL_CHECK(plevel, RS_VERTEXSHADER);
        DRS_CHECK(udword(value), DRS_VERTEXSHADER);
        return TRUE;
    }

    DECLARE_DRS(PixelShader, PixelShader*, null) {
        PL_CHECK(plevel, RS_PIXELSHADER);
        DRS_CHECK(udword(value), DRS_PIXELSHADER);
        return TRUE;
    }

    DECLARE_DRS(VertexDeclaration, VertexDeclaration*, null) {
        PL_CHECK(plevel, RS_VERTEXDECLARATION);
        DRS_CHECK(udword(value), DRS_VERTEXDECLARATION);
        return TRUE;
    }
...
#define DECLARE_TRS(name, type) virtual inline_ BOOL Set##name(udword i, type value)
    ...
    DECLARE_TRS(TextureColorOp, STAGEOP) {
        ...
        TPL_CHECK(plevel, mColorOp);
        TS_CHECK(value, mColorOp);
        return TRUE;
    }

    DECLARE_TRS(TextureColorArg1, STAGEARG) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mColorArg1);
        TS_CHECK(value, mColorArg1);
        return TRUE;
    }

    DECLARE_TRS(TextureColorArg2, STAGEARG) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mColorArg2);
        TS_CHECK(value, mColorArg2);
        return TRUE;
    }

    DECLARE_TRS(TextureAlphaOp, STAGEOP) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mAlphaOp);
        TS_CHECK(value, mAlphaOp);
        return TRUE;
    }

    DECLARE_TRS(TextureAlphaArg1, STAGEARG) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mAlphaArg1);
        TS_CHECK(value, mAlphaArg1);
        return TRUE;
    }

    DECLARE_TRS(TextureAlphaArg2, STAGEARG) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mAlphaArg2);
        TS_CHECK(value, mAlphaArg2);
        return TRUE;
    }

    DECLARE_TRS(TextureMinFilter, TEXTUREMINFILTER) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mMinFilter);
        TS_CHECK(value, mMinFilter);
        return TRUE;
    }

    DECLARE_TRS(TextureMagFilter, TEXTUREMAGFILTER) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mMagFilter);
        TS_CHECK(value, mMagFilter);
        return TRUE;
    }

    DECLARE_TRS(TextureMipFilter, TEXTUREMIPFILTER) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mMipFilter);
        TS_CHECK(value, mMipFilter);
        return TRUE;
    }

    DECLARE_TRS(TextureCoordIndex, STAGETEXCOORDINDEX) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mCoordIndex);
        TS_CHECK(value, mCoordIndex);
        return TRUE;
    }

    DECLARE_TRS(TextureTransFlags, TEXTURETRANSFLAGS) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mTransflags);
        TS_CHECK(value, mTransFlags);
        return TRUE;
    }

    DECLARE_TRS(TextureMipMapLODBias, float) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mMipMapLODBias);
        TS_CHECK(value, mMipMapLODBias);
        return TRUE;
    }

    DECLARE_TRS(TextureMaxMipMapLevel, udword) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mMaxMipMapLevel);
        TS_CHECK(value, mMaxMipMapLevel);
        return TRUE;
    }

    DECLARE_TRS(TextureMaxAnisotropy, udword) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mMaxAnisotropy);
        TS_CHECK(value, mMaxAnisotropy);
        return TRUE;
    }

    DECLARE_TRS(TextureAddressU, TEXTUREADDRESS) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mAddressU);
        TS_CHECK(value, mAddressU);
        return TRUE;
    }

    DECLARE_TRS(TextureAddressV, TEXTUREADDRESS) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mAddressV);
        TS_CHECK(value, mAddressV);
        return TRUE;
    }

    DECLARE_TRS(TextureBorder, udword) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mBorder);
        TS_CHECK(value, mBorder);
        return TRUE;
    }

    DECLARE_TRS(TextureWrapU, BOOL) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mWrapU);
        TS_CHECK(value, mWrapU);
        return TRUE;
    }

    DECLARE_TRS(TextureWrapV, BOOL) {
        RS_PREVENT(i >= mCaps.mMaxTextureBlendStages, "Texture stage index exceeds device caps");
        TPL_CHECK(plevel, mWrapV);
        TS_CHECK(value, mWrapV);
        return TRUE;
    }
...
#define DISABLE_ZBUFFER(rd)                         \
    udword __OldZBMode = rd->mRS->GetZBufferMode(); \
    rd->mRS->SetZBufferMode(ZB_FALSE);
#define ENABLE_ZBUFFER(rd) rd->mRS->SetZBufferMode((ZBUFFERMODE)__OldZBMode);

#define DISABLE_CULLING(rd)                        \
    udword __OldCullMode = rd->mRS->GetCullMode(); \
    rd->mRS->SetCullMode(CULL_NONE);
