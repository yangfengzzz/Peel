...
// Recordable:								Recordable ID						Stage number
// State / recorded
//	struct ICERENDERER_API BoolRS				{ BooleanRenderState	brs;		/* Undefined */
//udword		state;		};
//	struct ICERENDERER_API DwordRS				{ DwordRenderState		drs;		/* Undefined
//*/		udword		state;		};
//	struct ICERENDERER_API TextureRS			{ TextureRenderState	trs;		udword	stage;
//udword		state;		}; 	struct ICERENDERER_API TextureTransformRS	{ /* Implicit ID*/
//udword	stage;		Matrix4x4	transform;	};
...
#define SB_TTRS_SIZE (sizeof(TextureTransformRS) / sizeof(udword))
    ...
    void AddRenderState(const TextureTransformRS& rs) {
        Add(udword(rs.stage));
        AddMatrix(rs.transform);
    }
};
...
private:
    ...
    udword mRSFlagsValuesMask;               //!< Bitmask: gives the wanted values for render states selected by
                                             //!< mRSFlagsSelectionMask.
                                             // Containers
