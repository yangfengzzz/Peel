...
public:
    ...
    inline_ bool DrawSubset(udword i) const {
        ...
        //											return mRd->DrawIndexedPrimitive(mVB, mDesc,
        //mIB, i);
    ...
    /*		inline_	bool					DrawSubset2(udword i)	const
       ...
       GetRenderInfos(); IndexBuffer* IB = GetIndexBuffer(); VertexBuffer* VB = GetVertexBuffer();	// ### warning,
       creates a VB anyway

                                                                                            // Render from the system
       memory copy uword* VRefs = (uword*)IB->GetIndices(); Subset* SS = IB->GetSubset(i); return
       mRd->DrawIndexedPrimitive( IB->GetPrimType(), VB->GetVertexFormat(), Data->GetVBCopy() +
       VB->GetVertexSize()*SS->mMinIndex, Data->GetNbVerts() - SS->mMinIndex, &VRefs[SS->mOffset], SS->mCount, 0);
                                                                                    }*/
    ...
    inline_ bool SelectStrips(bool flag) {
        mUseStrips = flag;
