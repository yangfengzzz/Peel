...
enum BooleanRenderState {
    // Standard set
    BRS_TEXTUREPERSPECTIVE = (1 << 0),
    BRS_ZWRITE = (1 << 1),
    BRS_ALPHATEST = (1 << 2),
    BRS_DITHERING = (1 << 3),
    BRS_ALPHABLENDING = (1 << 4),
    BRS_FOG = (1 << 5),
    BRS_SPECULAR = (1 << 6),
    BRS_STENCILTEST = (1 << 7),
    BRS_LIGHTING = (1 << 8),
    BRS_TEXTURING = (1 << 9),
    BRS_LASTPIXEL = (1 << 10),
    BRS_STIPPLEDALPHA = (1 << 11),
    BRS_EDGEANTIALIAS = (1 << 12),
    BRS_COLORKEYMODE = (1 << 13),
    BRS_COLORKEYBLENDMODE = (1 << 14),
    BRS_RANGEBASEDFOG = (1 << 15),
    BRS_CLIPPING = (1 << 16),
    BRS_SCREENEXTENTS = (1 << 17),
    BRS_COLORVERTEX = (1 << 18),
    BRS_PERSPSPECHIGH = (1 << 19),
    BRS_NORMALIZE = (1 << 20),
    // New set
    BRS_SOFTWAREVERTEXPROCESSING = (1 << 21),
    BRS_POINTSPRITE = (1 << 22),
    BRS_POINTSCALE = (1 << 23),
    BRS_MULTISAMPLEANTIALIAS = (1 << 24),
    BRS_INDEXEDVERTEXBLEND = (1 << 25),

    BRS_FORCE_DWORD = 0x7fffffff
};

inline_ udword GetBRSIndex(BooleanRenderState brs) { return CodeSize2(udword(brs)) - 1; }
...
enum DwordRenderState {
    ...
    DRS_SRCBLEND = 8,
    DRS_DESTBLEND = 9,
    DRS_DIFFUSEMATSRC = 10,
    DRS_SPECULARMATSRC = 11,
    DRS_AMBIENTMATSRC = 12,
    DRS_EMISSIVEMATSRC = 13,
    DRS_VERTEXBLEND = 14,
    DRS_TEXTUREFACTOR = 15,
    DRS_FILLMODE = 16,
    DRS_SHADEMODE = 17,
    DRS_CULLMODE = 18,
    DRS_STENCILREF = 19,
    DRS_STENCILCMPMODE = 20,
    DRS_STENCILCMPMASK = 21,
    DRS_STENCILWRITEMASK = 22,
    DRS_STENCILFAILOP = 23,
    DRS_STENCILZFAILOP = 24,
    DRS_STENCILPASSOP = 25,
    DRS_FOGVERTEXMODE = 26,
    DRS_FOGMODE = 27,
    DRS_FOGDENSITY = 28,
    DRS_FOGCOLOR = 29,
    DRS_FOGSTART = 30,
    ...
    DRS_POINTSCALEB = 36,
    DRS_POINTSCALEC = 37,
    DRS_MULTISAMPLEMASK = 38,
    DRS_PATCHEDGESTYLE = 39,
    DRS_PATCHSEGMENTS = 40,
    // debug monitor token					41
    DRS_POINTSIZEMAX = 42,
    DRS_COLORWRITEENABLE = 43,
    DRS_TWEENFACTOR = 44,
    DRS_BLENDOP = 45,
    //
    DRS_VERTEXSHADER = 46,
...
enum RenderStateID {
    ...
    RS_DESTBLEND = 30,
    RS_DIFFUSEMATSRC = 31,
    RS_SPECULARMATSRC = 32,
    RS_AMBIENTMATSRC = 33,
    RS_EMISSIVEMATSRC = 34,
    RS_VERTEXBLEND = 35,
    RS_TEXTUREFACTOR = 36,
    RS_FILLMODE = 37,
    RS_SHADEMODE = 38,
    RS_CULLMODE = 39,
    RS_STENCILREF = 40,
    RS_STENCILCMPMODE = 41,
    RS_STENCILCMPMASK = 42,
    RS_STENCILWRITEMASK = 43,
    RS_STENCILFAILOP = 44,
    RS_STENCILZFAILOP = 45,
    RS_STENCILPASSOP = 46,
    RS_FOGVERTEXMODE = 47,
    RS_FOGMODE = 48,
    RS_FOGDENSITY = 49,
    ...
    RS_MATERIAL = 53,
    // New set
    RS_SOFTWAREVERTEXPROCESSING = 54,
    RS_POINTSPRITE = 55,
    RS_POINTSCALE = 56,
    RS_MULTISAMPLEANTIALIAS = 57,
    RS_INDEXEDVERTEXBLEND = 58,
    RS_POINTSIZE = 59,
    RS_POINTSIZEMIN = 60,
    RS_POINTSCALEA = 61,
    RS_POINTSCALEB = 62,
    RS_POINTSCALEC = 63,
    RS_MULTISAMPLEMASK = 64,
    RS_PATCHEDGESTYLE = 65,
    RS_PATCHSEGMENTS = 66,  // debug monitor token				= 67,
    RS_POINTSIZEMAX = 68,
    RS_COLORWRITEENABLE = 69,
    RS_TWEENFACTOR = 70,
    RS_BLENDOP = 71,
    //
    RS_VERTEXSHADER = 72,
...
enum TEXTUREADDRESS {
    ...
    TADDRESS_CLAMP = 3,   //!< Texture coordinates outside the range [0.0, 1.0] are set to the texture color at 0.0
                          //!< or 1.0, respectively.
    TADDRESS_BORDER = 4,  //!< Texture coordinates outside the range [0.0, 1.0] are set to the border color, which is a
                          //!< new render state.
    TADDRESS_MIRRORONCE =
            5,  //!< Similar to TADDRESS_MIRROR and TADDRESS_CLAMP. Takes the absolute value of the texture coordinate
                //!< (thus, mirroring around 0), and then clamps to the maximum value. The most common usage is for
                //!< volume textures, where support for the full TADDRESS_MIRRORONCE texture-addressing mode is not
...
enum TEXTUREMINFILTER {
    ...
    TFN_LINEAR = 2,  //!< Bilinear interpolation filtering. A weighted average of a 2�2 area of texels surrounding the
                     //!< desired pixel is used.
    TFN_ANISOTROPIC = 3,  //!< Anisotropic texture filtering. Compensates for distortion caused by the difference in
                          //!< angle between the texture polygon and the plane of the screen.
...
enum TEXTUREMAGFILTER {
    ...
    TFG_LINEAR = 2,  //!< Bilinear interpolation filtering. A weighted average of a 2�2 area of texels surrounding the
                     //!< desired pixel is used.
    TFG_FLATCUBIC = 3,      //!< Not currently supported; do not use.
    TFG_GAUSSIANCUBIC = 4,  //!< Not currently supported; do not use.
    TFG_ANISOTROPIC = 5,    //!< Anisotropic texture filtering. Compensates for distortion caused by the difference in
                            //!< angle between the texture polygon and the plane of the screen.
...
enum TEXTUREMIPFILTER {
    ...
    TFP_POINT = 2,   //!< Nearest-point mipmap filtering. The rasterizer uses the color from the texel of the nearest
                     //!< mipmap texture.
    TFP_LINEAR = 3,  //!< Trilinear mipmap interpolation. The rasterizer linearly interpolates pixel color, using the
                     //!< texels of the two nearest mipmap textures.
...
enum ANTIALIASMODE {
    ...
    ANTIALIAS_SORTDEPENDENT =
            1,  //!< Antialiasing is dependent on the sort order of the polygons (back to front or front to back).
...
//! Shade modes
enum SHADEMODE {
    SHADE_FLAT = 1,  //!< Flat shading mode. The color and specular component of the first vertex in the triangle are
                     //!< used to determine the color and specular component of the face. These colors remain constant
                     //!< across the triangle; that is, they are not interpolated.
    SHADE_GOURAUD = 2,  //!< Gouraud shading mode. The color and specular components of the face are determined by a
...
enum CMPFUNC {
    ...
    CMP_GREATEREQUAL =
            7,       //!< Accept the new pixel if its value is greater than or equal to the value of the current pixel.
...
enum STENCILOP {
    ...
    STENCILOP_INCR =
            7,  //!< Increment the stencil-buffer entry, wrapping to 0 if the new value exceeds the maximum value.
    STENCILOP_DECR =
            8,  //!< Decrement the stencil-buffer entry, wrapping to the maximum value if the new value is less than 0.
...
enum ALPHABLEND {
    ...
    ABLEND_BOTHINVSRCALPHA = 13,  //!< Source blend factor is (1-As, 1-As, 1-As, 1-As), and destination blend factor is
...
enum STAGEOP {
    // Control members
    STAGEOP_DISABLE = 1,  //!< Disables output from this texture stage and all stages with a higher index. To disable
                          //!< texture mapping, set this as the color operation for the first texture stage (stage 0).
                          //!< Alpha operations cannot be disabled when color operations are enabled. Setting the alpha
                          //!< operation to D3DTOP_DISABLE when color blending is enabled causes undefined behavior.
    ...
    STAGEOP_BUMPENVMAP = 22,  //!< Perform per-pixel bump mapping, using the environment map in the next texture stage
                              //!< (without luminance).
    STAGEOP_BUMPENVMAPLUMINANCE = 23,  //!< Perform per-pixel bump mapping, using the environment map in the next
                                       //!< texture stage (with luminance).
...
enum STAGEARG {
    // Argument flags
    STAGEARG_DIFFUSE =
            0,  //!< The texture argument is the diffuse color interpolated from vertex components during Gouraud
                //!< shading. If the vertex does not contain a diffuse color, the default color is 0xFFFFFFFF.
    STAGEARG_CURRENT = 1,  //!< The texture argument is the result of the previous blending stage. In the first texture
                           //!< stage (stage 0), this argument is equivalent to STAGEARG_DIFFUSE. If the previous
                           //!< blending stage uses a bump-map texture (the STAGEOP_BUMPENVMAP operation), the system
                           //!< chooses the texture from the stage before the bump-map texture.
    STAGEARG_TEXTURE = 2,  //!< The texture argument is the texture color for this texture stage. This is valid only for
                           //!< the first color and alpha arguments in a stage. (ARG1)
    STAGEARG_TFACTOR = 3,  //!< The texture argument is the texture factor set in a previous call to SetTextureFactor().
    STAGEARG_SPECULAR =
            4,  //!< The texture argument is the specular color interpolated from vertex components during Gouraud
                //!< shading. If the vertex does not contain a diffuse color, the default color is 0xFFFFFFFF.
    STAGEARG_TEMP = 5,  //!< The texture argument is a temporary register color for read or write. D3DTA_TEMP is
                        //!< supported if the D3DPMISCCAPS_TSSARGTEMP device capability is present. The default value
                        //!< for the register is (0.0, 0.0, 0.0, 0.0). Permissions are read/write.
    STAGEARG_SELECTMASK = 0xf,  //!< Mask value for all arguments; not used when setting texture arguments.
                                // Modifier flags
    STAGEARG_COMPLEMENT = 0x10,  //!< Invert the argument so that, if the result of the argument were referred to by the
                                 //!< variable x, the value would be 1.0 - x.
    STAGEARG_ALPHAREPLICATE =
            0x20,  //!< Replicate the alpha information to all color channels before the operation completes.
...
enum STAGETEXCOORDINDEX {
    ...
    STAGETCI_CAMERASPACENORMAL = 0x00010000,  //!< Use the vertex normal, transformed to camera space, as the input
                                              //!< texture coordinates for this stage's texture transformation.
    STAGETCI_CAMERASPACEPOSITION = 0x00020000,  //!< Use the vertex position, transformed to camera space, as the input
                                                //!< texture coordinates for this stage's texture transformation.
    STAGETCI_CAMERASPACEREFLECTIONVECTOR =
            0x00030000,  //!< Use the reflection vector, transformed to camera space, as the input texture coordinate
                         //!< for this stage's texture transformation. The reflection vector is computed from the input
...
enum TEXTURETRANSFLAGS {
    ...
    TTFF_PROJECTED = 256,  //!< The texture coordinates are all divided by the last element before being passed to the
                           //!< rasterizer.
...
//! Patch edge styles
enum PATCHEDGESTYLE {
    PES_DISCRETE = 0,    //!< Discrete edge style. In discrete mode, you can specify float tessellation but it will be
                         //!< truncated to integers.
    PES_CONTINUOUS = 1,  //!< Continuous edge style. In continuous mode, tessellation is specified as float values which
                         //!< can be smoothly varied to reduce "popping" artifacts.
