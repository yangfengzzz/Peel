...
struct ICERENDERER_API TEXTURECREATE {
    ...
    udword mStage;   //!< Texture stage
                     //!< Caution: the mStage member doesn't matter on a lot of HW, but needs to be set explicitly on HW
                    //!< where textures are stored in separate memory spaces depending on stage (3Dfx  Voodoo 2)for
                    //!< example)
    // udword		mMipmapLevel;
...
FUNCTION ICERENDERER_API bool Blit(
        const Picture& source, ubyte* dest, udword width, udword height, udword pitch, TextureFormat tf);
