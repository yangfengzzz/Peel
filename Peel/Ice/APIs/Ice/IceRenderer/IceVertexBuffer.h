...
/*	//! Vertex Buffer status flags
        enum VBStatusFlag
        {
                VBSFLAG_OPTIMIZED	= (1<<0),		//!< The vertex buffer has been optimized [mostly
   obsolete]
...
struct ICERENDERER_API VBDescriptor {
    ...
    udword mNbVertices;  //!< The maximum number of vertices that this vertex buffer can contain. The maximum number of
                         //!< vertices allowed is 0xFFFF.
};
...
private:
    uword mVI;  //!< Virtual index = buffer identifier
