...
//																Vertex		1/w
//Normal		Diffuse			Specular		Texture0		Texture1
//! Point + Normal + UV
struct ICERENDERER_API PNT_Vertex : public Allocateable {
    Point p;
    Point n;
    float u, v;
};
//! Point + diffuse color
struct ICERENDERER_API PD_Vertex : public Allocateable {
    Point p;
    udword color;
};
//! Point + diffuse color + UV
struct ICERENDERER_API PDT_Vertex : public Allocateable {
    Point p;
    udword color;
    float u, v;
};
//! Point + normal
struct ICERENDERER_API PN_Vertex : public Allocateable {
    Point p;
    Point n;
};
//! Point + UV
struct ICERENDERER_API PT_Vertex : public Allocateable {
    Point p;
    float u, v;
};
//! Vertex-color vertex
struct ICERENDERER_API PNDT_Vertex : public Allocateable {
    Point p;
    Point n;
    udword color;
    float u, v;
...
//! Transformed vertex
struct ICERENDERER_API PRT_Vertex : public Allocateable {
    Point p;
    float rhw;
    float u, v;
};
//!
struct ICERENDERER_API PRD_Vertex : public Allocateable {
    Point p;
    float rhw;
    udword color;
};
//!
struct ICERENDERER_API PRDT_Vertex : public Allocateable {
    Point p;
    float rhw;
    udword color;
    float u, v;
...
//! Standard TL vertex
struct ICERENDERER_API PRDST_Vertex : public Allocateable {
    Point p;
    float rhw;
    udword color;
    float specular;
...
//!
struct ICERENDERER_API PRDTT_Vertex : public Allocateable {
    Point p;
    float rhw;
    udword color;
    float u0, v0;
...
enum VBFormatFlag {
    ...
    VF_PSIZE = 0x020,   //!< Vertex format specified in point size. This size is expressed in camera space units for
                       //!< vertices that are not transformed and lit, and in device-space units for transformed and lit
                       //!< vertices.
