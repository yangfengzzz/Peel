...
public:
    ...
    bool Init(udword nb_faces,
              udword nb_verts,
              const Point* verts = null,
              const IndexedTriangle* faces = null,
              bool zero_vertices = true);
    ...
    virtual udword ComputeConvexSupportingVertex(const Point& dir,
                                                 udword* id = null,
                                                 udword* time = null,
                                                 udword* stamps = null);
...
//				void					CompressVertex(udword link, udword prevface, udword
//prevedge, PackInfo& info);
//				void					DecompressVertex(udword prevface, udword prevedge,
//DepackInfo& info);
#endif
    // Surface interaction
    ...
    inline_ const Adjacencies* GetAdjacencies() const {
        if (!(mFlags & SF_ADJACENCY))
            const_cast<IndexedSurface* const>(this)->ComputeAdjacencies();  // "mutable method"
        return mAdj;
    }
    inline_ PolygonNormalMask* GetNormalMasks(udword nbsubdiv) const {
        if (!(mFlags & SF_NORMAL_MASKS))
            const_cast<IndexedSurface* const>(this)->ComputeNormalMasks(nbsubdiv);  // "mutable method"
        return mPNM;
    }
    inline_ const Point& GetLocalGeomCenter(bool method = true) const {
        if (!(mFlags & SF_GEOM_CENTER) || method != ((mFlags & SF_GEOM_CENTER_MODE) != 0))
            const_cast<IndexedSurface* const>(this)->ComputeLocalGeomCenter();  // "mutable method"
        return mLocalGeomCenter;
    }
    inline_ const Valencies* GetValencies(bool adjlist = true) const {
        if (!(mFlags & SF_VALENCIES) || adjlist != ((mFlags & SF_VALENCIES_MODE) != 0))
            const_cast<IndexedSurface* const>(this)->ComputeValencies();  // "mutable method"
        return mValencies;
    }
    ...
    // Interface
    inline_ SurfaceInterface GetSurfaceInterface() const {
        return SurfaceInterface(mNbVerts, mVerts, mNbFaces, (const udword*)mFaces, null);
    }
...
private:
    ...
    // Recursive methods
    bool _WeldFace(udword curface,
                   udword prevface,
                   udword crossededge,
                   udword prevedge,
                   Point* newverts,
                   IndexedTriangle* newfaces,
                   bool* markers);
};
