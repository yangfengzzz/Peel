...
/*
  ...
  - The edge-collapse cost formula comes from Stan Melax. Many other ones may be used, from the simplest (edge-length
  only) to the most difficult (Hoppe's energy function).

  - Many issues have been chosen to match DX7's vertex buffers easily. (No geomorphs for example) For better or for
  worse.

  - The PM is computed independently for each submesh of a topochannel, i.e. the way we use meshes in Irion *DO* have
  drawbacks in the algorithm. E.g. along a texture seam I could have collapsed two vertices the same way, not to create
  holes in th mesh: since this implies a relation from one topo to another, my solution was just to mark those frontier
  edges as uncollapsable. For better or for worse, indeed.
...
struct MESHMERIZER_API LOD_Vertex {
    ...
    ~LOD_Vertex() {
        DELETEARRAY(mSharingFaces);
        DELETEARRAY(mNeighbors);
    }
...
public:
    ...
    udword ComputeTopology(udword nbfaces,
                           uword* wfaces,
                           udword* dfaces,
                           udword minref,
                           udword finalnbvtx,
