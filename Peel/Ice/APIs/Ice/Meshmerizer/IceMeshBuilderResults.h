...
struct MESHMERIZER_API MBGeometry : public Allocateable {
    ...
    //
    udword NbVerts;  //!< Number of vertices in the final mesh (some may have been duplicated) = sum of all NbVerts in
                     //!< MBSubmeshes
    // Indices
    ...
    udword* ColorRefs;   //!< Vertex-color indices (only !=null if mIndexedColors, else colors are duplicated). Index in
                         //!< CVerts.
