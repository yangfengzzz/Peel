...
// Removes t-junctions from an input mesh. Does not generate any new data points, but may possible produce additional
// triangles and new indices.
FUNCTION MESHMERIZER_API void* CreateTJunctionsRemover();
FUNCTION MESHMERIZER_API void ReleaseTJunctionsRemover(void* ptr);
FUNCTION MESHMERIZER_API udword RemoveTJunctions(
        REMOVETJUNCTIONSCREATE& create,
        void* ptr);  // returns number of triangles output and the descriptor is filled with the appropriate results.
