...
struct MESHMERIZER_API SurfaceInterface {
    inline_ SurfaceInterface() : mNbVerts(0), mVerts(null), mNbFaces(0), mDFaces(null), mWFaces(null) {}

    inline_ SurfaceInterface(
            udword nb_verts, const Point* verts, udword nb_faces, const udword* dfaces, const uword* wfaces)
        : mNbVerts(nb_verts), mVerts(verts), mNbFaces(nb_faces), mDFaces(dfaces), mWFaces(wfaces) {}
