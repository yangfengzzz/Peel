...
/*
        #define	MCU_EDGE_0					1					// Bitmask for
   first edge
        #define	MCU_EDGE_1					2					// Bitmask for
   second edge
        #define	MCU_EDGE_2					4					// Bitmask for
   third edge
*/

#define MESHBUILDER_REF_NOT_DONE -1  //!< Default cross reference in RebuildVertexData()

enum VertexCode { VCODE_VERTEX = 0, VCODE_UVW = 1, VCODE_COLOR = 2, VCODE_FORCEDWORD = 0x7fffffff };
#ifdef OLD
...
struct MESHMERIZER_API MESHBUILDERCREATE {
    ...
    bool ComputeNormInfo;  //!< Compute normal-information table (used to recompute'm dynamically according to smoothing
                           //!< groups)
    ...
    bool RelativeIndices;  //!< Reset indices from one submesh to another, else use absolute indices
                           /*		bool			CreateStrips;						//!< Create triangle strips, else create triangle
                              lists
                                           bool			ForceSingleStrip;					//!< Create a degenerated single strip for each
                              submesh
                                           bool			CreateWireData;						//!< Create and export wire-strips with
                              edges-visibilities
                                           bool			ProgressiveMesh;					//!< Create and export progressive mesh
                              information                        CustomArray*	ArrayForLogText;					//!< User-array for log text if
                              needed*/
};
...
struct MESHMERIZER_API MESHBUILDERGEOMETRY {
    ...
    uword NbVerts;    //!< Number of vertices in the final mesh (some may have been duplicated) = sum of all NbVerts in
                      //!< MESHBUILDERSUBMESHes
    // Indices
...
struct MESHMERIZER_API MESHBUILDERMATERIALS {
    MESHBUILDERMATERIALS() { MaterialInfo = null; }
...
//		// Draw mesh
//		if(mStripRuns)	renderer->DrawIndexedPrimitiveVB(PRIMTYPE_TRISTRIP, mVB, 0, mResult->Geometry.NbVerts,
//mStripRuns[i], mStripLengths[i]);
//		else			renderer->DrawIndexedPrimitiveVB(PRIMTYPE_TRILIST, mVB, 0,
//mResult->Geometry.NbVerts, VRefs, mResult->Materials.MaterialInfo[i].NbFaces*3);
...
public:
    ...
    bool AddFace(sdword materialID,
                 udword smgroup,
                 ubyte edgevis,
                 udword* vrefs,
                 udword* trefs,
...
private:
    ...
    CustomArray mArraySubmeshProperties;  //!< Array to store submesh properties: material ID, smoothing groups,
                                          //!< #substrips, #vertex-data.
                                          //		CustomArray			mArrayWire;						//!< Array to store line
                                          //strips.
                                          //		CustomArray			mArrayProgMesh;					//!< Array to store progressive mesh
                                          //information
    ...
    udword mNewVertsIndex;     //!< TO BE DOCUMENTED
                               /*		udword				mNbEdges;						//!< Number of edges in the object/scene
                                  (computed)
                                               udword				mNbConnectedFaces;				//!< Number of faces in extracted
                                  submesh
                               */
    ...
    /*		bool				mValidateTopology;				//!< true if topology-related extraction must be performed, else false
       (default) bool				mValidateUVW;					//!< true if UVW-related
       extraction must be performed, else false (default) bool				mValidateRGB;
       //!< true if RGB-related extraction must be performed, else false (default) bool
       mValidateWire;					//!< true if wire-strips must be computed, else false (default)
                    bool				mCreateStrips;					//!< true if triangle strips must be computed,
       else false (default)
                    bool				mForceSingleStrip;				//!< true if a single strip must be created
       for each submesh, else false (default)
                    bool				mProgressiveMesh;				//!< true if progressive mesh information must
       be computed, else false (default)
    */
    ...
    MeshBuilder& StoreVertex(udword* crosslist,
                             udword ref,
                             udword grp,
                             udword fnb,
                             udword& newvertsindex,
                             udword id,
                             CustomArray* array);
};
#endif
