...
public:
    ...
    //! Copy constructor
    //		inline_			PR(const PRS& prs) : mPos(prs.mPos), mRot(prs.mRot)		{}	// Drop the
    //scale
    //! Destructor
    inline_ ~PR() {}
    ...
    //! Operator for PR += PR.
    inline_ PR& operator+=(const PR& p) {
        mPos += p.mPos;
        mRot *= p.mRot;
        return *this;
    }
    ...
    inline_ operator Matrix4x4() const { return Matrix4x4(Matrix3x3(mRot), mPos); }

    //! Cast a PR to a PRS.
    inline_ operator PRS() const { return PRS(mPos, mRot, Point(1.0f, 1.0f, 1.0f)); }
