...
/*
   ...
   {}
                ...
                // WARNING: access to the PR(S) is given through these methods, but you should NOT modify it directly
   unless you REALLY
                // know what you're doing. Lazy evaluation is heavily used with things like PR(S), and it should be
   updated thanks to the
                // appropriate (virtual) accessors.

                // The compulsory mPR member is present in all inherited classes, that's the static PR.
                inline_			const	PR*		GetStaticPR()
   const	{ return &mPR;									} inline_
   const	Point*	GetStaticPos()					const	{ return &mPR.mPos;
   }
                inline_			const	Quat*	GetStaticRot()
   const	{ return &mPR.mRot;								}

                // Otherwise the anonymous "GetPR" can be either static or dynamic, courtesy of the app. The app is
   responsible for
                // making the internal pointer points to the dynamic PR, stored in higher levels.
                inline_			const	PR*		GetPR()
   const	{ return mPRPtr;								} inline_
   const	Point*	GetPos()						const	{ return mPRPtr->mPos;
   }
                inline_			const	Quat*	GetRot()
   const	{ return mPRPtr->mRot;							}

                virtual	inline_	void	ResetPRS() { mPR.Reset();
   }
                //
                virtual inline_	void	SetPR(const PR& pr)						{ mPR
   = pr;								} virtual inline_	void	SetPos(const
   Point& pos)				{ mPR.mPos	= pos; } virtual inline_	void	SetRot(const Quat& rot)
   { mPR.mRot	= rot;								}
                //
                virtual inline_	void	AddPR(const PR& pr)						{ mPR
   += pr;								} virtual inline_	void	AddPos(const
   Point& pos)				{ mPR.mPos	+= pos; } virtual inline_	void	AddRot(const Quat& rot)
   { mPR.mRot	*= rot;								}
   ...
   try to keep it private
                ...
                inline_					void	SetDynamicPR(const PR* ptr)
   { mPRPtr = ptr;									} private: const
...
enum PRComponent { PRS_POSITION = (1 << 0), PRS_ROTATION = (1 << 1), PRS_SCALE = (1 << 2) };
...
public:
    ...
    // WARNING: access to the PR(S) is given through these methods, but you should NOT modify it directly unless you
    // REALLY know what you're doing. Lazy evaluation is heavily used with things like PR(S), and it should be updated
    // thanks to the appropriate (virtual) accessors.
    ...
    */
    ...
    /*
                    // Dynamic PR control
                    inline_			void	ResetPRS()						{ if(mPRSFlags&PRS_LOCKED) return;
       mPRPtr->Reset();		InvalidatePR();	}
                    //
                    inline_			void	SetPR(const PR& pr)				{ if(mPRSFlags&PRS_LOCKED) return;
       *mPRPtr			= pr;	InvalidatePR();	} inline_			void	SetPos(const Point& pos)
       { if(mPRSFlags&PRS_LOCKED) return;	mPRPtr->mPos	= pos;	InvalidatePos();}
                    inline_			void	SetRot(const Quat& rot)			{ if(mPRSFlags&PRS_LOCKED) return;
       mPRPtr->mRot	= rot;	InvalidateRot();}
                    //
                    inline_			void	AddPR(const PR& pr)				{ if(mPRSFlags&PRS_LOCKED) return;
       *mPRPtr			+= pr;	InvalidatePR();	} inline_			void	AddPos(const Point& pos)
       { if(mPRSFlags&PRS_LOCKED) return;	mPRPtr->mPos	+= pos;	InvalidatePos();}
                    inline_			void	AddRot(const Quat& rot)			{ if(mPRSFlags&PRS_LOCKED) return;
       mPRPtr->mRot	*= rot;	InvalidateRot();}
    */
    // Helpers
    bool SetNonMovable();
    ...
    PR mStaticPR;  //!< Static PR data ### try to keep it private
                   /*
                                   inline_			void	SetDynamicPR(PR* ptr)
                                                                   {
                                                                           if(mPRSFlags&PRS_LOCKED) return;	// "Locked" means
                      "static by all means"!                if(!ptr) return;					// Basic checkings
                                                                           // ### CMOV
                                                                           if(mPRPtr!=ptr)	mPRPtr = ptr;		// mPRPtr now points
                      the app-controlled dynamic PR
                                                                   }
                   */
                   //		inline_			void	SetStaticPR()					{ mPRPtr = &mStaticPR;	/* Resets the dynamic pointer */
                   //}
                   /*		private:
                                                                   PR*		mPRPtr;							//!< Pointer to static
                      or dynamic PR
                   */
};
...
/*	### TEMP. REMOVED, redesigning stuff
        ...
        class ICEMATHS_API PRSNode : public HNode
        {
                ...
                // Access to the PR(S) is given through these methods, but you shouldn't modify it directly unless you
   really know
                // what you're doing. Lazy evaluation is heavily used with things like PR(S), and it should be updated
   thanks to the
                // appropriate (virtual) methods.
                inline_	const PRS*		GetPRS()	const
   { return &mPRS;									} inline_	const PR*
   GetPR()		const						{ return (const PR*)&mPRS;
   } inline_	const Point*	GetPos()	const						{ return &mPRS.mPos;
   }
                inline_	const Quat*		GetRot()	const
   { return &mPRS.mRot;							} inline_	const Point*	GetScale()
   const						{ return &mPRS.mScale;
   }

                virtual	inline_	void	ResetPRS() { mPRS.Reset();
   }
                //
                virtual inline_	void	SetPRS(const PRS& prs)					{ mPRS
   = prs;							} virtual inline_	void	SetPR(const PR& pr)
   { mPRS.mPos		= pr.mPos; mPRS.mRot = pr.mRot;	} virtual inline_	void	SetPos(const Point& pos)
   { mPRS.mPos		= pos;							} virtual inline_	void
   SetRot(const Quat& rot)					{ mPRS.mRot		= rot;
   } virtual inline_	void	SetScale(const Point& scale)			{ mPRS.mScale	= scale;
   }
                //
                virtual inline_	void	AddPos(const Point& pos)				{ mPRS.mPos
   += pos;							}
...
public:
    //! Constructor
    LazyPRNode() : mValidAddress(1), mWorldMultiplier(null), mMatrixHistory(null) { mPRSTimeStamp = 0; /*hum*/ }

    //! Destructor
    virtual ~LazyPRNode() { ReleaseMatrixHistory(); }

    override(PRNode) inline_ void InvalidatePR(udword flags) {
        mValidAddress |= 1;
        mPRSTimeStamp++;
    }
     ...
     *	That way all immovable nodes share the same matrix pointer, so you can batch them together and save SetTransform
     *calls.
     *
    ...
    inline_ void SetWorldMultiplier(const Matrix4x4* multiplier) {
        mWorldMultiplier = multiplier;
        InvalidatePR(PRS_POSITION | PRS_ROTATION);
    }
    inline_ const Matrix4x4* GetWorldMultiplier() const { return mWorldMultiplier; }

    // Master matrix support

    //						void				SetMasterMatrix(const Matrix4x4*
    //master_matrix);
private:
    // Cached world matrix
    size_t mValidAddress;                      //!< Matrix address (local or global)
