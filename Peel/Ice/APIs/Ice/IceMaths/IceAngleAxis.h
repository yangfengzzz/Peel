...
public:
    ...
    AngleAxis(float a, float x, float y, float z) {
        mAngle = a;
        mAxis.Set(x, y, z);
    }
    ...
    inline AngleAxis& Zero() {
        mAngle = 0.0;
        mAxis.Set(0.0f, 0.0f, 0.0f);
        return *this;
    }
    //! Assignment from floats.
    inline AngleAxis& Set(float angle, float x, float y, float z) {
        mAngle = angle;
        mAxis.Set(x, y, z);
        return *this;
    }
    //! Assignment from an angle and a vector.
    inline AngleAxis& Set(float angle, const Point& axis) {
        mAngle = angle;
        mAxis = axis;
        return *this;
    }
    ...
    inline AngleAxis& SetAngle(float angle) {
        mAngle = angle;
        return *this;
    }
    //! Sets the axis component only, from a Point.
    inline AngleAxis& SetAxis(const Point& axis) {
        mAxis = axis;
        return *this;
    }
    //! Sets the axis component only, from floats.
    inline AngleAxis& SetAxis(float x, float y, float z) {
        mAxis.Set(x, y, z);
        return *this;
    }
    ...
    //! Operator
    inline AngleAxis operator/(float s) const {
        s = 1.0f / s;
        return AngleAxis(mAngle * s, mAxis * s);
    }
    //! Operator
    inline float operator|(const AngleAxis& a) const {
        return mAxis | a.mAxis;
    }  // This is a mere dot product with the Axis component only!

    //! Operator
    inline AngleAxis& operator+=(const AngleAxis& a) {
        mAngle += a.mAngle;
        mAxis += a.mAxis;
        return *this;
    }
    ...
    //! Operator
    inline AngleAxis& operator*=(float s) {
        mAngle *= s;
        mAxis *= s;
        return *this;
    }
