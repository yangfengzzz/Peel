...
public:
    ...
    inline_ Quat(float _w, float x, float y, float z) {
        p.x = x;
        p.y = y;
        p.z = z;
        w = _w;
    ...
    inline_ Quat(float _w, const Point& _p) {
        p = _p;
        w = _w;
    }
    ...
    inline_ void Set(float x, float y, float z, float _w) {
        p.x = x;
        p.y = y;
        p.z = z;
        w = _w;
    ...
    inline_ void Set(const Point _p, float _w) {
        p = _p;
        w = _w;
    }
    ...
    Quat& Invert() {
        //								const float Norm = SquareMagnitude();	// ### wtf? Changed
        //march, 3, 2009
        const float Norm = Magnitude();
    ...
    inline_ void MakeClosest(const Quat& q) {
        if (((*this) | q) < 0.0f) *this = -*this; /* If dot product is < 0*/
    }
    ...
    //! Operator for Quat = Quat * Quat.
    inline_ Quat operator*(const Quat& q) const {
        return Quat((w * q.w) - (p | q.p), (w * q.p) + (p * q.w) + (p ^ q.p));
    }
    ...
    //! Operator for Quat = Quat / float.
    inline_ Quat operator/(float s) const {
        s = 1.0f / s;
        return Quat(w * s, p * s);
    }
    ...
    //} 		inline	bool	operator!=(const Quat& q)						{ return p != q.p ||
    //FastFabs(w - q.w) > ICE_QUAT_EPSILON;					}

    //! Operator for "if(Quat==Quat)"
    inline_ bool operator==(const Quat& q) const {
        return ((IR(w) == IR(q.w)) && (IR(p.x) == IR(q.p.x)) && (IR(p.y) == IR(q.p.y)) && (IR(p.z) == IR(q.p.z)));
    }
    //! Operator for "if(Quat!=Quat)"
    inline_ bool operator!=(const Quat& q) const {
        return ((IR(w) != IR(q.w)) || (IR(p.x) != IR(q.p.x)) || (IR(p.y) != IR(q.p.y)) || (IR(p.z) != IR(q.p.z)));
    ...
    inline_ void GetBasisVectors(Point& basis0, Point& basis1, Point& basis2) const {
        ...
        basis0.Set(1.0f - (yy + zz), (xy + zw), (xz - yw));

        basis1.Set((xy - zw), 1.0f - (xx + zz), (yz + xw));

        basis2.Set((xz + yw), (yz - xw), 1.0f - (xx + yy));
    }

    //! Retrieve the first basis vector
    inline_ Point GetBasisVector0() const {
        return Point(1.0f - 2.0f * (p.y * p.y + p.z * p.z), 2.0f * (p.x * p.y + p.z * w), 2.0f * (p.x * p.z - p.y * w));
    }

    //! Retrieve the second basis vector
    inline_ Point GetBasisVector1() const {
        return Point(2.0f * (p.x * p.y - p.z * w), 1.0f - 2.0f * (p.x * p.x + p.z * p.z), 2.0f * (p.y * p.z + p.x * w));
    }

    //! Retrieve the third basis vector
    inline_ Point GetBasisVector2() const {
        return Point(2.0f * (p.x * p.z + p.y * w), 2.0f * (p.y * p.z - p.x * w), 1.0f - 2.0f * (p.x * p.x + p.y * p.y));
    }
    ...
    inline_ Quat GetConjugate() const { return Quat(w, -p); }
...
inline_ Quat Quat::Slerp(const Quat& q, float t) {
    ...
    if (fabsf(Sin) < ICE_QUAT_EPSILON) return Quat(*this);
 ...
 *	Only works with unit quaternions, pay attention to what you do... I don't normalize quat before proceding for
 *speed reasons. \return		a 3x3 matrix built from the quaternion
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
inline_ Quat::operator Matrix3x3() const {
    const float xs = p.x + p.x;
    const float ys = p.y + p.y;
    const float zs = p.z + p.z;

    const float wx = w * xs;
    const float wy = w * ys;
    const float wz = w * zs;
    const float xx = p.x * xs;
    const float xy = p.x * ys;
    const float xz = p.x * zs;
    const float yy = p.y * ys;
    const float yz = p.y * zs;
    const float zz = p.z * zs;

    // Ugly but helps the RVO
    return Matrix3x3(1.0f - yy - zz, xy + wz, xz - wy,
 ...
 *	Only works with unit quaternions, pay attention to what you do... I don't normalize quat before proceding for
 *speed reasons. \return		a 4x4 matrix built from the quaternion
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
FUNCTION ICEMATHS_API void UnitRandomQuat(Quat& v, BasicRandom& rnd);
