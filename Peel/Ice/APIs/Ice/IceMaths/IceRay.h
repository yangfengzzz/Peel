...
class ICEMATHS_API Ray : public Allocateable {
public:
    //! Constructor
    inline_ Ray() {}
    //! Constructor
...
inline_ void ComputeReflexionVector(Point& reflected, const Point& source, const Point& impact, const Point& normal) {
    const Point V = impact - source;
    reflected = V - normal * 2.0f * (V | normal);
}

inline_ void ComputeNormalCompo(Point& normal_compo, const Point& outward_dir, const Point& outward_normal) {
    normal_compo = outward_normal * (outward_dir | outward_normal);
}

inline_ void ComputeTangentCompo(Point& outward_dir, const Point& outward_normal) {
    outward_dir -= outward_normal * (outward_dir | outward_normal);
}
...
inline_ void ComputeLocalDirection(Point& local_dir, const Point& world_dir, const Matrix4x4& world) {
    ...
    InverseTransformPoint3x3(local_dir, world_dir, world);
}
