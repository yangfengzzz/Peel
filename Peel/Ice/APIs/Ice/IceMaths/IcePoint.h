...
public:
    ...
    inline_ void Lerp(const Point& a, const Point& b, float t) {
        x = a.x + t * (b.x - a.x);
        y = a.y + t * (b.y - a.y);
        z = a.z + t * (b.z - a.z);
    }
...
#define TWEAKNOTMASK ~TWEAKMASK
    ...
    inline_ Point& ClampLength(float limit_length) {
        ...
        {
            const float CurrentSquareLength = SquareMagnitude();

            if (CurrentSquareLength > limit_length * limit_length) {
                const float Coeff = limit_length / sqrtf(CurrentSquareLength);
                x *= Coeff;
                y *= Coeff;
                z *= Coeff;
            }
    ...
    inline_ AxisIndex LargestAxis() const {
        const float* Vals = &x;
        AxisIndex m = X_;
    ...
    inline_ AxisIndex ClosestAxis() const {
        const float* Vals = &x;
        AxisIndex m = X_;
    ...
    inline_ AxisIndex SmallestAxis() const {
        const float* Vals = &x;
        AxisIndex m = X_;
        if (Vals[Y_] < Vals[m]) m = Y_;
        if (Vals[Z_] < Vals[m]) m = Z_;
    ...
    inline_ Point& operator*=(const Matrix3x3& mat) {
        ...
        const float xp = x * Mat->m[0][0] + y * Mat->m[1][0] + z * Mat->m[2][0];
        const float yp = x * Mat->m[0][1] + y * Mat->m[1][1] + z * Mat->m[2][1];
        const float zp = x * Mat->m[0][2] + y * Mat->m[1][2] + z * Mat->m[2][2];
    ...
    inline_ Point& operator*=(const Matrix4x4& mat) {
        ...
        const float xp = x * Mat->m[0][0] + y * Mat->m[1][0] + z * Mat->m[2][0] + Mat->m[3][0];
        const float yp = x * Mat->m[0][1] + y * Mat->m[1][1] + z * Mat->m[2][1] + Mat->m[3][1];
        const float zp = x * Mat->m[0][2] + y * Mat->m[1][2] + z * Mat->m[2][2] + Mat->m[3][2];
...
inline_ float CrossProductX(const Point& a, const Point& b) { return a.y * b.z - a.z * b.y; }
inline_ float CrossProductY(const Point& a, const Point& b) { return a.z * b.x - a.x * b.z; }
inline_ float CrossProductZ(const Point& a, const Point& b) { return a.x * b.y - a.y * b.x; }

ICE_COMPILE_TIME_ASSERT(sizeof(Point) == 12);

FUNCTION ICEMATHS_API void Normalize1(Point& a);
FUNCTION ICEMATHS_API void Normalize2(Point& a);

FUNCTION ICEMATHS_API void UnitRandomPt(Point& v, BasicRandom& rnd);
