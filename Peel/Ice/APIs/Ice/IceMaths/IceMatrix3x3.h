...
class ICEMATHS_API Matrix3x3 : public Allocateable {
public:
    //! Empty constructor
    inline_ Matrix3x3() {}
    ...
    void Rot(float angle, const Point& axis);

    //! Makes a rotation matrix from an angular velocity
    void Rot(const Point& w);

    //! Transposes the matrix.
    inline_ void Transpose() {
        {
            const udword m01 = IR(m[0][1]);
            const udword m10 = IR(m[1][0]);
            IR(m[0][1]) = m10;
            IR(m[1][0]) = m01;
        }
        {
            const udword m02 = IR(m[0][2]);
            const udword m20 = IR(m[2][0]);
            IR(m[0][2]) = m20;
            IR(m[2][0]) = m02;
        }
        {
            const udword m21 = IR(m[2][1]);
            const udword m12 = IR(m[1][2]);
            IR(m[2][1]) = m12;
            IR(m[1][2]) = m21;
        }

        //									IR(m[1][0]) ^= IR(m[0][1]);	IR(m[0][1]) ^=
        //IR(m[1][0]);	IR(m[1][0]) ^= IR(m[0][1]); 									IR(m[2][0]) ^= IR(m[0][2]);	IR(m[0][2]) ^= IR(m[2][0]);
        //IR(m[2][0]) ^= IR(m[0][2]); 									IR(m[2][1]) ^= IR(m[1][2]);	IR(m[1][2]) ^= IR(m[2][1]);	IR(m[2][1]) ^=
        //IR(m[1][2]);
    }

    //! this = Transpose(a)
    inline_ void Transpose(const Matrix3x3& a) {
        m[0][0] = a.m[0][0];
        m[0][1] = a.m[1][0];
        m[0][2] = a.m[2][0];
        m[1][0] = a.m[0][1];
    ...
    //! Computes the determinant of the matrix. We use the rule of Sarrus.
    inline_ float Determinant() const {
    ...
    */
    ...
    inline_ void Invert() {
        const float Det = Determinant();  // Must be !=0
