...
public:
    ...
    inline_ void SetYawLimits(float min, float max) {
        mYawMin = min;
        mYawMax = max;
    }
    inline_ void SetPitchLimits(float min, float max) {
        mPitchMin = min;
        mPitchMax = max;
    }
    inline_ void SetRollLimits(float min, float max) {
        mRollMin = min;
        mRollMax = max;
    }

    inline_ void SetYawUnlimited() {
        mYawMin = MIN_FLOAT;
        mYawMax = MAX_FLOAT;
    }
    inline_ void SetPitchUnlimited() {
        mPitchMin = MIN_FLOAT;
        mPitchMax = MAX_FLOAT;
    }
    inline_ void SetRollUnlimited() {
        mRollMin = MIN_FLOAT;
        mRollMax = MAX_FLOAT;
    }
