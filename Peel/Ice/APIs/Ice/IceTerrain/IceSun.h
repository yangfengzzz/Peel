...
public:
    ...
    Sun operator+(const Sun& s2) const { return Sun(mTheta + s2.mTheta, mPhi + s2.mPhi, mIntensity + s2.mIntensity); }
    Sun operator*(float f) const { return Sun(mTheta * f, mPhi * f, mIntensity * f); }
    friend Sun operator*(float f, const Sun& s) { return Sun(s.mTheta * f, s.mPhi * f, s.mIntensity * f); }
