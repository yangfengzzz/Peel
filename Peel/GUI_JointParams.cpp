...
static void EditBox_SetFloatValue(PEEL_EditBox* edit_box, float value, bool enabled, const char* label) {
    if (edit_box) {
        ...
        if (edit_box->mLabel && label) edit_box->mLabel->SetLabel(label);
    }
}
...
GUI_1DLimitJointParams::GUI_1DLimitJointParams() : mEditBox_MinLimit(null), mEditBox_MaxLimit(null) {}
...
udword GUI_1DLimitJointParams::Init(EditBoxInterface& owner, IceWidget* parent, udword y) {
    ...
    mEditBox_MinLimit = CreateFloatEditBox(owner, parent, x0, YOffset, "Min limit:", ParamsLabelWidth, EditBoxWidth,
                                           EditBoxHeight, 0, 0.0f);
    mEditBox_MaxLimit = CreateFloatEditBox(owner, parent, x1, YOffset, "Max limit:", ParamsLabelWidth, EditBoxWidth,
                                           EditBoxHeight, 0, 0.0f);
...
void GUI_1DLimitJointParams::SetVisible(bool is_visible) {
    if (mEditBox_MinLimit) mEditBox_MinLimit->SetVisible(is_visible);
    if (mEditBox_MaxLimit) mEditBox_MaxLimit->SetVisible(is_visible);
}

void GUI_1DLimitJointParams::InitFrom(
        Pint_Joint* joint_api, PintJointHandle handle, const char* min_label, const char* max_label, udword index) {
    if (!joint_api)  // Might not be implemented by all plugins
    {
        if (mEditBox_MinLimit) mEditBox_MinLimit->SetEnabled(false);
        if (mEditBox_MaxLimit) mEditBox_MaxLimit->SetEnabled(false);
    } else {
        PintLimits Limits;
        const bool EnableLimits = joint_api->GetLimits(handle, Limits, index);
        if (!EnableLimits) Limits.Set(0.0f, 0.0f);
...
void GUI_1DLimitJointParams::Cancel() {
    if (mEditBox_MinLimit) mEditBox_MinLimit->mSomethingChanged = false;
    if (mEditBox_MaxLimit) mEditBox_MaxLimit->mSomethingChanged = false;
}

bool GUI_1DLimitJointParams::SomethingChanged() const {
    if (mEditBox_MinLimit && mEditBox_MinLimit->mSomethingChanged) return true;
    if (mEditBox_MaxLimit && mEditBox_MaxLimit->mSomethingChanged) return true;
    return false;
}

void GUI_1DLimitJointParams::SaveParams(Pint_Joint* joint_api,
                                        PintJointHandle handle,
                                        bool& wake_up_actors,
                                        udword index) {
...
void GUI_SphericalJointParams::SetVisible(bool is_visible) { mLimit.SetVisible(is_visible); }
...
void GUI_SphericalJointParams::Cancel() { mLimit.Cancel(); }

bool GUI_SphericalJointParams::SomethingChanged() const {
    if (mLimit.SomethingChanged()) return true;
    return false;
}
...
void GUI_HingeJointParams::SetVisible(bool is_visible) { mLimit.SetVisible(is_visible); }
...
void GUI_HingeJointParams::Cancel() { mLimit.Cancel(); }
...
GUI_PrismaticJointParams::GUI_PrismaticJointParams() : mEditBox_SpringStiffness(null), mEditBox_SpringDamping(null) {}
...
udword GUI_PrismaticJointParams::Init(EditBoxInterface& owner, IceWidget* parent, udword y) {
    ...
    mEditBox_SpringStiffness = CreateFloatEditBox(owner, parent, x0, YOffset, "Spring stiffness:", ParamsLabelWidth,
                                                  EditBoxWidth, EditBoxHeight, 0, 0.0f);
    mEditBox_SpringDamping = CreateFloatEditBox(owner, parent, x1, YOffset, "Spring damping:", ParamsLabelWidth,
                                                EditBoxWidth, EditBoxHeight, 0, 0.0f);
    YOffset += EditBoxHeight + 4;
...
void GUI_PrismaticJointParams::SetVisible(bool is_visible) {
    ...
    if (mEditBox_SpringStiffness) mEditBox_SpringStiffness->SetVisible(is_visible);
    if (mEditBox_SpringDamping) mEditBox_SpringDamping->SetVisible(is_visible);
}
...
void GUI_PrismaticJointParams::InitFrom(Pint_Joint* joint_api, PintJointHandle handle) {
    ...
    if (!joint_api)  // Might not be implemented by all plugins
    {
        if (mEditBox_SpringStiffness) mEditBox_SpringStiffness->SetEnabled(false);
        if (mEditBox_SpringDamping) mEditBox_SpringDamping->SetEnabled(false);
    } else {
        {
            PintSpring Spring;
            const bool EnableSpring = joint_api->GetSpring(handle, Spring);
            if (!EnableSpring) Spring.mStiffness = Spring.mDamping = 0.0f;
...
void GUI_PrismaticJointParams::Cancel() {
    ...
    if (mEditBox_SpringStiffness) mEditBox_SpringStiffness->mSomethingChanged = false;
    if (mEditBox_SpringDamping) mEditBox_SpringDamping->mSomethingChanged = false;
}

bool GUI_PrismaticJointParams::SomethingChanged() const {
    if (mLimit.SomethingChanged()) return true;

    if (mEditBox_SpringStiffness && mEditBox_SpringStiffness->mSomethingChanged) return true;
    if (mEditBox_SpringDamping && mEditBox_SpringDamping->mSomethingChanged) return true;
    return false;
}
...
void GUI_DistanceJointParams::SetVisible(bool is_visible) { mLimit.SetVisible(is_visible); }
...
void GUI_DistanceJointParams::Cancel() { mLimit.Cancel(); }
...
bool GUI_D6JointParams::SomethingChanged() const {
    if (mLimitX.SomethingChanged()) return true;
    if (mLimitY.SomethingChanged()) return true;
    if (mLimitZ.SomethingChanged()) return true;
    return false;
}
...
GUI_GearJointParams::GUI_GearJointParams() : mEditBox_GearRatio(null) {}

GUI_GearJointParams::~GUI_GearJointParams() { DELETESINGLE(mEditBox_GearRatio); }
...
udword GUI_GearJointParams::Init(EditBoxInterface& owner, IceWidget* parent, udword y) {
    ...
    mEditBox_GearRatio =
            CreateFloatEditBox(owner, parent, 4, YOffset, "Gear ratio:", ParamsLabelWidth, 100, EditBoxHeight, 0, 0.0f);
    YOffset += EditBoxHeight + 4;
...
void GUI_GearJointParams::SetVisible(bool is_visible) {
    if (mEditBox_GearRatio) mEditBox_GearRatio->SetVisible(is_visible);
}
...
void GUI_GearJointParams::InitFrom(Pint_Joint* joint_api, PintJointHandle handle) {
    if (mEditBox_GearRatio) {
        if (!joint_api)  // Might not be implemented by all plugins
        {
            ...
            if (!EnableGearRatio) GearRatio = 0.0f;
...
void GUI_GearJointParams::Cancel() {
    if (mEditBox_GearRatio) mEditBox_GearRatio->mSomethingChanged = false;
}

bool GUI_GearJointParams::SomethingChanged() const {
    if (mEditBox_GearRatio && mEditBox_GearRatio->mSomethingChanged) return true;
    return false;
}
...
GUI_RackJointParams::GUI_RackJointParams() : mEditBox_GearRatio(null) {}

GUI_RackJointParams::~GUI_RackJointParams() { DELETESINGLE(mEditBox_GearRatio); }
