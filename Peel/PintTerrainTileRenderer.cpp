...
PintTerrainTileRenderer::PintTerrainTileRenderer(GLIndexBufferCollection& index_buffers,
                                                 udword nb_verts,
                                                 const Point* verts,
                                                 const Point* vnormals)
    : mIndexBuffers(index_buffers), mLOD(0) {
    SPY_ZONE("PintTerrainTileRenderer ctor")

    mVertexBuffer.Create(nb_verts, verts, vnormals);
    if (vnormals) mBaseFlags |= SHAPE_RENDERER_USES_EXPLICIT_VERTEX_NORMALS;
}
