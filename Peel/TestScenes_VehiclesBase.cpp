...
bool VehicleInput::KeyboardCallback(unsigned char key, int x, int y, bool down) {
    switch (key) {
        ...
        case '7': {
            if (down) mExtraDistance += 0.5f * mExtraDistanceScale;
            return true;
        } break;
        ...
        case '+': {
            if (down) NextCameraMode();
            return true;
        } break;

        case '-': {
            if (down) PreviousCameraMode();
            return true;
        } break;
    }
    return false;
...
void VehicleInput::LostFocus() { mInput.Reset(); }

const udword VehicleInput::mNbCameraModes = 6;

static inline_ float fsel(float a, float b, float c) { return (a >= 0.0f) ? b : c; }

void VehicleInput::UpdateCamera(Pint& pint, PintActorHandle focus_object) {
    if (!focus_object) return;

    if (!(pint.GetFlags() & PINT_IS_ACTIVE)) return;
    ...
    if (mCameraStyle) {
        ...
        mCameraRotateAngleY =
                fsel(mCameraRotateAngleY - 10.0f * PI, mCameraRotateAngleY - 10.0f * PI,
                     fsel(-mCameraRotateAngleY - 10.0f * PI, mCameraRotateAngleY + 10.0f * PI, mCameraRotateAngleY));
        ...
        PxVec3 cameraDir = PxVec3(0, 0, 1) * PxCos(mCameraRotateAngleY + cameraYRotExtra) +
                           PxVec3(1, 0, 0) * PxSin(mCameraRotateAngleY + cameraYRotExtra);
    ...
    const Point CamPos =
            Pose.mPos + Point(0.0f, Up, 0.0f) - mFilteredObjectDir * (mCamera.mDistToTarget + mExtraDistance);
...
udword VehicleInput::GetFlags() const { return mControlCamera ? TEST_FLAGS_USE_CURSOR_KEYS : TEST_FLAGS_DEFAULT; }
