...
private:
    ...
    bool ValidateTouchedActor(Pint& pint,
                              IceButton* button,
...
public:
    ...
    inline_ LinkActor* ReserveSlot() {
        mDirty = true;
        return ICE_RESERVE(LinkActor, mLinkActors);
    }

    inline_ udword GetNbLinkActors() const { return mLinkActors.GetNbEntries() / (sizeof(LinkActor) / sizeof(udword)); }
    inline_ const LinkActor* GetLinkActors() const {
        return reinterpret_cast<const LinkActor*>(mLinkActors.GetEntries());
    }
