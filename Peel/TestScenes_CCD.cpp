...
#define START_CCD_TEST(name, desc)                                        \
    ...
    public:                                                               \
        ...
        virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("CCD Test", 200); }

///////////////////////////////////////////////////////////////////////////////

static const char* gDesc_CCDTest_DynamicBoxesVsStaticContainer =
        "CCD: 10 dynamic boxes (1.0; 1.0; 1.0) moving with linear velocity 400 against thin static boxes.";
...
static const char* gDesc_CCDTest_DynamicBoxesVsStaticContainer2 =
        "CCD: 30 dynamic boxes (0.4; 0.4; 0.4) moving with linear velocity 200 against thin static boxes.";
...
static const char* gDesc_CCDTest_ThinRods =
        "CCD: 40 thin dynamic rods (0.04; 4.0; 0.04) falling with linear velocity 4 against a thin static planar mesh. The size and shape of these objects make the scene challenging even without CCD. \
...
static const char* gDesc_CCDTest_GameLevel =
        "CCD: 32*32 dynamic convexes falling with linear velocity 100 against a static mesh level.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    return CreateArrayOfDynamicConvexes(pint, ConvexCreate, NbX, NbY, AltitudeC, Extents.x - 8.0f, Extents.z - 8.0f,
                                        &Offset, &LinVel);
}
...
static const char* gDesc_CCDTest_SmallConvexes =
        "CCD: 32*32 dynamic convexes (of low complexity) moving down with linear velocity 100 against on a tessellated "
        "planar mesh.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportMeshes || !caps.mSupportConvexes || !caps.mSupportRigidBodySimulation) return false;
    ...
    for (int i = 0; i < C.mNbVerts; i++) C.mVerts[i] *= 0.1f;
...
static const char* gDesc_CCDTest_BoxVsStack =
        "CCD: dynamic vs dynamic. A fast moving dynamic box is thrown against a stack of dynamic boxes.";
...
static const char* gDesc_CCDTest_AngularCCD =
        "CCD: dynamic vs dynamic. Test for angular CCD. This test shows the limits & side-effects of various \
...
static const char* gDesc_CCDTest_GhostContacts =
        "CCD: this test shows the limitation of speculative contacts. With regular CCD or raycast CCD \
    ...
    for (udword j = 0; j < NbStacks; j++) {
        ...
        while (NbBoxes) {
            for (udword i = 0; i < NbBoxes; i++) {
                ...
                if (ID == 112 || ID == 97 || ID == 98 || ID == 81 || ID == 82 || ID == 83) CreateObject = false;
...
static const char* gDesc_CCDTest_GhostContacts2 =
        "CCD: this test shows the limitation of speculative contacts. With regular CCD or raycast CCD \
...
static const char* gDesc_CCDTest_ObjectVsComplexMesh =
        "CCD: a single dynamic convex thrown with linear velocity 1500 against a complex static mesh. This shows \
...
static const char* gDesc_CCDTest_ConvexCascade =
        "CCD: dynamic vs dynamic stress test. Multiple convexes & thin boxes falling down from the sky. Try to pick up "
        "& manipulate pieces after the fall...";
...
static const char* gDesc_CCDTest_ThinPyramidLayers =
        "CCD: dynamic vs dynamic. Pile of thin layers forming a pyramid. Try to pick up & manipulate pieces after the "
        "fall...";
...
/*static const char* gDesc_CCDTest_DynamicDynamic_PileOfThinBoxes2 = "CCD: dynamic vs dynamic. Pile of thin boxes. Try
to pick up & manipulate pieces after the fall...";
