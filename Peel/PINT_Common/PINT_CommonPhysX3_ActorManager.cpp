...
void ActorManager::Add(PxRigidActor* actor) {
    ...
    if (actor->getConcreteType() == PxConcreteType::eRIGID_STATIC) {
        ...
        ASSERT(actor->getConcreteType() == PxConcreteType::eRIGID_DYNAMIC ||
               actor->getConcreteType() == PxConcreteType::eARTICULATION_LINK);
    ...
    if (actor->getNbShapes() == 1) {
        ...
        if (1 == actor->getShapes(&shape, 1)) {
            ...
            if (LocalPose.p.x == 0.0f && LocalPose.p.y == 0.0f && LocalPose.p.z == 0.0f && LocalPose.q.x == 0.0f &&
                LocalPose.q.y == 0.0f && LocalPose.q.z == 0.0f && LocalPose.q.w == 1.0f) {
...
void ActorManager::UpdateCachedData() {
    if (mTimestamp == mCachedTimestamp) return;
...

    {
        ...
        ActorCachedData* CachedStatic = reinterpret_cast<ActorCachedData*>(
                mStaticActors_CachedData.Reserve(NbStaticActors * ACTOR_CACHED_DATA_SIZE));
...

    {
        ...
        ActorCachedData* CachedDynamic = reinterpret_cast<ActorCachedData*>(
                mDynamicActors_CachedData.Reserve(NbDynamicActors * ACTOR_CACHED_DATA_SIZE));
