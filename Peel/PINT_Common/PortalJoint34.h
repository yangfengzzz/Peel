...
public:
    ...
    PortalJoint(const PxTolerancesScale& /*scale*/,
                PxRigidActor* actor0,
                const PxTransform& localFrame0,
                PxRigidActor* actor1,
                const PxTransform& localFrame1)
        : PortalJointT(PxJointConcreteType::eFIXED, PxBaseFlag::eOWNS_MEMORY | PxBaseFlag::eIS_RELEASABLE) {
        PortalJointData* data =
                reinterpret_cast<PortalJointData*>(PX_ALLOC(sizeof(PortalJointData), "PortalJointData"));
        Cm::markSerializedMem(data, sizeof(PortalJointData));
        mData = data;
...
private:
    ...
    PX_FORCE_INLINE PortalJointData& data() const { return *static_cast<PortalJointData*>(mData); }
};
}  // namespace Ext
