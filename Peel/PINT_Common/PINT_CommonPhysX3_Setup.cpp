...
attributes0, PxFilterData filterData0, const PxActor* a0, const PxShape* s0, PxFilterObjectAttributes attributes1,
PxFilterData filterData1, const PxActor* a1, const PxShape* s1, PxPairFlags& pairFlags)
        {
                //printf("pairFound\n");
//		if(mFilterCallback->PairFound(*gPhysX, PintActorHandle(a0), PintShapeHandle(s0), PintActorHandle(a1),
PintShapeHandle(s1))) return PxFilterFlag::eDEFAULT;
//		else
//			return PxFilterFlag::eSUPPRESS;
        }
        virtual		void			pairLost(	PxU32 pairID,
                                                                                        PxFilterObjectAttributes
attributes0, PxFilterData filterData0, PxFilterObjectAttributes attributes1, PxFilterData filterData1, bool
objectRemoved)
        {
        }
        virtual		bool			statusChange(PxU32& pairID, PxPairFlags& pairFlags, PxFilterFlags&
filterFlags)
        {
                return false;
        }
...
void SetupSceneDesc(
        ...
        PxSimulationEventCallback* secb,
        PxContactModifyCallback* cmcb) {
...
#endif
    //		SetSceneFlag(sceneDesc, PxSceneFlag::eDEPRECATED_TRIGGER_TRIGGER_REPORTS,
    //true); 	if(!gEnableSSE) 		sceneDesc.flags					|= PxSceneFlag::eDISABLE_SSE;
    ...
    if (mParams.mEnableTwoDirFriction)
        //			sceneDesc.flags					|=
        //PxSceneFlag::eENABLE_TWO_DIRECTIONAL_FRICTION;
    ...
    if (mParams.mTGS) sceneDesc.solverType = PxSolverType::eTGS;
#endif

    if (!mParams.mEnableSleeping) sceneDesc.wakeCounterResetValue = 9999999999.0f;

    sceneDesc.cpuDispatcher = cpu_dispatcher;
    sceneDesc.filterShader = PhysX3_SimulationFilterShader;
    if (mParams.mEnableCCD) gFilterShaderExtraPairFlags |= PxPairFlag::eDETECT_CCD_CONTACT;
    ...
    if (mParams.mEnableContactNotif || desc.mContactNotifyCallback) {
        ...
        const udword DesiredFlags = desc.mContactNotifyCallback ? desc.mContactNotifyCallback->GetContactFlags()
                                                                : PintContactNotifyCallback::CONTACT_ALL;
...
//	sceneDesc.staticKineFilteringMode = PxPairFilteringMode::eKEEP;
    ...
    if (mParams.mMaxBiasCoeff >= 0.0f) sceneDesc.maxBiasCoefficient = mParams.mMaxBiasCoeff;
}
...
void SetupBroadphase(const PINT_WORLD_CREATE& desc, const EditableParams& params, PxScene* scene) {
    ...
    if (params.mBroadPhaseType == PxBroadPhaseType::eMBP) {
        ...
        const PxU32 nbRegions =
                PxBroadPhaseExt::createRegionsFromWorldBounds(regions, globalBounds, params.mMBPSubdivLevel);
