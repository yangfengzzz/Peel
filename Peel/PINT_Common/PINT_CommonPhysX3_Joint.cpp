...
//		case PxConcreteType::eCONTACT:					return PINT_JOINT_SPHERICAL;
        ...
        case PxJointConcreteType::eGEAR:
            return PINT_JOINT_GEAR;
#endif
#if PHYSX_SUPPORT_NEW_JOINT_TYPES
        case PxJointConcreteType::eRACK_AND_PINION:
            return PINT_JOINT_RACK_AND_PINION;
#endif
#endif
    }
...
bool PhysX_JointAPI::SetFrames(PintJointHandle handle, const PR* frame0, const PR* frame1) {
    ...
    if (frame0) Joint->setLocalPose(PxJointActorIndex::eACTOR0, ToPxTransform(*frame0));

    if (frame1) Joint->setLocalPose(PxJointActorIndex::eACTOR1, ToPxTransform(*frame1));
...
bool PhysX_JointAPI::SetLimits(PintJointHandle handle, const PintLimits& limits, udword index) {
    ...
    // Contact distance is a nasty parameter that creates annoying issues for no reason so I'll just overwrite it all
    // the time
    SharedPhysX& physx = static_cast<SharedPhysX&>(mPint);
    ...
    switch (GetType(handle)) {
        ...
        case PINT_JOINT_D6: {
            ...
            PxJointLinearLimitPair limit =
                    D6->getLinearLimit(Axis);                // Fetch current limits to preserve other parameters
            limit.PHYSX_CONTACT_DISTANCE = ContactDistance;  // ...except that one
...
void SetupD6Projection(PxD6Joint* j,
                       bool enable_projection,
                       float projection_linear_tolerance,
                       float projection_angular_tolerance) {
...
PxSphericalJoint* CreateSphericalJoint(const EditableParams& params,
                                       PxPhysics& physics,
                                       const PINT_SPHERICAL_JOINT_CREATE& jc,
                                       PxRigidActor* actor0,
                                       PxRigidActor* actor1,
...
PxFixedJoint* CreateFixedJoint(const EditableParams& params,
                               PxPhysics& physics,
                               const PINT_FIXED_JOINT_CREATE& jc,
                               PxRigidActor* actor0,
                               PxRigidActor* actor1,
                               const PxTransform& localFrame0,
                               const PxTransform& localFrame1) {
...
PxPrismaticJoint* CreatePrismaticJoint(const EditableParams& params,
                                       PxPhysics& physics,
                                       const PINT_PRISMATIC_JOINT_CREATE& jc,
                                       PxRigidActor* actor0,
                                       PxRigidActor* actor1,
                                       const PxTransform& localFrame0,
                                       const PxTransform& localFrame1) {
    ...
    if (j) {
        ...
        if (jc.mSpring.mStiffness == 0.0f && jc.mSpring.mDamping == 0.0f) {
            ...
            // ### these hard joints are better looking against limits but don't support "springs", at least in this
            // form
...
PxDistanceJoint* CreateDistanceJoint(const EditableParams& params,
                                     PxPhysics& physics,
                                     const PINT_DISTANCE_JOINT_CREATE& jc,
                                     PxRigidActor* actor0,
                                     PxRigidActor* actor1,
                                     const PxTransform& localFrame0,
                                     const PxTransform& localFrame1) {
    ...
    if (j) {
        if (IsMaxDistanceLimitEnabled(jc.mLimits)) {
            ...
            j->setDistanceJointFlag(PxDistanceJointFlag::eMAX_DISTANCE_ENABLED, false);
