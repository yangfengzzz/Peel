...
private:
    ...
    SampleVehicleWheelQueryResults() : mWheelQueryResults(NULL), mMaxNumWheels(0), mNumWheels(0) { init(); }
...
public:
    ...
    // Create a PxBatchQuery instance that will be used as a single batched raycast of multiple suspension lines of
    // multiple vehicles
...
public:
    ...
    PxVehicleDrive4W* create4WVehicle(SharedPhysX_Vehicles& sharedPhysX,
    ...
    PX_FORCE_INLINE const PxVehicleWheelQueryResult& getVehicleWheelQueryResults(const PxU32 i) const {
        return mVehicleWheelQueryResults[i];
    }
...
#if PINT_DEBUG_VEHICLE_ON
    void updateAndRecordTelemetryData(const PxF32 timestep,
