...
- need to revisit leaf-level / midphase API.....big change (PX-2271)
    - isn't there an inconsistency between raycasts & others w.r.t. the "midphase" query, i.e. why do we support
multiple hits per mesh natively for raycasts but not for overlaps?
...
- adapter redundant with validatePayload but not easy to unify. Maybe revisit markForUpdate and pass the bounds there if
needed => or pass adapter to all funcs that need it
...
static PX_FORCE_INLINE bool isDynamicActor(const PxRigidActor& actor) {
    return actor.getConcreteType() != PxConcreteType::eRIGID_STATIC;
}
...
public:
    ...
    virtual bool processPruner(PxU32 prunerIndex) const {
        if (prunerIndex == 0) return mFlags & PxQueryFlag::eSTATIC;
        if (prunerIndex == 1) return mFlags & PxQueryFlag::eDYNAMIC;
        PX_ASSERT(0);
        return false;
    }
};
...
namespace {
// We use PxSceneQuerySystem as a hook into PhysX but we could eventually bypass that entirely. It's a custom SQ system:
// we only re-implement here what we need in PEEL.
...
public:
    ...
    virtual void setDynamicTreeRebuildRateHint(PxU32 dynTreeRebuildRateHint) {
        mDynamicTreeRebuildHint = dynTreeRebuildRateHint;
    }
    ...
    virtual bool raycast(const PxVec3&,
                         const PxVec3&,
                         const PxReal,
                         PxRaycastCallback&,
                         PxHitFlags,
    ...
    virtual bool overlap(const PxGeometry&,
                         const PxTransform&,
                         PxOverlapCallback&,
                         const PxQueryFilterData&,
                         PxQueryFilterCallback*,
    ...
    virtual PxSQCompoundHandle addSQCompound(const PxRigidActor&, const PxShape**, const PxBVH&, const PxTransform*) {
        PX_ASSERT(!"Not supported");
        return 0;
    }
    ...
    bool _raycastAny(const PxVec3& origin,
                     const PxVec3& unitDir,
                     const PxReal distance,
                     const PxQueryFilterData& filterData,
                     PxQueryFilterCallback* filterCB) const;

    bool _raycastClosest(const PxVec3& origin,
                         const PxVec3& unitDir,
                         const PxReal distance,
                         PxRaycastHit& hit,
                         PxHitFlags hitFlags,
    ...
    bool _overlapAny(const PxGeometry& geometry,
                     const PxTransform& pose,
                     const PxQueryFilterData& filterData,
                     PxQueryFilterCallback* filterCB) const;

    PxU32 _overlapAll(const PxGeometry& geometry,
                      const PxTransform& pose,
                      PxOverlapHit* hits,
                      PxU32 nb,
                      const PxQueryFilterData& filterData,
    ...
    bool _sweepClosest(const PxGeometry& geometry,
                       const PxTransform& pose,
                       const PxVec3& unitDir,
                       const PxReal distance,
                       PxSweepHit& hit,
    ...
    // We only need to call commitUpdates() for edge cases where e.g. we move objects in Pause mode, and the regular SQ
...
PEEL_CSQS::PEEL_CSQS(PxU64 contextID,
                     Pruner* staticPruner,
                     Pruner* dynamicPruner,
                     PxU32 dynamicTreeRebuildRateHint,
                     PxSceneQueryUpdateMode::Enum mode)
...
void PEEL_CSQS::addSQShape(const PxRigidActor& actor,
                           const PxShape& shape,
                           const PxBounds3& bounds,
                           const PxTransform& transform,
                           const PxSQCompoundHandle* compoundHandle,
...
void PEEL_CSQS::sync(PxU32 prunerIndex,
                     const PxSQPrunerHandle* handles,
                     const PxU32* indices,
                     const PxBounds3* bounds,
                     const PxTransform32* transforms,
...
void PEEL_CSQS::finalizeUpdates() {
    switch (mSceneQueryUpdateMode) {
        ...
        case PxSceneQueryUpdateMode::eBUILD_ENABLED_COMMIT_DISABLED:
            mQuerySystem.update(true, false);
            break;
        case PxSceneQueryUpdateMode::eBUILD_DISABLED_COMMIT_DISABLED:
            mQuerySystem.update(false, false);
            break;
    }
}

namespace {
// We re-route this custom filter CB to what we need from the traditional PhysX filter callback. No need to test for
// prefilter/postfilter flags here, we know what we need in PEEL and we call our own code directly anyway. In fact we
// could skip the PxQueryFilterCallback object entirely and just re-do here in our PrunerFilterCallback what we did
// there (in SharedPhysX::preFilter). But reusing PxQueryFilterCallback allows us to keep sharing that code with
// previous PhysX versions etc.
struct PEEL_CustomFilterCB : PrunerFilterCallback {
    ...
    virtual const PxGeometry* validatePayload(const PrunerPayload& payload, PxHitFlags& hitFlags) {
        ...
        if (mFilterCB) {
            ...
            if (mFilterCB->preFilter(PxFilterData(), shape, actor, hitFlags) == PxQueryHitType::eNONE) return NULL;
        }
...
bool PEEL_CSQS::_raycastAny(const PxVec3& origin,
                            const PxVec3& unitDir,
                            const PxReal distance,
                            const PxQueryFilterData& filterData,
                            PxQueryFilterCallback* filterCB) const {
...
bool PEEL_CSQS::_raycastClosest(const PxVec3& origin,
                                const PxVec3& unitDir,
                                const PxReal distance,
                                PxRaycastHit& hit,
                                PxHitFlags hitFlags,
...
bool PEEL_CSQS::_overlapAny(const PxGeometry& geometry,
                            const PxTransform& pose,
                            const PxQueryFilterData& filterData,
                            PxQueryFilterCallback* filterCB) const {
    commitUpdates();
    ...
    struct CB : public DefaultPrunerOverlapCallback {
        ...
        CB(PrunerFilterCallback& filterCB,
           const GeomOverlapTable* funcs,
...
PxU32 PEEL_CSQS::_overlapAll(const PxGeometry& geometry,
                             const PxTransform& pose,
                             PxOverlapHit* hits,
                             PxU32 nb,
                             const PxQueryFilterData& filterData,
    ...
    struct CB : public DefaultPrunerOverlapCallback {
        ...
        CB(PrunerFilterCallback& filterCB,
           ...
           const PxGeometry& geometry,
           const PxTransform& pose)
            : DefaultPrunerOverlapCallback(filterCB, funcs, geometry, pose), mNbHits(0) {}
...
static bool _sweepClosestT(PxSweepHit& hit,
                           const QuerySystem& sqs,
                           const Gu::GeomSweepFuncs& funcs,
                           const PxGeometry& geometry,
                           const PxTransform& pose,
                           const PxVec3& unitDir,
                           float distance,
...
bool PEEL_CSQS::_sweepClosest(const PxGeometry& geometry,
                              const PxTransform& pose,
                              const PxVec3& unitDir,
                              const PxReal distance,
                              PxSweepHit& hit,
    ...
    switch (geometry.getType()) {
        case PxGeometryType::eSPHERE: {
            return _sweepClosestT<DefaultPrunerSphereSweepCallback>(hit, mQuerySystem, mCachedFuncs.mCachedSweepFuncs,
                                                                    geometry, pose, unitDir, distance, hitFlags,
                                                                    filterData, filterCB);
        }
        case PxGeometryType::eCAPSULE: {
            return _sweepClosestT<DefaultPrunerCapsuleSweepCallback>(hit, mQuerySystem, mCachedFuncs.mCachedSweepFuncs,
                                                                     geometry, pose, unitDir, distance, hitFlags,
                                                                     filterData, filterCB);
        }
        case PxGeometryType::eBOX: {
            return _sweepClosestT<DefaultPrunerBoxSweepCallback>(hit, mQuerySystem, mCachedFuncs.mCachedSweepFuncs,
                                                                 geometry, pose, unitDir, distance, hitFlags,
                                                                 filterData, filterCB);
        }
        case PxGeometryType::eCONVEXMESH: {
            return _sweepClosestT<DefaultPrunerConvexSweepCallback>(hit, mQuerySystem, mCachedFuncs.mCachedSweepFuncs,
                                                                    geometry, pose, unitDir, distance, hitFlags,
                                                                    filterData, filterCB);
        }
        default: {
...
PxSceneQuerySystem* CreatePEELCustomSceneQuerySystem() {
    ...
    gCSQS = PX_NEW(PEEL_CSQS)(contextID, staticPruner, dynamicPruner, 100,
                              PxSceneQueryUpdateMode::eBUILD_ENABLED_COMMIT_ENABLED);
    return gCSQS;
}

bool PEEL_RaycastClosest(const PxVec3& origin,
                         const PxVec3& unitDir,
                         const PxReal distance,
                         PxRaycastHit& hit,
                         PxHitFlags hitFlags,
...
bool PEEL_RaycastAny(const PxVec3& origin,
                     const PxVec3& unitDir,
                     const PxReal distance,
                     const PxQueryFilterData& filterData,
                     PxQueryFilterCallback* filterCB) {
...
bool PEEL_OverlapAny(const PxGeometry& geometry,
                     const PxTransform& pose,
                     const PxQueryFilterData& filterData,
                     PxQueryFilterCallback* filterCB) {
...
PxU32 PEEL_OverlapAll(const PxGeometry& geometry,
                      const PxTransform& pose,
                      PxOverlapHit* hits,
                      PxU32 nb,
                      const PxQueryFilterData& filterData,
...
bool PEEL_SweepClosest(const PxGeometry& geometry,
                       const PxTransform& pose,
                       const PxVec3& unitDir,
                       const PxReal distance,
                       PxSweepHit& hit,
