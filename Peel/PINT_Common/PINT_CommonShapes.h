...
struct InternalShapeData {
    InternalShapeData(void* engine_shape,
                      const void* engine_material,
                      PintShapeRenderer* renderer,
                      const PR& local_pose,
                      udword collision_group)
        : mShape(engine_shape),
    ...
    inline_ bool Compare(const void* material,
                         PintShapeRenderer* renderer,
                         const PR& local_pose,
                         udword collision_group) const {
        // TODO: shapes' names are ignored here. Good? Bad?
        // Also this will break if we change the local pose after shape creation.
        if (mMaterial == material && mRenderer == renderer && mLocalPose.mPos == local_pose.mPos &&
            mLocalPose.mRot == local_pose.mRot && mCollisionGroup == collision_group)
            return true;
        return false;
    }
};
...
struct InternalSphereShapeData : InternalShapeData {
    InternalSphereShapeData(float radius,
                            void* shape,
                            const void* material,
                            PintShapeRenderer* renderer,
                            const PR& local_pose,
                            udword collision_group)
        : mRadius(radius), InternalShapeData(shape, material, renderer, local_pose, collision_group) {}
    float mRadius;
};

struct InternalBoxShapeData : InternalShapeData {
    InternalBoxShapeData(const Point& extents,
                         void* shape,
                         const void* material,
                         PintShapeRenderer* renderer,
                         const PR& local_pose,
                         udword collision_group)
        : mExtents(extents), InternalShapeData(shape, material, renderer, local_pose, collision_group) {}
    Point mExtents;
};

struct InternalCapsuleShapeData : InternalShapeData {
    InternalCapsuleShapeData(float radius,
                             float half_height,
                             void* shape,
                             const void* material,
                             PintShapeRenderer* renderer,
...
struct InternalMeshShapeData : InternalShapeData {
    InternalMeshShapeData(const void* mesh,
                          void* shape,
                          const void* material,
                          PintShapeRenderer* renderer,
                          const PR& local_pose,
                          udword collision_group)
        : mMesh(mesh), InternalShapeData(shape, material, renderer, local_pose, collision_group) {}
    const void* mMesh;
};
...
public:
    ...
    inline_ udword RegisterShape(float radius,
                                 void* shape,
                                 const void* material,
                                 PintShapeRenderer* renderer,
                                 const PR& local_pose,
                                 udword collision_group) {
        return RegisterShapeData(
                InternalSphereShapeData(radius, shape, material, renderer, local_pose, collision_group));
    }

    void* FindShape(float radius,
                    const void* material,
                    PintShapeRenderer* renderer,
                    const PR& local_pose,
                    udword collision_group) const;
...
public:
    ...
    inline_ udword RegisterShape(const Point& extents,
                                 void* shape,
                                 const void* material,
                                 PintShapeRenderer* renderer,
                                 const PR& local_pose,
    ...
    void* FindShape(const Point& extents,
                    const void* material,
                    PintShapeRenderer* renderer,
                    const PR& local_pose,
                    udword collision_group) const;
...
public:
    ...
    inline_ udword RegisterShape(float radius,
                                 ...
                                 udword collision_group) {
        return RegisterShapeData(
                InternalCapsuleShapeData(radius, half_height, shape, material, renderer, local_pose, collision_group));
    }
...
public:
    ...
    inline_ udword RegisterShape(const void* mesh,
                                 void* shape,
                                 const void* material,
                                 PintShapeRenderer* renderer,
                                 const PR& local_pose,
    ...
    void* FindShape(const void* mesh,
                    const void* material,
                    PintShapeRenderer* renderer,
                    const PR& local_pose,
                    udword collision_group) const;
