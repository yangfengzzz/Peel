...
PEEL_PhysX3_AllocatorCallback::PEEL_PhysX3_AllocatorCallback()
    : mTotalNbAllocs(0), mNbAllocs(0), mCurrentMemory(0), mLog(false) {}
...
static int atomicIncrement(volatile int* val) { return (int)InterlockedIncrement((volatile LONG*)val); }

static int atomicDecrement(volatile int* val) { return (int)InterlockedDecrement((volatile LONG*)val); }
