...
public:
    ...
    void release() {
        for (PxU32 a = 0; a < mAllocatedBlocks.size(); ++a) PX_FREE(mAllocatedBlocks[a]);
    ...
    virtual ~BlockBasedAllocator() { release(); }
};
...
public:
    ...
    virtual PxU8* allocateCacheData(const PxU32 byteSize) { return mAllocator[currIdx].allocate(byteSize); }
...
#ifdef PX_DEBUG
struct ImmActorHandleImpl;
typedef ImmActorHandleImpl* ImmActorHandle;
struct ImmShapeHandleImpl;
typedef ImmShapeHandleImpl* ImmShapeHandle;
#else
...
struct ActorPair {
    ...
    PX_FORCE_INLINE bool operator==(const ActorPair& other) const { return mID0 == other.mID0 && mID1 == other.mID1; }
};
...
public:
    ImmediateShape(const PxGeometry& geom, const PxTransform& localPose, void* userData)
        : mGeom(geom), mLocalPose(localPose), mUserData(userData) {}
    ImmediateShape() {}
    ~ImmediateShape() {}
...
struct ShapePair {
    ...
    PX_FORCE_INLINE bool operator==(const ShapePair& other) const { return mID0 == other.mID0 && mID1 == other.mID1; }
};
...
public:
...
#ifdef SHAPE_CENTRIC
    ImmActorHandle createActor(PxU32 nbShapes,
...
#else
    ImmActorHandle createActor(ImmShapeHandle shape,
                               const PxTransform& pose,
                               PxU32 group,
                               const MassProps* massProps,
                               Dy::ArticulationLinkHandle link);
#endif
    //						bool								addShape(ImmActorHandle actor, ImmShapeHandle
    //shape);

    ImmActorHandle createGroundPlane() {
        const ImmShapeHandle planeShape =
                createShape(PxPlaneGeometry(), PxTransformFromPlaneEquation(PxPlane(0.0f, 1.0f, 0.0f, 0.0f)), null);
    ...
    void createContactConstraints(float dt,
                                  float invDt,
    ...
    PX_FORCE_INLINE PxU32 raycastShape(PxU32 i,
                                       PxU32 maxNbHits,
                                       PxGeomRaycastHit* hits,
                                       const PxVec3& origin,
                                       const PxVec3& dir,
                                       float dist,
                                       PxHitFlags flags) {
...
//																return
//PxGeometryQuery::raycastXP(origin, dir, geom, shapePose, dist, flags, maxNbHits, hits, sizeof(PxGeomRaycastHit));
#else
        return PxGeometryQuery::raycast(origin, dir, geom, shapePose, dist, flags, maxNbHits, hits);
#endif
    }
...
//																return
//PxGeometryQuery::overlapXP(queryVolume, queryPose, geom, shapePose);
#else
        return PxGeometryQuery::overlap(queryVolume, queryPose, geom, shapePose);
#endif
    }

    PX_FORCE_INLINE bool sweepShape(PxU32 i,
                                    PxGeomSweepHit& hit,
                                    const PxGeometry& queryVolume,
                                    const PxTransform& queryPose,
                                    const PxVec3& dir,
                                    float dist,
                                    PxHitFlags flags) {
        const PxBounds3& bounds = mShapeBounds[i];
    ...
    bool raycastClosest(PxGeomRaycastHit& pxHit,
                        float& minDist,
                        PxU32 i,
                        const PxVec3& origin,
                        const PxVec3& dir,
...
private:
    ...
    struct ContactPair {
        ...
        PX_FORCE_INLINE ContactPair(const ActorPair& actorPair,
    ...
    struct ContactPair : ActorPair {
        ...
        PX_FORCE_INLINE ContactPair(const ContactPair& that)
            : ActorPair(that), mNbContacts(that.mNbContacts), mStartContactIndex(that.mStartContactIndex) {}
    ...
    template <class T>
    void processContactHeader(PxConstraintBatchHeader& header,
                              ...
                              float correlationDistance);
    template <class T>
    void processJointHeader(PxConstraintBatchHeader& header,
                            PxSolverConstraintDesc* orderedDescs,
                            float dt,
                            float invDt,
                            float stepDt,
