...
PxPortalJoint* physx::PxPortalJointCreate(PxPhysics& physics,
                                          PxRigidActor* actor0,
    ...
    PX_CHECK_AND_RETURN_NULL((actor0 && actor0->is<PxRigidBody>()) || (actor1 && actor1->is<PxRigidBody>()),
                             "PxPortalJointCreate: at least one actor must be dynamic");
...
void PortalJoint::importExtraData(PxDeserializationContext& context) {
    if (mData) mData = context.readExtraData<PortalJointData, PX_SERIAL_ALIGN>();
...
static void PortalJointProject(const void* constantBlock,
...
static void PortalJointVisualize(PxConstraintVisualizer& viz,
...
PxConstraintShaderTable Ext::PortalJoint::sShaders = {PortalJointSolverPrep, PortalJointProject, PortalJointVisualize,
