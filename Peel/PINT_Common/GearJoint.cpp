...
void GearJoint::updateError() {
    ...
    if (!hingeJoint0 || !hingeJoint1) return;
...
void GearJoint::importExtraData(PxDeserializationContext& context) {
    if (mData) mData = context.readExtraData<GearJointData, PX_SERIAL_ALIGN>();
...
PxConstraintShaderTable Ext::GearJoint::sShaders = {GearJointSolverPrep, GearJointProject, GearJointVisualize,
