...
static void setupCommonLinkData(PxFeatherstoneArticulationLinkData& data,
...
static void SetModeAndLimits(PxFeatherstoneArticulationLinkData& data,
                             PxArticulationAxis::Enum axis,
                             float min_limit,
                             float max_limit) {
    if (min_limit > max_limit) data.inboundJoint.motion[axis] = PxArticulationMotion::eFREE;
    if (min_limit == max_limit) data.inboundJoint.motion[axis] = PxArticulationMotion::eLOCKED;
...
static void setupJointLinkData(PxFeatherstoneArticulationLinkData& data, const PINT_RC_ARTICULATED_BODY_CREATE& bc) {
    if (bc.mJointType == PINT_JOINT_SPHERICAL) {
        ...
        if (bc.mUseMotor) {
            ...
            data.inboundJoint.drives[Axis].driveType = bc.mMotor.mAccelerationDrive
...
void PhysXImm::SetGravity(const Point& gravity) { gGravity = ToPxVec3(gravity); }
...
udword PhysXImm::Update(float dt) {
    if (!mScene) return gDefaultAllocator->mCurrentMemory;
...

    {
        ...
        time = __rdtsc() - time;
        if (printTimings) printf("updateArticulations: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->updateBounds(gBoundsInflation);
        time = __rdtsc() - time;
        if (printTimings) printf("updateBounds: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->broadPhase();
        time = __rdtsc() - time;
        if (printTimings) printf("broadPhase: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->narrowPhase(gContactDistance, gMeshContactMargin, gToleranceLength, gStaticFriction, gDynamicFriction,
                            gRestitution);
        time = __rdtsc() - time;
        if (printTimings) printf("narrowPhase: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->buildSolverBodyData(dt, gGravity, gMaxDepenetrationVelocity, gMaxContactImpulse, gLinearDamping,
                                    gAngularDamping, gMaxLinearVelocity, gMaxAngularVelocity);
        time = __rdtsc() - time;
        if (printTimings) printf("buildSolverBodyData: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->buildSolverConstraintDesc();
        time = __rdtsc() - time;
        if (printTimings) printf("buildSolverConstraintDesc: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->createContactConstraints(dt, invDt, gBounceThreshold, gFrictionOffsetThreshold, gCorrelationDistance,
                                         1.0f, gNbIterPos);
        time = __rdtsc() - time;
        if (printTimings) printf("createContactConstraints: %d           \n", PxU32(time / 1024));
    }
    {
        unsigned long long time = __rdtsc();
        mScene->solveAndIntegrate(dt, gNbIterPos, gNbIterVel);
        time = __rdtsc() - time;
        if (printTimings) printf("solveAndIntegrate: %d           \n", PxU32(time / 1024));
    }

    return gDefaultAllocator->mCurrentMemory;
}

Point PhysXImm::GetMainColor() { return Point(238.0f / 255.0f, 223.0f / 255.0f, 204.0f / 255.0f); }
...
static ImmActorHandle CreateActor(const PINT_OBJECT_CREATE& desc,
...
PxConvexMesh* PhysXImm::CreatePhysXConvex(udword nb_verts, const Point* verts, PxConvexFlags flags) {
    ...
    if (!mCooking->cookConvexMesh(ConvexDesc, buf)) return null;
...
#ifdef IMM_NEEDS_PX_PHYSICS
    MemoryOutputStream buf;
    if (!mCooking->cookTriangleMesh(MeshDesc, buf)) return null;
...
void PhysXImm::ReportShape(const PINT_SHAPE_CREATE& create, udword index, void* user_data) {
    ...
    if (create.mType == PINT_SHAPE_SPHERE) {
        ...
        //			shape = CreateBoxShape(CurrentShape, actor, PxBoxGeometry(ToPxVec3(BoxCreate->mExtents)),
        //*ShapeMaterial, LocalPose, desc.mCollisionGroup); 			ASSERT(0);
        ...
        PxConvexMesh* ConvexMesh = CreateConvexMesh(ConvexCreate->mVerts, ConvexCreate->mNbVerts,
                                                    PxConvexFlag::eCOMPUTE_CONVEX, create.mRenderer, share_meshes);
        ...
        PxTriangleMesh* TriangleMesh = CreateTriangleMesh(MeshCreate->GetSurface(), create.mRenderer,
                                                          MeshCreate->mDeformable, share_meshes, dynamic);
    ...
    while (CurrentShape) {
        ...
        if (CurrentShape->mType == PINT_SHAPE_SPHERE) {
            ...
            PxConvexMesh* ConvexMesh =
                    CreateConvexMesh(ConvexCreate->mVerts, ConvexCreate->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX,
                                     CurrentShape->mRenderer, share_meshes);
            ...
            PxTriangleMesh* TriangleMesh = CreateTriangleMesh(MeshCreate->mSurface, CurrentShape->mRenderer,
                                                              MeshCreate->mDeformable, share_meshes);
...
bool PhysXImm::ReleaseObject(PintActorHandle handle) { return false; }
...
PintJointHandle PhysXImm::CreateJoint(const PINT_JOINT_CREATE& desc) {
    ...
    switch (desc.mType) {
        case PINT_JOINT_SPHERICAL: {
            ...
            mScene->createSphericalJoint(id0, id1, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos)),
                                         PxTransform(ToPxVec3(jc.mLocalPivot1.mPos)), &mScene->getActorGlobalPose(id0),
                                         &mScene->getActorGlobalPose(id1));
        } break;
    }
...
PintActorHandle PhysXImm::CreateRCArticulatedObject(const PINT_OBJECT_CREATE& oc,
    ...
    //			ImmActorHandle linkID = mScene->createActor(shapeHandle, pose, oc.mCollisionGroup, &massProps,
    //link);
    ImmActorHandle linkID = mScene->createActor(NbShapes, Shapes, pose, oc.mCollisionGroup, &props, link);

    if (parentID != ImmActorHandle(0xffffffff)) mScene->disableCollision(parentID, linkID);  // ###TODO: revisit this
...
udword PhysXImm::BatchRaycasts(PintSQThreadContext context,
                               udword nb,
                               PintRaycastHit* dest,
...
IceWindow* PhysXImm_InitGUI(IceWidget* parent, PintGUIHelper& helper) {
...

    {
        ...
        helper.CreateCheckBox(Main, IMM_GUI_ENABLE_PERSISTENT_CONTACTS, 4, y, CheckBoxWidth, 20, "Persistent contacts",
                              gImmGUI, gUsePersistentContacts, gCheckBoxCallback);
        y += YStepCB;

        helper.CreateCheckBox(Main, IMM_GUI_ENABLE_BATCHED_CONTACTS, 4, y, CheckBoxWidth, 20, "Batch contacts", gImmGUI,
                              gBatchContacts, gCheckBoxCallback);
        y += YStepCB;

        helper.CreateCheckBox(Main, IMM_GUI_ENABLE_TGS, 4, y, CheckBoxWidth, 20, "TGS", gImmGUI, gUseTGS,
                              gCheckBoxCallback);
        y += YStepCB;
    }
    Common_CreateDebugVizUI(Main, 290, 20, gCheckBoxCallback, IMM_GUI_ENABLE_DEBUG_VIZ, NB_DEBUG_VIZ_PARAMS,
                            gDebugVizParams, gDebugVizNames, gCheckBox_DebugVis, gImmGUI);
    ...
    /*	const sdword OffsetX = 90;
            ...
            const sdword ColumnOffsetX = 200;
            {
                    ...
                    gEditBox_SolverIter = helper.CreateEditBox(Main, BULLET_GUI_SOLVER_ITER, 4+OffsetX, y, EditBoxWidth,
       20, _F("%d", gSolverIterationCount), gBulletGUI, EDITBOX_INTEGER_POSITIVE, null);

                            helper.CreateLabel(Main, 4+ColumnOffsetX, y+LabelOffsetY, LabelWidth, 20, "Default
       friction:", gBulletGUI); gEditBox_DefaultFriction = helper.CreateEditBox(Main, BULLET_GUI_DEFAULT_FRICTION,
       4+OffsetX+ColumnOffsetX, y, EditBoxWidth, 20, helper.Convert(gDefaultFriction), gBulletGUI,
       EDITBOX_FLOAT_POSITIVE, null);

                    y += YStep;

                    helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "Linear damping:", gBulletGUI);
                    gEditBox_LinearDamping = helper.CreateEditBox(Main, BULLET_GUI_LINEAR_DAMPING, 4+OffsetX, y,
       EditBoxWidth, 20, helper.Convert(gLinearDamping), gBulletGUI, EDITBOX_FLOAT_POSITIVE, null); y += YStep;

                    helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "Angular damping:", gBulletGUI);
                    gEditBox_AngularDamping = helper.CreateEditBox(Main, BULLET_GUI_ANGULAR_DAMPING, 4+OffsetX, y,
       EditBoxWidth, 20, helper.Convert(gAngularDamping), gBulletGUI, EDITBOX_FLOAT_POSITIVE, null); y += YStep;

                    helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "ERP:", gBulletGUI);
                    gEditBox_Erp = helper.CreateEditBox(Main, BULLET_GUI_ERP, 4+OffsetX, y, EditBoxWidth, 20,
       helper.Convert(gErp), gBulletGUI, EDITBOX_FLOAT, null); y += YStep;

                    helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "ERP2:", gBulletGUI);
                    gEditBox_Erp2 = helper.CreateEditBox(Main, BULLET_GUI_ERP2, 4+OffsetX, y, EditBoxWidth, 20,
       helper.Convert(gErp2), gBulletGUI, EDITBOX_FLOAT, null); y += YStep;

                    helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "Tau:", gBulletGUI);
                    gEditBox_Tau = helper.CreateEditBox(Main, BULLET_GUI_TAU, 4+OffsetX, y, EditBoxWidth, 20,
       helper.Convert(gTau), gBulletGUI, EDITBOX_FLOAT, null); y += YStep;

                    helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "Collision margin:", gBulletGUI);
                    gEditBox_CollisionMargin = helper.CreateEditBox(Main, BULLET_GUI_COLLISION_MARGIN, 4+OffsetX, y,
       EditBoxWidth, 20, helper.Convert(gCollisionMargin), gBulletGUI, EDITBOX_FLOAT_POSITIVE, null); y += YStep;

                    {
                            ...
                            helper.CreateLabel(Main, 4, y+LabelOffsetY, LabelWidth, 20, "Gyroscopic force:",
       gBulletGUI); CBBD.mID		= BULLET_GUI_GYRO; CBBD.mY			= y;
...
void PhysXImm_CloseGUI() {
    ...
    /*	gEditBox_SolverIter = null;
            ...
            gComboBox_Gyro = null;*/
    for (udword i = 0; i < NB_DEBUG_VIZ_PARAMS; i++) gCheckBox_DebugVis[i] = null;
}
