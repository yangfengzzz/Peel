...
udword PhysX::BatchRaycasts(PintSQThreadContext context,
                            udword nb,
                            PintRaycastHit* dest,
...
#define BATCH_SIZE 32
        ...
        while (nb) {
            ...
            udword NbBatchHits = mScene->raycasts(NbToGo, (const PxVec3*)Origins, (const PxVec3*)Dirs, MaxDists, buf,
                                                  sqFlags, fd1, GetSQFilterCallback());
        ...
        while (nb--) {
            ...
            if (mScene->raycast((const PxVec3&)(raycasts->mOrigin), (const PxVec3&)(raycasts->mDir), raycasts->mMaxDist,
                                buf, sqFlags, fd1, GetSQFilterCallback(), null
...
udword PhysX::BatchRaycastAll(PintSQThreadContext context,
                              udword nb,
                              PintRaycastAllHit* dest,
                              Container& stream,
                              const PintRaycastData* raycasts) {
    ASSERT(mScene);
    ...
    udword NbHits = 0;
    while (nb--) {
        const PxI32 CurrentNbHits = raycastMultiple(mScene, ToPxVec3(raycasts->mOrigin), ToPxVec3(raycasts->mDir),
                                                    raycasts->mMaxDist, sqFlags, hitBuffer, hitBufferSize, blockingHit,
                                                    PF, GetSQFilterCallback() /*, &gQueryFilterCallback*/);
        ...
        if (CurrentNbHits) {
            PintRaycastHit* buffer =
                    (PintRaycastHit*)stream.Reserve(CurrentNbHits * (sizeof(PintRaycastHit) / sizeof(udword)));
...
udword PhysX::BatchBoxSweeps(PintSQThreadContext context,
                             udword nb,
                             PintRaycastHit* dest,
    ...
    while (nb--) {
        ...
        PxSweepHit Hit;
        //		if(mScene->sweepAny(PxBoxGeometry(ToPxVec3(sweeps->mBox.mExtents)), Pose, ToPxVec3(sweeps->mDir),
        //sweeps->mMaxDist, sweepQueryFlags, Hit, PF))
        if (sweepSingle(mScene, Geom, Pose, ToPxVec3(sweeps->mDir), sweeps->mMaxDist, sweepQueryFlags, Hit, PF,
                        GetSQFilterCallback())) {
            NbHits++;
            FillResultStruct(*dest, Hit);
        } else {
...
udword PhysX::BatchSphereSweeps(PintSQThreadContext context,
                                udword nb,
                                PintRaycastHit* dest,
    ...
    while (nb--) {
        ...
        PxSweepHit Hit;
        if (sweepSingle(mScene, PxSphereGeometry(sweeps->mSphere.mRadius), Pose, ToPxVec3(sweeps->mDir),
                        sweeps->mMaxDist, sweepQueryFlags, Hit, PF, GetSQFilterCallback())) {
            NbHits++;
            FillResultStruct(*dest, Hit);
        } else {
            dest->SetNoHit();
...
udword PhysX::BatchCapsuleSweeps(PintSQThreadContext context,
                                 udword nb,
                                 PintRaycastHit* dest,
...
udword PhysX::BatchConvexSweeps(PintSQThreadContext context,
                                udword nb,
                                PintRaycastHit* dest,
