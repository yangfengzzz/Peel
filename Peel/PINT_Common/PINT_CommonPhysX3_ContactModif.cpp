...
PEEL_ContactModifyCallback::PEEL_ContactModifyCallback()
    : mOwner(null), mPairs(null), mNbPairs(0), mContactModifyCallback(null), mContactModifyCallback2(null) {}
...
PintContactModifyPairHandle PEEL_ContactModifyCallback::GetContactPairData(udword pair_index,
                                                                           ContactPairData& data,
                                                                           PR* pose0,
                                                                           PR* pose1) {
    ASSERT(mPairs);
    ASSERT(pair_index < mNbPairs);
    if (pair_index >= mNbPairs) return null;
    ...
    if (pose0) *pose0 = ToPR(cp.transform[0]);
    if (pose1) *pose1 = ToPR(cp.transform[1]);
    return PintContactModifyPairHandle(&cp);
}

void PEEL_ContactModifyCallback::GetContactData(PintContactModifyPairHandle handle,
                                                ContactData* data,
                                                udword nb_contacts) {
    ASSERT(handle);
    ...
    for (PxU32 j = 0; j < nb_contacts; j++) {
        ...
        data[j].mInternalFaceIndex1 =
                const_cast<PxContactSet&>(set).getInternalFaceIndex1(j);  // ### const missing in PhysX
    }
}
...
void PEEL_ContactModifyCallback::SetContactData(PintContactModifyPairHandle handle,
                                                udword contact_index,
...
void PEEL_ContactModifyCallback::onContactModify(PxContactModifyPair* const pairs, PxU32 count) {
    ...
    if (!mContactModifyCallback && !mContactModifyCallback2) return;
    ...
    if (mContactModifyCallback) {
        struct DelayedContact {
            PX_FORCE_INLINE DelayedContact(PxU32 i, PxU32 j, PxU32 k)
                : mPairIndex(i), mContactIndex(j), mUserIndex(k) {}
            PxU32 mPairIndex;
            PxU32 mContactIndex;
            PxU32 mUserIndex;
        };
        ...
        for (PxU32 i = 0; i < count; i++) {
            ...
            if (mContactModifyCallback->PrepContactModify(*mOwner, nbContacts, PintActorHandle(actor0),
                                                          PintActorHandle(actor1), PintShapeHandle(shape0),
                                                          PintShapeHandle(shape1))) {
                ...
                for (PxU32 j = 0; j < nbContacts; j++) {
                    ...
                    const PintContactModifyCallback::ContactModif Action = mContactModifyCallback->ModifyContact(
                            *mOwner, IcePose0, IcePose1, p, n, s, feature0, feature1);
        ...
        for (PxU32 i = 0; i < nbDelayed; i++) {
            ...
            if (mContactModifyCallback->PrepContactModify(*mOwner, nbContacts, PintActorHandle(actor0),
                ...
                const PxU32 j = dc.mContactIndex;
                {
                    ...
                    const PintContactModifyCallback::ContactModif Action = mContactModifyCallback->ModifyDelayedContact(
                            *mOwner, IcePose0, IcePose1, p, n, s, feature0, feature1, dc.mUserIndex);
                    if (Action == PintContactModifyCallback::CONTACT_MODIFY) {
                        set.setPoint(j, ToPxVec3(p));
                        set.setNormal(j, ToPxVec3(n));
                        set.setSeparation(j, s);
