...
SampleVehicleWheelQueryResults* SampleVehicleWheelQueryResults::allocate(const PxU32 maxNumWheels) {
    ...
    //	SampleVehicleWheelQueryResults* resData = (SampleVehicleWheelQueryResults*)PX_ALLOC(size,
    //PX_DEBUG_EXP("SampleVehicleWheelQueryResults"));
    SampleVehicleWheelQueryResults* resData = (SampleVehicleWheelQueryResults*)gDefaultAllocator->allocate(
            size, "SampleVehicleWheelQueryResults", __FILE__, __LINE__);
    resData->init();
...
SampleVehicleSceneQueryData* SampleVehicleSceneQueryData::allocate(const PxU32 maxNumWheels, const bool use_sweeps) {
    ...
    SampleVehicleSceneQueryData* sqData = (SampleVehicleSceneQueryData*)gDefaultAllocator->allocate(
            size, "PxVehicleNWSceneQueryData", __FILE__, __LINE__);
...
static PxQueryHitType::Enum MySampleVehicleWheelRaycastPreFilter(PxFilterData filterData0,
    ...
    //	return ((0 == (filterData1.word3 & SAMPLEVEHICLE_DRIVABLE_SURFACE)) ? PxQueryHitType::eNONE :
    //PxQueryHitType::eBLOCK);
    return PxQueryHitType::eBLOCK;
}

static inline_ PxQueryHitType::Enum _preFilter(const PxFilterData& filterData,
                                               const PxShape* shape,
                                               const PxRigidActor* actor,
                                               PxHitFlags& queryFlags,
                                               bool useBlockingSweeps) {
#ifdef DISABLE_VEHICLE_SQ_VS_DYNAMICS
    if (actor->getConcreteType() == PxConcreteType::eRIGID_DYNAMIC) return PxQueryHitType::eNONE;
#endif
    ...
    if (shapeName) {
        ...
        {
            if (shapeName == gChassisDebugName || shapeName == gWheelDebugName) return PxQueryHitType::eNONE;
        }
        /*else
        {
...
public:
    ...
    virtual PxBatchQueryPostFilterShader getPostFilterShader() const { return null; }
    ...
    virtual void setUserMemory(const PxBatchQueryMemory&) { ASSERT(0); }
    ...
    virtual void release() { mRecorder.Release(); }
    ...
    virtual void raycast(const PxVec3& origin,
                         ...
                         const PxQueryFilterData& filterData = PxQueryFilterData(),
                         void* userData = NULL,
                         const PxQueryCache* cache = NULL) const {
        const_cast<MyBatchQuery*>(this)->raycast(origin, unitDir, distance, maxTouchHits, hitFlags, filterData,
                                                 userData, cache);
    }

    virtual void overlap(const PxGeometry& geometry,
    ...
    virtual void sweep(const PxGeometry& geometry,
                       const PxTransform& pose,
                       const PxVec3& unitDir,
        ...
        if (mUseBlockingSweeps) {
            PxSweepHit hit;
            if (sweepSingle(mScene, geometry, pose, unitDir, distance, hitFlags, hit, filterDataCopy, this, cache,
                            PX_DEFAULT_CLIENT, inflation)) {
            ...
            PxI32 nbHits =
                    sweepMultiple(mScene, geometry, pose, unitDir, distance, hitFlags, hits, NB_SWEEP_HITS_PER_WHEEL,
                                  blockingHit, filterDataCopy, this, cache, PX_DEFAULT_CLIENT, inflation);
            ASSERT(!blockingHit);
            mDesc.queryMemory.userSweepResultBuffer[mNb].hasBlock = false;
            mDesc.queryMemory.userSweepResultBuffer[mNb].nbTouches = nbHits;
            // mDesc.queryMemory.userSweepResultBuffer[mNb].touches =
            // gSqData->mSweepSqHitBuffer[mNb*NB_SWEEP_HITS_PER_WHEEL];
    ...
    virtual void sweep(const PxGeometry& geometry,
        ...
        const_cast<MyBatchQuery*>(this)->sweep(geometry, pose, unitDir, distance, maxTouchHits, hitFlags, filterData,
                                               userData, cache, inflation);
    }
...
static const PxF32 gDefaultSteerVsForwardSpeedData[2 * 8] = {
        0.0f,       1.0f,       5.0f,       0.75f,      30.0f,      0.125f,     120.0f,     0.1f,
        PX_MAX_F32, PX_MAX_F32, PX_MAX_F32, PX_MAX_F32, PX_MAX_F32, PX_MAX_F32, PX_MAX_F32, PX_MAX_F32};
...
public:
    ...
    void setCarKeyboardInputs(
            bool accel, bool brake, bool handbrake, bool steerleft, bool steerright, bool gearup, bool geardown) {
...
public:
    VehicleData() : mVehicle(null), mThresholdForwardSpeed(0.1f), mImmediateAutoReverse(false) {}
    ~VehicleData() {}
...
VehicleController::VehicleController() { clear(); }
...
void VehicleController::processRawInputs(const PxF32 dtime,
                                         const bool useAutoGears,
                                         PxVehicleDrive4WRawInputData& rawInputData) {
    // Switch to keyboard use if a key is pressed, switch to gamepad use if the gamepad is used
    {
        const bool useKeyboard =
                (mKeyPressedAccel || mKeyPressedBrake || mKeyPressedHandbrake || mKeyPressedSteerLeft ||
                 mKeyPressedSteerRight || mKeyPressedGearUp || mKeyPressedGearDown);
        if (useKeyboard) gUseKeyboard = true;

        const bool useGamepad = ((mGamepadAccel + mGamepadCarBrake + mGamepadCarSteer) != 0.0f || mGamepadGearup ||
                                 mGamepadGeardown || mGamepadCarHandbrake);
        if (useGamepad) gUseKeyboard = false;
    ...
    // Gamepad
    {
        if (mGamepadAccel < 0.0f || mGamepadAccel > 1.01f)
            // getSampleErrorCallback().reportError(PxErrorCode::eINTERNAL_ERROR, "Illegal accel value from gamepad",
            // __FILE__, __LINE__);
            ASSERT(0);

        if (mGamepadCarBrake < 0.0f || mGamepadCarBrake > 1.01f)
            //			getSampleErrorCallback().reportError(PxErrorCode::eINTERNAL_ERROR, "Illegal brake value from
            //gamepad", __FILE__, __LINE__);
            ASSERT(0);

        if (PxAbs(mGamepadCarSteer) > 1.01f)
            //			getSampleErrorCallback().reportError(PxErrorCode::eINTERNAL_ERROR, "Illegal steer value from
            //gamepad", __FILE__, __LINE__);
            ASSERT(0);
...
void VehicleController::processAutoReverse(const VehicleData& data,
                                           const PxVehicleWheels& focusVehicle,
                                           const PxVehicleDriveDynData& driveDynData,
                                           const PxVehicleWheelQueryResult& vehicleWheelQueryResults,
    ...
    if (!driveDynData.getUseAutoGears()) return;

    //	printf("ThresholdForwardSpeed: %f\n", data.mThresholdForwardSpeed);
    //	printf("mImmediateAutoReverse: %d\n", data.mImmediateAutoReverse);

    // If the car is travelling very slowly in forward gear without player input and the player subsequently presses the
    // brake then we want the car to go into reverse gear If the car is travelling very slowly in reverse gear without
    // player input and the player subsequently presses the accel then we want the car to go into forward gear If the
    // car is in forward gear and is travelling backwards then we want to automatically put the car into reverse gear.
    // If the car is in reverse gear and is travelling forwards then we want to automatically put the car into forward
    // gear. (If the player brings the car to rest with the brake the player needs to release the brake then reapply it
    ...
    if (!isInAir) {
        ...
        if (isMovingBackwards) {
            ...
            if ((currentGear > PxVehicleGearsData::eNEUTRAL) && brakeRaw && !accelRaw && (currentGear == targetGear)) {
                // The car was moving slowly in forward gear without player input and is now moving slowly with player
                // input that indicates the player wants to switch to reverse gear.
                toggleAutoReverse = true;
                // printf("CHECKPOINT1\n");
            } else if (currentGear == PxVehicleGearsData::eREVERSE && accelRaw && !brakeRaw &&
                       (currentGear == targetGear)) {
                // The car was moving slowly in reverse gear without player input and is now moving slowly with player
                // input that indicates the player wants to switch to forward gear.
                toggleAutoReverse = true;
                // printf("CHECKPOINT2\n");
            }
        }
        ...
        if (isMovingForwardSlowly) newIsMovingForwardSlowly = true;
    }
...
void VehicleController::update(const PxF32 timestep,
                               const PxVehicleWheelQueryResult& vehicleWheelQueryResults,
                               PxVehicleWheels& focusVehicle,
                               const VehicleData& data) {
    ...
    processAutoReverse(data, focusVehicle, *driveDynData, vehicleWheelQueryResults, carRawInputs, toggleAutoReverse,
                       newIsMovingForwardSlowly);
...

    {
        ...
        if (gUseKeyboard) {
            PxVehicleDrive4WSmoothDigitalRawInputsAndSetAnalogInputs(gKeySmoothingData, gSteerVsForwardSpeedTable,
                                                                     carRawInputs, timestep, isInAir, vehDrive4W
#if PHYSX_SUPPORT_STEER_FILTER
                                                                     ,
                                                                     data.mSteerFilter
#endif
...
SampleVehicle_VehicleManager::SampleVehicle_VehicleManager()
    ...
    for (PxU32 i = 0; i < MAX_NUM_4W_VEHICLES; i++) mVehicles[i] = NULL;
}

SampleVehicle_VehicleManager::~SampleVehicle_VehicleManager() {}

void SampleVehicle_VehicleManager::init(PxPhysics& physics,
                                        PxScene& scene,
                                        const PxMaterial** drivableSurfaceMaterials,
                                        const PxVehicleDrivableSurfaceType* drivableSurfaceTypes,
                                        const bool use_sweeps) {
    // Initialise the sdk.
...
#if PHYSX_SUPPORT_VEHICLE5
    //	mBatchQueryExt = createMyBatchQueryExt(scene, &gMyFilter, MAX_NUM_4W_VEHICLES*4, MAX_NUM_4W_VEHICLES*4,
    //MAX_NUM_4W_VEHICLES*4, MAX_NUM_4W_VEHICLES*4, 0, 0);
    mBatchQueryExt = createMyBatchQueryExt(scene, &gMyFilter, MAX_NUM_4W_VEHICLES * 4, 0, MAX_NUM_4W_VEHICLES * 4,
                                           MAX_NUM_4W_VEHICLES * 4 * NB_SWEEP_HITS_PER_WHEEL, 0, 0);
#else
    ...
    //	mSurfaceTirePairs = PxVehicleDrivableSurfaceToTireFrictionPairs::allocate(MAX_NUM_TIRE_TYPES,
    //MAX_NUM_SURFACE_TYPES); 	mSurfaceTirePairs->setup(MAX_NUM_TIRE_TYPES, MAX_NUM_SURFACE_TYPES,
...
void SampleVehicle_VehicleManager::suspensionRaycasts(PxScene* scene, const bool use_sweeps, float sweep_inflation) {
    ...
    if (use_sweeps) {
        ...
        PxVehicleSuspensionSweeps(mBatchQueryExt, mNumVehicles, mVehicles, NB_SWEEP_HITS_PER_WHEEL, null,
                                  sweepWidthScale, sweepRadiusScale, sweep_inflation);
#else
    ...
    if (!mSqWheelRaycastBatchQuery) mSqWheelRaycastBatchQuery = mSqData->setUpBatchedSceneQuery(scene, use_sweeps);
...
#if PHYSX_SUPPORT_VEHICLE_SUSPENSION_SWEEPS
#if PHYSX_SUPPORT_VEHICLE_SWEEP_INFLATION
            PxVehicleSuspensionSweeps(mSqWheelRaycastBatchQuery, mNumVehicles, mVehicles,
                                      mSqData->getSweepQueryResultBufferSize(), mSqData->getSweepQueryResultBuffer(),
                                      NB_SWEEP_HITS_PER_WHEEL, null, sweepWidthScale, sweepRadiusScale,
                                      sweep_inflation);
#else
            PxVehicleSuspensionSweeps(mSqWheelRaycastBatchQuery, mNumVehicles, mVehicles,
                                      mSqData->getSweepQueryResultBufferSize(), mSqData->getSweepQueryResultBuffer(),
                                      NB_SWEEP_HITS_PER_WHEEL
                                      //			,null, 0.5f, 0.5f
            );
#endif
#endif
...
void SampleVehicle_VehicleManager::updateAndRecordTelemetryData(const PxF32 timestep,
    ...
    PxVehicleUpdateSingleVehicleAndStoreTelemetryData(timestep, gravity, *mSurfaceTirePairs, focusVehicle,
                                                      focusVehicleWheelQueryResults, *telemetryData);
...
static void SetupVehicleShape(PxShape* shape,
                              const char* name,
                              const PxTransform& local_pose,
                              PintCollisionGroup group) {
    const EditableParams& Params = PhysX3::GetEditableParams();

    // We don't disable eSCENE_QUERY_SHAPE entirely, otherwise we couldn't pick the vehicle with the mouse or raytrace
    // it. Instead, the filtering (to make sure that the wheel's raycasts/sweeps ignore the wheels themselves and the
    // chassis) is done in the pre-filter during hardcoded chassis & wheel names.
    //
    ...
    if (Params.mSQFlag) shape->setQueryFilterData(PxFilterData(1, 0, 0, 0));

    if (gSetupCollisionGroups) PhysX3_SetGroup(*shape, group);
}
...
static void SetupVehicleWheelShape(SharedPhysX& physx, PxShape* shape, const PxTransform& local_pose, PxU32 wheel_id) {
    ...
    if (gSetupCollisionGroups) PhysX3_SetGroupCollisionFlag(gChassisCollisionGroup, gWheelCollisionGroup, false);
...
static void setupActor(SharedPhysX& physx,
    ...
    for (PxU32 i = 0; i < numWheelGeometries; i++) {
        ...
        //		printf("wheelLocalPoses[i]: %f %f %f\n", wheelLocalPoses[i].p.x, wheelLocalPoses[i].p.y,
        //wheelLocalPoses[i].p.z);
    }
    ...
    for (PxU32 i = 0; i < numChassisGeometries; i++) {
        ...
        PxShape* chassisShape =
                PxRigidActorExt::createExclusiveShape(*vehActor, *chassisGeometries[i], *chassisMaterial);
#else
...
static PxRigidDynamic* createVehicleActor4W(SharedPhysX& physx,
                                            const PxVehicleChassisData& chassisData,
    ...
    // We need to add chassis collision shapes, their local poses, a material for the chassis, and a simulation filter
    // for the chassis.
    ...
    if (useSphereWheels) {
        const Sphere S0(wheelConvexMeshes[0]->getNbVertices(),
                        reinterpret_cast<const Point*>(wheelConvexMeshes[0]->getVertices()));
        const Sphere S1(wheelConvexMeshes[1]->getNbVertices(),
                        reinterpret_cast<const Point*>(wheelConvexMeshes[1]->getVertices()));
        const Sphere S2(wheelConvexMeshes[2]->getNbVertices(),
                        reinterpret_cast<const Point*>(wheelConvexMeshes[2]->getVertices()));
        const Sphere S3(wheelConvexMeshes[3]->getNbVertices(),
                        reinterpret_cast<const Point*>(wheelConvexMeshes[3]->getVertices()));
        ...
        const PxTransform wheelLocalPoses[4] = {PxTransform(ToPxVec3(S0.mCenter)), PxTransform(ToPxVec3(S1.mCenter)),
                                                PxTransform(ToPxVec3(S2.mCenter)), PxTransform(ToPxVec3(S3.mCenter))};

        // Set up the physx rigid body actor with shapes, local poses, and filters.
        setupActor(physx, vehActor, wheelGeometries, wheelLocalPoses, 4, &wheelMaterial, chassisGeoms,
                   chassisLocalPoses, 1, &chassisMaterial, chassisData, &physics);

        return vehActor;
    } else {
        // We need to add wheel collision shapes, their local poses, a material for the wheels, and a simulation filter
        // for the wheels.
        ...
        const PxGeometry* wheelGeometries[4] = {&frontLeftWheelGeom, &frontRightWheelGeom, &rearLeftWheelGeom,
                                                &rearRightWheelGeom};
        const PxTransform wheelLocalPoses[4] = {PxTransform(PxIdentity), PxTransform(PxIdentity),
...
static void computeWheelWidthsAndRadii(PxU32 nbWheels,
                                       PxConvexMesh** wheelConvexMeshes,
                                       PxF32* wheelWidths,
                                       PxF32* wheelRadii) {
    for (PxU32 i = 0; i < nbWheels; i++) {
        if (1) {
            AABB Box;
            ComputeAABB(Box, reinterpret_cast<const Point*>(wheelConvexMeshes[i]->getVertices()),
                        wheelConvexMeshes[i]->getNbVertices());
...
static void createVehicle4WSimulationData(udword vehicle_id,
                                          PxConvexMesh* chassisConvexMesh,
                                          PxConvexMesh** wheelConvexMeshes,
...
// #define DEFAULT_CODE
    ...
    PxVec3 chassisMOI((chassisDims.y * chassisDims.y + chassisDims.z * chassisDims.z) * desc.mChassisMass / 12.0f,
    ...
    //		printf("chassisData.mCMOffset: %f %f %f\n", chassisData.mCMOffset.x, chassisData.mCMOffset.y,
    //chassisData.mCMOffset.z);
    ...
    susps[PxVehicleDrive4WWheelOrder::eFRONT_LEFT].mSprungMass =
            suspSprungMasses[PxVehicleDrive4WWheelOrder::eFRONT_LEFT];
    susps[PxVehicleDrive4WWheelOrder::eFRONT_RIGHT].mSprungMass =
            suspSprungMasses[PxVehicleDrive4WWheelOrder::eFRONT_RIGHT];
    susps[PxVehicleDrive4WWheelOrder::eREAR_LEFT].mSprungMass =
            suspSprungMasses[PxVehicleDrive4WWheelOrder::eREAR_LEFT];
    susps[PxVehicleDrive4WWheelOrder::eREAR_RIGHT].mSprungMass =
            suspSprungMasses[PxVehicleDrive4WWheelOrder::eREAR_RIGHT];

    // Set up the camber.
    // Remember that the left and right wheels need opposite camber so that the car preserves symmetry about the forward
    // direction. Set the camber to 0.0f when the spring is neither compressed or elongated.
    ...
    // We already know the wheel centers described as offsets from the actor center and the center of mass offset from
    // actor center. From here we can approximate application points for the tire and suspension forces. Lets assume
    // that the suspension travel directions are absolutely vertical. Also assume that we apply the tire and suspension
    // forces 30cm below the center of mass.
    ...
    ackermann.mAxleSeparation = fabsf(wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eFRONT_LEFT].z -
                                      wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eREAR_LEFT].z);
    ackermann.mFrontWidth = fabsf(wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eFRONT_RIGHT].x -
                                  wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eFRONT_LEFT].x);
    ackermann.mRearWidth = fabsf(wheelCentreOffsets[PxVehicleDrive4WWheelOrder::eREAR_RIGHT].x -
...
PxVehicleDrive4W* SampleVehicle_VehicleManager::create4WVehicle(SharedPhysX_Vehicles& sharedPhysX,
    ...
    // void SampleVehicle_VehicleManager::setTireFrictionMultiplier(udword vehicle_id, float
    // frontTireFrictionMultiplier, float rearTireFrictionMultiplier)
    {
    ...
    PxVehicleChassisData chassisData;
    createVehicle4WSimulationData(mNumVehicles, chassisConvexMesh, wheelConvexMeshes4, *wheelsSimData, driveSimData,
                                  chassisData, desc);

    // Instantiate and finalize the vehicle using physx.
    PxRigidDynamic* vehActor =
            createVehicleActor4W(sharedPhysX, chassisData, wheelConvexMeshes4, chassisConvexMesh, *scene, physics,
                                 material, ToPxTransform(desc.mChassisLocalPose), useSphereWheels);
    ...
    wheelsSimData->setFlags(PxVehicleWheelsSimFlag::eLIMIT_SUSPENSION_EXPANSION_VELOCITY |
                            PxVehicleWheelsSimFlag::eDISABLE_INTERNAL_CYLINDER_PLANE_INTERSECTION_TEST |
                            PxVehicleWheelsSimFlag::eDISABLE_SUSPENSION_FORCE_PROJECTION);
#endif
...
SharedPhysX_Vehicles::SharedPhysX_Vehicles(const EditableParams& params) : SharedPhysX(params), Pint_Vehicle(*this) {}
...
public:
    ...
    PxVehicleDrive4W* CreateVehicle(VehiclePtrs& vehicle_ptrs,
                                    ...
                                    PxConvexMesh* wheel_mesh0,
                                    PintShapeRenderer* renderer0,
                                    PxConvexMesh* wheel_mesh1,
                                    PintShapeRenderer* renderer1,
                                    PxConvexMesh* wheel_mesh2,
                                    PintShapeRenderer* renderer2,
                                    PxConvexMesh* wheel_mesh3,
                                    PintShapeRenderer* renderer3,
                                    const PINT_VEHICLE_CREATE& desc,
                                    bool useSphereWheels);
    ...
    void drawGraphsAndPrintTireSurfaceTypes(PintRender& renderer,
                                            const PxVehicleWheels& focusVehicle,
                                            const PxVehicleWheelQueryResult& focusVehicleWheelQueryResults);
#endif
...
public:
    virtual void onContactModify(PxContactModifyPair* const pairs, PxU32 count) {
        if (gLogXP) printf("VehicleContactModifyCallback %d\n", gVehicleTimestamp);
            ...
            for (PxU32 j = 0; j < 2; j++) {
                ...
                if (actor->getConcreteType() == PxConcreteType::eRIGID_DYNAMIC) {
                    ...
                    if (isVehicle) {
                        ...
                        {
                            // const PxU32 wheelId = ::atoi(ShapeName+5);	// ### I know.... we need to revisit this at
                            // some point
                            const PxU32 wheelId = isWheel->second;
                            PX_ASSERT(wheelId < vehicle->mWheelsSimData.getNbWheels());

                            // Modify wheel contacts.
                            PxVehicleModifyWheelContacts(*vehicle, wheelId, WHEEL_TANGENT_VELOCITY_MULTIPLIER,
                                                         MAX_IMPULSE, pairs[i]);
                        }
                    }
                }
...
VehicleTest::VehicleTest(SharedPhysX_Vehicles& owner, bool use_sweeps, float sweep_inflation)
    : mOwner(owner), mScene(null), mUseSweeps(use_sweeps), mSweepInflation(sweep_inflation) {
#if PINT_DEBUG_VEHICLE_ON
    ...
    /*	const PxF32 restitutions[MAX_NUM_SURFACE_TYPES] = {0.2f, 0.2f, 0.2f, 0.2f};
            ...
            for(PxU32 i=0;i<MAX_NUM_SURFACE_TYPES;i++)
            {
                    ...
                    mStandardMaterials[i] = physics.createMaterial(staticFrictions[i], dynamicFrictions[i],
       restitutions[i]); ASSERT(mStandardMaterials[i]);
    ...
    mVehicleManager.init(physics, *mScene, (const PxMaterial**)mStandardMaterials, mVehicleDrivableSurfaceTypes,
                         use_sweeps);
...
void VehicleTest::setupTelemetryData() {
    ...
    if (!mTelemetryData4W) {
        ...
        mTelemetryData4W->setup(graphSizeX, graphSizeY, engineGraphPosX, engineGraphPosY, wheelGraphPosX,
                                wheelGraphPosY, backgroundColor, lineColorHigh, lineColorLow);
    }
#endif
}
...
#if PINT_DEBUG_VEHICLE_ON
    if (mTelemetryData4W) mTelemetryData4W->clear();
#endif
}

void VehicleTest::Update(float dt, const PtrContainer& vehicles) {
    if (!gUpdateVehicles) return;
    gVehicleTimestamp++;

    {
        ...
        for (udword i = 0; i < NbVehicles; i++) {
            ...
            VD->mVehicleController.setCarKeyboardInputs(Input.mKeyboard_Accelerate, Input.mKeyboard_Brake,
                                                        Input.mKeyboard_HandBrake,
                                                        // ### TODO: investigate why left/right is flipped here
                                                        Input.mKeyboard_Right, Input.mKeyboard_Left,
                                                        false,  // Gear up key (manual)
                                                        false   // Gear down key (manual)
            );

            VD->mVehicleController.setCarGamepadInputs(Input.mGamepad_Accel, Input.mGamepad_Brake, Input.mGamepad_Steer,
                                                       false,  // Gear up key (manual)
                                                       false,  // Gear down key (manual)
            ...
            VD->mVehicleController.update(dt, mVehicleManager.getVehicleWheelQueryResults(i /*mPlayerVehicle*/),
                                          *mVehicleManager.getVehicle(i /*mPlayerVehicle*/), *VD);
        }
    }
    ...
    if (0) {
...
#if PINT_DEBUG_VEHICLE_ON
                if (mTelemetryData4W)
                    mVehicleManager.updateAndRecordTelemetryData(
                            vdt, mScene->getGravity(), mVehicleManager.getVehicle(mPlayerVehicle), mTelemetryData4W);
#else
        ...
        if (1 && dt > 0.0f) {
            ...
            if (mTelemetryData4W)
                mVehicleManager.updateAndRecordTelemetryData(
                        dt, mScene->getGravity(), mVehicleManager.getVehicle(mPlayerVehicle), mTelemetryData4W);
#else
...
PxVehicleDrive4W* VehicleTest::CreateVehicle(VehiclePtrs& vehicle_ptrs,
    ...
    PxVehicleDrive4W* Vehicle4W =
            mVehicleManager.create4WVehicle(mOwner, *mChassisMaterial, chassis_mesh, wm, true, desc, useSphereWheels);
    ASSERT(Vehicle4W);
    ...
    for (udword i = 0; i < 4; i++) vehicle_ptrs.mWheelhapes[i] = Shapes[i];
    ...
    for (udword i = 0; i < 4; i++) mWheelMap.insert(Shapes[i], i);
...
PintVehicleHandle SharedPhysX_Vehicles::CreateVehicle(PintVehicleData& data, const PINT_VEHICLE_CREATE& vehicle) {
    if (!vehicle.mChassis || vehicle.mChassis->mType != PINT_SHAPE_CONVEX) return null;

    if (!vehicle.mWheels[0] || vehicle.mWheels[0]->mType != PINT_SHAPE_CONVEX) return null;
    ...
    if (!gVehicleTest) gVehicleTest = ICE_NEW(VehicleTest)(*this, UseSweeps, SweepInflation);
    ...
    PxConvexMesh* ChassisMesh = CreateConvexMesh(ChassisDesc->mVerts, ChassisDesc->mNbVerts, convexFlags,
                                                 vehicle.mChassis->mRenderer, mParams.mShareMeshData);
    ASSERT(ChassisMesh);

    const PINT_CONVEX_CREATE* WheelDesc0 = static_cast<const PINT_CONVEX_CREATE*>(vehicle.mWheels[0]);
    PxConvexMesh* WheelMesh0 = CreateConvexMesh(WheelDesc0->mVerts, WheelDesc0->mNbVerts, convexFlags,
                                                WheelDesc0->mRenderer, mParams.mShareMeshData);
    ASSERT(WheelMesh0);
    ...
    if (vehicle.mWheels[1]) {
        ...
        WheelMesh1 = CreateConvexMesh(wheel1->mVerts, wheel1->mNbVerts, convexFlags, wheel1->mRenderer,
                                      mParams.mShareMeshData);
        ASSERT(WheelMesh1);
        ...
        WheelMesh2 = CreateConvexMesh(wheel2->mVerts, wheel2->mNbVerts, convexFlags, wheel2->mRenderer,
                                      mParams.mShareMeshData);
        ASSERT(WheelMesh2);
        ...
        WheelMesh3 = CreateConvexMesh(wheel3->mVerts, wheel3->mNbVerts, convexFlags, wheel3->mRenderer,
                                      mParams.mShareMeshData);
        ASSERT(WheelMesh3);
    ...
    PxVehicleDrive4W* Vehicle4W =
            gVehicleTest->CreateVehicle(VP, ChassisMesh, WheelMesh0, WheelDesc0->mRenderer, WheelMesh1, Renderer1,
                                        WheelMesh2, Renderer2, WheelMesh3, Renderer3, vehicle, forceSphereWheels);
    ASSERT(Vehicle4W);
    ...
    for (udword i = 0; i < 4; i++) data.mWheelShapes[i] = PintShapeHandle(VP.mWheelhapes[i]);
...
bool SharedPhysX_Vehicles::ResetVehicleData(PintVehicleHandle vehicle) {
    ...
    if (gVehicleTest) gVehicleTest->clearTelemetryData();
...
void SharedPhysX_Vehicles::UpdateVehiclesAndPhysX(float dt) {
    ...
    if (gDebugPts) gDebugPts->Reset();
    if (gDebugSegments) gDebugSegments->Reset();
    if (gDebugSegments2) gDebugSegments2->Reset();
...
//	gVehicleXP = false;
...
#if PHYSX_SUPPORT_VEHICLE5
    if (gVehicleTest) gVehicleTest->mVehicleManager.mBatchQueryExt->resetSQRecorder();
#else
    ...
    if (gVehicleTest && !gVehicleTest->mUseSweeps) useBlockingSweeps = true;  // Raycasts don't support touch hits
    ...
    if (0) {
        ...
        const float sdt = dt / float(NbSubsteps);
        for (udword i = 0; i < NbSubsteps; i++) {
            if (0 && gVehicleTest) gVehicleTest->Update(sdt, mVehicles);
...
//				mScene->fetchResults(false);
            ...
            if (1 && gVehicleTest) gVehicleTest->Update(sdt, mVehicles);
...
void drawPt(const PxVec3& pos) {
    if (!gDebugPts) gDebugPts = ICE_NEW(Vertices);
    gDebugPts->AddVertex(ToPoint(pos));
}

void drawSeg(const PxVec3& p0, const PxVec3& p1) {
    if (!gDebugSegments) gDebugSegments = ICE_NEW(Vertices);
...
void drawSeg(const PxVec3& p0, const PxVec3& p1, const PxVec3& color) {
    if (!gDebugSegments2) gDebugSegments2 = ICE_NEW(Vertices);
...
void SharedPhysX_Vehicles::RenderDebugData(PintRender& renderer) {
    ...
    if (gVehicleTest && mParams.mDrawPhysXTelemetry) {
        ...
        gVehicleTest->drawGraphsAndPrintTireSurfaceTypes(
                renderer, *gVehicleTest->mVehicleManager.getVehicle(mPlayerVehicle),
                gVehicleTest->mVehicleManager.getVehicleWheelQueryResults(mPlayerVehicle));
    }
#endif
...
#if PHYSX_SUPPORT_VEHICLE5
    if (gVehicleTest) {
        gVehicleTest->mVehicleManager.mBatchQueryExt->drawSQSweeps(
                renderer, mParams.mDrawSweptWheels && mParams.mUseBlockingSweeps);
...
#else
#if PHYSX_SUPPORT_VEHICLE_SUSPENSION_SWEEPS
    gMyBatchQuery.mRecorder.DrawSweeps(renderer, gMyBatchQuery.mDesc.queryMemory.userSweepResultBuffer,
                                       mParams.mDrawSweptWheels && mParams.mUseBlockingSweeps);
#endif
...
Pint_Vehicle* SharedPhysX_Vehicles::GetVehicleAPI() { return this; }
    ...
    /*	PxSceneReadLock scopedLock(*mScene);
            ...
            for(PxU32 i=0;i<mVehicleManager.getNbVehicles();i++)
            {
                    ...
                    for(PxU32 j=0;j<numWheels;j++)
                    {
                            ...
                            getDebugRenderer()->addLine(wheelTransform.p+offset,
       wheelTransform.p+offset+arrow*wheelRadius, colorPurple);
                    }
            }*/
}
...
void VehicleTest::drawVehicleDebug(PintRender& renderer) {
    ...
    const PxVehicleWheelQueryResult& vehicleWheelQueryResults =
            mVehicleManager.getVehicleWheelQueryResults(mPlayerVehicle);
...
/*static void drawBox2D(Renderer* renderer, PxF32 minX, PxF32 maxX, PxF32 minY, PxF32 maxY, const RendererColor& color,
PxF32 alpha)
{
        ScreenQuad sq;
        sq.mX0				= minX;
...
void VehicleTest::drawHud(PintRender& renderer) {
    ...
    const PxF32 maxRevs =
            driveSimData->getEngineData().mMaxOmega * 60 * 0.5f / PxPi;  // Convert from radians per second to rpm
    ...
    //	drawBox2D(renderer, x-length-textheight, x+length+textheight, y, y+length+textheight, RendererColor(255, 255,
    //255), 0.5f);
    renderer.DrawRectangle2D(x - length - textheight, x + length + textheight, y, y + length + textheight,
                             Point(1.0f, 1.0f, 1.0f), 0.5f);

    // Gear
    char gear[PxVehicleGearsData::eGEARSRATIO_COUNT][64] = {"R", "N", "1", "2", "3", "4", "5"};
...
static void convertY(PxF32* xy) {
    for (PxU32 i = 0; i < PxVehicleGraph::eMAX_NB_SAMPLES; i++) xy[2 * i + 1] = 1.0f - xy[2 * i + 1];
}

void drawGraphsAndPrintTireSurfaceTypesN(const PxVehicleTelemetryData& telemetryData,
                                         const PxU32* tireTypes,
                                         const PxU32* surfaceTypes,
                                         const PxU32 activeEngineGraphChannel,
                                         const PxU32 activeWheelGraphChannel,
                                         PintRender& renderer) {
...
void VehicleTest::drawGraphsAndPrintTireSurfaceTypes(PintRender& renderer,
                                                     const PxVehicleWheels& focusVehicle,
                                                     const PxVehicleWheelQueryResult& focusVehicleWheelQueryResults) {
    PxU32 tireTypes[8];
    ...
    drawGraphsAndPrintTireSurfaceTypesN(*vehTelData, tireTypes, surfaceTypes, mDebugRenderActiveGraphChannelEngine,
                                        mDebugRenderActiveGraphChannelWheel, renderer);
}
