...
void PhysX3_SetFilterCallback(bool enabled) { gFilterCallbackEnabled = enabled; }
...
PxFilterFlags PhysX3_SimulationFilterShader(PxFilterObjectAttributes attributes0,
    ...
    if (gFilterCallbackEnabled) defaultFlags |= PxFilterFlag::eCALLBACK;
    ...
    if (!gCollisionTable[filterData0.word0][filterData1.word0]) return PxFilterFlag::eKILL;
...
void PhysX3_InitFilterShader() {
    ...
    for (PxU16 j = 0; j < PEEL_GROUP_SIZE; j++)
        for (PxU16 i = 0; i < PEEL_GROUP_SIZE; i++) PhysX3_SetGroupCollisionFlag(i, j, true);
}
