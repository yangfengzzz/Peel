...
struct EditableParams {
    ...
    inline_ const char* ThreadIndexToName(udword index) const {
        if (index == 0) return "Single threaded";
        if (index == 1) return "1 main + 1 worker thread";
        if (index == 2) return "1 main + 2 worker threads";
        if (index == 3) return "1 main + 3 worker threads";
        if (index == 4) return "1 main + 4 worker threads";
        if (index == 5) return "1 main + 8 worker threads";
        if (index == 6) return "1 main + 12 worker threads";
        if (index == 7) return "1 main + 15 worker threads";
        return null;
    }

    inline_ udword NbThreadsToThreadIndex(udword nb_threads) const {
        if (nb_threads == 0 || nb_threads == 1) return 0;
        /*if(nb_threads==0)
                return 0;
        if(nb_threads==1)
                return 1;*/
        if (nb_threads == 2) return 2 + 1;
        if (nb_threads <= 4) return 3 + 1;
        if (nb_threads <= 8) return 4 + 1;
    ...
    inline_ PxConvexFlags GetConvexFlags() const {
        return mUseGPU ? (PxConvexFlag::eCOMPUTE_CONVEX | PxConvexFlag::eGPU_COMPATIBLE)
                       : PxConvexFlag::eCOMPUTE_CONVEX;
    }
#else
    inline_ PxConvexFlags GetConvexFlags() const { return PxConvexFlag::eCOMPUTE_CONVEX; }
#endif
...
class SharedPhysX : public Pint,
                    public MeshManager,
                    public PintShapeEnumerateCallback  //, public PxDeletionListener
#ifndef IS_PHYSX_3_2
    ,
...
public:
    ...
    // PxDeletionListener
    //		virtual	void						onRelease(const PxBase* observed, void* userData, PxDeletionEventFlag::Enum
    //deletionEvent); ~PxDeletionListener
    ...
    //action, const Point& pos);
    virtual void AddWorldImpulseAtWorldPos(PintActorHandle handle,
                                           const Point& world_impulse,
                                           const Point& world_pos) override;
    ...
    virtual PintHeightfieldHandle CreateHeightfieldObject(const PINT_HEIGHTFIELD_DATA_CREATE& desc,
                                                          ...
                                                          PintHeightfieldIndex* index) override;
    ...
    virtual PintActorHandle CreateArticulatedObject(const PINT_OBJECT_CREATE&,
                                                    ...
                                                    PintArticHandle articulation) override;
    ...
    virtual udword BatchRaycasts(PintSQThreadContext context,
                                 udword nb,
                                 PintRaycastHit* dest,
                                 const PintRaycastData* raycasts) override;
...
#if PHYSX_SUPPORT_ARTICULATIONS
    PintActorHandle CreateArticulationLink(PxArticulation* articulation,
                                           PxArticulationLink* parent,
                                           Pint& pint,
                                           const PINT_OBJECT_CREATE& desc);
#endif
#if PHYSX_SUPPORT_RCA
    PintActorHandle CreateRCArticulationLink(PxArticulationReducedCoordinate* articulation,
                                             PxArticulationLink* parent,
                                             Pint& pint,
                                             const PINT_OBJECT_CREATE& desc);
#endif
    ...
    inline_ void AddActorToManager(PxRigidActor* actor) { mActorManager.Add(actor); }
...
protected:
    ...
    //				PxConvexMesh*				CreateConvexMesh(const Point* verts, udword vertCount, PxConvexFlags flags,
    ...
    virtual PxTriangleMesh* CreatePhysXMesh(const PintSurfaceInterface& surface,
                                            bool deformable,
...
#ifdef IS_PHYSX_3_2
    inline_ PxSceneQueryFilterCallback* GetSQFilterCallback() { return null; }

    inline_ PxSceneQueryFilterData GetSQFilterData() {
        const PxQueryFlags qf = PxQueryFlag::eDYNAMIC | PxQueryFlag::eSTATIC;
        return PxSceneQueryFilterData(PxFilterData(!mParams.mSQFilterOutAllShapes, mParams.mSQFilterOutAllShapes, 0, 0),
                                      qf);
    }
#else
    inline_ PxQueryFilterCallback* GetSQFilterCallback() { return mInvisibles ? this : null; }

    inline_ PxQueryFilterData GetSQFilterData() {
        PxQueryFlags qf = PxQueryFlag::eDYNAMIC | PxQueryFlag::eSTATIC;
        if (mInvisibles) qf |= PxQueryFlag::ePREFILTER;
...
protected:
    ...
    virtual void CreateShapes(const PINT_OBJECT_CREATE& desc,
                              PxRigidActor* actor,
                              PintCollisionGroup group,
                              const char* forced_name) {}
#else
private:
    PxShape* CreateSphereShape(const PINT_SHAPE_CREATE* create,
                               PxRigidActor* actor,
                               const PxSphereGeometry& geometry,
                               const PxMaterial& material,
                               const PxTransform& local_pose,
                               PxU16 collision_group);
    PxShape* CreateBoxShape(const PINT_SHAPE_CREATE* create,
                            PxRigidActor* actor,
                            const PxBoxGeometry& geometry,
                            const PxMaterial& material,
                            const PxTransform& local_pose,
                            PxU16 collision_group);
    PxShape* CreateCapsuleShape(const PINT_SHAPE_CREATE* create,
                                PxRigidActor* actor,
                                ...
                                PxU16 collision_group);
#if PHYSX_SUPPORT_CUSTOM_GEOMETRY
    PxShape* CreateCylinderShape(const PINT_SHAPE_CREATE* create,
                                 PxRigidActor* actor,
                                 ...
                                 PxU16 collision_group);
#endif
    PxShape* CreateConvexShape(const PINT_SHAPE_CREATE* create,
                               PxRigidActor* actor,
                               const PxConvexMeshGeometry& geometry,
                               const PxMaterial& material,
                               const PxTransform& local_pose,
                               PxU16 collision_group);
    PxShape* CreateMeshShape(const PINT_SHAPE_CREATE* create,
                             PxRigidActor* actor,
                             ...
                             PxU16 collision_group);
#if PHYSX_SUPPORT_HEIGHTFIELDS
    PxShape* CreateHeightfieldShape(const PINT_SHAPE_CREATE* create,
                                    PxRigidActor* actor,
...
#endif
protected:
    void CreateShapes(const PINT_OBJECT_CREATE& desc,
                      PxRigidActor* actor,
                      PintCollisionGroup group,
                      const char* forced_name);
#endif
    ...
    struct LocalTorque {
        LocalTorque(PintActorHandle handle, const Point& local_torque) : mHandle(handle), mLocalTorque(local_torque) {}
...
#ifdef IS_PHYSX_3_2
    if (!enable_sleeping) dynamic->wakeUp(9999999999.0f);
#else
    dynamic->wakeUp();
    if (!enable_sleeping) dynamic->setWakeCounter(9999999999.0f);
#endif
}
...
inline_ void SetupShape(SharedPhysX& physx,
    ...
    if (create->mRenderer) shape.userData = create->mRenderer;
...
#ifndef IS_PHYSX_3_2
inline_ void SetupShape_(SharedPhysX& physx,
                         const PINT_SHAPE_CREATE* create,
                         PxShape* shape,
                         const PxTransform& local_pose,
                         PxU16 collision_group,
...
inline_ PxShape* CreateNonSharedShape(SharedPhysX& physx,
                                      const PINT_SHAPE_CREATE* create,
                                      PxRigidActor* actor,
...
inline_ PxShape* CreateSharedShape(SharedPhysX& physx,
                                   PxPhysics* physics,
                                   const PINT_SHAPE_CREATE* create,
                                   PxRigidActor* actor,
                                   const PxGeometry& geometry,
