...
struct NpOverflowBuffer : PxHitBuffer<HitType> {
    ...
    bool processCalled;
    PxU32 saveNbTouches;
    NpOverflowBuffer(HitType* hits, PxU32 count)
        : PxHitBuffer<HitType>(hits, count), overflow(false), processCalled(false), saveNbTouches(0) {}

    virtual PxAgain processTouches(const HitType* /*hits*/, PxU32 /*count*/) {
        if (processCalled) return false;
...
public:
    virtual void enableSQRecorder(bool b) { mRecorder.SetEnabled(b); }

    virtual void resetSQRecorder() { mRecorder.Reset(); }

    virtual void drawSQRaycasts(PintRender& renderer) { mRecorder.DrawRaycasts(renderer, mRaycasts.mBuffers); }
    ...
    virtual PxRaycastBuffer* raycast(const PxVec3& origin,
    ...
    virtual PxSweepBuffer* sweep(const PxGeometry& geometry,
    ...
    virtual PxOverlapBuffer* overlap(const PxGeometry& geometry,
                                     const PxTransform& pose,
...
private:
    ...
    struct Query {
        ...
        Query() : mBuffers(NULL), mQueries(NULL), mMaxNbBuffers(0), mTouches(NULL), mMaxNbTouches(0), mBufferTide(0) {}

        Query(PxHitBuffer<HitType>* buffers,
              QueryType* queries,
              const PxU32 maxNbBuffers,
              HitType* touches,
              const PxU32 maxNbTouches)
            : mBuffers(buffers),
              mQueries(queries),
        ...
        static void performQuery(const PxScene& scene,
                                 const Raycast& query,
                                 NpOverflowBuffer<PxRaycastHit>& hitBuffer,
                                 PxQueryFilterCallback* qfcb,
                                 SQRecorder& recorder,
                                 PxU32 index) {
            scene.raycast(query.origin, query.unitDir, query.distance, hitBuffer, query.hitFlags, query.filterData,
                          qfcb, query.cache);
        ...
        static void performQuery(const PxScene& scene,
                                 const Sweep& query,
                                 NpOverflowBuffer<PxSweepHit>& hitBuffer,
                                 PxQueryFilterCallback* qfcb,
                                 SQRecorder& recorder,
                                 PxU32 index) {
            bool status = scene.sweep(query.geometry.any(), query.pose, query.unitDir, query.distance, hitBuffer,
                                      query.hitFlags, query.filterData, qfcb, query.cache, query.inflation);
...
#ifdef PHYSX_SUPPORT_VEHICLE_XP
            if (gExperiment) {
                if (gLogXP) printf("sweep impact dist: %f\n", hitBuffer.block.distance);
                if (gHits[index].actor) {
                    if (gLogXP) printf("contact pen depth: %f\n", gHits[index].distance);
                    ...
                    if (gLogXP) printf("dp0:               %f\n", dp);

                    const PxVec3& ttt = gHits[index].normal * gHits[index].distance;
                    dp -= ttt.dot(query.unitDir);

                    /*						const PxVec3& newPos = gHits[index].position -
                    gHits[index].normal*gHits[index].distance;	//###

                    /*						const PxVec3& delta = gHits[index].position -
                    query.pose.p;
                    ...
                    // printf("%f %f %f\n", hitBuffer.block.position.x, hitBuffer.block.position.y,
                    ...
                    if (gLogXP) printf("fake sweep dist:   %f\n\n", hitBuffer.block.distance);
                } else {
        ...
        static void performQuery(const PxScene& scene,
                                 const Overlap& query,
                                 NpOverflowBuffer<PxOverlapHit>& hitBuffer,
                                 PxQueryFilterCallback* qfcb,
                                 SQRecorder& recorder,
                                 PxU32 index) {
            scene.overlap(query.geometry.any(), query.pose, hitBuffer, query.filterData, qfcb, query.cache);
        }
        ...
        void execute(const PxScene& scene, PxQueryFilterCallback* qfcb, SQRecorder& recorder) {
            ...
            for (PxU32 i = 0; i < mBufferTide; i++) {
                ...
                bool overflow = false;
                {
                    PX_ALIGN(16, NpOverflowBuffer<HitType> overflowBuffer)
                    (mBuffers[i].touches, mBuffers[i].maxNbTouches);
...
public:
    ExtBatchQueryDesc(const PxU32 maxNbResults, const PxU32 maxNbTouches)
        : mResults(NULL), mMaxNbResults(maxNbResults), mTouches(NULL), mMaxNbTouches(maxNbTouches) {}
    ExtBatchQueryDesc(PxHitBuffer<HitType>* results, const PxU32 maxNbResults, HitType* touches, PxU32 maxNbTouches)
        : mResults(results), mMaxNbResults(maxNbResults), mTouches(touches), mMaxNbTouches(maxNbTouches) {}
...
PxU32 computeByteSize(const ExtBatchQueryDesc<HitType>& queryDesc) {
    ...
    if (queryDesc.getNbResults() > 0) {
        ...
        if (!queryDesc.getResults()) {
            byteSize += sizeof(PxHitBuffer<HitType>) * queryDesc.getNbResults() +
                        sizeof(HitType) * queryDesc.getNbTouches();
        }
    }
    return byteSize;
}
...
template <typename HitType, typename QueryType>
PxU8* parseDesc(PxU8* bufIn,
                const ExtBatchQueryDesc<HitType>& queryDesc,
...
MyBatchQueryExt* create(const PxScene& scene,
                        PxQueryFilterCallback* queryFilterCallback,
                        const ExtBatchQueryDesc<PxRaycastHit>& raycastDesc,
                        const ExtBatchQueryDesc<PxSweepHit>& sweepDesc,
                        const ExtBatchQueryDesc<PxOverlapHit>& overlapDesc) {
    const PxU32 byteSize = sizeof(ExtBatchQuery) + computeByteSize<PxRaycastHit, Raycast>(raycastDesc) +
    ...
    buf = parseDesc<PxRaycastHit, Raycast>(buf, raycastDesc, raycastBuffers, raycastQueries, maxNbRaycasts,
                                           raycastTouches, maxNbRaycastTouches);
    ...
    buf = parseDesc<PxSweepHit, Sweep>(buf, sweepDesc, sweepBuffers, sweepQueries, maxNbSweeps, sweepTouches,
                                       maxNbSweepTouches);
    ...
    buf = parseDesc<PxOverlapHit, Overlap>(buf, overlapDesc, overlapBuffers, overlapQueries, maxNbOverlaps,
                                           overlapTouches, maxNbOverlapTouches);

    PX_ASSERT((reinterpret_cast<PxU8*>(bq) + byteSize) == buf);

    PX_PLACEMENT_NEW(bq, ExtBatchQuery)
    (scene, queryFilterCallback, raycastBuffers, raycastQueries, maxNbRaycasts, raycastTouches, maxNbRaycastTouches,
     sweepBuffers, sweepQueries, maxNbSweeps, sweepTouches, maxNbSweepTouches, overlapBuffers, overlapQueries,
     maxNbOverlaps, overlapTouches, maxNbOverlapTouches);

    return bq;
}

ExtBatchQuery::ExtBatchQuery(const PxScene& scene,
                             PxQueryFilterCallback* queryFilterCallback,
                             PxRaycastBuffer* raycastBuffers,
                             Raycast* raycastQueries,
                             const PxU32 maxNbRaycasts,
                             ...
                             PxOverlapHit* overlapTouches,
                             const PxU32 maxNbOverlapTouches)
    : mScene(scene), mQueryFilterCallback(queryFilterCallback) {
    ...
    PX_PLACEMENT_NEW(&mRaycasts, QueryRaycast)
    ...
    PX_PLACEMENT_NEW(&mOverlaps, QueryOverlap)
...
PxRaycastBuffer* ExtBatchQuery::raycast(const PxVec3& origin,
                                        const PxVec3& unitDir,
                                        const PxReal distance,
                                        const PxU16 maxNbTouches,
                                        PxHitFlags hitFlags,
...
PxSweepBuffer* ExtBatchQuery::sweep(const PxGeometry& geometry,
                                    const PxTransform& pose,
                                    const PxVec3& unitDir,
                                    const PxReal distance,
                                    const PxU16 maxNbTouches,
...
PxOverlapBuffer* ExtBatchQuery::overlap(const PxGeometry& geometry,
                                        const PxTransform& pose,
                                        PxU16 maxNbTouches,
                                        const PxQueryFilterData& filterData,
                                        const PxQueryCache* cache) {
...
MyBatchQueryExt* createMyBatchQueryExt(const PxScene& scene,
                                       PxQueryFilterCallback* queryFilterCallback,
                                       const PxU32 maxNbRaycasts,
                                       const PxU32 maxNbRaycastTouches,
                                       const PxU32 maxNbSweeps,
    ...
    return create(scene, queryFilterCallback, ExtBatchQueryDesc<PxRaycastHit>(maxNbRaycasts, maxNbRaycastTouches),
