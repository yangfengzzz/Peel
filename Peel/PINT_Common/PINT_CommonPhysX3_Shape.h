...
struct InternalShape {
    ...
    inline_ bool Compare(const PxMaterial* material,
                         PintShapeRenderer* renderer,
                         const PxTransform& local_pose,
                         PxU16 collision_group) const {
        ...
        if (mMaterial == material && mRenderer == renderer && mLocalPose.p.x == local_pose.p.x &&
            mLocalPose.p.y == local_pose.p.y && mLocalPose.p.z == local_pose.p.z && mLocalPose.q.x == local_pose.q.x &&
            mLocalPose.q.y == local_pose.q.y && mLocalPose.q.z == local_pose.q.z && mLocalPose.q.w == local_pose.q.w &&
            mCollisionGroup == collision_group)
            return true;
        return false;
    }
...
struct InternalSharedShape {
    InternalSharedShape(const PxMaterial* material,
                        PintShapeRenderer* renderer,
                        const PxTransform& local_pose,
                        PxU16 collision_group)
        : mMaterial(material), mRenderer(renderer), mLocalPose(local_pose), mCollisionGroup(collision_group) {}
    const PxMaterial* mMaterial;
    PintShapeRenderer* mRenderer;
    PxTransform mLocalPose;
    PxU16 mCollisionGroup;
...
{
#ifdef SHARED_SHAPES_USE_HASH
    InternalSphereShape(float radius,
                        const PxMaterial* material,
                        PintShapeRenderer* renderer,
                        const PxTransform& local_pose,
                        PxU16 collision_group)
...
struct InternalBoxShape : InternalShape {
    InternalBoxShape(const PxVec3& extents,
                     PxShape* shape,
                     const PxMaterial* material,
                     PintShapeRenderer* renderer,
                     const PxTransform& local_pose,
                     PxU16 collision_group)
        : mExtents(extents), InternalShape(shape, material, renderer, local_pose, collision_group) {}
    PxVec3 mExtents;
};

struct InternalCapsuleShape : InternalShape {
    InternalCapsuleShape(float radius,
                         float half_height,
                         PxShape* shape,
                         const PxMaterial* material,
                         PintShapeRenderer* renderer,
...
struct InternalConvexShape : InternalShape {
    InternalConvexShape(PxConvexMesh* convex_mesh,
                        PxShape* shape,
                        const PxMaterial* material,
                        PintShapeRenderer* renderer,
                        const PxTransform& local_pose,
                        PxU16 collision_group)
        : mConvexMesh(convex_mesh), InternalShape(shape, material, renderer, local_pose, collision_group) {}
    PxConvexMesh* mConvexMesh;
};

struct InternalMeshShape : InternalShape {
    InternalMeshShape(PxTriangleMesh* triangle_mesh,
                      PxShape* shape,
                      const PxMaterial* material,
                      PintShapeRenderer* renderer,
                      const PxTransform& local_pose,
                      PxU16 collision_group)
        : mTriangleMesh(triangle_mesh), InternalShape(shape, material, renderer, local_pose, collision_group) {}
    PxTriangleMesh* mTriangleMesh;
};

#if PHYSX_SUPPORT_HEIGHTFIELDS
struct InternalHeightfieldShape : InternalShape {
    InternalHeightfieldShape(PxHeightField* heightfield,
                             PxShape* shape,
                             const PxMaterial* material,
                             PintShapeRenderer* renderer,
                             const PxTransform& local_pose,
                             PxU16 collision_group)
        : mHeightfield(heightfield), InternalShape(shape, material, renderer, local_pose, collision_group) {}
...
public:
    ...
    virtual bool FindTouchedTriangles(Container& indices,
                                      ...
                                      const PR& pose,
                                      const PintSphereOverlapData& sphere) const override;
