...
void Common_CloseGUI(Widgets*& gui) { DELETESINGLE(gui); }

sdword Common_CreateDebugVizUI(IceWidget* parent,
                               sdword x,
                               sdword y,
                               CBCallback check_box_callback,
                               udword id,
                               udword nb_params,
                               const bool* debug_viz_params,
                               const char** debug_viz_names,
                               IceCheckBox** check_boxes,
                               Widgets* gui) {
    ASSERT(gui);
    ASSERT(check_boxes);
...
MyIceAllocator::MyIceAllocator(const char* name)
    : mName(name), mCurrentNbAllocs(0), mUsedMemory(0), mPreviousAllocator(null) {
...
void* MyIceAllocator::mallocDebug(
...
void* MyIceAllocator::shrink(void* memory, size_t size) { return memory; }
