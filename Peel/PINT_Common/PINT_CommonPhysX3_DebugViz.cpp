...
void SQRecorder::RecordSweep(
        const PxGeometry& geometry, const PxTransform& pose, const PxVec3& unitDir, float distance, udword index) {
    if (mEnabled) {
        ...
        if (Current.hasBlock) {
            ...
            if (Current.nbTouches) {
                for (PxU32 i = 0; i < Current.nbTouches; i++) {
                    ...
                    renderer.DrawLine(Record.mOrigin, ToPoint(hit.position), Point(1.0f, 0.0f, 0.0f));
                    renderer.DrawLine(ToPoint(hit.position), ToPoint(hit.position + hit.normal),
                                      Point(0.0f, 1.0f, 1.0f));
                }
            } else {
                renderer.DrawLine(Record.mOrigin, Record.mOrigin + Record.mDir * Record.mMaxDist,
