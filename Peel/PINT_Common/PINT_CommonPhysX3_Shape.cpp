...
PintShape PhysX_ShapeAPI::GetType(PintShapeHandle handle) const {
    ...
    switch (type) {
        case PxGeometryType::eSPHERE:
            return PINT_SHAPE_SPHERE;
        case PxGeometryType::eCAPSULE:
            return PINT_SHAPE_CAPSULE;
        case PxGeometryType::eBOX:
            return PINT_SHAPE_BOX;
        case PxGeometryType::eCONVEXMESH:
            return PINT_SHAPE_CONVEX;
        case PxGeometryType::eTRIANGLEMESH:
            return PINT_SHAPE_MESH;
...
bool PhysX_ShapeAPI::GetTriangleMeshData(SurfaceInterface& surface, PintShapeHandle handle, bool b) const {
    ...
    if (!Shape->getTriangleMeshGeometry(meshGeom)) return false;
...
static PxTriangleMesh* GetTriangleIndices(IndexedTriangle& tri, PintShapeHandle handle, udword index) {
    ...
    PxTriangleMeshGeometry meshGeom;
    if (!Shape->getTriangleMeshGeometry(meshGeom)) return null;

    PxTriangleMesh* mesh = meshGeom.triangleMesh;
    if (!mesh) return null;

    const PxU32 nbTris = mesh->getNbTriangles();
    if (index >= nbTris) return null;
...
bool PhysX_ShapeAPI::FindTouchedTriangles(Container& indices,
                                          PintSQThreadContext context,
                                          PintShapeHandle handle,
                                          const PR& pose,
                                          const PintSphereOverlapData& overlap) const {
    PxShape* Shape = reinterpret_cast<PxShape*>(handle);
    ASSERT(Shape);
    ...
    bool Overflow = false;
    const PxU32 Nb = PxMeshQuery::findOverlapTriangleMesh(PxSphereGeometry(overlap.mSphere.mRadius), Pose, meshGeom,
                                                          meshPose, Results, 8192, startIndex, Overflow);
    ASSERT(!Overflow);
