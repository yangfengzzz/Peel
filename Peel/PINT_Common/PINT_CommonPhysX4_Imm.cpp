...
void ImmediateScene::reset() {
    ...
    for (PxU32 i = 0; i < size; i++) PxReleaseArticulation(mArticulations[i]);
...
ImmActorHandle ImmediateScene::createActor(ImmShapeHandle shape,
...
ImmActorHandle ImmediateScene::createActor(PxU32 nbShapes,
                                           const ImmShapeHandle* shapes,
                                           const PxTransform& pose,
                                           PxU32 group,
                                           const MassProps* massProps,
                                           Dy::ArticulationLinkHandle link) {
...
void ImmediateScene::addArticulationToScene(Dy::ArticulationV* art) { mArticulations.pushBack(art); }

// TEST_IMMEDIATE_JOINTS
void ImmediateScene::createSphericalJoint(ImmActorHandle id0,
                                          ImmActorHandle id1,
...
void ImmediateScene::updateArticulations(float dt, const PxVec3& gravity, PxU32 nbIterPos) {
    ...
    if (mUseTGS) {
        ...
        for (PxU32 i = 0; i < nbArticulations; i++) PxComputeUnconstrainedVelocities(mArticulations[i], gravity, dt);
    }
}

void ImmediateScene::updateBounds(float boundsInflation) {
    struct Local {
        static void computeShapeBounds(PxBounds3& bounds,
                                       const PxGeometry& currentGeom,
    ...
    for (PxU32 i = 0; i < nbActors; i++) {
        ...
        for (PxU32 j = 0; j < nbShapes; j++) {
            ...
            PxGeometryQuery::computeGeomBounds(bounds, currentGeom, currentPose, boundsInflation, 1.0f,
...
void ImmediateScene::broadPhase() {
    ...
    for (PxU32 i = 0; i < nbEntries; i++) {
        ...
        for (PxU32 j = i + 1; j < nbEntries; j++) {
            ...
            if (entry0.mActor == entry1.mActor) continue;
            const ImmediateActor& actor1 = getActor(entry1.mActor);
            const ImmediateActor::Type type1 = actor1.mType;

            // Filtering
            {
                if (type0 == ImmediateActor::eSTATIC && type1 == ImmediateActor::eSTATIC) continue;

                if (isCollisionDisabled(entry0.mActor, entry1.mActor)) continue;
            }
            ...
            if (shapeBounds[i].intersects(shapeBounds[j])) {
                ...
                const _hashmap<ShapePair, PersistentContactPair>::Entry* e =
                        mPersistentPairs.find(ShapePair(entry0.mShape, entry1.mShape));
                if (e) {
    ...
    //	while(NbCandidates--)
    {
        ...
        for (PxU32 j = i + 1; j < nbActors; j++) {
            ...
            // Filtering
            {
                ...
                if (mActors[i].mCollisionGroup == 1 && mActors[j].mCollisionGroup == 1) continue;
#endif
            ...
            //			const ActorPair ids(ImmActorHandle(i), ImmActorHandle(j));	//### WTF... this one doesn't
            //compile
...
void ImmediateScene::narrowPhase(float contactDistance,
    ...
    public:
        ...
        virtual bool recordContacts(const Gu::ContactPoint* contactPoints,
                                    const PxU32 nbContacts,
                                    const PxU32 /*index*/) {
#ifdef SHAPE_CENTRIC
            mScene->mContactPairs.pushBack(
                    ImmediateScene::ContactPair(mActorPair, mShapePair, nbContacts, mScene->mContactPoints.size()));
#else
            mScene->mContactPairs.pushBack(
                    ImmediateScene::ContactPair(mPair, nbContacts, mScene->mContactPoints.size()));
#endif
            mHasContacts = true;
    ...
    for (PxU32 i = 0; i < nbPairs; i++) {
        ...
        if (mUsePersistency) {
            ...
            PxGenerateContacts(&pxGeom0, &pxGeom1, &tr0, &tr1, &persistentData.cache, 1, contactRecorder,
                               contactDistance, meshContactMargin, toleranceLength, *mCacheAllocator);
            ...
            PxCache cache;
            PxGenerateContacts(&pxGeom0, &pxGeom1, &tr0, &tr1, &cache, 1, contactRecorder, contactDistance,
                               meshContactMargin, toleranceLength, *mCacheAllocator);
        }
    }
#else
    ...
    if (0) printf("Narrow-phase: %d contacts    \r", mContactPoints.size());
}
...
void ImmediateScene::buildSolverBodyData(float dt,
    ...
    for (PxU32 i = 0; i < nbActors; i++) {
        if (mActors[i].mType == ImmediateActor::eSTATIC) {
            if (mUseTGS)
                PxConstructStaticSolverBodyTGS(mActorGlobalPoses[i], mTGSSolverBodies[i], mTGSSolverBodyTxInertias[i],
                                               mTGSSolverBodyData[i]);
            else
            ...
            if (mUseTGS)
                PxConstructSolverBodiesTGS(&data, &mTGSSolverBodies[i], &mTGSSolverBodyTxInertias[i],
                                           &mTGSSolverBodyData[i], 1, totalForce, dt);
            else
...
static void setupConstraintDesc(PxSolverConstraintDesc& desc,
                                const ImmediateActor* actors,
                                T* solverBodies,
                                ImmActorHandle handle) {
...
void ImmediateScene::buildSolverConstraintDesc() {
    ...
    for (PxU32 i = 0; i < nbContactPairs; i++) {
        ...
        if (mUseTGS) {
#ifdef SHAPE_CENTRIC
            setupConstraintDesc<PxTGSSolverBodyVel, false>(desc, mActors.begin(), mTGSSolverBodies.begin(),
                                                           pair.mActorPair.mID0);
            setupConstraintDesc<PxTGSSolverBodyVel, true>(desc, mActors.begin(), mTGSSolverBodies.begin(),
                                                          pair.mActorPair.mID1);
#else
...
#ifdef SHAPE_CENTRIC
            setupConstraintDesc<PxSolverBody, false>(desc, mActors.begin(), mSolverBodies.begin(),
                                                     pair.mActorPair.mID0);
        ...
        // Cache pointer to our contact data structure and identify which type of constraint this is. We'll need this
        // later after batching. If we choose not to perform batching and instead just create a single header per-pair,
        // then this would not be necessary because the constraintDescs would not have been reordered
...
static void setupCommonLinkData(PxFeatherstoneArticulationLinkData& data, Dy::ArticulationLinkHandle parent, const
PxTransform& pose, const MassProps& massProps, float linearDamping, float angularDamping, float maxLinearVelocity, float
maxAngularVelocity, float jointFrictionCoefficient)
{
        data.parent								= parent;
        data.pose								= pose;
...
static PxU32 SphericalJointSolverPrep(Px1DConstraint* constraints,
    ...
    if (cB2w.q.dot(cA2w.q) < 0.0f) cB2w.q = -cB2w.q;
...
template <const bool aorb>
static void setupDescTGS(PxTGSSolverContactDesc& contactDesc,
                         const ImmediateActor* actors,
                         const PxTGSSolverBodyTxInertia* txInertias,
                         const PxTGSSolverBodyData* solverBodyData,
                         const PxTransform* poses,
                         const ImmActorHandle handle) {
    ...
    if (link) {
        ...
        txI = &txInertias[id];
        bodyFrame = poses[id];
        bodyState = actors[id].mType == ImmediateActor::eDYNAMIC ? PxSolverConstraintPrepDescBase::eDYNAMIC_BODY
                                                                 : PxSolverConstraintPrepDescBase::eSTATIC_BODY;
    }
}

template <const bool aorb>
static void setupDescPGS(PxSolverContactDesc& contactDesc,
                         const ImmediateActor* actors,
...
template <const bool aorb>
static void setupJointDescTGS(PxTGSSolverConstraintPrepDesc& jointDesc,
                              const ImmediateActor* actors,
                              PxTGSSolverBodyTxInertia* txInertias,
                              PxTGSSolverBodyData* solverBodyData,
                              PxTransform* poses,
                              const PxU32 bodyDataIndex) {
    if (!aorb) {
...
template <const bool aorb>
static void setupJointDescPGS(PxSolverConstraintPrepDesc& jointDesc,
                              const ImmediateActor* actors,
...
template <class T>
void ImmediateScene::processContactHeader(PxConstraintBatchHeader& header,
    ...
    struct Local {
        static void setupContactDesc(PxTGSSolverContactDesc& contactDesc,
                                     const PxSolverConstraintDesc& constraintDesc,
                                     const ContactPair& pair,
                                     const ImmediateActor* actors,
                                     const PxSolverBodyData* solverBodyData,
                                     const PxTGSSolverBodyData* tgsSolverBodyData,
                                     const PxTGSSolverBodyTxInertia* inertias,
                                     const PxTransform* actorGlobalPoses) {
#ifdef SHAPE_CENTRIC
            setupDescTGS<false>(contactDesc, actors, inertias, tgsSolverBodyData, actorGlobalPoses,
                                pair.mActorPair.mID0);
            setupDescTGS<true>(contactDesc, actors, inertias, tgsSolverBodyData, actorGlobalPoses,
                               pair.mActorPair.mID1);
#else
        ...
        static void setupContactDesc(PxSolverContactDesc& contactDesc,
                                     const PxSolverConstraintDesc& constraintDesc,
                                     const ContactPair& pair,
                                     const ImmediateActor* actors,
                                     const PxSolverBodyData* solverBodyData,
        ...
        static PX_FORCE_INLINE void createContactConstraints(TestConstraintAllocator* allocator,
                                                             PxTGSSolverContactDesc* contactDescs,
                                                             PxConstraintBatchHeader& header,
                                                             float invDt,
                                                             float stepInvDt,
                                                             float bounceThreshold,
                                                             float frictionOffsetThreshold,
                                                             float correlationDistance) {
            PxCreateContactConstraintsTGS(&header, 1, contactDescs, *allocator, stepInvDt, invDt, bounceThreshold,
                                          frictionOffsetThreshold, correlationDistance);
        }
        ...
        static PX_FORCE_INLINE void createContactConstraints(TestConstraintAllocator* allocator,
                                                             ...
                                                             float frictionOffsetThreshold,
                                                             float correlationDistance) {
            PxCreateContactConstraints(&header, 1, contactDescs, *allocator, invDt, bounceThreshold,
                                       frictionOffsetThreshold, correlationDistance);
        }
    };
    ...
    for (PxU32 a = 0; a < header.stride; a++) {
        ...
        if (!constraintDesc.constraint) continue;
        ...
        Local::setupContactDesc(contactDesc, constraintDesc, pair, mActors.begin(), mSolverBodyData.begin(),
                                mTGSSolverBodyData.begin(), mTGSSolverBodyTxInertias.begin(),
                                mActorGlobalPoses.begin());
        ...
        contactDesc.invMassScales.angular0 = contactDesc.invMassScales.angular1 = contactDesc.invMassScales.linear0 =
                contactDesc.invMassScales.linear1 = 1.0f;
    }

    Local::createContactConstraints(mConstraintAllocator, contactDescs, header, invDt, stepInvDt, bounceThreshold,
                                    frictionOffsetThreshold, correlationDistance);
...
template <class T>
void ImmediateScene::processJointHeader(PxConstraintBatchHeader& header,
                                        PxSolverConstraintDesc* orderedDescs,
                                        float dt,
                                        float invDt,
                                        float stepDt,
                                        ...
                                        float lengthScale) {
    struct Local {
        static void setupJointDesc(PxTGSSolverConstraintPrepDesc& jointDesc,
                                   PxSolverConstraintDesc& constraintDesc,
                                   const ImmediateActor* actors,
                                   PxSolverBodyData* solverBodyData,
                                   PxTGSSolverBodyData* tgsSolverBodyData,
                                   PxTGSSolverBodyTxInertia* inertias,
                                   PxTransform* actorGlobalPoses) {
            jointDesc.body0 = constraintDesc.tgsBodyA;
            jointDesc.body1 = constraintDesc.tgsBodyB;
            setupJointDescTGS<false>(jointDesc, actors, inertias, tgsSolverBodyData, actorGlobalPoses,
                                     constraintDesc.bodyADataIndex);
            setupJointDescTGS<true>(jointDesc, actors, inertias, tgsSolverBodyData, actorGlobalPoses,
                                    constraintDesc.bodyBDataIndex);
        }

        static void setupJointDesc(PxSolverConstraintPrepDesc& jointDesc,
                                   PxSolverConstraintDesc& constraintDesc,
                                   const ImmediateActor* actors,
                                   PxSolverBodyData* solverBodyData,
                                   PxTGSSolverBodyData* tgsSolverBodyData,
        ...
        static PX_FORCE_INLINE void createJointConstraints(TestConstraintAllocator* allocator,
                                                           PxConstraintBatchHeader& header,
                                                           ...
                                                           float stepInvDt,
                                                           float lengthScale) {
            PxCreateJointConstraintsWithImmediateShadersTGS(&header, 1, constraints, jointDescs, *allocator, stepDt, dt,
                                                            stepInvDt, invDt, lengthScale);
        }
    ...
    for (PxU32 a = 0; a < header.stride; a++) {
        ...
        Local::setupJointDesc(jointDesc, constraintDesc, mActors.begin(), mSolverBodyData.begin(),
        ...
        jointDesc.invMassScales.angular0 = jointDesc.invMassScales.angular1 = jointDesc.invMassScales.linear0 =
                jointDesc.invMassScales.linear1 = 1.0f;
    }

    Local::createJointConstraints(mConstraintAllocator, header, constraints, jointDescs, dt, invDt, stepDt, stepInvDt,
                                  lengthScale);
}

void ImmediateScene::createContactConstraints(float dt,
                                              float invDt,
                                              float bounceThreshold,
                                              float frictionOffsetThreshold,
                                              float correlationDistance,
                                              float lengthScale,
                                              const PxU32 nbPosIterations) {
    ...
    if (mBatchContacts) {
        ...
        if (mUseTGS) {
            nbContactHeaders = PxBatchConstraintsTGS(mSolverConstraintDesc.begin(), mContactPairs.size(),
                                                     mTGSSolverBodies.begin(), nbBodies, mHeaders.begin(), orderedDescs,
                                                     mArticulations.begin(), mArticulations.size());

            // 2 batch the joints...
            nbJointHeaders = PxBatchConstraintsTGS(
                    mSolverConstraintDesc.begin() + mContactPairs.size(), mJointData.size(), mTGSSolverBodies.begin(),
                    nbBodies, mHeaders.begin() + nbContactHeaders, orderedDescs + mContactPairs.size(),
                    mArticulations.begin(), mArticulations.size());
        } else {
            // 1 batch the contacts
            nbContactHeaders = PxBatchConstraints(mSolverConstraintDesc.begin(), mContactPairs.size(),
                                                  mSolverBodies.begin(), nbBodies, mHeaders.begin(), orderedDescs,
                                                  mArticulations.begin(), mArticulations.size());

            // 2 batch the joints...
            nbJointHeaders = PxBatchConstraints(mSolverConstraintDesc.begin() + mContactPairs.size(), mJointData.size(),
                                                mSolverBodies.begin(), nbBodies, mHeaders.begin() + nbContactHeaders,
                                                orderedDescs + mContactPairs.size(), mArticulations.begin(),
                                                mArticulations.size());
        }
    ...
    for (PxU32 i = 0; i < nbContactHeaders; i++) {
        ...
        if (mUseTGS)
            processContactHeader<PxTGSSolverContactDesc>(header, orderedDescs, invDt, stepInvDt, bounceThreshold,
                                                         frictionOffsetThreshold, correlationDistance);
        else
            processContactHeader<PxSolverContactDesc>(header, orderedDescs, invDt, stepInvDt, bounceThreshold,
                                                      frictionOffsetThreshold, correlationDistance);
    }
    ...
    for (PxU32 i = 0; i < nbJointHeaders; i++) {
        ...
        if (mUseTGS)
            processJointHeader<PxTGSSolverConstraintPrepDesc>(header, orderedDescs, dt, invDt, stepDt, stepInvDt,
                                                              lengthScale);
        else
            processJointHeader<PxSolverConstraintPrepDesc>(header, orderedDescs, dt, invDt, stepDt, stepInvDt,
                                                           lengthScale);
    }
}
...
void ImmediateScene::solveAndIntegrate(float dt, PxU32 nbIterPos, PxU32 nbIterVel) {
    ...
    if (mUseTGS) {
        ...
        PxSolveConstraintsTGS(mHeaders.begin(), mHeaders.size(),
                              ...
                              mTGSSolverBodies.begin(), mTGSSolverBodyTxInertias.begin(), nbDynamic, nbIterPos,
                              nbIterVel, stepDt, 1.0f / stepDt, nbArticulations, articulations);
    } else {
        PxMemZero(mSolverBodies.begin(), mSolverBodies.size() * sizeof(PxSolverBody));

        PxSolveConstraints(mHeaders.begin(), mHeaders.size(),
                           mBatchContacts ? mOrderedSolverConstraintDesc.begin() : mSolverConstraintDesc.begin(),
                           mSolverBodies.begin(), mMotionLinearVelocity.begin(), mMotionAngularVelocity.begin(),
                           nbDynamic, nbIterPos, nbIterVel, dt, 1.0f / dt, nbArticulations, articulations);
    }
    ...
    if (mUseTGS) {
        PxIntegrateSolverBodiesTGS(mTGSSolverBodies.begin(), mTGSSolverBodyTxInertias.begin(),
                                   mActorGlobalPoses.begin(), nbDynamicActors, dt);
        for (PxU32 i = 0; i < nbArticulations; i++) PxUpdateArticulationBodiesTGS(articulations[i], dt);
        ...
        PxIntegrateSolverBodies(mSolverBodyData.begin(), mSolverBodies.begin(), mMotionLinearVelocity.begin(),
                                mMotionAngularVelocity.begin(), nbDynamicActors, dt);
        for (PxU32 i = 0; i < nbArticulations; i++) PxUpdateArticulationBodies(articulations[i], dt);
...
bool ImmediateScene::raycastClosest(PxGeomRaycastHit& pxHit,
                                    float& minDist,
                                    PxU32 i,
                                    const PxVec3& origin,
                                    const PxVec3& dir,
    ...
    if (PxGeometryQuery::raycast(origin, dir, geom, shapePose, dist, PxHitFlag::eDEFAULT, 1, &pxHit,
                                 sizeof(PxGeomRaycastHit), PxGeometryQueryFlag::Enum(0)))
#else
    if (PxGeometryQuery::raycast(origin, dir, geom, shapePose, dist, PxHitFlag::eDEFAULT, 1, &pxHit))
#endif
    {
