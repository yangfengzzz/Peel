...
void* SharedSphereShapes::FindShape(float radius,
                                    const void* material,
    ...
    while (Size--) {
        ...
        if (CurrentShape.mRadius == radius && CurrentShape.Compare(material, renderer, local_pose, collision_group))
            return CurrentShape.mShape;
    }
    return null;
}

void* SharedBoxShapes::FindShape(const Point& extents,
                                 const void* material,
                                 PintShapeRenderer* renderer,
                                 const PR& local_pose,
                                 udword collision_group) const {
    ...
    while (Size--) {
        ...
        if (CurrentShape.mExtents == extents && CurrentShape.Compare(material, renderer, local_pose, collision_group))
            return CurrentShape.mShape;
    }
    return null;
}

void* SharedCapsuleShapes::FindShape(float radius,
                                     float half_height,
                                     const void* material,
                                     PintShapeRenderer* renderer,
                                     const PR& local_pose,
    ...
    while (Size--) {
        ...
        if (CurrentShape.mRadius == radius && CurrentShape.mHalfHeight == half_height &&
...
void* SharedMeshShapes::FindShape(const void* mesh,
                                  const void* material,
                                  PintShapeRenderer* renderer,
                                  const PR& local_pose,
                                  udword collision_group) const {
    ...
    while (Size--) {
        ...
        if (CurrentShape.mMesh == mesh && CurrentShape.Compare(material, renderer, local_pose, collision_group))
            return CurrentShape.mShape;
    }
    return null;
}
