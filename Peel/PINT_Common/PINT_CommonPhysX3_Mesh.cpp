...
void MeshManager::ReleaseMeshes() {
    {
        ...
        for (udword i = 0; i < Size; i++) {
            ...
            if (Current.mConvexMesh) Current.mConvexMesh->release();
        }
        mConvexes.clear();
    }
    {
        ...
        for (udword i = 0; i < Size; i++) {
            ...
            if (Current.mTriangleMesh) Current.mTriangleMesh->release();
        }
        mMeshes.clear();
    }
}

PxConvexMesh* MeshManager::CreateConvexMesh(
        const Point* verts, udword nb_verts, PxConvexFlags flags, PintShapeRenderer* renderer, bool share_meshes) {
    ...
    if (renderer) mConvexes.push_back(ConvexRender(NewConvex, renderer));

    return NewConvex;
}

PxTriangleMesh* MeshManager::CreateTriangleMesh(const PintSurfaceInterface& surface,
    ...
    if (renderer) mMeshes.push_back(MeshRender(NewMesh, renderer));
...
bool MeshObjectManager::DeleteObject(void* object, const udword* index) {
    udword Index;
    if (index) {
        if (*index >= mObjects.GetNbEntries()) return false;
        Index = *index;
    } else {
        if (!mObjects.Contains(object, &Index)) return false;
    }

    if (object && mObjects[Index] != object) return false;
