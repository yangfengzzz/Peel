...
PxPBDParticleSystem* initParticles(PxPhysics& physics,
                                   PxScene& scene,
    ...
    if (!cudaContextManager) return null;

    PxPBDMaterial* defaultMat = physics.createPBDMaterial(
            params.mFluidFriction, params.mFluidDamping, params.mFluidAdhesion, params.mFluidViscosity,
            params.mFluidVorticityConfinement, params.mFluidSurfaceTension, params.mFluidCohesion, params.mFluidLift,
            params.mFluidDrag, params.mFluidCflCoefficient, params.mFluidGravityScale);
    ...
    PxPBDParticleSystem* particleSystem =
            physics.createPBDParticleSystem(maxParticles, maxInflatables, maxVolumes, maxRigids, maxCloths,
                                            *cudaContextManager, maxNeighborhood, maxDiffuseParticles);
    ...
    // const PxReal particleMass = fluidDensity * 1.333f * 3.14159f * particleSpacing * particleSpacing *
    // particleSpacing;
    ...
    const PxU32 particlePhase = particleSystem->createPhase(
            defaultMat, PxParticlePhaseFlags(PxParticlePhaseFlag::eParticlePhaseFluid |
                                             PxParticlePhaseFlag::eParticlePhaseSelfCollide));

    PxBuffer* activeIndexBuf =
            physics.createBuffer(maxParticles * sizeof(PxU32), PxBufferType::eHOST, cudaContextManager);
    PxBuffer* phaseBuf = physics.createBuffer(maxParticles * sizeof(PxU32), PxBufferType::eHOST, cudaContextManager);
    PxBuffer* positionInvMassBuf =
            physics.createBuffer(maxParticles * sizeof(PxVec4), PxBufferType::eHOST, cudaContextManager);
    PxBuffer* velocityBuf =
            physics.createBuffer(maxParticles * sizeof(PxVec4), PxBufferType::eHOST, cudaContextManager);
...
static void createVBO(GLuint* vbo,
                      CUgraphicsResource* vbo_res,
                      PxU32 size,
                      PxCudaInteropRegisterFlags flags = PxCudaInteropRegisterFlags()) {
...
SharedGLBuffer::SharedGLBuffer(PxCudaContextManager* contextManager)
    : mCudaContextManager(contextManager), mVboRes(null), mDevicePointer(null), mVbo(0), mSize(0) {}
...
void* SharedGLBuffer::mapForCudaAccess() {
    if (mDevicePointer) return mDevicePointer;
    size_t numBytes;
    if (mCudaContextManager) mCudaContextManager->acquireContext();
    if (mVboRes) mDevicePointer = mapCudaGraphicsResource(reinterpret_cast<CUgraphicsResource*>(&mVboRes), numBytes, 0);
    if (mCudaContextManager) mCudaContextManager->releaseContext();
    return mDevicePointer;
}

void SharedGLBuffer::unmap() {
    if (!mDevicePointer) return;
    if (mCudaContextManager) mCudaContextManager->acquireContext();
    if (mVboRes) unmapCudaGraphicsResource(reinterpret_cast<CUgraphicsResource*>(&mVboRes), 0);
    if (mCudaContextManager) mCudaContextManager->releaseContext();
    mDevicePointer = NULL;
}
...
void onBeforeRenderParticles(PxPBDParticleSystem* particleSystem) {
    //	return;
    if (particleSystem) {
        if (gPosBuffer) particleSystem->copyData(gPosBuffer->mapForCudaAccess(), PxParticleDataFlag::ePOSITION_INVMASS);
        if (gDiffusePosLifeBuffer)
            particleSystem->copyDiffuseParticleData(gDiffusePosLifeBuffer->mapForCudaAccess(),
                                                    PxDiffuseParticleDataFlag::eDIFFUSE_POSITION_LIFETIME);
    }
}

void DrawPoints(GLuint vbo,
                PxU32 numPoints,
                const PxVec3& color,
                float scale,
                PxU32 coordinatesPerPoint = 3,
...
void renderParticles(PintRender& renderer, PxPBDParticleSystem* particleSystem) {
    ...
    if (gPosBuffer) gPosBuffer->unmap();
    if (gDiffusePosLifeBuffer) gDiffusePosLifeBuffer->unmap();
...
void allocParticleBuffers(PxPhysics& physics, PxScene& scene, PxPBDParticleSystem* particleSystem) {
    ...
    if (gDiffusePosLifeBuffer) gDiffusePosLifeBuffer->allocate(maxDiffuseParticles * sizeof(PxVec4));

    if (gPosBuffer) gPosBuffer->allocate(maxParticles * sizeof(PxVec4));

    if (0)
        gPositionInvMassCPUBuf =
                physics.createBuffer(maxParticles * sizeof(PxVec4), PxBufferType::eHOST, cudaContexManager);
...
void clearupParticleBuffers() {
    ...
    if (gPosBuffer) gPosBuffer->release();
    if (gDiffusePosLifeBuffer) gDiffusePosLifeBuffer->release();
...
void renderCallback(PxPBDParticleSystem* particleSystem) {
    ...
    /*	stepPhysics(true);
       ...
       reinterpret_cast<PxActor**>(&actors[0]), nbActors); Snippets::renderActors(&actors[0],
       static_cast<PxU32>(actors.size()), true);
            }*/
