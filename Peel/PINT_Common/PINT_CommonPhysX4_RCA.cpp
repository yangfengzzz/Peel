...
// It is controlled by PHYSX_SUPPORT_RCA at the header level. At the cpp level, just add or don't add this file to the
// project.
...
PintArticHandle SharedPhysX::CreateRCArticulation(const PINT_RC_ARTICULATION_CREATE& create) {
    ...
    if (create.mFixBase) Articulation->setArticulationFlags(PxArticulationFlag::eFIX_BASE);
...
static void SetModeAndLimits(const EditableParams& params,
...
static void setupJoint(const EditableParams& params,
                       PxArticulationJointReducedCoordinate* j,
    ...
    /*
                    virtual	void								setMotion(PxArticulationAxis::Enum axis,
       PxArticulationMotion::Enum motion) = 0;
       ...
       = 0; virtual void getLimit(PxArticulationAxis::Enum axis, PxReal& lowLimit, PxReal& highLimit) = 0; virtual void
       setDrive(PxArticulationAxis::Enum axis, const PxReal stiffness, const PxReal damping, const PxReal maxForce, bool
       isAccelerationDrive = false) = 0; virtual void getDrive(PxArticulationAxis::Enum axis, PxReal& stiffness, PxReal&
       damping, PxReal& maxForce, bool& isAcceleration) = 0; virtual void setDriveTarget(PxArticulationAxis::Enum axis,
       const PxReal target) = 0; virtual void setDriveVelocity(PxArticulationAxis::Enum axis, const PxReal targetVel) =
       0; virtual PxReal getDriveTarget(PxArticulationAxis::Enum axis) = 0; virtual PxReal
       getDriveVelocity(PxArticulationAxis::Enum axis) = 0;

                    virtual	void			setFrictionCoefficient(const PxReal coefficient) = 0;
                    virtual	PxReal			getFrictionCoefficient() const = 0;
                    virtual	const char*		getConcreteTypeName() const { return
       "PxArticulationJointReducedCoordinate"; }
    ...
    */
    ...
    if (bc.mJointType == PINT_JOINT_SPHERICAL) {
        ...
        if (bc.mUseMotor) {
            ...
            j->setDrive(Axis, bc.mMotor.mStiffness, bc.mMotor.mDamping, bc.mMotor.mMaxForce,
                        bc.mMotor.mAccelerationDrive);
#else
            j->setDrive(Axis, bc.mMotor.mStiffness, bc.mMotor.mDamping, bc.mMotor.mMaxForce,
                        bc.mMotor.mAccelerationDrive ? PxArticulationDriveType::eACCELERATION
        ...
        /*		virtual void setDrive(PxArticulationAxis::Enum axis, const PxReal stiffness, const PxReal damping,
           const PxReal maxForce, bool isAccelerationDrive = false) = 0; virtual void getDrive(PxArticulationAxis::Enum
           axis, PxReal& stiffness, PxReal& damping, PxReal& maxForce, bool& isAcceleration) = 0; virtual void
           setDriveTarget(PxArticulationAxis::Enum axis, const PxReal target) = 0; virtual void
           setDriveVelocity(PxArticulationAxis::Enum axis, const PxReal targetVel) = 0;*/

        //	PxD6JointDrive(PxReal driveStiffness, PxReal driveDamping, PxReal driveForceLimit, bool isAcceleration =
        //false)
...
PintActorHandle SharedPhysX::CreateRCArticulatedObject(const PINT_OBJECT_CREATE& oc,
                                                       const PINT_RC_ARTICULATED_BODY_CREATE& bc,
    ...
    PxArticulationJointReducedCoordinate* joint =
            static_cast<PxArticulationJointReducedCoordinate*>(((PxArticulationLink*)h)->getInboundJoint());
...
bool SharedPhysX::SetRCADriveEnabled(PintActorHandle handle, bool flag) {
    ...
    if (!RigidActor || RigidActor->getConcreteType() != PxConcreteType::eARTICULATION_LINK) return false;
    ...
    // Ok this is clumsy: articulation joints don't have userData! So we could have stored the active axis there but
    // instead we need to figure it out at runtime here (or we could have used a hashmap but that's kind of heavy for
    // nothing)
    PxArticulationAxis::Enum Axis = PxArticulationAxis::eCOUNT;
    {
...
PintActorHandle SharedPhysX::CreateRCArticulationLink(PxArticulationReducedCoordinate* articulation,
