...
MemoryOutputStream::MemoryOutputStream(PEEL_PhysX3_AllocatorCallback* allocator)
    : mCallback(allocator), mData(NULL), mSize(0), mCapacity(0) {}

MemoryOutputStream::~MemoryOutputStream() {
    if (mData) {
        if (mCallback)
            mCallback->deallocate(mData);
        else
            delete[] mData;
    }
}
...
MemoryInputData::MemoryInputData(PxU8* data, PxU32 length) : mSize(length), mData(data), mPos(0) {}
...
PxU32 MemoryInputData::getLength() const { return mSize; }

void MemoryInputData::seek(PxU32 offset) { mPos = PxMin<PxU32>(mSize, offset); }

PxU32 MemoryInputData::tell() const { return mPos; }
...
PxRigidBody* PhysX3::GetRigidBody(PintActorHandle handle) {
    ...
    if (RigidActor->getConcreteType() == PxConcreteType::eRIGID_DYNAMIC) {
        ...
        if (RigidDynamic->getRigidDynamicFlags() & PxRigidDynamicFlag::eKINEMATIC) return null;
    ...
    if (RigidBody && !(RigidBody->getRigidBodyFlags() & PxRigidBodyFlag::eKINEMATIC)) return RigidBody;
#endif
    return null;
}
...
static void SetMassProperties(const PINT_OBJECT_CREATE& desc, PxRigidBody& rigid_body) {
    ...
    if (desc.mMassForInertia < 0.0f) {
        ...
        bool status = PxRigidBodyExt::setMassAndUpdateInertia(
                rigid_body, desc.mMass /*, desc.mCOMLocalOffset.IsNonZero() ? &LocalCOMOffset : NULL*/);
        //		bool status = PxRigidBodyExt::updateMassAndInertia(rigid_body, desc.mMass/*,
        //desc.mCOMLocalOffset.IsNonZero() ? &LocalCOMOffset : NULL*/);
        ASSERT(status);
    } else {
        // bool status = PxRigidBodyExt::setMassAndUpdateInertia(rigid_body, desc.mMassForInertia, &ZeroCOM);
        bool status = PxRigidBodyExt::setMassAndUpdateInertia(
                rigid_body, desc.mMassForInertia /*, desc.mCOMLocalOffset.IsNonZero() ? &LocalCOMOffset : NULL*/);
        ASSERT(status);
...
static void CaptureLocalVectors(const PxJoint& joint, const PxVec3* wsAnchor, const PxVec3* axisIn) {
    ...
    for (PxU32 i = 0; i < 2; i++) localPose[i] = PxTransform(PxIdentity);
    ...
    if (axisIn) {
        ...
        // Y to make the frame right handed. Note that the above call makes a right handed frame if we pass X --> Y,Z,
        // so it need not be changed.
...
/*static void PxSetJointGlobalFrame(PxJoint& joint, PxRigidActor* actor, const PxVec3& localAnchor, const PxVec3&
localAxis, PxU32 i)
{
        PxQuat localRot;
        {
...
static bool HingeNeedsLimits(float minLimitAngle, float maxLimitAngle) {
    if (minLimitAngle > maxLimitAngle) return false;
    ...
    switch (desc.mType) {
        case PINT_JOINT_SPHERICAL: {
            ...
            if (0) {
                printf("mObject0: %p\n", jc.mObject0);
                printf("mObject1: %p\n", jc.mObject1);
                printf("mLocalPivot0: %f %f %f\n", jc.mLocalPivot0.mPos.x, jc.mLocalPivot0.mPos.y,
                       jc.mLocalPivot0.mPos.z);
                printf("mLocalPivot1: %f %f %f\n", jc.mLocalPivot1.mPos.x, jc.mLocalPivot1.mPos.y,
                       jc.mLocalPivot1.mPos.z);
            }
        ...
        case PINT_JOINT_HINGE: {
            ...
            if (use_d6_joint) {
                //				const PxQuat q0 = PxShortestRotation(PxVec3(1.0f, 0.0f, 0.0f),
                //ToPxVec3(jc.mLocalAxis0)); 				const PxQuat q1 = PxShortestRotation(PxVec3(1.0f, 0.0f, 0.0f),
                //ToPxVec3(jc.mLocalAxis1));

                PxD6Joint* j = PxD6JointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0), q0), actor1,
                                               PxTransform(ToPxVec3(jc.mLocalPivot1), q1));
                ASSERT(j);

                /*			if(1)
                                        {
                                                PxSetJointGlobalFrame(*j, actor0, ToPxVec3(jc.mLocalPivot0),
                   ToPxVec3(jc.mLocalAxis0), 0); PxSetJointGlobalFrame(*j, actor1, ToPxVec3(jc.mLocalPivot1),
                   ToPxVec3(jc.mLocalAxis1), 1);
                                        }*/
                ...
                if (j) {
                    ...
                    if (jc.mUseMotor) {
                        const PxD6JointDrive drive(gHingeDriveStiffness, gHingeDriveDamping, gHingeDriveForceLimit,
                                                   gHingeDriveIsAcceleration);
                        j->setDrive(PxD6Drive::eTWIST, drive);
                        //	virtual void				setDrivePosition(const PxTransform& pose, bool
                        //autowake = true)	= 0; 					j->setDriveVelocity(PxVec3(0.0f), PxVec3(0.0f, 0.0f,
                        //jc.mDriveVelocity));
                        ...
                        /*					j->setRevoluteJointFlag(PxRevoluteJointFlag::eDRIVE_ENABLED,
                           true); j->setRevoluteJointFlag(PxRevoluteJointFlag::eDRIVE_FREESPIN, false);
                ...
                if (j) {
                    ...
                    if (IsLimitedHinge) {
                        j->setLimit(limit);
                        //				j->setLimit(PxJointLimitPair(minLimitAngle, maxLimitAngle,
                        //TWOPI));
                    ...
                    /*
                                    PxRevoluteJoint* rv = PxRevoluteJointCreate(physics, b0->mBody,
                       PxTransform::createIdentity(), b1->mBody, PxTransform::createIdentity()); mJoints[i] = rv;
                    ...
                    */
                    ...
                    if (jc.mUseMotor) {
                        ...
                        /*					ASSERT(!j->userData);
                                                                ...
                                                                j->userData = MD;
                                                                // We store the ptr in a redundant array just to be able
                           to conveniently release the objects when closing. mMotorData.push_back(MD);*/
                    }

                    //				j->getConstraint()->setFlag(PxConstraintFlag::eDRIVE_LIMITS_ARE_FORCES,
                    //true);
                }
            }
        } break;
        ...
        case PINT_JOINT_HINGE2: {
            ...
            if (use_d6_joint) {
                PxD6Joint* j = PxD6JointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0), actor1,
                                               PxTransform(ToPxVec3(jc.mLocalPivot1.mPos), q1));
                ASSERT(j);
                ...
                // "The axis of the hinge is specified as the direction of the x-axis in the body's joint frame."
                PxRevoluteJoint* j =
                        PxRevoluteJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0), actor1,
                                              PxTransform(ToPxVec3(jc.mLocalPivot1.mPos), q1));
                ASSERT(j);
                ...
                if (j) {
                    ...
                    if (jc.mUseMotor) {
                        ...
                        /*					ASSERT(!j->userData);
                                                        MotorData* MD = ICE_NEW(MotorData);
                                                        j->userData = MD;
                                                        // We store the ptr in a redundant array just to be able to
                           conveniently release the objects when closing. mMotorData.push_back(MD);*/
                    }
        ...
        case PINT_JOINT_FIXED: {
            ...
            if (actor0) pose0 = actor0->getGlobalPose();
            if (actor1) pose1 = actor1->getGlobalPose();
            ...
            if (use_d6_joint) {
                //				PxD6Joint* j = PxD6JointCreate(physics, actor0,
                //PxTransform(ToPxVec3(jc.mLocalPivot0)), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1)));
                PxD6Joint* j = PxD6JointCreate(physics, actor0, LocalFrame0, actor1, LocalFrame1);
                ASSERT(j);
                ...
                //				q.normalize();
                //				PxFixedJoint* j = PxFixedJointCreate(physics, actor0,
                //PxTransform(ToPxVec3(jc.mLocalPivot0), q), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1)));
        ...
        case PINT_JOINT_PRISMATIC: {
            ...
            const PxQuat q0 = jc.mLocalAxis0.IsNonZero() ? ComputeJointQuat(actor0, ToPxVec3(jc.mLocalAxis0))
                                                         : ToPxQuat(jc.mLocalPivot0.mRot);
            const PxQuat q1 = jc.mLocalAxis1.IsNonZero() ? ComputeJointQuat(actor1, ToPxVec3(jc.mLocalAxis1))
                                                         : ToPxQuat(jc.mLocalPivot1.mRot);
            ...
            if (1 && use_d6_joint) {
                ...
                PxD6Joint* j = PxD6JointCreate(physics, actor0, PxTransform(ToPxVec3(LocalPivot0), q0), actor1,
                                               PxTransform(ToPxVec3(LocalPivot1), q1));
                ASSERT(j);
                ...
                if (j) {
                    ...
                    if (IsPrismaticLimitEnabled(jc.mLimits)) {
                        ...
                        if (1)  // New API
                        {
                            ...
                            //						const PxJointLinearLimitPair Limits(MinLimit, MaxLimit,
                            //PxSpring(jc.mSpringStiffness, jc.mSpringDamping));
                            // ### these hard joints are better looking against limits but don't support "springs", at
                            // least in this form
                            //						const PxJointLinearLimitPair Limits(PxTolerancesScale(), MinLimit, MaxLimit,
                            //ContactDistance);

                            PxJointLinearLimitPair Limits(MinLimit, MaxLimit,
                                                          PxSpring(jc.mSpring.mStiffness, jc.mSpring.mDamping));
                            if (jc.mSpring.mStiffness == 0.0f && jc.mSpring.mDamping == 0.0f) {
                                // ### these hard joints are better looking against limits but don't support "springs",
                        ...
                        */
                        ...
                        /*
                                                                        const PxJointLinearLimitPair
                        Limits(jc.mMinLimit, jc.mMaxLimit, PxSpring(jc.mSpringStiffness, jc.mSpringDamping));
                                                                        //### these hard joints are better looking
                        against limits but don't support "springs", at least in this form
                                //					const PxJointLinearLimitPair
                        Limits(PxTolerancesScale(), jc.mMinLimit, jc.mMaxLimit);
                                ////				Limits.contactDistance	= 10.0f;
                                                                        j->setLinearLimitX(Limits);

                        //							j->setMotion(PxD6Axis::eX,
                        PxD6Motion::eLOCKED); j->setMotion(PxD6Axis::eY, PxD6Motion::eLIMITED);
                                                                                j->setLinearLimitY(Limits);
                        //							j->setMotion(PxD6Axis::eZ,
                        PxD6Motion::eLIMITED);
                ...
                CreatedJoint = CreatePrismaticJoint(mParams, physics, jc, actor0, actor1,
                                                    PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0),
                                                    PxTransform(ToPxVec3(jc.mLocalPivot1.mPos), q1));
            }
        } break;
...
// #define TEMP_TEST
            ...
            CreatedJoint = CreateDistanceJoint(mParams, physics, jc, actor0, actor1,
                                               PxTransform(ToPxVec3(jc.mLocalPivot0), q0),
                                               PxTransform(ToPxVec3(jc.mLocalPivot1), q1));
#else
            CreatedJoint =
                    CreateDistanceJoint(mParams, physics, jc, actor0, actor1, PxTransform(ToPxVec3(jc.mLocalPivot0)),
                                        PxTransform(ToPxVec3(jc.mLocalPivot1)));
#endif
        } break;
            ...
            if (j) {
                ...
                if (1) {
                    /*				const Point& AngularLimitsMin = jc.mAngularLimitsMin;
                                                    ...
                                                    {
                                                            j->setMotion(PxD6Axis::eTWIST, PxD6Motion::eLIMITED);
                                                            const PxJointAngularLimitPair Limits(AngularLimitsMin.x,
                    AngularLimitsMax.x, FLT_MAX);
                    //					const PxJointAngularLimitPair Limits(AngularLimitsMin.x,
                    AngularLimitsMax.x); j->setTwistLimit(Limits);
                                                    }
                                                    if(AngularLimitsMin.y<AngularLimitsMax.y)
                    ...
                    // Twist
                    if (jc.mMinTwist > jc.mMaxTwist) j->setMotion(PxD6Axis::eTWIST, PxD6Motion::eFREE);
                    if (jc.mMinTwist == jc.mMaxTwist) j->setMotion(PxD6Axis::eTWIST, PxD6Motion::eLOCKED);
                    if (jc.mMinTwist < jc.mMaxTwist) {
                        j->setMotion(PxD6Axis::eTWIST, PxD6Motion::eLIMITED);
                        const PxJointAngularLimitPair Limits(jc.mMinTwist, jc.mMaxTwist, ContactDistance);
                        //					const PxJointAngularLimitPair Limits(jc.mMinTwist,
                        //jc.mMaxTwist);
                    ...
                    if (MaxSwingY > 0.0f || MaxSwingZ > 0.0f) {
                        if (MaxSwingY < 0.0f) MaxSwingY = 0.0f;
                        if (MaxSwingZ < 0.0f) MaxSwingZ = 0.0f;

                        //					j->setSwingLimit(PxJointLimitCone(MaxSwingY,
                        //MaxSwingZ));
                        j->setSwingLimit(PxJointLimitCone(MaxSwingY, MaxSwingZ, ContactDistance));
                        //					j->setSwingLimit(PxJointLimitCone(MaxSwingY, MaxSwingZ,
                        //0.0f)); 					j->setSwingLimit(PxJointLimitCone(MaxSwingY, MaxSwingZ, PxSpring(100.0f, 10.0f)));
                    }
                }
                ...
                if (1) {
                    ...
                    for (udword Index = 0; Index < 3; Index++) {
                        ...
                        if (MinLimit > MaxLimit) j->setMotion(Axis, PxD6Motion::eFREE);
                        if (MinLimit == MaxLimit) j->setMotion(Axis, PxD6Motion::eLOCKED);
                        if (MinLimit < MaxLimit) {
                            j->setMotion(Axis, PxD6Motion::eLIMITED);
                            //				const PxJointLinearLimitPair Limits(jc.mMinLimit, jc.mMaxLimit,
                            //PxSpring(jc.mSpringStiffness, jc.mSpringDamping));
                            // ### these hard joints are better looking against limits but don't support "springs", at
                            // least in this form
                ...
                if (jc.mMotorFlags) {
                    ...
                    if (jc.mMotorFlags & PINT_D6_MOTOR_DRIVE_X) j->setDrive(PxD6Drive::eX, drive);
                    if (jc.mMotorFlags & PINT_D6_MOTOR_DRIVE_Y) j->setDrive(PxD6Drive::eY, drive);
                    if (jc.mMotorFlags & PINT_D6_MOTOR_DRIVE_Z) j->setDrive(PxD6Drive::eZ, drive);
                    /*
        ...
        case PINT_JOINT_GEAR: {
            ...
            PxGearJoint* j = PxGearJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0), actor1,
                                               PxTransform(ToPxVec3(jc.mLocalPivot1.mPos), q1));
            ASSERT(j);
        ...
        case PINT_JOINT_RACK_AND_PINION: {
            ...
            PxRackAndPinionJoint* j =
                    PxRackAndPinionJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0), actor1,
                                               PxTransform(ToPxVec3(jc.mLocalPivot1.mPos), q1));
            ASSERT(j);
        ...
        case PINT_JOINT_CHAIN: {
            ...
            PxChainJoint* j = PxChainJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0),
        ...
        case PINT_JOINT_PORTAL: {
            ...
            //			PxPortalJoint* j = PxPortalJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0)),
            //actor1, PxTransform(ToPxVec3(jc.mLocalPivot1)));
            PxPortalJoint* j = PxPortalJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0)), actor1,
                                                   PxTransform(ToPxVec3(jc.mLocalPivot1)));
            ASSERT(j);
    ...
    if (CreatedJoint) {
        ...
        // - Ropes and thin cables can go through each other easily if we limit their depenetration velocity. So we just
        // disable this feature for jointed & articulated objects.
        struct Local {
...
void SharedPhysX::SharedUpdateFromUI() {
    ...
    */
    // Commented out because it kills the hardcoded per-test debug viz settings in the demo. Not entirely sure it this
    // breaks something.
...
// Starting with PhysX 5 the BVH34 midphase can be either quantized or not.
    ...
    Params.meshPreprocessParams = mParams.mPrecomputeActiveEdges
                                          ? PxMeshPreprocessingFlags(0)
    ...
    if (mParams.mPCM) {
        ...
        //PxMeshPreprocessingFlag::eREMOVE_DUPLICATED_TRIANGLES); 		Params.meshPreprocessParams |=
    ...
    if (mParams.mUseGPU) Params.BUILD_GPU_DATA = true;
#endif
...
void SharedPhysX::SetupDynamic(PxRigidDynamic& rigidDynamic, const PINT_OBJECT_CREATE& desc) {
...
#endif
    if (!desc.mKinematic) ::SetMassProperties(desc, rigidDynamic);
...
#ifdef IS_PHYSX_3_2
    if (!mParams.mEnableSleeping) rigidDynamic.wakeUp(9999999999.0f);
    ...
    if (mEnableContactNotif) rigidDynamic.setContactReportThreshold(mContactNotifThreshold);
    if (mParams.mEnableContactNotif) rigidDynamic.setContactReportThreshold(mParams.mContactNotifThreshold);
#endif
...
#ifdef CAPTURE_VECTORS
    SaveAsSource("D://tmp//LocalVectors.cpp", "LocalVectors", gLocalVectors, gNbLocalVectors * sizeof(PxVec3),
                 gNbLocalVectors * sizeof(PxVec3), PACK_NONE);
#endif

    if (mScene) {
        if (1) {
#ifdef TEST_FLUIDS
            if (gParticleSystem) onBeforeRenderParticles(gParticleSystem);
#endif

            const udword NbSubsteps = mParams.mNbSubsteps;
            const float sdt = dt / float(NbSubsteps);
            for (udword i = 0; i < NbSubsteps; i++) {
                if (NbSubsteps > 1) ApplyLocalTorques();

#if PHYSX_SUPPORT_SQ_UPDATE_MODE
                const bool IsLastSubstep = i == NbSubsteps - 1;
                mScene->setSceneQueryUpdateMode(IsLastSubstep
...
#if PHYSX_SUPPORT_RAYCAST_CCD
    if (gRaycastCCDManager) gRaycastCCDManager->doRaycastCCD(mParams.mEnableRaycastCCDDynamic);
#endif
    // UpdateJointErrors();
}
...
PintActorHandle SharedPhysX::CreateObject(const PINT_OBJECT_CREATE& desc) {
    ...
    if (rigidDynamic) SetupDynamic(*rigidDynamic, desc);
    ...
    if (desc.mAddToWorld) {
        ...
        if (rigidDynamic && !desc.mKinematic) SetupSleeping(rigidDynamic, mParams.mEnableSleeping);
    } else {
        if (rigidDynamic && !desc.mKinematic) {
            // SetupSleeping(rigidDynamic, mParams.mEnableSleeping);
            if (!mParams.mEnableSleeping) rigidDynamic->setWakeCounter(9999999999.0f);
        }
    }
...
#else
    PxJoint* J = reinterpret_cast<PxJoint*>(handle);
    if (J->getConcreteType() != PxJointConcreteType::eD6) return false;
...
bool SharedPhysX::SetDriveVelocity(PintJointHandle handle, const Point& linear, const Point& angular) {
    ...
    if (J->getConcreteType() == PxJointConcreteType::eREVOLUTE) {
        PxRevoluteJoint* RevoluteJoint = static_cast<PxRevoluteJoint*>(J);
        // The revolute joint only supports one float so this is going to be clumsy. We use angular.x by convention for
        // now.
...
PR SharedPhysX::GetDrivePosition(PintJointHandle handle) {
    ...
    if (J->getConcreteType() != PxJointConcreteType::eD6) return PR(Idt);
...
/*void SharedPhysX::ApplyActionAtPoint(PintObjectHandle handle, PintActionType action_type, const Point& action, const
Point& pos)
{
...
void SharedPhysX::AddWorldImpulseAtWorldPos(PintActorHandle handle,
                                            const Point& world_impulse,
                                            const Point& world_pos) {
#ifndef PHYSX3_DISABLE_SHARED_APPLY_ACTION
    PxRigidBody* RigidBody = GetRigidBody(handle);
    if (RigidBody) {
        if (!(RigidBody->getActorFlags() & PxActorFlag::eDISABLE_SIMULATION))
            PxRigidBodyExt::addForceAtPos(*RigidBody, ToPxVec3(world_impulse), ToPxVec3(world_pos),
...
Point SharedPhysX::GetWorldLinearVelocity(PintActorHandle handle) { return GetWorldVelocity(handle, false); }

Point SharedPhysX::GetWorldAngularVelocity(PintActorHandle handle) { return GetWorldVelocity(handle, true); }
...
Point SharedPhysX::GetAngularVelocity(PintActorHandle handle) { return GetLocalVelocity(handle, true); }

Point SharedPhysX::GetLinearVelocity(PintActorHandle handle) { return GetLocalVelocity(handle, false); }
...
Point SharedPhysX::GetLocalInertia(PintActorHandle handle) {
    PxRigidBody* RigidBody = PhysX3::GetRigidBody(handle);
    if (!RigidBody) return Point(0.0f, 0.0f, 0.0f);
...
float SharedPhysX::GetLinearDamping(PintActorHandle handle) {
    PxRigidActor* Actor = GetActorFromHandle(handle);
    if (!Actor || Actor->getConcreteType() != PxConcreteType::eRIGID_DYNAMIC) return 0.0f;
...
void SharedPhysX::SetLinearDamping(PintActorHandle handle, float damping) {
    PxRigidActor* Actor = GetActorFromHandle(handle);
    if (!Actor || Actor->getConcreteType() != PxConcreteType::eRIGID_DYNAMIC) return;
...
bool SharedPhysX::SetKinematicPose(PintActorHandle handle, const Point& pos) {
    if (!IsKinematic(handle)) return false;
    PxRigidActor* Actor = GetActorFromHandle(handle);
    //	if(!Actor || Actor->getConcreteType()!=PxConcreteType::eRIGID_DYNAMIC)
    //		return false;
...
PintConvexHandle SharedPhysX::CreateConvexObject(const PINT_CONVEX_DATA_CREATE& desc, PintConvexIndex* index) {
    PxConvexMesh* ConvexMesh =
            ::CreatePhysXConvex(mPhysics, GetCooking(), desc.mNbVerts, desc.mVerts, mParams.GetConvexFlags());
...
bool SharedPhysX::DeleteConvexObject(PintConvexHandle handle, const PintConvexIndex* index) {
    if (!mConvexObjects.DeleteObject(handle, index)) return false;
...
static PxTriangleMesh* CreatePhysXMesh(PxPhysics* physics,
                                       ...
                                       PxCooking& cooking,
#endif
                                       const PintSurfaceInterface& surface,
                                       bool deformable,
                                       bool dynamic) {
    ASSERT(physics);
    ...
    if (dynamic) {
        ...
        const_cast<PxCookingParams&>(cookingParams)
                .meshPreprocessParams.clear(PxMeshPreprocessingFlag::eENABLE_INERTIA);
    }
#endif
...
bool SharedPhysX::DeleteMeshObject(PintMeshHandle handle, const PintMeshIndex* index) {
    if (!mMeshObjects.DeleteObject(handle, index)) return false;
...
/*PxTriangleMesh* SharedPhysX::CreateTriangleMesh(const SurfaceInterface& surface, PintShapeRenderer* renderer, bool
deformable)
{
        ASSERT(mCooking);
        ASSERT(mPhysics);
...
#if PHYSX_SUPPORT_HEIGHTFIELDS
PintHeightfieldHandle SharedPhysX::CreateHeightfieldObject(const PINT_HEIGHTFIELD_DATA_CREATE& desc,
                                                           PintHeightfieldData& data,
                                                           PintHeightfieldIndex* index) {
    ASSERT(mPhysics);
#if !PHYSX_SUPPORT_IMMEDIATE_COOKING
    ASSERT(mCooking);
#endif
...
// hfDesc.flags			= PxHeightFieldFlag::eNO_BOUNDARY_EDGES;
    ...
    if (Index >= mHFDataSize) {
        ...
        if (mHFData) CopyMemory(NewData, mHFData, mHFDataSize * sizeof(HFCompanionData));
        DELETEARRAY(mHFData);
        mHFData = NewData;
    }
...
bool SharedPhysX::DeleteHeightfieldObject(PintHeightfieldHandle handle, const PintHeightfieldIndex* index) {
    if (!mHeightfieldObjects.DeleteObject(handle, index)) return false;
...
PxMaterial* SharedPhysX::CreateMaterial(const PINT_MATERIAL_CREATE& desc) {
    ...
    for (PxU32 i = 0; i < NbMaterials; i++) {
        ...
        if (M->getRestitution() == desc.mRestitution && M->getStaticFriction() == desc.mStaticFriction &&
...
#ifndef IS_PHYSX_3_2
PxShape* SharedPhysX::CreateSphereShape(const PINT_SHAPE_CREATE* create,
                                        PxRigidActor* actor,
                                        const PxSphereGeometry& geometry,
                                        const PxMaterial& material,
                                        const PxTransform& local_pose,
                                        PxU16 collision_group) {
    ...
    if (!mSphereShapes) mSphereShapes = new (SharedSpheres);
    ...
    if (e.mShape) {
        ...
        PxShape* NewShape = CreateSharedShape(*this, mPhysics, create, actor, geometry, material, local_pose,
                                              collision_group, mParams);
        e.mShape = NewShape;
        return NewShape;
    }
...
/*	const InternalSphereShape iss(Radius, &material, create->mRenderer, local_pose, collision_group);
   ...
   collision_group, mParams);
        ...
        return NewShape;*/
#else
    PxShape* S = reinterpret_cast<PxShape*>(
            mSphereShapes.FindShape(Radius, &material, create->mRenderer, ToPR(local_pose), collision_group));
    if (S) {
    ...
    PxShape* NewShape =
            CreateSharedShape(*this, mPhysics, create, actor, geometry, material, local_pose, collision_group, mParams);
...
PxShape* SharedPhysX::CreateBoxShape(const PINT_SHAPE_CREATE* create,
                                     PxRigidActor* actor,
    ...
    PxShape* S = reinterpret_cast<PxShape*>(mBoxShapes.FindShape(ToPoint(geometry.halfExtents), &material,
                                                                 create->mRenderer, ToPR(local_pose), collision_group));
    if (S) {
        // printf("Sharing shape\n");
        actor->attachShape(*S);
        return S;
    ...
    mBoxShapes.RegisterShape(ToPoint(geometry.halfExtents), NewShape, &material, create->mRenderer, ToPR(local_pose),
                             collision_group);

    return NewShape;
}

PxShape* SharedPhysX::CreateCapsuleShape(const PINT_SHAPE_CREATE* create,
                                         PxRigidActor* actor,
    ...
    PxShape* S = reinterpret_cast<PxShape*>(mCapsuleShapes.FindShape(
            geometry.radius, geometry.halfHeight, &material, create->mRenderer, ToPR(local_pose), collision_group));
    if (S) {
        // printf("Sharing shape\n");
        actor->attachShape(*S);
        return S;
    ...
    mCapsuleShapes.RegisterShape(geometry.radius, geometry.halfHeight, NewShape, &material, create->mRenderer,
                                 ToPR(local_pose), collision_group);

    return NewShape;
}

#if PHYSX_SUPPORT_CUSTOM_GEOMETRY
PxShape* SharedPhysX::CreateCylinderShape(const PINT_SHAPE_CREATE* create,
                                          PxRigidActor* actor,
                                          const PxCustomGeometry& geometry,
                                          const PxMaterial& material,
                                          const PxTransform& local_pose,
                                          PxU16 collision_group) {
    if (!create->CanShare(mParams.mShareShapes))
    ...
    PxCustomGeometryExt::CylinderCallbacks* CustomCylinder =
            (PxCustomGeometryExt::CylinderCallbacks*)geometry.callbacks;
    ...
    PxShape* S = reinterpret_cast<PxShape*>(mCylinderShapes.FindShape(CustomCylinder->radius, CustomCylinder->height,
                                                                      &material, create->mRenderer, ToPR(local_pose),
                                                                      collision_group));
    if (S) {
        // printf("Sharing shape\n");
        actor->attachShape(*S);
    ...
    mCylinderShapes.RegisterShape(CustomCylinder->radius, CustomCylinder->height, NewShape, &material,
                                  create->mRenderer, ToPR(local_pose), collision_group);
    return NewShape;
}
#endif

PxShape* SharedPhysX::CreateConvexShape(const PINT_SHAPE_CREATE* create,
                                        PxRigidActor* actor,
    ...
    PxShape* S = reinterpret_cast<PxShape*>(mConvexShapes.FindShape(geometry.convexMesh, &material, create->mRenderer,
                                                                    ToPR(local_pose), collision_group));
    if (S) {
        // printf("Sharing shape\n");
        actor->attachShape(*S);
    ...
    mConvexShapes.RegisterShape(geometry.convexMesh, NewShape, &material, create->mRenderer, ToPR(local_pose),
                                collision_group);
    return NewShape;
}

PxShape* SharedPhysX::CreateMeshShape(const PINT_SHAPE_CREATE* create,
                                      PxRigidActor* actor,
    ...
    PxShape* S = reinterpret_cast<PxShape*>(mMeshShapes.FindShape(geometry.triangleMesh, &material, create->mRenderer,
                                                                  ToPR(local_pose), collision_group));
    if (S) {
        // printf("Sharing shape\n");
        actor->attachShape(*S);
    ...
    mMeshShapes.RegisterShape(geometry.triangleMesh, NewShape, &material, create->mRenderer, ToPR(local_pose),
                              collision_group);
    return NewShape;
}

#if PHYSX_SUPPORT_HEIGHTFIELDS
PxShape* SharedPhysX::CreateHeightfieldShape(const PINT_SHAPE_CREATE* create,
                                             PxRigidActor* actor,
    ...
    PxShape* S = reinterpret_cast<PxShape*>(mHeightfieldShapes.FindShape(
            geometry.heightField, &material, create->mRenderer, ToPR(local_pose), collision_group));
    if (S) {
        // printf("Sharing shape\n");
        actor->attachShape(*S);
        return S;
    ...
    mHeightfieldShapes.RegisterShape(geometry.heightField, NewShape, &material, create->mRenderer, ToPR(local_pose),
                                     collision_group);
    return NewShape;
}
#endif
...
//	if(!(observed->getConcreteType()==PxConcreteType:: eRIGID_DYNAMIC ||
observed->getConcreteType()==PxConcreteType:: eRIGID_STATIC ||
...
void SharedPhysX::ReportShape(const PINT_SHAPE_CREATE& create, udword index, void* user_data) {
    ...
    if (create.mType == PINT_SHAPE_SPHERE) {
        ...
        shape = CreateSphereShape(&create, actor, PxSphereGeometry(SphereCreate.mRadius), *ShapeMaterial, LocalPose,
                                  group);
        // shape = CreateCapsuleShape(&create, actor, PxCapsuleGeometry(SphereCreate.mRadius, 0.0f), *ShapeMaterial,
        // LocalPose, group);
    } else if (create.mType == PINT_SHAPE_BOX) {
        const PINT_BOX_CREATE& BoxCreate = static_cast<const PINT_BOX_CREATE&>(create);
        shape = CreateBoxShape(&create, actor, PxBoxGeometry(ToPxVec3(BoxCreate.mExtents)), *ShapeMaterial, LocalPose,
                               group);
        ...
        */
        ...
        shape = CreateCapsuleShape(&create, actor, PxCapsuleGeometry(CapsuleCreate.mRadius, CapsuleCreate.mHalfHeight),
                                   *ShapeMaterial, LocalPose, group);
    }
#if PHYSX_SUPPORT_CUSTOM_GEOMETRY
        ...
        //			const PxCustomGeometry CustomGeom =
        //InternalCustomCylinder::getGeometry(CylinderCreate.mHalfHeight * 2.0f, CylinderCreate.mRadius, 1, 0.0f);
        // ####
        // where is this released?
        // PxCustomGeometryExt::Callbacks* cb = PxCustomGeometryExt::createCylinderCallbacks(CylinderCreate.mHalfHeight
        // * 2.0f, CylinderCreate.mRadius, 1, 0.0f); const PxCustomGeometry CustomGeom(*cb);

        // PxCustomGeometryExt::CylinderCallbacks* cb = new
        // PxCustomGeometryExt::CylinderCallbacks(CylinderCreate.mHalfHeight * 2.0f, CylinderCreate.mRadius, 1, 0.0f);
        PxCustomGeometryExt::CylinderCallbacks* cb = new PxCustomGeometryExt::CylinderCallbacks(
                CylinderCreate.mHalfHeight * 2.0f, CylinderCreate.mRadius, 1, 0.0f);
        ...
        //			PxConvexMesh* ConvexMesh = CreateConvexMesh(ConvexCreate.mVerts, ConvexCreate.mNbVerts,
        //PxConvexFlag::eCOMPUTE_CONVEX|PxConvexFlag::eINFLATE_CONVEX, create.mRenderer); 			PxConvexMesh* ConvexMesh =
        //CreateConvexMesh(ConvexCreate.mVerts, ConvexCreate.mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX, create.mRenderer);
        PxConvexMesh* ConvexMesh = CreateConvexMesh(ConvexCreate.mVerts, ConvexCreate.mNbVerts,
                                                    mParams.GetConvexFlags(), create.mRenderer, mParams.mShareMeshData);
        ASSERT(ConvexMesh);
        ...
        PxTriangleMesh* TriangleMesh =
                CreateTriangleMesh(MeshCreate.GetSurface(), create.mRenderer, MeshCreate.mDeformable,
                                   mParams.mShareMeshData, Params->mIsDynamic);
        ASSERT(TriangleMesh);

        if (1)
            shape = CreateMeshShape(&create, actor, PxTriangleMeshGeometry(TriangleMesh), *ShapeMaterial, LocalPose,
                                    group);
        else {
    ...
    if (forced_name && shape) shape->setName(forced_name);
}

void SharedPhysX::CreateShapes(const PINT_OBJECT_CREATE& desc,
                               PxRigidActor* actor,
                               PintCollisionGroup group,
...
PintArticHandle SharedPhysX::CreateArticulation(const PINT_ARTICULATION_CREATE&) {
    if (mParams.mDisableArticulations) return null;
...
bool SharedPhysX::AddArticulationToScene(PintArticHandle articulation) {
    ...
    // TODO: hack to detect that adding the articulation failed on the GPU. We should revisit this and return bool or
    // something.
    const udword NbErrors = gNbPhysXErrors;
    mScene->addArticulation(*Articulation);
    if (gNbPhysXErrors != NbErrors) return false;
    ...
    for (udword i = 0; i < NbLinks; i++) AddActorToManager(Links[i]);
    ICE_FREE(Links);
    return true;
}
...
static void setupJoint(PxArticulationJoint* j, const PINT_ARTICULATED_BODY_CREATE& bc) {
    ...
    if (bc.mEnableSwingLimit) j->setSwingLimit(bc.mSwingYLimit, bc.mSwingZLimit);

    j->setTwistLimitEnabled(bc.mEnableTwistLimit);
    if (bc.mEnableTwistLimit) j->setTwistLimit(bc.mTwistLowerLimit, bc.mTwistUpperLimit);

    if (bc.mUseMotor) {
        if (bc.mMotor.mExternalCompliance != 0.0f) j->setExternalCompliance(bc.mMotor.mExternalCompliance);
        if (bc.mMotor.mInternalCompliance != 0.0f) j->setInternalCompliance(bc.mMotor.mInternalCompliance);
...
#endif
        if (!bc.mMotor.mTargetVelocity.IsNotUsed()) j->setTargetVelocity(ToPxVec3(bc.mMotor.mTargetVelocity));
        if (!bc.mMotor.mTargetOrientation.IsNotUsed()) j->setTargetOrientation(ToPxQuat(bc.mMotor.mTargetOrientation));
    }
}

PintActorHandle SharedPhysX::CreateArticulatedObject(const PINT_OBJECT_CREATE& oc,
                                                     const PINT_ARTICULATED_BODY_CREATE& bc,
                                                     PintArticHandle articulation) {
...
void SharedPhysX::SetArticulatedMotor(PintActorHandle handle, const PINT_ARTICULATED_MOTOR_CREATE& motor) {
    ...
    if (RigidBody) {
        ...
        if (j) {
            if (motor.mExternalCompliance != 0.0f) j->setExternalCompliance(motor.mExternalCompliance);
            if (motor.mInternalCompliance != 0.0f) j->setInternalCompliance(motor.mInternalCompliance);
...
PintActorHandle SharedPhysX::CreateArticulationLink(PxArticulation* articulation,
                                                    PxArticulationLink* parent,
                                                    Pint& pint,
                                                    const PINT_OBJECT_CREATE& desc) {
    ...
    if (actor) SetupArticulationLink(*actor, desc);
...
#ifndef IS_PHYSX_3_2
PxQueryHitType::Enum SharedPhysX::preFilter(const PxFilterData& filterData,
                                            const PxShape* shape,
                                            const PxRigidActor* actor,
                                            PxHitFlags& queryFlags) {
    if (mInvisibles && mInvisibles->contains(PintActorHandle(actor))) return PxQueryHitType::eNONE;
...
bool SharedPhysX::SetSQFlag(PintActorHandle actor, bool flag) {
    if (!mInvisibles) mInvisibles = new _hashset<PintActorHandle>;
...
static inline_ void RenderShape(PintRender& renderer, PxShape* shape, const PxTransform& shapePose) {
    if (!renderer.SetCurrentShape(PintShapeHandle(shape))) return;
    ...
    // Reactivated for internal XPs
        ...
        RenderShapeGeneric(renderer, shape, IcePose);
}
...
#ifdef TEST_FLUIDS0
    if (gParticleSystem) renderParticles(renderer, gParticleSystem);
#endif
...
#define USE_ACTOR_MANAGER_CACHE
        ...
        for (udword pass = 0; pass < 2; pass++) {
            ...
            const ActorCachedData* CachedData =
                    pass ? mActorManager.GetStaticActorsCachedData() : mActorManager.GetDynamicActorsCachedData();
#endif
            if (!FrustumPlanes) nbVisibleActors += NbActors;
                ...
                if (renderer.SetCurrentActor(PintActorHandle(rigidActor))) {
                    ...
                    if (CurrentActor.mSingleShape) {
                        ...
                        for (PxU32 j = 0; j < nbShapes; j++) {
                            ...
                            /*#if PHYSX_SUPPORT_SHARED_SHAPES
                                                                                    // TODO: keep current poses in actor
                            manager (for sleeping objects) const PxTransform Pose = PxShapeExt::getGlobalPose(*shape,
                            *rigidActor); #else const PxTransform Pose = PxShapeExt::getGlobalPose(*shape); #endif*/
...
#ifdef IS_PHYSX_3_2
        const PxActorTypeSelectionFlags selectionFlags =
                PxActorTypeSelectionFlag::eRIGID_STATIC | PxActorTypeSelectionFlag::eRIGID_DYNAMIC;
#else
...
//					const PxTransform Pose = rigidActor->getGlobalPose();
...
#endif
                        // TODO: don't create the ICE pose, convert to matrix directly here with SIMD, pass OpenGL data
                        // to renderers
                        const PR IcePose(ToPoint(Pose.p), ToQuat(Pose.q));

                        //					QuatGetMat33V(const QuatVArg q, Vec3V& column0, Vec3V& column1,
                        //Vec3V& column2)
                        ...
                        if (shape->userData) {
                            ...
                            if (geomType == PxGeometryType::eCAPSULE) {
                                ...
                                /*							Matrix3x3 Rot;
                                                                                        Rot.RotZ(HALFPI);
                                                                                        Quat QQ = IcePose.mRot *
                                   Quat(Rot);*/

                                // ### precompute
                                /*							const PxQuat q =
                                   PxShortestRotation(PxVec3(0.0f, 1.0f, 0.0f), PxVec3(1.0f, 0.0f, 0.0f)); Quat QQ =
                                   IcePose.mRot * ToQuat(q);

                                                                                        shapeRenderer->Render(PR(IcePose.mPos,
                                   QQ));*/
                    ...
                    if (shape->userData) {
                        ...
                        if (geomType == PxGeometryType::eCAPSULE) {
                            ...
                            /*							const PxQuat q = PxShortestRotation(PxVec3(0.0f, 1.0f, 0.0f), PxVec3(1.0f,
                               0.0f, 0.0f)); Quat QQ = IcePose.mRot * ToQuat(q);
                        ...
                        if (geomType == PxGeometryType::eSPHERE) {
                            ...
                            /*							const PxQuat q = PxShortestRotation(PxVec3(0.0f, 1.0f, 0.0f), PxVec3(1.0f,
                               0.0f, 0.0f)); Quat QQ = IcePose.mRot * ToQuat(q);
                                                                                    ...
                                                                                    renderer.DrawCapsule(geometry.radius,
                               geometry.halfHeight*2.0f, PR(IcePose.mPos, QQ));*/
                            PR CapsuleTransform;
                            PhysX3::ComputeCapsuleTransform(CapsuleTransform, IcePose);
...
void SharedPhysX::RenderDebugData(PintRender& renderer) {
    ...
    for (udword i = 0; i < NbTris; i++) {
        renderer.DrawTriangle(ToPoint(Triangles[i].pos0), ToPoint(Triangles[i].pos1), ToPoint(Triangles[i].pos2),
                              TrisColor);
    }
...
void SharedPhysX::ReleaseControllerManager() { SAFE_RELEASE(mControllerManager); }
...
PintCharacterHandle SharedPhysX::CreateCharacter(const PINT_CHARACTER_CREATE& create) {
    ...
    if (CCT) {
        ...
        if (dyna) {
            ...
            if (shape) shape->userData = create.mCapsule.mRenderer;
...
const EditableParams& PhysX3::GetEditableParams() { return gParams; }
...
PhysXUI::PhysXUI(UICallback& callback, udword nb_debug_viz_params, bool* debug_viz_params, const char** debug_viz_names)
    ...
    for (udword i = 0; i < MAX_NB_DEBUG_VIZ_PARAMS; i++) mCheckBox_DebugVis[i] = null;
}

PhysXUI::~PhysXUI() { Common_CloseGUI(mPhysXGUI); }
...
static void GetDebugVizOptionsFromGUI() {
    ...
    Common_GetFromEditBox(gParams.mDebugVizScale, gPhysXUI->mEditBox_DebugVizScale, 0.0f, FLT_MAX);
    for (udword i = 0; i < gNbDebugVizParams; i++) gDebugVizParams[i] = gPhysXUI->mCheckBox_DebugVis[i]->IsChecked();
}

//	TODO: revisit gPEEL_GetOptionsFromGUI: some edit boxes are missing here so is it really useful? => the ones here
//are changed when test is reset, not at runtime
...
void PhysX3::GetOptionsFromGUI(const char* test_name) {
...
#endif
    Common_GetFromEditBox(gParams.mProjectionLinearTolerance, gPhysXUI->mEditBox_ProjectionLinearTolerance, 0.0f,
                          FLT_MAX);
    Common_GetFromEditBox(gParams.mProjectionAngularTolerance, gPhysXUI->mEditBox_ProjectionAngularTolerance, 0.0f,
                          FLT_MAX);
#ifndef IS_PHYSX_3_2
...
#endif
#if PHYSX_SUPPORT_TGS
    if (gPhysXUI->mCheckBox_TGS) gParams.mTGS = gPhysXUI->mCheckBox_TGS->IsChecked();
#endif
#if PHYSX_SUPPORT_GYROSCOPIC_FORCES
    if (gPhysXUI->mCheckBox_Gyro) gParams.mGyro = gPhysXUI->mCheckBox_Gyro->IsChecked();
#endif
#if PHYSX_SUPPORT_STABILIZATION_FLAG
    if (gPhysXUI->mCheckBox_Stabilization) gParams.mStabilization = gPhysXUI->mCheckBox_Stabilization->IsChecked();
#endif
    if (gPhysXUI->mCheckBox_Sleeping) gParams.mEnableSleeping = gPhysXUI->mCheckBox_Sleeping->IsChecked();
    if (gPhysXUI->mCheckBox_CCD) gParams.mEnableCCD = gPhysXUI->mCheckBox_CCD->IsChecked();
#if PHYSX_SUPPORT_ANGULAR_CCD
    if (gPhysXUI->mCheckBox_AngularCCD) gParams.mEnableAngularCCD = gPhysXUI->mCheckBox_AngularCCD->IsChecked();
#endif
#if PHYSX_SUPPORT_RAYCAST_CCD
    ...
    Common_GetFromEditBox(gParams.mFluidVorticityConfinement, gPhysXUI->mEditBox_FluidVorticityConfinement, -FLT_MAX,
                          FLT_MAX);
    ...
    if (test_name) GetPintPlugin()->ApplyTestUIParams(test_name);
    if (gParams.mForceJointLimitDebugViz) {
        ...
        if (0)  // ###DEMO	// Hardcoded for demo script
        {
            if (strcmp(test_name, "ConvexStack") == 0) {
                ...
                gDebugVizParams[0] = true;
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eBODY_MASS_AXES) gDebugVizParams[i] = true;
                }
            } else if (strcmp(test_name, "StableSphericalChain") == 0 || strcmp(test_name, "CylinderStack") == 0 ||
                       strcmp(test_name, "CatenaryBridge") == 0) {
                gParams.mDebugVizScale = 1.0f;
                gDebugVizParams[0] = true;
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eCOLLISION_DYNAMIC) gDebugVizParams[i] = true;
                }
            } else if (strcmp(test_name, "CCDTest_DynamicBoxesVsStaticContainer2") == 0) {
                gParams.mDebugVizScale = 1.0f;
                gDebugVizParams[0] = true;
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eBODY_LIN_VELOCITY) gDebugVizParams[i] = true;
                }
                ...
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eJOINT_LOCAL_FRAMES ||
                        gDebugVizIndex[i] == PxVisualizationParameter::eJOINT_LIMITS)
                        gDebugVizParams[i] = true;
                }
                ...
                gDebugVizParams[0] = true;
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eJOINT_LOCAL_FRAMES) gDebugVizParams[i] = true;
                }
            } else if (strcmp(test_name, "KinematicCharacter") == 0) {
                gParams.mDebugVizScale = 1.0f;
                gDebugVizParams[0] = true;
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eCOLLISION_COMPOUNDS) gDebugVizParams[i] = true;
                }
            } else if (strcmp(test_name, "ContactAndRestOffsets") == 0) {
                gParams.mDebugVizScale = 1.0f;
                gDebugVizParams[0] = true;
                for (udword i = 0; i < gNbDebugVizParams; i++) {
                    if (gDebugVizIndex[i] == PxVisualizationParameter::eCONTACT_NORMAL ||
                        gDebugVizIndex[i] == PxVisualizationParameter::eCONTACT_POINT)
                        gDebugVizParams[i] = true;
                }
            }
        }
...
public:
    ...
    virtual void OnComboBoxEvent(ComboBoxEvent event) {
        if (event == CBE_SELECTION_CHANGED) {
            ...
            if (gPhysXUI->mEditBox_MBPSubdivLevel) gPhysXUI->mEditBox_MBPSubdivLevel->SetEnabled(UsesMBP);

            if (gPhysXUI->mEditBox_MBPRange) gPhysXUI->mEditBox_MBPRange->SetEnabled(UsesMBP);
        }
    }
};
#endif
...
static void gCheckBoxCallback(const IceCheckBox& check_box, bool checked, void* user_data) {
    ...
    switch (id) {
        ...
        case PHYSX_GUI_ENABLE_SQ:
            ...
            if (gPhysXUI->mCheckBox_SQ_InitialOverlap) gPhysXUI->mCheckBox_SQ_InitialOverlap->SetEnabled(checked);
            //			if(gPhysXUI->mCheckBox_SQ_ManualFlushUpdates)
            //				gPhysXUI->mCheckBox_SQ_ManualFlushUpdates->SetEnabled(checked);
            if (gPhysXUI->mCheckBox_SQ_PreciseSweeps) gPhysXUI->mCheckBox_SQ_PreciseSweeps->SetEnabled(checked);
            if (gPhysXUI->mCheckBox_SQ_BothSides) gPhysXUI->mCheckBox_SQ_BothSides->SetEnabled(checked);
            if (gPhysXUI->mEditBox_SQ_RebuildRateHint) gPhysXUI->mEditBox_SQ_RebuildRateHint->SetEnabled(checked);
            if (gPhysXUI->mComboBox_StaticPruner) gPhysXUI->mComboBox_StaticPruner->SetEnabled(checked);
            if (gPhysXUI->mComboBox_DynamicPruner) gPhysXUI->mComboBox_DynamicPruner->SetEnabled(checked);
            break;
        ...
        case PHYSX_GUI_USE_PVD:
            ...
            if (gPhysXUI->mCheckBox_FullPVD) gPhysXUI->mCheckBox_FullPVD->SetEnabled(checked);
            break;
...
//			break;
        ...
        case PHYSX_GUI_USE_SUSPENSION_SWEEPS:
            ...
            if (gPhysXUI->mCheckBox_ForceSphereWheels) gPhysXUI->mCheckBox_ForceSphereWheels->SetEnabled(checked);
            if (gPhysXUI->mCheckBox_UseBlockingSweeps) gPhysXUI->mCheckBox_UseBlockingSweeps->SetEnabled(checked);
            if (gPhysXUI->mCheckBox_DrawSweptWheels) gPhysXUI->mCheckBox_DrawSweptWheels->SetEnabled(checked);
            break;
...
static udword GetNumberOfLogicalThreads() {
    ...
    for (uword i = 0; i < groups; i++) totalProcessors += udword(GetActiveProcessorCount(i));
...
static IceEditBox* CreateEditBox(PintGUIHelper& helper,
                                 IceWindow* parent,
                                 sdword x,
                                 sdword& y,
                                 const char* label,
    ...
    IceEditBox* EB = helper.CreateEditBox(parent, INVALID_ID, x + edit_box_x, y, EditBoxWidth, 20, value,
                                          gPhysXUI->mPhysXGUI, filter, null);
    y += YStep;
...
IceWindow* PhysX3::InitSharedGUI(IceWidget* parent, PintGUIHelper& helper, UICallback& callback) {
    // UICallback should be called when a UI modification appears, to take the change into account immediately (without
    // restarting the scene). This is currently used e.g. for checkboxes controlling the debug viz params. This can also
    // be used for edit boxes with this class:
    struct Callbacks {
    ...
    IceWindow* Tabs[TAB_COUNT];
    {
        ...
        // TAB_MAIN
        {
            ...
            gPhysXUI->mComboBox_NbThreads = CreateComboBox<IceComboBox>(
                    TabWindow, PHYSX_GUI_NB_THREADS, 4 + OffsetX, y, 150, 20, "Num threads", gPhysXUI->mPhysXGUI, null);
            ...
            gPhysXUI->mComboBox_ScratchSize =
                    CreateComboBox<IceComboBox>(TabWindow, PHYSX_GUI_SCRATCH_BUFFER, 4 + OffsetX, y, 150, 20,
                                                "Scratch buffer", gPhysXUI->mPhysXGUI, null);
            ...
            gPhysXUI->mComboBox_BroadPhase =
                    CreateComboBox<BPComboBox>(TabWindow, PHYSX_GUI_BROAD_PHASE, 4 + OffsetX, y, 150, 20, "Broad phase",
                                               gPhysXUI->mPhysXGUI, null);
            ...
            {
                gPhysXUI->mCheckBox_CCD =
                        helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_CCD, 4, y, CheckBoxWidth, 20, "Enable CCD",
                                              gPhysXUI->mPhysXGUI, gParams.mEnableCCD, gCheckBoxCallback);
                y += YStepCB;

#if PHYSX_SUPPORT_ANGULAR_CCD
                gPhysXUI->mCheckBox_AngularCCD = helper.CreateCheckBox(
                        TabWindow, PHYSX_GUI_ENABLE_ANGULAR_CCD, 4, y, CheckBoxWidth, 20, "Enable angular CCD",
                        gPhysXUI->mPhysXGUI, gParams.mEnableAngularCCD, gCheckBoxCallback);
                y += YStepCB;
#endif

#if PHYSX_SUPPORT_RAYCAST_CCD
                gPhysXUI->mCheckBox_RaycastCCDStatic = helper.CreateCheckBox(
                        TabWindow, PHYSX_GUI_ENABLE_RAYCAST_CCD, 4, y, CheckBoxWidth, 20, "Enable raycast CCD",
                        gPhysXUI->mPhysXGUI, gParams.mEnableRaycastCCDStatic, gCheckBoxCallback);
                y += YStepCB;

                gPhysXUI->mCheckBox_RaycastCCDDynamic =
                        helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_RAYCAST_CCD_DYNA_DYNA, 4, y, CheckBoxWidth,
                                              20, "Enable raycast CCD for dynamics", gPhysXUI->mPhysXGUI,
                                              gParams.mEnableRaycastCCDDynamic, gCheckBoxCallback);
                y += YStepCB;
#endif
            }
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_SHARE_MESH_DATA, 4, y, CheckBoxWidth, 20, "Share mesh data",
                                  gPhysXUI->mPhysXGUI, gParams.mShareMeshData, gCheckBoxCallback);
            y += YStepCB;

            helper.CreateCheckBox(TabWindow, PHYSX_GUI_SHARE_SHAPES, 4, y, CheckBoxWidth, 20, "Share shapes",
                                  gPhysXUI->mPhysXGUI, gParams.mShareShapes, gCheckBoxCallback);
            y += YStepCB;

#if PHYSX_SUPPORT_TIGHT_CONVEX_BOUNDS
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_TIGHT_CONVEX_BOUNDS, 4, y, CheckBoxWidth, 20,
                                  "Tight convex bounds", gPhysXUI->mPhysXGUI, gParams.mUseTightConvexBounds,
                                  gCheckBoxCallback);
            y += YStepCB;
#endif
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_PCM, 4, y, CheckBoxWidth, 20, "Enable PCM", gPhysXUI->mPhysXGUI,
                                  gParams.mPCM, gCheckBoxCallback);
            y += YStepCB;

#ifdef PHYSX_SUPPORT_SSE_FLAG
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_SSE, 4, y, CheckBoxWidth, 20, "Enable SSE",
                                  gPhysXUI->mPhysXGUI, gParams.mEnableSSE, gCheckBoxCallback);
            y += YStepCB;
#endif
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_ACTIVE_TRANSFORMS, 4, y, CheckBoxWidth, 20,
                                  "Enable active transforms", gPhysXUI->mPhysXGUI, gParams.mEnableActiveTransforms,
                                  gCheckBoxCallback);
            y += YStepCB;

            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_CONTACT_CACHE, 4, y, CheckBoxWidth, 20,
                                  "Enable contact cache", gPhysXUI->mPhysXGUI, gParams.mEnableContactCache,
                                  gCheckBoxCallback);
            y += YStepCB;

#if PHYSX_SUPPORT_CONTACT_NOTIFICATIONS
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_CONTACT_NOTIF, 4, y, CheckBoxWidth, 20,
                                  "Enable contact notifications", gPhysXUI->mPhysXGUI, gParams.mEnableContactNotif,
                                  gCheckBoxCallback);
            y += YStepCB;
#endif

            helper.CreateCheckBox(TabWindow, PHYSX_GUI_FLUSH_SIMULATION, 4, y, CheckBoxWidth, 20,
                                  "Flush simulation buffers", gPhysXUI->mPhysXGUI, gParams.mFlushSimulation,
                                  gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_PVD =
                    helper.CreateCheckBox(TabWindow, PHYSX_GUI_USE_PVD, 4, y, CheckBoxWidth, 20, "Use PVD",
                                          gPhysXUI->mPhysXGUI, gParams.mUsePVD, gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_FullPVD = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_USE_FULL_PVD_CONNECTION, 4, y, CheckBoxWidth, 20, "Full connection",
                    gPhysXUI->mPhysXGUI, gParams.mUseFullPvdConnection, gCheckBoxCallback);
#ifdef PINT_SUPPORT_PVD  // Defined in project's properties
//			gPhysXUI->mCheckBox_FullPVD->SetEnabled(gUsePVD);
...
#ifdef PHYSX_SUPPORT_GPU
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_USE_GPU, 4, y, CheckBoxWidth, 20, "Use GPU", gPhysXUI->mPhysXGUI,
                                  gParams.mUseGPU, gCheckBoxCallback);
            y += YStepCB;
#endif
            y += YStepCB;
            ...
            //		helper.CreateLabel(TabWindow, x2, y+LabelOffsetY, LabelWidth, 20, "World bounds size:",
            //gPhysXUI->mPhysXGUI); 		gPhysXUI->mEditBox_GlobalBoxSize = helper.CreateEditBox(TabWindow,
            //PHYSX_GUI_GLOBAL_BOX_SIZE, 4+EditBoxX, y, EditBoxWidth, 20, helper.Convert(gGlobalBoxSize),
            //gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null); 		y += YStep;

            gPhysXUI->mEditBox_ContactOffset =
                    CreateEditBox(helper, TabWindow, x2, y, "Contact offset:", helper.Convert(gParams.mContactOffset),
                                  EDITBOX_FLOAT, LabelWidth, EditBoxX);
            gPhysXUI->mEditBox_RestOffset =
                    CreateEditBox(helper, TabWindow, x2, y, "Rest offset:", helper.Convert(gParams.mRestOffset),
                                  EDITBOX_FLOAT, LabelWidth, EditBoxX);
#if PHYSX_SUPPORT_CONTACT_NOTIFICATIONS
            gPhysXUI->mEditBox_ContactNotifThreshold =
                    CreateEditBox(helper, TabWindow, x2, y,
                                  "Contact notif threshold:", helper.Convert(gParams.mContactNotifThreshold),
                                  EDITBOX_FLOAT, LabelWidth, EditBoxX);
#endif
#if PHYSX_SUPPORT_SUBSTEPS
            gPhysXUI->mEditBox_NbSubsteps =
                    CreateEditBox(helper, TabWindow, x2, y, "Nb substeps:", _F("%d", gParams.mNbSubsteps),
                                  EDITBOX_INTEGER_POSITIVE, LabelWidth, EditBoxX);
#endif
#if PHYSX_SUPPORT_PX_BROADPHASE_TYPE
            gPhysXUI->mEditBox_MBPSubdivLevel =
                    CreateEditBox(helper, TabWindow, x2, y, "MBP subdiv level:", _F("%d", gParams.mMBPSubdivLevel),
                                  EDITBOX_INTEGER_POSITIVE, LabelWidth, EditBoxX);
            gPhysXUI->mEditBox_MBPRange =
                    CreateEditBox(helper, TabWindow, x2, y, "MBP range:", _F("%f", gParams.mMBPRange),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
...
//											gPhysXUI->mCheckBox_TGS->SetChecked(gParams.mTGS);
            ...
            {
                gPhysXUI->mComboBox_Config =
                        CreateComboBox<ConfigComboBox>(TabWindow, PHYSX_GUI_CONFIG_COMBO_BOX, x3, y, 250, 20,
                                                       "Configuration", gPhysXUI->mPhysXGUI, null);
            ...
            gPhysXUI->mEditBox_ConfigDesc = helper.CreateEditBox(TabWindow, PHYSX_GUI_CONFIG_DESC, x3, y, 250, 60, "",
                                                                 gPhysXUI->mPhysXGUI, EDITBOX_TEXT, null);
            gPhysXUI->mEditBox_ConfigDesc->SetMultilineText(ConfigDesc0);
        ...
        // TAB_DYNAMICS
        {
...

            {
                ...
                helper.CreateCheckBox(TabWindow, PHYSX_GUI_DISABLE_STRONG_FRICTION, xf2, y, CheckBoxWidth, 20,
                                      "Disable strong friction", gPhysXUI->mPhysXGUI, gParams.mDisableStrongFriction,
                                      gCheckBoxCallback);
                y += YStepCB;

                helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_ONE_DIR_FRICTION, xf2, y, CheckBoxWidth, 20,
                                      "Enable one dir. friction", gPhysXUI->mPhysXGUI, gParams.mEnableOneDirFriction,
                                      gCheckBoxCallback);
                y += YStepCB;

                helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_TWO_DIR_FRICTION, xf2, y, CheckBoxWidth, 20,
                                      "Enable two dir. friction", gPhysXUI->mPhysXGUI, gParams.mEnableTwoDirFriction,
                                      gCheckBoxCallback);
                y += YStepCB;

#if PHYSX_SUPPORT_IMPROVED_PATCH_FRICTION
                gPhysXUI->mCheckBox_ImprovedPatchFriction =
                        helper.CreateCheckBox(TabWindow, PHYSX_GUI_IMPROVED_PATCH_FRICTION, xf2, y, CheckBoxWidth, 20,
                                              "Improved patch friction", gPhysXUI->mPhysXGUI,
                                              gParams.mImprovedPatchFriction, gCheckBoxCallback);
                y += YStepCB;
#endif
#if PHYSX_SUPPORT_FRICTION_EVERY_ITERATION
                gPhysXUI->mCheckBox_FrictionEveryIteration =
                        helper.CreateCheckBox(TabWindow, PHYSX_GUI_FRICTION_EVERY_ITERATION, xf2, y, CheckBoxWidth, 20,
                                              "Friction every iteration", gPhysXUI->mPhysXGUI,
                                              gParams.mFrictionEveryIteration, gCheckBoxCallback);
                y += YStepCB;
#endif
                y += YStepCB;

                gPhysXUI->mEditBox_DefaultStaticFriction =
                        CreateEditBox(helper, TabWindow, xf2, y,
                                      "Default static friction:", helper.Convert(gParams.mDefaultStaticFriction),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidth2, EditBoxX2);
                gPhysXUI->mEditBox_DefaultDynamicFriction =
                        CreateEditBox(helper, TabWindow, xf2, y,
                                      "Default dynamic friction:", helper.Convert(gParams.mDefaultDynamicFriction),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidth2, EditBoxX2);
                gPhysXUI->mEditBox_FrictionOffsetThreshold =
                        CreateEditBox(helper, TabWindow, xf2, y,
                                      "Friction offset threshold:", helper.Convert(gParams.mFrictionOffsetThreshold),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidth2, EditBoxX2);

#if PHYSX_SUPPORT_TORSION_FRICTION
                gPhysXUI->mEditBox_TorsionalPatchRadius =
                        CreateEditBox(helper, TabWindow, xf2, y,
                                      "Torsional patch radius:", helper.Convert(gParams.mTorsionalPatchRadius),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidth2, EditBoxX2);
                gPhysXUI->mEditBox_MinTorsionalPatchRadius =
                        CreateEditBox(helper, TabWindow, xf2, y,
                                      "Min torsional patch radius:", helper.Convert(gParams.mMinTorsionalPatchRadius),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidth2, EditBoxX2);
#endif
                {
            ...
            gPhysXUI->mCheckBox_Sleeping = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_ENABLE_SLEEPING, 4, y, CheckBoxWidth, 20, "Enable sleeping",
                    gPhysXUI->mPhysXGUI, gParams.mEnableSleeping, gCheckBoxCallback);
            y += YStepCB;

#if PHYSX_SUPPORT_ADAPTIVE_FORCE
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ADAPTIVE_FORCE, 4, y, CheckBoxWidth, 20, "Adaptive force",
                                  gPhysXUI->mPhysXGUI, gParams.mAdaptiveForce, gCheckBoxCallback);
            y += YStepCB;
#endif

#if PHYSX_SUPPORT_STABILIZATION_FLAG
            gPhysXUI->mCheckBox_Stabilization =
                    helper.CreateCheckBox(TabWindow, PHYSX_GUI_STABILIZATION, 4, y, CheckBoxWidth, 20, "Stabilization",
                                          gPhysXUI->mPhysXGUI, gParams.mStabilization, gCheckBoxCallback);
            y += YStepCB;
#endif
#if PHYSX_SUPPORT_TGS
            gPhysXUI->mCheckBox_TGS =
                    helper.CreateCheckBox(TabWindow, PHYSX_GUI_TGS, 4, y, CheckBoxWidth, 20, "Enable TGS",
                                          gPhysXUI->mPhysXGUI, gParams.mTGS, gCheckBoxCallback);
            y += YStepCB;
#endif
#if PHYSX_SUPPORT_GYROSCOPIC_FORCES
            gPhysXUI->mCheckBox_Gyro = helper.CreateCheckBox(TabWindow, PHYSX_GUI_GYRO, 4, y, CheckBoxWidth, 20,
                                                             "Enable gyroscopic forces", gPhysXUI->mPhysXGUI,
                                                             gParams.mGyro, gCheckBoxCallback);
            y += YStepCB;
#endif
            y += YStep;

#ifndef IS_PHYSX_3_2
            gPhysXUI->mEditBox_MaxNbCCDPasses =
                    CreateEditBox(helper, TabWindow, 4, y, "Max nb CCD passes:", _F("%d", gParams.mMaxNbCCDPasses),
                                  EDITBOX_INTEGER_POSITIVE, LabelWidth, EditBoxX);
#endif
            gPhysXUI->mEditBox_SolverIterPos = CreateEditBox(
                    helper, TabWindow, 4, y, "Solver iter pos:", _F("%d", gParams.mSolverIterationCountPos),
                    EDITBOX_INTEGER_POSITIVE, LabelWidth, EditBoxX);
            gPhysXUI->mEditBox_SolverIterVel = CreateEditBox(
                    helper, TabWindow, 4, y, "Solver iter vel:", _F("%d", gParams.mSolverIterationCountVel),
                    EDITBOX_INTEGER_POSITIVE, LabelWidth, EditBoxX);
            gPhysXUI->mEditBox_LinearDamping =
                    CreateEditBox(helper, TabWindow, 4, y, "Linear damping:", helper.Convert(gParams.mLinearDamping),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
            gPhysXUI->mEditBox_AngularDamping =
                    CreateEditBox(helper, TabWindow, 4, y, "Angular damping:", helper.Convert(gParams.mAngularDamping),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
            gPhysXUI->mEditBox_MaxAngularVelocity = CreateEditBox(
                    helper, TabWindow, 4, y, "Max angular velocity:", helper.Convert(gParams.mMaxAngularVelocity),
                    EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
#if PHYSX_SUPPORT_MAX_DEPEN_VELOCITY
            gPhysXUI->mEditBox_MaxDepenVelocity = CreateEditBox(
                    helper, TabWindow, 4, y, "Max depen. velocity:", helper.Convert(gParams.mMaxDepenVelocity),
                    EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
#endif
            gPhysXUI->mEditBox_SleepThreshold =
                    CreateEditBox(helper, TabWindow, 4, y, "Sleep threshold:", helper.Convert(gParams.mSleepThreshold),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
#if PHYSX_SUPPORT_STABILIZATION_FLAG
            gPhysXUI->mEditBox_StabilizationThreshold =
                    CreateEditBox(helper, TabWindow, 4, y,
                                  "Stabilization threshold:", helper.Convert(gParams.mStabilizationThreshold),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidth, EditBoxX);
#endif
            gPhysXUI->mEditBox_MaxBiasCoeff =
                    CreateEditBox(helper, TabWindow, 4, y, "Max bias coeff:", helper.Convert(gParams.mMaxBiasCoeff),
                                  EDITBOX_FLOAT, LabelWidth, EditBoxX);
        }
        ...
        // TAB_JOINTS
        {
            ...
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_USE_D6_JOINT, xj, y, CheckBoxWidth, 20,
                                  "Use D6 joint if possible", gPhysXUI->mPhysXGUI, gParams.mUseD6Joint,
                                  gCheckBoxCallback);
            y += YStepCB;

#if PHYSX_SUPPORT_DISABLE_PREPROCESSING
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_DISABLE_PREPROCESSING, xj, y, CheckBoxWidth, 20,
                                  "Disable preprocessing", gPhysXUI->mPhysXGUI, gParams.mDisablePreprocessing,
                                  gCheckBoxCallback);
            y += YStepCB;
#endif
#ifndef IS_PHYSX_3_2
#if PHYSX_REMOVE_JOINT_32_COMPATIBILITY
#else
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_JOINT_32_COMPATIBILITY, xj, y, CheckBoxWidth, 20,
                                  "Enable 3.2 compatibility", gPhysXUI->mPhysXGUI, gParams.mEnableJoint32Compatibility,
                                  gCheckBoxCallback);
            y += YStepCB;
#endif
#endif
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_JOINT_PROJECTION, xj, y, CheckBoxWidth, 20,
                                  "Enable joint projection", gPhysXUI->mPhysXGUI, gParams.mEnableJointProjection,
                                  gCheckBoxCallback);
            //			y += YStepCB;

            //			helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_JOINT_CONTACT_DISTANCE, xj, y, CheckBoxWidth, 20,
            //"Enable contact distance (for limits)", gPhysXUI->mPhysXGUI, gParams.mEnableJointContactDistance,
            //gCheckBoxCallback); 			y += YStepCB;

            y += YStep;

            const sdword LabelWidthJ = 150;
            gPhysXUI->mEditBox_ProjectionLinearTolerance =
                    CreateEditBox(helper, TabWindow, xj, y,
                                  "Projection linear tolerance:", helper.Convert(gParams.mProjectionLinearTolerance),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
            gPhysXUI->mEditBox_ProjectionLinearTolerance->SetEnabled(gParams.mEnableJointProjection);

            gPhysXUI->mEditBox_ProjectionAngularTolerance =
                    CreateEditBox(helper, TabWindow, xj, y,
                                  "Projection angular tolerance:", helper.Convert(gParams.mProjectionAngularTolerance),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
            gPhysXUI->mEditBox_ProjectionAngularTolerance->SetEnabled(gParams.mEnableJointProjection);

#ifndef IS_PHYSX_3_2
            gPhysXUI->mEditBox_InverseInertiaScale = CreateEditBox(
                    helper, TabWindow, xj, y, "Inverse inertia scale:", helper.Convert(gParams.mInverseInertiaScale),
                    EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
            gPhysXUI->mEditBox_InverseMassScale = CreateEditBox(
                    helper, TabWindow, xj, y, "Inverse mass scale:", helper.Convert(gParams.mInverseMassScale),
                    EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
#endif

#ifdef PHYSX_SUPPORT_LINEAR_COEFF
            gPhysXUI->mEditBox_LinearCoeff =
                    CreateEditBox(helper, TabWindow, xj, y, "Linear coefficient:", helper.Convert(gParams.mLinearCoeff),
                                  EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
#endif

            {
                helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, 90, 20, "Contact distance:", gPhysXUI->mPhysXGUI);
                gPhysXUI->mComboBox_LimitsContactDistance =
                        CreateComboBox<IceComboBox>(TabWindow, PHYSX_GUI_LIMITS_CONTACT_DISTANCE, 4 + OffsetX, y, 150,
                                                    20, "Limits contact distance", gPhysXUI->mPhysXGUI, null);
...
#if PHYSX_SUPPORT_ARTICULATIONS || PHYSX_SUPPORT_RCA
                helper.CreateCheckBox(TabWindow, PHYSX_GUI_DISABLE_ARTICULATIONS, xa, y, CheckBoxWidth, 20,
                                      "Disable articulations", gPhysXUI->mPhysXGUI, gParams.mDisableArticulations,
                                      gCheckBoxCallback);
                y += YStepCB;
                y += YStepCB;
#endif
#if PHYSX_SUPPORT_ARTICULATIONS
                gPhysXUI->mEditBox_MaxProjectionIterations =
                        CreateEditBox(helper, TabWindow, xa, y,
                                      "Max Projection Iterations:", _F("%d", gParams.mMaxProjectionIterations),
                                      EDITBOX_INTEGER_POSITIVE, LabelWidthJ, LabelWidthJ);
                gPhysXUI->mEditBox_SeparationTolerance = CreateEditBox(
                        helper, TabWindow, xa, y, "Separation tolerance:", helper.Convert(gParams.mSeparationTolerance),
                        EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
                gPhysXUI->mEditBox_InternalDriveIterations =
                        CreateEditBox(helper, TabWindow, xa, y,
                                      "Internal drive iterations:", _F("%d", gParams.mInternalDriveIterations),
                                      EDITBOX_INTEGER_POSITIVE, LabelWidthJ, LabelWidthJ);
                gPhysXUI->mEditBox_ExternalDriveIterations =
                        CreateEditBox(helper, TabWindow, xa, y,
                                      "External drive iterations:", _F("%d", gParams.mExternalDriveIterations),
                                      EDITBOX_INTEGER_POSITIVE, LabelWidthJ, LabelWidthJ);
#endif

#if PHYSX_SUPPORT_RCA_CFM_SCALE
                gPhysXUI->mEditBox_RCACfmScale =
                        CreateEditBox(helper, TabWindow, xa, y, "RCA CFM scale:", helper.Convert(gParams.mRCACfmScale),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
#endif
#if PHYSX_SUPPORT_RCA_DOF_SCALE
                gPhysXUI->mEditBox_RCADofScale =
                        CreateEditBox(helper, TabWindow, xa, y, "RCA DOF scale:", helper.Convert(gParams.mRCADofScale),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
#endif
#if PHYSX_SUPPORT_RCA_ARMATURE
                gPhysXUI->mEditBox_RCAArmature =
                        CreateEditBox(helper, TabWindow, xa, y, "RCA armature:", helper.Convert(gParams.mRCAArmature),
                                      EDITBOX_FLOAT_POSITIVE, LabelWidthJ, LabelWidthJ);
#endif
                {
                    // Cannot use helper function here because "mType" is missing from wrapper.
        ...
        // TAB_SCENE_QUERIES
        {
            ...
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_TEST, 4, y, CheckBoxWidth, 20, "Test", gPhysXUI->mPhysXGUI,
                                  gTest, gCheckBoxCallback);
            y += YStepCB;

            helper.CreateCheckBox(TabWindow, PHYSX_GUI_ENABLE_SQ, 4, y, CheckBoxWidth, 20, "Enable scene queries",
                                  gPhysXUI->mPhysXGUI, gParams.mSQFlag, gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_SQ_FilterOutAllShapes = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_SQ_FILTER_OUT, 4, y, CheckBoxWidth, 20, "Filter out all shapes (DEBUG)",
                    gPhysXUI->mPhysXGUI, gParams.mSQFilterOutAllShapes, gCheckBoxCallback);
            gPhysXUI->mCheckBox_SQ_FilterOutAllShapes->SetEnabled(gParams.mSQFlag);
            y += YStepCB;

            gPhysXUI->mCheckBox_SQ_InitialOverlap = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_SQ_INITIAL_OVERLAP, 4, y, CheckBoxWidth, 20, "eINITIAL_OVERLAP flag (sweeps)",
                    gPhysXUI->mPhysXGUI, gParams.mSQInitialOverlap, gCheckBoxCallback);
            gPhysXUI->mCheckBox_SQ_InitialOverlap->SetEnabled(gParams.mSQFlag);
            y += YStepCB;

            /*		gPhysXUI->mCheckBox_SQ_ManualFlushUpdates = helper.CreateCheckBox(TabWindow,
               PHYSX_GUI_SQ_MANUAL_FLUSH_UPDATES, 4, y, CheckBoxWidth, 20, "Manual flush updates", gPhysXUI->mPhysXGUI,
               gParams.mSQManualFlushUpdates, gCheckBoxCallback);
                            gPhysXUI->mCheckBox_SQ_ManualFlushUpdates->SetEnabled(gParams.mSQFlag);
                            y += YStepCB;*/

            gPhysXUI->mCheckBox_SQ_PreciseSweeps = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_SQ_PRECISE_SWEEPS, 4, y, CheckBoxWidth, 20, "Precise sweeps",
                    gPhysXUI->mPhysXGUI, gParams.mSQPreciseSweeps, gCheckBoxCallback);
            gPhysXUI->mCheckBox_SQ_PreciseSweeps->SetEnabled(gParams.mSQFlag);
            y += YStepCB;

            gPhysXUI->mCheckBox_SQ_BothSides =
                    helper.CreateCheckBox(TabWindow, PHYSX_GUI_SQ_BOTH_SIDES, 4, y, CheckBoxWidth, 20, "Both sides",
                                          gPhysXUI->mPhysXGUI, gParams.mSQBothSides, gCheckBoxCallback);
            ...
            gPhysXUI->mEditBox_SQ_RebuildRateHint = CreateEditBox(
                    helper, TabWindow, xj, y, "Tree rebuild rate hint:", _F("%d", gParams.mSQDynamicRebuildRateHint),
                    EDITBOX_INTEGER_POSITIVE, LabelWidthJ, LabelWidthJ);
            ...
            gPhysXUI->mComboBox_StaticPruner =
                    CreateComboBox<IceComboBox>(TabWindow, PHYSX_GUI_STATIC_PRUNER, 4 + OffsetX, y, 150, 20,
                                                "Static pruner", gPhysXUI->mPhysXGUI, null);
            ...
            gPhysXUI->mComboBox_DynamicPruner =
                    CreateComboBox<IceComboBox>(TabWindow, PHYSX_GUI_DYNAMIC_PRUNER, 4 + OffsetX, y, 150, 20,
                                                "Dynamic pruner", gPhysXUI->mPhysXGUI, null);
        ...
        // TAB_COOKING
        {
...
#if PHYSX_SUPPORT_PX_MESH_COOKING_HINT
            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LocalLabelWidth, 20,
                               "Mesh cooking hint:", gPhysXUI->mPhysXGUI);
            gPhysXUI->mComboBox_MeshCookingHint =
                    CreateComboBox<IceComboBox>(TabWindow, PHYSX_GUI_MESH_COOKING_HINT, 4 + LocalOffsetX, y, 150, 20,
                                                "Mesh cooking hint", gPhysXUI->mPhysXGUI, null);
...
#endif
#if PHYSX_SUPPORT_PX_MESH_MIDPHASE2
                        if (gPhysXUI->mEditBox_NbTrisPerLeaf) gPhysXUI->mEditBox_NbTrisPerLeaf->SetEnabled(UsesBVH34);
#if PHYSX_SUPPORT_PX_MESH_COOKING_HINT
                        if (gPhysXUI->mComboBox_MeshCookingHint)
            ...
            gPhysXUI->mComboBox_MidPhase =
                    CreateComboBox<MPComboBox>(TabWindow, PHYSX_GUI_MID_PHASE, 4 + LocalOffsetX, y, 150, 20, "Midphase",
                                               gPhysXUI->mPhysXGUI, null);
...
#if PHYSX_SUPPORT_PX_MESH_BUILD_STRATEGY
            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LocalLabelWidth, 20,
                               "Mesh build strategy:", gPhysXUI->mPhysXGUI);
            gPhysXUI->mComboBox_MeshBuildStrategy =
                    CreateComboBox<IceComboBox>(TabWindow, PHYSX_GUI_MESH_BUILD_STRATEGY, 4 + LocalOffsetX, y, 150, 20,
                                                "Mesh build strategy", gPhysXUI->mPhysXGUI, null);
...
#endif
#if PHYSX_SUPPORT_QUANTIZED_TREE_OPTION
            gPhysXUI->mCheckBox_QuantizedTrees = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_QUANTIZED_TREES, 4, y, CheckBoxWidth, 20, "Quantized trees",
                    gPhysXUI->mPhysXGUI, gParams.mQuantizedTrees, gCheckBoxCallback);
            y += YStepCB;
#endif
#if PHYSX_SUPPORT_PX_MESH_MIDPHASE2
            gPhysXUI->mEditBox_NbTrisPerLeaf =
                    CreateEditBox(helper, TabWindow, 4, y, "Nb tris per leaf:", _F("%d", gParams.mNbTrisPerLeaf),
                                  EDITBOX_INTEGER_POSITIVE, 90, EditBoxX);
#endif
            y += YStepCB;
#if PHYSX_SUPPORT_USER_DEFINED_GAUSSMAP_LIMIT
            gPhysXUI->mEditBox_GaussMapLimit =
                    CreateEditBox(helper, TabWindow, 4, y, "GaussMap limit:", _F("%d", gParams.mGaussMapLimit),
                                  EDITBOX_INTEGER_POSITIVE, 90, EditBoxX);
#endif
#if PHYSX_SUPPORT_DISABLE_ACTIVE_EDGES_PRECOMPUTE
            helper.CreateCheckBox(TabWindow, PHYSX_GUI_PRECOMPUTE_ACTIVE_EDGES, 4, y, CheckBoxWidth, 20,
                                  "Precompute active edges", gPhysXUI->mPhysXGUI, gParams.mPrecomputeActiveEdges,
                                  gCheckBoxCallback);
            y += YStepCB;
#endif

#if PHYSX_SUPPORT_PX_MESH_MIDPHASE
            if (gPhysXUI->mComboBox_MidPhase) gPhysXUI->mComboBox_MidPhase->OnComboBoxEvent(CBE_SELECTION_CHANGED);
#endif
        }
...
#if PHYSX_SUPPORT_VEHICLE_SUSPENSION_SWEEPS
            gPhysXUI->mCheckBox_UseSuspensionSweeps =
                    helper.CreateCheckBox(TabWindow, PHYSX_GUI_USE_SUSPENSION_SWEEPS, 4, y, CheckBoxWidth, 20,
                                          "Use sweeps (instead of raycasts)", gPhysXUI->mPhysXGUI,
                                          gParams.mUseSuspensionSweeps, gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_ForceSphereWheels = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_FORCE_SPHERE_WHEELS, 4, y, CheckBoxWidth, 20, "Force sphere wheels",
                    gPhysXUI->mPhysXGUI, gParams.mForceSphereWheels, gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_UseBlockingSweeps = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_USE_BLOCKING_SWEEPS, 4, y, CheckBoxWidth, 20, "Use blocking sweeps",
                    gPhysXUI->mPhysXGUI, gParams.mUseBlockingSweeps, gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_DrawSweptWheels = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_DRAW_SWEPT_WHEELS, 4, y, CheckBoxWidth, 20, "Debug draw swept wheels",
                    gPhysXUI->mPhysXGUI, gParams.mDrawSweptWheels, gCheckBoxCallback);
            y += YStepCB;
#endif
            gPhysXUI->mCheckBox_DrawSuspensionCasts = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_DRAW_SUSPENSION_CASTS, 4, y, CheckBoxWidth, 20, "Debug draw hit data",
                    gPhysXUI->mPhysXGUI, gParams.mDrawSuspensionCasts, gCheckBoxCallback);
            y += YStepCB;

            gPhysXUI->mCheckBox_DrawPhysXTelemetry = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_DRAW_PHYSX_TELEMETRY, 4, y, CheckBoxWidth, 20, "Debug PhysX telemetry",
                    gPhysXUI->mPhysXGUI, gParams.mDrawPhysXTelemetry, gCheckBoxCallback);
            y += YStepCB;

#ifdef PHYSX_SUPPORT_VEHICLE_FIX
            gPhysXUI->mCheckBox_UseFix =
                    helper.CreateCheckBox(TabWindow, PHYSX_GUI_USE_FIX, 4, y, CheckBoxWidth, 20, "Use experimental fix",
                                          gPhysXUI->mPhysXGUI, gParams.mUseFix, gCheckBoxCallback);
            y += YStepCB;
#endif
#if PHYSX_SUPPORT_STEER_FILTER
            y += YStep;
            gPhysXUI->mEditBox_SteerFilter =
                    CreateEditBox(helper, TabWindow, 4, y, "Steer filter:", helper.Convert(gParams.mSteerFilter),
                                  EDITBOX_FLOAT_POSITIVE, 90, EditBoxX);
#endif
#if PHYSX_SUPPORT_VEHICLE_SWEEP_INFLATION
            y += YStep;
            gPhysXUI->mEditBox_SweepInflation =
                    CreateEditBox(helper, TabWindow, 4, y, "Sweep inflation:", helper.Convert(gParams.mSweepInflation),
                                  EDITBOX_FLOAT_POSITIVE, 90, EditBoxX);
#endif
        ...
        // TAB_FLUIDS
        {
            ...
            gPhysXUI->mEditBox_FluidFriction =
                    CreateEditBox(helper, TabWindow, 4, y, "Friction:", helper.Convert(gParams.mFluidFriction),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidDamping =
                    CreateEditBox(helper, TabWindow, 4, y, "Damping:", helper.Convert(gParams.mFluidDamping),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidAdhesion =
                    CreateEditBox(helper, TabWindow, 4, y, "Adhesion:", helper.Convert(gParams.mFluidAdhesion),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidViscosity =
                    CreateEditBox(helper, TabWindow, 4, y, "Viscosity:", helper.Convert(gParams.mFluidViscosity),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidVorticityConfinement = CreateEditBox(
                    helper, TabWindow, 4, y, "Vorticity:", helper.Convert(gParams.mFluidVorticityConfinement),
                    EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidSurfaceTension = CreateEditBox(
                    helper, TabWindow, 4, y, "Surface tension:", helper.Convert(gParams.mFluidSurfaceTension),
                    EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidCohesion =
                    CreateEditBox(helper, TabWindow, 4, y, "Cohesion:", helper.Convert(gParams.mFluidCohesion),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidLift = CreateEditBox(
                    helper, TabWindow, 4, y, "Lift:", helper.Convert(gParams.mFluidLift), EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidDrag = CreateEditBox(
                    helper, TabWindow, 4, y, "Drag:", helper.Convert(gParams.mFluidDrag), EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidCflCoefficient =
                    CreateEditBox(helper, TabWindow, 4, y, "Cfl coeff:", helper.Convert(gParams.mFluidCflCoefficient),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidGravityScale =
                    CreateEditBox(helper, TabWindow, 4, y, "Gravity scale:", helper.Convert(gParams.mFluidGravityScale),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mEditBox_FluidParticleMass =
                    CreateEditBox(helper, TabWindow, 4, y, "Particle mass:", helper.Convert(gParams.mFluidParticleMass),
                                  EDITBOX_FLOAT, 90, EditBoxX);
            gPhysXUI->mCheckBox_FluidParticleCCD = helper.CreateCheckBox(
                    TabWindow, PHYSX_GUI_FLUID_PARTICLE_CCD, 4, y, CheckBoxWidth, 20, "Particle CCD",
                    gPhysXUI->mPhysXGUI, gParams.mFluidParticleCCD, gCheckBoxCallback);
            y += YStepCB;
        }
#endif
        ...
        // TAB_DEBUG_VIZ
        {
            ...
            gPhysXUI->mEditBox_DebugVizScale =
                    helper.CreateEditBox(TabWindow, PHYSX_GUI_DEBUG_VIZ_SCALE, 4 + EditBoxX, y, EditBoxWidth, 20,
                                         _F("%f", gParams.mDebugVizScale), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE,
                                         Callbacks::EditBoxChange);
            gPhysXUI->mEditBox_DebugVizScale->SetUserData(&callback);

            y += YStep;

            sdword LastY = Common_CreateDebugVizUI(TabWindow, 10, y, gCheckBoxCallback, PHYSX_GUI_ENABLE_DEBUG_VIZ,
                                                   NB_DEBUG_VIZ_PARAMS, gDebugVizParams, gDebugVizNames,
                                                   gPhysXUI->mCheckBox_DebugVis, gPhysXUI->mPhysXGUI);
        }
    }
    //	gPhysXUI->mCheckBox_DrawMBPRegions = helper.CreateCheckBox(Main, PHYSX_GUI_DRAW_MBP_REGIONS, 290+10, LastY,
    //CheckBoxWidth, 20, "Draw MBP regions", gPhysXUI->mPhysXGUI, gVisualizeMBPRegions, gCheckBoxCallback);
...
void PhysX3::CloseSharedGUI() { DELETESINGLE(gPhysXUI); }
