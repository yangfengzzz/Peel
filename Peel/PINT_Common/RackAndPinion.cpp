...
PxRackAndPinionJoint* physx::PxRackAndPinionJointCreate(PxPhysics& physics,
    ...
    if (j->attachConstraint(physics, actor0, actor1)) return j;
...
bool RackAndPinionJoint::setJoints(PxJoint* hinge, PxJoint* prismatic) {
    ...
    if (type0 != PxJointConcreteType::eREVOLUTE && type0 != PxJointConcreteType::eD6) {
        Ps::getFoundation().error(
                ...
                "PxRackAndPinionJoint::setJoints: passed hinge joint must be either a revolute joint or a D6 joint.");
        return false;
    }
    ...
    if (type1 != PxJointConcreteType::ePRISMATIC && type1 != PxJointConcreteType::eD6) {
        Ps::getFoundation().error(PxErrorCode::eINVALID_PARAMETER, __FILE__, __LINE__,
                                  "PxRackAndPinionJoint::setJoints: passed prismatic joint must be either a prismatic "
                                  "joint or a D6 joint.");
        return false;
    }
...
void RackAndPinionJoint::updateError() {
    ...
    if (!hingeJoint || !prismaticJoint) return;
...
void RackAndPinionJoint::importExtraData(PxDeserializationContext& context) {
    if (mData) mData = context.readExtraData<RackAndPinionJointData, PX_SERIAL_ALIGN>();
...
PxConstraintShaderTable Ext::RackAndPinionJoint::sShaders = {RackAndPinionJointSolverPrep, RackAndPinionJointProject,
                                                             RackAndPinionJointVisualize, PxConstraintFlag::Enum(0)};
