...
static inline_ PintActorHandle CreateHandle(PxActor* actor) { return CreateHandle(static_cast<PxRigidActor*>(actor)); }

static void FillContactData(PintContactData& dst,
                            const PxContactPairPoint& src,
                            const PintActorHandle handle0,
                            const PintActorHandle handle1) {
...
void PEEL_SimulationEventCallback::Release() { mContactData.Empty(); }
...
void PEEL_SimulationEventCallback::onContact(const PxContactPairHeader& pairHeader,
                                             const PxContactPair* pairs,
                                             PxU32 nbPairs) {
    // printf("CHECKPOINT\n");

    if (!mContactCallback) return;
        ...
        //		if (!(cp.events & (PxPairFlag::eNOTIFY_TOUCH_LOST | PxPairFlag::eNOTIFY_THRESHOLD_FORCE_LOST)))
        {
            ...
            if (nbContacts) {
                for (PxU32 j = 0; j < nbContacts; j++) {
                    ...
                    PintContactData* C =
                            (PintContactData*)mContactData.Reserve(sizeof(PintContactData) / sizeof(udword));
    ...
    if (!bufferContacts) sendContacts();
}
...
void PEEL_SimulationEventCallback::onAdvance(const PxRigidBody* const* bodyBuffer,
