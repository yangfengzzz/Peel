...
void PhysX_SceneAPI::GetActors(Reporter& reporter) const {
    ...
    for (udword pass = 0; pass < 2; pass++) {
        ...
        while (NbActors--) {
            ...
            if (!reporter.ReportObject(PintActorHandle(CurrentActor.mActor))) return;
        }
    }
}
...
void PhysX_SceneAPI::Cull(udword nb_planes, const Plane* planes, Reporter& reporter) const {
    ...
    if (!physx.GetScene() || !nb_planes || !planes) return;
    ...
    for (udword pass = 0; pass < 2; pass++) {
        ...
        while (NbActors--) {
...

            {
                ...
                udword OutClipMask;
                if (!PlanesAABBOverlap((const AABB&)ActorBounds, planes, OutClipMask, Mask)) continue;
            }

            if (!reporter.ReportObject(PintActorHandle(rigidActor))) return;
        }
    }
}
