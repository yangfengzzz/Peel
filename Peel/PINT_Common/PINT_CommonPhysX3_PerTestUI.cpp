...
// We cannot tweak the PhysX-specific parameters per-scene, since we would have to expose all of them to the shared PINT
// API. So instead we tweak them here based on the test names. This gives us the same flexibility as for regular
// PhysX/Bullet/etc samples, but in the context of PEEL.
//
// We can also create specific per-test per-engine UI dialogs to clearly show which parameters are important/useful for
// any combination of test & physics engine. It's getting complicated but that's a pretty powerful feature.
...
struct UICREATE {
    UICREATE() : mTGS(UI_ELEM_DISABLED) {}
...
static IceWindow* CreateUI_Generic(IceWidget* parent, PintGUIHelper& helper, Widgets& owner, const UICREATE& create) {
    ...
    struct Override {
        static void Generic(const IceCheckBox& check_box, bool checked, void* user_data) {
    ...
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::Generic, null);
    y += YStep;

#if PHYSX_SUPPORT_TGS
    if (create.mTGS != UI_ELEM_DISABLED) {
        IceCheckBox* CB = helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Enable TGS", &owner,
                                                create.mTGS == UI_ELEM_ENABLED_CHECKED, null, null);

        gCheckBox_Generic0 = y += YStep;
    }
#endif
    return TabWindow;
}
...
static IceWindow* CreateUI_TGS(IceWidget* parent, PintGUIHelper& helper, Widgets& owner, bool enabled) {
    ...
    struct Override {
        static void TGS(const IceCheckBox& check_box, bool checked, void* user_data) {
#if PHYSX_SUPPORT_TGS
            gCheckBox_Generic0->SetEnabled(checked);
#endif
        }
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::TGS, null);
    y += YStep;

#if PHYSX_SUPPORT_TGS
    ASSERT(!gCheckBox_Generic0);
    gCheckBox_Generic0 =
            helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Enable TGS", &owner, enabled, null, null);
    y += YStep;
#endif
    return TabWindow;
}
...
static IceWindow* CreateUI_Winch(IceWidget* parent, PintGUIHelper& helper, Widgets& owner, bool enabled) {
    ...
    struct Override {
        static void Winch(const IceCheckBox& check_box, bool checked, void* user_data) {
#if PHYSX_SUPPORT_TGS
            gCheckBox_Generic0->SetEnabled(checked);
#endif
    ...
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, x + EditBoxX, y, EditBoxWidth, 20, "2", &owner,
...
static IceWindow* CreateUI_HighMassRatio(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void HighMassRatio(const IceCheckBox& check_box, bool checked, void* user_data) {
            gEditBox_Generic0->SetEnabled(checked);
#if PHYSX_SUPPORT_TGS
            gCheckBox_Generic0->SetEnabled(checked);
#endif
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::HighMassRatio, null);
    y += YStep;

#if PHYSX_SUPPORT_TGS
    ASSERT(!gCheckBox_Generic0);
    gCheckBox_Generic0 =
            helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Enable TGS", &owner, true, null, null);
    y += YStep;
#endif
    helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Solver iter pos:", &owner);
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, 4 + EditBoxX, y, EditBoxWidth, 20, "16", &owner,
                                             EDITBOX_INTEGER_POSITIVE, null, null);
    y += YStep;
...
static IceWindow* CreateUI_InitialPenetration(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void InitialPenetration(const IceCheckBox& check_box, bool checked, void* user_data) {
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::InitialPenetration, null);
    y += YStep;

#if PHYSX_SUPPORT_MAX_DEPEN_VELOCITY
    helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, 100, 20, "Max depen. velocity:", &owner);
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, 4 + 20 + EditBoxX, y, EditBoxWidth, 20, "2", &owner,
...
static IceWindow* CreateUI_FrictionRamp(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void FrictionRamp(const IceCheckBox& check_box, bool checked, void* user_data) {
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::FrictionRamp, null);
    y += YStep;

#if PHYSX_SUPPORT_IMPROVED_PATCH_FRICTION
    ASSERT(!gCheckBox_Generic0);
    gCheckBox_Generic0 = helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Improved patch friction", &owner,
                                               true, null, null);
    y += YStep;
#endif
#if PHYSX_SUPPORT_STABILIZATION_FLAG
    ...
    gCheckBox_Generic1 =
            helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Stabilization", &owner, false, null, null);
    y += YStep;
#endif
    return TabWindow;
}
...
static IceWindow* CreateUI_Dzhanibekov(IceWidget* parent, PintGUIHelper& helper, Widgets& owner, bool enabled) {
    ...
    struct Override {
        static void Gyro(const IceCheckBox& check_box, bool checked, void* user_data) {
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::Gyro, null);
    y += YStep;

#if PHYSX_SUPPORT_GYROSCOPIC_FORCES
    ASSERT(!gCheckBox_Generic0);
    gCheckBox_Generic0 = helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Enable gyroscopic forces",
                                               &owner, enabled, null, null);
    y += YStep;
#endif
    return TabWindow;
}
...
static IceWindow* CreateUI_KinematicWalker(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void Walker(const IceCheckBox& check_box, bool checked, void* user_data) {
            gEditBox_Generic0->SetEnabled(checked);
        }
    };
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::Walker, null);
    y += YStep;
    ...
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, x + EditBoxX, y, EditBoxWidth, 20,
                                             helper.Convert(MaxBiasCoeff), &owner, EDITBOX_FLOAT_POSITIVE, null, null);
    y += YStep;
...
static IceWindow* CreateUI_DoubleDominoEffect(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    ASSERT(!gCheckBox_Generic0);
    gCheckBox_Generic0 =
            helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Enable sleeping", &owner, false, null, null);
    y += YStep;
...
static IceWindow* CreateUI_SolverIterPos_Substeps(
        IceWidget* parent, PintGUIHelper& helper, Widgets& owner, udword nb_iter, udword nb_substeps) {
    IceWindow* TabWindow = CreateTabWindow(parent, owner);
    ...
    gEditBox_Generic1 = helper.CreateEditBox(TabWindow, 0, 4 + EditBoxX, y, EditBoxWidth, 20, _F("%d", nb_iter), &owner,
                                             EDITBOX_INTEGER_POSITIVE, null, null);
    y += YStep;
    ...
    helper.CreateLabel(TabWindow, x, y + LabelOffsetY, LabelWidth, 20, "Nb substeps:", &owner);
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, x + EditBoxX, y, EditBoxWidth, 20, _F("%d", nb_substeps),
                                             &owner, EDITBOX_INTEGER_POSITIVE, null, null);
    y += YStep;
#endif
    return TabWindow;
}
...
static IceWindow* CreateUI_ArticulationDriveVsStaticObstacle(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void ArticulationDriveVsStaticObstacle(const IceCheckBox& check_box, bool checked, void* user_data) {
            gEditBox_Generic0->SetEnabled(checked);
        }
    };
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::ArticulationDriveVsStaticObstacle, null);
    y += YStep;

    helper.CreateLabel(TabWindow, x, y + LabelOffsetY, LabelWidth, 20, "Contact offset:", &owner);
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, x + EditBoxX, y, EditBoxWidth, 20, "0.2", &owner,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
    y += YStep;
...
static IceWindow* CreateUI_MultiBodyVehicle(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void MultiBodyVehicle(const IceCheckBox& check_box, bool checked, void* user_data) {
#if PHYSX_SUPPORT_SUBSTEPS
            gEditBox_Generic0->SetEnabled(checked);
#endif
        }
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::MultiBodyVehicle, null);
    y += YStep;

#if PHYSX_SUPPORT_SUBSTEPS
    helper.CreateLabel(TabWindow, x, y + LabelOffsetY, LabelWidth, 20, "Nb substeps:", &owner);
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, x + EditBoxX, y, EditBoxWidth, 20, "16", &owner,
                                             EDITBOX_INTEGER_POSITIVE, null, null);
    y += YStep;
#endif
    return TabWindow;
...
static IceWindow* CreateUI_VehicleTest(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void VehicleTest(const IceCheckBox& check_box, bool checked, void* user_data) {
#if PHYSX_SUPPORT_SUBSTEPS
            gEditBox_Generic0->SetEnabled(checked);
#endif
        }
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::VehicleTest, null);
    y += YStep;

#if PHYSX_SUPPORT_SUBSTEPS
    helper.CreateLabel(TabWindow, x, y + LabelOffsetY, LabelWidth, 20, "Nb substeps:", &owner);
    ASSERT(!gEditBox_Generic0);
    gEditBox_Generic0 = helper.CreateEditBox(TabWindow, 0, x + EditBoxX, y, EditBoxWidth, 20, "4", &owner,
                                             EDITBOX_INTEGER_POSITIVE, null, null);
    y += YStep;
#endif
    return TabWindow;
...
public:
    ...
    void SetNbIters(udword nb) { SetValue((nb - 1) / 79.0f); }
...
static IceWindow* CreateUI_FixedJointsTorus(
        IceWidget* parent, PintGUIHelper& helper, Widgets& owner, PhysXPlugIn* plugin, udword nb_iters) {
    IceWindow* TabWindow = CreateTabWindow(parent, owner);
    ...
    struct Override {
        static void FixedJointsTorus(const IceCheckBox& check_box, bool checked, void* user_data) {
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::FixedJointsTorus, null);
    y += YStep;
...
static IceWindow* CreateUI_AddLocalTorque(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void AddLocalTorque(const IceCheckBox& check_box, bool checked, void* user_data) {
#if PHYSX_SUPPORT_STABILIZATION_FLAG
            gCheckBox_Generic1->SetEnabled(checked);
#endif
        }
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::AddLocalTorque, null);
    y += YStep;
...
static IceWindow* CreateUI_BoxStacks(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void BoxStacks(const IceCheckBox& check_box, bool checked, void* user_data) {
#if PHYSX_SUPPORT_TGS
            gCheckBox_Generic0->SetEnabled(checked);
#endif
#if PHYSX_SUPPORT_IMPROVED_PATCH_FRICTION
...
#if PHYSX_SUPPORT_IMPROVED_PATCH_FRICTION
    ASSERT(!gCheckBox_Generic1);
    gCheckBox_Generic1 = helper.CreateCheckBox(TabWindow, 0, 4, y, CheckBoxWidth, 20, "Improved patch friction", &owner,
                                               true, null, null);
    y += YStep;
#endif
    return TabWindow;
...
static IceWindow* CreateUI_JacobsLadder(IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    struct Override {
        static void JacobsLadder(const IceCheckBox& check_box, bool checked, void* user_data) {
            gEditBox_Generic0->SetEnabled(checked);
#if PHYSX_SUPPORT_TGS
            gCheckBox_Generic0->SetEnabled(checked);
#endif
    ...
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::JacobsLadder, null);
    y += YStep;
...
/*static IceWindow* CreateUI_LimitedHingeJoint(IceWidget* parent, PintGUIHelper& helper, Widgets& owner)
{
        ...
        ASSERT(!gCheckBox_Override);
        gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
true, Override::LimitedHingeJoint, null); y += YStep;

        ASSERT(!gCheckBox_Generic0);
        gCheckBox_Generic0 = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Visualize joint limits", &owner, true,
null, null); y += YStep;
...
static IceWindow* CreateUI_CCD(IceWidget* parent, PintGUIHelper& helper, Widgets& owner, const char* test_name) {
    ...
    if (NeedsAngularCCD) EnableCCD = false;
#endif
    ...
    const sdword CheckBoxWidth2 = 220;
    {
        ...
        ASSERT(!gCheckBox_Generic0);
        gCheckBox_Generic0 = helper.CreateCheckBox(TabWindow, 0, x2, y, CheckBoxWidth2, 20, "Enable default PhysX CCD",
                                                   &owner, EnableCCD, null, null);
        y += YStep;
...
#if PHYSX_SUPPORT_ANGULAR_CCD
    {
        ...
        ASSERT(!gCheckBox_Generic1);
        gCheckBox_Generic1 =
                helper.CreateCheckBox(TabWindow, 0, x2, y, CheckBoxWidth2, 20,
                                      "Enable angular CCD/speculative contacts", &owner, NeedsAngularCCD, null, null);
        y += YStep;
...
#if PHYSX_SUPPORT_RAYCAST_CCD
    {
        ...
        gCheckBox_Generic2 = helper.CreateCheckBox(TabWindow, 0, x2, y, CheckBoxWidth2, 20, "Enable raycast CCD",
                                                   &owner, false, null, null);
        y += YStep;
...
static bool IsJointTest(const char* test_name) {
    if (strcmp(test_name, "BasicJointAPI") == 0 || strcmp(test_name, "CheckCollisionBetweenJointed") == 0 ||
        strcmp(test_name, "LimitedHingeJoint") == 0 || strcmp(test_name, "LimitedHingeJoint2") == 0 ||
        strcmp(test_name, "ConfigurableLimitedHingeJoint") == 0 || strcmp(test_name, "LimitedPrismaticJoint") == 0 ||
        strcmp(test_name, "LimitedPrismaticJoint2") == 0 || strcmp(test_name, "PrismaticSpring") == 0 ||
        strcmp(test_name, "D6Joint_PointInPlane") == 0 || strcmp(test_name, "D6Joint_PointInBox") == 0)
        return true;
    return false;
}
...
IceWindow* PhysXPlugIn::InitTestGUI(const char* test_name, IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    ...
    if (strcmp(test_name, "LegoTechnicKart") == 0) return CreateUI_SolverIterPos_Substeps(parent, helper, owner, 4, 4);

    if (strcmp(test_name, "LegoTechnicBuggy") == 0)
        return CreateUI_SolverIterPos_Substeps(parent, helper, owner, 64, 4);

    if (strcmp(test_name, "CDStack") == 0) return CreateUI_SolverIterPos_Substeps(parent, helper, owner, 32, 4);

    if (strcmp(test_name, "Dzhanibekov") == 0) return CreateUI_Dzhanibekov(parent, helper, owner, true);

    if (strcmp(test_name, "Walker") == 0) return CreateUI_KinematicWalker(parent, helper, owner);

    if (strcmp(test_name, "DoubleDominoEffect") == 0) return CreateUI_DoubleDominoEffect(parent, helper, owner);

    if (strcmp(test_name, "ArticulatedChain_RegularJoints") == 0 ||
        strcmp(test_name, "ConfigurableSphericalChain") == 0 || strcmp(test_name, "SwingSet_RegularJoints") == 0 ||
        strcmp(test_name, "Crane_RegularJoints") == 0 || strcmp(test_name, "VoronoiFracture3") == 0 ||
    ...
    if (strcmp(test_name, "ArticulatedChain_MCArticulation") == 0 ||
        strcmp(test_name, "SwingSet_MCArticulation") == 0 || strcmp(test_name, "Crane_MCArticulation") == 0 ||
        strcmp(test_name, "ConnectedMCArticulations") == 0)
        return CreateUI_TGS(parent, helper, owner, false);

    if (strcmp(test_name, "Winch") == 0) return CreateUI_Winch(parent, helper, owner, false);

    if (strcmp(test_name, "BoxStacks") == 0) return CreateUI_BoxStacks(parent, helper, owner);

    if (strcmp(test_name, "JacobsLadder") == 0) return CreateUI_JacobsLadder(parent, helper, owner);

    if (strcmp(test_name, "AddLocalTorque") == 0) return CreateUI_AddLocalTorque(parent, helper, owner);

    if (strcmp(test_name, "FixedJointsTorus") == 0) return CreateUI_FixedJointsTorus(parent, helper, owner, this, 4);

    if (strcmp(test_name, "FixedJointCantilever") == 0)
        return CreateUI_FixedJointsTorus(parent, helper, owner, this, 33);

    if (strcmp(test_name, "HighMassRatio") == 0) return CreateUI_HighMassRatio(parent, helper, owner);

    if (strcmp(test_name, "InitialPenetration") == 0) return CreateUI_InitialPenetration(parent, helper, owner);

    if (strcmp(test_name, "FrictionRamp") == 0 || strcmp(test_name, "FrictionRamp2") == 0 ||
        strcmp(test_name, "FrictionRamp3") == 0)
        return CreateUI_FrictionRamp(parent, helper, owner);

    if (strcmp(test_name, "MultiBodyVehicle") == 0) return CreateUI_MultiBodyVehicle(parent, helper, owner);
...
void PhysXPlugIn::ApplyTestUIParams(const char* test_name) {
    ...
    if (IsJointTest(test_name)) EP.mForceJointLimitDebugViz = true;
    ...
    struct Local {
        static void OverrideSubsteps(EditableParams& EP) {
            ...
            if (NbSubsteps > EP.mNbSubsteps) EP.mNbSubsteps = NbSubsteps;
#endif
        }
    };
    ...
    if (strcmp(test_name, "ArticulatedChain_RegularJoints") == 0 ||
        ...
        strcmp(test_name, "ConfigurableSphericalChain") == 0 || strcmp(test_name, "SwingSet_RegularJoints") == 0 ||
        strcmp(test_name, "SwingSet_MCArticulation") == 0 || strcmp(test_name, "Crane_RegularJoints") == 0 ||
        strcmp(test_name, "Crane_MCArticulation") == 0 || strcmp(test_name, "VoronoiFracture3") == 0 ||
        strcmp(test_name, "ConnectedMCArticulations") == 0 || strcmp(test_name, "CardHouse") == 0) {
#if PHYSX_SUPPORT_TGS
    ...
    if (strcmp(test_name, "FixedJointsTorus") == 0 || strcmp(test_name, "FixedJointCantilever") == 0) {
#if PHYSX_SUPPORT_TGS
        EP.mTGS = gCheckBox_Generic0->IsChecked();
#endif
    ...
    if (strcmp(test_name, "HighMassRatio") == 0) {
        ...
        if (NbIters > EP.mSolverIterationCountPos) EP.mSolverIterationCountPos = NbIters;
#if PHYSX_SUPPORT_TGS
        EP.mTGS = gCheckBox_Generic0->IsChecked();
#endif
        return;
    ...
    if (strcmp(test_name, "CompoundTowerTweaked") == 0 || strcmp(test_name, "LegoTechnicKart") == 0 ||
        strcmp(test_name, "LegoTechnicBuggy") == 0 || strcmp(test_name, "CDStack") == 0) {
        Local::OverrideSubsteps(EP);
        udword NbIters;
    ...
    if (strcmp(test_name, "ArticulationDriveVsStaticObstacle") == 0 ||
        ...
        if (ContactOffset > EP.mContactOffset) EP.mContactOffset = ContactOffset;
        return;
    }
