...
// #define CHECK_MSG(exp, msg) (!!(exp) || (physx::shdfnd::getFoundation().error(physx::PxErrorCode::eINVALID_PARAMETER,
// __FILE__, __LINE__, msg), 0) )
#define CHECK_MSG(exp, msg) \
    (!!(exp) ||             \
     (gDefaultErrorCallback->reportError(physx::PxErrorCode::eINVALID_PARAMETER, msg, __FILE__, __LINE__), 0))
...
public:
    ...
    virtual PxQueryHitType::Enum preFilter(const PxFilterData& filterData,
                                           ...
                                           PxHitFlags& queryFlags) {
        if (shape->getName() == gChassisDebugName || shape->getName() == gWheelDebugName) return PxQueryHitType::eNONE;
    ...
    virtual const PxBatchQueryMemory& getUserMemory() { return mDesc.queryMemory; }
...
static PxF32 gSteerVsForwardSpeedData[2 * 8] = {
        ...
        */
        ...
        0.0f, 1.0f,
        //	0.0f,		0.75f,
        5.0f, 0.75f, 30.0f, 0.125f, 120.0f, 0.1f,
        /*
                // Not bad
                0.0f,		1.0f,
                5.0f,		0.75f,
...
SampleVehicle_VehicleController::SampleVehicle_VehicleController() { clear(); }
...
void SampleVehicle_VehicleController::processRawInputs(const PxF32 dtime,
    ...
    // Gamepad
    {
        ...
        /*		if(mUseKeyInputs && ((mGamepadAccel+mGamepadCarBrake+mGamepadCarSteer)!=0.0f ||  mGamepadGearup ||
...
void SampleVehicle_VehicleController::processAutoReverse(const PxVehicleWheels& focusVehicle,
    ...
    if (driveDynData.getUseAutoGears()) {
        // If the car is travelling very slowly in forward gear without player input and the player subsequently presses
        // the brake then we want the car to go into reverse gear If the car is travelling very slowly in reverse gear
        // without player input and the player subsequently presses the accel then we want the car to go into forward
        // gear If the car is in forward gear and is travelling backwards then we want to automatically put the car into
        // reverse gear. If the car is in reverse gear and is travelling forwards then we want to automatically put the
        // car into forward gear. (If the player brings the car to rest with the brake the player needs to release the
        //brake then reapply it to indicate they want to toggle between forward and reverse.)
        ...
        if (!isInAir) {
            ...
            } else if (PxVehicleGearsData::eREVERSE == currentGear &&
                       forwardSpeed > THRESHOLD_ROLLING_BACKWARDS_SPEED) {
                isMovingBackwards = true;
            }
            ...
            if (isMovingBackwards) {
                ...
                if ((currentGear > PxVehicleGearsData::eNEUTRAL) && brakeRaw && !accelRaw &&
                    (currentGear == targetGear)) {
                    // The car was moving slowly in forward gear without player input and is now moving slowly with
                    // player input that indicates the player wants to switch to reverse gear.
                    toggleAutoReverse = true;
                } else if (currentGear == PxVehicleGearsData::eREVERSE && accelRaw && !brakeRaw &&
                           (currentGear == targetGear)) {
                    // The car was moving slowly in reverse gear without player input and is now moving slowly with
                    // player input that indicates the player wants to switch to forward gear.
                    toggleAutoReverse = true;
                }
            }
...
void SampleVehicle_VehicleController::update(const PxF32 timestep,
                                             const PxVehicleWheelQueryResult& vehicleWheelQueryResults,
    ...
    {
        processAutoReverse(focusVehicle, *driveDynData, vehicleWheelQueryResults, carRawInputs, toggleAutoReverse,
...
SampleVehicle_VehicleManager::SampleVehicle_VehicleManager() : mNumVehicles(0), mSqWheelRaycastBatchQuery(NULL) {}
...
void SampleVehicle_VehicleManager::setTireFrictionMultiplier(float frontTireFrictionMultiplier,
                                                             float rearTireFrictionMultiplier) {
    // ###############
    /*	for(PxU32 i=0;i<MAX_NUM_SURFACE_TYPES;i++)
            {
                    for(PxU32 j=0;j<MAX_NUM_TIRE_TYPES;j++)
...
void SampleVehicle_VehicleManager::suspensionRaycasts(PxScene* scene) {
    ...
    /*
            ...
            void PxVehicleSuspensionSweeps
                    (PxBatchQuery* batchQuery,
                     const PxU32 nbVehicles, PxVehicleWheels** vehicles,
                     const PxU32 nbSceneQueryResults, PxSweepQueryResult* sceneQueryResults,  const PxU16
       nbHitsPerQuery, const bool* vehiclesToSweep = NULL, const PxF32 sweepWidthScale = 1.0f, const PxF32
       sweepRadiusScale = 1.0f);
    */
...
static void setupActor(PxRigidDynamic* vehActor,
...
static PxRigidDynamic* createVehicleActor4W(const PxVehicleChassisData& chassisData,
    ...
    // We need to add wheel collision shapes, their local poses, a material for the wheels, and a simulation filter for
    // the wheels.
    ...
    const PxGeometry* wheelGeometries[4] = {&frontLeftWheelGeom, &frontRightWheelGeom, &rearLeftWheelGeom,
                                            &rearRightWheelGeom};
    const PxTransform wheelLocalPoses[4] = {PxTransform(PxIdentity), PxTransform(PxIdentity), PxTransform(PxIdentity),
    ...
    setupActor(vehActor, wheelGeometries, wheelLocalPoses, 4, &wheelMaterial, chassisGeoms, chassisLocalPoses, 1,
               &chassisMaterial, chassisData, &physics);
...
static void createVehicle4WSimulationData(PxConvexMesh* chassisConvexMesh,
                                          PxConvexMesh** wheelConvexMeshes,
...
PxVehicleDrive4W* SampleVehicle_VehicleManager::create4WVehicle(SharedPhysX_Vehicles* sharedPhysX,
    ...
    PxVehicleChassisData chassisData;
    createVehicle4WSimulationData(chassisConvexMesh, wheelConvexMeshes4, *wheelsSimData, driveSimData, chassisData,
                                  desc);

    // Instantiate and finalize the vehicle using physx.
    PxRigidDynamic* vehActor = createVehicleActor4W(chassisData, wheelConvexMeshes4, chassisConvexMesh, *scene, physics,
                                                    material, ToPxTransform(desc.mChassisLocalPose));
...
VehicleTest::~VehicleTest() { mVehicleManager.shutdown(); }
...
void VehicleTest::Update(float dt) {
    mVehicleController.setCarKeyboardInputs(
            ...
            mInput.mAccelerate, mInput.mBrake, false,
            //			false,	mInput.mBrake,
            mInput.mRight, mInput.mLeft, false, false);
    ...
    mVehicleController.update(dt, mVehicleManager.getVehicleWheelQueryResults(mPlayerVehicle),
                              *mVehicleManager.getVehicle(mPlayerVehicle));
    ...
    if (1 && dt > 0.0f) {
        ...
        //		mVehicleManager.updateAndRecordTelemetryData(dt, mScene->getGravity(),
        //mVehicleManager.getVehicle(mPlayerVehicle), mTelemetryData4W); #else
...
PxVehicleDrive4W* VehicleTest::Init(SharedPhysX_Vehicles* physx,
                                    PxConvexMesh* chassis_mesh,
                                    PxConvexMesh* wheel_mesh0,
                                    PintShapeRenderer* renderer0,
                                    PxConvexMesh* wheel_mesh1,
    ...
    PxVehicleDrive4W* Vehicle4W =
            mVehicleManager.create4WVehicle(physx, physics, cooking, *mChassisMaterial, chassis_mesh, wm, true, desc);
    ASSERT(Vehicle4W);
...
PintVehicleHandle SharedPhysX_Vehicles::CreateVehicle(PintVehicleData& data, const PINT_VEHICLE_CREATE& vehicle) {
    ...
    PxConvexMesh* ChassisMesh = CreateConvexMesh(ChassisDesc->mVerts, ChassisDesc->mNbVerts,
                                                 PxConvexFlag::eCOMPUTE_CONVEX, vehicle.mChassis->mRenderer);
    ASSERT(ChassisMesh);

    const PINT_CONVEX_CREATE* WheelDesc0 = static_cast<const PINT_CONVEX_CREATE*>(vehicle.mWheels[0]);
    PxConvexMesh* WheelMesh0 = CreateConvexMesh(WheelDesc0->mVerts, WheelDesc0->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX,
                                                WheelDesc0->mRenderer);
    ASSERT(WheelMesh0);
    ...
    if (vehicle.mWheels[1]) {
        ...
        WheelMesh1 =
                CreateConvexMesh(wheel1->mVerts, wheel1->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX, wheel1->mRenderer);
        ASSERT(WheelMesh1);
        ...
        WheelMesh2 =
                CreateConvexMesh(wheel2->mVerts, wheel2->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX, wheel2->mRenderer);
        ASSERT(WheelMesh2);
        ...
        WheelMesh3 =
                CreateConvexMesh(wheel3->mVerts, wheel3->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX, wheel3->mRenderer);
        ASSERT(WheelMesh3);
        Renderer3 = wheel3->mRenderer;
    }

    PxVehicleDrive4W* Vehicle4W = vt->Init(this, ChassisMesh, WheelMesh0, WheelDesc0->mRenderer, WheelMesh1, Renderer1,
                                           WheelMesh2, Renderer2, WheelMesh3, Renderer3, vehicle);
    ASSERT(Vehicle4W);
