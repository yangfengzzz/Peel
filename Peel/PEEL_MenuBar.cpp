...
void UpdateToolsCheckmark() {
    if (gMenu_Tools) {
        ...
        for (udword i = 0; i < NbTools; i++) gMenu_Tools->SetChecked(MENU_TOOL + i, i == CurrentToolIndex);
    }
}
...
void CreatePEELMenuBar(IceWindow* parent, Widgets& widgets) {
    ...
    IceMenu* Menu_Tools = ICE_NEW(IceMenu);
    {
        ...
        const udword NbTools = GetNbTools();
        for (udword i = 0; i < NbTools; i++) Menu_Tools->Add(GetToolName(i), MENU_TOOL + i);
...
void ReleasePEELMenuBar() { DELETESINGLE(gRecentFiles); }
...
void ProcessMenuEvent(udword menu_item_id) {
    ...
    if (menu_item_id >= MENU_RECENT_FILE_0 && menu_item_id <= MENU_RECENT_FILE_9) {
        if (gRecentFiles) {
            String FileName;
            if (gRecentFiles->GetSymbolicValue(menu_item_id - MENU_RECENT_FILE_0, FileName)) ImportFile(FileName);
        }
        ...
        if (GetEditor())
            ...
            IceCore::MessageBox(
                    0, "Export only available in editor mode.\n\nEnable the editor checkbox when starting PEEL.",
        ...
        if (!FileselectOpenSingle(Create, ScriptFilename)) return;

        RunScript(ScriptFilename);
    } else if (menu_item_id == MENU_ABOUT) {
        IceCore::MessageBox(0,
                            "-= PEEL v2.0 =-\n\nCreated by Pierre Terdiman (2012-2020)\n\nUses the ICE framework - "
                            "Pierre Terdiman (1999-2020)",
                            "About PEEL", MB_OK);
