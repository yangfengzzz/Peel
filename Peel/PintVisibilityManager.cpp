...
VisibilityManager::VisibilityManager() : mEngine(null), mNeedsTranslation(false) {}
...
void VisibilityManager::SetRenderable(PintActorHandle handle, bool visible) {
    ...
    if (mNeedsTranslation) handle = Editor_GetNativeHandle(handle);
...
bool VisibilityManager::IsRenderable(PintActorHandle handle) const { return !mInvisibles.contains(handle); }
...
/*	struct SceneActorShape
        {
                PX_FORCE_INLINE	SceneActorShape(uint64_t scene, uint64_t actor, uint64_t shape) : mScene(scene),
   mActor(actor), mShape(shape)	{}
