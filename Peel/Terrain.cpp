...
CustomLayer::~CustomLayer() { ICE_FREE(mHeights); }
...
TerrainData::~TerrainData() { Release(); }
...
void TerrainData::AddLayer(const HeightLayer* layer) { mLayers.AddPtr(layer); }
...
void TerrainData::UpdateVertices(Point* v) {
    ...
    for (udword i = 0; i < NbVerts; i++) {
        if (v) v[i].y = *NewHeights;

        V[i].y = *NewHeights++;
    }
}

void TerrainData::UpdateRenderer() { mRenderer->UpdateVerts(mSurface.GetVerts(), null); }
...
bool RegisterTerrainData(PintActorHandle owner, TerrainData* terrain_data) {
    if (!gTerrainCache) gTerrainCache = new TerrainCache;

    return gTerrainCache->insert(owner, terrain_data);
}

bool UnregisterTerrainData(PintActorHandle owner) { return gTerrainCache ? gTerrainCache->erase(owner) : false; }

TerrainData* GetTerrainData(PintActorHandle owner) {
    if (!gTerrainCache) return null;
...
void UnregisterAllTerrainData() { DELETESINGLE(gTerrainCache); }
...
// static
void CreateAlternativeGridTopologyLOD(
        IndexedTriangle* tris, udword nb_faces, udword nbu, udword nbv, udword nbu2, udword nbv2, udword lod) {
    udword k = 0;
...
bool MakePlane(IndexedSurface& surface, udword nbu, udword nbv, Quad* fit, bool alternative_topology) {
    ...
    if (alternative_topology) {
        ...
        //
        // The order in which vertices are created is normalized, so that further routines can assume the underlying
        // plane topology is well-known. (For example it is useful to create springs in cloth simulation).
        //
        ...
        // nbu  nbu+1 ...
        //
        // The grid is centered around the origin. Triangles are created as above. Number of triangles is
        // (nbu-1)*(nbv-1)*2.
        //
    ...
    if (!surface.Init(NbFaces, NbVerts, null, null, false)) return false;
    ...
    if (!fit) {
        ...
        for (udword y = 0; y < nbv; y++) {
            for (udword x = 0; x < nbu; x++) {
                Verts[x + y * nbu] =
                        Point(float(x) - (float(nbu - 1) * 0.5f), 0.0f, float(y) - (float(nbv - 1) * 0.5f));
                Verts[x + y * nbu] *= size;
            }
        }
    } else {
...
static inline_ float PERLIN_at2(const float* q, float rx, float rz) { return rx * q[0] + rz * q[2]; }
...
public:
    float Compute2(const Point& vector) const {
        if (1) {
            ...
            int bx0, bx1;
            float rx0, rx1;
            PerlinSetup(vector.x, bx0, bx1, rx0, rx1);
            int bz0, bz1;
            float rz0, rz1;
            PerlinSetup(vector.z, bz0, bz1, rz0, rz1);
            ...
            int by0, by1;
            float ry0, ry1;
            PerlinSetup(vector.y, by0, by1, ry0, ry1);
            int bz0, bz1;
            float rz0, rz1;
            PerlinSetup(vector.z, bz0, bz1, rz0, rz1);
...
static inline_ void SetupParams(Params& params,
                                Point* V,
                                Point* N,
                                udword nbu,
                                udword nbv,
...
void CreateProceduralTerrain(Point* V,
                             Point* N,
                             udword nbu,
                             udword nbv,
                             const Point& min,
