...
static PintShapeRenderer* RegisterConvexInstanceRenderer(PintShapeRenderer* renderer) {
    ...
    if (!gConvexInstancesRenderers) gConvexInstancesRenderers = ICE_NEW(PtrContainer);
...
static const char* testVertexShader = "#version 330 core\n" STRINGIFY(
        layout(location = 0) in vec3 aPos; layout(location = 1) in vec4 instanceRot;
        ...
        void main() {
            ...
            vec4 row0 = vec4(1.0 - yy_yz_zz.x - yy_yz_zz.z, xx_xy_xz.y + wx_wy_wz.z, xx_xy_xz.z - wx_wy_wz.y, 0.0);

            vec4 row1 = vec4(xx_xy_xz.y - wx_wy_wz.z, 1.0 - xx_xy_xz.x - yy_yz_zz.z, yy_yz_zz.y + wx_wy_wz.x, 0.0);

            vec4 row2 = vec4(xx_xy_xz.z + wx_wy_wz.y, yy_yz_zz.y - wx_wy_wz.x, 1.0 - xx_xy_xz.x - yy_yz_zz.x, 0.0);
...
static const char* testVertexShader =
        "#version 330 core\n" STRINGIFY(layout(location = 0) in vec3 aPos; layout(location = 1) in mat4 instanceMatrix;
                                        ...
                                        void main() {
                                            ...
                                            //		gl_Position = vec4(aPos.x, aPos.y, aPos.z, 1.0);
                                            //		gl_Position = vec4(aPos, 1.0);
                                            //		gl_Position = projection * view * instanceMatrix *
                                            //vec4(aPos, 1.0);
                                            gl_Position = viewProj * p;
                                            //		gl_Position = projection * view * vec4(aPos, 1.0);
...
static const char* testPixelShader = "#version 330 core\n" STRINGIFY(
        out vec4 FragColor; in vec4 varyPos; uniform vec3 parallelLightDir; uniform vec3 mainColor;
...
PintConvexInstanceRenderer::PintConvexInstanceRenderer(udword nb_verts, const Point* verts)
    ...
    glVertexAttribPointer(baseLoc + 0, 4, GL_FLOAT, GL_FALSE, Stride, (void*)0);
    glEnableVertexAttribArray(baseLoc + 1);
    glVertexAttribPointer(baseLoc + 1, 4, GL_FLOAT, GL_FALSE, Stride, (void*)(vec4Size));
#ifndef QUAT2MATRIXSHADER
    glEnableVertexAttribArray(baseLoc + 2);
    glVertexAttribPointer(baseLoc + 2, 4, GL_FLOAT, GL_FALSE, Stride, (void*)(2 * vec4Size));
    glEnableVertexAttribArray(baseLoc + 3);
    glVertexAttribPointer(baseLoc + 3, 4, GL_FLOAT, GL_FALSE, Stride, (void*)(3 * vec4Size));
#endif
...
#endif
    if (mCurrentNbInstances == mMaxNbInstances) DrawBatch();
}
