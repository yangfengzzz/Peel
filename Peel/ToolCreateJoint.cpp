...
static IceButton* GUI_CreatePushButton(IceWidget* parent,
                                       udword id,
                                       sdword x,
                                       sdword y,
                                       sdword width,
    ...
    if (owner) owner->Register(B);

    if (tooltip) B->AddToolTip(tooltip);
    return B;
}
...
void JointListBoxes::Reset() {
    if (mJointsFromActor0) mJointsFromActor0->RemoveAll();
    if (mJointsFromActor1) mJointsFromActor1->RemoveAll();
}
...
bool JointListBoxes::FillListBox(
        Pint& pint, PintActorHandle actor, udword id, PintJoint jt0, PintJoint jt1, PintJoint jt2) {
    Pint_Actor* ActorAPI = pint.GetActorAPI();
    if (!ActorAPI) return false;

    Pint_Joint* JointAPI = pint.GetJointAPI();
    if (!JointAPI) return false;
    ...
    for (udword i = 0; i < NbJoints; i++) {
        ...
        if (jh) {
            ...
            if (jt == jt0 || jt == jt1 || jt == jt2) {
                ...
                if (!JointName) JointName = "(unnamed joint)";

                LB->Add(JointName);
                if (!Index) LB->Select(0);
...
void GearJointWindow::Reset() {
    if (mEditBox_NbTeeth0) mEditBox_NbTeeth0->SetText("0");
    if (mEditBox_NbTeeth1) mEditBox_NbTeeth1->SetText("0");
    mListBoxes.Reset();
}
...
void GearJointWindow::Create(PintGUIHelper& helper, Widgets& owner) {
    ...
    mEditBox_NbTeeth0 = helper.CreateEditBox(this, 0, x + OffsetX, y, EditBoxWidth, 20, "0", &owner,
                                             EDITBOX_INTEGER_POSITIVE, null, null);

    y += 20;
    helper.CreateLabel(this, x, y + LabelOffsetY, LabelWidth, 20, "Nb teeth1:", &owner);
    mEditBox_NbTeeth1 = helper.CreateEditBox(this, 0, x + OffsetX, y, EditBoxWidth, 20, "0", &owner,
                                             EDITBOX_INTEGER_POSITIVE, null, null);

    y += 20;
    /*	helper.CreateLabel(this, x, y+LabelOffsetY, LabelWidth, 20, "Error sign:", &owner);
            mComboBox_ErrorSign = CreateComboBox<IceComboBox>(this, 0, x+OffsetX, y, 150, 20, "Error sign", &owner,
       null); mComboBox_ErrorSign->Add("No error correction"); mComboBox_ErrorSign->Add("+1");
...
void RackJointWindow::Reset() {
    if (mEditBox_NbTeethPinion) mEditBox_NbTeethPinion->SetText("0");
    if (mEditBox_NbTeethRack) mEditBox_NbTeethRack->SetText("0");
    if (mEditBox_RackLength) mEditBox_RackLength->SetText("1.0");
    if (mCheckBox_SelectShape) mCheckBox_SelectShape->SetChecked(false);
    mListBoxes.Reset();
    mRackBounds.SetEmpty();
}
...
void RackJointWindow::Create(PintGUIHelper& helper, Widgets& owner) {
    ...
    mEditBox_NbTeethPinion = helper.CreateEditBox(this, 0, x + OffsetX, y, EditBoxWidth, 20, "0", &owner,
                                                  EDITBOX_INTEGER_POSITIVE, null, null);

    y += 20;
    helper.CreateLabel(this, x, y + LabelOffsetY, LabelWidth, 20, "Nb teeth rack:", &owner);
    mEditBox_NbTeethRack = helper.CreateEditBox(this, 0, x + OffsetX, y, EditBoxWidth, 20, "0", &owner,
                                                EDITBOX_INTEGER_POSITIVE, null, null);

    y += 20;
    helper.CreateLabel(this, x, y + LabelOffsetY, LabelWidth, 20, "Rack length:", &owner);
    mEditBox_RackLength = helper.CreateEditBox(this, 0, x + OffsetX, y, EditBoxWidth, 20, "1.0", &owner,
...
void ToolCreateJointBase::Reset(udword pint_index) {
    if (mJointType) mJointType->Select(IJT_SPHERICAL_X);
    OnComboBoxChange();

    mPint = null;
    mPendingAction = NONE;

    if (mEditPos0) mEditPos0->Reset();
    if (mEditPos1) mEditPos1->Reset();

    mEditRot0.Identity();
    mEditRot1.Identity();

    if (mGearJointWindow) static_cast<GearJointWindow*>(mGearJointWindow)->Reset();
    if (mRackJointWindow) static_cast<RackJointWindow*>(mRackJointWindow)->Reset();
}

void ToolCreateJointBase::PostRenderCallback() { mPendingAction = NONE; }

void ToolCreateJointBase::CreateJointComboBox(
        PintGUIHelper& helper, IceWidget* parent, Widgets& owner, sdword x, sdword y) {
    helper.CreateLabel(parent, x, y, 90, 20, "Joint type:", &owner);
    mJointType = CreateComboBox<JointTypeComboBox>(parent, 0, x + 60, y, 150, 20, "Joint type", &owner,
                                                   null /*gTooltip_CurrentTool*/);
...
void ToolCreateJointBase::CreateJointWindows(
        PintGUIHelper& helper, IceWidget* parent, Widgets& owner, sdword x, sdword y) {
    struct Callbacks {
    ...
    sdword SavedY;
    {
        ...
        mSnap0 = helper.CreateButton(parent, 0, x + Spacing * 0, y, 90, 20, "Snap to 1", &owner, Callbacks::Snap, null);
        mSnap0->SetUserData(this);
        mSnap1 = helper.CreateButton(parent, 1, x + Spacing * 1, y, 90, 20, "Snap to 0", &owner, Callbacks::Snap, null);
        mSnap1->SetUserData(this);
        if (0) {
            IceButton* B2 = helper.CreateButton(parent, 2, x + Spacing * 2, y, 90, 20, "Snap to 1", &owner,
                                                Callbacks::Snap, null);
            B2->SetUserData(this);
            IceButton* B3 = helper.CreateButton(parent, 3, x + Spacing * 3, y, 90, 20, "Snap to 0", &owner,
                                                Callbacks::Snap, null);
            B3->SetUserData(this);
        }

        y += YStep;
        mFromShape0 = helper.CreateButton(parent, 0, x + Spacing * 0, y, 90, 20, "From shape", &owner,
                                          Callbacks::FromShape, null);
        mFromShape0->SetUserData(this);
        mFromShape1 = helper.CreateButton(parent, 1, x + Spacing * 1, y, 90, 20, "From shape", &owner,
                                          Callbacks::FromShape, null);
        mFromShape1->SetUserData(this);
    }
...
ToolCreateJoint::ToolCreateJoint() : mActor0(null), mShape0(null), mActor1(null), mShape1(null) {}
...
void ToolCreateJoint::Reset(udword pint_index) {
    ...
    if (mButton0) mButton0->SetButtonDown(false);
    if (mButton1) mButton1->SetButtonDown(false);
    mActiveButton = null;
    mActor0 = null;
    mShape0 = null;
    mActor1 = null;
    mShape1 = null;
    if (mEditBoxName) mEditBoxName->SetText("JointName");
...
static void RenderJointFrames(Pint& pint,
                              PintActorHandle actor,
                              const Point& jointFramePos,
                              const Quat& jointFrameRot) {
    if (!actor) return;
...
static void Snap0(Pint& pint,
                  PintActorHandle actor0,
                  PintActorHandle actor1,
                  EditPosWindow* edit_pos0,
                  const EditPosWindow* edit_pos1,
                  Quat& edit_rot0,
                  const Quat& edit_rot1) {
    // ASSERT(actor0);
    // ASSERT(actor1);
    ASSERT(edit_pos0);
    ASSERT(edit_pos1);
...
static void Snap1(Pint& pint,
                  PintActorHandle actor0,
                  PintActorHandle actor1,
                  const EditPosWindow* edit_pos0,
                  EditPosWindow* edit_pos1,
                  const Quat& edit_rot0,
                  Quat& edit_rot1) {
    //	ASSERT(actor0);
    //	ASSERT(actor1);
    ASSERT(edit_pos0);
...
void ToolCreateJoint::CreateJoint(Pint& pint) {
    struct Local {
        ...
        /*			static PintJointHandle FindJoint(Pint_Actor* actor_api, Pint_Joint* joint_api,
        PintActorHandle actor, PR& pose, PintJoint type0, PintJoint type1)
                        {
        ...
        //						if(jt==PINT_JOINT_HINGE || jt==PINT_JOINT_HINGE2)	// ###
        or D6 I guess? if(jt==type0 || jt==type1)	// ### or D6 I guess?
                                                {
                                                        PintActorHandle a0, a1;
                                                        joint_api->GetActors(jh, a0, a1);
    ...
    switch (JT) {
        ...
        case IJT_HINGE_Z: {
            if (1) {
                ...
                if (mEditPos0) Desc.mLocalPivot0 = mEditPos0->mEdited;
                if (mEditPos1) Desc.mLocalPivot1 = mEditPos1->mEdited;
        ...
        case IJT_GEAR: {
            ...
            if (ok) {
                ...
                //					Desc.mHinge0 = Local::FindJoint(ActorAPI, JointAPI, mActor0,
                //Desc.mLocalPivot0, PINT_JOINT_HINGE, PINT_JOINT_HINGE2); 					Desc.mHinge1 = Local::FindJoint(ActorAPI,
                //JointAPI, mActor1, Desc.mLocalPivot1, PINT_JOINT_HINGE, PINT_JOINT_HINGE2);
                ...
                if (!Desc.mHinge0 || !Desc.mHinge1) {
                    IceCore::MessageBox(0, "Selected actors must have a hinge joint", "Cannot create gear joint",
                                        MB_OK);
                } else {
                    ...
                    if (!NbTeeth0 || !NbTeeth1) {
                        ...
                        /*							if(mActor0)
                           ...
                           Desc.mLocalPivot0.mRot.GetConjugate(); if(mActor1) Desc.mLocalPivot1.mRot =
                           Desc.mLocalPivot1.mRot.GetConjugate();*/
        ...
        case IJT_RACK: {
            ...
            if (ok) {
                ...
                //					Desc.mHinge = Local::FindJoint(ActorAPI, JointAPI, mActor0,
                //Desc.mLocalPivot0, PINT_JOINT_HINGE, PINT_JOINT_HINGE2); 					Desc.mPrismatic = Local::FindJoint(ActorAPI,
                //JointAPI, mActor1, Desc.mLocalPivot1, PINT_JOINT_PRISMATIC, PINT_JOINT_PRISMATIC);
                ...
                if (!Desc.mHinge || !Desc.mPrismatic) {
                    IceCore::MessageBox(0, "Selected actors must have a hinge & prismatic joint respectively",
                                        "Cannot create rack joint", MB_OK);
                } else {
    ...
    if (j) {
        ...
        if (LastError) IceCore::MessageBox(0, LastError->mErrorText, "Error", MB_OK);
    }
}
...
bool ToolCreateJoint::ValidateTouchedActor(Pint& pint,
                                           IceButton* button,
                                           PintActorHandle& dstActor,
                                           PintShapeHandle& dstShape,
                                           const PintRaycastHit& hit,
                                           PintActorHandle other_actor,
                                           bool& validate) {
...
void ToolCreateJoint::RightDownCallback(Pint& pint, udword pint_index) {
    ...
    if (!mActiveButton || !mActiveButton->GetButtonDown()) return;
    ...
    if (Raycast(pint, Hit, mOrigin, mDir) && !IsDefaultEnv(pint, Hit.mTouchedActor)) {
        ...
        /*		bool ValidateHit = true;
                        if(mPint && mPint!=&pint)
                        {
                                IceCore::MessageBox(0, "You cannot join actors from different engines!", "Nice try",
           MB_OK); ValidateHit = false;
                        }
                        else
                        {
                                ValidateTouchedActor(pint, mButton0, mActor0, mShape0, Hit, mActor1, ValidateHit);
                        ...
                        if(ValidateHit)
                        {
                                ...
                                if(API)
                                {
                                        const char* Name = API->GetName(Hit.mTouchedActor);
                                        if(Name)
                                                mActiveButton->SetLabel(_F("%s - 0x%x", Name,
           size_t(Hit.mTouchedActor))); else mActiveButton->SetLabel(_F("0x%x", size_t(Hit.mTouchedActor)));
                                }
                                else
                                        mActiveButton->SetLabel(_F("0x%x", size_t(Hit.mTouchedActor)));
...
void ToolCreateJoint::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
    ...
    {
        mButton0 = GUI_CreatePushButton(parent, 0, x, y, 200, 20, gButtonLabel0, &owner, Callbacks::SelectActor, this,
                                        gTooltip_SelectActor_Picking);
        helper.CreateButton(parent, 0, x + 204, y, 16, 20, "--", &owner, Callbacks::SelectActorFromList, this,
                            gTooltip_SelectActor_List);
        y += YStep;

        mButton1 = GUI_CreatePushButton(parent, 0, x, y, 200, 20, gButtonLabel1, &owner, Callbacks::SelectActor, this,
                                        gTooltip_SelectActor_Picking);
        helper.CreateButton(parent, 1, x + 204, y, 16, 20, "--", &owner, Callbacks::SelectActorFromList, this,
                            gTooltip_SelectActor_List);
        y += YStep;

        IceButton* B = helper.CreateButton(parent, 0, 260, 4, 200, 20, "Create joint", &owner, Callbacks::CreateJoint,
                                           this, null);
    }

    {
        ...
        mEditBoxName =
                helper.CreateEditBox(parent, 0, NameX + 40, NameY, 200, 20, "JointName", &owner, EDITBOX_TEXT, null);
    }
...
void ToolCreateJoint::ResetButtonNames(bool reset0, bool reset1) {
    if (!mButton0) reset0 = false;
    if (!mButton1) reset1 = false;

    ASSERT(mJointType.mWidget);
    const udword SelectedIndex = mJointType->GetSelectedIndex();
    if (SelectedIndex == IJT_GEAR) {
        if (reset0) mButton0->SetLabel("Select 1st gear");
        if (reset1) mButton1->SetLabel("Select 2nd gear");
    } else if (SelectedIndex == IJT_RACK) {
        if (reset0) mButton0->SetLabel("Select pinion");
        if (reset1) mButton1->SetLabel("Select rack");
    } else {
        if (reset0) mButton0->SetLabel(gButtonLabel0);
        if (reset1) mButton1->SetLabel(gButtonLabel1);
    }
}
    ...
    if (SelectedIndex == IJT_GEAR) {
        ...
        if (mActiveButton == mButton0) {
            ...
            if (RJW->mCheckBox_SelectShape && RJW->mCheckBox_SelectShape->IsChecked()) {
                ...
                if (API) ValidBounds = API->GetWorldBounds(mActor1, mShape1, Bounds);
            } else {
                Pint_Actor* API = mPint->GetActorAPI();
                if (API) ValidBounds = API->GetWorldBounds(mActor1, Bounds);
            }
...
bool ToolCreateChain::ValidateActor(PintActorHandle actor) const {
    ...
    for (udword i = 0; i < NbLinkActors; i++) {
        if (Actors[i].mActor == actor) return false;
    }
    return true;
}
        ...
        if (NbLinkActors) {
            ...
            if (!mActiveButton) {
                /*				// ### to refactor / optimize
                                                ...
                                                const float Coeff = 1.0f/float(NbLinkActors);
                                                Point* Positions =
                   reinterpret_cast<Point*>(StackAlloc(sizeof(Point)*NbLinkActors)); for(udword i=0;i<NbLinkActors;i++)
                                                {
                                                        const PR ActorPose = pint.GetWorldTransform(Actors[i].mActor);
                                                        Center += ActorPose.mPos * Coeff;
                                                        Positions[i] = ActorPose.mPos;
                                                ...
                                                for(udword i=0;i<NbLinkActors;i++)
                                                {
                                                        ...
                                                        GLRenderHelpers::DrawLine(CurrentPos, Center, Point(ColorCoeff,
                   ColorCoeff, ColorCoeff));

                                                        const udword j = (i+1)%NbLinkActors;
                                                        const Point& NextPos = Positions[Sorted[j]];
                                                        GLRenderHelpers::DrawLine(CurrentPos, NextPos, Point(0.0f,
                   0.0f, 1.0f));
                                                        ...
                                                        RenderJointFrames(pint, CurrentActor, mEditPos0->mEdited,
                   mEditRot0); RenderJointFrames(pint, NextActor, mEditPos1->mEdited, mEditRot1);
                                                }*/
...
void ToolCreateChain::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
                    ...
                    for (udword i = 0; i < NbCurrentlySelected; i++) {
                        ...
                        if (tcc->mPint && tcc->mPint != pint) {
                            ...
                            if (ValidateHit) {
                                ...
                                //  ### hack to make selected links more visible
                                // DefaultControlInterface& DCI = GetDefaultControlInterface();
                                // SelectedObject* Selected = DCI.mSelMan.AddToSelection(mPint, Hit.mTouchedActor,
                                // pint_index);
                            }
                            // else
                            //	IceCore::MessageBox(0, "You cannot select the same actor multiple times", "Error",
                            //MB_OK);
                        }
                    }
                }
            ...
            /*			ASSERT(tcc->mPint);
                                    ...
                                    if(API)
                                    {
                                            if(tcj->mActor0 && API->GetMass(tcj->mActor0)==0.0f)
                                            {
                                                    if(tcj->mActor1 && API->GetMass(tcj->mActor1)==0.0f)
                                                    {
                                                            IceCore::MessageBox(0, "Cannot create joint between two
               static actors", "Error", MB_OK); return;
                                                    }
                                            }
                                    }*/
    ...
    mButton0 = GUI_CreatePushButton(parent, 0, x, y, 200, 20, gButtonLabel2, &owner, Callbacks::SelectLinkActors, this,
                                    null);

    IceButton* B = helper.CreateButton(parent, 0, 260, 4, 200, 20, "Create joints", &owner, Callbacks::CreateJoints,
                                       this, null);
