...
EditPosWindow::EditPosWindow(const WindowDesc& desc, EditBoxInterface* owner)
    ...
    struct Local {
        static void gEBCallback(const IceEditBox& edit_box, udword param, void* user_data) {
            if (param != IceEditBox::CB_PARAM_RETURN) return;
            ...
            if (EPW->mOwner) EPW->mOwner->ChangeNotification();
        }
    };
...
void EditPosWindow::ChangeNotification() {
    if (mOwner) mOwner->ChangeNotification();
}

void EditPosWindow::SetPos(const Point& pos) {
    if (mEditBoxX) mEditBoxX->SetText(_F("%f", pos.x));
    if (mEditBoxY) mEditBoxY->SetText(_F("%f", pos.y));
    if (mEditBoxZ) mEditBoxZ->SetText(_F("%f", pos.z));
    mEdited = pos;
}
...
void EditPosWindow::Reset() {
    mHasEdit = false;
    mEdited.Zero();
    if (mEditBoxX) mEditBoxX->SetText("0");
    if (mEditBoxY) mEditBoxY->SetText("0");
    if (mEditBoxZ) mEditBoxZ->SetText("0");
}

void EditPosWindow::SetEnabled(bool b) {
    if (mEnabled == b) return;
    mEnabled = b;

    if (mEditBoxX) mEditBoxX->SetEnabled(b);
    if (mEditBoxY) mEditBoxY->SetEnabled(b);
    if (mEditBoxZ) mEditBoxZ->SetEnabled(b);
}

bool EditPosWindow::SomethingChanged() const {
    if (mEditBoxX && mEditBoxX->mSomethingChanged) return true;
    if (mEditBoxY && mEditBoxY->mSomethingChanged) return true;
    if (mEditBoxZ && mEditBoxZ->mSomethingChanged) return true;
    return mHasEdit;
}

void EditPosWindow::ResetChangedState() {
    if (mEditBoxX) mEditBoxX->mSomethingChanged = false;
    if (mEditBoxY) mEditBoxY->mSomethingChanged = false;
    if (mEditBoxZ) mEditBoxZ->mSomethingChanged = false;
    mHasEdit = false;
}
