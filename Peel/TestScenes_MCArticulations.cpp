...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 10;
        {
            ...
            mEditBox_NbSimCalls = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20,
                                                       _F("%d", mValues.mNbSimCallsPerRenderFrame), &UIElems,
                                                       EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            mCheckBox_Articulations = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use articulations", &UIElems,
                                                            mValues.mUseArticulations, null, null);
            y += YStep;

            //				mCheckBox_RCArticulations = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use RC
            //articulations", &UIElems, mValues.mUseRCArticulations, null, null); 				y +=
            //YStep;

            mCheckBox_DistanceConstraints =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use extra distance constraints", &UIElems,
                                          mValues.mUseExtraDistanceConstraints, null, null);
            y += YStep;

            //			mCheckBox_OverlappingLinks = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Create
            //overlapping capsules", &UIElems, false, null, null); 			y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Nb links:", &UIElems);
            mEditBox_NbLinks = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, _F("%d", mValues.mNbLinks),
                                                    &UIElems, EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Capsule radius:", &UIElems);
            mEditBox_CapsuleRadius =
                    helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, _F("%.2f", mValues.mCapsuleRadius),
                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Capsule mass:", &UIElems);
            mEditBox_CapsuleMass =
                    helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, _F("%.2f", mValues.mCapsuleMass),
                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Capsule mass for inertia:", &UIElems);
            mEditBox_CapsuleMassForInertia = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20,
                                                                  _F("%.2f", mValues.mCapsuleMassForInertia), &UIElems,
                                                                  EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Box mass:", &UIElems);
            mEditBox_BoxMass =
                    helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, _F("%.2f", mValues.mBoxMass),
                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Initial angular velocity:", &UIElems);
            mEditBox_InitialAngularVel = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20,
                                                              _F("%.2f", mValues.mInitialAngularVelocity), &UIElems,
...
public:
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        const bool UseArticulations =
                mCheckBox_Articulations ? mCheckBox_Articulations->IsChecked() : mValues.mUseArticulations;
        bool UseRCArticulations = false;
        //		if(!UseArticulations)
        //			UseRCArticulations = mCheckBox_RCArticulations ? mCheckBox_RCArticulations->IsChecked()
        //: mValues.mUseRCArticulations;

        const bool UseDistanceConstraints = mCheckBox_DistanceConstraints ? mCheckBox_DistanceConstraints->IsChecked()
                                                                          : mValues.mUseExtraDistanceConstraints;
        //		const bool UseOverlappingLinks = mCheckBox_OverlappingLinks ?
        // mCheckBox_OverlappingLinks->IsChecked() : false; 		const float AngularVel =
        // GetFromEditBox(mValues.mInitialAngularVelocity, mEditBox_InitialAngularVel, MIN_FLOAT, MAX_FLOAT);
        ...
        if (UseRCArticulations && !caps.mSupportRCArticulations) return false;

        if (UseDistanceConstraints && !caps.mSupportDistanceJoints) return false;
        ...
        PintArticHandle RCArticulation =
                UseRCArticulations ? pint.CreateRCArticulation(PINT_RC_ARTICULATION_CREATE(true)) : null;
...

        {
            ...
            if (Articulation) {
                ...
                //				Handles[0] = pint.CreateArticulatedObject(articulation, null, *this,
                // ObjectDesc);
        ...
        for (udword i = 1; i < NbCapsules - 1; i++) {
            Positions[i] = Pos;
            {
                ...
                ObjectDesc.mCollisionGroup = 1 + GroupBit;
                GroupBit = 1 - GroupBit;
                //				Handles[i] = CreateArticulationLink(articulation,
                //(PxArticulationLink*)Handles[i-1], *this, ObjectDesc);
                if (Articulation) {
                    PINT_ARTICULATED_BODY_CREATE ArticulatedDesc;
                    ...
                    PintJointHandle JointHandle =
                            pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[i - 1], Handles[i], Offset, -Offset));
                    ASSERT(JointHandle);
                }
                if (0) {
            ...
            //			Handles[i] = CreateDynamicObject(pint, &SphereDesc, Pos);
            {
                ...
                if (Articulation) {
                    ...
                    /*				PxArticulationJoint* joint =
                       ((PxArticulationLink*)Handles[i])->getInboundJoint(); if(joint)
                                                    {
                                                            joint->setParentPose(PxTransform(ToPxVec3(Extents +
                       PosOffset))); joint->setChildPose(PxTransform(PxVec3(-BoxExtents.x, 0.0f, 0.0f)));
                                                            setupJoint(joint);
                                                    }*/
                } else if (RCArticulation) {
                    PINT_RC_ARTICULATED_BODY_CREATE ArticulatedDesc;
                    ...
                    PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(
                            Handles[i - 1], Handles[i], Extents + PosOffset, Point(-BoxExtents.x, 0.0f, 0.0f)));
                    ASSERT(JointHandle);
                }
        ...
        if (0) {
            ...
            ObjectDesc.mPosition =
                    Point(0.0f * Scale, InitY - float(NbCapsules) * (Radius + HalfHeight) * 2.0f - BoxExtents.y,
                          -15.0f * Scale);
...
static const char* gDesc_ArticulatedChain_Regular =
        "Articulated chain. The chain is made of 30 capsules. Each capsule has a mass of 1. \
...
static const char* gDesc_ArticulatedChain_MCA =
        "Same setup as ArticulatedChain_RegularJoints, this time using a MC articulation (as initially included in "
        "PEEL 1.1).";
...
static void AttachLink(PintActorHandle h, Pint& pint, const Point& pos, float radius, float half_height) {
    ...
    const PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(
            CreatePintObject(pint, ObjectDesc), h, Point(0.0f, 0.0f, 0.0f), Point(0.0f, -half_height - radius, 0.0f)));
    ASSERT(JointHandle);
}

static void CreateChain(Pint& pint,
                        bool use_articulations,
                        bool use_overlapping_links,
                        bool use_distance_constraints,
                        const Point& p0,
                        const Point& p1,
                        udword nb_links,
                        float radius,
                        float mass,
                        float mass_for_inertia,
                        PintActorHandle heavy_object,
                        const Point& local_pivot0) {
    ...
    float HalfHeight;
    {
        ...
        CreateCapsuleRope2(pint, HalfHeight, Articulation, null, p0, p1, nb_links, radius, mass, mass_for_inertia,
                           use_overlapping_links, use_distance_constraints, Handles);
        if (Articulation) pint.AddArticulationToScene(Articulation);
    }
    AttachLink(Handles[0], pint, p0, radius, HalfHeight);

    PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(
            heavy_object, Handles[nb_links - 1], local_pivot0, Point(0.0f, HalfHeight + radius, 0.0f)));
    ASSERT(JointHandle);
}
...
public:
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (UseArticulations && !caps.mSupportMCArticulations) return false;
        if (UseDistanceConstraints && !caps.mSupportDistanceJoints) return false;
        ...
        CreateChain(pint, b0, b1, b2, Point(x - ex, y + h, z - ez), Point(x - ex, y, z - ez), NbLinks, Radius,
                    CapsuleMass, CapsuleMassForInertia, HeavyObject, Point(-ex, BoxDesc.mExtents.y, -ez));
        CreateChain(pint, b0, b1, b2, Point(x + ex, y + h, z - ez), Point(x + ex, y, z - ez), NbLinks, Radius,
                    CapsuleMass, CapsuleMassForInertia, HeavyObject, Point(ex, BoxDesc.mExtents.y, -ez));
        CreateChain(pint, b0, b1, b2, Point(x - ex, y + h, z + ez), Point(x - ex, y, z + ez), NbLinks, Radius,
                    CapsuleMass, CapsuleMassForInertia, HeavyObject, Point(-ex, BoxDesc.mExtents.y, ez));
...
static const char* gDesc_SwingSet_Regular =
        "A test inspired by a swing set. We all did that on the playground as kids!... This is not \
...
static const char* gDesc_SwingSet_MCA =
        "Same setup as SwingSet_RegularJoints, this time using a MC articulation (as initially included in PEEL 1.1).";
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (1) {
            ...
            if (0) {
                ...
                for (udword i = 0; i < NbLinks; i++) CollisionGroups[i] = 1;
        ...
        CreateCapsuleRope2(pint, HalfHeight, Articulation, null, TopPos, BasePos, NbLinks, Radius, CapsuleMass,
                           CapsuleMassForInertia, false, UseDistanceConstraints, Handles, CollisionGroups);
        ...
        if (1) {
            ...
            PintActorHandle Root[4];
            {
                ...
                CreateCapsuleRope2(pint, HalfHeight, Articulation, Parent, RootPos[0], Point(x - ex, y, z - ez),
                                   NbLinks, Radius, CapsuleMass, CapsuleMassForInertia, false, UseDistanceConstraints,
                                   Handles, CollisionGroups);
                Root[0] = Handles[0];

                PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(
                        HeavyObject, Handles[NbLinks - 1], Point(-ex * Offset, BoxDesc.mExtents.y, -ez * Offset),
                        Point(0.0f, HalfHeight + Radius, 0.0f)));
                ASSERT(JointHandle);
            }

            {
                RootPos[1] = Point(x + ex2, y + h, z - ez2);
                CreateCapsuleRope2(pint, HalfHeight, Articulation, Parent, RootPos[1], Point(x + ex, y, z - ez),
                                   NbLinks, Radius, CapsuleMass, CapsuleMassForInertia, false, UseDistanceConstraints,
                                   Handles, CollisionGroups);
                Root[1] = Handles[0];
...

            {
                ...
                CreateCapsuleRope2(pint, HalfHeight, Articulation, Parent, RootPos[2], Point(x - ex, y, z + ez),
                                   NbLinks, Radius, CapsuleMass, CapsuleMassForInertia, false, UseDistanceConstraints,
                                   Handles, CollisionGroups);
                Root[2] = Handles[0];
...

            {
                ...
                CreateCapsuleRope2(pint, HalfHeight, Articulation, Parent, RootPos[3], Point(x + ex, y, z + ez),
                                   NbLinks, Radius, CapsuleMass, CapsuleMassForInertia, false, UseDistanceConstraints,
                                   Handles, CollisionGroups);
                Root[3] = Handles[0];
...
static const char* gDesc_Crane_Regular =
        "Crane test. This variation uses regular joints and the TGS solver in PhysX 4.0, \
but it remains challenging without articulations. Running it side-by-side with PhysX 3.x shows clear improvements though.";
...
static const char* gDesc_Crane_MCA =
        "Same setup as Crane_RegularJoints, this time using a MC articulation (as initially included in PEEL 1.1).";
...
static const char* gDesc_ConnectedMCArticulations =
        "The PhysX MC articulations are limited to 64 links per articulation. This test shows how to \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportMCArticulations || !caps.mSupportRigidBodySimulation) return false;
    ...
    for (udword i = 0; i < NbArticulations; i++) {
        ...
        if (!Articulations[i]) return false;
    }
        ...
        for (udword i = 0; i < NbCapsules; i++) {
            {
                ...
                PINT_ARTICULATED_BODY_CREATE ArticulatedDesc;
                if (i) ArticulatedDesc.mParent = H[i - 1];
                ArticulatedDesc.mLocalPivot0 = Extents + PosOffset;
                ArticulatedDesc.mLocalPivot1 = -Extents - PosOffset;
    ...
    for (udword j = 0; j < NbArticulations - 1; j++) {
        ...
        const Point Offset = Extents + PosOffset;
        const PintJointHandle JointHandle =
                pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(H[NbCapsules - 1], H[NbCapsules], Offset, -Offset));
        ASSERT(JointHandle);
    ...
    if (AttachToWorld) {
        ...
        if (0) {
            ...
            const PintJointHandle JointHandle = pint.CreateJoint(
                    PINT_SPHERICAL_JOINT_CREATE(h, Handles[0], Point(0.0f, 0.0f, 0.0f), Point(0.0f, 0.0f, 0.0f)));
            ASSERT(JointHandle);
        }
    }
    return true;
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("Winch", 20); }
        ...
        for (udword i = 0; i < NbCapsules; i++) {
            {
                ...
                if (i || j) {
                    ...
                    ObjectDesc.mPosition = Pos;
                    //						ObjectDesc.mRotation	= m2;
                    //						ObjectDesc.mAngularVelocity	= Point(0.0f, 0.0f,
                    // 100.0f);
                }
                ...
                ArticulatedDesc.mLocalPivot0 = Extents + PosOffset;
                //					else
                //					ArticulatedDesc.mLocalPivot0 = (Dir * CapsuleDesc2.mRadius) +
                // PosOffset;
                ArticulatedDesc.mLocalPivot1 = -Extents - PosOffset;
                H[i] = pint.CreateArticulatedObject(ObjectDesc, ArticulatedDesc, Articulation);
                P[i] = Pos;
...
virtual udword Update(Pint& pint, float dt) {
    ...
    if (h) pint.SetAngularVelocity(h, Point(0.0f, 0.0f, 8.0f));
...
static const char* gDesc_ArticulationDriveTest =
        "Articulation drive test. This one is mainly to get a feeling for the MC articulation drive \
...
struct WheelTestData {
    WheelTestData() : mDriveType(DRIVE_VELOCITY) {}
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) {
...

    {
        ...
        mData.mEditBox_TargetVelocity = helper.CreateEditBox(UI, ID++, 4 + OffsetX, y, EditBoxWidth, 20, "2", &UIElems,
                                                             EDITBOX_FLOAT, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Target position:", &UIElems);
        mData.mEditBox_TargetPosition = helper.CreateEditBox(UI, ID++, 4 + OffsetX, y, EditBoxWidth, 20, "0", &UIElems,
                                                             EDITBOX_FLOAT, null, null);
        mData.mEditBox_TargetPosition->SetEnabled(false);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Stiffness:", &UIElems);
        mData.mEditBox_Stiffness = helper.CreateEditBox(UI, ID++, 4 + OffsetX, y, EditBoxWidth, 20, "0", &UIElems,
                                                        EDITBOX_FLOAT, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Damping:", &UIElems);
        mData.mEditBox_Damping = helper.CreateEditBox(UI, ID++, 4 + OffsetX, y, EditBoxWidth, 20, "1000", &UIElems,
                                                      EDITBOX_FLOAT, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Internal compliance:", &UIElems);
        mData.mEditBox_InternalCompliance = helper.CreateEditBox(UI, ID++, 4 + OffsetX, y, EditBoxWidth, 20, "1",
                                                                 &UIElems, EDITBOX_FLOAT, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "External compliance:", &UIElems);
        mData.mEditBox_ExternalCompliance = helper.CreateEditBox(UI, ID++, 4 + OffsetX, y, EditBoxWidth, 20, "1",
                                                                 &UIElems, EDITBOX_FLOAT, null, null);
        y += YStep;
    }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    if (0) {
        PintJointHandle j1 = pint.CreateJoint(
                PINT_SPHERICAL_JOINT_CREATE(null, ArticulatedObjectRoot, ArticulationPos, Point(0.0f, 0.0f, 0.0f)));
    }
    if (0) {
    ...
    PintActorHandle Parent = ArticulatedObjectRoot;
    {
        ...
        //			ArticulatedDesc.mMotor.mTargetVelocity = Point(TargetVelocity, TargetVelocity,
        // TargetVelocity);
...
static const char* gDesc_ArticulationDriveVsObstacle =
        "Articulation drive blocked by a static obstacle. This is a case where speculative contacts (angular CCD) or just an increased \
...
static const char* gDesc_ArticulatedCaterpillarTrack =
        "Articulated caterpillar track. This test demonstrates how to overcome two \
...
static const char* gDesc_ArticulatedVehicle =
        "A basic articulated vehicle with large mass ratios between the jointed parts. \
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 0;
        {
            mCheckBox_Articulations =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use articulations", &UIElems, true, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Constraint multiplier:", &UIElems);
            mEditBox_Multiplier = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1", &UIElems,
                                                       EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Ground friction:", &UIElems);
            mEditBox_GroundFriction = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1", &UIElems,
                                                           EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Wheel friction:", &UIElems);
            mEditBox_WheelFriction = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                                                          EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Chassis mass:", &UIElems);
            mEditBox_ChassisMass = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "40.0", &UIElems,
                                                        EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Wheel mass:", &UIElems);
            mEditBox_WheelMass = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "15.0", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Struct mass:", &UIElems);
            mEditBox_StructMass = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Wheel radius:", &UIElems);
            mEditBox_WheelRadius = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
                                                        EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Wheel width:", &UIElems);
            mEditBox_WheelWidth = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.3", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Wheel tessellation:", &UIElems);
            mEditBox_WheelTess = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "60", &UIElems,
                                                      EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Chassis size X:", &UIElems);
            mEditBox_ChassisX = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.5", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Chassis size Y:", &UIElems);
            mEditBox_ChassisY = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Chassis size Z:", &UIElems);
            mEditBox_ChassisZ = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.7", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "COM local offset (Y):", &UIElems);
            mEditBox_COMLocalOffset = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "-1.0", &UIElems,
                                                           EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Axle radius:", &UIElems);
            mEditBox_AxleRadius = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Axle width:", &UIElems);
            mEditBox_AxleWidth = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.1", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (!caps.mSupportRigidBodySimulation || !caps.mSupportMeshes) return false;

        const bool UseArticulations = mCheckBox_Articulations ? mCheckBox_Articulations->IsChecked() : true;
        if (UseArticulations && !caps.mSupportMCArticulations) return false;

        if (!UseArticulations && !caps.mSupportHingeJoints) return false;
        ...
        if (!CreateMeshesFromRegisteredSurfaces(pint, caps, &GroundMaterial)) return false;

        // We just disable collisions between all parts of the car for now. That's because some engines don't
        // automatically disable collisions between jointed objects. We could do that better but the focus of this test
        // is the stability of the vehicle, and we don't really need proper collision filtering setup to evaluate this.
        const PintDisabledGroups DG(1, 1);
        pint.SetDisabledGroups(1, &DG);
        ...
        const PintArticHandle Articulation =
                UseArticulations ? pint.CreateArticulation(PINT_ARTICULATION_CREATE()) : null;
            ...
            /*			if(0)
                                    {
                                            ...
                                            PINT_OBJECT_CREATE ObjectDesc(&BoxDesc);
                                            ObjectDesc.mPosition	= CarPos + Point(0.0f,
               Chassis.y+BoxDesc.mExtents.y, 0.0f); ObjectDesc.mMass		= 4.0f; PintActorHandle h =
               CreatePintObject(pint, ObjectDesc);
        ...
        {
            const Point RearAxleOffset(-Chassis.x, -Chassis.y - Axle.y,
                                       0.0f);  // delta between chassis center and rear axle center.
            const float RearWheelMass =
                    StructMass + WheelMass * 2.0f;  // 1 axle + 2 wheels all put in the same compound object
        ...
        // Front wheels + front axle. The "FrontAxleObject" here will be cut into two smaller parts, but otherwise it is
        // the same as the rear axle (same size & location compared to the chassis).
        Point WheelPt[2];
        ...
        const Point FrontAxle = Point(AxleRadius, AxleRadius,
                                      AxleWidth - Chassis.z);  // ### Object to whom the wheel is directly attached
        ...
        for (udword i = 0; i < 2; i++) {
            ...
            // *  +Chassis.x to put the objects' centers exactly at the front of the chassis (similar to the rear axle,
            // which was -Chassis.x)
            // *  "-Chassis.y - Axle.y" is exactly the same as for the rear axle
            // *  Now things are different for Z, i.e. along the axle. For the rear wheels we only had one compound
            // object for the whole thing
            //    so the Z offset was 0.0f. But for the front wheels we have 2 compounds, created left & right in such a
            //    way that the centers of the object are exactly located on the sides of the chassis (hence +/-
            //    Chassis.z). Note that the compound's position will be the center of the "FrontAxle" box.
            ...
            {
                // So that's the "front axle" box, i.e. the object similar to the rear axle but which got cut in two
                // pieces.
                PINT_BOX_CREATE BoxDesc(FrontAxle);
                BoxDesc.mRenderer = CreateBoxRenderer(BoxDesc.mExtents);

                // And then that one is the extra (new) box that doesn't exist for the rear axle. It goes back along -x
                // towards the rear axle, and we'll attach some kind of sway bar to these guys later.
                ...
                // "-BoxDesc.mExtents.x - BoxDesc2.mExtents.x" aligns it perfectly along x, and as we said, towards the
                // rear axle. "FrontAxle.z*0.5f" = z2 = BoxDesc.mExtents.z so we align it with the front axle but let
                // the objects overlap on that axis.
                BoxDesc2.mLocalPos = Point(-BoxDesc.mExtents.x - BoxDesc2.mExtents.x, 0.0f, -Sign * z2);
                BoxDesc.SetNext(&BoxDesc2);

                // So far these two boxes are in the same rigid body compound so collisions will be disabled between
                // them. This time we cannot put the wheels in the same compound, since they will need to rotate freely
                // around Z, and the front axles + sway bar thing cannot do so. So we'll attach the wheels to these
                // compounds separately. For now we add these compounds to our articulation. We want them to rotate
                // around Y (i.e. to steer!). That's another hinge.

                /*				PINT_CONVEX_CREATE WheelDesc;
                                                WheelDesc.mNbVerts	= TotalNbVerts;
                                                WheelDesc.mVerts	= Verts;
                                                WheelDesc.mRenderer	= CreateConvexRenderer(WheelDesc.mNbVerts,
                   WheelDesc.mVerts); WheelDesc.mLocalPos	= Point(0.0f, 0.0f, Coeff*FrontAxle.z); BoxDesc.mNext =
                   &WheelDesc;*/
                ...
                // We attach these to the chassis, so collisions between the chassis and the compound will be disabled
                // by default in most engines.
                if (Articulation) {
            ...
            if (1) {
                ...
                // We simply attach this to the previous front-axle compound, using an hinge to let the wheel rotate
                // freely.
                if (Articulation) {
                    PINT_ARTICULATED_BODY_CREATE ArticulatedDesc;
                    ...
                    PintActorHandle WheelObject =
                            pint.CreateArticulatedObject(ObjectDesc, ArticulatedDesc, Articulation);
                } else {
