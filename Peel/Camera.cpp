...
static void BuildFrustumPlanes() {
    ...
    if (!gFreezeFrustum) gFrustum.ExtractPlanes(Combo, true, true);
...
Point GetCameraPos() { return gEye; }

Point GetCameraDir() { return gDir; }
...
void MoveCameraForward(float dt) { gEye += gDir * gCameraSpeed * dt; }

void MoveCameraBackward(float dt) { gEye -= gDir * gCameraSpeed * dt; }

void MoveCameraRight(float dt) { gEye -= gViewY * gCameraSpeed * dt; }

void MoveCameraLeft(float dt) { gEye += gViewY * gCameraSpeed * dt; }
...
public:
    ...
    inline_ float magnitudeSquared() const { return x * x + y * y + z * z + w * w; }

    inline_ float dot(const MyQuat& v) const { return x * v.x + y * v.y + z * v.z + w * v.w; }
    ...
    inline_ MyQuat(const float angle, const Point& axis)  // creates a Quat from an Angle axis -- note that if Angle >
                                                          // 360 the resulting rotation is Angle mod 360
    {
    ...
    inline_ void rotate(
            Point& v) const  // rotates passed vec by rot expressed by quaternion.  overwrites arg ith the result.
    {
...
void CaptureFrustum() { BuildFrustumPts(gCapturedFrustum); }
...
const Plane* GetFrustumPlanes() {
    if (gInvalidPlanes) BuildFrustumPlanes();
...
static void lookAt() {
    ...
    if (gMirror) {
        ...
        float matGL[16] = {r0.x, r1.x, r2.x, 0.0f, r0.y, r1.y, r2.y, 0.0f, r0.z, r1.z, r2.z, 0.0f, t.x, t.y, t.z, 1.0f};
...
#define NEW_VERSION
#ifdef NEW_VERSION
// Fetched from the old ICE renderer. More accurate and MUCH faster than the previous crazy gluUnProject-based
// version...
...
Point ComputeWorldRay(int xs, int ys) {
    ...
    if (!ret) printf("gluUnProject failed\n");
    Point tmp(float(wx1 - wx0), float(wy1 - wy0), float(wz1 - wz0));
    tmp.Normalize();

    if (tmp.Dot(gDir) < 0.0f) tmp = -tmp;
