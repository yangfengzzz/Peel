...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportConvexes || !caps.mSupportMeshes) return false;
...
static bool MakeVoronoi(Picture& pic, udword nb_seeds, udword flags = PIXEL_FULL) {
    ...
    sdword* Seeds = (sdword*)ICE_ALLOC_TMP(sizeof(sdword) * nb_seeds * 2);
    CHECKALLOC(Seeds);
    RGBAPixel* Colors = ICE_NEW_TMP(RGBAPixel[nb_seeds]);
    CHECKALLOC(Colors);
