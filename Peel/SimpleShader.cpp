...
SimpleShader::SimpleShader(bool needs_vertex_normals) : mNeedsVertexNormals(needs_vertex_normals) {
    for (int i = 0; i < 16; i++) {
...
void SimpleShader::Activate(const ShaderProps& mat) {
    ...
    PxVec3 eyeDir = camTrans.rotate((const PxVec3&)Mtl.mBackLightDir);
    eyeDir.normalize();
