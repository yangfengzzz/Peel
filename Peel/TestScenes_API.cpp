...
static const char* gDesc_VisibilityAndSQFlag =
        "This test checks that support for renderable & scene-query flags has been properly implemented by each engine. If the test works, \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportRigidBodySimulation || !caps.mSupportRaycasts || !caps.mSupportSphereSweeps ||
        !caps.mSupportSphereOverlaps)
        return false;
...
static const char* gDesc_CollisionGroups =
        "Simple filtering test, replicating the old collision groups from the NovodeX SDK. If the test works, \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportCollisionGroups || !caps.mSupportRigidBodySimulation) return false;
...
static const char* gDesc_COMLocalOffset =
        "Tests that the center-of-mass (COM) local offset is properly taken into account. The box should rotate on its own if \
...
static const char* gDesc_AngularVelocity =
        "Tests that the SetAngularVelocity and GetAngularVelocity functions work as expected. Set the angular damping to zero \
...
static const char* gDesc_AddLocalTorque =
        "Tests that the AddLocalTorque function works as expected. Set the angular damping to zero \
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("AddLocalTorque", 20); }
...
public:
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (!caps.mSupportRigidBodySimulation || !caps.mSupportContactNotifications) return false;
    ...
    public:
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            if (!caps.mSupportRigidBodySimulation || !caps.mSupportContactModifications) return false;
...
#include "TestScenes_Heightfield.h"
        ...
        public:
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                if (!HeightfieldTest::Init(pint, caps, Point(0.0f, 0.0f, 0.0f))) return false;
