...
static const char* gDesc_DominosConfigurable =
        "Dominos. Things to look for are whether the dominoes drift or not after they have all been knocked down. \
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 0;
        {
            ...
            mEditBox_Friction = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
            ...
            mEditBox_CircleRadius = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "10.0", &UIElems,
                                                         EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_CircleRadius->SetEnabled(false);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Nb dominos per circle:", &UIElems);
            mEditBox_NbPerCircle = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "128", &UIElems,
            ...
            mEditBox_ShrinkFactor = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
            ...
            mEditBox_NbRounds = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1", &UIElems,
                                                     EDITBOX_INTEGER_POSITIVE, null, null);
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        for (udword j = 0; j < NbRounds; j++) {
            for (udword i = 0; i < Nb; i++) {
                ...
                if (0) Articulation = pint.CreateArticulation(PINT_ARTICULATION_CREATE());

                PintArticHandle RCArticulation = null;
                if (0) RCArticulation = pint.CreateRCArticulation(PINT_RC_ARTICULATION_CREATE());
                ...
                if (1) {
                    if (ApplyInitialImpulse && i == 0) {
                        if (PushLine || j == 0) {
                            //						pint.AddWorldImpulseAtWorldPos(h, Point(0.0f,
                            //0.0f, 4.0f), ObjectDesc.mPosition + Point(0.0f, Extents.y, 0.0f));
                ...
                if (0) {
                    if (ApplyInitialImpulse && i == 0) {
                        if (PushLine || j == 0) {
                            pint.AddWorldImpulseAtWorldPos(h, Point(0.0f, 0.0f, 4.0f),
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("DoubleDominoEffect", 20); }
...
static const char* gDesc_CapsuleStack =
        "The old capsule stack from the NovodeX SDK. Engines using 'adaptive force' tend to be stable here... until "
...
static const char* gDesc_StackedSpheres =
        "Stacked spheres. Just to check if they collapse or not. What is more realistic here? \
...
static const char* gDesc_StackedCapsules =
        "Stacked capsules. Mainly to check that the capsule parameters (height? half-height? from top to bottom, or \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < 10; i++)
        CreateDynamicObject(
                pint, &Create,
                Point(0.0f, Radius + HalfHeight * 2.0f + (Radius + HalfHeight * 2.0f + 1.0f) * float(i), 0.0f));
    return true;
}
...
static const char* gDesc_InitialVelocities =
        "Initial velocities. This test fails with the default max angular velocity value in old PhysX versions, \
...
static const char* gDesc_GyroscopicPrecession =
        "Gyroscopic Precession (a.k.a. spinning tops from the old NovodeX SDK). This test fails \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (sdword i = 0; i < numSegs; i++) hullPoints[i] = Point(r * cosf(i * dphi), h, r * sinf(i * dphi));
...
static const char* gDesc_HighMassRatioCollisions =
        "High mass ratio collisions. A heavy box moves against a light box, and should easily push it away. \
...
static const char* gDesc_HighMassRatio =
        "Basic high mass ratio test. The top box in those box stacks is 100, 200 or 300 times heavier than the other boxes. \
...
static const char* gDesc_InitialPenetration =
        "A test to check how engines deal with objects created in an initially overlapping state. Ideally this should be resolved \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < NbBoxes; i++) {
        ...
        H = CreateDynamicBox(pint, BoxExtent, BoxExtent, BoxExtent,
                             Point(0.0f, BoxPosY + Coeff * BoxExtent * 1.5f, 0.0f));
        //		H = CreateDynamicBox(pint, BoxExtent, BoxExtent, BoxExtent, Point(0.0f, BoxPosY +
        // Coeff*BoxExtent*5.0f, 0.0f));
        //			ASSERT(H);
    }
...
static const char* gDesc_FrictionRamp =
        "Classical friction ramp test. Static friction is 0.0. Dynamic friction coeffs range from 0.0 to 1.0. \
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("FrictionRamp", 20); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < NbBoxes; i++) {
        ...
        PintActorHandle CubeHandle =
                CreateDynamicBox(pint, 1.0f, 1.0f, 1.0f, Point(x, 18.0f, -6.0f), &ObjectDesc.mRotation, &MatDesc);
        ASSERT(CubeHandle);
    }
    return true;
}
...
static const char* gDesc_FrictionRamp2 =
        "A variation on the classical friction ramp test. \
Note that some engines like PhysX support several friction models, and the default one is not the most realistic. Enable the other ones in the PhysX plugin UI.";

START_TEST(FrictionRamp2, CATEGORY_BEHAVIOR, gDesc_FrictionRamp2)

virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("FrictionRamp2", 20); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < NbBoxes; i++) {
        ...
        for (udword j = 0; j < 4; j++) {
            PintActorHandle BoxHandle = CreateDynamicBox(
                    pint, 4.0f, 0.5f, 4.0f, Point(x, 18.0f + float(j) * 2.0f, -6.0f), &ObjectDesc.mRotation, &MatDesc);
            ASSERT(BoxHandle);
        }
    }
    return true;
...
static const char* gDesc_FrictionRamp3 =
        "A variation on the classical friction ramp test. Each box has an initial angular velocity of 10. \
Note that some engines like PhysX support several friction models, and the default one is not the most realistic. Enable the other ones in the PhysX plugin UI.";

START_TEST(FrictionRamp3, CATEGORY_BEHAVIOR, gDesc_FrictionRamp3)

virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("FrictionRamp3", 20); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < NbBoxes; i++) {
        ...
        //		PintActorHandle BoxHandle = CreateDynamicBox(pint, 4.0f, 0.5f, 4.0f, Point(x, 13.5f, -4.0f),
        //&ObjectDesc.mRotation, &MatDesc); 		ASSERT(BoxHandle);
        {
...
static const char* gDesc_Restitution =
        "Classical restitution test. Coeffs of restitution range from 0.0 to 1.0. Ideally when restitution is 1.0, the falling object should \
...
static const char* gDesc_GravityRace =
        "A dynamic box is dropped 1000 units away from the origin, and falls towards it. In theory boxes simulated with different engines \
...
static const char* gDesc_ConvexCompoundsChain =
        "A chain made of 20 torus object. Each torus is a compound of 32 convexes. This test shows the artefacts caused \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportConvexes || !caps.mSupportRigidBodySimulation || !caps.mSupportCompounds) return false;
    ...
    for (udword j = 0; j < NbSlices; j++) {
        ...
        for (udword i = 0; i < NbPtsSmallCircle; i++) Verts[Index++] = Trans + Pts[i] * Rot;
    }
    ...
    for (udword s = 0; s < NbSlices; s++) {
        ...
        if (1)  // Recenter vertices
        {
            ...
            for (udword i = 0; i < NbPtsSmallCircle * 2; i++) Center += ConvexPts[Offset + i] * Coeff;

            for (udword i = 0; i < NbPtsSmallCircle * 2; i++) ConvexPts[Offset + i] -= Center;
        ...
        if (s != NbSlices - 1) ConvexCreate[s].SetNext(&ConvexCreate[s + 1]);
...
static const char* gDesc_BoxCompoundsChain =
        "A chain made of 20 torus object. Each torus is a compound of 32 boxes. This is a variation on previous test.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportRigidBodySimulation || !caps.mSupportCompounds) return false;
    ...
    for (udword s = 0; s < NbSlices; s++) {
        ...
        if (s != NbSlices - 1) BoxCreate[s].SetNext(&BoxCreate[s + 1]);
        Offset += NbPtsSmallCircle * 2;
    }
    DELETEARRAY(Verts);
...
static const char* gDesc_CompoundTowerTweaked =
        "Compound tower. Can you tweak the settings so that the tower doesn't collapse?";

START_TEST(CompoundTowerTweaked, CATEGORY_BEHAVIOR, gDesc_CompoundTowerTweaked)

virtual float GetRenderData(Point& center) const { return 400.0f; }

virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("CompoundTowerTweaked", 20); }
...
static const char* gDesc_BoxOnRotatingPlanarMesh =
        "Unity bug report: a (potentially sleeping) box on a rotating planar mesh. Although the mesh is static, its rotation is \
...
virtual void OnObjectReleased(Pint& pint, PintActorHandle removed_object) {
    if (pint.mUserData == removed_object) pint.mUserData = null;
}
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("CardHouse", 20); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    Matrix3x3 Rot0;
    Rot0.RotX(25.0f * DEGTORAD);
    Matrix3x3 Rot1;
    Rot1.RotX(-25.0f * DEGTORAD);
    Matrix3x3 Rot2;
    Rot2.RotX(90.0f * DEGTORAD);
...
static const char* gDesc_Dzhanibekov =
        "Dzhanibekov effect. In the PhysX family only PhysX5 handles it correctly, after enabling gyroscopic forces.";

START_TEST(Dzhanibekov, CATEGORY_BEHAVIOR, gDesc_Dzhanibekov)

virtual float GetRenderData(Point& center) const { return 20.0f; }

virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("Dzhanibekov", 20); }
