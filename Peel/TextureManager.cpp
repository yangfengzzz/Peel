...
ManagedTexture::ManagedTexture(udword width, udword height, const RGBAPixel* pixels, const char* filename, udword index)
    : mWidth(width), mHeight(height), mGLID(0), mID(index), mExportID(INVALID_ID) {
    if (filename) mFilename.Set(filename);
...
udword GetNbManagedTextures() { return gManagedTextures ? gManagedTextures->GetNbEntries() : 0; }
...
const ManagedTexture* CreateManagedTexture(udword width, udword height, const RGBAPixel* pixels, const char* filename) {
    if (!gManagedTextures) gManagedTextures = ICE_NEW(PtrContainer);
...
bool ReleaseManagedTexture(const ManagedTexture* texture) {
    if (!texture || !gManagedTextures) return false;
    const udword Nb = GetNbManagedTextures();
    if (texture->mID >= Nb) return false;
...
udword GetManagedTexturesTimestamp() { return gMT_Timestamp; }
...
SystemTexture* CreateSystemTexture(udword width, udword height, GLuint id, const char* name) {
    ...
    if (!gSystemTextures) gSystemTextures = ICE_NEW(PtrContainer);
...
udword GetNbSystemTextures() { return gSystemTextures ? gSystemTextures->GetNbEntries() : 0; }

const SystemTexture* GetSystemTexture(udword index) {
    if (index < GetNbSystemTextures()) return reinterpret_cast<const SystemTexture*>(gSystemTextures->GetEntry(index));
    return null;
}
...
udword GetSystemTexturesTimestamp() { return gST_Timestamp; }
