...
void ToolRotate::Deselect() { Reset(INVALID_ID); }
...
void ToolRotate::RightDragCallback(Pint& pint, udword pint_index) {
    ...
    if (RD.mObject) {
        ...
        Quat q;
        VirtualTrackBall(q, GetCameraPos(), Pose.mPos, mdir_old, mdir_new, 1.0f /*WorldSphere.mRadius*/);
        q *= Pose.mRot;

        Pose.mRot = q;
#ifdef SUPPORT_ROTATE_TOOL_UI
        if (mEditPos) mEditPos->SetEulerAngles(q);
#endif
...
void ToolRotate::RightUpCallback(Pint& pint, udword pint_index) {
    ...
    if (RD.mObject) RD.mTransformer.Stop(pint, RD.mObject);
...
void ToolRotate::RenderCallback(PintRender& render, Pint& pint, udword pint_index) {
    ASSERT(pint_index < MAX_NB_ENGINES)
    RotateData& RD = mData[pint_index];
    if (RD.mObject) DrawBoxCorners(pint, RD.mObject, TOOLS_BOX_CORNERS_COLOR);

#ifdef SUPPORT_ROTATE_TOOL_UI
    if (mEditPos) mEditPos->SetEnabled(RD.mObject != null);
