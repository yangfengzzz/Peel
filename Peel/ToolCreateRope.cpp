...
ToolCreateRope::ToolCreateRope() : mPendingAction(NONE) {}
...
void ToolCreateRope::RenderCallback(PintRender& render, Pint& pint, udword pint_index) {
    ...
    if (mPendingAction == CREATE_ROPE) CreateRope(pint);
}

void ToolCreateRope::PostRenderCallback() { mPendingAction = NONE; }
    ...
    udword GroupBit = 0;
    {
        ...
        for (udword i = 0; i < NbSpheres - 1; i++) {
            PintJointHandle JointHandle =
                    pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[i], Handles[i + 1], LocalPivot0, LocalPivot1));
            ASSERT(JointHandle);
        }
