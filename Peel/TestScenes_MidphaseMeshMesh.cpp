...
struct MeshMeshHits : Allocateable {
    MeshMeshHits(PintActorHandle mesh_handle0, PintActorHandle mesh_handle1)
        : mMeshHandle0(mesh_handle0), mMeshHandle1(mesh_handle1) {}
...
static const char* gDesc_MeshMesh =
        "MESH-MESH MIDPHASE: test collision detection between arbitrary triangle meshes. Usually the domain of "
        "dedicated collision libraries like Opcode. In the PhysX family only PhysX5 supports it.";
...
public:
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (!caps.mSupportMeshMeshOverlaps) return false;
    ...
    virtual udword Update(Pint& pint, float dt) {
        ...
        return pint.FindTriangles_MeshMeshOverlap(pint.mSQHelper->GetThreadContext(), MeshHandle0, MeshHandle1,
                                                  MMH->mResults);
    }
