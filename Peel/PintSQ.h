...
class Hits {
public:
    Hits() : mNbHits(0), mMaxNbHits(0), mHits(null) {}
    ~Hits() { ASSERT(!mHits); }
...
private:
    ...
    void DrawSphereSweepData(udword nb,
                             const PintSphereSweepData* data,
                             const PintRaycastHit* hits,
                             const Point& color);
    void DrawCapsuleSweepData(udword nb,
                              const PintCapsuleSweepData* data,
                              const PintRaycastHit* hits,
                              const Point& color);
    void DrawConvexSweepData(udword nb,
                             const PintConvexSweepData* data,
                             const PintRaycastHit* hits,
                             const Point& color);

    void DrawSphereOverlapAnyData(udword nb,
                                  const PintSphereOverlapData* data,
                                  const PintBooleanHit* hits,
                                  const Point& color);
    void DrawSphereOverlapObjectsData(udword nb,
                                      const PintSphereOverlapData* data,
                                      const PintMultipleHits* hits,
                                      const Point& color);
    void DrawBoxOverlapAnyData(udword nb,
                               const PintBoxOverlapData* data,
                               const PintBooleanHit* hits,
                               const Point& color);
    void DrawBoxOverlapObjectsData(udword nb,
                                   const PintBoxOverlapData* data,
                                   const PintMultipleHits* hits,
                                   const Point& color);
    void DrawCapsuleOverlapAnyData(udword nb,
                                   const PintCapsuleOverlapData* data,
                                   const PintBooleanHit* hits,
                                   const Point& color);
    void DrawCapsuleOverlapObjectsData(udword nb,
                                       const PintCapsuleOverlapData* data,
                                       const PintMultipleHits* hits,
                                       const Point& color);
    void DrawConvexOverlapAnyData(udword nb,
                                  const PintConvexOverlapData* data,
                                  const PintBooleanHit* hits,
                                  const Point& color);
    void DrawConvexOverlapObjectsData(udword nb,
                                      const PintConvexOverlapData* data,
                                      const PintMultipleHits* hits,
                                      const Point& color);
};
