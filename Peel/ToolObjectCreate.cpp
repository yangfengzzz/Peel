...
ToolCreateObject::ToolCreateObject()
    : mDir(0.0f, 0.0f, 0.0f), mOrigin(0.0f, 0.0f, 0.0f), mImpact(0.0f, 0.0f, 0.0f), mX(0), mY(0) {}
...
void ToolCreateObject::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
    ...
    helper.CreateEditBox(parent, 0, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gBoxSize), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::gEBCallback, gTooltip_BoxSize);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 100, 20, "Box mass:", &owner);
    helper.CreateEditBox(parent, 1, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gBoxMass), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::gEBCallback, gTooltip_BoxMass);
    y += YStep;

    //	helper.CreateLabel(parent, 0, y+LabelOffsetY, 100, 20, "Box velocity:", &owner);
    //	helper.CreateEditBox(parent, 2, OffsetX, y, EditBoxWidth, 20, helper.Convert(gBoxVelocity), &owner,
    //EDITBOX_FLOAT_POSITIVE, Local::gEBCallback, gTooltip_BoxVelocity); 	y += YStep;
}
