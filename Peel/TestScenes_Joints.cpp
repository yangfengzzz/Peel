...
public:
    ...
    /*	virtual	IceTabControl*	InitUI(PintGUIHelper& helper)
            {
                    ...
                    sdword y = 0;
                    {
                            ...
                            class MyComboBox : public IceComboBox
                            {
                                    GearJoint&		mTest;
                                    public:
                                                                    MyComboBox(const ComboBoxDesc& desc, GearJoint&
    test) : IceComboBox(desc), mTest(test)	{} virtual			~MyComboBox()
    {}
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (!caps.mSupportGearJoints) return false;
        ...
        PintActorHandle SupportObject;
        {
            ...
            //			SupportObject = CreateStaticObject(pint, &BoxDesc, Point(0.0f, Extents.y+Altitude,
            // 0.0f));
        }
        ...
        PintActorHandle Gear0 = null;
        {
            ...
            BoxDesc2.mLocalPos =
                    Point(mGearRadius0 - BoxDesc2.mExtents.x - 0.05f, mGearThickness + BoxDesc2.mExtents.y, 0.0f);
            Current->SetNext(&BoxDesc2);

            Gear0 = CreateDynamicObject(pint, &ConvexDesc,
                                        Point(-mGearRadius0, Altitude + mGearThickness * 3.0f, 0.0f));
            //			Gear0 = CreateStaticObject(pint, &ConvexDesc, Point(-mGearRadius0,
            //Altitude+mGearThickness*3.0f, 0.0f));
            mGearObject0 = Gear0;
        }
        ...
        if (1) {
            ...
            //			BoxDesc.mRenderer	= CreateBoxRenderer(Extents);
                                    ...
                                    BoxDesc2.mRenderer	= CreateBoxRenderer(BoxDesc2.mExtents);
                                    BoxDesc2.mLocalPos	= Point(-GearRadius+BoxDesc2.mExtents.x+0.05f,
            GearThickness+BoxDesc2.mExtents.y, 0.0f); BoxDesc.mNext = &BoxDesc2;*/
            ...
            BoxDesc2.mRenderer = CreateBoxRenderer(BoxDesc2.mExtents);
            BoxDesc2.mLocalPos =
                    Point(-mGearRadius1 + BoxDesc2.mExtents.x + 0.05f, mGearThickness + BoxDesc2.mExtents.y, 0.0f);
            Current->SetNext(&BoxDesc2);
    ...
    virtual float DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale) {
        ...
        if (mHingeJoint0) {
            float TwistAngle;
            if (!GetHingeTwistAngle(pint, mHingeJoint0, TwistAngle)) return y;
        ...
        if (mHingeJoint1) {
            float TwistAngle;
            if (!GetHingeTwistAngle(pint, mHingeJoint1, TwistAngle)) return y;
        ...
        if (mGearRatio > 1.0f) {
            while (Angle0 < 0.0f) Angle0 += TWOPI;
            while (Angle1 < Angle0) Angle1 += TWOPI;
        } else if (mGearRatio < 1.0f) {
            while (Angle1 < 0.0f) Angle1 += TWOPI;
            while (Angle0 < Angle1) Angle0 += TWOPI;
        }
        //		Angle0 = fmodf(Angle0, TWOPI);
        //		Angle1 = fmodf(Angle1, TWOPI);
...
#include "ZCB/Pint_ZCB2.h"
    ...
    public:
        TwoLegoGears() : mHingeJoint0(null), mHingeJoint1(null) {}
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            ...
            if (!mZB2Factory) return false;

            const udword SelectedIndex = mComboBox_Preset->GetSelectedIndex();
            if (SelectedIndex == 0) return CreateZB2Scene(pint, caps);

            if (SelectedIndex == 1) {
                if (!caps.mSupportConvexes) return false;
                ...
                // Clumsy test to rotate the scene and transform the gear axis from Z to X (to test the joint shader
                // works with arbitrary axes)
                if (UseGearAxisX) {
                    Matrix4x4 SceneRot;
                ...
                struct Local {
                    static PintJointHandle CreateHinge(Pint& pint,
                                                       PintActorHandle gear,
                                                       PintActorHandle support,
                                                       const Point& pos,
                                                       const Point& axis,
                                                       bool limits) {
                        if (!gear) return null;
                        ...
                        if (0) {
                            ...
                            if (0) {
                                ...
                                /*						{
                                                                                        const PR pose1 =
                                   pint.GetWorldTransform(gear); const Matrix3x3 M1 = pose1.mRot; const Point&
                                   CurrentWorldAxisX1 = M1[2]; Matrix3x3 FromTo; FromTo.FromTo(CurrentWorldAxisX1,
                                   axis); Desc.mLocalPivot1.mRot	= FromTo;
                                                                                }*/
                                ...
                                if (support)
                                    ...
                                    Desc.mLocalPivot0.mPos = pos;  // pos = pose0.pos
                                                                   //							Desc.mLocalPivot0.mRot
                                                                   //= FromTo;
                                    /*							{
                                                                                                    const Point
                                       CurrentWorldAxisX0(1.0f, 0.0f, 0.0f);                                Matrix3x3
                                       FromTo; FromTo.FromTo(CurrentWorldAxisX0, axis); Desc.mLocalPivot0.mRot	=
                                       FromTo;
                                                                                            }*/
                                    ...
                                    /*							const Matrix3x3 M0 = pose0.mRot;
                                                                                            const Point&
                                       CurrentWorldAxisX0 = M0[0]; Matrix3x3 FromTo; FromTo.FromTo(CurrentWorldAxisX0,
                                       Point(0.0f, 0.0f, 1.0f)); Desc.mLocalPivot0.mRot	= FromTo;*/

                                    Desc.mLocalPivot0.mRot = pose0.mRot * FromTo;
                                }

                                if (limits) Desc.mLimits.Set(-HALFPI / 2.0f, HALFPI / 2.0f);
        ...
        static void PreprocessMesh(Point& center, Point& extents, const PINT_OBJECT_CREATE* create) {
            ...
            if (1) {
                ...
                for (udword i = 0; i < NbVerts; i++) V[i] -= center;
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            if (!caps.mSupportRackJoints) return false;

            if (0) return CreateZB2Scene(pint, caps);
            ...
            if (1) {
                ...
                if (1 && Rack) {
                    ...
                    struct Local {
                        static PintJointHandle CreatePrismatic(Pint& pint,
                                                               PintActorHandle rack,
                                                               PintActorHandle support,
                                                               const Point& pos) {
                            if (!rack) return null;
        ...
        static PintActorHandle CreateMeshAsBox(Pint& pint,
                                               const PINT_OBJECT_CREATE* create,
                                               const Point& center,
                                               const Point& extents,
                                               const Matrix3x3& rot,
                                               float mass,
        ...
        virtual bool CommonSetup() { return TestBase::CommonSetup(); }
        ...
        virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
            ...
            if (Filename) {
                ...
                if (mZB2Factory) {
                    ...
                    for (udword i = 0; i < 3; i++) PreprocessMesh(mCenter[i], mExtents[i], Actors[i].mCreate);
                }
            }
        }

        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            if (!caps.mSupportHingeJoints || !caps.mSupportCollisionGroups) return false;
            ...
            Matrix3x3 RotY;
            RotY.RotY(-HALFPI / 2.0f);
            Matrix3x3 RotX;
            RotX.RotX(HALFPI / 2.0f);

            const float Mass = 1.0f;
            const PintActorHandle Part0 =
                    CreateMeshAsBox(pint, Create0, mCenter[0], mExtents[0], Matrix3x3(Idt) /*SceneRot*/, Mass, "Part0");
            const PintActorHandle Part1 =
                    CreateMeshAsBox(pint, Create1, mCenter[1], mExtents[1], RotX /*SceneRot*/, Mass, "Part1");
            const PintActorHandle Part2 =
                    CreateMeshAsBox(pint, Create2, mCenter[2], mExtents[2], RotY /*SceneRot*/, Mass, "Part2");
        ...
        public:
            ...
            virtual IceTabControl* InitUI(PintGUIHelper& helper) {
                ...
                sdword y = 0;
                {
                    mCheckBox_EnableMotor =
                            helper.CreateCheckBox(UI, 0, 4, y, 100, 20, "Enable motor", &UIElems, true, null, null);
                    y += YStep;

                    helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Target vel.:", &UIElems);
                    mEditBox_TargetVel = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "-2.0", &UIElems,
                                                              EDITBOX_FLOAT, null, null);
                ...
                {
                    IceComboBox* CB =
                            CreateComboBox<IceComboBox>(UI, 0, 4 + OffsetX, y, 140, 20, "Joint type", &UIElems, null);
                    CB->Add("Revolute");
                    CB->Add("RCA");
                    CB->Select(0);
                ...
                {
                    mCheckBox_TweakInertia =
                            helper.CreateCheckBox(UI, 0, 4, y, 100, 20, "Tweak inertia", &UIElems, false, null, null);
                    y += YStep;
                }
            ...
            //			ObjectDesc.mCollisionGroup	= 1;
                                    ...
                                    if(RCA)
                                    {
                                            ...
                                            StaticObject = pint.CreateRCArticulatedObject(ObjectDesc, ArticulatedDesc,
            RCA);
                                    }
                                    else
                                    {
            ...
            //				LTD->mJoint			= JointHandle;
                    ...
                    virtual	float		DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float
            text_scale)
                    {
            ...
            Float(AngVel.x), Float(AngVel.y), Float(AngVel.z))); renderer.print(0.0f, y, text_scale, _F("Angular
            velocity: %f\n", AngVel.z)); y -= text_scale;
                    //		return PrintTwistAngle(pint, renderer, y, text_scale);
                            }
                            return y;
                    }*/
            ...
            PintActorHandle CreateObject(Pint& pint,
                                         const PINT_SHAPE_CREATE* shape,
                                         float mass,
                                         const Point& pos,
                                         const Quat* rot,
                ...
                if (rot) ObjectDesc.mRotation = *rot;
                return CreatePintObject(pint, ObjectDesc);
            }

            void Connect(Pint& pint,
                         PINT_HINGE2_JOINT_CREATE& Desc,
                         PINT_BOX_CREATE& SmallLinkDesc,
                         PINT_BOX_CREATE& BigLinkDesc,
                         float SideY,
                         PintActorHandle object0,
                         PintActorHandle object1,
                         const Point& Object0Pos,
                         const Point& Object1Pos) {
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                if (!caps.mSupportHingeJoints || !caps.mSupportRigidBodySimulation) return false;
                ...
                if (0) {
                    ...
                    ObjectDesc.mRenderer = CreateColorShapeRenderer(
                            ObjectDesc.mRenderer, RGBAColor(248.0f / 255.0f, 232.0f / 255.0f, 199.0f / 255.0f));
                }
                ...
                if (1) {
                    SmallLinkDesc.mRenderer =
                            __CreateMeshRenderer(SmallLinkExtents + Point(0.0f, ZFightingTweak, 0.0f));
                    SmallLinkDesc.mRenderer =
                            CreateColorShapeRenderer(SmallLinkDesc.mRenderer, RGBAColor(0.0f, 0.0f, 1.0f));
                } else {
                ...
                if (1) {
                    ...
                    BigLinkDesc.mRenderer =
                            CreateColorShapeRenderer(BigLinkDesc.mRenderer, RGBAColor(0.0f, 0.0f, 1.0f));
                } else {
...
#define NB_OBJECTS 6
                ...
                /*		Connect(pint, Desc, SmallLinkDesc, BigLinkDesc, -1.0f, Objects[0], Objects[1],
                   Object0Pos, Object1Pos); Connect(pint, Desc, SmallLinkDesc, BigLinkDesc, 1.0f, Objects[1],
                   Objects[2], Object1Pos, Object2Pos); Connect(pint, Desc, SmallLinkDesc, BigLinkDesc, -1.0f,
                   Objects[2], Objects[3], Object2Pos, Object3Pos); Connect(pint, Desc, SmallLinkDesc,
                   BigLinkDesc, 1.0f, Objects[3], Objects[4], Object3Pos, Object4Pos); Connect(pint, Desc,
                   SmallLinkDesc, BigLinkDesc, -1.0f, Objects[4], Objects[5], Object4Pos, Object5Pos);*/
                for (udword i = 0; i < NB_OBJECTS - 1; i++)
                    Connect(pint, Desc, SmallLinkDesc, BigLinkDesc, (i & 1) ? 1.0f : -1.0f, Objects[i], Objects[i + 1],
            ...
            virtual udword Update(Pint& pint, float dt) {
                ...
                if (LTD) {
                    ...
                    //			const float TargetVel = Coeff * GetFromEditBox(1.0f, mEditBox_TargetVel,
                    //-FLT_MAX, FLT_MAX); 			printf("TargetVel: %f\n", TargetVel);
                    ...
                    if (LTD->mJoint) {
                        ...
                        if (EnableMotor) pint.SetRCADriveVelocity(LTD->mDynamicObject, mTargetVel);
                    }
                }
#endif
                return 0;
            ...
            virtual float DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale) {
                const PintActorHandle* Handles = (const PintActorHandle*)pint.mUserData;
                for (udword i = 0; i < 5; i++) y = PrintAngularVelocity(pint, renderer, Handles[i], y, text_scale);
                return y;
            }
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                if (!caps.mSupportFixedJoints || !caps.mSupportRigidBodySimulation) return false;
            ...
            static void GenerateFixedJointsTorus(Pint& pint,
                                                 const Point& torus_pos,
                                                 const Quat& torus_rot,
                                                 float mass,
                                                 udword nb_loops,
                                                 bool use_rca) {
                // Generate torus. We generate a torus by sweeping a small circle along a large circle, taking N samples
                // (slices) along the way.
                ...
                if (0 && mass != 0.0f) Articulation = pint.CreateArticulation(PINT_ARTICULATION_CREATE());
                PintArticHandle RCArticulation = null;
                if (use_rca) RCArticulation = pint.CreateRCArticulation(PINT_RC_ARTICULATION_CREATE(mass == 0.0f));

                // Next, we generate a convex object for each part of the torus. A part is a section connecting two of
                // the previous slices.
                ...
                for (udword s = 0; s < NbSlices; s++) {
                    ...
                    Point Center(0.0f, 0.0f, 0.0f);
                    {
                        ...
                        for (udword i = 0; i < NbPtsSmallCircle * 2; i++) Center += ConvexPts[i] * Coeff;

                        for (udword i = 0; i < NbPtsSmallCircle * 2; i++) ConvexPts[i] -= Center;
                    }
                ...
                for (int s = 0; s < NbSlices; s++) {
                    ...
                    //		Handles[s] = CreateDynamicObject(pint, &ConvexCreate, pos+Center);
                    {
                        ...
                        // Each convex has been computed from rotated small circles, and we only re-centered the verts
                        // but didn't cancel the rotation. In theory we'd only need one convex object that we could
                        // translate/rotate, but it was easier to create different parts with different rotations. As a
                        // result we don't need to set a rotation for the object itself, it's captured in the vertices.
                        ...
                        if (Articulation) {
                            ...
                            if (s) {
                                const udword SliceIndex0 = (s - 1) % NbSlices;
                                const udword SliceIndex1 = (s) % NbSlices;
                                ArticulatedDesc.mLocalPivot0.mPos =
                                        SliceCenters[SliceIndex1] - ObjectCenters[SliceIndex0];
                                ArticulatedDesc.mLocalPivot1.mPos =
                                        SliceCenters[SliceIndex1] - ObjectCenters[SliceIndex1];
                            } else {
                ...
                if (Articulation) pint.AddArticulationToScene(Articulation);
                if (RCArticulation) pint.AddRCArticulationToScene(RCArticulation);
...
#define NB_FIXED_JOINTS_TORUS_PRESETS 3
            ...
            public:
                ...
                virtual IceTabControl* InitUI(PintGUIHelper& helper) {
                    ...
                    sdword y = 0;
                    {
                            ...
                            virtual void OnComboBoxEvent(ComboBoxEvent event) {
                                if (event == CBE_SELECTION_CHANGED) {
                                    ...
                                    if (SelectedIndex < NB_FIXED_JOINTS_TORUS_PRESETS)
                                        mTest.mEditBox_Desc->SetMultilineText(
                                                gPreset_FixedJointsTorus[SelectedIndex].mDesc);
                                    else
                        ...
                        y += YStep;
                        mEditBox_Desc =
                                helper.CreateEditBox(UI, 0, 4, y, 350 - 16, 200, "", &UIElems, EDITBOX_TEXT, null);
                    ...
                    mCheckBox_StressTest =
                            helper.CreateCheckBox(UI, 0, 4, y, 200, 20, "Use more complex 'stress test' scene",
                                                  &UIElems, false, Local::gCheckBoxCallback, null);
                ...
                virtual bool Setup(Pint& pint, const PintCaps& caps) {
                    ...
                    if (mComboBox_Preset->GetSelectedIndex() == 2) {
                        if (!caps.mSupportRCArticulations) return false;
                        UseRCA = true;
                    }

                    udword NbLoops = 1;
                    if (mComboBox_Preset->GetSelectedIndex() == 1) NbLoops = 8;
            ...
            static const char* gDesc_BridgeUsingHinges =
                    "Bridges made of 20 planks connected by hinge joints. Pick & drag objects to check each engine's "
                    "stability.";
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                ...
                for (udword j = 0; j < NbRows; j++) {
                    ...
                    for (udword i = 1; i < NbBoxes - 1; i++) {
                        ...
                        Pos += PosOffset * 2.0f;
                        // you must allow a wiggle room otherwise the bridge the mass matrix is singular leading to a
                        // explosion of engine with force base solvers
                        //				Pos += (PosOffset - Point (Extents.x * 0.01f, 0.0f, 0.0f))
                        //*2.0f;
                    }
            ...
            virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
                ...
                //		desc.mCamera[0] = PintCameraPose(Point(-0.04f, 49.89f, 26.35f), Point(0.46f, -0.22f,
                //-0.86f));
                SetDefEnv(desc, true);
            }
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                ...
                if (!CreateBridges(pint, caps, Create)) return false;
            ...
            static const char* gDesc_SphericalJointNet =
                    "Stress test for spherical joints. The net is made of 40*40 spheres, all connected by spherical "
                    "joints.";
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                if (!caps.mSupportSphericalJoints || !caps.mSupportRigidBodySimulation) return false;
                ...
                for (udword y = 0; y < NbY; y++) {
                    ...
                    for (udword x = 0; x < NbX - 1; x++) {
                        //				pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[Base+x],
                        // Handles[Base+x+1], Point(CenterX, 0.0f, 0.0f), Point(-CenterX, 0.0f, 0.0f)));
                        pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[Base + x], Handles[Base + x + 1],
                                                                     Point(CenterX * 2.0f, 0.0f, 0.0f),
                ...
                for (udword x = 0; x < NbX; x++) {
                    ...
                    for (udword y = 0; y < NbY - 1; y++) {
                        //				pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[x+(y*NbX)],
                        // Handles[x+(y+1)*NbX], Point(0.0f, 0.0f, CenterY), Point(0.0f, 0.0f, -CenterY)));
                        pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[x + (y * NbX)], Handles[x + (y + 1) * NbX],
                                                                     Point(0.0f, 0.0f, CenterY * 2.0f),
                                                                     Point(0.0f, 0.0f, 0.0f)));
                    }
                }
                return true;
            ...
            static const char* gDesc_SphericalJointNet2 =
                    "Stress test for spherical joints. The net is made of 40*40 spheres, all connected by spherical "
                    "joints.";
            ...
            virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
                ...
                const float Size = 50.0f * gNetScale;
                desc.mGlobalBounds.SetMinMax(Point(-Size, -Size, -Size), Point(Size, Size, Size));
                desc.mCamera[0] = PintCameraPose(Point(60.07f * gNetScale, 35.87f * gNetScale, 60.52f * gNetScale),
                                                 Point(-0.64f, -0.40f, -0.65f));
                SetDefEnv(desc, false);
            }
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                ...
                for (udword y = 0; y < NbY; y++) {
                    ...
                    for (udword x = 0; x < NbX; x++) {
                        ...
                        if (x == 0 && y == 0) ObjectDesc.mMass = 0.0f;
                        if (x == 0 && y == NbY - 1) ObjectDesc.mMass = 0.0f;
                        if (x == NbX - 1 && y == 0) ObjectDesc.mMass = 0.0f;
                        if (x == NbX - 1 && y == NbY - 1) ObjectDesc.mMass = 0.0f;
                        ObjectDesc.mPosition.x = CoeffX * Scale;
                        ObjectDesc.mPosition.y = Altitude;
                        ObjectDesc.mPosition.z = CoeffY * Scale;
                        Handles[Index++] = CreatePintObject(pint, ObjectDesc);
            ...
            static const udword gRagdollData[] = {
                    0x00000013, 0x01c7d2f8, 0x41afcf8e, 0x40f61aee, 0xc3173a20, 0x3f000000, 0x3f000000, 0x41200000,
                    0x0000001e, 0x41afcdbc, 0x41031a76, 0xc3171df7, 0x3c5c4318, 0x3f4df85f, 0xbf17fcea, 0x3f7aaa38,
                    0x3de0bf0b, 0x3e2efcb6, 0x3e4f816e, 0xbf1568a0, 0xbf494cd1, 0,          0x3f962963, 0x3f82a747,
                    0x3facfaae, 0x41200000, 0x01c211d0, 0x41b19fcc, 0x4115b5da, 0xc317093f, 0x3f000000, 0x3f000000,
                    0x41200000, 0x0000001e, 0x41b2e15d, 0x411ead21, 0xc31710d4, 0x3dc82982, 0x3f538bba, 0xbf0dface,
                    0x3f7e8ea1, 0xbd6deaec, 0x3db59f18, 0x3d283688, 0xbf0f6602, 0xbf53ce8b, 0,          0x3f85e78f,
                    0x3f3fbd05, 0x3f72ad19, 0x41200000, 0x00cb87d8, 0x41b2a164, 0x412a6224, 0xc316fb60, 0x3f000000,
                    0x3f000000, 0x41200000, 0x0000001e, 0x41b46871, 0x4132a8ec, 0xc3171ea4, 0x3de9e76c, 0x3f41b1eb,
                    0xbf24d011, 0x3f7da826, 0xbe08e43e, 0x3c98e5c0, 0xbd93578c, 0xbf23d9a2, 0xbf43d4a7, 0,
                    0x3fff4dd5, 0x3fa9a6c6, 0x3fd64ffc, 0x41200000, 0x02e2f720, 0x41b44d4e, 0x41477671, 0xc3171d0b,
                    0x3f000000, 0x3f000000, 0x41200000, 0x0000001e, 0x41b60d03, 0x414fb8e9, 0xc317482b, 0x3ebb5b9a,
                    0x3f2c083e, 0xbf24d17f, 0x3f62aa31, 0xbeedcb3d, 0x3c975790, 0xbe92bdb3, 0xbf13a99b, 0xbf43d3c3,
                    0,          0x3f47be8b, 0x3f0e353f, 0x3f0259e7, 0x41200000, 0x01c0ab78, 0x41b71239, 0x4154dd09,
                    0xc3176272, 0x3f000000, 0x3f000000, 0x41200000, 0x0000001e, 0x
                    0xbd99e60f, 0x3b1d1530, 0xbca81be7, 0x3df66b16, 0xbcc81a50, 0xbd7d86e8, 0xb5d20000, 0xb65a0000,
                    0x3f7fffff, 0xbf7097b6, 0x3cfa6f6c, 0x3eae3d85, 0xbe0cd812, 0xbb1f3f00, 0x3b21b1c0, 0x3d0e0ad5,
                    0x3b1d11dc, 0xbca81b81, 0xb56e0000, 0x35f80000, 0x3f800000, 0xbe9f9ad3, 0x3ef52cda, 0x3f521848,
                    0xbdb2864b, 0xbc51a470, 0x3b9e83c0, 0x3de21c74, 0xbb1f3ab8, 0x3b21b240, 0xb6100000, 0xb5da0000,
                    0x3f7ffffb, 0xb5930000, 0x36200000, 0x3f7fffff, 0xbd7317ed, 0xbc139c52, 0xbc30bd61, 0x3de05658,
                    0xbc51a540, 0x3b9e8330, 0xb60c0000, 0x35540000, 0x3f800000, 0xbe6ae3ae, 0xbf6ef4e1, 0xbe8d3e51};

            TestRagdoll::TestRagdoll() {
                for (udword i = 0; i < 18; i++) mJoints[i] = null;
            }
            ...
            const BoneData* TestRagdoll::FindBoneByName(int name) const {
                for (udword i = 0; i < 19; i++)
                    if (mBones[i].mID == name) return &mBones[i];
                return NULL;
            }

            static PintActorHandle createBodyPart(Pint& pint,
                                                  const Point& extents,
                                                  const PR& pose,
                                                  float mass,
                                                  PintCollisionGroup group,
                ...
                ObjectDesc.mMass = mass;
                ObjectDesc.mCollisionGroup = group;
                if (use_compound) ObjectDesc.mAddToWorld = false;
            ...
            bool TestRagdoll::Init(Pint& pint,
                                   const Point& offset,
                                   PintCollisionGroup group,
                                   bool use_compound,
                                   udword constraint_multiplier) {
                ...
                if (use_compound) Aggregate = pint.CreateAggregate(NbBones, false);
                ...
                for (int i = 0; i < NbBones; i++) {
                    ...
                    if (Aggregate) pint.AddToAggregate(mBones[i].mBody, Aggregate);
                }
                ...
                if (Aggregate) pint.AddAggregateToScene(Aggregate);
            ...
            static bool GenerateArrayOfRagdolls(Pint& pint,
                                                const PintCaps& caps,
                                                udword NbX,
                                                udword NbY,
                                                float Scale,
                ...
                if (use_aggregates && !caps.mSupportAggregates) return false;
            ...
            static bool GenerateColumnOfRagdolls(Pint& pint,
                                                 const PintCaps& caps,
                                                 udword nb,
                                                 udword constraint_multiplier) {
                if (!caps.mSupportHingeJoints || !caps.mSupportCollisionGroups || !caps.mSupportRigidBodySimulation)
            ...
            static const char* gDesc_RagdollConfigurable =
                    "Each ragdoll is made of 19 bones connected by 18 hinge joints. Ragdoll-ragdoll collisions are "
                    "disabled.";
...
#define NB_PRESETS 7
            ...
            public:
                ...
                virtual IceTabControl* InitUI(PintGUIHelper& helper) {
                    ...
                    sdword y = 0;
                    {
                        mCheckBox_Aggregates = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use aggregates", &UIElems,
                                                                     false, null, null);
                        mCheckBox_Aggregates->SetEnabled(false);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Ragdoll array size:", &UIElems);
                        mEditBox_Size = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1", &UIElems,
                        ...
                        mEditBox_Multiplier = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1",
                                                                   &UIElems, EDITBOX_INTEGER_POSITIVE, null, null);
                ...
                virtual void SetDefaultSubscene(const char* sub_scene) {
                    if (!sub_scene || !mComboBox_Preset) return;
                ...
                virtual bool Setup(Pint& pint, const PintCaps& caps) {
                    ...
                    return GenerateArrayOfRagdolls(pint, caps, Size, Size, 2.0f * float(Size), null, UseAggregates,
                                                   Multiplier);
                }
...
// TODO: fix this. It doesn't work when multiple engines are selected
            ...
            static const char* gDesc_Ragdoll256_OnTerrain =
                    "256 ragdolls on a terrain. Ragdoll-ragdoll collisions are disabled.";

            START_TEST(Ragdolls_256_OnTerrain, CATEGORY_JOINTS, gDesc_Ragdoll256_OnTerrain)

            virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
                TestBase::GetSceneParams(desc);
                //		desc.mCamera[0] = PintCameraPose(Point(17.08f, 2.98f, -1.78f), Point(-0.75f, -0.15f,
                //-0.65f));
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                ...
                //		static bool GenerateArrayOfRagdolls(Pint& pint, const PintCaps& caps, udword NbX, udword
                // NbY, float Scale, Point* offset=null)
                {
                    udword NbX = 16;
                    udword NbY = 16;
                    float Scale = 2000.0f;
            ...
            static const char* gDesc_PileOfRagdolls16 =
                    "A pile of 16 ragdolls. Ragdoll-ragdoll collisions are enabled.";
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) { return GenerateColumnOfRagdolls(pint, caps, 16, 1); }
            ...
            static const char* gDesc_ConfigurableSphericalChain =
                    "Spherical chain. This tutorial demonstrates various ways to make \
...
in the test UI itself, and must/can be tweaked in the per-engine UI.";
            ...
            virtual IceTabControl* InitUI(PintGUIHelper& helper) {
...

                {
                    ...
                    mData.mEditBox_NbSimCalls = helper.CreateEditBox(UI, 1, 4 + LabelSize, y, EditBoxWidth, 20, "4",
                                                                     &UIElems, EDITBOX_INTEGER_POSITIVE, null, null);
                    y += YStep;

                    helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelSize, 20,
                                       "Nb spheres (more spheres is less stable):", &UIElems);
                    mData.mEditBox_NbSpheres =
                            helper.CreateEditBox(UI, CHAIN_DATA_NB_SPHERES, 4 + LabelSize, y, EditBoxWidth, 20, "64",
                                                 &UIElems, EDITBOX_INTEGER_POSITIVE, null);
                    y += YStep;

                    helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelSize, 20,
                                       "Sphere radius (larger radius is more stable):", &UIElems);
                    mData.mEditBox_Radius = helper.CreateEditBox(UI, CHAIN_DATA_RADIUS, 4 + LabelSize, y, EditBoxWidth,
                                                                 20, "0.5", &UIElems, EDITBOX_FLOAT_POSITIVE, null);
                    y += YStep;

                    helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelSize, 20, "Sphere mass:", &UIElems);
                    mData.mEditBox_Mass = helper.CreateEditBox(UI, CHAIN_DATA_MASS, 4 + LabelSize, y, EditBoxWidth, 20,
                                                               "1.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null);
                    y += YStep;

                    helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelSize, 20, "Gap between spheres:", &UIElems);
                    mData.mEditBox_Gap = helper.CreateEditBox(UI, CHAIN_DATA_GAP, 4 + LabelSize, y, EditBoxWidth, 20,
                                                              "0.0", &UIElems, EDITBOX_FLOAT, null);
                    y += YStep;
                }

                {
                    ...
                    CB = helper.CreateCheckBox(UI, CHAIN_DATA_CENTERED_PIVOTS, 4, y, 400, 20,
                                               "Place pivots at sphere centers (more stable).", &UIElems,
                                               mData.mCenteredPivots, ConfigurableSphericalChain::CheckBoxCallback,
                                               null);
                    CB->SetUserData(&mData);
                    y += YStep;

                    CB = helper.CreateCheckBox(UI, CHAIN_DATA_USE_DISTANCE_CONSTAINTS, 4, y, 400, 20,
                                               "Use extra distance constraints between spheres (more stable)", &UIElems,
                                               mData.mUseExtraDistanceConstraints,
                                               ConfigurableSphericalChain::CheckBoxCallback, null);
                    CB->SetUserData(&mData);
                    y += YStep;

                    CB = helper.CreateCheckBox(UI, CHAIN_DATA_USE_LARGER_INERTIA, 4, y, 400, 20,
                                               "Make the inertia tensor artificially larger (can be more stable)",
                                               &UIElems, mData.mUseLargerInertia,
                                               ConfigurableSphericalChain::CheckBoxCallback, null);
                    CB->SetUserData(&mData);
                    y += YStep;

                    CB = helper.CreateCheckBox(UI, CHAIN_DATA_ATTACH_HEAVY_BOX, 4, y, 400, 20,
                                               "Attach heavy box (100X sphere mass) to last sphere (less stable)",
                                               &UIElems, mData.mAttachHeavyBox,
                                               ConfigurableSphericalChain::CheckBoxCallback, null);
                    CB->SetUserData(&mData);
                    y += YStep;

                    CB = helper.CreateCheckBox(UI, CHAIN_DATA_USE_ARTICULATION, 4, y, 400, 20,
                                               "Use dedicated MC articulation feature (more stable)", &UIElems,
                                               mData.mUseArticulation, ConfigurableSphericalChain::CheckBoxCallback,
                                               null);
                    CB->SetUserData(&mData);
                    y += YStep;

                    CB = helper.CreateCheckBox(UI, CHAIN_DATA_FIRST_SPHERE_IS_STATIC, 4, y, 400, 20,
                                               "Make first sphere static (else attach it to static anchor)", &UIElems,
                                               mData.mFirstSphereIsStatic, ConfigurableSphericalChain::CheckBoxCallback,
                                               null);
                    CB->SetUserData(&mData);
                    y += YStep;
                }
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                if (!caps.mSupportRigidBodySimulation || !caps.mSupportSphericalJoints) return false;
                if (mData.mUseExtraDistanceConstraints && !caps.mSupportDistanceJoints) return false;
                if (mData.mUseArticulation && !caps.mSupportMCArticulations) return false;
                if (mData.mUseLargerInertia && !caps.mSupportMassForInertia) return false;
                ...
                PintArticHandle Articulation =
                        mData.mUseArticulation ? pint.CreateArticulation(PINT_ARTICULATION_CREATE()) : null;
                ...
                udword GroupBit = 0;
                {
                    ...
                    for (udword i = 0; i < NbSpheres; i++) {
                        ...
                        PINT_OBJECT_CREATE ObjectDesc(&SphereDesc);
                        ObjectDesc.mMass = Mass;
                        if (mData.mFirstSphereIsStatic && i == 0) ObjectDesc.mMass = 0.0f;

                        if (mData.mUseLargerInertia) ObjectDesc.mMassForInertia = Mass * 10.0f;
                        ObjectDesc.mPosition = Pos;
                        ObjectDesc.mCollisionGroup = 1 + GroupBit;
                        GroupBit = 1 - GroupBit;
                    ...
                    if (!Articulation) {
                        for (udword i = 0; i < NbSpheres - 1; i++) {
                            PintJointHandle JointHandle = pint.CreateJoint(
                                    PINT_SPHERICAL_JOINT_CREATE(Handles[i], Handles[i + 1], LocalPivot0, LocalPivot1));
                            ASSERT(JointHandle);
                        }
                    }
                        ...
                        /*				for(udword i=0;i<NbSpheres-1;i++)
                                                        {
                                                                ...
                                                                Desc.mObject1		= Handles[i+1];
                                                                Desc.mMaxDistance	=
                           Positions[i].Distance(Positions[i+1]); PintJointHandle JointHandle = pint.CreateJoint(Desc);
                                                                ASSERT(JointHandle);
                                                        }*/
                    }
                }
                ...
                if (!mData.mFirstSphereIsStatic) {
                    ...
                    PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(
                            h, Handles[0], Point(0.0f, 0.0f, 0.0f), Point(0.0f, 0.0f, 0.0f)));
                    ASSERT(JointHandle);
                }
                ...
                if (mData.mAttachHeavyBox) {
                    ...
                    PintJointHandle JointHandle = pint.CreateJoint(
                            PINT_SPHERICAL_JOINT_CREATE(Handles[NbSpheres - 1], HeavyBox, Point(Radius, 0.0f, 0.0f),
                                                        Point(-BoxExtents.x, 0.0f, 0.0f)));
                    ASSERT(JointHandle);
                }
            ...
            static const char* gDesc_StableSphericalChain =
                    "Demonstrates how to make a long, stable spherical chain with regular joints (no articulations). \
...
distance constraints and inertia tensor tweaks. This worked fairly well even back in PhysX 2.x.";
            ...
            static const char* gDesc_CaterpillarTrack =
                    "Rough/experimental caterpillar track using regular joints. Not properly tuned or optimized.";
            ...
            virtual IceTabControl* InitUI(PintGUIHelper& helper) {
                ...
                helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Constraint multiplier:", &UIElems);
                mEditBox_Multiplier = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "32", &UIElems,
                                                           EDITBOX_INTEGER_POSITIVE, null, null);
                y += YStep;
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                if (!caps.mSupportRigidBodySimulation || !caps.mSupportConvexes || !caps.mSupportCompounds ||
                    !caps.mSupportHingeJoints)
                    return false;

                const udword Multiplier = GetInt(32, mEditBox_Multiplier);

                CaterpillarTrackObjects* TO = ICE_NEW(CaterpillarTrackObjects);
                CreateCaterpillarTrack_OLD(pint, *TO, &mHighFrictionMaterial, Point(0.0f, 2.0f, 0.0f), mCylinder,
                                           GetRegisteredRenderers()[0], mCylinder2, GetRegisteredRenderers()[1],
                                           Multiplier);
            ...
            virtual udword Update(Pint& pint, float dt) {
                ...
                if (1) {
                    ...
                    for (udword i = 0; i < TO->mNbGears; i++) {
                        if (TO->mGears[i]) pint.SetAngularVelocity(TO->mGears[i], Point(0.0f, 0.0f, -5.0f));
                    }
                }
                return TestBase::Update(pint, dt);
            }
            ...
            static const char* gDesc_CaterpillarTrackOnBridge =
                    "Caterpillar track on bridge. Artificial benchmark scene involving a lot of joints.";
            ...
            virtual IceTabControl* InitUI(PintGUIHelper& helper) {
                ...
                mEditBox_NbX = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "2", &UIElems,
                                                    EDITBOX_INTEGER_POSITIVE, null, null);
                y += YStep;

                helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "NbZ:", &UIElems);
                mEditBox_NbZ = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "8", &UIElems,
                                                    EDITBOX_INTEGER_POSITIVE, null, null);
                y += YStep;
            ...
            void CreateScene(Pint& pint, const Point& pos) {
                ...
                CreateCaterpillarTrack_OLD(pint, TO, &mHighFrictionMaterial, pos + Point(3.0f, 2.0f, 0.0f), mCylinder,
                                           GetRegisteredRenderers()[0], mCylinder2, GetRegisteredRenderers()[1],
                                           Multiplier);
                ...
                for (udword i = 0; i < TO.mNbGears; i++) UserData->mGears.AddPtr(TO.mGears[i]);
                    ...
                    udword j = 0;
                    {
                        ...
                        for (udword i = 0; i < NbBoxes; i++) {
...

                            {
                                ...
                                ObjectDesc.mMass = Mass;
                                ObjectDesc.mPosition = Pos;
                                //					ObjectDesc.mPosition.y		= (Chainette[i]
                                //+
                                // Chainette[i+1])*0.5f; 					ObjectDesc.mPosition.y		+=
                                // Chainette[i] - 10.0f;
            ...
            virtual void OnObjectReleased(Pint& pint, PintActorHandle removed_object) {
                ...
                for (udword i = 0; i < Gears.GetNbEntries(); i++) {
                    if (PintActorHandle(Gears.GetEntry(i)) == removed_object) Gears[i] = null;
                }
            }
...
#include "Camera.h"
            ...
            struct ChassisData : public Allocateable {
                ChassisData()
                    : mChassis(null)  //, mNbWheels(0)
                {
                    for (udword i = 0; i < 2; i++) mFrontAxleObject[i] = null;
                }
                ...
                if (mClampAngularVelocity) {
                    ...
                    for (udword i = 0; i < 2; i++) {
                        if (vehicle_data.mFront[i].mWheel && vehicle_data.mFront[i].mParent) {
                            ...
                            // pint.GetAngularVelocity(vehicle_data.mFront[i].mWheel); 				printf("Angular velocity %d:
                            // %f %f %f\n", i, AngularVelocity.x, AngularVelocity.y, AngularVelocity.z);

                            if (::ClampAngularVelocity(pint, vehicle_data.mFront[i].mParent,
                                                       vehicle_data.mFront[i].mWheel, MaxAngularVelocity))
                                CanAccelerate = false;
                        }
                    }
                    for (udword i = 0; i < 2; i++) {
                        if (vehicle_data.mRear[i].mWheel && vehicle_data.mRear[i].mParent) {
                            //				Point AngularVelocity =
                            // pint.GetAngularVelocity(vehicle_data.mRear[i].mWheel); 				printf("Angular velocity %d:
                            // %f %f %f\n", i, AngularVelocity.x, AngularVelocity.y, AngularVelocity.z);

                            if (::ClampAngularVelocity(pint, vehicle_data.mRear[i].mParent,
                                                       vehicle_data.mRear[i].mWheel, MaxAngularVelocity))
                                CanAccelerate = false;
                        }
                    }
                }
...
#define START_VEHICLE_TEST(name, category, desc)                    \
    ...
    public:                                                         \
        ...
        virtual TestCategory GetCategory() const { return category; }
            ...
            static const char* gDesc_Tank =
                    "Tank test. This uses regular joints (no articulation). This is an experimental, work-in-progress test. It has not been properly tuned or \
...
camera views. Controls are experimental.";
            ...
            virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
                ...
                //		desc.mCamera[0] = PintCameraPose(Point(10.76f, 11.91f, 14.07f), Point(-0.54f, -0.49f,
                //-0.68f));
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                ...
                CreateCaterpillarTrack_OLD(pint, TO0, &mHighFrictionMaterial, Pos0, mCylinder,
                                           GetRegisteredRenderers()[0], mCylinder2, GetRegisteredRenderers()[1]);

                CaterpillarTrackObjects TO1;
                CreateCaterpillarTrack_OLD(pint, TO1, &mHighFrictionMaterial, Pos1, mCylinder,
                                           GetRegisteredRenderers()[0], mCylinder2, GetRegisteredRenderers()[1]);
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) { return CreateZB2Scene(pint, caps); }

            END_TEST(SteeringTest4b)

            ///////////////////////////////////////////////////////////////////////////////

            static const char* gDesc_DuneBuggySteeringTest =
                    "Lego Technic Dune Buggy steering test. This one exposed two modelling issues in the original Mecabricks file: some of the \
            ...
            public:
                struct PintData : public Allocateable {
                    PintData() : mChassis(null), mSteeringJoint(null) {
                        for (udword i = 0; i < 4; i++) mWheels[i] = null;
                    }
            ...
            public:
                ...
                public:
                    ...
                    virtual void CustomizeActor(PINT_OBJECT_CREATE& create) {
                        ...
                        if (create.mName) {
                            //				if(strcmp(create.mName, "RearWheel")==0 || strcmp(create.mName,
                            //"FrontWheel")==0) 					create.mMass = 10.0f; 				if(strcmp(create.mName,
                            //"pipo")==0) 					create.mMass = 0.1f;
                        }
                    }
                ...
                virtual bool CommonSetup() {
                        ...
                        if (1) {
                            ...
                            for (udword i = 0; i < NbJoints; i++) {
                                PINT_JOINT_CREATE* Create =
                                        reinterpret_cast<PINT_JOINT_CREATE*>(mZB2Factory->mJoints[i]);
                                if (Create) {
                                    //						printf("Joint %d (type %d): %s\n", i,
                                    //Create->mType, Create->mName);
                    ...
                    if (0) mPlanarMeshHelper.Generate(200, 0.5f);
                ...
                public:
                    ...
                    virtual void NotifyCreatedObjects(Pint& pint,
                                                      const ZB2CreatedObjects& objects,
                                                      ZCB2Factory*) override {
                        ...
                        for (udword i = 0; i < 4; i++) {
                            if (PD->mWheels[i]) {
                                ...
                                if (TargetSpeed == 0.0f) {
                                    ...
                                    pint.SetDriveVelocity(PD->mWheels[i], Point(0.0f, 0.0f, 0.0f),
                ...
                static const char* gDesc_LegoTechnicBuggy =
                        "Lego Technic Buggy 8048. Use cursor keys to drive, num keypad keys to control the camera.";
                ...
                public:
                    ...
                    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
                        ...
                        mEditBox_Gravity = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "-981.0",
                                                                &UIElems, EDITBOX_FLOAT, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Scale:", &UIElems);
                        // mEditBox_Scale = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "0.35",
                        // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        //  Real world scale is about 26*15 cm
                        // mEditBox_Scale = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "0.02",
                        // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_Scale = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "2.0", &UIElems,
                                                              EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Mass:", &UIElems);
                        // mEditBox_Mass = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                        // EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_Mass = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.01", &UIElems,
                                                             EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Spring stiffness:", &UIElems);
                        // mEditBox_Stiffness = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "1000.0",
                        // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_Stiffness = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "200.0",
                                                                  &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Spring damping:", &UIElems);
                        // mEditBox_Damping = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "100.0",
                        // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_Damping = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "10.0",
                                                                &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Steer stiffness:", &UIElems);
                        // mEditBox_SteerStiffness = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20,
                        // "2000.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_SteerStiffness =
                                helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "200000.0", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Steer damping:", &UIElems);
                        mEditBox_SteerDamping = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "10000.0",
                                                                     &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Steer limit:", &UIElems);
                        // mEditBox_SteerLimit = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "0.75",
                        // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_SteerLimit = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0",
                                                                   &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Max speed:", &UIElems);
                        // mEditBox_MaxSpeed = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20, "15.0",
                        // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); mEditBox_MaxSpeed = helper.CreateEditBox(UI,
                        // 1, 4+OffsetX, y, EditBoxWidth, 20, "50.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        mEditBox_MaxSpeed = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "30.0",
                                                                 &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
                        y += YStep;

                        mCheckBox_AddMotors =
                                helper.CreateCheckBox(UI, 0, 4, y, 100, 20, "Add motors", &UIElems, true, null, null);
                        y += YStep;

                        mCheckBox_AutoDrive =
                                helper.CreateCheckBox(UI, 0, 4, y, 100, 20, "Auto-drive", &UIElems, false, null, null);
                        y += YStep;

                        mCheckBox_UserSteer =
                                helper.CreateCheckBox(UI, 0, 4, y, 100, 20, "User-steer", &UIElems, true, null, null);
                        y += YStep;

                        mCheckBox_FreeCamera =
                                helper.CreateCheckBox(UI, 0, 4, y, 100, 20, "Free camera", &UIElems, false, null, null);
                        y += YStep;
                    ...
                    class MyCustomizeCallback : public ZB2CustomizeCallback {
                    public:
                        MyCustomizeCallback(float mass,
                                            float stiffness,
                                            float damping,
                                            float steer_stiffness,
                                            float steer_damping,
                                            bool user_steering)
                            : mNbActors(0),
                              mNbSprings(0),
                    ...
                    virtual bool CommonSetup() {
                        ...
                        // const char* Filename = FindPEELFile("LegoTechnic_Buggy2.zb2");	// Scaled down +
                        // automatic drive + terrain const char* Filename = FindPEELFile("LegoTechnic_Buggy2b.zb2");
                        // // + Blue scheme const char* Filename = FindPEELFile("LegoTechnic_Buggy2c.zb2");	//
                        // Without render sources
                        const char* Filename =
                                FindPEELFile("LegoTechnic_Buggy3.zb2");  // Non scaled, default color scheme, no
                                                                         // terrain, D6 prismatic rack
                        if (Filename) {
                            ...
                            MyCustomizeCallback CB(Mass, Stiffness, Damping, SteerStiffness, SteerDamping,
                                                   UserSteering);
                            PINT_WORLD_CREATE desc;
                            ImportZB2File(desc, Filename, &CB);
                            ...
                            if (mCheckBox_AddMotors->IsChecked()) {
                                ...
                                for (udword i = 0; i < NbJoints; i++) {
                                    ...
                                    if (Create && Create->mName) {
                                        if (Create->mType == PINT_JOINT_HINGE2) {
                                            PINT_HINGE2_JOINT_CREATE* hjc =
                                                    static_cast<PINT_HINGE2_JOINT_CREATE*>(Create);

                                            const bool IsFreeRearWheel =
                                                    strcmp(Create->mName, "Free rear wheel hinge") == 0;
                                            const bool IsConnectedRearWheel =
                                                    strcmp(Create->mName, "Connected rear wheel hinge") == 0;
                                ...
                                for (udword i = 0; i < NbJoints; i++) {
                                    ...
                                    if (Create && Create->mName && strcmp(Create->mName, "Rack prismatic") == 0) {
                                        if (SwitchToD6 && Create->mType == PINT_JOINT_PRISMATIC) {
                                            ...
                                            const PINT_PRISMATIC_JOINT_CREATE* pjc =
                                                    static_cast<const PINT_PRISMATIC_JOINT_CREATE*>(Create);
                                            ...
                                            const PINT_D6_JOINT_CREATE* d6jc =
                                                    static_cast<const PINT_D6_JOINT_CREATE*>(Create);

                                            PINT_PRISMATIC_JOINT_CREATE* NewJoint =
                                                    ICE_NEW(PINT_PRISMATIC_JOINT_CREATE);
                                            NewJoint->mName = Create->mName;
                                            NewJoint->mObject0 = Create->mObject0;
                                            NewJoint->mObject1 = Create->mObject1;
                                ...
                                for (udword i = 0; i < objects.mNbJoints; i++) {
                                    ...
                                    if (Name) {
                                        ...
                                        } else  // if(JT==PINT_JOINT_HINGE2)
                                        {
                                            ...
                                            const bool IsConnectedRearWheel =
                                                    strcmp(Name, "Connected rear wheel hinge") == 0;
                                            if (IsFreeRearWheel) mData.mWheels[2] = CurrentJoint;
                                            if (IsConnectedRearWheel) mData.mWheels[3] = CurrentJoint;
                                        }
                                    }
                                }
                    ...
                    virtual bool Setup(Pint& pint, const PintCaps& caps) {
                        ...
                        if (0 && caps.mSupportConvexes) {
                            ...
                            const udword nb_y = Size;
                            {
                                ...
                                //				Create.mRenderer	=
                                //CreateConvexRenderer(Create.mNbVerts, Create.mVerts);
                                ...
                                // ~500 fps
                                PINT_MESH_CREATE Create;
                                Create.SetSurfaceData(MergedVerts.GetNbVertices(), MergedVerts.GetVertices(),
                                                      MergedTris.GetNbEntries() / 3, MergedTris.GetEntries(), null);
                                Create.mRenderer = CreateMeshRenderer(Create.GetSurface());
                        ...
                        if (0) {
                            ...
                            const udword nb_y = Size;
                            {
                                ...
                                ConvexCreate.mRenderer =
                                        CreateConvexRenderer(ConvexCreate.mNbVerts, ConvexCreate.mVerts);
