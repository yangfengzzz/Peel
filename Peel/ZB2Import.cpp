...
// This is only for shape renderers that haven't been created by the import process, i.e. this is for "implicit"
// renderers that can be derived from collision shapes (and thus haven't been exported, to make files smaller)
...
static void CreateImplicitShapeRenderers(ZCB2Factory* factory) {
    ...
    public:
        ...
        CreateImplicitShape(ZCB2Factory* factory, PintShapeRenderer** mesh_renderers, udword nb_meshes)
            : mFactory(factory), mMeshRenderers(mesh_renderers), mNbMeshes(nb_meshes) {}
            ...
            if (!CurrentShape->mRenderer) {
                switch (CurrentShape->mType) {
                    ...
                    case PINT_SHAPE_CAPSULE: {
                        ...
                        CurrentShape->mRenderer =
                                CreateCapsuleRenderer(ShapeCreate->mRadius, ShapeCreate->mHalfHeight * 2.0f);
                    } break;

                    case PINT_SHAPE_CYLINDER: {
                        printf(".....creating cylinder shape renderer\n");
                        const PINT_CYLINDER_CREATE* ShapeCreate =
                                static_cast<const PINT_CYLINDER_CREATE*>(CurrentShape);
                        CurrentShape->mRenderer =
                                CreateCylinderRenderer(ShapeCreate->mRadius, ShapeCreate->mHalfHeight * 2.0f);
                    } break;
                    ...
                    case PINT_SHAPE_MESH2: {
                        ...
                        const udword MeshID = udword(
                                size_t(ShapeCreate->mTriangleMesh));  // ### I think this is converted to a pointer when
                                                                      // creating the zb2 scene later?
                        ASSERT(MeshID < mNbMeshes);
                        if (mMeshRenderers[MeshID]) {
                            //							if(!Color)
                            CurrentShape->mRenderer = mMeshRenderers[MeshID];
                            //							else
                            //								CurrentShape->mRenderer = CreateColorShapeRenderer(mMeshRenderers[MeshID],
                            //*Color);
                        } else {
                            const PINT_MESH_DATA_CREATE* Create =
                                    (const PINT_MESH_DATA_CREATE*)mFactory->mMeshObjects[MeshID];
                            ...
                            //							else
                            //								CurrentShape->mRenderer = CreateColorShapeRenderer(R,
                            //*Color);
                        }

                        ZCB2Factory::ZCB2MeshRecord* Record =
                                ICE_RESERVE(ZCB2Factory::ZCB2MeshRecord, mFactory->mZCB2MeshRecords);
    ...
    for (udword i = 0; i < NbActors; i++) Actors[i].mCreate->GetNbShapes(&CB);

    ICE_FREE(MeshRenderers);
}

bool ImportZB2File(PINT_WORLD_CREATE& desc,
                   const char* filename,
                   ZCB2Factory* factory,
                   ZB2CustomizeCallback* callback) {
    ASSERT(factory);
    ...
    public:
        ProgressBarCB() {}
        virtual ~ProgressBarCB() { ReleaseProgressBar(); }
        virtual void OnImportStart(udword nb_chunks) { CreateProgressBar(nb_chunks, "Importing ZB2 file..."); }

        virtual void OnNewChunk(udword index) { SetProgress(index); }
    };
    ...
    if (1) CreateImplicitShapeRenderers(factory);
...
bool CreateZB2Scene(Pint& pint, const PintCaps& caps, ZCB2Factory* factory, ZB2CreationCallback* callback) {
    ...
    // Create meshes
    const udword NbMeshRecords =
            factory->mZCB2MeshRecords.GetNbEntries() / (sizeof(ZCB2Factory::ZCB2MeshRecord) / sizeof(udword));
    const ZCB2Factory::ZCB2MeshRecord* Records =
            (const ZCB2Factory::ZCB2MeshRecord*)factory->mZCB2MeshRecords.GetEntries();
    ...
    if (NbActors) {
        ...
        for (udword i = 0; i < NbActors; i++) {
            ...
            if (callback) UserCreatedActor = callback->CreateActor(h, pint, *Actors[i].mCreate);
            if (!UserCreatedActor) {
                h = CreatePintObject(pint, *Actors[i].mCreate);
                ASSERT(h);

                const udword Flags = Actors[i].mFlags;
                if (Flags) pint.mVisHelper->SetRenderable(h, false);
            }
        ...
        for (udword i = 0; i < NbJoints; i++) {
            ...
            if (Create) {
                switch (Create->mType) {
                    ...
                    case PINT_JOINT_D6: {
                        if (!caps.mSupportD6Joints) return false;
                    } break;
                    ...
                    case PINT_JOINT_PORTAL: {
                        if (!caps.mSupportPortalJoints) return false;
                    } break;
                };
    ...
    if (NbAggregates) {
        if (!caps.mSupportAggregates) return false;
    ...
    if (callback) callback->NotifyCreatedObjects(pint, Created.mData, factory);
