...
static const char* gDesc_SeaOfStaticConvexes =
        "64*64 static convexes. Used to measure memory usage. (Check or uncheck the per-engine options in the UI to share convexes). \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) { return CreateSeaOfStaticConvexes(pint, caps, 64, 64, 40.0f); }
...
static const char* gDesc_SeaOfStaticBoxes =
        "128*128 static boxes. Used to measure memory usage. \
This also exposes issues in some engines that take a significant amount of time to simulate a scene where everything is static.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) { return CreateSeaOfStaticBoxes(pint, 40.0f, 128, 128, 0.0f); }

END_TEST(SeaOfStaticBoxes)

static const char* gDesc_SeaOfStaticBoxes2 =
        "128*128 static boxes. Used to measure memory usage. \
This also exposes issues in some engines that take a significant amount of time to simulate a scene where everything is static.";
...
static const char* gDesc_SeaOfStaticBoxes3 =
        "255*255 static boxes. Used to measure memory usage. \
This also exposes issues in some engines that take a significant amount of time to simulate a scene where everything is static.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) { return CreateSeaOfStaticBoxes(pint, 200.0f, 255, 255, 0.0f); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) { return CreateMeshesFromRegisteredSurfaces(pint, caps); }
...
public:
    ...
    virtual bool CommonSetup() {
        ...
        if (mComboBox_Filename) {
            ...
            // LoadMeshesFromFile_(*this, gMeshInfo[Index].mFilename, HasScale ? &Scale : null, false,
            // gMeshInfo[Index].mTess);
            BinLoaderSettings BLS;
            if (HasScale) BLS.mScale = Scale;
    ...
    virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
        ...
        if (mComboBox_Filename) {
            ...
            if (Index == MESH_VENUS || Index == MESH_TESSVENUS) {
                ...
                desc.mCamera[1] = PintCameraPose(Point(1565.97f, 73.77f, -919.54f),
                                                 Point(-0.65f, 0.41f, -0.63f));  // Zoom on a crack
    ...
    static const char* gDesc_CustomerStaticMeshes =
            "Customer static mesh. Used to check memory usage and for raytracing tests.";
    ...
    public:
        ...
        virtual IceTabControl* InitUI(PintGUIHelper& helper) {
            ...
            sdword y = 0;
            {
                ...
                {
                    IceComboBox* CB =
                            CreateComboBox<IceComboBox>(UI, 0, 4 + OffsetX, y, 170, 20, "Filename", null, null);
                    CB->Add("Empty scene");
                    CB->Add("PlanetSide");
                    CB->Add("Valve");
                    CB->Add("Specular");
            ...
            if (mComboBox_Filename) {
                ...
                if (gCustomerMeshInfo[Index].mFilename) {
                    if (gCustomerMeshInfo[Index].mRepX)
                        mRepX = CreateRepXContext(gCustomerMeshInfo[Index].mFilename, gCustomerMeshInfo[Index].mScale,
                                                  gCustomerMeshInfo[Index].mZIsUp);
                    //				else
                ...
                if (Index == MESH_PLANETSIDE) {
                    ...
                    if (mComboBox_Filename) {
                        ...
                        if (gCustomerMeshInfo[Index].mFilename) {
                            ...
                            if (Filename) ImportZB2File(desc, Filename);
                        }
                    }
                }
            }
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            ...
            if (mRepX) return AddToPint(pint, mRepX);
