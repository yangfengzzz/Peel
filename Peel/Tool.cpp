...
udword GetNbTools() { return TOOL_COUNT; }

static const char* gToolNames[] = {
        "Picking",      "Add impulse",  "Shoot object", "Translate object", "Rotate object", "Create object",
        "Create joint", "Create chain", "Create rope",  "Terrain editor",   "Texture",       "Bullet",
};

ICE_COMPILE_TIME_ASSERT(TOOL_COUNT == ARRAYSIZE(gToolNames));

const char* GetToolName(udword i) {
    if (i >= TOOL_COUNT) return null;
    return gToolNames[i];
}
...
ToolInterface* GetCurrentTool() { return gCurrentTool; }

ToolIndex GetCurrentToolIndex() { return gCurrentToolIndex; }

ToolInterface* GetTool(udword index) {
    if (index >= TOOL_COUNT) return null;
    return gTools[index];
}
...
void SelectTool(udword index) {
    ...
    for (udword i = 0; i < TOOL_COUNT; i++) {
        ...
        if (ToolWindow) ToolWindow->SetVisible(i == gCurrentToolIndex);
    }

    if (gCurrentTool) gCurrentTool->Deselect();
    gCurrentTool = gTools[index];
    if (gCurrentTool) gCurrentTool->Select();
...
void CloseTools() {
    for (udword i = 0; i < TOOL_COUNT; i++) DELETESINGLE(gTools[i]);
}
...
void DrawBoxCorners(Pint& pint, PintActorHandle actor, const Point& color) {
    ...
    if (API) {
        AABB Bounds;
        if (API->GetWorldBounds(actor, Bounds)) GLRenderHelpers::DrawBoxCorners(Bounds, color, 0.2f);
    }
}
