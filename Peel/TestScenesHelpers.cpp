...
bool CreateSeaOfStaticBoxes(Pint& pint,
                            float amplitude,
                            udword nb_x,
                            udword nb_y,
                            float altitude,
                            bool add_to_scene,
                            PtrContainer* created_objects) {
    BasicRandom Rnd(42);
    ...
    for (udword y = 0; y < nb_y; y++) {
        ...
        for (udword x = 0; x < nb_x; x++) {
            ...
            if (created_objects) created_objects->AddPtr(h);
        }
    }
    return true;
}
...
udword GenerateConvex(
        Point* verts, udword nbInsideCirclePts, udword nbOutsideCirclePts, float scale0, float scale1, float z) {
...
PintActorHandle CreateSimpleObject(Pint& pint,
    ...
    if (linVel) ObjectDesc.mLinearVelocity = *linVel;
    if (angVel) ObjectDesc.mAngularVelocity = *angVel;
    return CreatePintObject(pint, ObjectDesc);
}

///////////////////////////////////////////////////////////////////////////////

PintActorHandle CreateDynamicObject(Pint& pint,
                                    const PINT_SHAPE_CREATE* shape,
                                    const Point& pos,
                                    const Quat* rot,
                                    const Point* linVel,
...
PintActorHandle CreateStaticObject(
        Pint& pint, const PINT_SHAPE_CREATE* shape, const Point& pos, const Quat* rot, const char* name) {
    return CreateSimpleObject(pint, shape, 0.0f, pos, rot, null, null, name);
}

///////////////////////////////////////////////////////////////////////////////

PintActorHandle CreateDynamicBox(Pint& pint,
                                 float size_x,
                                 float size_y,
                                 float size_z,
                                 const Point& pos,
...
bool CreateTestScene_ConvexStack_Generic(
        Pint& pint, udword NbX, udword NbY, udword NbLayers, udword NbInsideCirclePts, udword NbOutsideCirclePts) {
...
void GenerateConvexPile(
        Pint& pint, udword nb_x, udword nb_y, udword nb_layers, float amplitude, udword nb_random_pts, float scatter) {
...
bool CreateArrayOfDynamicConvexes(Pint& pint,
                                  const PINT_CONVEX_CREATE& convex_create,
                                  udword nb_x,
                                  udword nb_y,
                                  float altitude,
                                  float scale_x,
...
static bool GenerateArrayOfShapes(Pint& pint,
                                  const PINT_SHAPE_CREATE* shape,
                                  udword nb_x,
                                  udword nb_y,
                                  float altitude,
...
bool GenerateArrayOfSpheres(Pint& pint,
                            float radius,
                            udword nb_x,
                            udword nb_y,
                            float altitude,
...
bool GenerateArrayOfBoxes(Pint& pint,
                          const Point& extents,
                          udword nb_x,
                          udword nb_y,
                          float altitude,
...
bool GenerateArrayOfCapsules(Pint& pint,
                             float radius,
                             float half_height,
                             udword nb_x,
                             udword nb_y,
...
bool GenerateArrayOfConvexes(
        Pint& pint, const PintCaps& caps, bool is_static, float scale, udword nb_x, udword nb_y, udword convex_id) {
    if (!caps.mSupportConvexes) return false;
...
bool GenerateArrayOfObjects(Pint& pint,
                            const PintCaps& caps,
                            PintShape type,
                            udword type_data,
                            udword nb_x,
...
bool Setup_PotPourri_Raycasts(
        Pint& pint, const PintCaps& caps, float mass, udword nb_layers, udword nb_x, udword nb_y) {
...
#ifdef REMOVED
static bool CreateRope(Pint& pint,
                       PintActorHandle articulation,
                       PintActorHandle parent,
                       const Point& p0,
                       const Point& p1,
                       udword nb_capsules,
                       float capsule_radius,
                       float capsule_mass,
                       /*bool articulation,*/ bool overlapping_links,
                       bool create_distance_constraints,
                       PintActorHandle* handles = null,
                       float* half_height = null,
                       Quat* qq = null) {
    if (nb_capsules > MAX_LINKS) return false;
    ...
    if (half_height) *half_height = HalfHeight;
    ...
    for (udword i = 0; i < nb_capsules; i++) {
        Positions[i] = Pos;
        {
            ...
            if (handles) handles[i] = Handles[i];
        }
        Pos += Extents * 2.0f;
    }
...
/*static bool CreateRope(Pint& pint, const Point& p0, const Point& p1, udword nb_capsules, float capsule_radius, float
capsule_mass, bool articulation, bool overlapping_links, bool create_distance_constraints, PintActorHandle*
handles=null, float* half_height=null)
{
//	PintArticHandle Articulation = articulation ? pint.CreateArticulation() : null;
        return CreateRope(pint, articulation, p0, p1, nb_capsules, capsule_radius, capsule_mass, articulation,
overlapping_links, create_distance_constraints, handles, half_height);
//	if(Articulation)
//		pint.AddArticulationToScene(Articulation);
}*/
#endif

static bool CreateCapsuleRope(Pint& pint,
                              PintArticHandle articulation,
                              PintActorHandle parent,
                              float capsule_radius,
                              float half_height,
                              float capsule_mass,
                              float capsule_mass_for_inertia,
                              const Point& capsule_dir,
                              const Point& start_pos,
                              udword nb_capsules,
                              bool create_distance_constraints,
                              PintActorHandle* handles,
                              const PintCollisionGroup* collision_groups) {
    if (nb_capsules > MAX_LINKS) return false;
    ...
    for (udword i = 0; i < nb_capsules; i++) {
        ...
        GroupBit = 1 - GroupBit;
#endif
        if (collision_groups) ObjectDesc.mCollisionGroup = collision_groups[i];
    ...
    if (!articulation) {
        if (parent) {
            PintJointHandle JointHandle =
                    pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(parent, Handles[0], CapsuleOffset, -CapsuleOffset));
            ASSERT(JointHandle);
        }
        ...
        //		for(udword j=0;j<32;j++)
        {
            for (udword i = 1; i < nb_capsules; i++) {
                PintJointHandle JointHandle = pint.CreateJoint(
                        PINT_SPHERICAL_JOINT_CREATE(Handles[i - 1], Handles[i], CapsuleOffset, -CapsuleOffset));
                ASSERT(JointHandle);
            }
        }
    }
...
#pragma optimize("", off)
bool CreateCapsuleRope2(Pint& pint,
                        float& half_height,
                        PintArticHandle articulation,
                        PintActorHandle parent,
                        const Point& p0,
    ...
    return CreateCapsuleRope(pint, articulation, parent, capsule_radius, HalfHeight, capsule_mass,
                             capsule_mass_for_inertia, CapsuleDir, StartPos, nb_capsules, create_distance_constraints,
                             handles, collision_groups);
}
#pragma optimize("", on)
...
void CreateCaterpillarTrack_OLD(Pint& pint,
                                CaterpillarTrackObjects& objects,
                                const PINT_MATERIAL_CREATE* material,
                                const Point& pos,
                                const CylinderMesh& mCylinder,
                                PintShapeRenderer* cylinder_renderer,
                                const CylinderMesh& mCylinder2,
                                PintShapeRenderer* cylinder_renderer2,
                                udword multiplier) {
    ...
    for (udword i = 0; i < NbCurvePts; i++) {
        ...
        if (y > LimitP) y = LimitP;
        if (y < -LimitN) y = -LimitN;
    ...
    // the system explodes easily with regular joints & high counts when things are not properly setup (like not
    // physically possible). That's the case here with some settings that make the track impossible to actually wrap
    // around the gears, etc. So using articulations exposes that and explodes.
    // - then it looks like the default friction doesn't work well with this. It works much better with the other
    // friction modes....... Looks like a bug in PhysX we'll have to investigate & fix.
        ...
        for (udword i = 0; i < NbBoxes; i++) {
            ...
            if (i == 0 || i == NbBoxes - 1) {
                ...
                if (create.mIncludeSupport) {
                    ...
                    ObjectDesc.mMass = Mass;
                    ObjectDesc.mPosition += Point(Sign * (PlatformExtents.x + RampExtents.x),
                                                  -fabsf(sinf(RampAngle) * RampExtents.x), 0.0f);
                    Matrix3x3 RotRamp;
...

            {
                ...
                if (TestStatic) ObjectDesc.mMass = 0.0f;
                ObjectDesc.mPosition = Pos;
...
bool ClampAngularVelocity(Pint& pint, PintActorHandle wheel_parent, PintActorHandle wheel, float max_velocity) {
    ...
    if (Clamped) pint.SetAngularVelocity(wheel, RelativeVelocity + ParentAngularVelocity);
    return Clamped;
}
...
bool GetHingeTwistAngle(Pint& pint, PintJointHandle handle, float& angle) {
    ...
    if (!API->GetHingeDynamicData(handle, Data)) return false;
