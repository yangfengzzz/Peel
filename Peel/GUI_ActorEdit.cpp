...
EditJointSubWindow::EditJointSubWindow(const WindowDesc& desc, EditBoxInterface& owner)
    : IceWindow(desc),
      ...
      mPint(null),
      mHandle(null) {
    for (udword i = 0; i < PINT_JOINT_NB; i++) mJointParams[i] = null;
    ...
    udword YOffset = 4;
    mJointNameUI = CreateTextEditBox(owner, this, 4, YOffset, "Joint name:", JointNameLabelWidth, JointNameEditBoxWidth,
                                     EditBoxHeight, 1, "Default name");
    ...
    if (1) {
        ...
        mButton0 = GUI_CreateButton(this, 0, 300, YOffset, ButtonWidth, ButtonHeight, "Swap actors", null,
                                    Callbacks::OnSwapButton, this, null);
        mButton1 = GUI_CreateButton(this, 1, 400, YOffset, ButtonWidth, ButtonHeight, "Delete", null,
                                    Callbacks::OnDeleteButton, this, null);
    }
    ...
    for (udword i = 0; i < PINT_JOINT_NB; i++) {
        if (mJointParams[i]) mJointParams[i]->Init(owner, this, YStart[i]);
    }
}

EditJointSubWindow::~EditJointSubWindow() {
    for (udword i = 0; i < PINT_JOINT_NB; i++) DELETESINGLE(mJointParams[i]);
...
void EditJointSubWindow::SaveParams() {
    if (!mPint) return;  // Can be null if we never clicked on a joint
    ...
    if (mJointNameUI && mJointNameUI->mSomethingChanged) {
        ...
        if (JointAPI) {
            ...
            if (gEditActorWindow) gEditActorWindow->JointNameHasChanged(mHandle);
        }
    }
    ...
    // #### hmmm
    for (udword i = 0; i < PINT_JOINT_NB; i++) {
        if (mJointParams[i]) mJointParams[i]->SaveParams(JointAPI, mHandle, WakeUpActors);
    }
    ...
    if (WakeUpActors && JointAPI) {
        ...
        if (ActorAPI) {
            ...
            if (ah0) ActorAPI->WakeUp(ah0);
            if (ah1) ActorAPI->WakeUp(ah1);
        }
    }
}

void EditJointSubWindow::Cancel() {
    if (mJointNameUI) mJointNameUI->mSomethingChanged = false;
    if (mEditPos0) mEditPos0->ResetChangedState();
    if (mEditPos1) mEditPos1->ResetChangedState();

    for (udword i = 0; i < PINT_JOINT_NB; i++) {
        if (mJointParams[i]) mJointParams[i]->Cancel();
    }
}

bool EditJointSubWindow::SomethingChanged() const {
    if (mJointNameUI && mJointNameUI->mSomethingChanged) return true;
    if (mEditPos0 && mEditPos0->SomethingChanged()) return true;
    if (mEditPos1 && mEditPos1->SomethingChanged()) return true;

    for (udword i = 0; i < PINT_JOINT_NB; i++) {
        if (mJointParams[i])
            if (mJointParams[i]->SomethingChanged()) return true;
    }
...
void EditJointSubWindow::AskUserForSave() {
    if (SomethingChanged()) {
        int ret = IceCore::MessageBoxA(null, "Do you want to save your changes?",
                                       "Something changed in Edit Joint dialog", MB_OKCANCEL);
        if (ret == IDOK) SaveParams();
...
void EditJointSubWindow::InitFrom(Pint* pint, PintJointHandle handle, PintActorHandle owner) {
    ...
    {
        if (mActor0Label) mActor0Label->SetLabel("Actor0: (not available)");
        if (mActor1Label) mActor1Label->SetLabel("Actor1: (not available)");
    }
    ...
    {
        if (mJointNameUI) mJointNameUI->SetText(gNotAvailable);
        if (mTypeLabel) mTypeLabel->SetLabel("Joint type: (not available)");

        if (mEditPos0) mEditPos0->SetEnabled(false);
        if (mEditPos1) mEditPos1->SetEnabled(false);
    } else {
        ...
        if (mJointNameUI) mJointNameUI->SetText(JointName ? JointName : gNoName);

        if (mTypeLabel) mTypeLabel->SetLabel(_F("Joint type: %s", GetPintJointName(Type)));
        ...
        if (ActorAPI) {
            ...
            if (mActor0Label) mActor0Label->SetLabel(_F("Actor0: %s", Actor0Name ? Actor0Name : gNoName));
            if (mActor1Label) mActor1Label->SetLabel(_F("Actor1: %s", Actor1Name ? Actor1Name : gNoName));
        }
    ...
    if (mEditPos0) mEditPos0->SetVisible(HasLocalFrameEditBoxes);
    if (mEditPos1) mEditPos1->SetVisible(HasLocalFrameEditBoxes);
}

void EditJointSubWindow::ReleaseJoint() {
    if (!mPint || !mHandle) return;
...
void EditJointSubWindow::SwapActors() {
    ...
    if (mEditPos0) mEditPos0->SetPos(LocalFrame0.mPos);

    if (mEditPos1) mEditPos1->SetPos(LocalFrame1.mPos);
}
...
void EditJointSubWindow::Render() {
    if (!mPint || !mHandle || !IsVisible()) return;
    Pint_Joint* JointAPI = mPint->GetJointAPI();
    if (!JointAPI) return;
...
public:
    EAListBox(const ListBoxDesc& desc, EditActorWindow& owner)
        : IceListBox(desc), mOwner(owner), mNbShapes(0), mNbJoints(0), mSelectedJointIndex(INVALID_ID) {}
    virtual ~EAListBox() {}
...
void EditActorWindow::Cancel() {
    if (mActorNameUI) mActorNameUI->mSomethingChanged = false;
    if (mActorMassUI) mActorMassUI->mSomethingChanged = false;
    if (mActorLinearDampingUI) mActorLinearDampingUI->mSomethingChanged = false;
    if (mActorAngularDampingUI) mActorAngularDampingUI->mSomethingChanged = false;
    if (mJointEditWindow) mJointEditWindow->Cancel();
}

bool EditActorWindow::SomethingChanged() const {
    if (mActorNameUI && mActorNameUI->mSomethingChanged) return true;
    if (mActorMassUI && mActorMassUI->mSomethingChanged) return true;
    if (mActorLinearDampingUI && mActorLinearDampingUI->mSomethingChanged) return true;
    if (mActorAngularDampingUI && mActorAngularDampingUI->mSomethingChanged) return true;
    if (mJointEditWindow && mJointEditWindow->SomethingChanged()) return true;
    return false;
}

void EditActorWindow::AskUserForSave() {
    if (SomethingChanged()) {
        const int ret = IceCore::MessageBoxA(null, "Do you want to save your changes?",
                                             "Something changed in Edit Actor dialog", MB_OKCANCEL);
        if (ret == IDOK) SaveParams();
        ...
        if (NbShapes != INVALID_ID) {
            ...
            if (ShapeAPI) {
                for (udword i = 0; i < NbShapes; i++) {
                    ...
                    if (!ShapeName) ShapeName = gNoName;
                    ...
                    list_box->Add(_F("Shape %d (0x%x) - %s - %s - %s (%d)", i, size_t(sh), ShapeTypeName, ShapeName,
                                     RendererName, NbRenderers));
        ...
        if (NbJoints != INVALID_ID) {
            ...
            if (JointAPI) {
                for (udword i = 0; i < NbJoints; i++) {
                    ...
                    if (!JointName) JointName = gNoName;
...
void EditActorWindow::InitFrom(Pint* pint, PintActorHandle handle) {
    ...
    if (mActorNameUI) mActorNameUI->SetText(ActorName);
    ...
    if (mShapesAndJoints) mShapesAndJoints->RemoveAll();
    ...
    if (mJointEditWindow) {
        ...
        if (!pint && !handle) mJointEditWindow->InitFrom(null, null, null);
    }
}
...
void EditActorWindow::SaveParams() {
    ...
    if (mJointEditWindow) mJointEditWindow->SaveParams();
}

int EditActorWindow::handleEvent(IceGUIEvent* event) {
    if (event->mType == EVENT_CLOSE) AskUserForSave();
    return 0;
}

void EditActorWindow::PreRenderCallback() {
    if (mJointEditWindow) mJointEditWindow->Render();
}
...
EditActorWindow* CreateActorEditGUI(IceWidget* parent, sdword x, sdword y) {
    ...
    sdword YOffset = 4;
    {
        ...
        EAW->mActorNameUI = CreateTextEditBox(*EAW, EAW, OffsetX, YOffset, "Name:", NameLabelWidth, NameEditBoxWidth,
                                              EditBoxHeight, 0, "Default name");
        YOffset += EditBoxHeight;

        EAW->mShapesAndJointsLabel =
                GUI_CreateLabel(EAW, OffsetX, YOffset + 2, Width - OffsetX * 2 - 8, 20, "Nb shapes:", null);
        YOffset += 20;

        const udword NewLabelWidth = 100;
        EAW->mActorMassUI = CreateFloatEditBox(*EAW, EAW, OffsetX, YOffset, "Mass:", NewLabelWidth, FloatEditBoxWidth,
                                               EditBoxHeight, 0, 0.0f);
        YOffset += EditBoxHeight;

        EAW->mActorLinearDampingUI = CreateFloatEditBox(*EAW, EAW, OffsetX, YOffset, "Linear damping:", NewLabelWidth,
                                                        FloatEditBoxWidth, EditBoxHeight, 0, 0.0f);
        YOffset += EditBoxHeight;

        EAW->mActorAngularDampingUI = CreateFloatEditBox(*EAW, EAW, OffsetX, YOffset, "Angular damping:", NewLabelWidth,
                                                         FloatEditBoxWidth, EditBoxHeight, 0, 0.0f);
        YOffset += EditBoxHeight;

        EAW->mInfoLabel =
                GUI_CreateLabel(EAW, OffsetX, YOffset + 2, Width - OffsetX * 2 - 8, 20,
                                "Shape ID  -  Shape type  -  Shape name  -  Renderer name  -  Nb renderers:", null);
        YOffset += 22;
...
void CloseActorEditGUI() { DELETESINGLE(gEditActorWindow); }

void HideEditActorWindow() {
    if (!gEditActorWindow) return;
...
EditActorWindow* GetVisibleActorEditWindow() {
    if (!gEditActorWindow || !gEditActorWindow->IsVisible()) return null;
