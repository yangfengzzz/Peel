...
static const char* gDesc_DynamicConvexVsVeryLargeStaticSphere =
        "A dynamic convex vs a very large static sphere. The sphere is so large that it looks more like a plane. \
Most engines are not prepared for such a case and objects jitter on top of the sphere...";

START_TEST(DynamicConvexVsVeryLargeStaticSphere,
           CATEGORY_CONTACT_GENERATION,
           gDesc_DynamicConvexVsVeryLargeStaticSphere)
...
static const char* gDesc_ThinCapsuleOnBox =
        "Very thin capsule on static box. This is a stress test for the capsule-box contact generation.";
...
static const char* gDesc_ThinCapsuleOnBoxes =
        "Very thin capsule on static boxes. This is a stress test for the capsule-box contact generation.";
...
static const char* gDesc_PCM_Stress =
        "Stress test for the Persistent Contact Manifold (PCM) algorithm. Some engines using PCM can have a hard time with that test (at least with CCD disabled). \
...
static const char* gDesc_BoxSlidingOnPlanarMesh =
        "A box sliding on a tessellated planar mesh. This is to test how each engine handles collisions with 'internal edges'. \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    PintActorHandle CubeHandle =
            CreateDynamicBox(pint, 1.0f, 1.0f, 1.0f, Point(0.0f, 40.0f, -41.0f), &ObjectDesc.mRotation, &MatDesc);
    ASSERT(CubeHandle);
    return true;
}
...
static const char* gDesc_InternalEdges =
        "An object sliding on a tessellated curved mesh. This is to test how each engine handles collisions with 'internal edges'. \
...
static const udword gPreset_NbX[NB_PRESETS] = {4, 4, 4, 16, 4, 4, 4, 16, 4, 4, 4, 16, 4, 4, 4, 16};
static const float gPreset_ScaleX[NB_PRESETS] = {0.1f, 0.1f, 0.01f, 0.01f, 0.1f, 0.1f, 0.01f, 0.01f,
...
static const PintShape gPreset_Shape[NB_PRESETS] = {
        ...
        PINT_SHAPE_CAPSULE, PINT_SHAPE_CAPSULE, PINT_SHAPE_CAPSULE, PINT_SHAPE_CAPSULE};
static const float gPreset_PosZ[NB_PRESETS] = {-55.0f, -54.0f, -55.0f, -54.0f, -55.0f, -54.0f, -55.0f, -54.0f,
                                               -55.0f, -54.0f, -55.0f, -54.0f, -55.0f, -54.0f, -55.0f, -54.0f};
static const bool gPreset_RotateShape[NB_PRESETS] = {false, false, false, false, false, false, false, false,
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        const bool Enabled = false;
        {
            ...
            mEditBox_NbX = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "4", &UIElems,
            ...
            mEditBox_NbY = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "32", &UIElems,
                                                EDITBOX_INTEGER_POSITIVE, null, null);
            mEditBox_NbY->SetEnabled(Enabled);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "ScaleX:", &UIElems);
            mEditBox_ScaleX = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1", &UIElems,
            ...
            mEditBox_ScaleZ = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1", &UIElems,
                                                   EDITBOX_FLOAT_POSITIVE, null, null);
            ...
            mComboBox_ShapeType = CreateShapeTypeComboBox(UI, 4 + OffsetX, y, Enabled,
            ...
            mEditBox_ShapePosX = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
            ...
            mEditBox_ShapePosY = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "80.0", &UIElems,
                                                      EDITBOX_FLOAT, null, null);
            mEditBox_ShapePosY->SetEnabled(Enabled);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Shape pos z:", &UIElems);
            mEditBox_ShapePosZ = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "-55.0", &UIElems,
                                                      EDITBOX_FLOAT, null, null);
            mEditBox_ShapePosZ->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_RotateShape =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Rotate shape", &UIElems, false, null, null);
        ...
        }
        {
            ...
            public:
                MyComboBox(const ComboBoxDesc& desc, InternalEdges& test) : IceComboBox(desc), mTest(test) {}
                virtual ~MyComboBox() {}
...
static const char* gDesc_VeryLargeTriangle =
        "Objects on a very large triangle. Used to test accuracy limits of various contact routines.";
...
static const char* gDesc_SphereMeshUnitTest =
        "Sphere-vs-mesh unit test. A repro for an old contact generation bug in PhysX.";
...
virtual bool CommonSetup() {
    ...
    const float verts[] = {477.0f, 50.811596f, 853.0f, 476.0f, 50.858505f, 852.0f, 476.0f, 48.431885f, 854.0f,
...
static const char* gDesc_SphereMeshUnitTest_FC =
        "Sphere-vs-mesh unit test. Face contact. The sphere center initially exactly overlaps the triangle to test \
...
static const char* gDesc_SphereMeshUnitTest_VC =
        "Sphere-vs-mesh unit test. Vertex contact. This should generate a single vertical contact.";
...
static const char* gDesc_SphereMeshUnitTest_EC =
        "Sphere-vs-mesh unit test. Edge contact. This should generate a single vertical contact. \
...
virtual bool CommonSetup() {
    ...
    for (udword i = 0; i < 8; i++) Pts[i] += gOffset;

    for (udword i = 0; i < 4; i++) Pts[8 + i] = Point(Pts[i].x, Pts[i].y, Pts[i].z + Size * 4.0f);
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    if (!CreateMeshesFromRegisteredSurfaces(pint, caps, &Material)) return false;
...
/*static*/ void DecomposeMeshWithHACD(
        Pint& pint, udword nb, const Point* pos, const SurfaceInterface& surface, bool dynamic, bool use_render_mesh);
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportRigidBodySimulation || !caps.mSupportDynamicMeshes) return false;
    ...
    for (udword i = 0; i < NbMeshes; i++) {
        ...
        if (0) {
            ...
            MeshCreate.SetSurfaceData(Part->GetNbVerts(), Part->GetVerts(), Part->GetNbTris(), Part->GetIndices(),
                                      null);
            ...
            if (0 && i == 0) {
                ...
                for (udword j = 0; j < NbSpheres; j++) {
                    ...
                    if (j != NbSpheres - 1) SphereCreate[j].SetNext(&SphereCreate[j + 1]);
                    // else
                ...
                //				ObjectDesc.mPosition	= Point(float(i)*3.0f, 0.0f, 0.0f);
                ObjectDesc.mPosition = Part->mPos;
                if (i == 0) ObjectDesc.mPosition.y += 1.8f * gSceneScale;
                CreatePintObject(pint, ObjectDesc);
            }
        }
    }
