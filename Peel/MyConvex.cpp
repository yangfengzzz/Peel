...
MyConvex::MyConvex() : mNbVerts(0), mVerts(NULL), mNbPolys(0), mPolys(NULL) {}

MyConvex::~MyConvex() { Release(); }
...
bool MyConvex::LoadFromFile(const char* filename) {
    ...
    Scale = 1.0f / Scale;
    for (int i = 0; i < mNbVerts; i++) mVerts[i] *= Scale;
...
bool MyConvex::LoadFile(udword i) {
    ...
    ASSERT(status);
    if (!status) printf("Failing to load convex object!\n");
    return status;
}

void MyConvex::Scale(float s) {
    for (int i = 0; i < mNbVerts; i++) mVerts[i] *= s;
}
