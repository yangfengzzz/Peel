...
CollisionGroupsData::~CollisionGroupsData() { DELETEARRAY(mGroups); }
...
SceneData::SceneData(const PINT_WORLD_CREATE& desc, const char* scene_desc)
    ...
    if (desc.GetTestName()) mName.Set(desc.GetTestName());
    //	if(desc.GetTestDesc())
    //		mDesc.Set(desc.GetTestDesc());
    if (scene_desc) mDesc.Set(scene_desc);
}
...
ActorData::ActorData(const PINT_OBJECT_CREATE& desc)
    ...
    if (desc.mName) mName.Set(desc.mName);
}

MaterialData::MaterialData(const PINT_MATERIAL_CREATE& desc)
    : mStaticFriction(desc.mStaticFriction), mDynamicFriction(desc.mDynamicFriction), mRestitution(desc.mRestitution) {
    if (desc.mName) mName.Set(desc.mName);
}
...
SphereShapeData::SphereShapeData(const PINT_SPHERE_CREATE& desc) : ShapeData(desc), mRadius(desc.mRadius) {}

CapsuleShapeData::CapsuleShapeData(const PINT_CAPSULE_CREATE& desc)
    : ShapeData(desc), mRadius(desc.mRadius), mHalfHeight(desc.mHalfHeight) {}

CylinderShapeData::CylinderShapeData(const PINT_CYLINDER_CREATE& desc)
    : ShapeData(desc), mRadius(desc.mRadius), mHalfHeight(desc.mHalfHeight) {}

BoxShapeData::BoxShapeData(const PINT_BOX_CREATE& desc) : ShapeData(desc), mExtents(desc.mExtents) {}

ConvexShapeData::ConvexShapeData(const PINT_CONVEX_CREATE& desc) : ShapeData(desc) {
    mNbVerts = desc.mNbVerts;
    mVerts = ICE_NEW(Point)[desc.mNbVerts];
    CopyMemory(mVerts, desc.mVerts, desc.mNbVerts * sizeof(Point));
}

ConvexShapeData::~ConvexShapeData() { DELETEARRAY(mVerts); }

MeshShapeData::MeshShapeData(const PINT_MESH_CREATE& desc) : ShapeData(desc) {
...
MeshShapeData2::MeshShapeData2(const PINT_MESH_CREATE2& desc) : ShapeData(desc), mMeshData(null) {
...
ConvexMeshData::~ConvexMeshData() { DELETEARRAY(mVerts); }
...
JointData::JointData(const PINT_JOINT_CREATE& desc)
    : mType(desc.mType), mObject0((ActorData*)desc.mObject0), mObject1((ActorData*)desc.mObject1) {
    if (desc.mName) mName.Set(desc.mName);
}

SphericalJointData::SphericalJointData(const PINT_SPHERICAL_JOINT_CREATE& desc)
    : JointData(desc), mLocalPivot0(desc.mLocalPivot0), mLocalPivot1(desc.mLocalPivot1), mLimits(desc.mLimits) {}

FixedJointData::FixedJointData(const PINT_FIXED_JOINT_CREATE& desc)
    : JointData(desc), mLocalPivot0(desc.mLocalPivot0), mLocalPivot1(desc.mLocalPivot1) {}
...
AggregateData::AggregateData(udword max_size, bool enable_self_collision)
    : mMaxSize(max_size), mSelfCollision(enable_self_collision), mAddedToScene(false) {}
