...
static SelectedObject* ReplaceSelectionWith(SelectionManager& manager, Pint* pint, udword index, PintActorHandle h) {
    ...
    // Add merged object to selection to give an immediate visual feedback to users (they "see" that the merge
    // succeeded)
...
ReportActorCallback::ReportActorCallback(SelectionManager& manager,
                                         Pint* engine,
                                         udword index,
                                         bool ignore_hidden_objects)
    : mSelMan(manager),
      mEngine(engine),
      mEngineIndex(index),
...
ReportActorCallback::~ReportActorCallback() {
    if (mHideActorWindow) UI_HideEditWindows();
}

bool ReportActorCallback::ReportObject(PintActorHandle h) {
    ASSERT(mEngine);
    // Ignore hidden objects
    if (mIgnoreHiddenObjects && mEngine->mVisHelper && !mEngine->mVisHelper->IsRenderable2(h)) return true;
...
void DefaultControlInterface::LeftDragCallback(const MouseInfo& mouse) {
    ...
    if (gMenuIsVisible || !mValidateLeftDownCB || mLockSelection) return;
    mLeftDrag = true;
    ...
    if (RectangleSelection) {
        ...
        if (!CurrentTool || !CurrentTool->IsControllingCamera()) RotateCamera(mouse.mRelMouseX, mouse.mRelMouseY);
    }
}
...
void DefaultControlInterface::LeftUpCallback(const MouseInfo& mouse) {
    ...
    if (gMenuIsVisible || !mValidateLeftDownCB) return;
    //	printf("gLeftDrag: %d\n", gLeftDrag);
    gRunningTest->LeftUpCallback(mouse);

    if (mLockSelection) return;
...
void DefaultControlInterface::MiddleUpCallback(const MouseInfo& mouse) {
    ...
    if (0 && mMouseX == mouse.mMouseX && mMouseY == mouse.mMouseY) DoPopupMenu(mouse);
...
bool DefaultControlInterface::RightDownCallback(const MouseInfo& mouse) {
    ...
    if (IsKeyPressed(VK_CONTROL) /* && gMouseX==mouse.mMouseX && gMouseY==mouse.mMouseY*/)
        ...
        if (CurrentTool) {
            ...
            const udword NbEngines = GetNbEngines();
            for (udword i = 0; i < NbEngines; i++) {
                if (ValidateEngine(i)) CurrentTool->RightDownCallback(*GetEngine(i), i);
            }
        }
    }
    return true;
...
void DefaultControlInterface::RightDragCallback(const MouseInfo& mouse) {
    ...
    if (CurrentTool) {
        ...
        const udword NbEngines = GetNbEngines();
        for (udword i = 0; i < NbEngines; i++) {
            if (ValidateEngine(i)) CurrentTool->RightDragCallback(*GetEngine(i), i);
        }
    }
}
...
void DefaultControlInterface::RightUpCallback(const MouseInfo& mouse) {
    ...
    if (CurrentTool) {
        ...
        const udword NbEngines = GetNbEngines();
        for (udword i = 0; i < NbEngines; i++) {
            if (ValidateEngine(i)) CurrentTool->RightUpCallback(*GetEngine(i), i);
        }
    }
}
...
void DefaultControlInterface::RightDblClkCallback(const MouseInfo& mouse) {
    ...
    if (CurrentTool) {
        const udword NbEngines = GetNbEngines();
        for (udword i = 0; i < NbEngines; i++) {
            if (ValidateEngine(i)) CurrentTool->RightDblClkCallback(*GetEngine(i), i);
        }
    }
}
...
void DefaultControlInterface::MouseMoveCallback(const MouseInfo& mouse) {
    ...
    {
        ToolInterface* CurrentTool = GetCurrentTool();
        if (CurrentTool) CurrentTool->MouseMoveCallback(mouse);
    }
}
...
bool DefaultControlInterface::KeyboardCallback(unsigned char key, int x, int y, bool down) {
    ...
    if (key == 127) mSelMan.DeleteSelected();
    ...
    // if(gUseEditor)
    {
        ...
        // ### this doesn't work for the same reason, IsKeyPressed returns true but the key isn't 'a' anymore (it's 1
        // for some reason)
...
void DefaultControlInterface::OnButtonEvent(udword button_id, bool down) {
    if (gMenuIsVisible) return;
    if (gRunningTest) gRunningTest->OnButtonEvent(button_id, down);
}

void DefaultControlInterface::OnAnalogButtonEvent(udword button_id, ubyte old_value, ubyte new_value) {
    if (gMenuIsVisible) return;
    if (gRunningTest) gRunningTest->OnAnalogButtonEvent(button_id, old_value, new_value);
}
...
void DefaultControlInterface::OnAxisEvent(udword axis_id, float value) {
    ...
    if (gRunningTest) gRunningTest->OnAxisEvent(axis_id, value);
}
...
void DefaultControlInterface::CallRenderCallbacks(GUI_RenderInterface& ri, PintRender& render) const {
    ...
    const udword NbEngines = GetNbEngines();
    for (udword i = 0; i < NbEngines; i++) {
        if (ValidateEngine(i)) ri.RenderCallback(render, *GetEngine(i), i);
    }
...
void DefaultControlInterface::RenderCallback(PintRender& render) {
    ...
    {
        ToolInterface* CurrentTool = GetCurrentTool();
        if (CurrentTool) CallRenderCallbacks(*CurrentTool, render);
    }
    ...
    // only have 2 of these windows in the system though, so hardcode will do.
    {
        ...
        if (CCW) CallRenderCallbacks(*CCW, render);

        EditActorWindow* EAW = GetVisibleActorEditWindow();
        if (EAW) CallRenderCallbacks(*EAW, render);
    }

    {
        ...
        for (udword i = 0; i < NbSelected; i++) {
            if (ValidateEngine(Selected->mEngineIndex)) DrawBoxCorners(*Selected->mEngine, Selected->mActor, Color);
                    ...
                    for(udword i=0;i<NbEngines;i++)
                    {
                            if(ValidateEngine(i))
                            {
                                    ...
                                    if(P==GetEditorPlugin()->GetPint())
                                    {
                                            ...
                                            {
                                                    SelectedObject* Selected = AddToSelection(P,
       PintActorHandle(AllActors.GetEntry(j)), i);
                                            }
                                    }
                            }
                    }
...
HideUnselectedCallback::HideUnselectedCallback(SelectionManager& manager, Pint* engine)
    : mSelMan(manager), mEngine(engine) {
...
bool HideUnselectedCallback::ReportObject(PintActorHandle h) {
    ...
    if (mNeedsTranslation) h = Editor_GetNativeHandle(h);
...
SelectHiddenCallback::SelectHiddenCallback(SelectionManager& manager, Pint* engine, udword index)
    : mSelMan(manager), mEngine(engine), mEngineIndex(index), mHideActorWindow(false) {}
...
bool SelectHiddenCallback::ReportObject(PintActorHandle h) {
    ...
    if (mEngine->mVisHelper && mEngine->mVisHelper->IsRenderable2(h)) return true;
...
udword DefaultControlInterface::GetNbHidden() const {
    ...
    for (udword i = 0; i < NbEngines; i++) {
        if (ValidateEngine(i)) {
            Pint* P = GetEngine(i);
            if (P->mVisHelper) Nb += P->mVisHelper->GetNbHidden();
        }
    }
    return Nb;
}
...
ShowAllCallback::ShowAllCallback(Pint* engine) : mEngine(engine) {}
...
Pint* DefaultControlInterface::CanMergeSelected(udword& index) const {
    ...
    while (NbSelected--) {
        if (!P) {
            ...
            if (P != Selected->mEngine || Index != Selected->mEngineIndex) return null;
        }
        Selected++;
    }
    index = Index;
...
void DefaultControlInterface::SetSelectionEnabled(bool enabled) {
    ...
    for (udword i = 0; i < NbSelected; i++) {
        ...
        if (API) API->SetSimulationFlag(Selected->mActor, enabled);
        Selected++;
    }
}
...
void DefaultControlInterface::CreateCompound(CompoundCreateWindow& ccw) {
    String CompoundName;
    if (ccw.mNameEditBox) ccw.mNameEditBox->GetTextAsString(CompoundName);

    const float Mass = GetFloat(-1.0f, ccw.mMassEditBox);

    PintActorHandle h = Editor_CreateCompound(ccw.mHandles.GetNbEntries(), (PintActorHandle*)ccw.mHandles.GetEntries(),
                                              MergeParams(CompoundName, Mass, &ccw.mCompoundPR));
    /*	if(1)
            {
                    mSelMan.DeleteSelected();
                    // Add merged object to selection to give an immediate visual feedback to users (they "see" that the
       merge succeeded) SelectedObject* Selected = mSelMan.AddToSelection(ccw.mPint, h, ccw.mIndex);
            }*/
...
void DefaultControlInterface::CreateJoints() { ShowEditJointWindow(); }
...
void DefaultControlInterface::DoPopupMenu(const MouseInfo& mouse) {
    if (0) {
        ...
        for (udword i = 0; i < NbTools; i++) AddPopupMenuEntry(Menu, GetToolName(i), i, i == CurrentToolIndex, false);
        ...
        HMENU SelectionSubMenu = StartPopupMenu();
        {
            ...
            IceCore::AppendMenu(SelectionSubMenu, MF_SEPARATOR, 0, 0);
            AddPopupMenuEntry(SelectionSubMenu, "Merge to compound", SELECTION_MERGE_COMPOUND, false,
                              !gUseEditor || NbSelected < 2);
            AddPopupMenuEntry(SelectionSubMenu, "Merge to compound Ex", SELECTION_MERGE_COMPOUND2, false,
                              !gUseEditor || NbSelected < 2);
            AddPopupMenuEntry(SelectionSubMenu, "Merge to single convex", SELECTION_MERGE_TO_CONVEX, false,
                              !gUseEditor || NbSelected < 2);
            AddPopupMenuEntry(SelectionSubMenu, "Merge to single mesh", SELECTION_MERGE_TO_MESH, false,
                              !gUseEditor || NbSelected < 2);
            IceCore::AppendMenu(SelectionSubMenu, MF_SEPARATOR, 0, 0);
            AddPopupMenuEntry(SelectionSubMenu, "Parent orphan joints", SELECTION_PARENT_ORPHAN_JOINTS, false,
                              !gUseEditor || !NbSelected);
        ...
        AddPopupMenuEntry(Menu, _F("Delete %d selected object(s) (Del)", NbSelected), SELECTION_DELETE, false,
                          !Enabled.mDeleteSelection);
        AddPopupMenuEntry(Menu, "Edit selected (Left dbl-clk on object)", SELECTION_EDIT, false,
                          !Enabled.mEditSelection);
        ...
        if (Choice < NbTools) {
            ...
            /*			if(0)
                                    {
                                            ...
                                            Editor_HideActors(EditorObjects.GetNbEntries(),
               (PintActorHandle*)EditorObjects.GetEntries());
                                    }
                                    else
                                    {
            ...
            if (P) {
                PtrContainer EditorObjects;
                GetSelectedEditorObjects(EditorObjects);
                PintActorHandle h = Editor_CreateCompound(EditorObjects.GetNbEntries(),
                                                          (PintActorHandle*)EditorObjects.GetEntries(), MergeParams());
                /*				if(1)
                                                {
                                                mSelMan.DeleteSelected();
                                                // Add merged object to selection to give an immediate visual feedback
                   to users (they "see" that the merge succeeded) SelectedObject* Selected = mSelMan.AddToSelection(P,
                   h, Index);
                                                }*/
            ...
            if (P) {
                PtrContainer EditorObjects;
                GetSelectedEditorObjects(EditorObjects);
                ShowCompoundCreateWindow(P, Index, EditorObjects.GetNbEntries(),
                                         (PintActorHandle*)EditorObjects.GetEntries(), this);
            } else
                IceCore::MessageBoxA(0, "Cannot merge objects from different engines.", "Error", MB_OK);
            ...
            if (P) {
                PtrContainer EditorObjects;
                GetSelectedEditorObjects(EditorObjects);
                PintActorHandle h = Editor_CreateSingleConvex(EditorObjects.GetNbEntries(),
                                                              (PintActorHandle*)EditorObjects.GetEntries());
                /*				if(1)
                                                {
                                                mSelMan.DeleteSelected();
            ...
            if (P) {
                PtrContainer EditorObjects;
                GetSelectedEditorObjects(EditorObjects);
                PintActorHandle h = Editor_CreateSingleMesh(EditorObjects.GetNbEntries(),
                                                            (PintActorHandle*)EditorObjects.GetEntries());
                ReplaceSelectionWith(mSelMan, P, Index, h);
            } else
                IceCore::MessageBoxA(0, "Cannot merge objects from different engines.", "Error", MB_OK);
            ...
            if (NbOrphanJoints) ShowActorSelectionWindow(this);
        ...
        /*		else if(Choice==SELECTION_KINEMATIC)
                        {
                                ...
                                {
                                        public:
                                        virtual	PintActorHandle	ModifySelected(PintActorHandle h)	{ return
           Editor_MakeKinematic(h);	} }MI;
