...
public:
    ...
    ASWListBox(const ListBoxDesc& desc, ItemSelectionWindow& owner)
        : IceListBox(desc), mOwner(owner), mSelectedActor(null), mSelectedShape(null), mSelectedPint(null) {}
    virtual ~ASWListBox() {}
...
ItemSelectionWindow::ItemSelectionWindow(const WindowDesc& desc)
    : IceWindow(desc), mActorCallback(null), mShapeCallback(null), mUserData(null), mActorOwner(null) {}
...
void ItemSelectionWindow::OnOK() {
    ...
    if (mActorCallback) mActorCallback->OnSelectedActor(MyListBox->mSelectedPint, MyListBox->mSelectedActor, mUserData);

    if (mShapeCallback) mShapeCallback->OnSelectedShape(MyListBox->mSelectedShape, mUserData);
}
...
ReportActorCallback::ReportActorCallback(Pint& pint, ASWListBox& list_box, udword engine_index)
    : mPint(pint), mListBox(list_box), mEngineIndex(engine_index), mID(0) {
...
bool ReportActorCallback::ReportObject(PintActorHandle h) {
    ...
    if (IsDefaultEnv(mPint, h)) return true;

    const char* ActorName = null;
    if (mActorAPI) ActorName = mActorAPI->GetName(h);

    if (!ActorName) ActorName = gNoName;
    ...
    if (NbShapes != INVALID_ID) {
        ...
        if (ShapeAPI) {
            for (udword i = 0; i < NbShapes; i++) {
                ...
                mItems->Add(_F("Shape %d (0x%x) - %s - %s - %s (%d)", i, size_t(sh), ShapeTypeName, ShapeName,
...
void CloseItemSelectionWindow() { DELETESINGLE(gItemSelectionWindow); }

void HideItemSelectionWindow() {
    if (!gItemSelectionWindow) return;
...
ItemSelectionWindow* GetVisibleItemSelectionWindow() {
    if (!gItemSelectionWindow || !gItemSelectionWindow->IsVisible()) return null;
