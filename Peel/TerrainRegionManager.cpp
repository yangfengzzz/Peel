...
void TerrainRegionManager::PreUpdate(const AABB& streaming_bounds) { streaming_bounds.GetCenter(mFocus); }
...
void TerrainRegionManager::PostUpdate(udword timestamp) {
    ...
    if (timestamp == 0 && mParams.mCreateAllInitialTiles) {
        for (udword i = 0; i < Nb; i++) AddRegionInternal(mQueue[i]);
        mQueue.clear();
    } else {
        // ######## test
        //		const Point CamPos = GetCameraPos();
        ...
        for (udword i = 0; i < NbAddedPerFrame; i++) {
            ...
            AddRegionInternal(R);
            ASSERT(R.mUserData);
            R.mUserData = null;  // Mark for removal below, we cannot remove immediately since it would invalidate
                                 // sorted indices
        }
        ...
        while (NbToGo--) {
            if (!mQueue[i].mUserData) {
                ...
                NbRemoved++;
                if (NbRemoved == NbAddedPerFrame) break;
            } else
                i++;
        }
    }
...
void TerrainRegionManager::AddRegionInternal(const StreamRegion& region) {
    ...
    */
    ...
    if (1) {
        ...
        if (1) {
            CreateProceduralTerrain(V, RD->mVertexNormals, NbU, NbV, region.mCellBounds.mMin, region.mCellBounds.mMax,
                                    mFBM, C);
        ...
        if (0) {
            const SurfaceInterface& surface = IS.GetSurfaceInterface();
            BuildSmoothNormals(surface.mNbFaces, surface.mNbVerts, surface.mVerts, surface.mDFaces, surface.mWFaces,
                               RD->mVertexNormals, true);
        }
...
#ifdef USE_TERRAIN_TILE_RENDERER
        PintTerrainTileRenderer* TTR = ICE_NEW(PintTerrainTileRenderer)(mIndexBuffers, mdc.GetSurface().mNbVerts,
                                                                        mdc.GetSurface().mVerts, RD->mVertexNormals);
        MeshDesc.mRenderer = TTR;
        if (gEnableLOD) SetRegionLOD(region, TTR);
#else
    ...
    if (0) {
        ...
        /*				const float CoeffX = float(i)/float(NbU-1);
                                        const float x = region.mCellBounds.mMin.x + CoeffX * (region.mCellBounds.mMax.x
           - region.mCellBounds.mMin.x);

                                                const float CoeffZ = float(j)/float(NbV-1);
                                                const float z = region.mCellBounds.mMin.z + CoeffZ *
           (region.mCellBounds.mMax.z - region.mCellBounds.mMin.z); float val = Evaluate(mFBM, ScaleY, Scale, x, z);*/
...
void TerrainRegionManager::UpdateRegion(const StreamRegion& region) {
    ...
    if (RD->mInQueue) return;

#ifdef USE_TERRAIN_TILE_RENDERER
    if (gEnableLOD) SetRegionLOD(region, RD->mTTR);
