...
public:
    ...
    inline_ bool Equal(const Data& p) const {
        // We use CRCs to avoid keeping an extra copy of verts & tris data here (and to avoid memcmp). The risk of
        // collisions is probably acceptable?
        return mData.mFlags == p.mFlags && mData.mNbVerts == p.mNbVerts && mData.mNbFaces == p.mNbFaces &&
               mData.mCRC32_Verts == p.mCRC32_Verts && mData.mCRC32_Faces == p.mCRC32_Faces;
    };
