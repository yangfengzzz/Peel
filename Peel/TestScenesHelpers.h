...
PintActorHandle CreateSimpleObject(Pint& pint,
                                   ...
                                   const Point& pos,
                                   const Quat* rot = null,
                                   const Point* linVel = null,
                                   const Point* angVel = null,
...
PintActorHandle CreateStaticObject(
        Pint& pint, const PINT_SHAPE_CREATE* shape, const Point& pos, const Quat* rot = null, const char* name = null);
PintActorHandle CreateDynamicBox(Pint& pint,
                                 ...
                                 const Point& pos,
                                 const Quat* rot = null,
                                 const PINT_MATERIAL_CREATE* material = null);

void CreateSingleTriangleMesh(SurfaceManager& test, float scale, Triangle* tri = null, bool reverse_winding = false);
bool CreateBoxStack(
        Pint& pint, const PintCaps& caps, const udword nb_stacks, udword nb_base_boxes, bool use_convexes = false);
...
bool CreateSeaOfStaticBoxes(Pint& pint,
                            ...
                            PtrContainer* created_objects = null);
...
bool CreateArrayOfDynamicConvexes(Pint& pint,
                                  ...
                                  float scale_x,
                                  float scale_y,
                                  const Point* offset = null,
                                  const Point* lin_vel = null);
...
/*	bool CreateCapsuleRope(	Pint& pint, PintActorHandle articulation, PintActorHandle parent,
                                                        float capsule_radius, float half_height, float capsule_mass,
   float capsule_mass_for_inertia, const Point& capsule_dir, const Point& start_pos, udword nb_capsules, bool
   create_distance_constraints, PintActorHandle* handles, const PintCollisionGroup* collision_groups=null);*/
bool CreateCapsuleRope2(Pint& pint,
                        ...
                        const PintCollisionGroup* collision_groups = null);
...
struct CaterpillarTrackObjects : public Allocateable {
    ...
    void OnObjectReleased(PintActorHandle removed_object) {
        if (mChassis == removed_object) mChassis = null;
        for (udword i = 0; i < mNbGears; i++) {
            if (mGears[i] == removed_object) mGears[i] = null;
        }
    }
};
