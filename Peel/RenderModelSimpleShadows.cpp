...
bool RenderModel_SimpleShadows::HasSpecialGroundPlane() const { return true; }
...
void RenderModel_SimpleShadows::InitScene(const Point& c, float size) {
    ...
    if (moveLightsWithTarget) gLightPos[0] += center;
    ...
    if (moveLightsWithTarget) gLightPos[1] += center;
    ...
    if (moveLightsWithTarget) gLightPos[2] += center;
...
static void RenderScene_(/*SimpleViewerCallbacks& callbacks, */ PintRenderPass render_pass, bool useShader) {
    ...
    if (useShader) {
        if (gPhongPower != gCurrentPhongPower) {
            ...
            gEnvTexId = GLTexture::CreateTexture(256, 256, EnvMap.GetPixels(), true);
            if (gEnvST) gEnvST->mGLID = gEnvTexId;
        }
    ...
    if (useShader) gDefaultShader->Deactivate();
...
static void RenderShadowCasters_() {
    ...
    if (gFlatShader) gFlatShader->Deactivate();

    // gWireframePass = false;
    gShadowPass = false;
}

void RenderModel_SimpleShadows::Render(
        udword width, udword height, const Point& cam_pos, const Point& cam_dir, float fov) {
    // shadows
    if (gNumShadows > 0) {
                        ...
                        else*/
        {
            ...
            for (int i = 0; i < gNumLights; i++) {
                ...
                if (i < gNumShadows && gShadowMaps[i]) {
                    if (!gShadowMaps[i])
                        gShadowMaps[i] =
                                new ShadowMap(gShadowMapWidth, gShadowMapHeight, fov, i, gShadowMapRes, gNbSplits);

                    gDefaultShader->SetShadowMap(i, gShadowMaps[i]);

                    //				gCallbacks = &callbacks;
                    gShadowMaps[i]->makeShadowMap(cam_pos, cam_dir, gLightDirs[i], gShadowClipNear, gShadowClipFar,
                                                  &RenderShadowCasters_);
                }
            }
#ifdef PEEL_USE_MSAA
            if (g_msaaFbo) glBindFramebuffer(GL_FRAMEBUFFER, g_msaaFbo);
#endif
        }
    }
    ...
    for (int i = 0; i < gNumLights; i++) gDefaultShader->SetSpotLight(i, gLightPos[i], gLightDirs[i]);
    ...
    if (gShowReflections) {
        ...
        gReflectedSceneTarget->BeginCapture();
        {
            ...
            if (w != width || h != height) glViewport(0, 0, w, h);
        ...
        if (w != width || h != height) glViewport(0, 0, width, height);
...
void RenderModel_SimpleShadows::UpdateCamera(bool mirrored) {
    if (mirrored) {
        ...
        const float matGL[16] = {r0.x, r1.x, r2.x, 0.0f, r0.y, r1.y, r2.y, 0.0f,
                                 r0.z, r1.z, r2.z, 0.0f, t.x,  t.y,  t.z,  1.0f};
...
void RenderModel_SimpleShadows::SetupCurrentModelMatrix(const float* m) {
    if (gDefaultShader) gDefaultShader->SetWorldMatrix(m);
}
...
static inline_ udword GetMeshFlags(bool active_edges, bool direct_data) {
    ...
    if (active_edges) Flags |= DL_MESH_USE_ACTIVE_EDGES;
    if (direct_data) Flags |= DL_MESH_USE_DIRECT_DATA;
    return Flags;
}

PintShapeRenderer* RenderModel_SimpleShadows::_CreateMeshRenderer(const PintSurfaceInterface& surface,
                                                                  const Point* normals,
                                                                  bool active_edges,
                                                                  bool direct_data) {
...
PintShapeRenderer* RenderModel_SimpleShadows::_CreateMeshRenderer(const MultiSurface& multi_surface,
                                                                  bool active_edges,
                                                                  bool direct_data) {
    if (1)
...
PintShapeRenderer* RenderModel_SimpleShadows::_CreateColorShapeRenderer(PintShapeRenderer* renderer,
...
IceWindow* RenderModel_SimpleShadows::InitGUI(IceWidget* parent, Widgets* owner, PintGUIHelper& helper) {
...

    {
        struct Local {
            static void gCheckBoxCallback(const IceCheckBox& check_box, bool checked, void* user_data) {
                gShowReflections = checked;
            }
        };
        helper.CreateCheckBox(parent, 0, 4, y, 200, 20, "Reflections", owner, gShowReflections,
                              Local::gCheckBoxCallback, null);
    }
    y += YStep;
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Reflection coeff:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gReflectionCoeff), owner,
                             EDITBOX_FLOAT, Local::gEBCallback, null);
        //		y += YStep;
    }
    ...
    helper.CreateLabel(parent, 180 + 4, y + LabelOffsetY, 90, 20, "Env coeff:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 180 + 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gEnvCoeff), owner,
                             EDITBOX_FLOAT, Local::gEBCallback, null);
        y += YStep;
    }
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Shadow clip near:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gShadowClipNear), owner,
                             EDITBOX_FLOAT, Local::gEBCallback, null);
        //		y += YStep;
    }
    ...
    helper.CreateLabel(parent, 180 + 4, y + LabelOffsetY, 90, 20, "Shadow ambient:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 180 + 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gShadowAmbient),
                             owner, EDITBOX_FLOAT, Local::gEBCallback, null);
        y += YStep;
    }
    ...
    helper.CreateLabel(parent, 180 + 4, y + LabelOffsetY, 90, 20, "Shadow offset:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 180 + 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gShadowOffset),
                             owner, EDITBOX_FLOAT, Local::gEBCallback, null);
        // y += YStep;
    }
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Shadow clip far:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gShadowClipFar), owner,
                             EDITBOX_FLOAT, Local::gEBCallback, null);
        //		y += YStep;
    }
    ...
    helper.CreateLabel(parent, 180 + 4, y + LabelOffsetY, 90, 20, "Phong power:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 180 + 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gPhongPower), owner,
                             EDITBOX_FLOAT, Local::gEBCallback, null);
        y += YStep;
    }
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Nb splits:", owner);
    {
        ...
        IceEditBox* EB = helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, _F("%d", gNbSplits),
                                              owner, EDITBOX_INTEGER_POSITIVE, Local::gEBCallback, null);
        EB->SetUserData(this);
        y += YStep;
    }
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Resolution:", owner);
    {
        ...
        IceEditBox* EB = helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, _F("%d", gShadowMapRes),
                                              owner, EDITBOX_INTEGER_POSITIVE, Local::gEBCallback, null);
        EB->SetUserData(this);
        y += YStep;
    }
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Map width:", owner);
    {
        ...
        IceEditBox* EB =
                helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, _F("%d", gShadowMapWidth), owner,
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Map height:", owner);
    {
        ...
        IceEditBox* EB =
                helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, _F("%d", gShadowMapHeight),
                                     owner, EDITBOX_INTEGER_POSITIVE, Local::gEBCallback, null);
        EB->SetUserData(this);
        y += YStep;
    }
    ...
    helper.CreateLabel(parent, 4, y + LabelOffsetY, 90, 20, "Lit area size:", owner);
    {
        ...
        helper.CreateEditBox(parent, 0, 4 + 10 + OffsetX, y, EditBoxWidth, 20, helper.Convert(gLitAreaSize), owner,
                             EDITBOX_FLOAT, Local::gEBCallback, null);
        y += YStep;
    }
