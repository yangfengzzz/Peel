...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
...

        {
            ...
            mCheckBox_UseConvexesForAll =
                    helper.CreateCheckBox(UI, 0, 4, y, 200, 20, "Use convexes for all", &UIElems, false, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Cylinder tessellation:", &UIElems);
            mEditBox_CylinderTess = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "60", &UIElems,
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (!caps.mSupportCylinders && !caps.mSupportConvexes) return false;
...
static const char* gDesc_RollingCylinder =
        "Rolling cylinder. Some engines do not support proper cylinder shapes. This test investigates the rolling behavior of \
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!CylinderTest::Setup(pint, caps)) return false;
...
static const char* gDesc_CylinderStack =
        "Cylinder stack. The problem with using a tessellated convex instead of a proper cylinder shape is of course performance. \
    ...
    while (Nb) {
        for (udword i = 0; i < Nb; i++) {
            if (!caps.mSupportCylinders || UseConvexes) {
                PINT_CONVEX_CREATE ConvexCreate(mCylinder.mNbVerts, mCylinder.mVerts);
                ConvexCreate.mRenderer = GetRegisteredRenderers()[0];
                PintActorHandle Handle = CreateDynamicObject(
                        pint, &ConvexCreate, Point(OffsetX + float(i) * Radius * 2.2f, Altitude, 0.0f), &R);
                ASSERT(Handle);
            } else {
                PINT_CYLINDER_CREATE CylinderCreate(mCylinder.mRadius, mCylinder.mHalfHeight);
                CylinderCreate.mRenderer = GetRegisteredRenderers()[1];
                //					const Quat Q = ShortestRotation(Point(0.0f, 1.0f, 0.0f),
                //Point(0.0f, 0.0f, 1.0f));
                PintActorHandle Handle = CreateDynamicObject(
                        pint, &CylinderCreate, Point(OffsetX + float(i) * Radius * 2.2f, Altitude, 0.0f) /*, &R*/);
                ASSERT(Handle);
            }
        }
        Nb--;
