...
SelectionManager::SelectionManager()
    : mSelectedNative(null)  //, mLockSelection(false)
{}

SelectionManager::~SelectionManager() { Release(true); }
...
void SelectionManager::Release(bool from_dtor) {
    ...
    if (!from_dtor) UpdateSelectionItems();
}
...
void SelectionManager::CancelSelection() {
    ...
    mSelection.Reset();
    if (mSelectedNative) mSelectedNative->clear();
...
SelectedObject* SelectionManager::AddToSelection(Pint* pint, PintActorHandle actor, udword index) {
    ...
    if (IsDefaultEnv(*pint, actor)) return null;

    {
        if (!mSelectedNative) mSelectedNative = new ps::HashSet<PintActorHandle>;

        PintActorHandle NativeActor = actor;
        if (IsEditor(pint)) NativeActor = Editor_GetNativeHandle(NativeActor);
    ...
    while (NbSelected--) {
        ...
        if (h) {
...

            {
                ...
                if (IsEditor(Selected->mEngine)) NativeActor = Editor_GetNativeHandle(NativeActor);
...
bool SelectionManager::IsSelectedNative(Pint* pint, PintActorHandle h) const {
    ...
    //	udword Time;
    //	StartProfile(Time);
    if (mSelectedNative) return mSelectedNative->contains(h);
    //	EndProfile(Time);
    //	printf("Time: %d\n", Time);
    ...
    while (Nb--) {
        if (Selection->mEngine == pint) {
            ...
            if (IsEditor(Selection->mEngine)) CurrentSelectedActor = Editor_GetNativeHandle(CurrentSelectedActor);

            if (CurrentSelectedActor == h) return true;
        }
        Selection++;
    }
