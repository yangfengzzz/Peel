        ...
        for (udword i = 0; i < NbShapes; i++) {
...

            {
                ...
                for (udword j = 0; j < NbPts; j++) {
                    if (S.mPts[j].mX < MinX) MinX = S.mPts[j].mX;
                    if (S.mPts[j].mX > MaxX) MaxX = S.mPts[j].mX;
                    if (S.mPts[j].mY < MinY) MinY = S.mPts[j].mY;
                    if (S.mPts[j].mY > MaxY) MaxY = S.mPts[j].mY;
                }
                for (udword j = 0; j < NbPts; j++) {
            ...
            if (1) {
                ...
                for (udword j = 0; j < NbPts; j++) {
                    ...
                    if (ShapeVerts[j].x < MinX) MinX = ShapeVerts[j].x;
                    if (ShapeVerts[j].x > MaxX) MaxX = ShapeVerts[j].x;
                    if (ShapeVerts[j].y < MinY) MinY = ShapeVerts[j].y;
                    if (ShapeVerts[j].y > MaxY) MaxY = ShapeVerts[j].y;
                }
