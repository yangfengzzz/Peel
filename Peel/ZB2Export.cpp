...
static void _ExportRenderSource(
        RenderDataChunk* chunk, ZCB2Exporter& database, PtrContainer& textures, udword& render_id, udword& texture_id) {
    if (!chunk || chunk->GetID() != INVALID_ID) return;
...
static void ExportShapes(ZCB2Exporter& database,
                         const PtrContainer& shapes,
                         PtrContainer& textures,
                         udword& shape_id,
                         udword& material_id,
                         udword& render_id,
                         udword& texture_id) {
            ...
            if (Current->mRenderer) {
                ...
                /*				RenderDataChunk* RenderChunk = Current->mRenderer->mRenderSource;
                                                if(RenderChunk)
                                                {
                                                        struct Export
                                                        {
                                                                static void Chunk(ZCB2Exporter& database,
                   RenderDataChunk* chunk, udword& render_id)
                                                                {
                                                                        if(chunk->GetID()==INVALID_ID)
                                                                        {
                                                                                chunk->SetID(render_id++);
                                                                                database.RegisterUserDefinedChunk(chunk,
                   FALSE);
                                                                        }
                                                                }
                                                        };
                                                        ...
                                                        if(MT)
                                                        {
                                                                if(MT->mExportID == INVALID_ID)
                                                                {
                                                                        if(MT->mSource)	//#### temporary
                                                                        {
                                                                                MT->mExportID = texture_id++;
                                                                                TXMPChunk* txmp =
                   static_cast<TXMPChunk*>(database.CreateChunk('TXMP'));
                                                                                // TODO: support external files
                   (filename/URL) txmp->SetSourceBitmap(MT->mSource, null, null); const char* error = txmp->Validate();
                                                                                ASSERT(!error);
                                                                        }
                                                                }
                                                        }

                                                        // Takes care of ColorRenderDataChunk. Only deals with one child
                   here. RenderDataChunk* ChildChunk = RenderChunk->GetChildRenderChunk(); if(ChildChunk)
...
void ExportZB2(const EditorPlugin& editor, const char* filename) {
    ...
    PtrContainer TexturesToReset;
    {
        ...
        if (EditorSceneData) Database.RegisterUserDefinedChunk(ICE_NEW(SceneChunk)(*EditorSceneData));

        const CollisionGroupsData* EditorGroupsData = editor.GetEditorCollisionGroupsData();
        if (EditorGroupsData) Database.RegisterUserDefinedChunk(ICE_NEW(CollisionGroupsChunk)(*EditorGroupsData));
        ...
        // Export all materials, or just initialize their IDs for partial export
        {
            ...
            if (gExportAllMaterials) printf("-- ZB2: %d exportable materials\n", ID);
        }
        ...
        /*		// - second pass to parse shapes and mark the renderers they use
                        ...
                        // => could be merged with second pass but we want the IDs to be assigned in order (for easier
           loading code)
                        {
                                udword ID = 0;
                                const udword NbRenderers = GetNbShapeRenderers();
        ...
        // Export all shapes & used materials
        {
            ...
            udword RenderID = 0;
            udword TextureID = 0;
            ExportShapes<SphereShapeData, SphereShapeChunk>(Database, editor.GetEditorSphereShapes(), TexturesToReset,
                                                            ShapeID, MaterialID, RenderID, TextureID);
            ExportShapes<CapsuleShapeData, CapsuleShapeChunk>(Database, editor.GetEditorCapsuleShapes(),
                                                              TexturesToReset, ShapeID, MaterialID, RenderID,
                                                              TextureID);
            ExportShapes<CylinderShapeData, CylinderShapeChunk>(Database, editor.GetEditorCylinderShapes(),
                                                                TexturesToReset, ShapeID, MaterialID, RenderID,
                                                                TextureID);
            ExportShapes<BoxShapeData, BoxShapeChunk>(Database, editor.GetEditorBoxShapes(), TexturesToReset, ShapeID,
                                                      MaterialID, RenderID, TextureID);
            ExportShapes<ConvexShapeData, ConvexShapeChunk>(Database, editor.GetEditorConvexShapes(), TexturesToReset,
                                                            ShapeID, MaterialID, RenderID, TextureID);
            ExportShapes<MeshShapeData, MeshShapeChunk>(Database, editor.GetEditorMeshShapes(), TexturesToReset,
                                                        ShapeID, MaterialID, RenderID, TextureID);
            ExportShapes<MeshShapeData2, MeshShapeChunk2>(Database, editor.GetEditorMeshShapes2(), TexturesToReset,
                                                          ShapeID, MaterialID, RenderID, TextureID);
            ...
            for (udword i = 0; i < NbActors; i++) {
                ...
                if (CanExport(CurrentActor)) {
                    ...
                    const bool IsRenderable =
                            editor.mVisHelper ? editor.mVisHelper->IsRenderable2(PintActorHandle(CurrentActor)) : true;
            ...
            for (udword i = 0; i < NbJoints; i++) {
                ...
                switch (CurrentJoint->mType) {
                    case PINT_JOINT_SPHERICAL: {
                        NewJoint = ICE_NEW(SphericalJointChunk)(*static_cast<const SphericalJointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_HINGE: {
                        NewJoint = ICE_NEW(HingeJointChunk)(*static_cast<const HingeJointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_HINGE2: {
                        NewJoint = ICE_NEW(Hinge2JointChunk)(*static_cast<const Hinge2JointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_PRISMATIC: {
                        NewJoint = ICE_NEW(PrismaticJointChunk)(*static_cast<const PrismaticJointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_FIXED: {
                        NewJoint = ICE_NEW(FixedJointChunk)(*static_cast<const FixedJointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_DISTANCE: {
                        NewJoint = ICE_NEW(DistanceJointChunk)(*static_cast<const DistanceJointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_D6: {
                        NewJoint = ICE_NEW(D6JointChunk)(*static_cast<const D6JointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_GEAR: {
                        NewJoint = ICE_NEW(GearJointChunk)(*static_cast<const GearJointData*>(CurrentJoint));
                    } break;
                    case PINT_JOINT_RACK_AND_PINION: {
                        NewJoint = ICE_NEW(RackJointChunk)(*static_cast<const RackJointData*>(CurrentJoint));
                    } break;
                };
                ASSERT(NewJoint);
                if (NewJoint) Database.RegisterUserDefinedChunk(NewJoint);
            }
