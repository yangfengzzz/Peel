...
Streamer::Streamer(float world_extent, udword nb_cells_per_side)
    : mTimestamp(0), mWorldExtent(world_extent), mNbCellsPerSide(nb_cells_per_side), mDebugDrawStreamingBounds(false) {
...
void StreamInterface::RenderDebugRegion(PintRender& render, const Streamer& owner, const StreamRegion& region) {
    if (owner.mDebugDrawStreamingBounds) render.DrawWireframeAABB(1, &region.mCellBounds, Point(0.0f, 1.0f, 0.0f));
}
    ...
    /*	for(sdword j=z0;j<z1;j++)
            {
                    for(sdword i=x0;i<x1;i++)
                    {
                            ...
                            if(Region.mTimestamp==INVALID_ID)
                            {
                                    ...
                                    Region.mKey = Key;
                                    Region.mTimestamp = mTimestamp;
                                    Region.mCellBounds.SetMinMax(Point(float(i)*CellSizeX, 0.0f, float(j)*CellSizeZ),
       Point(float(i+1)*CellSizeX, 0.0f, float(j+1)*CellSizeZ));
