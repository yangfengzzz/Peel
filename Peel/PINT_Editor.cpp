...
void SetEditedPlugin(PintPlugin* edited) { mEditedPlugin = edited; }
...
EditorObject::EditorObject(Type type) : mEditorType(type), mNativeHandle(null), mRefCount(1), mExportable(true) {}

EditorActor::EditorActor() : EditorObject(ACTOR), mIsStatic(false), mIsCompound(false) {
    mInitPos.Zero();
    mInitRot.Identity();
}
...
EditorShape::EditorShape() : EditorObject(SHAPE), mShapeType(PINT_SHAPE_UNDEFINED), mNbTris(0), mNbVerts(0) {}

EditorShape::~EditorShape() {}

EditorJoint::EditorJoint() : EditorObject(JOINT) {}

EditorJoint::~EditorJoint() {}

EditorMesh::EditorMesh() : EditorObject(MESH) {}

EditorMesh::~EditorMesh() {}

EditorMaterial::EditorMaterial() : EditorObject(MATERIAL) {}

EditorMaterial::~EditorMaterial() {}

EditorAggregate::EditorAggregate() : EditorObject(AGGREGATE) {}

EditorAggregate::~EditorAggregate() {}

EditorCharacter::EditorCharacter() : EditorObject(CHARACTER) { dummy = null; }

EditorCharacter::~EditorCharacter() {}

EditorVehicle::EditorVehicle() : EditorObject(VEHICLE) { dummy = null; }
    ...
    public:
        ...
        virtual bool ReportObject(PintActorHandle engine_handle) {
            /*			// TODO: optimize that loop
               ...
               HandleToActorData(mEditor.mEditorActors.GetEntry(j));
...
// derived value = computed by native engines but not explicitly defined by users, i.e. not available in ActorData
// structures. We can use "pass through" macros then, that just re-route calls to underlying native engine.
...
bool EditorJointAPI::SetActors(PintJointHandle handle, PintActorHandle actor0, PintActorHandle actor1) {
    ...
    if (OldActor0 != NewActor0) {
        if (OldActor0) OldActor0->mJoints.Delete(JD);

        if (NewActor0) NewActor0->mJoints.AddPtr(JD);

        JD->mObject0 = NewActor0;
    }

    if (OldActor1 != NewActor1) {
        if (OldActor1) OldActor1->mJoints.Delete(JD);

        if (NewActor1) NewActor1->mJoints.AddPtr(JD);
...
bool EditorJointAPI::GetLimits(PintJointHandle handle, PintLimits& limits, udword index) const {
    ...
    if (JD->GetLimits(limits, index)) return true;
...
/*		virtual	bool				ResetVehicleData(PintVehicleHandle vehicle);
                ...
                virtual	bool				AddShape(PintVehicleHandle vehicle, const PINT_SHAPE_CREATE&
   create);*/
...
udword EditorPlugin::GetFlags() const {
    if (mEdited) return mEdited->GetFlags();
...
void EditorPlugin::SetGravity(const Point& gravity) {
    if (mEdited) mEdited->SetGravity(gravity);
    //	return;
...
#ifdef TEST_NVD
    NvdCapture* Nvd = getNvd();
    if (Nvd) Nvd->setPropertyValue_vec3(InstanceHandle(gEditor), "Gravity", ToPxVec3(gravity));
#endif
}

void EditorPlugin::SetDisabledGroups(udword nb_groups, const PintDisabledGroups* groups) {
    if (mEdited) mEdited->SetDisabledGroups(nb_groups, groups);
    //	return;
...
static bool DeleteShape(EditorShape* editor_shape,
                        ShapeData* shape_data,
                        PtrContainer& editor_shapes,
                        Stats& stats,
                        Stats::Shared* shape_stats) {
    ASSERT(editor_shape->mRefCount);
    editor_shape->mRefCount--;

    if (shape_stats) DecreaseStats(editor_shape, *shape_stats);

    if (editor_shape->mRefCount) return false;
    ...
    if (editor_shapes.Delete(shape_data)) DeleteObject(shape_data);
    return true;
}
        ...
        for (udword i = 0; i < NbShapes; i++) {
            ...
            if (ShapeObject) {
                ...
                if (ShapeObject->mShapeType == PINT_SHAPE_SPHERE) {
                    ...
                    DeleteShape(ShapeObject, CurrentShape, editor.GetEditorCapsuleShapes(), stats,
                                &stats.mCapsuleShapes);
                } else if (ShapeObject->mShapeType == PINT_SHAPE_CYLINDER) {
                    DeleteShape(ShapeObject, CurrentShape, editor.GetEditorCylinderShapes(), stats,
                                &stats.mCylinderShapes);
...
udword EditorPlugin::Update(float dt) {
    ...
    udword Ret = 0;
    if (mEdited) Ret = mEdited->Update(dt);
...
Point EditorPlugin::GetMainColor() {
    if (mEdited) return mEdited->GetMainColor();
...
public:
    ...
    virtual ~PEELRenderInterface() { renderSelected(); }
    ...
    virtual void renderHandle(const PxTransform& pose, RenderHandle handle, WeakHandle wh) {
        ...
        if (gNVDControlInterface.mSelected && gNVDControlInterface.mSelected->contains(wh)) {
            SelectedRenderItem* SRI =
                    (SelectedRenderItem*)mSelectedRenderData.Reserve(sizeof(SelectedRenderItem) / sizeof(udword));
    ...
    virtual void renderSphere(const PxTransform& pose, float radius) { mRenderer.DrawSphere(radius, ToPR(pose)); }
...
void EditorPlugin::Render(PintRender& renderer, PintRenderPass render_pass) {
    ...
    if (Nvd && !Nvd->isCaptureEnabled()) {
        if (render_pass == PINT_RENDER_PASS_REFLECTIONS) return;
...
#endif
    if (1) {
        if (mEdited) mEdited->Render(renderer, render_pass);
        return;
    }
...
void EditorPlugin::RenderDebugData(PintRender& renderer) {
    ...
    if (Nvd && !Nvd->isCaptureEnabled()) {
        ...
        if (NvDb) {
            ...
            if (gDrawVisibleBounds) NvDb->renderVisibleBounds(BoundsRenderInterface(renderer));
        }
        return;
    }
#endif

    if (mEdited) mEdited->RenderDebugData(renderer);
    //	return;
...
PR EditorPlugin::GetWorldTransform(PintActorHandle handle) {
    ...
    if (mEdited) return mEdited->GetWorldTransform(GetNativeActorHandle(Data));
...
void EditorPlugin::SetWorldTransform(PintActorHandle handle, const PR& pose) {
    ...
    if (mEdited) mEdited->SetWorldTransform(GetNativeActorHandle(Data), pose);
...
bool EditorPlugin::SetName(PintActorHandle handle, const char* name) {
    ...
    if (mEdited) {
        Pint_Actor* API = mEdited->GetActorAPI();
        if (API) return API->SetName(GetNativeActorHandle(Data), name);
    }
    return true;
}
#endif
...
PintMeshHandle EditorPlugin::CreateMeshObject(const PINT_MESH_DATA_CREATE& desc, PintMeshIndex* index) {
    ...
    if (Nvd) {
        ...
        Nvd->setMeshData(InstanceHandle(NewMesh), SI.mNbVerts, reinterpret_cast<const PxVec3*>(SI.mVerts), SI.mNbFaces,
                         SI.mDFaces, false);
    }
#endif
...
MaterialData* EditorPlugin::FindMaterial(const PINT_MATERIAL_CREATE& create) const {
    struct Materials {
        static inline_ bool AreEqual(const MaterialData& data, const PINT_MATERIAL_CREATE& create) {
            ...
            if (data.mStaticFriction != create.mStaticFriction) return false;
            if (data.mDynamicFriction != create.mDynamicFriction) return false;
            if (data.mRestitution != create.mRestitution) return false;
            return true;
        }
    };
    ...
    for (udword i = 0; i < Nb; i++) {
        ...
        if (Object->mExportable && Materials::AreEqual(*Current, create)) return Current;
    }
    return null;
}

const MaterialData* EditorPlugin::RegisterMaterial(const PINT_MATERIAL_CREATE& create, bool add_to_database) {
    if (add_to_database) mStats.mMaterials.mNbRef++;
    ...
    if (NewMaterial) {
        ...
        if (add_to_database) mStats.mMaterials.mNbUnique++;
        ...
        if (Nvd) {
            ...
            if (create.mName) Nvd->setPropertyValue_string(h, "Name", create.mName);
...
static void SetupNewShape(ShapeData* new_shape,
                          const PINT_SHAPE_CREATE* src,
    ...
    if (Nvd) {
        ...
        if (material) Nvd->addLink(h, InstanceHandle(material));

        if (src->mName) Nvd->setPropertyValue_string(h, "Name", src->mName);
        ...
        if (new_shape->mType == PINT_SHAPE_SPHERE) {
            ...
            if (!Nvd->instanceExists(InstanceHandle(MSD->mRenderer))) {
                ...
                Nvd->setMeshData(InstanceHandle(MSD->mRenderer), MSD->mNbVerts,
                                 reinterpret_cast<const PxVec3*>(MSD->mVerts), MSD->mNbTris, MSD->mTris, false);
            }
...
static inline_ bool AreBaseShapesEqual(const ShapeData& data,
                                       const PINT_SHAPE_CREATE& create,
                                       const MaterialData* material) {
    ...
    if (data.mMaterial != material) return false;

    // ###TODO: what about the names?

    {
        // Necessary for cases where collision shapes don't match the rendered prim...
        if (data.mRenderer != create.mRenderer) return false;
    }
    ...
    if (data.mRenderer && create.mRenderer) {
        ...
        if ((color0 && !color1) || (!color0 && color1)) return false;
        if (color0 && color1) {
            if (*color0 != *color1) return false;
        }
    }

    if (data.mLocalPos != create.mLocalPos) return false;
    if (data.mLocalRot != create.mLocalRot) return false;
    return true;
}
...
static T0* FindShape(const T1& create, const PtrContainer& shapes, const MaterialData* material) {
    ...
    for (udword i = 0; i < Nb; i++) {
        ...
        if (Object->mExportable && T2::AreEqual(*Current, create, material)) return Current;
    }
    return null;
}

template <class T0, class T1>
const T0* RegisterShape(T0* new_shape,
                        const T1& create,
                        PtrContainer& shapes,
                        const MaterialData* material,
                        Stats::Shared& stats,
                        bool add_to_database) {
    if (add_to_database) stats.mNbRef++;
    ...
    if (new_shape) {
        ...
        if (add_to_database) stats.mNbUnique++;
...
SphereShapeData* EditorPlugin::FindSphereShape(const PINT_SPHERE_CREATE& create, const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const SphereShapeData& data,
                                     const PINT_SPHERE_CREATE& create,
                                     const MaterialData* material) {
            if (data.mRadius != create.mRadius) return false;
...
const SphereShapeData* EditorPlugin::RegisterSphereShape(const PINT_SPHERE_CREATE& create,
                                                         const MaterialData* material,
                                                         bool add_to_database) {
...
CapsuleShapeData* EditorPlugin::FindCapsuleShape(const PINT_CAPSULE_CREATE& create,
                                                 const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const CapsuleShapeData& data,
                                     const PINT_CAPSULE_CREATE& create,
                                     const MaterialData* material) {
            if (data.mRadius != create.mRadius) return false;
            if (data.mHalfHeight != create.mHalfHeight) return false;
            return AreBaseShapesEqual(data, create, material);
        }
    };
...
const CapsuleShapeData* EditorPlugin::RegisterCapsuleShape(const PINT_CAPSULE_CREATE& create,
                                                           const MaterialData* material,
                                                           bool add_to_database) {
...
CylinderShapeData* EditorPlugin::FindCylinderShape(const PINT_CYLINDER_CREATE& create,
                                                   const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const CylinderShapeData& data,
                                     const PINT_CYLINDER_CREATE& create,
                                     const MaterialData* material) {
            if (data.mRadius != create.mRadius) return false;
            if (data.mHalfHeight != create.mHalfHeight) return false;
            return AreBaseShapesEqual(data, create, material);
...
const CylinderShapeData* EditorPlugin::RegisterCylinderShape(const PINT_CYLINDER_CREATE& create,
                                                             const MaterialData* material,
                                                             bool add_to_database) {
...
BoxShapeData* EditorPlugin::FindBoxShape(const PINT_BOX_CREATE& create, const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const BoxShapeData& data,
                                     const PINT_BOX_CREATE& create,
                                     const MaterialData* material) {
            if (data.mExtents != create.mExtents) return false;
            return AreBaseShapesEqual(data, create, material);
        }
    };
...
const BoxShapeData* EditorPlugin::RegisterBoxShape(const PINT_BOX_CREATE& create,
                                                   const MaterialData* material,
                                                   bool add_to_database) {
...
ConvexShapeData* EditorPlugin::FindConvexShape(const PINT_CONVEX_CREATE& create, const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const ConvexShapeData& data,
                                     const PINT_CONVEX_CREATE& create,
                                     const MaterialData* material) {
...
const ConvexShapeData* EditorPlugin::RegisterConvexShape(const PINT_CONVEX_CREATE& create,
                                                         const MaterialData* material,
                                                         bool add_to_database) {
...
MeshShapeData* EditorPlugin::FindMeshShape(const PINT_MESH_CREATE& create, const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const MeshShapeData& data,
                                     const PINT_MESH_CREATE& create,
                                     const MaterialData* material) {
            // ###TODO: this legacy shape used the renderer pointer to detect sharing, which isn't great.
            //   We could actually compare the vertex data or use a checksum instead.
            if (data.mRenderer != create.mRenderer) return false;
...
const MeshShapeData* EditorPlugin::RegisterMeshShape(const PINT_MESH_CREATE& create,
                                                     const MaterialData* material,
                                                     bool add_to_database) {
...
MeshShapeData2* EditorPlugin::FindMeshShape2(const PINT_MESH_CREATE2& create, const MaterialData* material) const {
    struct Shapes {
        static inline_ bool AreEqual(const MeshShapeData2& data,
                                     const PINT_MESH_CREATE2& create,
                                     const MaterialData* material) {
            if (data.mMeshData != reinterpret_cast<MeshData*>(create.mTriangleMesh)) return false;
            return AreBaseShapesEqual(data, create, material);
        }
    };
...
const MeshShapeData2* EditorPlugin::RegisterMeshShape2(const PINT_MESH_CREATE2& create,
                                                       const MaterialData* material,
                                                       bool add_to_database) {
...
void EditorPlugin::ReportShape(const PINT_SHAPE_CREATE& create, udword index, void* user_data) {
    ...
    if (CurrentShape->mMaterial) NewMaterial = RegisterMaterial(*CurrentShape->mMaterial, AddToDatabase);

    ShapeData* NewShape = null;
    if (CurrentShape->mType == PINT_SHAPE_SPHERE) {
        NewShape = const_cast<SphereShapeData*>(
                RegisterSphereShape(*static_cast<const PINT_SPHERE_CREATE*>(CurrentShape), NewMaterial, AddToDatabase));
    } else if (CurrentShape->mType == PINT_SHAPE_CAPSULE) {
        NewShape = const_cast<CapsuleShapeData*>(RegisterCapsuleShape(
                *static_cast<const PINT_CAPSULE_CREATE*>(CurrentShape), NewMaterial, AddToDatabase));
        ...
        NewShape = const_cast<CylinderShapeData*>(RegisterCylinderShape(
                *static_cast<const PINT_CYLINDER_CREATE*>(CurrentShape), NewMaterial, AddToDatabase));
    } else if (CurrentShape->mType == PINT_SHAPE_BOX) {
        NewShape = const_cast<BoxShapeData*>(
                RegisterBoxShape(*static_cast<const PINT_BOX_CREATE*>(CurrentShape), NewMaterial, AddToDatabase));
    } else if (CurrentShape->mType == PINT_SHAPE_CONVEX) {
        NewShape = const_cast<ConvexShapeData*>(
                RegisterConvexShape(*static_cast<const PINT_CONVEX_CREATE*>(CurrentShape), NewMaterial, AddToDatabase));
...
//		}
        ...
        const_cast<PINT_MESH_CREATE2*>(MeshCreate)->mTriangleMesh =
                PintMeshHandle(Object->mNativeHandle);  // ###TODO: undo this before returning, else can crash
    } else
        ASSERT(0);
...
PintActorHandle EditorPlugin::CreateObject(const PINT_OBJECT_CREATE& desc) {
    ...
    if (desc.mAddToDatabase) {
        ...
        if (desc.mMass == 0.0f) {
            ...
            mStats.mNbStatics++;
            if (IsCompound) mStats.mNbStaticCompounds++;
        } else {
            Object->mIsStatic = false;
            mStats.mNbDynamics++;
            if (IsCompound) mStats.mNbDynamicCompounds++;
        }
    }
    ...
    if (Nvd) {
        ...
        if (desc.mName) Nvd->setPropertyValue_string(h, "Name", desc.mName);
        ...
        if (API) {
            ...
            if (NbNativeShapes == NewActor->mShapes.GetNbEntries()) {
                for (udword i = 0; i < NbNativeShapes; i++) {
                    ...
                    if (ShapeObject) ShapeObject->mNativeHandle = API->GetShape(EngineHandle, i);
                }
            }
        }
    }
...
bool EditorPlugin::ReleaseObject(PintActorHandle handle) {
...
#ifdef TEST_NVD
    NvdCapture* Nvd = getNvdForCapture();
    if (Nvd) Nvd->releaseInstance(InstanceHandle(Data));
#endif

    if (mEdited) /*return*/ mEdited->_ReleaseObject(GetNativeActorHandle(Data));
    ...
    if (mEditorActors.Delete(Data)) DeleteActor(Data, *this, mStats);
...
PintJointHandle EditorPlugin::CreateJoint(const PINT_JOINT_CREATE& desc) {
    ...
    if (mEdited) {
        ...
        if (Data0) desc2.mObject0 = GetNativeActorHandle(Data0);
        if (Data1) desc2.mObject1 = GetNativeActorHandle(Data1);
        ...
        if (desc.mType == PINT_JOINT_GEAR) {
            ...
            if (Hinge0) gjc.mHinge0 = GetNativeJointHandle(Hinge0);
            if (Hinge1) gjc.mHinge1 = GetNativeJointHandle(Hinge1);
            ...
            if (Hinge) gjc.mHinge = GetNativeJointHandle(Hinge);
            if (Prismatic) gjc.mPrismatic = GetNativeJointHandle(Prismatic);
    ...
    if (NewJoint) {
        ...
        if (Data0) Data0->mJoints.AddPtr(NewJoint);
        if (Data1) Data1->mJoints.AddPtr(NewJoint);
        ...
        if (Nvd) {
            ...
            // Nvd->setPropertyValue_renderTransform(InstanceHandle(NewActor), PxTransform(ToPxVec3(desc.mPosition),
            // ToPxQuat(desc.mRotation)));
...
bool EditorPlugin::ReleaseJoint(PintJointHandle handle) {
    ...
    if (mEdited) /*return*/ mEdited->ReleaseJoint(GetNativeJointHandle(Data));
    ...
    if (ActorData0) ActorData0->mJoints.Delete(Data);
    ActorData* ActorData1 = Data->mObject1;
    if (ActorData1) ActorData1->mJoints.Delete(Data);
...
PintActorHandle StatsPint::CreateArticulatedObject(const PINT_OBJECT_CREATE&, const PINT_ARTICULATED_BODY_CREATE&,
...
bool EditorPlugin::GetBounds(PintActorHandle handle, AABB& bounds) {
    ...
    if (mEdited) {
        Pint_Actor* API = mEdited->GetActorAPI();
        if (API) return API->GetBounds(GetNativeActorHandle(Data), bounds);
    }
...
void EditorPlugin::UpdateNonProfiled(float dt) {
    if (mEdited) mEdited->UpdateNonProfiled(dt);
}
#endif
...
bool EditorPlugin::ResetSQFilters() { return mEdited ? mEdited->ResetSQFilters() : false; }

PintActorHandle EditorPlugin::RemapActorHandle(PintActorHandle native_handle) const {
    if (!native_handle) return null;

    // TODO: optimize that loop
    for (udword j = 0; j < mEditorActors.GetNbEntries(); j++) {
        const ActorData* CurrentActor = HandleToActorData(mEditorActors.GetEntry(j));
        if (GetNativeActorHandle(CurrentActor) == native_handle) return PintActorHandle(CurrentActor);
    }
...
PintActorHandle EditorPlugin::RemapActorAndShapeHandles(PintActorHandle native_handle,
                                                        PintShapeHandle& shape_handle) const {
    if (!native_handle) return null;
...
udword EditorPlugin::BatchRaycasts(PintSQThreadContext context,
                                   udword nb,
                                   PintRaycastHit* dest,
                                   const PintRaycastData* raycasts) {
    if (!mEdited) return 0;
    ...
    for (udword i = 0; i < Nb; i++) {
        /*		PintActorHandle EngineHandle = dest[i].mTouchedActor;
                        ...
                        ASSERT(dest[i].mTouchedActor!=EngineHandle);	// this can now happen for "default environment"
           plane
                        */
...
#ifdef TOSEE
udword EditorPlugin::BatchRaycastAny(PintSQThreadContext context,
                                     udword nb,
                                     PintBooleanHit* dest,
                                     const PintRaycastData* raycasts) {
    if (mEdited) return mEdited->BatchRaycastAny(context, nb, dest, raycasts);
    return 0;
}

udword EditorPlugin::BatchRaycastAll(PintSQThreadContext context,
                                     udword nb,
                                     PintMultipleHits* dest,
                                     const PintRaycastData* raycasts) {
    if (mEdited) return mEdited->BatchRaycastAll(context, nb, dest, raycasts);
    return 0;
}

udword EditorPlugin::BatchBoxSweeps(PintSQThreadContext context,
                                    udword nb,
                                    PintRaycastHit* dest,
                                    const PintBoxSweepData* sweeps) {
    if (mEdited) return mEdited->BatchBoxSweeps(context, nb, dest, sweeps);
    return 0;
}

udword EditorPlugin::BatchSphereSweeps(PintSQThreadContext context,
                                       udword nb,
                                       PintRaycastHit* dest,
                                       const PintSphereSweepData* sweeps) {
    if (mEdited) return mEdited->BatchSphereSweeps(context, nb, dest, sweeps);
    return 0;
}
#endif

udword EditorPlugin::BatchCapsuleSweeps(PintSQThreadContext context,
                                        udword nb,
                                        PintRaycastHit* dest,
                                        const PintCapsuleSweepData* sweeps) {
    if (!mEdited) return 0;
...
#ifdef TOSEE
udword EditorPlugin::BatchSphereOverlapAny(PintSQThreadContext context,
                                           udword nb,
                                           PintBooleanHit* dest,
                                           const PintSphereOverlapData* overlaps) {
    if (mEdited) return mEdited->BatchSphereOverlapAny(context, nb, dest, overlaps);
    return 0;
}

udword EditorPlugin::BatchSphereOverlapObjects(PintSQThreadContext context,
                                               udword nb,
                                               PintMultipleHits* dest,
                                               const PintSphereOverlapData* overlaps) {
    if (mEdited) return mEdited->BatchSphereOverlapObjects(context, nb, dest, overlaps);
    return 0;
}

udword EditorPlugin::BatchBoxOverlapAny(PintSQThreadContext context,
                                        udword nb,
                                        PintBooleanHit* dest,
                                        const PintBoxOverlapData* overlaps) {
    if (mEdited) return mEdited->BatchBoxOverlapAny(context, nb, dest, overlaps);
    return 0;
}

udword EditorPlugin::BatchBoxOverlapObjects(PintSQThreadContext context,
                                            udword nb,
                                            PintMultipleHits* dest,
                                            const PintBoxOverlapData* overlaps) {
    if (mEdited) return mEdited->BatchBoxOverlapObjects(context, nb, dest, overlaps);
    return 0;
}

udword EditorPlugin::BatchCapsuleOverlapAny(PintSQThreadContext context,
                                            udword nb,
                                            PintBooleanHit* dest,
                                            const PintCapsuleOverlapData* overlaps) {
    if (mEdited) return mEdited->BatchCapsuleOverlapAny(context, nb, dest, overlaps);
    return 0;
}

udword EditorPlugin::BatchCapsuleOverlapObjects(PintSQThreadContext context,
                                                udword nb,
                                                PintMultipleHits* dest,
                                                const PintCapsuleOverlapData* overlaps) {
    if (mEdited) return mEdited->BatchCapsuleOverlapObjects(context, nb, dest, overlaps);
    return 0;
}

udword EditorPlugin::FindTriangles_MeshSphereOverlap(PintSQThreadContext context,
                                                     PintActorHandle handle,
                                                     udword nb,
                                                     const PintSphereOverlapData* overlaps) {
    if (mEdited) return mEdited->FindTriangles_MeshSphereOverlap(context, handle, nb, overlaps);
    return 0;
}

udword EditorPlugin::FindTriangles_MeshBoxOverlap(PintSQThreadContext context,
                                                  PintActorHandle handle,
                                                  udword nb,
                                                  const PintBoxOverlapData* overlaps) {
    if (mEdited) return mEdited->FindTriangles_MeshBoxOverlap(context, handle, nb, overlaps);
    return 0;
}

udword EditorPlugin::FindTriangles_MeshCapsuleOverlap(PintSQThreadContext context,
                                                      PintActorHandle handle,
                                                      udword nb,
                                                      const PintCapsuleOverlapData* overlaps) {
    if (mEdited) return mEdited->FindTriangles_MeshCapsuleOverlap(context, handle, nb, overlaps);
    return 0;
}
#endif

udword EditorPlugin::FindTriangles_MeshMeshOverlap(PintSQThreadContext context,
                                                   PintActorHandle handle0,
                                                   PintActorHandle handle1,
                                                   Container& results) {
    if (!mEdited) return 0;
    ...
    return mEdited->FindTriangles_MeshMeshOverlap(context, GetNativeActorHandle(Data0), GetNativeActorHandle(Data1),
                                                  results);
}

void EditorPlugin::AddWorldImpulseAtWorldPos(PintActorHandle handle,
                                             const Point& world_impulse,
                                             const Point& world_pos) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) mEdited->AddWorldImpulseAtWorldPos(GetNativeActorHandle(Data), world_impulse, world_pos);
}

void EditorPlugin::AddLocalTorque(PintActorHandle handle, const Point& local_torque) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) mEdited->AddLocalTorque(GetNativeActorHandle(Data), local_torque);
}

Point EditorPlugin::GetLinearVelocity(PintActorHandle handle) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->GetLinearVelocity(GetNativeActorHandle(Data));
...
void EditorPlugin::SetLinearVelocity(PintActorHandle handle, const Point& linear_velocity) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) mEdited->SetLinearVelocity(GetNativeActorHandle(Data), linear_velocity);
}

Point EditorPlugin::GetAngularVelocity(PintActorHandle handle) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->GetAngularVelocity(GetNativeActorHandle(Data));
    return Point(0.0f, 0.0f, 0.0f);  // TODO: what should we return here?
}

void EditorPlugin::SetAngularVelocity(PintActorHandle handle, const Point& angular_velocity) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) mEdited->SetAngularVelocity(GetNativeActorHandle(Data), angular_velocity);
}

#ifdef TOSEE

udword EditorPlugin::GetShapes(PintActorHandle* shapes, PintActorHandle handle) {
    if (mEdited) return mEdited->GetShapes(shapes, handle);
    return 0;
}

void EditorPlugin::SetLocalRot(PintActorHandle handle, const Quat& q) {
    if (mEdited) mEdited->SetLocalRot(handle, q);
}
#endif
...
float EditorPlugin::GetLinearDamping(PintActorHandle handle) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->GetLinearDamping(GetNativeActorHandle(Data));
    return 0.0f;  // TODO: what should we return here?
}

float EditorPlugin::GetAngularDamping(PintActorHandle handle) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->GetAngularDamping(GetNativeActorHandle(Data));
    return 0.0f;  // TODO: what should we return here?
}

void EditorPlugin::SetLinearDamping(PintActorHandle handle, float damping) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) mEdited->SetLinearDamping(GetNativeActorHandle(Data), damping);
}

void EditorPlugin::SetAngularDamping(PintActorHandle handle, float damping) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) mEdited->SetAngularDamping(GetNativeActorHandle(Data), damping);
}
#endif

bool EditorPlugin::SetKinematicPose(PintActorHandle handle, const Point& pos) {
    ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->SetKinematicPose(GetNativeActorHandle(Data), pos);
    return false;
}

bool EditorPlugin::SetKinematicPose(PintActorHandle handle, const PR& pr) {
    ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->SetKinematicPose(GetNativeActorHandle(Data), pr);
    return false;
}

bool EditorPlugin::IsKinematic(PintActorHandle handle) {
    const ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->IsKinematic(GetNativeActorHandle(Data));
    return false;
}

bool EditorPlugin::EnableKinematic(PintActorHandle handle, bool flag) {
    ActorData* Data = HandleToActorData(handle);
    if (mEdited) return mEdited->EnableKinematic(GetNativeActorHandle(Data), flag);
    return false;
}

PintSQThreadContext EditorPlugin::CreateSQThreadContext() {
    if (mEdited) return mEdited->CreateSQThreadContext();
    return null;
}

void EditorPlugin::ReleaseSQThreadContext(PintSQThreadContext context) {
    if (mEdited) mEdited->ReleaseSQThreadContext(context);
}

#ifdef TOSEE
udword EditorPlugin::BatchConvexSweeps(PintSQThreadContext context,
                                       udword nb,
                                       PintRaycastHit* dest,
                                       const PintConvexSweepData* sweeps) {
    if (mEdited) return mEdited->BatchConvexSweeps(context, nb, dest, sweeps);
    return 0;
}
#endif
...
PintArticHandle EditorPlugin::CreateArticulation(const PINT_ARTICULATION_CREATE& desc) {
    if (mEdited) return mEdited->CreateArticulation(desc);
    return 0;
}

PintActorHandle EditorPlugin::CreateArticulatedObject(const PINT_OBJECT_CREATE& oc,
                                                      const PINT_ARTICULATED_BODY_CREATE& bc,
                                                      PintArticHandle articulation) {
    if (mEdited) return mEdited->CreateArticulatedObject(oc, bc, articulation);
    return 0;
}

bool EditorPlugin::AddArticulationToScene(PintArticHandle articulation) {
    if (mEdited) return mEdited->AddArticulationToScene(articulation);
    return false;
}

void EditorPlugin::SetArticulatedMotor(PintActorHandle object, const PINT_ARTICULATED_MOTOR_CREATE& motor) {
    if (mEdited) mEdited->SetArticulatedMotor(object, motor);
}

PintArticHandle EditorPlugin::CreateRCArticulation(const PINT_RC_ARTICULATION_CREATE& create) {
    if (mEdited) return mEdited->CreateRCArticulation(create);
    return 0;
}

PintActorHandle EditorPlugin::CreateRCArticulatedObject(const PINT_OBJECT_CREATE& oc,
                                                        const PINT_RC_ARTICULATED_BODY_CREATE& bc,
                                                        PintArticHandle articulation) {
    if (mEdited) return mEdited->CreateRCArticulatedObject(oc, bc, articulation);
    return 0;
}

bool EditorPlugin::AddRCArticulationToScene(PintArticHandle articulation) {
    if (mEdited) return mEdited->AddRCArticulationToScene(articulation);
    return false;
}

bool EditorPlugin::AddRCArticulationToAggregate(PintArticHandle articulation, PintActorHandle aggregate) {
    if (mEdited) return mEdited->AddRCArticulationToAggregate(articulation, aggregate);
    return false;
}

bool EditorPlugin::SetRCADriveEnabled(PintActorHandle handle, bool flag) {
    if (mEdited) return mEdited->SetRCADriveEnabled(handle, flag);
    return false;
}

bool EditorPlugin::SetRCADriveVelocity(PintActorHandle handle, float velocity) {
    if (mEdited) return mEdited->SetRCADriveVelocity(handle, velocity);
    return false;
}
#endif
...
void EditorPlugin::SetGearJointError(PintJointHandle handle, float error) {
    if (mEdited) return mEdited->SetGearJointError(handle, error);
}
#endif

Pint_Scene* EditorPlugin::GetSceneAPI() { return &mSceneAPI; }

Pint_Actor* EditorPlugin::GetActorAPI() { return &mActorAPI; }

Pint_Shape* EditorPlugin::GetShapeAPI() { return &mShapeAPI; }

Pint_Joint* EditorPlugin::GetJointAPI() { return &mJointAPI; }

Pint_Vehicle* EditorPlugin::GetVehicleAPI() { return &mVehicleAPI; }

Pint_Character* EditorPlugin::GetCharacterAPI() { return &mCharacterAPI; }

#ifdef TOSEE
void EditorPlugin::TestNewFeature() {
    if (mEdited) return mEdited->TestNewFeature();
}
#endif
...
void Editor_Close() {
    if (mEditedPlugin) mEditedPlugin->Close();
...
EditorPlugin* GetEditor() { return gEditor; }
...
void EditorPlugin::UpdateUI() {
    ...
    if (gEditBox_Stats) {
        ...
        if (mEditorScene) {
            ...
            const udword TotalNbUniqueShapes = stats.mBoxShapes.mNbUnique + stats.mSphereShapes.mNbUnique +
                                               stats.mCapsuleShapes.mNbUnique + stats.mCylinderShapes.mNbUnique +
                                               stats.mConvexShapes.mNbUnique + stats.mMeshShapes.mNbUnique;

            const udword TotalNbRefShapes = stats.mBoxShapes.mNbRef + stats.mSphereShapes.mNbRef +
                                            stats.mCapsuleShapes.mNbRef + stats.mCylinderShapes.mNbRef +
                                            stats.mConvexShapes.mNbRef + stats.mMeshShapes.mNbRef;

            CA.StoreASCII(_F("Gravity:   %.4f  |  %.4f  |  %.4f\n\n", mEditorScene->mGravity.x,
                             mEditorScene->mGravity.y, mEditorScene->mGravity.z));
            CA.StoreASCII(
                    _F("%d unique materials (%d referenced)\n", stats.mMaterials.mNbUnique, stats.mMaterials.mNbRef));
            ...
            CA.StoreASCII(_F("    %d unique box shapes (%d referenced)\n", stats.mBoxShapes.mNbUnique,
                             stats.mBoxShapes.mNbRef));
...
void ExportScene(String* string) {
    ...
    String Filename;
    if (!FileselectSave(Create, Filename, true)) return;
    ...
    if (EditorSceneData) UI_GetSceneDesc(EditorSceneData->mDesc);
...
#ifdef REMOVED
static void gExportButtonCallback(IceButton& /*button*/, void* /*user_data*/) { ExportScene(); }
#endif
...
public:
    ...
    virtual RenderHandle createConvexRenderer(const NvdConvexInterface& data) {
        return gUseRenderCaches ? gCurrentRenderModel->_CreateConvexRenderer(
                                          data.mNbVerts, reinterpret_cast<const Point*>(data.mVerts))
                                : null;
    }

    virtual RenderHandle createMeshRenderer(const SurfaceInterface& surface) {
        // TODO: revisit active edges param
        return gUseRenderCaches
                       ? gCurrentRenderModel->_CreateMeshRenderer(PintSurfaceInterface(surface), null, false, false)
                       : null;
    }
...
udword FrameSlider::SelectFrame(udword frame_index, bool update_tree_view, bool update_slider_pos) {
    ...
    if (update_slider_pos) SetIntValue(frame_index);
...
#endif
            if (mObjectTreeView) mObjectTreeView->RemoveAll();
        }
    }
...
void FrameSlider::OnSliderEvent(SliderEvent event) {
    ...
    */
    ...
    if (mEditBox) mEditBox->SetText(_F("%d", NewFrame));
...
static void gLoadCaptureButtonCallback(IceButton& button, void* user_data) {
    ...
    if (Nvd) {
        String Filename;
        {
            ...
            if (!FileselectOpenSingle(Create, Filename)) return;
        }
...
IceWindow* Editor_InitGUI(IceWidget* parent, PintGUIHelper& helper) {
    if (mEditedPlugin) mEditedPlugin->InitGUI(parent, helper);
...

    {
        ...
        gCheckBox_DrawSceneBounds = helper.CreateCheckBox(
                Main, EDITOR_GUI_DRAW_SCENE_BOUNDS, 4, y, CheckBoxWidth, 20, "Draw scene bounds (if available)",
                gEditorGUI, gDrawSceneBounds, gCheckBoxCallback, gTooltip_DrawSceneBounds);
        y += YStepCB;
    ...
    {
        //		y += YStep*2;
        helper.CreateButton(Main, 0, 4, y, 300, 20, "Export", gEditorGUI, gExportButtonCallback,
                            null /*helper.GetRootDirectory()*/);
        y += YStep * 2;
    }
#endif
    ...
    IceWindow* Tabs[TAB_COUNT];
    {
        ...
        // TAB_STATS
        {
...

            {
                ...
                gEditBox_Stats = helper.CreateEditBox(TabWindow, EDITOR_GUI_STATS, 4, y, 300, 550, "", gEditorGUI,
                                                      EDITBOX_TEXT, null);
        ...
        // TAB_NVD
        {
            ...
            FrameSlider* FS = null;
            {
                //				if(!mSlider)
                {
...

                    {
                        ...
                        //						SD->SetEditBox(helper.CreateEditBox(TabWindow,
                        //EDITOR_GUI_CURRENT_FRAME, EditBoxX, y, EditBoxWidth, 20, "0", gEditorGUI,
                        //EDITBOX_INTEGER_POSITIVE, Local::gEBCallback, null));
                        {
                            IceEditBox* EB = CreateSpinBox(TabWindow, EditBoxX, y, 0, EDITOR_GUI_CURRENT_FRAME);
                            EB->SetVisible(true);
                            EB->SetCallback(Local::gEBCallback);
                            if (gEditorGUI) gEditorGUI->Register(EB);
                            SD->SetEditBox(EB);
                        }
                    }
            ...
            /*			{
                                            ...
                                            helper.CreateCheckBox(Main, EDITOR_GUI_DRAW_SCENE_BOUNDS, 4, y,
               CheckBoxWidth, 20, "Draw scene bounds", gEditorGUI, gDrawSceneBounds, gCheckBoxCallback,
               gTooltip_DrawSceneBounds); y += YStepCB;

                                            y += YStepCB;
                                    }*/

            if (1) {
                helper.CreateButton(TabWindow, EDITOR_GUI_STOP_CAPTURE, 4, y, 400, 20, "Stop capture", gEditorGUI,
                                    gStopCaptureButtonCallback, FS /*helper.GetRootDirectory()*/);
                y += YStep * 2;
            }

            if (1) {
                helper.CreateButton(TabWindow, EDITOR_GUI_LOAD_CAPTURE, 4, y, 400, 20, "Load capture", gEditorGUI,
                                    gLoadCaptureButtonCallback, FS /*helper.GetRootDirectory()*/);
                y += YStep * 2;
            }
...
//				TVD.mStyle		= WSTYLE_CLIENT_EDGES;
            ...
            if (1) {
                ...
                if (0) {
                    ...
                    for (udword i = 0; i < 2000; i++) {
                        //					IceTreeViewItem* Group = TV->Add(RootItem, _F("Group%d",
                        //i));
                        IceTreeViewItem* Group = TV->Add(RootItem, "pipo");
                        for (udword j = 0; j < 256; j++) {
                            //						IceTreeViewItem* Item0 = TV->Add(Group, _F("Item%d",
                            //Index++));
        ...
        // TAB_NVD_SETTINGS
        {
            ...
            {
                /*gCheckBox_DrawSceneBounds =*/helper.CreateCheckBox(
                        TabWindow, EDITOR_GUI_DRAW_VISIBLE_BOUNDS, 4, y, CheckBoxWidth, 20, "Draw all bounds",
                        gEditorGUI, gDrawVisibleBounds, gCheckBoxCallback, null /*gTooltip_DrawSceneBounds*/);
                y += YStepCB;
            }
            {
                /*gCheckBox_DrawSceneBounds =*/helper.CreateCheckBox(
                        TabWindow, EDITOR_GUI_DRAW_GRID, 4, y, CheckBoxWidth, 20, "Draw grid", gEditorGUI, gDrawGrid,
                        gCheckBoxCallback, null /*gTooltip_DrawSceneBounds*/);
                y += YStepCB;
            }
            {
                /*gCheckBox_DrawSceneBounds =*/helper.CreateCheckBox(
                        TabWindow, EDITOR_GUI_OBJECT_NAMES_TREEVIEW, 4, y, CheckBoxWidth, 20,
                        "Use object names in tree-view", gEditorGUI, gUseObjectNamesInTreeView, gCheckBoxCallback,
                        null /*gTooltip_DrawSceneBounds*/);
                y += YStepCB;
            }
            {
                /*gCheckBox_DrawSceneBounds =*/helper.CreateCheckBox(
                        TabWindow, EDITOR_GUI_DISPLAY_INTERNAL_DATA, 4, y, CheckBoxWidth, 20,
                        "Display internal data in tree-view", gEditorGUI, gDisplayInternalData, gCheckBoxCallback,
                        null /*gTooltip_DrawSceneBounds*/);
                y += YStepCB;
            }
...
void Editor_CloseGUI() {
    if (mEditedPlugin) mEditedPlugin->CloseGUI();
...
public:
    ...
    virtual void CloseTestGUI() {
        if (mEditedPlugin) mEditedPlugin->CloseTestGUI();
    }
    virtual const char* GetTestGUIName() { return mEditedPlugin ? mEditedPlugin->GetTestGUIName() : null; }
    virtual void ApplyTestUIParams(const char* test_name) {
        if (mEditedPlugin) mEditedPlugin->ApplyTestUIParams(test_name);
    }
};
static EditorPlugIn gPlugIn;

PintPlugin* GetEditorPlugin() { return &gPlugIn; }

bool IsEditor(Pint* pint) { return pint == gEditor; }
...
static void ComputeAveragePosAndMass(
        float& mass, Point& pos, Pint* pint, udword nb, const PintActorHandle* handles, Quat* first_rot = null) {
    float Mass = 0.0f;
    ...
    for (udword i = 0; i < nb; i++) {
        ...
        if (i == 0 && first_rot) *first_rot = ActorPR.mRot;
    }
...
// Creates compound
PintActorHandle EditorPlugin::MergeActors(udword nb,
                                          const PintActorHandle* handles,
                                          MergeInterface& merge_interface,
        ...
        for (udword i = 0; i < nb; i++) {
            ...
            for (udword j = 0; j < NbShapes; j++) {
                ...
                if (dst) {
                    ...
                    if (!ActorDesc.mShapes) ActorDesc.SetShape(dst);
                    if (LastShape) LastShape->mNext = dst;
                    LastShape = dst;
                }
#endif
            }
...
PintActorHandle EditorPlugin::CreateSingleMesh(udword nb, const PintActorHandle* handles) {
    ...
    MeshCreate.SetSurfaceData(Verts.GetNbVertices(), Verts.GetVertices(), Indices.GetNbEntries() / 3,
                              Indices.GetEntries(), null);
...
PintActorHandle Editor_MakeSphere(PintActorHandle h) {
    if (gEditor) return gEditor->MergeActors(1, &h, MakeSphere(), MergeParams());
    return null;
}

PintActorHandle Editor_MakeBox(PintActorHandle h) {
    if (gEditor) return gEditor->MergeActors(1, &h, MakeBox(), MergeParams());
    return null;
}

PintActorHandle Editor_MakeConvex(PintActorHandle h) {
    if (gEditor) return gEditor->MergeActors(1, &h, MakeConvex(), MergeParams());
    return null;
}

PintActorHandle Editor_MakeMesh(PintActorHandle h) {
    if (gEditor) return gEditor->MergeActors(1, &h, MakeMesh(), MergeParams());
    return null;
}

#ifdef TOSEE
PintActorHandle Editor_MakeConvexDecomp(PintActorHandle h) {
    if (gEditor) return gEditor->ConvexDecomp(h);
    return null;
}
#endif
