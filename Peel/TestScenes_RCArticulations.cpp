...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    // static_cast<PxArticulationJointReducedCoordinate*>(leftRoot->getInboundJoint());
    ...
    for (udword i = 0; i < linkHeight; ++i) {
        ...
        //			PxArticulationLink* leftLink = gArticulation->createLink(currLeft, PxTransform(pos +
        // PxVec3(0.f, sinAng*(2 * i + 1), 0.f), leftRot));
        ...
        //			joint->setParentPose(PxTransform(currLeft->getGlobalPose().transformInv(leftAnchorLocation),
        // leftParentRot)); 			joint->setChildPose(PxTransform(PxVec3(0.f, 0.f, -1.f), rightRot));
        ...
        //			PxArticulationLink* rightLink = gArticulation->createLink(currRight, PxTransform(pos +
        // PxVec3(0.f, sinAng*(2 * i + 1), 0.f), rightRot));
        ...
        //			joint =
        // static_cast<PxArticulationJointReducedCoordinate*>(rightLink->getInboundJoint());
        //			joint->setJointType(PxArticulationJointType::eREVOLUTE);
        //			joint->setParentPose(PxTransform(currRight->getGlobalPose().transformInv(rightAnchorLocation),
        // rightParentRot)); 			joint->setChildPose(PxTransform(PxVec3(0.f, 0.f, 1.f), leftRot));
        //			joint->setMotion(PxArticulationAxis::eTWIST, PxArticulationMotion::eLIMITED);
        ...
        {
            //			PxD6Joint* d6joint = PxD6JointCreate(*gPhysics, leftLink, PxTransform(PxIdentity),
            //rightLink, PxTransform(PxIdentity)); 			d6joint->setMotion(PxD6Axis::eTWIST,
            // PxD6Motion::eFREE); 			d6joint->setMotion(PxD6Axis::eSWING1, PxD6Motion::eFREE);
    ...
    PxArticulationLink* leftTop = gArticulation->createLink(
            currLeft, currLeft->getGlobalPose().transform(PxTransform(PxVec3(-0.5f, 0.f, -1.0f), leftParentRot)));
    ...
    /*
    -		combo	{q={x=0.000000000 y=0.000000000 z=0.000000000 ...} p={x=-1.78813934e-007 y=3.46533942
    z=-0.900000334 } }	physx::PxTransform -		q	{x=0.000000000 y=0.000000000 z=0.000000000 ...}
    physx::PxQuat x	0.000000000	float y	0.000000000	float z	0.000000000	float w	1.00000012	float -
    p	{x=-1.78813934e-007 y=3.46533942 z=-0.900000334 }	physx::PxVec3 x	-1.78813934e-007	float
                    y	3.46533942	float
                    z	-0.900000334	float
    ...
    */
    ...
    PintActorHandle rightTop;
    {
        ...
        //		joint->setParentPose(PxTransform(PxVec3(0.f, 0.f, 1.f),
        // currRight->getGlobalPose().q.getConjugate())); 		joint->setChildPose(PxTransform(PxVec3(0.5f, 0.f,
        // 0.f), rightTop->getGlobalPose().q.getConjugate()));
    ...
    for (udword i = 0; i < linkHeight; ++i) {
        ...
        //		PxArticulationLink* leftLink = gArticulation->createLink(currLeft, PxTransform(pos + PxVec3(0.f,
        // sinAng*(2 * i + 1), 0.f), leftRot)); 		PxRigidActorExt::createExclusiveShape(*leftLink,
        // PxBoxGeometry(0.05f, 0.05f, 1.f), *gMaterial);
        // PxRigidBodyExt::updateMassAndInertia(*leftLink, 1.f);
        ...
        //		PxArticulationLink* rightLink = gArticulation->createLink(currRight, PxTransform(pos +
        //PxVec3(0.f,
        // sinAng*(2 * i + 1), 0.f), rightRot)); 		PxRigidActorExt::createExclusiveShape(*rightLink,
        // PxBoxGeometry(0.05f, 0.05f, 1.f), *gMaterial);
        // PxRigidBodyExt::updateMassAndInertia(*rightLink, 1.f);
        ...
        //		joint->setParentPose(PxTransform(currRight->getGlobalPose().transformInv(rightAnchorLocation),
        // rightParentRot)); 		joint->setJointType(PxArticulationJointType::eREVOLUTE);
    ...
    {
        /*		PxD6Joint* d6joint = PxD6JointCreate(*gPhysics, currLeft, PxTransform(PxVec3(0.f, 0.f, -1.f)),
           leftTop, PxTransform(PxVec3(-0.5f, 0.f, 0.f))); d6joint->setMotion(PxD6Axis::eTWIST, PxD6Motion::eFREE);
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportRCArticulations || !caps.mSupportRigidBodySimulation) return false;
    ...
    //		for(sdword yyy=0;yyy<50;yyy++)
    {
        //			for(sdword xxx=0;xxx<50;xxx++)
        {
            ...
            PintActorHandle box0;
            {
                ...
                //					const Point StaticPos(x+float(xxx-15)*Size, y,
                // z+float(yyy-15)*Size);
            ...
            for (udword i = 0; i < Nb; i++) {
                ...
                PintActorHandle box1;
                {
                    ...
                    //						const Point DynamicPos(x+float(xxx-15)*Size, y,
                    // z+float(yyy-15)*Size);
                    ...
                    if (RCA) {
                        //					PINT_RC_ARTICULATED_BODY_CREATE ArticulatedDesc;
                        //					box0 = pint.CreateRCArticulatedObject(ObjectDesc,
                        // ArticulatedDesc, RCA);
                        ...
                        /*					ArticulatedDesc.mLocalPivot0.mPos	=
                           TransformInv(pint, currLeft, leftAnchorLocation); ArticulatedDesc.mLocalPivot0.mRot	=
                           leftParentRot; ArticulatedDesc.mLocalPivot1.mPos	= Point(0.f, 0.f, -1.f);
                           ArticulatedDesc.mLocalPivot1.mRot	= rightRot;*/

                        ArticulatedDesc.mLocalPivot0.mPos = localAnchor0;
                        ArticulatedDesc.mLocalPivot1.mPos = Point(-ex, 0.0f, -ez);
                        //					ArticulatedDesc.mLocalAxis0		= Point(1.0f,
                        //0.0f, 0.0f); 					ArticulatedDesc.mLocalAxis1
            ...
            if (RCA) pint.AddRCArticulationToScene(RCA);
        }
    }
    return true;
}
...
static Point MujocoPosition(const Point& p) { return Point(p.x, p.z, p.y); }
...
static void SetupArticBodyDesc(PINT_RC_ARTICULATED_BODY_CREATE& desc) {
    ...
    if (gUseFixedJoints) {
        if (desc.mParent) desc.mJointType = PINT_JOINT_FIXED;
    }
...
static void SetupLink(
        PINT_OBJECT_CREATE& desc, const char* name, const Point& pos, const PINT_SHAPE_CREATE* shapes, float mass) {
    desc.mName = name;
    desc.mMass = mass;
...
void MatrixStack::Pop() { mNbMatrices--; }
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) {
...

    {
        ...
        mCheckBox_Debug_UseFakeLimits =
                helper.CreateCheckBox(UI, 0, 4, y, 200, 20, "DEBUG: use small limits", &UIElems, false, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Joint friction:", &UIElems);
        //			mEditBox_JointFriction = helper.CreateEditBox(UI, 1, 4+OffsetX, y, EditBoxWidth, 20,
        //"0.02", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
        mEditBox_JointFriction = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Max joint velocity:", &UIElems);
        mEditBox_MaxJointVelocity = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "20.0", &UIElems,
                                                         EDITBOX_FLOAT_POSITIVE, null, null);
        y += YStep;

        helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Timestep:", &UIElems);
        mEditBox_Timestep = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.005", &UIElems,
                                                 EDITBOX_FLOAT_POSITIVE, null, null);
        y += YStep;
    }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportRCArticulations || !caps.mSupportRigidBodySimulation || !caps.mSupportCompounds) return false;
    ...
    Point lwaistPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                // Child
                //					ArticulatedDesc.mLocalPivot1.mPos = ObjectDesc.mPosition -
                // JointRelPos;

                //					ArticulatedDesc.mLocalPivot0.mPos = MujocoPosition(Point(0.0f,
                //0.0f, 0.065f)); MS.Transform(ArticulatedDesc.mLocalPivot0.mPos, ArticulatedDesc.mLocalPivot0.mRot);
                //					ArticulatedDesc.mLocalPivot0.mPos =
                ...
                //<joint name="abdomen_z" type="hinge" pos="0 0 0.065" axis="0 0 1" range="-45 45" damping="5"
                // stiffness="20" armature="0.02" />
                ...
                //<joint name="abdomen_y" type="hinge" pos="0 0 0.065" axis="0 1 0" range="-75 30" damping="5"
                // stiffness="10" armature="0.02" />
    ...
    Point pelvisPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //<joint name="abdomen_x" type="hinge" pos="0 0 0.1" axis="1 0 0" range="-35 35" damping="5"
                // stiffness="10" armature="0.02" />
                ArticulatedDesc.mJointType = PINT_JOINT_HINGE;
                ArticulatedDesc.mAxisIndex = X_;
    ...
    Point right_thighPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //<joint name="right_hip_x" type="hinge" pos="0 0 0" axis="1 0 0" range="-25 5"   damping="5"
                // stiffness="10" armature="0.01" />
                ...
                //<joint name="right_hip_z" type="hinge" pos="0 0 0" axis="0 0 1" range="-60 35"  damping="5"
                // stiffness="10" armature="0.01" />
                ...
                //<joint name="right_hip_y" type="hinge" pos="0 0 0" axis="0 1 0" range="-120 20" damping="5"
                // stiffness="20" armature="0.01" />
    ...
    Point right_shinPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //<joint name="right_knee" type="hinge" pos="0 0 .02" axis="0 -1 0" range="-160 -2" stiffness="1"
                // armature="0.0060" />
                ArticulatedDesc.mJointType = PINT_JOINT_HINGE;
                ArticulatedDesc.mAxisIndex = Z_;
    ...
    Point right_footPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //<joint name="right_ankle_y" type="hinge" pos="0 0 0.08" axis="0 1 0"   range="-50 50" stiffness="4"
                // armature="0.0008" /> <joint name="right_ankle_x" type="hinge" pos="0 0 0.04" axis="1 0 0.5"
                // range="-50 50" stiffness="1"  armature="0.0006" />
    ...
    Point left_thighPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //<joint name="left_hip_x" type="hinge" pos="0 0 0" axis="-1 0 0" range="-25 5"  damping="5"
                // stiffness="10" armature="0.01" />
                ...
                //<joint name="left_hip_z" type="hinge" pos="0 0 0" axis="0 0 -1" range="-60 35" damping="5"
                // stiffness="10" armature="0.01" />
                ...
                //<joint name="left_hip_y" type="hinge" pos="0 0 0" axis="0 1 0" range="-120 20" damping="5"
                // stiffness="20" armature="0.01" />
                //					ArticulatedDesc.mMinSwing2Limit		= -20.0f * DEGTORAD;
                //					ArticulatedDesc.mMaxSwing2Limit		= 120.0f * DEGTORAD;
                //						ArticulatedDesc.mMinSwing2Limit		= 0.0f;
    ...
    Point left_shinPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //<joint name="left_knee" type="hinge" pos="0 0 .02" axis="0 -1 0" range="-160 -2" stiffness="1"
                // armature="0.0060" />
                ArticulatedDesc.mJointType = PINT_JOINT_HINGE;
                ArticulatedDesc.mAxisIndex = Z_;
                ArticulatedDesc.mMinLimit = -160.0f * DEGTORAD;
    ...
    Point left_footPos;
    {
        ...
        if (RCA) {
            ...
            if (1) {
                ...
                //	<joint name="left_ankle_y" type="hinge" pos="0 0 0.08" axis="0 1 0"   range="-50 50"
                // stiffness="4" armature="0.0008" /> 	<joint name="left_ankle_x" type="hinge" pos="0 0 0.04" axis="1 0
                // 0.5" range="-50 50"  stiffness="1"  armature="0.0006" />
        ...
        Point right_upper_armPos;
        {
            ...
            if (RCA) {
                ...
                if (1) {
                    ...
                    Point axis0(2.0f, 1.0f, 1.0f);
                    axis0.Normalize();
                    Point axis1(0.0f, -1.0f, 1.0f);
                    axis1.Normalize();
                    ...
                    //                <joint name="right_shoulder1" type="hinge" pos="0 0 0" axis="2 1 1"  range="-85
                    //                60" stiffness="1" armature="0.0068" /> <joint name="right_shoulder2" type="hinge"
                    //                pos="0 0 0" axis="0 -1 1" range="-85 60" stiffness="1"  armature="0.0051" />

                    //						ArticulatedDesc.mMinTwistLimit		= -85.0f *
                    // DEGTORAD; 						ArticulatedDesc.mMaxTwistLimit		= 60.0f *
                    // DEGTORAD;
                    SetupTwistLimits(ArticulatedDesc, -85.0f, 60.0f);

                    //						ArticulatedDesc.mMinSwing1Limit		= 0.0f *
                    // DEGTORAD; 						ArticulatedDesc.mMaxSwing1Limit		= 0.0f *
                    // DEGTORAD;
                    SetupSwing1Limits(ArticulatedDesc, 0.0f, 0.0f);

                    //						ArticulatedDesc.mMinSwing2Limit		= -85.0f *
                    // DEGTORAD; 						ArticulatedDesc.mMaxSwing2Limit		= 60.0f *
                    // DEGTORAD;
        ...
        Point left_upper_armPos;
        {
            ...
            if (RCA) {
                ...
                if (1) {
                    ...
                    //                <joint name="left_shoulder1" type="hinge" pos="0 0 0" axis="2 -1 1" range="-60 85"
                    //                stiffness="1" armature="0.0068" /> <joint name="left_shoulder2" type="hinge"
                    //                pos="0 0 0" axis="0 1 1" range="-60 85"  stiffness="1" armature="0.0051" />

                    //						ArticulatedDesc.mMinTwistLimit		= -60.0f *
                    // DEGTORAD; 						ArticulatedDesc.mMaxTwistLimit		= 85.0f *
                    // DEGTORAD;
                    ...
                    //						ArticulatedDesc.mMinSwing2Limit		= -60.0f *
                    // DEGTORAD; 						ArticulatedDesc.mMaxSwing2Limit		= 85.0f *
                    // DEGTORAD;
        ...
        Point left_lower_armPos;
        {
            ...
            if (RCA) {
                ...
                if (1) {
                    ...
                    //                    <joint name="left_elbow" type="hinge" pos="0 0 0" axis="0 -1 -1" range="-90
                    //                    50" stiffness="0" armature="0.0028" />
                    ArticulatedDesc.mJointType = PINT_JOINT_HINGE;
                    ArticulatedDesc.mAxisIndex = Z_;
                    ArticulatedDesc.mMinLimit = -90.0f * DEGTORAD;
                    ArticulatedDesc.mMaxLimit = 50.0f * DEGTORAD;
