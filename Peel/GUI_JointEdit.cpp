...
EditJointWindow::EditJointWindow(const WindowDesc& desc) : IceWindow(desc) {}
...
void CloseJointEditGUI() { DELETESINGLE(gEditJointWindow); }

void HideEditJointWindow() {
    if (!gEditJointWindow) return;
...
JointTypeComboBox* CreateJointComboBox(IceWidget* parent, sdword x, sdword y, void* user_data, EditJointTypeCallback&
callback)
{
        ComboBoxDesc CBD;
        CBD.mID		= 0;
