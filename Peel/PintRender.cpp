...
void DefaultRenderer::DrawWireframeAABB(udword nb_boxes, const AABB* boxes, const Point& color) {
    ...
    while (NbRemaining) {
        ...
        if (NbToGo > BatchSize) NbToGo = BatchSize;
...
void DefaultRenderer::DrawSphere(float radius, const PR& pose) { GLRenderHelpers::DrawSphere(radius, pose); }

void DefaultRenderer::DrawBox(const Point& extents, const PR& pose) { GLRenderHelpers::DrawBox(extents, pose); }

void DefaultRenderer::DrawCapsule(float r, float h, const PR& pose) { GLRenderHelpers::DrawCapsule(r, h, pose); }
...
const Plane* DefaultRenderer::GetFrustumPlanes() { return gCameraVFC ? ::GetFrustumPlanes() : null; }
...
DefaultRenderer::~DefaultRenderer() { Release(); }
...
void DefaultRenderer::EndRender() {
...

    {
        ...
        if (mTransparentShapes.GetNbEntries()) {
            ...
            //  TODO: not sure this is enough, different batched renderers might be transparent with different colors
            //  etc so we need to replicate what we do in the opaque pipeline
            StartBatchConvexRender();
            StartInstanceRender();
...
bool DefaultRenderer::SetCurrentActor(PintActorHandle h) {
    ...
    if (mPass == PINT_RENDER_PASS_MAIN || mPass == PINT_RENDER_PASS_REFLECTIONS) {
        if (RenderEnabled && mSelManager) {
            if (mSelManager->IsSelectedNative(mPint, h)) Flags = HIGHLIGHT_SELECTION;
        }
...
inline_ void DefaultRenderer::ProcessShape(PintShapeRenderer* shape, const PR& pose) {
    if ((shape->mBaseFlags & SHAPE_RENDERER_TRANSPARENT) && mPass != PINT_RENDER_PASS_SHADOW &&
        mPass != PINT_RENDER_PASS_WIREFRAME_OVERLAY) {
        ...
        bool Switch = false;
        if (Color != gColor || Texture != gTexture || NeedsVertexNormals != gNeedsVertexNormals) {
            gColor = Color;
            gTexture = Texture;
        ...
        // ### this isn't really about a "render switch", since the batched convex renderers are different for each
        // convex for example! But they internally refer to the same shared/global memory buffer.
...
void DefaultRenderer::DrawShape(PintShapeRenderer* shape, const PR& pose) {
    ...
    if (0 && (shape->mBaseFlags & SHAPE_RENDERER_TRANSPARENT) && mPass != PINT_RENDER_PASS_SHADOW &&
        mPass != PINT_RENDER_PASS_WIREFRAME_OVERLAY) {
        BatchedData* BD = ICE_RESERVE(BatchedData, mTransparentShapes);
    ...
    // - default implementation calls a DefaultRenderer's function doing the transparent stuff above + the SetupMaterial
    // / RenderShape calls
