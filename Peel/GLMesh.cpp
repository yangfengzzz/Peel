...
static inline_ void UpdateVertexBuffer(
        udword nb_verts, const Point* verts, const Point* vertex_normals, GLuint pos_id, GLuint nrm_id) {
    if (verts) {
    ...
    if (verts || vertex_normals) glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
GLIndexBuffer::~GLIndexBuffer() { Release(); }
...
void GLIndexBuffer::Select() const { glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mIndicesIBO); }

void GLIndexBuffer::Unselect() const { glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0); }

///////////////////////////////////////////////////////////////////////////////

GLVertexBuffer::GLVertexBuffer() : mNbVerts(0), mPositionsVBO(0), mNormalsVBO(0) {}

GLVertexBuffer::~GLVertexBuffer() { Release(); }
...
void GLVertexBuffer::Unselect() const {
    ...
    if (mNormalsVBO) glDisableClientState(GL_NORMAL_ARRAY);
...
void GLIndexBufferCollection::AddIndexBuffer(const GLIndexBuffer& index_buffer) { mIndexBuffers.AddPtr(&index_buffer); }
...
static GLMesh* CreateMesh_DL(udword nb_verts,
                             const Point* verts,
...
static GLMesh* CreateMesh_VBO(udword nb_verts,
                              const Point* verts,
                              const Point* vnormals,
                              udword nb_tris,
                              const udword* indices32,
...
GLMesh* OpenGLMesh::Create(udword nb_verts,
                           const Point* verts,
                           const Point* vnormals,
                           udword nb_tris,
                           const udword* indices32,
...
void OpenGLMesh::Draw(GLMesh* m /*, const Matrix44& xform, const Vec3& color*/) {
    ...
    if (m->mType == GL_MESH_DISPLAY_LIST) {
        ...
        /*		GLint program;
                        ...
                        if (program)
                                glUniformMatrix4fv( glGetUniformLocation(program, "objectTransform"), 1, false,
           xform);*/
        ...
        glDisableClientState(GL_VERTEX_ARRAY);
        if (mesh->mNormalsVBO) glDisableClientState(GL_NORMAL_ARRAY);
...
GLMeshEx* OpenGLMesh::Create(const MultiSurface& multi_surface, udword /*flags*/, bool b) {
    ...
    for (udword i = 0; i < Create.NbFaces; i++) {
        ...
        if (UVSurface) FD.TRefs = UVSurface->GetFace(i)->mRef;

        if (Normals) FD.CRefs = VRefs;
        /*
                        FD.CRefs			= ColorSurface		? (udword*)ColorSurface ->GetFace(i)
        : null; #ifdef MMZ_SUPPORT_SECOND_MAPPING_CHANNEL FD.TRefs2			= UVSurface2		?
        (udword*)UVSurface2	->GetFace(i)	: null; #endif
                        FD.SmoothingGroups	= create.mSMG		? create.mSMG[i]
        : 1;
                        FD.MaterialID		= create.mDMatID	? create.mDMatID[i]	: create.mWMatID
        ? create.mWMatID[i]	: INVALID_ID;*/
...
void OpenGLMesh::Draw(GLMeshEx* mesh) {
    ...
    glDisableClientState(GL_VERTEX_ARRAY);
    if (mesh->mNormalsVBO) glDisableClientState(GL_NORMAL_ARRAY);
    if (mesh->mTexCoordsVBO) glDisableClientState(GL_TEXTURE_COORD_ARRAY);
...
static const char* testVertexShader =
        "#version 330 core\n" STRINGIFY(layout(location = 0) in vec3 aPos; layout(location = 1) in mat4 instanceMatrix;
...
static const char* testPixelShader = "#version 330 core\n" STRINGIFY(out vec4 FragColor; in vec4 varyPos;
                                                                     ...
                                                                     void main() {
                                                                         ...
                                                                         //		float diffuse = (0.3 + 0.7 *
                                                                         // max(dot(normal, -parallelLightDir),0.0));
                                                                         //	float diffuse = max(dot(normal,
                                                                         //-parallelLightDir), 0.0);
                                                                         //		diffuse = clamp(diffuse,
                                                                         // 0.0, 1.0); 		vec4 color = gl_Color *
                                                                         // diffuse;

                                                                         FragColor = vec4(normal, 1.0f);
                                                                         //		FragColor = vec4(1.0f, 0.5f,
                                                                         // 0.2f, 1.0f);
                                                                     });
...
void ModernTest() {
    ...
    static float vertices[] = {0.5f, 0.5f, -0.5f, 0.5f, -0.5f, -0.5f, -0.5f, -0.5f, -0.5f, -0.5f, 0.5f, -0.5f,
                               0.5f, 0.5f, 0.5f,  0.5f, -0.5f, 0.5f,  -0.5f, -0.5f, 0.5f,  -0.5f, 0.5f, 0.5f};
    static udword indices[] = {0, 1, 3, 3, 1, 2, 4, 7, 5, 5, 7, 6, 0, 4, 1, 1, 4, 5,
                               7, 3, 6, 6, 3, 2, 0, 3, 4, 4, 3, 7, 5, 6, 1, 1, 6, 2};
...
#define NB_INSTANCES 100
    ...
    if (!InitDone) {
        ...
        glEnableVertexAttribArray(baseLoc + 0);
        glVertexAttribPointer(baseLoc + 0, 4, GL_FLOAT, GL_FALSE, sizeof(Matrix4x4), (void*)0);
        glEnableVertexAttribArray(baseLoc + 1);
        glVertexAttribPointer(baseLoc + 1, 4, GL_FLOAT, GL_FALSE, sizeof(Matrix4x4), (void*)(vec4Size));
        glEnableVertexAttribArray(baseLoc + 2);
        glVertexAttribPointer(baseLoc + 2, 4, GL_FLOAT, GL_FALSE, sizeof(Matrix4x4), (void*)(2 * vec4Size));
        glEnableVertexAttribArray(baseLoc + 3);
        glVertexAttribPointer(baseLoc + 3, 4, GL_FLOAT, GL_FALSE, sizeof(Matrix4x4), (void*)(3 * vec4Size));
