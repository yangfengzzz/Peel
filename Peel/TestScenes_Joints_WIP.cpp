...
VehicleBase::VehicleBase()
    : mAcceleration(0.0f), mMaxAngularVelocity(0.0f), mSteeringForce(0.0f), mClampAngularVelocity(false) {}
    ...
    if (mClampAngularVelocity) {
        ...
        for (udword i = 0; i < 2; i++) {
            if (vehicle_data.mFront[i].mWheel && vehicle_data.mFront[i].mParent) {
                if (::ClampAngularVelocity(pint, vehicle_data.mFront[i].mParent, vehicle_data.mFront[i].mWheel,
                                           MaxAngularVelocity))
                    CanAccelerate = false;
            }
        }
        for (udword i = 0; i < 2; i++) {
            if (vehicle_data.mRear[i].mWheel && vehicle_data.mRear[i].mParent) {
                if (::ClampAngularVelocity(pint, vehicle_data.mRear[i].mParent, vehicle_data.mRear[i].mWheel,
                                           MaxAngularVelocity))
                    CanAccelerate = false;
            }
        }
...
static const char* gDesc_VehicleTest =
        "Vehicle test. This uses regular joints (no articulation). This is an experimental, work-in-progress test. It has not been properly tuned or \
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("VehicleTest settings", 20); }
...
virtual void CreateVehicle(Pint& pint,
                           const Point& pos,
                           ChassisData* UserData,
                           const PINT_MATERIAL_CREATE& LowFrictionMaterial,
                           PintCollisionGroup ChassisGroup,
                           PintCollisionGroup WheelGroup) {
...

    {
        ...
        if (UserData) UserData->mChassis = Chassis;
    }
    ...
    for (udword j = 0; j < 2; j++) {
        ...
        for (udword i = 0; i < 2; i++) {
            ...
            const Point Offset(x * (Extents.x - SuspExtents.x), -Extents.y - SuspExtents.y,
                               z * (Extents.z - SuspExtents.z));
            ...
            if (FrontWheels) {
                ...
                WheelPosOffset.x = 0.0f;
                {
                    ...
                    if (UserData) UserData->mFrontAxleObject[j] = HingeObject;
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportHingeJoints || !caps.mSupportPrismaticJoints || !caps.mSupportRigidBodySimulation) return false;
    ...
    const PintDisabledGroups DG[3 * 2] = {PintDisabledGroups(1, 2), PintDisabledGroups(1, 1), PintDisabledGroups(2, 2),

                                          PintDisabledGroups(3, 4), PintDisabledGroups(3, 3), PintDisabledGroups(4, 4)};
...
/*static*/ PintActorHandle GenerateTireCompound(Pint& pint,
                                                const Point& torus_pos,
                                                const Quat& torus_rot,
                                                float mass,
                                                PintCollisionGroup group,
                                                const PINT_MATERIAL_CREATE& material,
                                                float scale,
                                                float mass_inertia_coeff) {
    // Generate torus. We generate a torus by sweeping a small circle along a large circle, taking N samples (slices)
    // along the way.
    ...
    Point SliceCenters[NbSlices];
    {
        ...
        for (udword j = 0; j < NbSlices; j++) {
            ...
            for (udword i = 0; i < NbPtsSmallCircle; i++) Verts[Index++] = (Trans + SmallCirclePts[i] * Rot) * TRot;
    ...
    //  Next, we generate a convex object for each part of the torus. A part is a section connecting two of the previous
    //  slices.
        ...
        //		Handles[s] = CreateDynamicObject(pint, &ConvexCreate, pos+Center);
                        {
                                ...
                                // Each convex has been computed from rotated small circles, and we only re-centered the
        verts but
                                // didn't cancel the rotation. In theory we'd only need one convex object that we could
        translate/rotate,
                                // but it was easier to create different parts with different rotations. As a result we
        don't need to set
...
static void GenerateTire_Issue_TGS_64(
        Pint& pint, const Point& torus_pos, const Quat& torus_rot, float mass, udword nb_loops, Vertices& debug_pts) {
    // Generate torus. We generate a torus by sweeping a small circle along a large circle, taking N samples (slices)
    // along the way.
    ...
    if (0) {
        for (udword i = 0; i < NbPtsSmallCircle; i++) debug_pts.AddVertex(SmallCirclePts[i]);
    }
    ...
    if (0) {
        for (udword i = 0; i < NbSlices; i++) debug_pts.AddVertex(SliceCenters[i]);

        for (udword i = 0; i < TotalNbVerts; i++) debug_pts.AddVertex(Verts[i]);
        return;
    }
    ...
    if (CreateFixedJoints && mass != 0.0f) {
        for (udword i = 0; i < nb_loops; i++) {
            for (udword s = 0; s < NbSlices; s++) {
                const udword SliceIndex0 = s;
                const udword SliceIndex1 = (s + 1) % NbSlices;
                //				const Point Delta = (ObjectCenters[SliceIndex1] -
                // ObjectCenters[SliceIndex0])*0.5f;
...
static void GenerateTire_Issue_Explosion(
        Pint& pint, const Point& torus_pos, const Quat& torus_rot, float mass, udword nb_loops, Vertices& debug_pts) {
    // Generate torus. We generate a torus by sweeping a small circle along a large circle, taking N samples (slices)
    // along the way.
    const float BigRadius = 3.0f;
...
static PintActorHandle GenerateTire_Prismatic(Pint& pint,
                                              const Point& torus_pos,
                                              const Quat& torus_rot,
                                              float mass,
                                              udword nb_loops,
    ...
    if (0 && mass != 0.0f) RCArticulation = pint.CreateRCArticulation(PINT_RC_ARTICULATION_CREATE());
...
static PintActorHandle CreateAggregatedPintObject(Pint& pint, PINT_OBJECT_CREATE& desc, PintAggregateHandle aggregate) {
    ...
    if (aggregate) pint.AddToAggregate(H, aggregate);
    return H;
}

// ### copied from bulldozer

static PintActorHandle CreateBoxObject(Pint& pint,
                                       PintAggregateHandle aggregate,
                                       PintArticHandle rca,
                                       const Point& extents,
                                       const Point& pos,
                                       const Quat& rotation,
                                       float mass,
                                       PtrContainer* joints,
                                       PintActorHandle anchor,
                                       const Point& anchor_pivot,
                                       bool top_or_bottom,
                                       const PINT_MATERIAL_CREATE* material,
                                       udword& nb_rca_links) {
    PINT_BOX_CREATE BoxDesc(extents);
...
static void CreatePrismatic(Pint& pint,
                            PtrContainer* joints,
                            const Point& p0,
                            const Point& p1,
                            PintActorHandle top_anchor,
                            const Point& top_pivot,
                            PintActorHandle bottom_anchor,
                            const Point& bottom_pivot,
                            float mass,
                            float stiffness,
                            float damping,
                            PintAggregateHandle aggregate,
    ...
    const PintActorHandle TopPrismaticObject =
            CreateBoxObject(pint, aggregate, rca, TopPrismaticExtents, TopPrismaticPos, Q, mass, joints, top_anchor,
                            top_pivot, true, material, nb_rca_links);
    const PintActorHandle BottomPrismaticObject =
            CreateBoxObject(pint, aggregate, rca, BottomPrismaticExtents, BottomPrismaticPos, Q, mass, joints,
                            bottom_anchor, bottom_pivot, false, material, nb_rca_links);
...
public:
    ...
    void Release();
    void GenerateTireCompound(const Quat& torus_rot,
                              const PINT_MATERIAL_CREATE& material,
                              float big_radius,
                              float small_radius,
                              udword nb_slices,
                              udword nb_pts_small_circle);
...
WheelCreator::WheelCreator() : mNbSlices(0), mVertsCopy(null), mShapes(null), mVerts(null) {}

WheelCreator::~WheelCreator() { Release(); }
...
void WheelCreator::GenerateTireCompound(const Quat& torus_rot,
                                        const PINT_MATERIAL_CREATE& material,
                                        float big_radius,
                                        float small_radius,
                                        udword nb_slices,
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        // TAB_GENERAL
        {
            ...
            mCheckBox_DriveVehicle =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Drive vehicle", &UIElems, true, null, null);
            y += YStep;

            mCheckBox_AttachCar =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Attach car", &UIElems, false, null);
            y += YStep;

            mCheckBox_AttachWheels =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Attach wheels", &UIElems, true, null);
            y += YStep;

            mCheckBox_AttachSteering =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Attach steering", &UIElems, true, null);
            y += YStep;

            mCheckBox_UseRCA = helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Use RCA", &UIElems, true, null);
            y += YStep;

            mCheckBox_UseAggregates =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Use aggregates", &UIElems, false, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Friction:", &UIElems);
            mEditBox_Friction = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "RCA Friction:", &UIElems);
            mEditBox_RCAFriction = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
                                                        EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Auto speed:", &UIElems);
            mEditBox_Speed = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
                                                  EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Inertia coeff:", &UIElems);
            mEditBox_InertiaCoeff = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "-1.0",
                                                         &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Multiplier:", &UIElems);
            mEditBox_Multiplier = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "1", &UIElems,
                                                       EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Global scale:", &UIElems);
            mEditBox_GlobalScale = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
                                                        EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camera up offset:", &UIElems);
            mEditBox_CameraUpOffset = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "5.0",
                                                           &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camera distance:", &UIElems);
            mEditBox_CameraDistToTarget = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "15.0",
                                                               &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;
        }
        ...
        // TAB_CHASSIS
        {
            ...
            mEditBox_ChassisWidth = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Height:", &UIElems);
            mEditBox_ChassisHeight = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.2",
                                                          &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Length:", &UIElems);
            mEditBox_ChassisLength = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "5.0",
                                                          &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Mass chassis:", &UIElems);
            mEditBox_ChassisMass = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1500.0",
                                                        &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Friction:", &UIElems);
            mEditBox_ChassisFriction = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.2",
                                                            &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "COM X offset:", &UIElems);
            mEditBox_ChassisCOMOffsetX = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                              &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "COM Y offset:", &UIElems);
            mEditBox_ChassisCOMOffsetY = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                              &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "COM Z offset:", &UIElems);
            mEditBox_ChassisCOMOffsetZ = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                              &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "MOI Y coeff:",
               &UIElems); mEditBox_ChassisMOICoeffY = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth, 20,
               "0.8", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Force app. CM Y
               offset (~body roll):", &UIElems); mEditBox_ForceApplicationCMOffsetY = helper.CreateEditBox(TabWindow, 1,
               4+OffsetX, y, EditBoxWidth, 20, "0.3", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;*/
        }
        ...
        // TAB_WHEEL_TIRES
        {
            ...
            mEditBox_WheelRadius = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.5", &UIElems,
                                                        EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Width:", &UIElems);
            mEditBox_WheelWidth = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Tessellation R:", &UIElems);
            mEditBox_WheelTessR = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "32", &UIElems,
                                                       EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Tessellation W:", &UIElems);
            mEditBox_WheelTessW = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "6", &UIElems,
                                                       EDITBOX_INTEGER_POSITIVE, null, null);
            y += YStep;

            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Coeff X:", &UIElems);
                                    mEditBox_WheelCoeffX = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "0.85", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Coeff Z:",
               &UIElems); mEditBox_WheelCoeffZ = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth, 20,
               "0.85", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;*/

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Mass:", &UIElems);
            mEditBox_WheelMass = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "16.0", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Friction:", &UIElems);
            mEditBox_WheelFrontFriction = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.8",
                                                               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            //			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Rear friction:",
            //&UIElems); 			mEditBox_WheelRearFriction = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
            //EditBoxWidth, 20, "1.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); 			y += YStep;

            mCheckBox_TweakWheelsInertia =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 100, 20, "Tweak inertia", &UIElems, false, null);
            y += YStep;

            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Max brake torque
               front:", &UIElems); mEditBox_WheelMaxBrakeTorqueFront = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "150.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Max brake torque
               rear:", &UIElems); mEditBox_WheelMaxBrakeTorqueRear = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "1500.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Max steer front:",
               &UIElems); mEditBox_WheelMaxSteerFront = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth,
               20, "1.047", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Max steer rear:",
               &UIElems); mEditBox_WheelMaxSteerRear = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth,
               20, "0.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Tire friction
               multiplier (~grip):", &UIElems); mEditBox_TireFrictionMultiplier = helper.CreateEditBox(TabWindow, 1,
               4+OffsetX, y, EditBoxWidth, 20, "1.1", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;*/
        ...
        // TAB_ENGINE_GEARBOX
        {
            ...
            mEditBox_EngineTorque = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "500.0",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Engine peak torque:",
               &UIElems); mEditBox_EnginePeakTorque = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth, 20,
               "1000.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Engine max
               omega:", &UIElems); mEditBox_EngineMaxOmega = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "1000.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Gears switch
               time:", &UIElems); mEditBox_GearsSwitchTime = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "0.5", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Clutch strength:",
               &UIElems); mEditBox_ClutchStrength = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth, 20,
               "10.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    IceEditBox* tmp = helper.CreateEditBox(TabWindow, 0, 4, y+20, 250, 100, "",
               &UIElems, EDITBOX_TEXT, null); tmp->SetMultilineText("Most engine & gearbox params are
               currently\nhardcoded and not exposed to the UI."); tmp->SetReadOnly(true);*/
        }
        ...
        // TAB_SUSPENSION
        {
            ...
            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Suspension:",
               &UIElems);
                                    {
                                            ComboBoxDesc CBBD;
                                            CBBD.mID		= 0;
            ...
            mEditBox_MassElements = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "10.0",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Stiffness:", &UIElems);
            mEditBox_SuspStiffness = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "80000.0",
                                                          &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Damping:", &UIElems);
            mEditBox_SuspDamping = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "8000.0",
                                                        &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Height:", &UIElems);
            mEditBox_SuspHeight = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "2.0", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Limit angle:", &UIElems);
            mEditBox_SuspLimitAngle = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "45.0",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Wishbone width:", &UIElems);
            mEditBox_WishboneWidth = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.75",
                                                          &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Wishbone height:", &UIElems);
            mEditBox_WishboneHeight = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Wishbone length:", &UIElems);
            mEditBox_WishboneLength = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Attach width:", &UIElems);
            mEditBox_AttachWidth = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.25",
                                                        &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Attach height:", &UIElems);
            mEditBox_AttachHeight = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.25",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Attach length:", &UIElems);
            mEditBox_AttachLength = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Max compression:",
               &UIElems); mEditBox_SuspMaxCompression = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth,
               20, "0.3", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Max droop:",
               &UIElems); mEditBox_SuspMaxDroop = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth, 20,
               "0.1", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Spring strength:",
               &UIElems); mEditBox_SuspSpringStrength = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth,
               20, "35000.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Spring damper
               rate:", &UIElems); mEditBox_SuspSpringDamperRate = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "4500.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Camber angle at
               rest:", &UIElems); mEditBox_SuspCamberAngleAtRest = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y,
               EditBoxWidth, 20, "0.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Camber angle at
               max compression:", &UIElems); mEditBox_SuspCamberAngleAtMaxCompr = helper.CreateEditBox(TabWindow, 1,
               4+OffsetX, y, EditBoxWidth, 20, "0.01", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;

                                    helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Camber angle at
               max droop:", &UIElems); mEditBox_SuspCamberAngleAtMaxDroop = helper.CreateEditBox(TabWindow, 1,
               4+OffsetX, y, EditBoxWidth, 20, "0.01", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null); y += YStep;*/
        }
        ...
        // TAB_DIFFERENTIAL
        {
            ...
            /*			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Differential:",
               &UIElems);
                                    {
                                            ComboBoxDesc CBBD;
                                            CBBD.mID		= 0;
        ...
        // TAB_STEERING
        {
            ...
            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Stiffness:", &UIElems);
            //			mEditBox_SteerStiffness = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth,
            //20, "10000.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_SteerStiffness = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "30000.0",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Damping:", &UIElems);
            mEditBox_SteerDamping = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "5000.0",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Limit angle:", &UIElems);
            mEditBox_SteerLimitAngle = helper.CreateEditBox(TabWindow, 2, 4 + OffsetX, y, EditBoxWidth, 20, "40.0",
                                                            &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
...
#define APP_TO_GW(x) -(x)
                ...
                Keyframe k;
                k.mX = 5.0f / 120.0f;
                k.mY = APP_TO_GW(0.75f);
                GW->CreateKey(k);
                k.mX = 30.0f / 120.0f;
                k.mY = APP_TO_GW(0.125f);
                GW->CreateKey(k);
                k.mX = 120.0f / 120.0f;
                k.mY = APP_TO_GW(0.1f);
                GW->CreateKey(k);
            ...
            if (Index == 0 || Index == 1) {
                ...
                if (Index == 1) {
                    if (1) {
                        ...
                        IS->Init(LTL.mNbTris, LTL.mNbVerts, LTL.mVerts,
                                 reinterpret_cast<const IndexedTriangle*>(LTL.mIndices));
    ...
    virtual bool CommonSetup() {
        ...
        if (!AttachCar) InitLevel(null, null, null);

        return VehicleInput::CommonSetup();
    }

    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        //		if(!caps.mSupportRigidBodySimulation || !caps.mSupportConvexes || !caps.mSupportFixedJoints ||
        //! caps.mSupportCollisionGroups) 			return false;
        ...
        //		mCameraMode = 0;
        const bool TweakWheelsInertia =
                mCheckBox_TweakWheelsInertia ? mCheckBox_TweakWheelsInertia->IsChecked() : false;
        ...
        if (mComboBox_WheelType) WT = WheelType(mComboBox_WheelType->GetSelectedIndex());
        ...
        if (mComboBox_DriveType) DT = DriveType_(mComboBox_DriveType->GetSelectedIndex());
        ...
        const PintAggregateHandle Aggregate =
                UseAggregates ? pint.CreateAggregate(128, false) : null;  // TODO: revisit params
        ...
        for (udword j = 0; j < NbWheels / 2; j++) {
            ...
            for (udword i = 0; i < 2; i++) {
                ...
                if (!IsFront) {
                    // We don't create the second attachment for back wheels so we increase the sides a bit so that
                    // front & back wheels align
                    BottomSideExtents.z += WheelAttachExtents.z * 2.0f;
                    TopSideExtents.z += WheelAttachExtents.z * 2.0f;
                }

                const Point BottomSidePos = BasePos + Point((-BaseExtents.x + BottomSideExtents.x) * SideX, 0.0f,
                                                            (BaseExtents.z + BottomSideExtents.z) * SideZ);
                PintActorHandle BottomSideHandle;
                {
                    ...
                    if (RCA) {
                        ...
                        /*						ObjectDesc.mAddToWorld	= !Aggregate;
                                                                        BottomSideHandle = CreatePintObject(pint,
                           ObjectDesc); if(Aggregate) pint.AddToAggregate(BottomSideHandle, Aggregate);*/
                ...
                /*				const float h = WheelAttachHeight*2.0f;
                                                ...
                                                PintActorHandle TopSideHandle;
                                                {
                                                        ...
                                                        PINT_OBJECT_CREATE ObjectDesc(&Desc);
                                                        ObjectDesc.mMass			= ElemMass;
                                                        ObjectDesc.mMassForInertia	=
                   ObjectDesc.mMass*MassInertiaCoeff; ObjectDesc.mPosition		= TopSidePos;
                                                        ObjectDesc.mCollisionGroup	= 1;

                                                        const Point Pivot0((-BaseExtents.x+BottomSideExtents.x)*SideX,
                   h, BaseExtents.z*SideZ); const Point Pivot1(0.0f, 0.0f, -TopSideExtents.z*SideZ);

                                                        if(RCA && 0)	// Break the loop here
                                                        {
                                                                PINT_RC_ARTICULATED_BODY_CREATE ArticulatedDesc;
                                                                ArticulatedDesc.mJointType			=
                   PINT_JOINT_HINGE; ArticulatedDesc.mParent				= BaseHandle;
                                                                ArticulatedDesc.mAxisIndex			= X_;
                                                                ArticulatedDesc.mLocalPivot0.mPos	= Pivot0;
                                                                ArticulatedDesc.mLocalPivot1.mPos	= Pivot1;
                                                                ArticulatedDesc.mFrictionCoeff		= RCAFriction;
                   ...
                   pint.CreateRCArticulatedObject(ObjectDesc, ArticulatedDesc, RCA);
                                                        ...
                                                        else
                                                        {
...

                                                                {
                                                                        ...
                                                                        fjc.mLocalAxis0		= Point(1.0f, 0.0f,
                   0.0f); fjc.mLocalAxis1		= Point(1.0f, 0.0f, 0.0f); fjc.mLocalPivot0	= Pivot0;
                                                                        fjc.mLocalPivot1	= Pivot1;

                                //					PintJointHandle j0 = pint.CreateJoint(fjc);
                                                                                PINT_HINGE_JOINT_CREATE* Copy =
                   ICE_NEW(PINT_HINGE_JOINT_CREATE)(fjc); Joints.Add(udword(Copy));
                                                                }
                                                        }
                                                }
                */
                ...
                const Point WheelAttachPos =
                        BottomSidePos + Point(0.0f, WheelAttachExtents.y, (BottomSideExtents.z) * SideZ);
                PintActorHandle AttachHandle;
                {
                    ...
                    if (RCA) {
                        ...
                        /*						ObjectDesc.mAddToWorld	= !Aggregate;
                                                                        AttachHandle = CreatePintObject(pint,
                           ObjectDesc); if(Aggregate) pint.AddToAggregate(AttachHandle, Aggregate);*/
                ...
                PintActorHandle TopSideHandle;
                {
                    ...
                    if (RCA) {
                        ...
                        /*						ObjectDesc.mAddToWorld	= !Aggregate;
                                                                        TopSideHandle = CreatePintObject(pint,
                           ObjectDesc); if(Aggregate) pint.AddToAggregate(TopSideHandle, Aggregate);*/
                ...
                if (1) {
                    ...
                    const Point Ext1((-BaseExtents.x + BottomSideExtents.x) * SideX, BaseExtents.y + SuspHeight,
                                     BaseExtents.z * SideZ);
                    //					const Point Ext1((-BaseExtents.x+BottomSideExtents.x)*SideX,
                    //SuspHeight, BaseExtents.z*SideZ);
                    ...
                    CreatePrismatic(pint, &Joints, p0, p1, BottomSideHandle, Ext0, BaseHandle, Ext1, ElemMass,
                                    SuspStiffness, SuspDamping, Aggregate, RCA, &ChassisMaterial, GlobalScale,
                                    NbLinksInRCA
                                    /*,MassInertiaCoeff*/);
                }
                ...
                //				const Point WheelAttachPos2 = WheelAttachPos + Point(0.0f,
                // 0.0f, 2.0f*WheelAttachExtents.z*SideZ);
                ...
                /*				PintActorHandle AttachHandle2;
                   ...
                   Extents);
                                                        ...
                                                        ObjectDesc.mMass			= ElemMass;
                                                        ObjectDesc.mMassForInertia	=
                   ObjectDesc.mMass*MassInertiaCoeff; ObjectDesc.mPosition		= WheelAttachPos2;
                                                        ObjectDesc.mCollisionGroup	= 1;
                ...
                if (IsFront) {
...

                    {
                        ...
                        if (RCA) {
                            ...
                            /*							ObjectDesc.mAddToWorld	= !Aggregate;
                                                                                    AttachHandle2 =
                               CreatePintObject(pint, ObjectDesc); if(Aggregate) pint.AddToAggregate(AttachHandle2,
                               Aggregate);*/
                    ...
                    /*					PINT_FIXED_JOINT_CREATE fjc;
                                                            fjc.mObject0 = AttachHandle;
                                                            fjc.mObject1 = AttachHandle2;
                                                            fjc.mLocalPivot0 = Point(0.0f, 0.0f,
                    WheelAttachExtents.z*SideZ); fjc.mLocalPivot1 = Point(0.0f, 0.0f, -WheelAttachExtents.z*SideZ);

                    //					PintJointHandle j0 = pint.CreateJoint(fjc);
                                                                    PINT_FIXED_JOINT_CREATE* Copy =
                    ICE_NEW(PINT_FIXED_JOINT_CREATE)(fjc); Joints.Add(udword(Copy));*/
                }
                ...
                if (AttachWheels) {
                    ...
                    /*					WheelCreator WC;
                                                            ...
                                                            WC.mObjectDesc.mAddToWorld		= !Aggregate;
                                                            WC.mObjectDesc.mMass			= WheelMass;
                                                            WC.mObjectDesc.mMassForInertia	=
                       WheelMass*MassInertiaCoeff;*/
                    WC.mObjectDesc.mPosition = WheelAttachPos2;

                    /*					if(WT==WHEEL_SPHERE_VISUAL_SPHERE ||
                    WT==WHEEL_SPHERE_VISUAL_CYLINDER)
                                                            {
                                                                    WC.mSphereDesc.mRadius	= _WheelRadius;
                                                                    if(WT==WHEEL_SPHERE_VISUAL_SPHERE)
                                                                            WC.mSphereDesc.mRenderer	=
                    CreateSphereRenderer(_WheelRadius); else WC.mSphereDesc.mRenderer	=
                    CreateConvexRenderer(TotalNbVerts, Verts); WC.mSphereDesc.mMaterial		= &UserMaterial;
                                                                    WC.mObjectDesc.SetShape(&WC.mSphereDesc);
                                                            }
                                                            else if(WT==WHEEL_CYLINDER)
                                                            {
                                                                    WC.mConvexDesc.mNbVerts		= TotalNbVerts;
                                                                    WC.mConvexDesc.mVerts		= Verts;
                                                                    WC.mConvexDesc.mRenderer	=
                    CreateConvexRenderer(TotalNbVerts, Verts); WC.mConvexDesc.mMaterial	= &UserMaterial;
                    //						WC.mConvexDesc.mMaterial	= IsFront ? &UserMaterial
                    : &LowFrictionMaterial; WC.mObjectDesc.SetShape(&WC.mConvexDesc);
                                                            }
                                                            else if(WT==WHEEL_TORUS)
                                                            {
                                                                    const float SmallRadius = WheelWidth*0.5f;
                                                                    const float BigRadius = WheelRadius - SmallRadius;
                                                                    WC.GenerateTireCompound(Q, UserMaterial, BigRadius,
                    SmallRadius, WheelTessR, WheelTessW);
                                                            }*/

                    if (RCA) {
                        /*						PINT_SPHERE_CREATE WheelDesc(_WheelRadius);
                        //						WheelDesc.mRenderer	=
                        CreateSphereRenderer(_WheelRadius); WheelDesc.mRenderer	= CreateConvexRenderer(TotalNbVerts,
                        Verts);
                        */
                        /*						PINT_CONVEX_CREATE WheelDesc(TotalNbVerts,
                           Verts); WheelDesc.mRenderer	= CreateConvexRenderer(WheelDesc.mNbVerts, WheelDesc.mVerts);
                        ...
                        if (AutoSpeed != 0.0f) {
                            if (DT == DRIVE_TYPE_4WD || (DT == DRIVE_TYPE_RWD && !IsFront) ||
                                (DT == DRIVE_TYPE_FWD && IsFront)) {
                        ...
                        WT==WHEEL_SPHERE_VISUAL_CYLINDER)
                                                                        {
                                                                                PINT_SPHERE_CREATE
                        WheelDesc(_WheelRadius); if(WT==WHEEL_SPHERE_VISUAL_SPHERE) WheelDesc.mRenderer	=
                        CreateSphereRenderer(_WheelRadius); else WheelDesc.mRenderer	=
                        CreateConvexRenderer(TotalNbVerts, Verts); WheelDesc.mMaterial		= &UserMaterial;
                                                                                WC.mObjectDesc.SetShape(&WheelDesc);
                                                                                Tire = CreatePintObject(pint,
                        WC.mObjectDesc); if(Aggregate) pint.AddToAggregate(Tire, Aggregate);
                                                                        }
                                                                        else if(WT==WHEEL_CYLINDER)
                                                                        {
                                                                                PINT_CONVEX_CREATE
                        WheelDesc(TotalNbVerts, Verts); WheelDesc.mRenderer		=
                        CreateConvexRenderer(WheelDesc.mNbVerts, WheelDesc.mVerts); WheelDesc.mMaterial		=
                        &UserMaterial; WC.mObjectDesc.SetShape(&WheelDesc); Tire = CreatePintObject(pint,
                        WC.mObjectDesc); if(Aggregate) pint.AddToAggregate(Tire, Aggregate);
                                                                        }
                                                                        else if(WT==WHEEL_TORUS)
                                                                        {
                        //							Tire = GenerateTireCompound(pint,
                        WheelAttachPos2, Q, WheelMass, 1, UserMaterial, 1.5f, MassInertiaCoeff);

                                                                                WC.GenerateTireCompound(Q,
                        UserMaterial, 1.5f); Tire = CreatePintObject(pint, WC.mObjectDesc); if(Aggregate)
                                                                                        pint.AddToAggregate(Tire,
                        Aggregate);
                                                                        }*/
                        if (WT >= WHEEL_SPHERE_VISUAL_SPHERE && WT <= WHEEL_TORUS) {
                            Tire = CreatePintObject(pint, WC.mObjectDesc);
                            if (Aggregate) pint.AddToAggregate(Tire, Aggregate);
                        } else if (WT == WHEEL_PRISMATIC) {
                            const float PrismaticWheelRadius = GlobalScale * 0.5f;
                            //				const Point TirePos = WheelAttachPos + Point(0.0f, 0.0f,
                            // PrismaticWheelRadius*2.0f*SideZ);
                            const Point TirePos =
                                    WheelAttachPos2 + Point(0.0f, 0.0f, PrismaticWheelRadius * 2.0f * SideZ);
...

                        {
                            ...
                            //						fjc.mLocalPivot1 = Point(0.0f, 0.0f,
                            //-WheelRadius*2.0f*SideZ);
                            fjc.mLocalPivot1 = Pivot1;
                    ...
                    if (Tire) UserData->AddWheel(Tire, IsFront);
                    ...
                    if (Tire && TweakWheelsInertia) {
                        ...
                        if (API) {
                            ...
                            //							API->SetLocalInertia(Tire,
                            // LocalInertia*0.1f); 							API->SetLocalInertia(Tire, Point(LocalInertia.x*0.1f,
                            // LocalInertia.y, LocalInertia.z));
                        }
                    }
                }
            }
            ...
            udword j = 1;
            {
                ...
                if (1) {
                    ...
                    PintActorHandle SteeringRackHandle;
                    {
                        ...
                        ObjectDesc.mCollisionGroup = 1;
                        /*						ObjectDesc.mAddToWorld		= !Aggregate;
                                                                        SteeringRackHandle = CreatePintObject(pint,
                           ObjectDesc); if(Aggregate) pint.AddToAggregate(SteeringRackHandle, Aggregate);*/
                        ...
                        // TODO: add this to articulation and use a motor there
                        {
                            /*						PINT_PRISMATIC_JOINT_CREATE pjc;
                                                                            ...
                                                                            pjc.mLocalPivot0 = Point((P0.x + P1.x)*0.5f
                               - BasePos.x, BaseExtents.y, 0.0f); pjc.mLocalPivot1 = Point(0.0f, -RodExtents.y, 0.0f);
                                                                            pjc.mLocalAxis0 = Point(0.0f, 0.0f, 1.0f);
                                                                            pjc.mLocalAxis1 = Point(0.0f, 0.0f, 1.0f);
                                                                            PintJointHandle j0 =
                               pint.CreateJoint(pjc);*/
                    ...
                    PintActorHandle LeftLinkHandle;
                    {
                        ...
                        if (RCA) {
                            ...
                            /*							ObjectDesc.mAddToWorld	= !Aggregate;
                                                                                    LeftLinkHandle =
                               CreatePintObject(pint, ObjectDesc); if(Aggregate) pint.AddToAggregate(LeftLinkHandle,
                               Aggregate);*/
                    ...
                    PintActorHandle RightLinkHandle;
                    {
                        ...
                        if (RCA) {
                            ...
                            /*							ObjectDesc.mAddToWorld	= !Aggregate;
                                                                                    RightLinkHandle =
                               CreatePintObject(pint, ObjectDesc); if(Aggregate) pint.AddToAggregate(RightLinkHandle,
                               Aggregate);*/
                ...
                /*				else
                                                {
                                                        const Point RodExtents(0.05f, 0.05f,
                   0.5f*Pos[0].Distance(Pos[1])); PintActorHandle ConnectingRodHandle;
                                                        {
                                                                const Point P0 = Pos[0] + Pivots[0]*SideX;
                                                                const Point P1 = Pos[1] + Pivots[1]*SideX;
                                                                ...
                                                                ObjectDesc.mPosition		= (P0 + P1)*0.5f;
                                                                ObjectDesc.mCollisionGroup	= 1;
                                                                ConnectingRodHandle = CreatePintObject(pint,
                   ObjectDesc);
                                                        }
                                                        ...
                                                        for(udword i=0;i<2;i++)
                                                        {
                                                                ...
                                                                fjc.mLocalPivot1 = Point(0.0f, 0.0f,
                   RodExtents.z*SideZ); PintJointHandle j0 = pint.CreateJoint(fjc);
                                                        }
                                                }*/
            }
        }
        ...
        if (Aggregate) {
            if (RCA) pint.AddRCArticulationToAggregate(RCA, Aggregate);

            pint.AddAggregateToScene(Aggregate);
        } else if (RCA)
            pint.AddRCArticulationToScene(RCA);

        if (RCA) printf("Nb RCA links: %d\n", NbLinksInRCA);

        if (AttachCar) return true;

        return InitLevel(&pint, &caps, &UserMaterial);
    }

    virtual udword GetFlags() const {
        return (mCheckBox_DriveVehicle && mCheckBox_DriveVehicle->IsChecked()) ? TEST_FLAGS_USE_CURSOR_KEYS
                                                                               : TEST_FLAGS_DEFAULT;
    }
        ...
        if (DriveVehicle) {
            ...
            if (EngineMode == 1) {
                ...
                if (mInput.mKeyboard_Brake) {
                    ...
                    mCurrentVel = 0.0f;
                    if (mCurrentVel < 0.0f) mCurrentVel = 0.0f;
                    //				Handbrake = true;
                } else if (mInput.mKeyboard_Accelerate) {
                    mCurrentVel += Coeff;
                    if (mCurrentVel > MaxVel) mCurrentVel = MaxVel;
                } else {
                    mCurrentVel -= Coeff;
                    if (mCurrentVel < 0.0f) mCurrentVel = 0.0f;
                }
                ...
                for (udword i = 0; i < NbWheels; i++) {
                    if (!Wheels[i].GetHandle()) continue;
                    ...
                    if (!IsFront && Handbrake) {
                        ...
                        //						pint.SetAngularVelocity(UserData->mRCAWheel[j*2+i],
                        // Point(0.0f, 0.0f, 0.0f));
                    } else if (DT == DRIVE_TYPE_4WD || (DT == DRIVE_TYPE_RWD && !IsFront) ||
                               (DT == DRIVE_TYPE_FWD && IsFront)) {
                        pint.SetRCADriveVelocity(Wheels[i].GetHandle(), mCurrentVel);
                        //						pint.SetAngularVelocity(UserData->mRCAWheel[j*2+i],
                        // Point(mCurrentVel, 0.0f, 0.0f));
                    }
                }
            }
            ...
            if (0) {
                for (udword i = 0; i < 4; i++) {
                    if (UserData->mRCAWheel[i].GetHandle()) {
                        ...
                        //					renderer.print(0.0f, y, text_scale, _F("Ang vel: %.3f | %.3f
                        //|
