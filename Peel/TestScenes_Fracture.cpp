...
virtual void OnObjectReleased(Pint& pint, PintActorHandle removed_object) {
    ...
    for (udword i = 0; i < Nb; i++) {
        ...
        if (MC->DeleteConvex(pint, removed_object)) return;
    }
}
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < numSegs; i++) polyPoints.push_back(Point(-r * cosf(i * dphi), 0.0f, -r * sinf(i * dphi)));
    ...
    //				MC->randomFracture(convexNr, ray, 2, 5.0f);
            ...
            if (1) {
                ...
                //					Ray R(Point(0.0f, -10.0f, 0.0f)+Offset, Point(0.0f, 1.0f,
                // 0.0f));
...
static const char* gDesc_VoronoiFracture2 =
        "Runtime fracture. Press SHIFT + left mouse button to fracture the object. You can also use the 'Bullet' tool "
        "to shoot bullets at the scene.";
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
...

        {
            ...
            mEditBox_NbCells = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "100", &UIElems,
...

        {
            ...
            mEditBox_BiasExp = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "8.0", &UIElems,
...

        {
            ...
            mEditBox_SizeX = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "30.0", &UIElems,
                                                  EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        {
            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Size Y:", &UIElems);
            mEditBox_SizeY = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "30.0", &UIElems,
                                                  EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        {
            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Size Z:", &UIElems);
            mEditBox_SizeZ = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "30.0", &UIElems,
                                                  EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        {
            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Min size:", &UIElems);
            mEditBox_MinSize = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1", &UIElems,
                                                    EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        {
            mCheckBox_Kinematic =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Start as kinematic", &UIElems, true, null, null);
            y += YStep;
        }
    ...
    float ComputeRoughSize(udword nb_verts, const Point* verts) const {
        ...
        if (gFractureLog) printf("Rough size: %f\n", Diagonal.Magnitude());
    ...
    void CreateInitialConvex(Pint& pint,
                             udword nb_verts,
                             const Point* verts,
                             udword nb_polygons,
                             const Container& polygon_data,
                             bool kinematic,
                             float h) {
        ...
        mFractureManager.RegisterConvex(pint, c, RoughSize, Offset, Quat(Idt), Point(0.0f, 0.0f, 0.0f),
                                        Point(0.0f, 0.0f, 0.0f), kinematic);
    }
    ...
    virtual bool LeftDownCallback(const MouseInfo& mouse) {
        if (!IsKeyPressed(VK_SHIFT)) return true;
        ...
        for (udword i = 0; i < NbEngines; i++) {
            ...
            if (NbHits) DoFracture(Hit.mTouchedActor, Hit.mImpact, Engine);
        }
    ...
    bool DoFracture(PintActorHandle object, const Point& world_pt, Pint* pint) {
        ...
        if (RoughSize < MinSize) {
            if (gFractureLog) printf("Too small - cannot fracture\n");
            return false;
        } else {
            ...
            if (NbNewConvexes) {
                ...
                for (udword i = 0; i < NbNewConvexes; i++) {
                    ...
                    float RoughSize = 0.0f;
                    {
                        ...
                        //						Kinematic =
                        // ConvexBounds.ContainsPoint(LocalHit)==0; 						Kinematic = KineParent
                        // ? SphereAABB(LocalHit, 0.2f, ConvexBounds)==0 : false;
                        const Sphere S(LocalHit, 1.0f);
                        Kinematic = KineParent ? S.Contains(ConvexBounds) == 0 : false;
                    }
                    if (RoughSize < MinSize) {
                        if (gFractureLog) printf("Too small - discarded\n");
                        cn->Release();
                        continue;
                    }
                    ...
                    mFractureManager.RegisterConvex(*pint, cn, RoughSize, posei.GetTrans(), Rot, LinVel * Rot,
                                                    AngVel * Rot, Kinematic);
                }
            }
        }
        return true;
...
/*	virtual	udword					Update(Pint& pint, float dt)
    ...
    static const char* gDesc_VoronoiFracture3 =
            "Fracturable box stack. Press SHIFT + left mouse button to fracture the object. You can also use the "
            "'Bullet' tool to shoot bullets at the scene.";
    ...
    public:
                ...
                for (udword j = 0; j < NbStacks; j++) {
                    ...
                    while (NbBoxes) {
                        for (udword i = 0; i < NbBoxes; i++) {
                            ...
                            // CreateInitialConvex(pint, CH->GetNbVerts(), CH->GetVerts(), NbPolygons, tmp, false,
                            // 0.0f);
                            ...
                            mFractureManager.RegisterConvex(pint, c, RoughSize, Pos, Quat(Idt), Point(0.0f, 0.0f, 0.0f),
                                                            Point(0.0f, 0.0f, 0.0f), false);
                        }
        ...
        static const char* gDesc_VoronoiFracture4 =
                "Fracturable tiles. Press SHIFT + left mouse button to fracture the object. You can also use the "
                "'Bullet' tool to shoot bullets at the scene.";
            ...
            virtual bool Setup(Pint& pint, const PintCaps& caps) {
                ...
                for (udword j = 0; j < Nb; j++) {
                    ...
                    for (udword i = 0; i < Nb; i++) {
                        ...
                        // mFractureManager.RegisterConvex(pint, c, RoughSize, Pos, Quat(Idt), Point(0.0f, 0.0f, 0.0f),
                        // Point(0.0f, 0.0f, 0.0f), true);
                        mFractureManager.RegisterConvex(pint, c, 0.0f, Pos, Quat(Idt), Point(0.0f, 0.0f, 0.0f),
                                                        Point(0.0f, 0.0f, 0.0f), false);
                    }
                }
                DELETESINGLE(CH);
...
#include "Voronoi2D.h"
            ...
            static const char* gDesc_VoronoiFracture5 =
                    "Fracturable walls. Press SHIFT + left mouse button to fracture the object. You can also use the "
                    "'Bullet' tool to shoot bullets at the scene.";
                ...
                virtual void OnGeneratedCell(const Point2D& site, udword nb_verts, const Point2D* verts) {
                    ...
                    if (1) {
...

                        {
                            ...
                            mFractureManager.RegisterConvex(*mPint, c, RoughSize, Pos, Quat(Idt),
                                                            Point(0.0f, 0.0f, 0.0f), Point(0.0f, 0.0f, 0.0f), Kine);
                        }

                        {
                            ...
                            FractureConvex* c = ICE_NEW(FractureConvex)(nb_verts, Pts, h);
                            mFractureManager.RegisterConvex(*mPint, c, RoughSize, Pos * M, M, Point(0.0f, 0.0f, 0.0f),
                                                            Point(0.0f, 0.0f, 0.0f), Kine);
                        }
                    ...
                    for (udword i = 0; i < nb_verts * 2; i++) Pts[i] -= Center;
                    ...
                    mFractureManager.RegisterConvex(*mPint, c, RoughSize, Center, Quat(Idt), Point(0.0f, 0.0f, 0.0f),
                                                    Point(0.0f, 0.0f, 0.0f), Kine);
                    DELETESINGLE(CH);
