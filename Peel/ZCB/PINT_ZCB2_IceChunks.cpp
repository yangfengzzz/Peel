...
bool ZCB2FactoryEx::NewMaterial(const MATLChunk* material) {
    ...
    /*	bool Found = false;
            const udword NbTextures =
       mImportHelper_TextureData.GetNbEntries()/(sizeof(ZCB2FactoryEx::TextureData)/sizeof(udword)); const
       ZCB2FactoryEx::TextureData* Textures = (const ZCB2FactoryEx::TextureData*)mImportHelper_TextureData.GetEntries();
...
static bool CheckMeshHasUniqueMaterial(const MESHChunk* mesh, udword& unique_material_id) {
    ...
    if (NbDIDs && NbWIDs) OutputConsoleError("Unexpected: MESH chunk has both word & dword material IDs!\n");
    if (NbDIDs) {
        const udword* DIDs = mesh->GetMaterialIDs().GetDIDs();
        const udword UniqueID = DIDs[0];
        for (udword i = 1; i < NbDIDs; i++) {
            if (DIDs[i] != UniqueID) return false;
        }
        ...
        for (udword i = 1; i < NbWIDs; i++) {
            if (WIDs[i] != UniqueID) return false;
        }
        unique_material_id = UniqueID;
    }
    return true;
...
bool ZCB2FactoryEx::NewMesh(const MESHChunk* mesh) {
    ...
    MeshDataChunk* Chunk;
    {
        ...
        mdc.SetSurfaceData(mesh->GetVertices().GetNbPts(), mesh->GetVertices().GetPts(), mesh->GetFaces().GetNbFaces(),
                           mesh->GetFaces().GetDFaces(), mesh->GetFaces().GetWFaces());
    ...
    PINT_MESH_CREATE* Shape;
    {
        ...
        Shape->SetSurfaceData(Chunk->mVertexData.GetNbPts(), Chunk->mVertexData.GetPts(),
        ...
        if (CheckMeshHasUniqueMaterial(mesh, UniqueMaterialID)) {
            const udword NbMaterials =
                    mImportHelper_MaterialData.GetNbEntries() / (sizeof(ZCB2FactoryEx::MaterialData) / sizeof(udword));
            const ZCB2FactoryEx::MaterialData* Materials =
                    (const ZCB2FactoryEx::MaterialData*)mImportHelper_MaterialData.GetEntries();
        ...
        if (0) {
            ...
            if (Found) {
                ...
                if (mesh->GetUVs().GetNbPts() && mesh->GetTFaces().GetNbFaces()) {
                    MultiSurface MS;
                    MS.IndexedSurface::Init(mesh->GetFaces().GetNbFaces(), mesh->GetVertices().GetNbPts(),
                                            mesh->GetVertices().GetPts(),
                                            (const IndexedTriangle*)mesh->GetFaces().GetDFaces());
                    {
                        ...
                        CopyMemory(TFaces, mesh->GetTFaces().GetDFaces(),
                                   mesh->GetTFaces().GetNbFaces() * sizeof(IndexedTriangle));
...
bool ZCB2FactoryEx::NewLegacyChunk(const BaseChunk* chunk) {
    ...
    switch (chunk->GetChunkType()) {
        case 'SCEN': {
            return NewScene((const SCENChunk*)chunk);
        } break;

        case 'FCAM': {
            return NewFreeCamera((const FCAMChunk*)chunk);
        } break;
        case 'TCAM': {
            return NewTargetCamera((const TCAMChunk*)chunk);
        } break;

        case 'MESH': {
            return NewMesh((const MESHChunk*)chunk);
        } break;

        case 'PLIT': {
            return NewPLight((const PLITChunk*)chunk);
        } break;
        case 'SLIT': {
            return NewSLight((const SLITChunk*)chunk);
        } break;
        case 'DLIT': {
            return NewDLight((const DLITChunk*)chunk);
        } break;

#ifndef _WIN64  // "temporary"
        case 'MATL': {
            return NewMaterial((const MATLChunk*)chunk);
        } break;
#endif
        case 'TXMP': {
            return NewTextureMap((const TXMPChunk*)chunk);
        } break;

            //		case 'GIZM':	{	return NewGizmo			((const GIZMChunk*)chunk);	}break;
        case 'BGIZ': {
            return NewBoxGizmo((const BGIZChunk*)chunk);
        } break;
        case 'SGIZ': {
            return NewSphereGizmo((const SGIZChunk*)chunk);
        } break;
        case 'CGIZ': {
            return NewCylinderGizmo((const CGIZChunk*)chunk);
        } break;
        case 'HELP': {
            return NewHelper((const HELPChunk*)chunk);
        } break;
        case 'SKEL': {
            return NewSkeleton((const SKELChunk*)chunk);
        } break;

        case 'SHAP': {
            return NewShape((const SHAPChunk*)chunk);
        } break;

        case 'SAMP': {
            return NewSampler((const SAMPChunk*)chunk);
        } break;
        case 'ROTA': {
            return NewRotator((const ROTAChunk*)chunk);
        } break;

#ifndef _WIN64  // "temporary"
        case 'MOVE': {
            return NewMotion((const MOVEChunk*)chunk);
        } break;
#endif
    }
    return true;
