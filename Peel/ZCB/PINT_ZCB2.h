...
class MaterialChunk : public BaseChunk {
    ...
    inline_ bool IsTheSameAs(const PINT_MATERIAL_CREATE& create) const {
        if (mStaticFriction != create.mStaticFriction) return false;
        if (mDynamicFriction != create.mDynamicFriction) return false;
        if (mRestitution != create.mRestitution) return false;
        return true;
    }
...
public:
    ...
    struct ActorCreate {
        ...
        inline_ void Release() { DELETESINGLE(mCreate); }
    ...
    inline_ const AggregateChunk** GetAggregates() const {
        return (const AggregateChunk**)mAggregateChunks.GetEntries();
    }
