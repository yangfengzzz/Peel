...
#define ZCB_CHECKALLOC(x)                                              \
    {                                                                  \
        if (!x) {                                                      \
            ZCBImportError("Out of memory.", ZCB_ERROR_OUT_OF_MEMORY); \
            return false;                                              \
        }                                                              \
    }
#define ZCB_CHECKVERSION(file_version, current_version, chunk_name)                                 \
    {                                                                                               \
        if (file_version > current_version) {                                                       \
            ZCBImportError("Obsolete code! Get the latest ZCB reader.", ZCB_ERROR_UNKNOWN_VERSION); \
            return false;                                                                           \
        } else if (file_version < current_version)                                                  \
            ZCBImportError(_F("Found obsolete %s chunk. Please resave the file.", chunk_name),      \
                           ZCB_ERROR_OBSOLETE_FILE);                                                \
    }
...
static void InitProgressBar() {
    ...
    if (it) gProgressBar = it->CreateProgressBar();
}

static void ReleaseProgressBar() {
    GUIRequestInterface* it = GetGUIRequestInterface();
    if (it) it->DeleteProgressBar(gProgressBar);
    gProgressBar = null;
}
#endif
...
static void UpdateProgressBar0() {
    ...
    //	GetCallbacksManager()->ExecCallbacks(ICCB_REQUEST, ICCB_REQUEST, gParams);
    if (gProgressBar) gProgressBar->Update0();
#endif
}
...
static void UpdateProgressBar1() {
    ...
    //	GetCallbacksManager()->ExecCallbacks(ICCB_REQUEST, ICCB_REQUEST, gParams);
    if (gProgressBar) gProgressBar->Update1();
#endif
}
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
ZCBTextureInfo::~ZCBTextureInfo() { ICE_FREE(mBitmap); }
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
ZCBControllerInfo::~ZCBControllerInfo() { ICE_FREE(mSamples); }
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
ZCBBreaker::~ZCBBreaker() { ReleaseRam(); }
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void ZCBBreaker::ReleaseRam() { DELETESINGLE(mImportArray); }
...
bool ZCBBreaker::Import() {
    ...
    if (b4 == 'P') {
        ...
        if (Compression == ZCB_COMPRESSION_ZLIB) {
            ...
            int ErrorCode = uncompress((Bytef*)Depacked, (uLongf*)&OriginalSize, (Bytef*)mImportArray->GetAddress(),
                                       PackedSize);
            if (ErrorCode != Z_OK) {
            ...
            int ErrorCode = BZ2_bzBuffToBuffDecompress((char*)Depacked, &OriginalSize,
                                                       (char*)mImportArray->GetAddress(), PackedSize, 0, 0);
            if (ErrorCode != BZ_OK) {
                ZCBImportError("Depacking failed.", ZCB_ERROR_CANT_DEPACK);
                return false;
            }
    ...
    if (mFileType == ZCB_FILE_SCENE) {
        ...
        if (!ImportCameras(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportLights(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportTextures(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportMaterials(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportMeshes(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportShapes(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportHelpers(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
        if (!ImportControllers(*mImportArray)) return Cleanup();
        UpdateProgressBar0();
...
bool ZCBBreaker::ImportMotion(ImportArray& importer) {
    ...
    if (Status) {
        ...
        // Since version 2
        if (mMOVEVersion >= 2)
            Mot.mLocalPRS = importer.ReadBool();  // Local/global PRS
        else
            Mot.mLocalPRS = true;  // Arbitrary
        // Since version 4
        if (mMOVEVersion >= 4)
            Mot.mD3DCompliant = importer.ReadBool();  // D3D compatible
        else
            Mot.mD3DCompliant = false;
        // Since version 3
        if (mMOVEVersion >= 3)
            Mot.mName.Set((const char*)importer.ReadString());  // The motion's name
        else
            Mot.mName.Set((const char*)"MotionName");
        if (mMOVEVersion >= 3)
            Mot.mType.Set((const char*)importer.ReadString());  // The motion's type
        else
            Mot.mType.Set((const char*)"MotionType");
    ...
    if (mNbLights) {
        ...
        //		if(importer.GetChunk(mZCBGlobalVersion ? "LITECHUNK" : "LITE"))
        {
            ...
            for (udword n = 0; n < mNbLights; n++) {
                ...
                if (mLITEVersion >= 2)
                    CurLight.mCastShadows = importer.ReadBool();  // Cast shadows
                else
                    CurLight.mCastShadows = false;
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool ZCBBreaker::ImportCroppingValues(TextureCrop& cvalues,
                                      TextureMatrix& tmtx,
                                      ImportArray& importer,
                                      bool& wrap_u,
                                      bool& mirror_u,
                                      bool& wrap_v,
                                      bool& mirror_v) {
    ...
    if (mNbMaterials) {
        ...
        //		if(importer.GetChunk(mZCBGlobalVersion ? "MATLCHUNK" : "MATL"))
        {
            ...
            for (udword n = 0; n < mNbMaterials; n++) {
                ...
                // Texture IDs
                CurMaterial.mAmbientMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mAmbientMapID);
                CurMaterial.mDiffuseMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mDiffuseMapID);
                CurMaterial.mSpecularMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mSpecularMapID);
                CurMaterial.mShininessMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mShininessMapID);
                CurMaterial.mShiningStrengthMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mShiningStrengthMapID);
                CurMaterial.mSelfIllumMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mSelfIllumMapID);
                CurMaterial.mOpacityMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mOpacityMapID);
                CurMaterial.mFilterMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mFilterMapID);
                CurMaterial.mBumpMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mBumpMapID);
                CurMaterial.mReflexionMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mReflexionMapID);
                CurMaterial.mRefractionMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mRefractionMapID);
                CurMaterial.mDisplacementMapID = importer.ReadDword();
                PatchTextureID(CurMaterial.mDisplacementMapID);
                ...
                if (mMATLVersion >= 2)
                    ImportCroppingValues(CurMaterial.mCValues_Diffuse, CurMaterial.mTMtx_Diffuse, importer,
                                         CurMaterial.mWrapU_Diffuse, CurMaterial.mMirrorU_Diffuse,
                                         CurMaterial.mWrapV_Diffuse, CurMaterial.mMirrorV_Diffuse);
                if (mMATLVersion >= 8)
                    ImportCroppingValues(CurMaterial.mCValues_Filter, CurMaterial.mTMtx_Filter, importer,
                                         CurMaterial.mWrapU_Filter, CurMaterial.mMirrorU_Filter,
                                         CurMaterial.mWrapV_Filter, CurMaterial.mMirrorV_Filter);
    ...
    if (mNbTextures) {
        ...
        //		if(importer.GetChunk(mZCBGlobalVersion ? "TEXMCHUNK" : "TEXM"))
        {
            ...
            for (udword n = 0; n < mNbTextures; n++) {
                ...
                // Cropping values & texture matrix
                ImportCroppingValues(CurTexture.mCValues, CurTexture.mTMtx, importer, CurTexture.mWrapU,
                                     CurTexture.mMirrorU, CurTexture.mWrapV, CurTexture.mMirrorV);
        ...
        //		if(importer.GetChunk(mZCBGlobalVersion ? "MESHCHUNK" : "MESH"))
        {
            ...
            for (udword n = 0; n < Total; n++) {
                ...
                if (!CurMesh.mIsInstance) {
                    ...
                    if (!CurMesh.mIsSkin)
                        ImportVertices(CurMesh, importer);
                    else
                        ImportSkinData(CurMesh, importer);
...
bool ZCBBreaker::ImportSkinData(ZCBMeshInfo& curmesh, ImportArray& importer) {
    ...
    if (curmesh.mFlags & ZCB_ONE_BONE_PER_VERTEX) {
        ...
        for (udword i = 0; i < curmesh.mNbVerts; i++) {
            ...
            if (curmesh.mD3DCompliant && mMESHVersion < 6)
                TSwap(curmesh.mOffsetVectors[i].y, curmesh.mOffsetVectors[i].z);
        }
    ...
    if (faces) {
        if (1) {
            ...
            for (udword j = 0; j < nb_faces * 3; j++)
                //				faces[j] = word_faces ? (sword)importer.ReadWord() :
                //importer.ReadDword();
...
bool ZCBBreaker::ImportFaces(ZCBMeshInfo& curmesh, ImportArray& importer) {
    ...
    if (NbFaces) {
        if (curmesh.mNbVerts) {
            curmesh.mFaces = (udword*)ICE_ALLOC(sizeof(udword) * NbFaces * 3);
            ZCB_CHECKALLOC(curmesh.mFaces);
        }
        if (curmesh.mNbTVerts) {
            curmesh.mTFaces = (udword*)ICE_ALLOC(sizeof(udword) * NbFaces * 3);
            ZCB_CHECKALLOC(curmesh.mTFaces);
        }
        if (curmesh.mNbCVerts) {
            curmesh.mCFaces = (udword*)ICE_ALLOC(sizeof(udword) * NbFaces * 3);
            ZCB_CHECKALLOC(curmesh.mCFaces);
        }
    }
    ...
    for (udword j = 0; j < NbFaces; j++) {
        curmesh.mFaceProperties[j].MatID = importer.ReadDword();
        PatchMaterialID(curmesh.mFaceProperties[j].MatID);
    }
    for (udword j = 0; j < NbFaces; j++) curmesh.mFaceProperties[j].Smg = importer.ReadDword();
    for (udword j = 0; j < NbFaces; j++)
        curmesh.mFaceProperties[j].EdgeVis = (curmesh.mFlags & ZCB_EDGE_VIS) ? importer.ReadByte() : 0;
...
bool ZCBBreaker::ImportConsolidated(ZCBMeshInfo& curmesh, ImportArray& importer) {
    ...
    if (curmesh.mFlags & ZCB_CONSOLIDATION) {
        ...
        // Get topology back
        {
            ...
            for (udword i = 0; i < Topo.NbSubmeshes; i++) {
                if (mMESHVersion < 7)
                    Topo.FacesInSubmesh[i] = importer.ReadWord();
                else
                    Topo.FacesInSubmesh[i] = importer.ReadDword();
        ...
        //		if(importer.GetChunk(mZCBGlobalVersion ? "CTRLCHUNK" : "CTRL"))
        {
            ...
            for (udword n = 0; n < mNbControllers; n++) {
                ...
                if (CurController.mCtrlMode == ZCB_CTRL_SAMPLES) {
                    if (CurController.mCtrlType == ZCB_CTRL_VERTEXCLOUD)
                        CurController.mNbVertices = importer.ReadDword();
                    ...
                    switch (CurController.mCtrlType) {
                        ...
                        case ZCB_CTRL_VERTEXCLOUD: {
                            float* Samples = (float*)ICE_ALLOC(sizeof(float) * 3 * CurController.mNbSamples *
                                                               CurController.mNbVertices);
                            ZCB_CHECKALLOC(Samples);
                    ...
                    return false;  // only sampling for now.....
    ...
    if (mNbShapes) {
        ...
        //		if(importer.GetChunk(mZCBGlobalVersion ? "SHAPCHUNK" : "SHAP"))
        {
            ...
            for (udword n = 0; n < mNbShapes; n++) {
                ...
                // Get type
                if (mSHAPVersion >= 3)
                    CurShape.mType = (ZCBShapeType)importer.ReadDword();
                else
                    CurShape.mType = ZCB_SHAP_UNDEFINED;
                ...
                if (CurShape.mNbLines) {
                    CurShape.mNbVerts = (udword*)ICE_ALLOC(sizeof(udword) * CurShape.mNbLines);
                    ZCB_CHECKALLOC(CurShape.mNbVerts);
                    CurShape.mClosed = (bool*)ICE_ALLOC(sizeof(bool) * CurShape.mNbLines);
                    ZCB_CHECKALLOC(CurShape.mClosed);
...
bool ZCBConverter::Convert(const char* filename) {
    ...
    // The three first bytes must be "ZCB"
    if (b1 != 'Z' || b2 != 'C' || b3 != 'B') return false;
    ...
    const udword Data = mImportArray->ReadDword();
    // Since 1.14 we need to mask out reserved bits & get back the global version
    udword mZCBGlobalVersion = Data >> 16;
    ZCBFile mFileType = ZCBFile(Data & ZCB_FILE_MASK);
    if (mFileType != ZCB_FILE_SCENE) return false;

    gZCBVersion = mZCBGlobalVersion;

    // Find MAIN chunk
    if (!mImportArray->GetChunk(mZCBGlobalVersion ? "MAINCHUNK" : "MAIN")) return false;

    // Get chunk version
    udword mMAINVersion = mImportArray->ReadDword();
    //	ZCB_CHECKVERSION(mMAINVersion, CHUNK_MAIN_VER, "MAIN");
    ...
    if (mMAINVersion < 7) return false;

    udword mMeshOffset = mImportArray->ReadDword();
    udword mCameraOffset = mImportArray->ReadDword();
    udword mLightOffset = mImportArray->ReadDword();
    udword mShapeOffset = mImportArray->ReadDword();
    udword mHelperOffset = mImportArray->ReadDword();
    udword mTexmapOffset = mImportArray->ReadDword();
    udword mMaterialOffset = mImportArray->ReadDword();
    udword mControllerOffset = mImportArray->ReadDword();
    udword mMotionOffset = mImportArray->ReadDword();

    // Get number of expected elements
    udword mNbMeshes = SceneInfo.mNbMeshes = mImportArray->ReadDword();
    udword mNbDerived = SceneInfo.mNbDerived = mImportArray->ReadDword();
    udword mNbCameras = SceneInfo.mNbCameras = mImportArray->ReadDword();
    udword mNbLights = SceneInfo.mNbLights = mImportArray->ReadDword();
    udword mNbShapes = SceneInfo.mNbShapes = mImportArray->ReadDword();
    udword mNbHelpers = SceneInfo.mNbHelpers = mImportArray->ReadDword();
    udword mNbControllers = SceneInfo.mNbControllers = mImportArray->ReadDword();
    udword mNbMaterials = SceneInfo.mNbMaterials = mImportArray->ReadDword();
    udword mNbTextures = SceneInfo.mNbTextures = mImportArray->ReadDword();
    udword mNbUnknowns = SceneInfo.mNbUnknowns = mImportArray->ReadDword();
    udword mNbInvalidNodes = SceneInfo.mNbInvalidNodes = mImportArray->ReadDword();
