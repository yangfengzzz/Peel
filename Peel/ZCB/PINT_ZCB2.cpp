...
ZCB2Factory::ZCB2Factory() : mCustomize(null), mNbDisabledGroups(0), mDisabledGroups(null) { ::Reset(mScene); }

ZCB2Factory::~ZCB2Factory() { Reset(); }
...
void ZCB2Factory::Reset() {
    ...
    // DeleteOwnedObjects<ZCB2Factory::ActorCreate>(mActorsCreate);
    {
        ...
        for (udword i = 0; i < Nb; i++) Actors[i].Release();
        mActorsCreate.Empty();
    }
...
static void SetupJoint(PINT_JOINT_CREATE* create,
                       const JointChunk& chunk,
                       PtrContainer& joints,
                       /*Container& joints_actor_indices, */ const udword nb_actors,
                       const char* name) {
    create->mName = name;
...
void ZCB2Factory::NewUnknownChunk(udword type, const char* name, const VirtualFile& file) {
    ...
    // since you can deserialize the objects on-the-fly in this function. But it's not enough in PEEL where the same
    // object can be deserialized N times in different engines. We could fix it by making the string table have the same
    // lifetime as the ZCB2Importer object itself but that requires a change in ICE. So for now, we do this:
    if (name) {
    ...
    switch (type) {
        ...
        case MaterialType: {
            ...
            if (mCustomize) mCustomize->CustomizeMaterial(*Create);
        } break;
        ...
        case MeshType: {
            ...
            Create->SetSurfaceData(Chunk->mVertexData.GetNbPts(), Chunk->mVertexData.GetPts(),
                                   Chunk->mTriangleData.GetNbFaces(), Chunk->mTriangleData.GetDFaces(),
                                   Chunk->mTriangleData.GetWFaces());

            ASSERT(mMeshObjects.GetNbEntries() == Chunk->GetID());
            mMeshObjects.AddPtr(Create);
            if (mCustomize) mCustomize->CustomizeMesh(*Create);
        } break;
        ...
        case SphereShapeType: {
            ...
            if (mCustomize) mCustomize->CustomizeShape(*Create);
        } break;
        ...
        case ActorType: {
            ...
            if (mCustomize) mCustomize->CustomizeActor(*Create);
        } break;
        ...
        case FixedJointType: {
            ...
            if (mCustomize) mCustomize->CustomizeJoint(*Create);
        } break;
        ...
        case PrismaticJointType: {
            ...
            //				SetupJoint(Create, Chunk, mJoints, /*mJointsActorIndices, */GetNbActors(),
            //name);
            //			}
            //			counter++;
            if (mCustomize) mCustomize->CustomizeJoint(*Create);
        ...
        case AggregateType: {
            ...
            /*			AggregateChunk Chunk;
                                    ...
                                    PINT_AGGREGATE_CREATE* Create = ICE_NEW(PINT_AGGREGATE_CREATE)(Chunk.mMaxSize,
               Chunk.GetCore().IsSet(AGGREGATE_CHUNK_FLAG_SELF_COLLISIONS));
                                    ...
                                    for(udword i=0;i<NbActors;i++)
                                    {
                                            ...
                                            PINT_OBJECT_CREATE* CurrentActor =
               (PINT_OBJECT_CREATE*)mActors.GetEntry(ActorID);
                                    }
...
static void Export(const Point& p, CustomArray& array) { array.Store(p.x).Store(p.y).Store(p.z); }

static void Export(const AABB& bounds, CustomArray& array) {
    array.Store(bounds.mMin.x)
            .Store(bounds.mMin.y)
            .Store(bounds.mMin.z)
            .Store(bounds.mMax.x)
            .Store(bounds.mMax.y)
...
static void Export(const Quat& q, CustomArray& array) { array.Store(q.p.x).Store(q.p.y).Store(q.p.z).Store(q.w); }
...
static void Export(const Container& c, CustomArray& array) {
    ...
    if (Nb) array.Store(c.GetEntries(), Nb * sizeof(udword));
}

static void Export(const PintLimits& limit, CustomArray& array) { array.Store(limit.mMinValue).Store(limit.mMaxValue); }
...
CollisionGroupsChunk::CollisionGroupsChunk(const CollisionGroupsData& data) { Init(data.mNbGroups, data.mGroups); }
...
bool MaterialChunk::Export(CustomArray& array) {
    ...
    array.Store(mStaticFriction).Store(mDynamicFriction).Store(mRestitution);
    return true;
}
...
void ShapeChunk::InitBase(const char* name,
    ...
    if (renderer) {
        ...
        if (renderer->mRenderSource) mRendererID = renderer->mRenderSource->GetID();
    }
...
ShapeChunk::ShapeChunk(const ShapeData& data) {
    ...
    if (data.mMaterial) mMaterialID = data.mMaterial->mID;
}
...
bool ShapeChunk::Export(CustomArray& array) {
    ...
    if (GetCore().IsSet(SHAPE_CHUNK_FLAG_COLOR)) array.Store(mColor.R).Store(mColor.G).Store(mColor.B).Store(mColor.A);
...
SphereShapeChunk::SphereShapeChunk(const PINT_SPHERE_CREATE& create) : ShapeChunk(create) { Init(create.mRadius); }

SphereShapeChunk::SphereShapeChunk(const SphereShapeData& data) : ShapeChunk(data) { Init(data.mRadius); }
...
bool SphereShapeChunk::Import(const VirtualFile& file) {
    ...
    if (gUseRenderScale && gZCB2_RenderScale != 1.0f) mRadius *= gZCB2_RenderScale;
    return true;
}
...
BoxShapeChunk::BoxShapeChunk(const PINT_BOX_CREATE& create) : ShapeChunk(create) { Init(create.mExtents); }

BoxShapeChunk::BoxShapeChunk(const BoxShapeData& data) : ShapeChunk(data) { Init(data.mExtents); }
...
ConvexShapeChunk::ConvexShapeChunk(const ConvexShapeData& data) : ShapeChunk(data) { Init(data.mNbVerts, data.mVerts); }
...
bool ConvexShapeChunk::Import(const VirtualFile& file) {
    ...
    if (gUseRenderScale && gZCB2_RenderScale != 1.0f) {
        ...
        for (udword i = 0; i < NbPts; i++) Pts[i] *= gZCB2_RenderScale;
    }
    return true;
}
...
MeshShapeChunk2::MeshShapeChunk2(const PINT_MESH_CREATE2& create) : ShapeChunk(create) { Init(); }

MeshShapeChunk2::MeshShapeChunk2(const MeshShapeData2& data) : ShapeChunk(data) {
    Init();

    if (data.mMeshData) mMeshID = data.mMeshData->mID;
}
...
ConvexDataChunk::ConvexDataChunk(const ConvexMeshData& data) {
    ...
    if (data.mName.Get()) SetName(data.mName);
...
JointChunk::JointChunk(const PINT_JOINT_CREATE& create) {
    ...
    if (create.mName) SetName(create.mName);
}
...
bool SphericalJointChunk::Import(const VirtualFile& file) {
    ...
    if (mSphericalJointCore.mVersion == 1) {
        ...
        if (mSphericalJointCore.mVersion >= 3) ::Import(mLimits, file);
    }
...
bool DistanceJointChunk::Import(const VirtualFile& file) {
    ...
    if (gUseRenderScale && gZCB2_RenderScale != 1.0f) {
        mLocalPivot0 *= gZCB2_RenderScale;
        mLocalPivot1 *= gZCB2_RenderScale;
        if (IsMinDistanceLimitEnabled(mLimits)) mLimits.mMinValue *= gZCB2_RenderScale;
        if (IsMaxDistanceLimitEnabled(mLimits)) mLimits.mMaxValue *= gZCB2_RenderScale;
    }
    return true;
}
...
bool HingeJointChunk::Import(const VirtualFile& file) {
    ...
    if (gUseRenderScale && gZCB2_RenderScale != 1.0f) {
        ...
        if (!mGlobalAnchor.IsNotUsed()) mGlobalAnchor *= gZCB2_RenderScale;
    }
...
bool GearJointChunk::Import(const VirtualFile& file) {
    ...
    if (mGearJointCore.mVersion == 2) file.ReadFloat();
...
bool RackJointChunk::Import(const VirtualFile& file) {
    ...
    if (mRackJointCore.mVersion == 2) file.ReadFloat();
...
bool ActorChunk::Export(CustomArray& array) {
    ...
    array.Store(mMass).Store(mMassForInertia).Store(mCollisionGroup);
...
bool SceneChunk::Export(CustomArray& array) {
    ...
    array.Store(mNbSimulateCallsPerFrame)
            .Store(mTimestep)
