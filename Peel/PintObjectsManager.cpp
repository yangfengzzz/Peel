...
void ReleasePintObject(Pint& pint, PintActorHandle removed_object, bool release_from_selection) {
    ...
    if (gRunningTest) gRunningTest->OnObjectReleased(pint, removed_object);

    ToolInterface* CurrentTool = GetCurrentTool();
    if (CurrentTool) CurrentTool->OnObjectReleased(pint, removed_object);

    if (release_from_selection) GetDefaultControlInterface().mSelMan.RemoveFromSelection(&pint, removed_object);
    ...
    if (pint.mVisHelper) pint.mVisHelper->SetRenderable(removed_object, true);
...
bool ReleasePintJoints(Pint& pint, PintActorHandle actor) {
    ...
    if (NbJoints) {
        // We don't know if each plugin will support deleting contained items while iterating the container, so we copy
        // the handles first.
        PintJointHandle* Handles = reinterpret_cast<PintJointHandle*>(StackAlloc(sizeof(PintJointHandle) * NbJoints));
        for (udword i = 0; i < NbJoints; i++) Handles[i] = ActorAPI->GetJoint(actor, i);

        // And then we delete in second pass
        for (udword i = 0; i < NbJoints; i++) pint.ReleaseJoint(Handles[i]);
    }
    return true;
}

bool IsDefaultEnv(Pint& pint, PintActorHandle h) { return (h && h == pint.mDefaultEnvHandle); }
