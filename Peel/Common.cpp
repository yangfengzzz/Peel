...
Quat ShortestRotation(const Point& v0, const Point& v1) {
    ...
    Quat q = d > -1.0f ? Quat(1.0f + d, cross.x, cross.y, cross.z)
             //					: fabsf(v0.x)<0.1f ? Quat(0.0f, 0.0f, v0.z, -v0.y) : Quat(0.0f, v0.y, -v0.x,
             //0.0f);
             : fabsf(v0.x) < 0.1f ? Quat(0.0f, 0.0f, v0.z, -v0.y)
                                  : Quat(0.0f, v0.y, -v0.x, 0.0f);
    //	PxQuat q = d > -1 ? PxQuat(cross.x, cross.y, cross.z, 1 + d) : PxAbs(v0.x) < 0.1f ? PxQuat(0.0f, v0.z, -v0.y,
    //0.0f) 	                                                                                  : PxQuat(v0.y, -v0.x, 0.0f, 0.0f);
