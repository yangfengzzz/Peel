...
struct Indices {
    ...
    inline_ bool operator!=(const Indices& v) const {
        return mRef[0] != v.mRef[0] || mRef[1] != v.mRef[1] || mRef[2] != v.mRef[2];
    }
};
...
MeshCleaner::MeshCleaner(
        udword nbVerts, const Point* srcVerts, udword nbTris, const udword* srcIndices, float meshWeldTolerance) {
    ...
    if (meshWeldTolerance != 0.0f) {
        ...
        for (udword i = 0; i < nbVerts; i++) {
            vertexIndices[i] = i;
            cleanVerts[i] =
                    Point(floorf(srcVerts[i].x * weldTolerance + 0.5f), floorf(srcVerts[i].y * weldTolerance + 0.5f),
    ...
    for (udword i = 0; i < nbTris; i++) {
        ...
        if (vref0 >= nbVerts || vref1 >= nbVerts || vref2 >= nbVerts) continue;
        ...
        if (vref0 == vref1 || vref1 == vref2 || vref2 == vref0) continue;
    ...
    for (udword i = 0; i < nbToGo; i++) {
        ...
        while (offset != 0xffffffff && I[offset] != v) offset = next[offset];

        if (offset == 0xffffffff) {
            remapTriangles[nbCleanedTris] = i;
            if (i != nbCleanedTris) idtRemap = false;
            I[nbCleanedTris] = v;
    ...
    if (vertexIndices) {
        for (udword i = 0; i < nbCleanedVerts; i++) cleanVerts[i] = srcVerts[vertexIndices[i]];
