    ...
    udword* Keys = (udword*)ICE_ALLOC_TMP(sizeof(void*) * gNbTests);
    {
        ...
        for (udword i = 0; i < gNbTests; i++) {
            ...
            if (CT != CATEGORY_INTERNAL)  // Always hide internal tests
            {
                ...
                if (gShowDisabledTests || CT != CATEGORY_DISABLED) gNbAvailableTests++;
            }
        }
        ...
        for (udword i = 0; i < gNbTests; i++) gTests[i] = tmp[Ranks[i]];

        ICE_FREE(tmp);
#else
        for (udword i = 0; i < gNbTests; i++) Keys[i] = reinterpret_cast<udword>(gTests[i]);

        for (udword i = 0; i < gNbTests; i++) gTests[i] = reinterpret_cast<PhysicsTest*>(Keys[Ranks[i]]);
#endif
    }
    ICE_FREE(Keys);
}
...
void PhysicsTest::CloseUI() { mUIElems.Release(); }
...
TestBase::PlanarMeshHelper::PlanarMeshHelper() : mSurface(null), mRenderer(null) {}

TestBase::PlanarMeshHelper::~PlanarMeshHelper() { ASSERT(!mSurface); }

void TestBase::PlanarMeshHelper::Release() { DELETESINGLE(mSurface); }

void TestBase::PlanarMeshHelper::Generate(
        udword nb_x, udword nb_z, const Point& scale, float sine_freq, float sine_amplitude) {
    {
...
PintActorHandle TestBase::PlanarMeshHelper::CreatePlanarMesh(Pint& pint,
                                                             float altitude,
...
bool TestBase::Init(Pint& pint, bool create_def_env) {
    ...
    if (!Setup(pint, Caps)) return false;
...
PintRaycastData* TestBase::GetRegisteredRaycasts() const { return (PintRaycastData*)mRaycastData.GetEntries(); }
...
PintBoxSweepData* TestBase::GetRegisteredBoxSweeps() const { return (PintBoxSweepData*)mBoxSweepData.GetEntries(); }
...
void TestBase::RegisterConvexSweep(
        const udword convex_object_index, PintShapeRenderer* renderer, const PR& pr, const Point& dir, float max_dist) {
...
void TestBase::RegisterRenderer(PintShapeRenderer* renderer) { mRenderers.AddPtr(renderer); }

udword TestBase::GetNbRegisteredRenderers() const { return mRenderers.GetNbEntries(); }

PintShapeRenderer** TestBase::GetRegisteredRenderers() const { return (PintShapeRenderer**)mRenderers.GetEntries(); }
...
void RegisterArrayOfRaycasts(TestBase& test,
                             udword nb_x,
                             udword nb_y,
                             float altitude,
                             float scale_x,
...
void RegisterArrayOfBoxSweeps(TestBase& test,
                              udword nb_x,
                              udword nb_y,
                              float altitude,
                              float scale_x,
...
void RegisterArrayOfSphereSweeps(TestBase& test,
                                 udword nb_x,
                                 udword nb_y,
                                 float altitude,
                                 float scale_x,
...
void RegisterArrayOfCapsuleSweeps(TestBase& test,
                                  udword nb_x,
                                  udword nb_y,
                                  float altitude,
                                  float scale_x,
...
bool GenerateArrayOfVerticalCapsuleSweeps(TestBase& test, float scale, udword nb_x, udword nb_y, float max_dist) {
    ...
    RegisterArrayOfCapsuleSweeps(test, nb_x, nb_y, Altitude, scale, scale, Dir, CapsuleRadius, HalfHeight, Offset,
                                 max_dist);
    return true;
}
...
udword DoBatchRaycasts(TestBase& test, Pint& pint) {
    ...
    if (gRaycastMode == 0) {
        ...
        NbHits = pint.BatchRaycastAll(pint.mSQHelper->GetThreadContext(), Nb, Dest,
                                      pint.mSQHelper->GetRaycastAllStream(), Data);
    }
    return NbHits;
}
...
udword DoBatchSphereOverlaps(TestBase& test, Pint& pint, BatchOverlapMode mode) {
    ...
    if (mode == OVERLAP_ANY) {
        ...
        return pint.BatchSphereOverlapObjects(pint.mSQHelper->GetThreadContext(), Nb, Dest,
                                              pint.mSQHelper->GetSphereOverlapObjectsStream(), Data);
    }
    return 0;
}
...
udword DoBatchBoxOverlaps(TestBase& test, Pint& pint, BatchOverlapMode mode) {
    ...
    if (mode == OVERLAP_ANY) {
        ...
        return pint.BatchBoxOverlapObjects(pint.mSQHelper->GetThreadContext(), Nb, Dest,
                                           pint.mSQHelper->GetBoxOverlapObjectsStream(), Data);
    }
    return 0;
}
...
udword DoBatchCapsuleOverlaps(TestBase& test, Pint& pint, BatchOverlapMode mode) {
    ...
    if (mode == OVERLAP_ANY) {
        ...
        return pint.BatchCapsuleOverlapObjects(pint.mSQHelper->GetThreadContext(), Nb, Dest,
                                               pint.mSQHelper->GetCapsuleOverlapObjectsStream(), Data);
    }
    return 0;
}
...
udword DoBatchConvexOverlaps(TestBase& test, Pint& pint, BatchOverlapMode mode) {
    ...
    if (mode == OVERLAP_ANY) {
        ...
        return pint.BatchConvexOverlapObjects(pint.mSQHelper->GetThreadContext(), Nb, Dest,
                                              pint.mSQHelper->GetConvexOverlapObjectsStream(), Data);
    }
    return 0;
}
...
static bool LoadRays(const char* filename, TestBase& test, bool only_rays, bool no_processing) {
    ...
    if (no_processing) {
        for (udword i = 0; i < NbRays; i++) {
            ...
            const Point Origin(RayData.m_Start.x + RayData.m_StartOffset.x, RayData.m_Start.y + RayData.m_StartOffset.y,
        ...
        for (udword i = 0; i < NbRays; i++) {
            ...
            if (RayData.m_IsRay) NbIsRays++;
            if (RayData.m_IsSwept) NbIsSwept++;
            ...
            if (MaxDist != 0.0f) {
                ...
                if (only_rays || RayData.m_IsRay) {
                    ...
                    const Point Extents(RayData.m_Extents.x * Scale, RayData.m_Extents.z * Scale,
                                        RayData.m_Extents.y * Scale);
                    const OBB Box(Origin, Extents, Idt);
                    test.RegisterBoxSweep(Box, Dir, MaxDist * Scale);
                }
            }
...
void LoadRaysFile(TestBase& test, const char* filename, bool only_rays, bool no_processing) {
    ...
    if (!File || !LoadRays(File, test, only_rays, no_processing)) printf(_F("Failed to load '%s'\n", filename));
