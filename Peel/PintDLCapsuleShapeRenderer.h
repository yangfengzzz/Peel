...
public:
    ...
    struct Data {
        inline_ Data(float radius, float height, bool uses_vertex_normals)
            : mRadius(radius), mHeight(height), mUsesVertexNormals(uses_vertex_normals) {}
    ...
    inline_ bool Equal(const Data& p) const {
        return mData.mRadius == p.mRadius && mData.mHeight == p.mHeight &&
               mData.mUsesVertexNormals == p.mUsesVertexNormals;
    };
