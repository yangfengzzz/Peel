...
// Check file pointer
#define CHECK_FILE_PTR(err)                                                 \
    if (!mFp) {                                                             \
        SetIceError("IceFile::CHECK_FILE_PTR: invalid file pointer", null); \
        return err;                                                         \
    }
...
ubyte* IceFile2::Load(udword& length) {
    ...
    if (!length) {
        SetIceError("IceFile::Load: file has zero length", null);
        return null;
    }
    ...
    if (!mBuffer) {
        SetIceError("IceFile::Load: out of memory", null);
        return null;
    }
...
VirtualFile2::VirtualFile2(const VirtualFileHandle& handle)
    : mSize(0), mBuffer(null), mCurrentAddress(null), mFile(null) {
...
// TODO: these IceCore functions don't work on x64 for some reason (the disassembly is super strange). Redefining them
...
bool /*IceCore::*/ _GetPath(const char* filename, String& path) {
    ...
    if (!Separator0 && !Separator1) return false;
