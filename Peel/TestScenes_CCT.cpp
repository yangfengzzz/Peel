...
public:
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (!caps.mSupportCharacters || !API) return false;
    ...
    virtual bool RepositionMouse() const { return true; }

    virtual void DrawDebugInfo(Pint& pint, PintRender& render) {
        if (1) {
            AABB CCTBounds;
            if (GetCCTBounds(pint, CCTBounds)) render.DrawWireframeAABB(1, &CCTBounds, Point(1.0f, 0.0f, 0.0f));
        } else {
