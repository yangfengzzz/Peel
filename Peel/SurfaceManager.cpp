...
SurfaceManager::SurfaceManager() { mGlobalBounds.SetEmpty(); }
...
void SurfaceManager::RegisterSurface(const IndexedSurface* surface,
                                     PintShapeRenderer* renderer,
                                     const PR* pose,
                                     const PINT_MATERIAL_CREATE* material) {
    ASSERT(surface);

    if (!renderer) renderer = CreateMeshRenderer(PintSurfaceInterface(surface->GetSurfaceInterface()));
...
udword SurfaceManager::GetNbRegisteredSurfaces() const { return mSurfaceData.GetNbEntries() / gNb; }
...
bool SurfaceManager::CreateMeshesFromRegisteredSurfaces(Pint& pint,
                                                        const PintCaps& caps,
                                                        const PINT_MATERIAL_CREATE* material,
    ...
    if (Regular) {
        ...
        for (udword i = 0; i < Nb2; i++) {
            ...
            if (created_objects && h) created_objects->AddPtr(h);
        }
    }
    ...
    if (MergeGrid) {
        ...
        //			Counters[CellIndex]++;
                                ...
                                IndexedTriangle* Buffer =
        (IndexedTriangle*)C.Reserve(sizeof(IndexedTriangle)/sizeof(udword)); *Buffer = T[i];
                        }
                        ...
                        for(udword i=0;i<NbCells;i++)
                        {
                                ...
                                if(C.GetNbEntries())
                                {
                                        ...
                                        GridMesh.Init(NbTris, TotalNbVerts, Merged.GetVerts(), (const
        IndexedTriangle*)C.GetEntries()); GridMesh.Optimize();
