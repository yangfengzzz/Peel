...
static const PR gStartPoses[] = {
        ...
        //	PR(Point(60.5f, 5.9f, 40.1f), ShortestRotation(Point(0.0f, 0.0f, 1.0f), Point(1.0f, 0.0f, 0.0f))),
        //// SCENE_PLAYGROUND
...
static const float gDefaultSmoothingData[] = {
        //				Rise		Fall
        // Keyboard
        /*Accel*/ 3.0f,
        5.0f,
...
struct VehicleSDKData : public Allocateable {
    ...
    ~VehicleSDKData() {
        ...
        if (mStreamer) {
            ...
            public:
                ...
                virtual void ProcessRegion(const StreamRegion& region) { mStreamer.RemoveRegion(region); }
    ...
    inline_ void InitStreamer(const TerrainRegionManager::Params& params, udword tiles_per_side, bool deform_terrain) {
        ...
        if (deform_terrain) mDeformManager = ICE_NEW(DeformManager)(mOwner);
    }
...
static const PINT_SHAPE_CREATE* FindChassis(const PINT_OBJECT_CREATE* desc, const PINT_SHAPE_CREATE** wheels) {
    ...
    udword NbWheels = 0;
    if (Part0 != Chassis) {
        wheels[NbWheels++] = Part0;
    }
    if (Part1 != Chassis) {
        wheels[NbWheels++] = Part1;
    }
    if (Part2 != Chassis) {
        wheels[NbWheels++] = Part2;
    }
    if (Part3 != Chassis) {
        wheels[NbWheels++] = Part3;
    }
    if (Part4 != Chassis) {
        wheels[NbWheels++] = Part4;
    }
    ASSERT(NbWheels == 4);
...
static const PINT_SHAPE_CREATE* FindWheel(const PINT_SHAPE_CREATE** wheels, bool posx, bool posz) {
    ...
    if (WheelMatch(Wheel0, posx, posz)) return Wheel0;

    if (WheelMatch(Wheel1, posx, posz)) return Wheel1;

    if (WheelMatch(Wheel2, posx, posz)) return Wheel2;

    if (WheelMatch(Wheel3, posx, posz)) return Wheel3;
...
static const char* gDesc_VehicleSDK =
        "Basic test for the Vehicle SDK. Mainly based on the PhysX API & plugin for now. \
...
public:
    VehicleSDK(VehicleScene default_scene = SCENE_RACE_TRACK)
        ...
        for (udword i = 0; i < 16; i++) mEditBox_SmoothingData[i] = null;
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 0;
        {
            mCheckBox_DriveVehicle =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Drive vehicle", &UIElems, true, null, null);
            y += YStep;
        }
        ...
        // TAB_SCENE
        {
            ...
            mCheckBox_UseRenderMeshes =
                    helper.CreateCheckBox(TabWindow, 0, 4, y, 400, 20, "Use render meshes", &UIElems, true, null, null);
            ...
            sdword SavedY = y;
            {
                mRaceTrackWidgets[0] = helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                                                          "Race track size:", &UIElems);
                mRaceTrackWidgets[1] = mEditBox_RaceTrackSize =
                        helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "200.0", &UIElems,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
                y += YStep;

                mRaceTrackWidgets[2] = helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                                                          "Race track amplitude:", &UIElems);
                mRaceTrackWidgets[3] = mEditBox_RaceTrackAmplitude =
                        helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
                y += YStep;

                mRaceTrackWidgets[4] = helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                                                          "Race track width:", &UIElems);
                mRaceTrackWidgets[5] = mEditBox_RaceTrackWidth =
                        helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "6.0", &UIElems,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
                y += YStep;

                mRaceTrackWidgets[6] = helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                                                          "Race track wall height:", &UIElems);
                mRaceTrackWidgets[7] = mEditBox_RaceTrackWallHeight =
                        helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "2.0", &UIElems,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
                y += YStep;
            }
        ...
        // TAB_CAMERA
        {
            ...
            // mEditBox_CameraHeight = helper.CreateEditBox(TabWindow, 1, 4+OffsetX, y, EditBoxWidth, 20, "3.0",
            // &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_CameraHeight = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camera distance:", &UIElems);
            mEditBox_CameraDist = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "8.0", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camera sharpness (pos):", &UIElems);
            mEditBox_CameraSharpnessPos = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.4",
                                                               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camera sharpness (rot):", &UIElems);
            mEditBox_CameraSharpnessRot = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.05",
                                                               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        ...
        // TAB_CHASSIS
        {
            ...
            mEditBox_VehicleScale = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.35",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_VehicleScale->SetEnabled(false);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Width:", &UIElems);
            mEditBox_ChassisX = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Height:", &UIElems);
            mEditBox_ChassisY = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Length:", &UIElems);
            mEditBox_ChassisZ = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.5", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
            ...
            mEditBox_ChassisMOICoeffY = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0",
                                                             &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "CM Y offset:", &UIElems);
            mEditBox_ChassisCMOffsetY = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                             &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "CM Z offset:", &UIElems);
            mEditBox_ChassisCMOffsetZ = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                             &UIElems, EDITBOX_FLOAT, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                               "Force app. CM Y offset (~body roll):", &UIElems);
            mEditBox_ForceApplicationCMOffsetY =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.3", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                               "Ground clearance cut offset:", &UIElems);
            mEditBox_GroundClearanceCutOffset =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.3", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        ...
        // TAB_WHEEL_TIRES
        {
            ...
            mEditBox_WheelRadius = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5", &UIElems,
                                                        EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Width:", &UIElems);
            mEditBox_WheelWidth = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.3", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Coeff X:", &UIElems);
            mEditBox_WheelCoeffX = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.85",
                                                        &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Coeff Z:", &UIElems);
            mEditBox_WheelCoeffZ = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.85",
                                                        &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Mass:", &UIElems);
            mEditBox_WheelMass = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "20.0", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Max steer front:", &UIElems);
            mEditBox_WheelMaxSteerFront = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.047",
                                                               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Max steer rear:", &UIElems);
            mEditBox_WheelMaxSteerRear = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                              &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Antiroll bar stiffness:", &UIElems);
            mEditBox_AntirollBarStiffness = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                                 &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                               "Front tire friction multiplier (~grip):", &UIElems);
            mEditBox_TireFrictionMultiplierFront =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.1", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                               "Rear tire friction multiplier (~grip):", &UIElems);
            mEditBox_TireFrictionMultiplierRear =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.1", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Substeps:", &UIElems);
            mEditBox_Substeps = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "8", &UIElems,
            ...
            mEditBox_WheelMaxBrakeTorqueFront =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "150.0", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Max brake torque rear:", &UIElems);
            mEditBox_WheelMaxBrakeTorqueRear =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1500.0", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Max handbrake torque front:", &UIElems);
            mEditBox_WheelMaxHandBrakeTorqueFront =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Max handbrake torque rear:", &UIElems);
            mEditBox_WheelMaxHandBrakeTorqueRear =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "4000.0", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        ...
        // TAB_ENGINE_GEARBOX
        {
            ...
            mEditBox_EngineMOI = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Engine peak torque:", &UIElems);
            mEditBox_EnginePeakTorque = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "500.0",
                                                             &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Engine max omega:", &UIElems);
            mEditBox_EngineMaxOmega = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "600.0",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Gears switch time:", &UIElems);
            mEditBox_GearsSwitchTime = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5",
                                                            &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Clutch strength:", &UIElems);
            mEditBox_ClutchStrength = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "10.0",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        ...
        // TAB_SUSPENSION
        {
            ...
            mEditBox_SuspMaxCompression = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.3",
                                                               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Max droop:", &UIElems);
            mEditBox_SuspMaxDroop = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1",
                                                         &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Spring strength:", &UIElems);
            mEditBox_SuspSpringStrength = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20,
                                                               "35000.0", &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Spring damper rate:", &UIElems);
            mEditBox_SuspSpringDamperRate =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "4500.0", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camber angle at rest:", &UIElems);
            mEditBox_SuspCamberAngleAtRest = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0",
                                                                  &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20,
                               "Camber angle at max compression:", &UIElems);
            mEditBox_SuspCamberAngleAtMaxCompr =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.01", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Camber angle at max droop:", &UIElems);
            mEditBox_SuspCamberAngleAtMaxDroop =
                    helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.01", &UIElems,
                                         EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        ...
        // TAB_DIFFERENTIAL
        {
            ...
            mEditBox_FrontRearSplit = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.45",
                                                           &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
            ...
            mEditBox_FrontLeftRightSplit = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5",
                                                                &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
            ...
            mEditBox_RearLeftRightSplit = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.5",
                                                               &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            //\note Only applied to DIFF_TYPE_LS_4WD
            helper.CreateLabel(TabWindow, 4, y + LabelOffsetY, LabelWidth, 20, "Centre bias:", &UIElems);
            mEditBox_CentreBias = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.3", &UIElems,
                                                       EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
            ...
            mEditBox_FrontBias = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.3", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
            ...
            mEditBox_RearBias = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.3", &UIElems,
                                                     EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
        }
        ...
        // TAB_STEERING
        {
            ...
            for (udword i = 0; i < 4; i++) {
                ...
                helper.CreateLabel(TabWindow, LocalX, y + LabelOffsetY, LocalLabelWidth, 20, _F("Speed %d:", i),
                                   &UIElems);
                LocalX += LocalLabelWidth;
                mEditBox_SteeringSpeed[i] =
                        helper.CreateEditBox(TabWindow, 1, LocalX, y, EditBoxWidth, 20,
                                             helper.Convert(desc.mSteerVsForwardSpeedData[i * 2 + 0]), &UIElems,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
                LocalX += EditBoxWidth + 20;

                helper.CreateLabel(TabWindow, LocalX, y + LabelOffsetY, LocalLabelWidth, 20, _F("Value %d:", i),
                                   &UIElems);
                LocalX += LocalLabelWidth;
                mEditBox_SteeringValue[i] =
                        helper.CreateEditBox(TabWindow, 1, LocalX, y, EditBoxWidth, 20,
                                             helper.Convert(desc.mSteerVsForwardSpeedData[i * 2 + 1]), &UIElems,
                                             EDITBOX_FLOAT_POSITIVE, null, null);
                y += YStep;
            }
            ...
            mEditBox_AckermannAccuracy = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0",
                                                              &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;
...
#define APP_TO_GW(x) -(x)
        ...
        // TAB_CONTROLS
        {
            ...
            for (udword j = 0; j < 2; j++) {
                for (udword i = 0; i < 4; i++) {
                    ...
                    helper.CreateLabel(TabWindow, LocalX, y + LabelOffsetY, LocalLabelWidth, 20,
                                       "Rise rate:", &UIElems);
                    LocalX += LocalLabelWidth;
                    mEditBox_SmoothingData[k++] = helper.CreateEditBox(TabWindow, 1, LocalX, y, EditBoxWidth, 20,
                                                                       _F("%.3f", *SmoothingDataPtr++), &UIElems,
                                                                       EDITBOX_FLOAT_POSITIVE, null, null);
                    LocalX += EditBoxWidth + 20;
            ...
            mEditBox_ThresholdForwardSpeed = helper.CreateEditBox(TabWindow, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.1",
                                                                  &UIElems, EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            mCheckBox_ImmediateAutoReverse = helper.CreateCheckBox(
                    TabWindow, 0, 4, y, 400, 20, "Immediate auto reverse", &UIElems, false, null, null);
            y += YStep;
        }
        return null;
    }
    ...
    void OnComboBoxChange() {
        if (mComboBox_Level) {
            ...
            for (udword i = 0; i < NB_RACE_TRACK_WIDGETS; i++) mRaceTrackWidgets[i]->SetVisible(RaceTrackSelected);
    ...
    void FillDescFromUI(PINT_VEHICLE_CREATE& VehicleDesc) const {
        ...
        VehicleDesc.mImmediateAutoReverse =
                mCheckBox_ImmediateAutoReverse ? mCheckBox_ImmediateAutoReverse->IsChecked() : false;

        const float kph_to_mps = 1.0f / 3.6f;
        for (udword i = 0; i < 4; i++) {
            VehicleDesc.mSteerVsForwardSpeedData[i * 2 + 0] =
                    GetFloat(VehicleDesc.mSteerVsForwardSpeedData[i * 2 + 0], mEditBox_SteeringSpeed[i]) * kph_to_mps;
            VehicleDesc.mSteerVsForwardSpeedData[i * 2 + 1] =
                    GetFloat(VehicleDesc.mSteerVsForwardSpeedData[i * 2 + 1], mEditBox_SteeringValue[i]);
        }
    ...
    bool CreateLevelMeshes(Pint& pint, const PintCaps& caps) {
        ...
        const udword Index = mComboBox_Level->GetSelectedIndex();
        if (Index == SCENE_FACTORY || Index == SCENE_FLAT || Index == SCENE_PLAYGROUND ||
            Index == SCENE_PLAYGROUND_BRIDGE || Index == SCENE_PLAYGROUND_LOGS || Index == SCENE_SINE ||
            Index == SCENE_STRAIGHT_ROAD) {
        ...
        if (Index == SCENE_PLAYGROUND || Index == SCENE_PLAYGROUND_BRIDGE || Index == SCENE_PLAYGROUND_LOGS) {
...

            {
...

                {
                    ...
                    const PintDisabledGroups DG[] = {PintDisabledGroups(1, 2), PintDisabledGroups(1, 31),
                                                     PintDisabledGroups(2, 31)};
            ...
            if (!caps.mSupportRigidBodySimulation || !caps.mSupportKinematics) return false;
            ...
            const TerrainRegionManager::Params Params(
                    mTerrainStreamingUI.GetNbAddedRegionsPerFrame(), mTerrainStreamingUI.GetVerticesPerSide(),
                    mTerrainStreamingUI.GetWorldSize(), mTerrainStreamingUI.CreateAllInitialTiles());

            UserData->InitStreamer(Params, mTerrainStreamingUI.GetTilesPerSide(), mTerrainStreamingUI.DeformTerrain());
        }
        return true;
    }
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (!caps.mSupportVehicles || !API) return false;
        ...
        //		VehicleDesc.mWheels.mRenderer	= CreateConvexRenderer(VehicleDesc.mWheels.mNbVerts,
        //VehicleDesc.mWheels.mVerts);
        ...
        if (mKinePositions) {
            ...
            if (Handles) {
                ...
                while (Nb--) {
                    ...
                    if (h) pint.SetKinematicPose(h, Pos);
                }
            }
        }
        return 0;
    ...
    virtual udword Update(Pint& pint, float dt) {
        ...
        if (mCheckBox_DriveVehicle && mCheckBox_DriveVehicle->IsChecked()) {
            ...
            if (0 && UserData->mData.mChassisActor) {
                ...
                //		const Point CamPos = Pose.mPos + Point(0.0f, mCameraUpOffset, 0.0f) - Point(1.0f, 0.0f,
                //0.0f)*mCameraDistToTarget;
            ...
            if (UserData->GetStreamer() && UserData->GetStreamInterface()) {
                ...
                if (mTerrainStreamingUI.DrawWheelOverlapSpheres()) {
                    ...
                    if (ShapeAPI) {
                        const float OverlapRadius = mTerrainStreamingUI.GetWheelOverlapSphereRadius();
                        for (udword i = 0; i < 4; i++) {
                            const PR WheelPose = ShapeAPI->GetWorldTransform(UserData->mData.mChassisActor,
                                                                             UserData->mData.mWheelShapes[i]);
                ...
                static_cast<TerrainRegionManager*>(SI)->mDebugDrawVertexNormals =
                        mTerrainStreamingUI.DrawVertexNormals();
            ...
            /*if(Index==SCENE_TERRAIN_STREAMING)
            {
...

                    {
                            ...
                            for(sdword j=z0;j<z1;j++)
                            {
                                    ...
                                    {
                                            AABB CellBounds;
                                            CellBounds.SetMinMax(Point(float(i)*CellSizeX, 0.0f, float(j)*CellSizeZ),
            Point(float(i+1)*CellSizeX, 0.0f, float(j+1)*CellSizeZ)); render.DrawWireframeAABB(1, &CellBounds,
    ...
    void SaveCarData(const char* filename) const {
        ...
        {
            for (udword i = 0; i < 4; i++) {
                SteerVsForwardSpeedData[i] =
                        GetFloat(gDefaultSteerVsForwardSpeedData[i * 2 + 0], mEditBox_SteeringSpeed[i]);
                SteerVsForwardSpeedValue[i] =
                        GetFloat(gDefaultSteerVsForwardSpeedData[i * 2 + 1], mEditBox_SteeringValue[i]);
            }
        }
        ...
        const bool ImmediateAutoReverse =
                mCheckBox_ImmediateAutoReverse ? mCheckBox_ImmediateAutoReverse->IsChecked() : false;
        ...
        for (udword i = 0; i < 16; i++) CA.Store(SmoothingData[i]);
    ...
    void LoadCarData(const char* filename) {
        ...
        if (Version >= 2) AntirollBarStiffness = F.LoadFloat();
        udword WheelSubsteps = 8;
        if (Version >= 3) WheelSubsteps = F.LoadDword();
        ...
        if (Version >= 2) AckermannAccuracy = F.LoadFloat();
        ...
        float SmoothingData[16];
        for (udword i = 0; i < 16; i++) SmoothingData[i] = gDefaultSmoothingData[i];
        if (Version >= 6) {
            for (udword i = 0; i < 16; i++) SmoothingData[i] = F.LoadFloat();
        }
        ...
        for (udword i = 0; i < 16; i++) mEditBox_SmoothingData[i]->SetText(_F("%.3f", SmoothingData[i]));
...
public:
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (!caps.mSupportMeshes || !caps.mSupportRigidBodySimulation) return false;
    ...
    virtual udword GetFlags() const { return TEST_FLAGS_USE_CURSOR_KEYS; }
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (1) {
            ...
            udword GroupBit = 0;
            {
                ...
                pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[0], VehicleData->mChassis0,
                                                             Point(0.0f, 0.0f, Radius),
                                                             Point(0.0f, 0.0f, -ChassisExtents.z)));
                pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Handles[NbSpheres - 1], VehicleData->mChassis1,
                                                             Point(0.0f, 0.0f, -Radius),
                                                             Point(0.0f, 0.0f, ChassisExtents.z)));
            }
            ...
            /*			if(1)
                                    {
                                            ...
                                            Desc.mObject1		= h;
                                            Desc.mLocalPivot0	= Point(0.0f, ChassisExtents.y, 0.0f);
                                            Desc.mLocalPivot1	= Point(0.0f, -BoxDesc.mExtents.y, Pose0.z -
               MiddlePose.z); Desc.mLocalAxis0	= Point(0.0f, 1.0f, 0.0f); Desc.mLocalAxis1	= Point(0.0f, 1.0f,
               0.0f); PintJointHandle JointHandle = pint.CreateJoint(Desc); ASSERT(JointHandle);
                                    }
                                    if(1)
                                    {
                                            PINT_FIXED_JOINT_CREATE Desc;
                                            ...
                                            Desc.mObject1		= h;
                                            Desc.mLocalPivot0	= Point(0.0f, ChassisExtents.y, 0.0f);
                                            Desc.mLocalPivot1	= Point(0.0f, -BoxDesc.mExtents.y, Pose1.z -
               MiddlePose.z); PintJointHandle JointHandle = pint.CreateJoint(Desc); ASSERT(JointHandle);
                                    }*/
        }
        return true;
    }
...
public:
    ...
    virtual bool CommonSetup() {
        if (gVehicleFilename.IsValid()) mVehicleFile = gVehicleFilename.Get();
        ...
        if (Filename) {
            ...
            if (mCheckBox_UseRenderMeshes) mCheckBox_UseRenderMeshes->SetEnabled(true);
            if (mEditBox_VehicleScale) mEditBox_VehicleScale->SetEnabled(true);
            ...
            if (mZB2Factory) {
                ...
                if (NbActors == 1) {
                    ...
                    if (Scale != 1.0f) {
                        ...
                        for (udword i = 0; i < ConvexWheel0->mNbVerts; i++) v0[i] *= Scale;

                        Point* v1 = const_cast<Point*>(ConvexWheel1->mVerts);
                        for (udword i = 0; i < ConvexWheel1->mNbVerts; i++) v1[i] *= Scale;

                        Point* v2 = const_cast<Point*>(ConvexWheel2->mVerts);
                        for (udword i = 0; i < ConvexWheel2->mNbVerts; i++) v2[i] *= Scale;

                        Point* v3 = const_cast<Point*>(ConvexWheel3->mVerts);
                        for (udword i = 0; i < ConvexWheel3->mNbVerts; i++) v3[i] *= Scale;
                    }
                    ...
                    mEditBox_ChassisX->SetText(_F("%.3f", ChassisExtents.x * Scale));
                    mEditBox_ChassisX->SetEnabled(false);
                    mEditBox_ChassisY->SetText(_F("%.3f", ChassisExtents.y * Scale));
                    mEditBox_ChassisY->SetEnabled(false);
                    mEditBox_ChassisZ->SetText(_F("%.3f", ChassisExtents.z * Scale));
                    mEditBox_ChassisZ->SetEnabled(false);
                    ...
                    const float CutOffset = GetFloat(0.3f, mEditBox_GroundClearanceCutOffset);
                    {
                        ...
                        for (udword i = 0; i < ConvexChassis->mNbVerts; i++) {
                            ...
                            if (ppp[i].y < limit) ppp[i].y = limit;
                        }
                    }
                }
            }
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        //		CreateMeshesFromRegisteredSurfaces(pint, caps, *this, &mHighFrictionMaterial);
        if (!CreateLevelMeshes(pint, caps)) return false;
        ...
        if (mZB2Factory) {
            ...
            if (NbActors == 1) {
                ...
                //				VehicleDesc.mWheelOffset[0] = Point( ChassisExtents.x*CoeffX, -WheelRadius,
                //ChassisExtents.z*CoeffZ); 				VehicleDesc.mWheelOffset[1] = Point(-ChassisExtents.x*CoeffX, -WheelRadius,
                //ChassisExtents.z*CoeffZ); 				VehicleDesc.mWheelOffset[2] = Point( ChassisExtents.x*CoeffX, -WheelRadius,
                //-ChassisExtents.z*CoeffZ); 				VehicleDesc.mWheelOffset[3] = Point(-ChassisExtents.x*CoeffX, -WheelRadius,
                //-ChassisExtents.z*CoeffZ);
                /*++
                -+
                +-
                --
                ...
                1
                2*/
                /*				const PINT_SHAPE_CREATE* Wheel0 = FindWheel(Create->mShapes, true,
                   true); const PINT_SHAPE_CREATE* Wheel1 = FindWheel(Create->mShapes, false, true); const
                   PINT_SHAPE_CREATE* Wheel2 = FindWheel(Create->mShapes, true, false); const PINT_SHAPE_CREATE* Wheel3
                   = FindWheel(Create->mShapes, false, false);*/
                ...
                /*				{
                                                        ...
                                                        VehicleDesc.mWheels.mRenderer	=
                CreateConvexRenderer(VehicleDesc.mWheels.mNbVerts, VehicleDesc.mWheels.mVerts);
                ...
                /*				FILE* fp = fopen("D:\\tmp\\wheel_verts.bin", "wb");
                                                if(fp)
                                                {
                                                        fwrite(VehicleDesc.mWheel.mVerts,
                   VehicleDesc.mWheel.mNbVerts*sizeof(Point), 1, fp); fclose(fp);
                                                }*/
                ...
                /*
                                                struct Local
                                                {
                                                        static Point FindWheelOffset(const Point& pt0, const Point& pt1,
                   const Point& pt2, const Point& pt3, bool plusx, bool plusz)
                                                        {
                                                        }
                                                }*/
                ...
                if (0) {
                    ...
                    Desc.mMaterial = &Zero;
                    Desc.mLocalPos = Point(0.0f, VehicleDesc.mWheelOffset[0].y + VehicleDesc.mSuspMaxCompression,
                                           VehicleDesc.mWheelOffset[0].z);
                    bool b0 = API->AddShape(VehicleHandle, Desc);
                    Desc.mLocalPos = Point(0.0f, VehicleDesc.mWheelOffset[2].y + VehicleDesc.mSuspMaxCompression,
                                           VehicleDesc.mWheelOffset[2].z);
    ...
    public:                                                                                         \
        ...
        virtual IceTabControl* InitUI(PintGUIHelper& helper) {                                      \
            ...
            if (pvf) {                                                                              \
                ...
                if (VehicleFile) LoadCarData(VehicleFile);                                          \
            }                                                                                       \
            if (mCheckBox_DriveVehicle) mCheckBox_DriveVehicle->SetChecked(scene != SCENE_FACTORY); \
            return tc;                                                                              \
        }                                                                                           \
    } name;
...
DRIVABLE_LEGO_CAR(LegoPolicePursuitTruck,
                  "PolicePursuit_Truck_Vehicle.zb2",
                  "PursuitTruck.pvf",
                  SCENE_PLAYGROUND_BRIDGE)
...
DRIVABLE_LEGO_CAR(LegoSpeedChampions_Mustang,
                  "1968_Ford_Mustang_Fastback_Vehicle.zb2",
                  "Mustang1968.pvf",
                  SCENE_RACE_TRACK)
...
DRIVABLE_LEGO_CAR(LegoSpeedChampions_DodgeCharger,
                  "1970_Dodge_Charger_RT_Vehicle.zb2",
                  "DodgeCharger.pvf",
                  SCENE_RACE_TRACK)

static const char* gDesc_LegoSpeedChampions_DodgeChallenger = "Lego Speed Champions - 2018 Dodge Challenger SRT Demon";
DRIVABLE_LEGO_CAR(LegoSpeedChampions_DodgeChallenger,
                  "2018_Dodge_Challenger_SRT_Demon_Vehicle.zb2",
                  "DodgeChallenger.pvf",
                  SCENE_RACE_TRACK)
...
DRIVABLE_LEGO_CAR(Lego_Lamborghini_Centenario_Brown,
                  "Lamborghini_Centenario_(Brown)_Vehicle.zb2",
                  "SportCar3.pvf",
                  SCENE_RACE_TRACK)

static const char* gDesc_Lego_Lamborghini_Centenario_Blue = "Lego - Lamborghini Centenario (Blue)";
DRIVABLE_LEGO_CAR(Lego_Lamborghini_Centenario_Blue,
                  "Lamborghini_Centenario_(Blue)_Vehicle.zb2",
                  "SportCar3.pvf",
                  SCENE_RACE_TRACK)
