...
float GetDefaultEnvironmentSize() { return gDefaultEnvSize; }

bool SetupDefaultEnvironment(Pint& pint, bool enabled) {
    if (pint.mDefaultEnvHandle) return false;

    if (gCurrentRenderModel) gCurrentRenderModel->SetGroundPlane(enabled);
...
bool ReleaseDefaultEnvironment(Pint& pint) {
    ...
    if (gCurrentRenderModel) gCurrentRenderModel->SetGroundPlane(false);
    return true;
}
