...
static inline_ void ComputeShading(RGBAPixel& color,
                                   const PintRaycastHit& hit,
                                   const Point& light_dir,
                                   const Point& light_color) {
    if (1) {
...
udword RaytracingTest(Picture& pic,
                      Pint& pint,
                      udword& total_time,
                      udword screen_width,
...
struct RaytracingParams {
    ...
    inline_ void SetParams(Pint& pint,
                           RGBAPixel* pixels,
                           const Point& origin,
                           const Point& light_dir,
                           /*udword nb_to_go,*/ udword y_start,
                           udword y_end,
                           float max_dist,
                           float screen_width,
                           float screen_height) {
        mPint = &pint;
        mPixels = pixels;
        mOrigin = origin, mLightDir = light_dir;
        //		mNbToGo			= nb_to_go;
        mYStart = y_start;
        mYEnd = y_end;
        mMaxDist = max_dist;
...
udword RaytracingTestMT(Picture& pic,
                        Pint& pint,
                        udword& total_time,
                        udword screen_width,
                        udword screen_height,
    ...
    RaytracingParams Params[NB_RT_THREADS];
    {
        ...
        for (udword i = 0; i < NB_RT_THREADS; i++) {
            ...
            Params[i].SetParams(pint, Pixels + i * NbPixelsPerThread, origin, LightDir, /*NbPixelsPerThread,*/ YStart,
                                YEnd, max_dist, fScreenWidth, fScreenHeight);
        ...
        StartProfile(Time);
        {
            if (0) {
                ...
                for (udword i = 0; i < NB_RT_THREADS - 1; i++) WaitThread(Threads[i], null);
            } else {
                for (udword i = 0; i < NB_RT_THREADS - 1; i++) PEEL_AddThreadWork(i, gRaytracingThreadFunc, &Params[i]);
