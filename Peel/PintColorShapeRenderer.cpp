...
PintColorShapeRenderer::PintColorShapeRenderer(PintShapeRenderer* renderer,
                                               const RGBAColor& color,
                                               const ManagedTexture* texture)
    : mData(renderer, color, texture)
// PintShapeRenderer	(SHAPE_RENDERER_COLOR),
{
    ...
    if (renderer->_UseExplicitVertexNormals()) mBaseFlags |= SHAPE_RENDERER_USES_EXPLICIT_VERTEX_NORMALS;
...
void PintColorShapeRenderer::_Render(const PR& pose) const {
    ...
    if (0 && SetupRS) {
        if (!gExternalColorControl && mData.mColor.A != 1.0f) {
            if (!gTransparent) gTransparent = ICE_NEW(Container);
