...
struct MidphaseHits : Allocateable {
    MidphaseHits(PintActorHandle mesh_handle, PintActorHandle query_shape_handle)
        : mMeshHandle(mesh_handle), mQueryShapeHandle(query_shape_handle) {
        mHits.SetNoHit();
    }
...
static bool MidphaseSetup(Pint& pint, SurfaceManager& sm, const PintCaps& caps, PintActorHandle query_shape_handle) {
    ...
    if (!sm.CreateMeshesFromRegisteredSurfaces(pint, caps, null, &Objects)) return false;

    ASSERT(Objects.GetNbEntries() == 1);
    if (!Objects.GetNbEntries()) return false;
...
public:
    ...
    virtual udword Update(Pint& pint, float dt) {
        ...
        MH->mStream.Reset();
        return pint.FindTriangles_MeshSphereOverlap(pint.mSQHelper->GetThreadContext(), MeshHandle, 1, &MH->mHits,
                                                    MH->mStream, &Data);
    }
    ...
    public:
        ...
        virtual udword Update(Pint& pint, float dt) {
            ...
            MH->mStream.Reset();
            return pint.FindTriangles_MeshBoxOverlap(pint.mSQHelper->GetThreadContext(), MeshHandle, 1, &MH->mHits,
                                                     MH->mStream, &Data);
        }
        ...
        public:
            ...
            virtual udword Update(Pint& pint, float dt) {
                ...
                MH->mStream.Reset();
                return pint.FindTriangles_MeshCapsuleOverlap(pint.mSQHelper->GetThreadContext(), MeshHandle, 1,
                                                             &MH->mHits, MH->mStream, &Data);
            }
