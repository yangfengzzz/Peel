...
static float PrintTwistAngle(Pint& pint, GLFontRenderer& renderer, float y, float text_scale) {
    ...
    float TwistAngle;
    if (!GetHingeTwistAngle(pint, Handle, TwistAngle)) return y;
...
static bool CreateJointedCubes(
        Pint& pint, const PintCaps& caps, const Point& static_pos, const Point& local_axis, float limit) {
    if (!caps.mSupportHingeJoints || !caps.mSupportRigidBodySimulation || !caps.mSupportCollisionGroups) return false;
    ...
    if (limit > 0.0f) Desc.mLimits.Set(degToRad(-limit), degToRad(limit));
...
static const char* gDesc_BasicJointAPI =
        "Test for basic joint API. Creates a single joint of a chosen type. Each test has one dynamic object linked to one \
...
public:
    ...
    virtual const char* GetSubName() const {
        if (mComboBox_Preset) {
            ...
            if (SelectedIndex == 0) return "FixedJoint";
            if (SelectedIndex == 1) return "DistanceJoint";
            if (SelectedIndex == 2) return "SphericalJoint";
            if (SelectedIndex == 3) return "HingeJoint";
            if (SelectedIndex == 4) return "PrismaticJoint";
        }
        return null;
    }
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (!mComboBox_Preset) return false;
        const udword SelectedIndex = mComboBox_Preset->GetSelectedIndex();

        if (SelectedIndex == 0) {
            if (!caps.mSupportFixedJoints) return false;
            ...
            if (!caps.mSupportDistanceJoints) return false;
            ...
            if (!caps.mSupportSphericalJoints) return false;
            ...
            const PintJointHandle JointHandle =
                    pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(StaticObject, DynamicObject, -Extents, Extents));
            ASSERT(JointHandle);
        } else if (SelectedIndex == 3) {
            if (!caps.mSupportHingeJoints) return false;
            return CreateJointedCubes(pint, caps, Point(0.0f, 20.0f, 0.0f), Point(0.0f, 0.0f, 1.0f), -1.0f);
        } else if (SelectedIndex == 4) {
            if (!caps.mSupportPrismaticJoints) return false;
...
static const char* gDesc_CheckCollisionBetweenJointed =
        "Test to check if collision is enabled or disabled by default, between objects \
...
static const char* gDesc_LimitedHingeJoint =
        "Simple test scene for hinge joints with (angular) limits. Filtering is used to disable collisions between "
        "jointed objects.";
...
static const char* gDesc_LimitedHingeJoint2 =
        "Simple test scene for hinge joints with (angular) limits, this time between two dynamic objects.";
...
static const char* gDesc_ConfigurableLimitedHingeJoint =
        "Hinge joints with configurable limits. Different engines support different limit ranges. Some only support "
        "-PI to +PI.";
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 0;
        {
            ...
            mEditBox_LimitAngle = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "90.0", &UIElems,
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (!caps.mSupportPrismaticJoints || !caps.mSupportRigidBodySimulation) return false;
    ...
    static const char* gDesc_LimitedPrismaticJoint2 =
            "Simple test scene for prismatic joints with (linear) limits, this time between two dynamic objects. \
The top cube should rest on top of the bottom cube.";
    ...
    static const char* gDesc_PrismaticSpring =
            "Simple test scene for prismatic springs. Some engines have support for either soft or hard limits. \
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        for (udword j = 0; j < 2; j++) {
            ...
            for (udword i = 0; i < 2; i++) {
                ...
                const Point Offset(x * (Extents.x - ChildExtents.x), -Extents.y - ChildExtents.y,
                                   z * (Extents.z - ChildExtents.z));
    ...
    public:
        ...
        virtual IceTabControl* InitUI(PintGUIHelper& helper) {
            ...
            sdword y = 0;
            {
                ...
                mCheckBox_EnableUpdate =
                        helper.CreateCheckBox(UI, 0, 4, y, 150, 20, "Enable motor update", &UIElems, true, null, null);
                y += YStep;

                helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Target vel.:", &UIElems);
                mEditBox_TargetVel = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1.0", &UIElems,
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            ...
            udword Type = 0;
            if (mComboBox_JointType) Type = mComboBox_JointType->GetSelectedIndex();
            ...
            if (EnableUpdate) {
                ...
                if (LTD) {
                    ...
                    if (LTD->mJoint) {
                        ...
                        if (EnableMotor) pint.SetRCADriveVelocity(LTD->mDynamicObject, TargetVel);
                    }
                }
            }
            return 0;
        ...
        static const char* gDesc_HingeJointMotorVsObstacle =
                "Hinge joints motor vs static obstacle. This is a case where increasing the shape's contact offset \
or enabling angular CCD could improve the behavior on impact.";
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            ...
            //		PintActorHandle Obstacle = CreateSimpleObject(pint, &BoxDesc, 1000000.0f, static_pos +
            //Point(-8.0f,
            // 0.0f, 0.0f)); 		PintActorHandle Obstacle = CreateSimpleObject(pint, &BoxDesc, 10.0f, static_pos
        ...
        public:
            ...
            //			printf("TargetVel: %f\n", TargetVel);
                                    if(LTD->mJoint)
                                    {
                                            pint.SetDriveEnabled(LTD->mJoint, EnableMotor);
                                            if(EnableMotor)
                                                    pint.SetDriveVelocity(LTD->mJoint, Point(0.0f, 0.0f, 0.0f),
            Point(TargetVel, 0.0f, 0.0f));
                                    }
                                    else
                                    {
                                            pint.SetRCADriveEnabled(LTD->mDynamicObject, EnableMotor);
