...
public:
    ...
    override(Allocator) void* mallocDebug(
            size_t size, const char* filename, udword line, const char* class_name, MemoryType type, bool from_new);
...
DefaultAllocator::DefaultAllocator()
    : mNbAllocatedBytes(0), mHighWaterMark(0), mTotalNbAllocs(0), mNbAllocs(0), mNbReallocs(0) {
...
DefaultAllocator::~DefaultAllocator() { Release(); }
...
void DefaultAllocator::Release() {
    ...
    // Ok, it is a bad idea to use _F() here, because it internally uses the allocator (for the log string). So let's
    // use good old C style here.
    char Buffer[4096];
    ...
    if (mMemBlockList && mNbAllocs) {
        ...
        for (udword i = 0; i < mMemBlockUsed; i++) {
            if (size_t(mMemBlockList[i]) & 1) continue;
...
void* DefaultAllocator::mallocDebug(
        size_t size, const char* filename, udword line, const char* class_name, MemoryType type, bool from_new) {
...
void DefaultAllocator::FrameAnalysis() {
    ...
    if (mNbAllocs <= mPreviousNbAllocs) mNoLeak = true;
    ...
    if (mMemBlockList && mMemBlockUsed) {
        ...
        for (udword i = 1; i <= NbStrings; i++)  // One more time on purpose
        {
            ...
            if (Current && strcmp(Current, CurrentClass) == 0) {
                ...
                if (CurrentClass) {
                    Classes[NbClasses] =
                            (udword)CurrentClass;  // We can store this pointer now because it's unique in our new array
...
Allocator* CreatePEELDefaultAllocator() { return ::new DefaultAllocator; }
