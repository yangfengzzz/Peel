...
static PintActorHandle createRenderable(
        FractureConvex& convex, Pint& pint, const Matrix4x4& pose, bool isStatic, const Point& vel) {
    const udword NbVerts = convex.GetNbVerts();
    const Point* Vertices = convex.GetVerts();

    if (!NbVerts || !Vertices) return null;
...
void MultiConvex::DeleteConvex(Pint* pint, PintActorHandle actor, FractureConvex* convex) {
    if (actor && pint) ReleasePintObject(*pint, actor, true);
...
void MultiConvex::clear() {
    const udword Nb = mConvexData.size();
    for (udword i = 0; i < Nb; i++) DeleteConvex(mConvexData[i]);
...
bool MultiConvex::randomFracture(Pint& pint, const int convexNr, const Ray& ray, int numCuts, float vel) {
    if (convexNr < 0 || convexNr >= int(mConvexData.size())) return false;
    ...
    for (int i = 0; i < numCuts; i++) {
        ...
        for (int j = 0; j < num; j++) {
            ...
            if (!newConvexes[j]->cut(planeN, planeD, c0, c1)) continue;
...
bool MultiConvex::patternFracture(
        Pint& pint, const int convexNr, const Ray& ray, const FracturePattern& pattern, float scale, float vel) {
    if (convexNr < 0 || convexNr >= int(mConvexData.size())) return false;
