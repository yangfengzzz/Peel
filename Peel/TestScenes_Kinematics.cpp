...
void KinematicTestScene::OnObjectReleased(Pint& pint, PintActorHandle removed_object) {
    ...
    if (Handles) {
        for (udword i = 0; i < mNbLayers * mNbX * mNbY; i++) {
            if (Handles[i] == removed_object) Handles[i] = null;
        }
    }
}
...
bool KinematicTestScene::Setup(Pint& pint, const PintCaps& caps) {
    ...
    if (mAddDynamicObjects) return GenerateArrayOfBoxes(pint, Point(1.0f, 1.0f, 1.0f), 32, 32, 10.0f, 60.0f, 60.0f);
    return true;
}
...
static const char* gDesc_KinematicObjects_NoOverlap =
        "64*64 (non overlapping) kinematic objects. There should be no broadphase entries for kinematics.";
...
static const char* gDesc_KinematicObjects_Overlap =
        "64*64 (overlapping) kinematic objects. There could be broadphase entries for kinematics, memory usage could "
        "increase.";
...
static const char* gDesc_KinematicObjects_NoOverlap_DynamicObjects =
        "64*64 (non overlapping) kinematic objects, plus 32*32 dynamic cubes.";
...
static const char* gDesc_KinematicObjects_Overlap_DynamicObjects =
        "64*64 (overlapping) kinematic objects, plus 32*32 dynamic cubes.";
...
static const char* gDesc_KinematicObjects_Overlap_StaticObjects =
        "64*64 (overlapping) kinematic objects, plus 128*128 static cubes. The presence of statics may decrease "
        "performance compared to the same scene without statics.";
...
public:
    KinematicCharacterTest()
        ...
        for (udword i = 0; i < NB_BONES; i++) mBoneRenderer[i] = null;
    }
    virtual ~KinematicCharacterTest() { DELETEARRAY(mCapturedData); }
    ...
    virtual bool CommonSetup() {
        ...
        if (mCapturedData) {
            ...
            for (udword i = 0; i < NB_BONES; i++) mBoneRenderer[i] = CreateBoxRenderer(Src[i].mExtents);
        }

        BasicRandom Rnd(42);
        for (udword i = 0; i < MAX_NB_CHARACTERS; i++) mCharacters[i].Init(Rnd);
        return true;
    }
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (!caps.mSupportKinematics) return false;
        if ((mUseAggregates_Characters || mUseAggregates_Level) && !caps.mSupportAggregates) return false;
        ...
        for (udword j = 0; j < mNbCharacters; j++) {
            PintAggregateHandle Aggregate =
                    mUseAggregates_Characters ? pint.CreateAggregate(NB_BONES, mAggregatesSelfCollide) : null;
            const OBB* Src = mCapturedData;
            for (udword i = 0; i < NB_BONES; i++) {
                ...
                const PintActorHandle Handle = CreatePintObject(pint, ObjectDesc);
                *Handles++ = Handle;
                if (Aggregate) pint.AddToAggregate(Handle, Aggregate);
            }
    ...
    virtual void CommonUpdate(float dt) {
        ...
        for (udword i = 0; i < mNbCharacters; i++) mCharacters[i].Update();
    }
            ...
            if (Handles) {
                for (udword j = 0; j < mNbCharacters; j++) {
                    ...
                    for (udword i = 0; i < NB_BONES; i++) {
                        ...
                        Pose.mRot.Normalize();
                        const PintActorHandle h = *Handles++;
                        if (h) pint.SetKinematicPose(h, Pose);
                    }
                }
            }
        }
...
static const char* gDesc_KinematicCharacter =
        "Kinematic character with or without aggregates, with or without self-collisions, moving in a mockup static "
        "world.";
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        const bool Enabled = true;
        {
            ...
            mEditBox_NbCharacters = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1", &UIElems,
                                                         EDITBOX_INTEGER_POSITIVE, null, null);
            mEditBox_NbCharacters->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_UseAggregates =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use aggregates", &UIElems, false, null, null);
            mCheckBox_UseAggregates->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_AggregateSelfCollision = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Aggregate self-collision",
                                                                     &UIElems, false, null, null);
            mCheckBox_AggregateSelfCollision->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_Level =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Add static level mockup", &UIElems, false, null, null);
            mCheckBox_Level->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_UseAggregatesForLevel = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Use aggregates for level",
                                                                    &UIElems, false, null, null);
            mCheckBox_UseAggregatesForLevel->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_ProfileKinePoseUpdate = helper.CreateCheckBox(
                    UI, 0, 4, y, 400, 20, "Profile kinematic pose updates", &UIElems, false, null, null);
            mCheckBox_ProfileKinePoseUpdate->SetEnabled(Enabled);
            y += YStep;

            mCheckBox_SkipKinePoseUpdate = helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Skip kinematic pose updates",
                                                                 &UIElems, false, null, null);
        ...
        }
        {
            ...
            public:
                MyComboBox(const ComboBoxDesc& desc, KinematicCharacter& test) : IceComboBox(desc), mTest(test) {}
                virtual ~MyComboBox() {}
                virtual void OnComboBoxEvent(ComboBoxEvent event) {
                    if (event == CBE_SELECTION_CHANGED) {
                        ...
                        if (!Enabled && SelectedIndex < NB_PRESETS) {
                            ...
                            mTest.mCheckBox_AggregateSelfCollision->SetChecked(
                                    gPreset[SelectedIndex].mAggregatesSelfCollide);
                            mTest.mCheckBox_Level->SetChecked(gPreset[SelectedIndex].mAddStaticObjects);
                            mTest.mCheckBox_UseAggregatesForLevel->SetChecked(
                                    gPreset[SelectedIndex].mUseAggregates_Level);
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) { return KinematicCharacterTest::Setup(pint, caps); }
...
static float SawTooth(float t) {
    ...
    if (t < HalfPi) return t / HalfPi;

    if (t < PI) return 1.0f - ((t - HalfPi) / HalfPi);

    return -SawTooth(t - PI);
}

static udword CircleCircleIntersection(
        Point* ip, const Point& center0, float radius0, const Point& center1, float radius1) {
...
public:
    ...
    void Update(
            float elapsed_time, bool is_moving, float current_direction, float walk_speed, float walk_speed_multiplier);
...
void WalkCycle::Update(
        float elapsed_time, bool is_moving, float current_direction, float walk_speed, float walk_speed_multiplier) {
    if (1) {
        mCurrentStride = gCycleStride;  // * walk_speed_multiplier;
        if (!is_moving) elapsed_time = 0.0f;
    }
    ...
    if (0) {
        ...
        if (is_moving) {
            ...
            if (mCurrentStride > gCycleStride) mCurrentStride = gCycleStride;
        } else {
            mCurrentStride -= fabsf(elapsed_time) * 4.0f;
            if (mCurrentStride < 0.0f) mCurrentStride = 0.0f;
        }
    }
    ...
    /*	b2RaycastHit hit = Raycast(mWorld, rayOrigin, LowerPos - Point(0.0f, gProbeLength), 0xffffffff &
    ~(FILTER_GROUP_LEGS|FILTER_GROUP_PLAYER|FILTER_GROUP_TREE)); if(hit.mFixture)
            {
...
void WalkCycle::Render(PintRender& render) {
    {
        /*		g_debugDraw.DrawCircle(mPos, gCycleStride, b2Color(1.0f, 0.0f, 0.0f));
                        ...
                        g_debugDraw.DrawSolidCircle(mLowerPos, 0.1f, Point(0.0f, 0.0f), mFixed ? b2Color(0.0f,
           0.0f, 1.0f) : b2Color(1.0f, 1.0f, 1.0f));
        ...
        render.DrawLine(mLowerPos + Point(0.0f, gProbeOffset, 0.0f), mLowerPos - Point(0.0f, gProbeLength, 0.0f),
...
static const char* gDesc_Walker =
        "Kinematic walker. The dynamic feet are attached to kinematic legs and forced into an 'impossible' situation. \
...
virtual IceTabControl* InitUI(PintGUIHelper& helper) { return CreateOverrideTabControl("Walker", 20); }
...
virtual void Walker::CommonRelease() {
    for (udword i = 0; i < 2; i++) DELETESINGLE(mWalkCycle[i]);
...
virtual void Walker::CommonDebugRender(PintRender& render) {
    if (0) {
        for (udword i = 0; i < 2; i++) {
            if (mWalkCycle[i]) mWalkCycle[i]->Render(render);
        }
    }
...
virtual bool Walker::Setup(Pint& pint, const PintCaps& caps) {
    ...
    for (udword i = 0; i < 2; i++) {
        ...
        }
        {
            ...
            BoxDesc.mRenderer = CreateBoxRenderer(BoxDesc.mExtents);
            BoxDesc.mMaterial = &mZeroFrictionMaterial;  // To avoid friction forces resisting the motion of the upper
                                                         // kinematic legs
