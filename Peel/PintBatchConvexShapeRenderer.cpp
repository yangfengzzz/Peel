...
void StartBatchConvexRender() {
    gCurrentNbVerts = 0;
    gCurrentNbTris = 0;
    if (!gBatchIndices) gBatchIndices = ICE_NEW(Container);
    if (!gBatchVertices) gBatchVertices = ICE_NEW(Vertices);
    if (!gBatchNormals /* && gCurrentRenderModel->NeedsVertexNormals()*/) gBatchNormals = ICE_NEW(Vertices);
}
...
static void BatchRender(
        udword nb_tris, udword nb_verts, const Point* verts, const Point* normals, const udword* indices) {
    if (verts) CopyMemory(gBatchVertices->Reserve(nb_verts * 3), verts, sizeof(Point) * nb_verts);
    if (normals) CopyMemory(gBatchNormals->Reserve(nb_verts * 3), normals, sizeof(Point) * nb_verts);

    udword* BatchedIndices = gBatchIndices->Reserve(nb_tris * 3);
    udword Nb = nb_tris * 3;
    while (Nb--) *BatchedIndices++ = gCurrentNbVerts + *indices++;
...
PintConvexBatchRendererCPUTransformNoNormals::PintConvexBatchRendererCPUTransformNoNormals(udword nb_verts,
                                                                                           const Point* verts) {
    Cvh* CHull = CreateConvexHull(nb_verts, verts);
    ASSERT(CHull);
