...
static const char *fragmentPointShader = "#version 130\n" STRINGIFY(
        ...
        uniform vec3 lightDir; uniform vec3 lightPos; uniform float spotMin; uniform float spotMax; uniform int mode;
...
void GLPointRenderer::Close() { glDeleteBuffers(1, &mPositionVBO); }
    ...
    if (1) {
        ...
        //screenAspect, float fov)
        {
            if (sprogram) {
                ...
                glUniform1f(glGetUniformLocation(sprogram, "pointScale"),
...
void PointRenderer::Close() { glDeleteBuffers(1, &mPositionVBO); }
...
const char *fragmentPointShader = STRINGIFY(
        ...
        void main() {
            ...
            vec3 Lo = diffuse * reflectance * max(0.0, sqr(-dot(gl_TexCoord[2].xyz, normal) * 0.5 + 0.5)) *
                      max(0.2, shadow) * attenuation;
...
const char *vertexShader = "#version 130\n" STRINGIFY(
        ...
        uniform mat4 lightTransform; uniform vec3 lightDir; uniform float bias; uniform vec4 clipPlane;
...
const char *fragmentShader = STRINGIFY(
        ...
        uniform vec3 lightDir; uniform vec3 lightPos; uniform float spotMin; uniform float spotMax; uniform vec3 color;
        ...
        vec2 bump(vec2 x) { return (floor((x) / 2) + 2.f * max(((x) / 2) - floor((x) / 2) - .5f, 0.f)); }
        ...
        void main() {
            ...
            // direct light term
            float wrap = 0.0;
            vec3 diffuse = color * vec3(1.0, 1.0, 1.0) * max(0.0, (-dot(lightDir, n) + wrap) / (1.0 + wrap) * shadow) *
                           attenuation;
...
void PointRenderer::Draw(int n, int offset, float radius, float screenWidth, float screenAspect, float fov
    ...
    if (sprogram) {
        ...
        if (d != -1) glDisableVertexAttribArray(d);
        if (p != -1) glDisableVertexAttribArray(p);
