...
static inline_ void StartProfile_TimeGetTime(udword& val) { val = timeGetTime(); }

static inline_ void EndProfile_TimeGetTime(udword& val) { val = timeGetTime() - val; }

static inline_ void StartProfile_QPC() { mQPCTimer.getElapsedSeconds(); }

static inline_ void EndProfile_QPC(QPCTime::Second& val) { val = mQPCTimer.getElapsedSeconds(); }
...
static void NoProfileUpdate(EngineData& engine, float dt) { engine.mEngine->Update(dt); }
...
template <const bool combined, const bool post_update>
static inline_ udword
ProfileTestUpdate(ProfilingUnits profiling_units, PhysicsTest* test, EngineData& engine, float dt) {
    if (profiling_units == PROFILING_UNITS_RDTSC)
...
void Simulate(float timestep,
              ProfilingUnits profiling_units,
              UserProfilingMode user_profiling_mode,
              PhysicsTest* test,
              udword nb_engines,
              EngineData* engines,
    ...
    if (test) {
        ...
        if (IceCore::GetFocus() != GetWindowHandle()) test->LostFocus();
    ...
    if (user_profiling_mode == USER_PROFILING_SIM)
        ...
        if (PFlags & PROFILING_TEST_POST_UPDATE) NewFlags = PROFILING_TEST_POST_UPDATE;
    } else if (user_profiling_mode == USER_PROFILING_COMBINED) {
        NewFlags = PROFILING_SIM_UPDATE;
        if (PFlags & PROFILING_TEST_UPDATE) NewFlags |= PROFILING_TEST_UPDATE;
        if (PFlags & PROFILING_TEST_POST_UPDATE) NewFlags |= PROFILING_TEST_POST_UPDATE;
    }

    for (udword ii = 0; ii < nb_engines; ii++) {
        const udword i = P[ii];
        if (!engines[i].mEnabled || !engines[i].mSupportsCurrentTest) continue;
