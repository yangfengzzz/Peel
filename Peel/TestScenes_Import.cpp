...
public:
    ImportSceneFromFile() : mOBJ(null) {
        ...
        if (!gDragAndDropTest) gDragAndDropTest = this;
    }
    virtual ~ImportSceneFromFile() {}
    virtual const char* GetName() const { return "ImportSceneFromFile"; }
    virtual const char* GetDescription() const {
        if (mZB2Factory && mZB2Factory->mScene.mDesc.IsValid()) return mZB2Factory->mScene.mDesc;
    ...
    bool SetupLego(Pint& pint, const PintCaps& caps) {
        if (0) {
            ...
            if (NbParts) {
                if (!caps.mSupportMeshes) return false;
                ...
                for (udword i = 0; i < NbParts; i++) {
                    ...
                    PINT_MESH_CREATE MeshCreate;
                    MeshCreate.SetSurfaceData(Part->GetNbVerts(), Part->GetVerts(), Part->GetNbTris(),
                                              Part->GetIndices(), null);
                    MeshCreate.mRenderer = CreateMeshRenderer(MeshCreate.GetSurface());
            ...
            if (NbActors) {
                if (mImportMode == IMPORT_MODE_DYNAMIC_CONVEXES && !caps.mSupportConvexes) return false;

                if (mImportMode == IMPORT_MODE_STATIC_MESHES && !caps.mSupportMeshes) return false;
                ...
                for (udword i = 0; i < NbActors; i++) {
                    ...
                    if (mImportMode == IMPORT_MODE_STATIC_MESHES) {
                        //					MeshShapeCreate.mName				=
                        //Mesh->mStringID;
                        ////###
                        MeshShapeCreate.SetSurfaceData(Mesh->GetNbVerts(), Mesh->GetVerts(), Mesh->GetNbTris(),
                    ...
                    if (Mesh->mUserData && 1) {
                        ...
                        ShapeCreate->mRenderer =
                                CreateColorShapeRenderer(Renderer, Actor->mMaterial->mEffect->mDiffuse, MT);
                        ...
                        if (0) {
                            ...
                            ShapeCreate->mRenderer =
                                    CreateColorShapeRenderer(Renderer, Actor->mMaterial->mEffect->mDiffuse);
                        }
                        // #endif
                        // #ifdef USE_STATIC_BOXES
                        if (0) {
                        ...
                        if (1) {
                            ...
                            if (Mesh->GetNbTVerts()) {
                                MultiSurface MS;
                                MS.IndexedSurface::Init(Mesh->GetNbTris(), Mesh->GetNbVerts(), Mesh->GetVerts(),
                                                        (const IndexedTriangle*)Mesh->GetIndices());
                                {
                                    ...
                                    CopyMemory(TFaces, Mesh->GetTIndices(),
                                               Mesh->GetNbTTris() * sizeof(IndexedTriangle));
                                ...
                                //							Renderer =
                                // CreateMeshRenderer(ShapeCreate.mSurface, true);
                            }
                            //					PintShapeRenderer* Renderer	=
                            //CreateMeshRenderer(MeshCreate.mSurface, true); 							ShapeCreate.mRenderer		=
                            //CreateColorShapeRenderer(Renderer, Actor->mMaterial->mEffect->mDiffuse, &MT);
                            ShapeCreate->mRenderer =
                                    CreateColorShapeRenderer(Renderer, Actor->mMaterial->mEffect->mDiffuse, MT);
                        }
                    ...
                    ObjectDesc.mMass = 0.0f;
                    if (mImportMode == IMPORT_MODE_DYNAMIC_CONVEXES) ObjectDesc.mMass = 1.0f;
...
static LegoCarData gCarData[] = {
        {"/Lego/#Sunset Track Racer/SunsetTrackRacer.dae",
         PintCameraPose(Point(8.60f, 8.34f, 10.20f), Point(-0.57f, -0.53f, -0.63f))},
        {"/Lego/#Lumenairo�_728_Tempesta/Lumenairo�_728_Tempesta.dae",
         PintCameraPose(Point(19.58f, 6.45f, -0.37f), Point(-0.55f, -0.52f, -0.66f))},
        {"/Lego/#458 Italia GT2/458_Italia_GT2.dae",
         PintCameraPose(Point(8.87f, 7.31f, 11.71f), Point(-0.51f, -0.52f, -0.69f))},
        {"/Lego/#1968 Ford Mustang Fastback/1968_Ford_Mustang_Fastback.dae",
         PintCameraPose(Point(9.26f, 7.59f, 12.31f), Point(-0.68f, -0.39f, -0.61f))},
        {"/Lego/#1974 Porsche 911 Turbo 3.0/1974_Porsche_911_Turbo_3_0.dae",
         PintCameraPose(Point(9.50f, 8.03f, 12.35f), Point(-0.53f, -0.58f, -0.62f))},
        {"/Lego/#2016 Ford GT & 1966 Ford GT40/2016_Ford_GT_&_1966_Ford_GT40.dae",
         PintCameraPose(Point(-13.05f, 12.06f, 19.17f), Point(0.46f, -0.56f, -0.69f))},
        {"/Lego/#2018 Dodge Challenger SRT Demon and 1970 Dodge Charger R_T/Dodge.dae",
         PintCameraPose(Point(15.99f, 13.01f, 14.44f), Point(-0.62f, -0.49f, -0.62f))},
        {"/Lego/#Chevrolet Camaro ZL1 Race Car/Chevrolet_Camaro_ZL1_Race_Car.dae",
         PintCameraPose(Point(11.62f, 6.89f, 10.95f), Point(-0.64f, -0.41f, -0.65f))},
        {"/Lego/#Chevrolet Corvette Z06/Chevrolet Corvette Z06.dae",
         PintCameraPose(Point(1.02f, 9.29f, 13.38f), Point(0.10f, -0.55f, -0.83f))},
        {"/Lego/#Ferrari 488 GT3 Scuderia Corsa/Ferrari_488_GT3_Scuderia_Corsa.dae",
         PintCameraPose(Point(-9.81f, 5.93f, 10.89f), Point(0.44f, -0.38f, -0.81f))},
        {"/Lego/#Ferrari F40 Competizione/Ferrari_F40_Competizione.dae",
         PintCameraPose(Point(-13.64f, 11.89f, 8.37f), Point(0.61f, -0.70f, -0.36f))},
        {"/Lego/#Ford Fiesta M-Sport WRC - reoriented/Ford_Fiesta_M-Sport_WRC.dae",
         PintCameraPose(Point(-11.20f, 9.58f, 6.50f), Point(0.59f, -0.66f, -0.46f))},
        {"/Lego/#Ford Mustang/Ford_Mustang.dae",
         PintCameraPose(Point(-14.79f, 15.71f, 25.08f), Point(0.59f, -0.47f, -0.66f))},
        {"/Lego/#McLaren Senna/McLaren_Senna.dae",
         PintCameraPose(Point(8.52f, 8.68f, 14.01f), Point(-0.49f, -0.54f, -0.68f))},
        {"/Lego/#McLaren P1/McLaren_P1.dae",
         PintCameraPose(Point(8.54f, 11.91f, 11.41f), Point(-0.51f, -0.69f, -0.51f))},
        {"/Lego/#Porsche 911 GT Finish Line/Porsche_911_GT_Finish_Line.dae",
         PintCameraPose(Point(-34.30f, 14.58f, -33.83f), Point(0.76f, -0.31f, 0.58f))},
        {"/Lego/#Porsche 911 RSR and 911 Turbo 3.0/Porsche_911_RSR_and_911_Turbo_3_0.dae",
         PintCameraPose(Point(16.34f, 12.80f, -6.43f), Point(-0.75f, -0.53f, 0.38f))},
        {"/Lego/#Porsche 919 Hybrid and 917K Pit Lane/Porsche_919_Hybrid_and_917K_Pit_Lane.dae",
         PintCameraPose(Point(24.20f, 25.61f, 25.39f), Point(-0.56f, -0.65f, -0.51f))},
        {"/Lego/#Ferrari Ultimate Garage/Ferrari_Ultimate_Garage.dae",
         PintCameraPose(Point(-50.37f, 29.04f, 37.05f), Point(0.47f, -0.57f, -0.67f))},
        {"/Lego/#First Responder/First Responder.dae",
         PintCameraPose(Point(17.20f, 19.77f, 15.91f), Point(-0.64f, -0.56f, -0.52f))},
        {"/Lego/#Forklift Truck/Forklift_Truck.dae",
         PintCameraPose(Point(-8.66f, 19.99f, 20.55f), Point(0.48f, -0.54f, -0.69f))},
        {"/Lego/#Dune Buggy/Dune_Buggy.dae",
         PintCameraPose(Point(18.02f, 11.67f, -6.14f), Point(-0.86f, -0.36f, 0.37f))},
        {"/Lego/#Rally Car/Rally_Car.dae",
         PintCameraPose(Point(33.48f, 16.52f, 32.02f), Point(-0.72f, -0.32f, -0.62f))},
        {"/Lego/#Classic Go-Kart/Go-Kart.dae",
         PintCameraPose(Point(11.92f, 15.00f, 11.91f), Point(-0.55f, -0.66f, -0.51f))},
        ...
        {"/Lego/#Bucket Wheel Excavator/Bucket Wheel Excavator.dae",
         PintCameraPose(Point(13.91f, 8.72f, 11.96f), Point(-0.72f, -0.31f, -0.61f))},
};
...
public:
    ...
    virtual void GetSceneParams(PINT_WORLD_CREATE& desc) {
        ...
        if (mComboBox_Filename) Index = mComboBox_Filename->GetSelectedIndex();
