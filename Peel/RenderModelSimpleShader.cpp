...
IceWindow* RenderModel_SimpleShader::InitGUI(IceWidget* parent, Widgets* owner, PintGUIHelper& helper) { return null; }

bool RenderModel_SimpleShader::HasSpecialGroundPlane() const { return false; }
...
void RenderModel_SimpleShader::Render(
...
static inline_ udword GetMeshFlags(bool active_edges, bool vertex_normals, bool direct_data) {
    ...
    if (vertex_normals) Flags |= DL_MESH_USE_VERTEX_NORMALS;
    if (direct_data) Flags |= DL_MESH_USE_DIRECT_DATA;
    return Flags;
}

PintShapeRenderer* RenderModel_SimpleShader::_CreateMeshRenderer(const PintSurfaceInterface& surface,
                                                                 const Point* normals,
                                                                 bool active_edges,
                                                                 bool direct_data) {
...
PintShapeRenderer* RenderModel_SimpleShader::_CreateMeshRenderer(const MultiSurface& multi_surface,
                                                                 bool active_edges,
                                                                 bool direct_data) {
    return CreatePintDLMeshShapeRenderer(multi_surface, GetMeshFlags(active_edges, mNeedsVertexNormals, direct_data));
}

PintShapeRenderer* RenderModel_SimpleShader::_CreateColorShapeRenderer(PintShapeRenderer* renderer,
