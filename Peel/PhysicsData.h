...
public:
    ...
    virtual bool SetFrames(const PR* frame0, const PR* frame1) {
    ...
    virtual bool SetSpring(const PintSpring& spring) {
        ASSERT(!"not implemented");
        return false;
    }
...
inline_ bool SetPivots(Point& pivot0, Point& pivot1, const PR* frame0, const PR* frame1) {
    if (frame0) pivot0 = frame0->mPos;
    if (frame1) pivot1 = frame1->mPos;
    return true;
}

inline_ bool SetPivots(PR& pivot0, PR& pivot1, const PR* frame0, const PR* frame1) {
    if (frame0) pivot0 = *frame0;
    if (frame1) pivot1 = *frame1;
    return true;
}
...
public:
    ...
    virtual bool SetFrames(const PR* frame0, const PR* frame1) override {
        return SetPivots(mLocalPivot0, mLocalPivot1, frame0, frame1);
    }
...
public:
    ...
    virtual bool SetSpring(const PintSpring& spring) override {
        mSpring = spring;
        return true;
    }
...
public:
    ...
    virtual bool SetGearRatio(float ratio) override {
        mGearRatio = ratio;
        return true;
    }
