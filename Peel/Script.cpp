...
AutomatedTests* GetAutomatedTests() { return gAutomatedTests; }

void ReleaseAutomatedTests() { DELETESINGLE(gAutomatedTests); }
...
bool AutomatedTests::IsValid() const { return mDefaultNbFrames != 0 && mTests.GetNbEntries() != 0; }

AutomatedTest* AutomatedTests::GetCurrentTest() const {
    if (!mTests.GetNbEntries()) return null;
...
static PhysicsTest* FindTest(const char* name) {
    ...
    for (udword i = 0; i < NbTests; i++) {
        ...
        if (strcmp(CurrentTest->GetName(), name) == 0) return CurrentTest;
    }
    return null;
}
...
static void StartAutomatedTest(const AutomatedTest& test) {
    if (test.mSubScene) test.mTest->SetDefaultSubscene(test.mSubScene->Get());
    ...
    if (test.mTool != INVALID_ID) SelectTool(test.mTool);
}
...
void StartTest(const char* name) { gWantedTest = name; }
...
void UpdateAutomatedTests(udword frame_nb, bool menu_is_visible) {
    ...
    if (frame_nb > Limit || menu_is_visible || SoundPos > CurrentTest->mSoundPos) {
        if (AutoTests->mSaveExcelFile) TestCSVExport();
