...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportCapsuleSweeps || !mZB2Factory) return false;

    return CreateZB2Scene(pint, caps);
}

virtual udword Update(Pint& pint, float dt) { return DoBatchCapsuleSweeps(*this, pint); }
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportCapsuleSweeps) return false;

    const float Verts[] = {
            8,   0,    0,  8,   0,    -4, 8,   0.25, 0,  8,   0.25, -4, 0,   0,    -4, 0,   0,    0,  0,   0.25, -4,
            0,   0.25, 0,  0.5, 0,    0,  0.5, 0.75, 0,  1,   0,    0,  1,   0.25, 0,  0.5, 0.75, -4, 0.5, 0,    -4,
            1,   0.25, -4, 1,   0,    -4, 7,   0,    0,  7.5, 0,    0,  7.5, 0.75, 0,  7,   0.25, 0,  7.5, 0.75, -4,
            7,   0,    -4, 7,   0.25, -4, 7.5, 0,    -4, 1.5, 0,    0,  1.5, 0.75, 0,  2,   0,    0,  2,   0.25, 0,
            1.5, 0.75, -4, 1.5, 0,    -4, 2,   0.25, -4, 2,   0,    -4, 3,   0,    0,  3.5, 0,    0,  3.5, 0.75, 0,
            3,   0.25, 0,  4,   0,    0,  4,   0.25, 0,  3.5, 0.75, -4, 3,   0,    -4, 3,   0.25, -4, 3.5, 0,    -4,
            4,   0.25, -4, 4,   0,    -4, 5,   0,    0,  5.5, 0,    0,  5.5, 0.75, 0,  5,   0.25, 0,  6,   0,    0,
            6,   0.25, 0,  5.5, 0.75, -4, 5,   0,    -4, 5,   0.25, -4, 5.5, 0,    -4, 6,   0.25, -4, 6,   0,    -4,
            6.5, 0,    0,  6.5, 0.75, 0,  6.5, 0.75, -4, 6.5, 0,    -4, 4.5, 0,    0,  4.5, 0.75, 0,  4.5, 0.75, -4,
            4.5, 0,    -4, 2.5, 0,    0,  2.5, 0.75, 0,  2.5, 0.75, -4, 2.5, 0,    -4};
...
public:
    SceneSweepVsShapes()
        : mQueryShapeType(PINT_SHAPE_UNDEFINED),
          mSceneShapeType(PINT_SHAPE_UNDEFINED),
          mTypeData(4),
          mSize(32),
          mDynamic(false),
          mRotate(false) {}
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (mQueryShapeType == PINT_SHAPE_BOX && !caps.mSupportBoxSweeps) return false;
        if (mQueryShapeType == PINT_SHAPE_SPHERE && !caps.mSupportSphereSweeps) return false;
        if (mQueryShapeType == PINT_SHAPE_CAPSULE && !caps.mSupportCapsuleSweeps) return false;
    ...
    virtual void CommonUpdate(float dt) {
        ...
        if (mRotate) {
            if (mQueryShapeType == PINT_SHAPE_BOX) UpdateBoxSweeps(*this, mCurrentTime);
            //			if(mQueryShapeType==PINT_SHAPE_SPHERE)
            //				UpdateSphereSweeps(*this, mCurrentTime);
            if (mQueryShapeType == PINT_SHAPE_CAPSULE) UpdateCapsuleSweeps(*this, mCurrentTime);
        }
    }

    virtual udword Update(Pint& pint, float dt) {
        if (mQueryShapeType == PINT_SHAPE_BOX) return DoBatchBoxSweeps(*this, pint);
        if (mQueryShapeType == PINT_SHAPE_SPHERE) return DoBatchSphereSweeps(*this, pint);
        if (mQueryShapeType == PINT_SHAPE_CAPSULE) return DoBatchCapsuleSweeps(*this, pint);
        return 0;
    }
...
static const char* gDesc_SceneSweepsVsShapes =
        "A grid of shape-sweeps against a grid of static or dynamic shapes. Select undefined scene-shape to create an "
        "empty scene and measure the operating overhead of shape-sweeps.";
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        {
            mCheckBox_Dynamic =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Dynamic scene shapes", &UIElems, false, null, null);
            y += YStep;

            mCheckBox_Rotate =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Rotate swept shapes", &UIElems, true, null, null);
            y += YStep;
            ...
            mComboBox_QueryShapeType = CreateShapeTypeComboBox(UI, 4 + OffsetX, y, true,
                                                               SSM_UNDEFINED | SSM_SPHERE | SSM_CAPSULE | SSM_BOX);
            RegisterUIElement(mComboBox_QueryShapeType);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Scene shape:", &UIElems);
            mComboBox_SceneShapeType = CreateShapeTypeComboBox(
                    UI, 4 + OffsetX, y, true, SSM_UNDEFINED | SSM_SPHERE | SSM_CAPSULE | SSM_BOX | SSM_CONVEX);
    ...
    virtual bool CommonSetup() {
        ...
        if (mComboBox_QueryShapeType) mQueryShapeType = PintShape(mComboBox_QueryShapeType->GetSelectedIndex());

        if (mComboBox_SceneShapeType) mSceneShapeType = PintShape(mComboBox_SceneShapeType->GetSelectedIndex());

        if (mComboBox_ConvexIndex) mTypeData = ConvexIndex(mComboBox_ConvexIndex->GetSelectedIndex());
...
static PintShapeRenderer* CommonSetup_SceneConvexSweep(MyConvex& convex,
                                                       TestBase& test,
                                                       udword nb_x,
                                                       udword nb_y,
                                                       float altitude,
...
static const char* gDesc_SceneConvexSweepVsStaticConvexes =
        "A grid of convex-sweeps against a grid of static convexes.";
...
public:
    SceneConvexSweepVsStaticConvexes() : mRenderer(null), mGridSize(0) {
    ...
    virtual bool CommonSetup() {
        ...
        mRenderer = CommonSetup_SceneConvexSweep(mConvex, *this, Size, Size, Altitude, 50.0f, 50.0f, Dir, Offset,
                                                 gSQMaxDist, mTypeData);
        return mRenderer != null;
    }

    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        if (!caps.mSupportConvexSweeps) return false;
    ...
    virtual udword Update(Pint& pint, float dt) { return DoBatchConvexSweeps(*this, pint); }
} SceneConvexSweepVsStaticConvexes;

///////////////////////////////////////////////////////////////////////////////

static const char* gDesc_SweepAccuracy =
        "Tests the accuracy of linear sweep tests. Ideally all engines should return the same impact distance.";
...
static const char* gDesc_SweepAccuracy2 =
        "Tests the accuracy of linear sweep tests. Ideally all engines should return the same impact distance.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportMeshes || !caps.mSupportCapsuleSweeps) return false;
...
public:
    SceneLongSweepVsSeaOfStatics() : mQueryShapeType(PINT_SHAPE_UNDEFINED) {}
...
static const char* gDesc_SceneShapeSweepVsSingleTriangle =
        "One shape-sweep against a single triangle, various configurations.";
...
public:
    ConfigurableSceneSweepVsSingleTriangle()
        : mRenderer(null), mQueryShapeType(PINT_SHAPE_UNDEFINED), mTestMode(TEST_VERTICAL_SWEEP), mDoubleSided(false) {}
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 10;
        {
            ...
            {
                IceComboBox* CB =
                        CreateComboBox<IceComboBox>(UI, 0, 4 + OffsetX, y, 150, 20, "Configuration", &UIElems, null);
            ...
            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Query shape:", &UIElems);
            mComboBox_QueryShapeType = CreateShapeTypeComboBox(
                    UI, 4 + OffsetX, y, true, SSM_UNDEFINED | SSM_SPHERE | SSM_CAPSULE | SSM_BOX | SSM_CONVEX);
            RegisterUIElement(mComboBox_QueryShapeType);
            mComboBox_QueryShapeType->Select(PINT_SHAPE_CAPSULE);
            y += YStep;

            mCheckBox_DoubleSided =
                    helper.CreateCheckBox(UI, 0, 4, y, 400, 20, "Flip triangle winding", &UIElems, false, null, null);
            y += YStep;
        }
    ...
    virtual bool CommonSetup() {
        ...
        if (mComboBox_TestMode) mTestMode = TestMode(mComboBox_TestMode->GetSelectedIndex());
        ...
        if (mQueryShapeType == PINT_SHAPE_BOX) {
            ...
            // There's a design issue for convex sweeps. The sweeps now need data from each Pint (the convex mesh),
            // which was not the case before. Creating things in CommonSetup() doesn't work since there's no Pint
            // pointer. Creating things in Setup() doesn't work since we UnregisterAllConvexSweeps() and the previous
            // Pint-provided objects are lost. We end up calling all Pint plugins with the convex data from the last
            // Pint. Which happens to work just fine between PhysX 3.3 and 3.4, which is mental (we send a PxConvexMesh
            // created in the 3.3 plugin to the 3.4 SDK and things just work. Madness!)
            //
            // The workaround is to create things in CommonSetup while using the future convex indices created later in
            // Setup. For example here we use "0", and we expect "pint.CreateConvexObject()" to later return 0 for each
            // PINT plugin.
    ...
    virtual bool Setup(Pint& pint, const PintCaps& caps) {
        ...
        if (mQueryShapeType == PINT_SHAPE_BOX) return caps.mSupportBoxSweeps;

        if (mQueryShapeType == PINT_SHAPE_SPHERE) return caps.mSupportSphereSweeps;

        if (mQueryShapeType == PINT_SHAPE_CAPSULE) return caps.mSupportCapsuleSweeps;
    ...
    virtual void CommonUpdate(float dt) {
        ...
        //		mCurrentTime = 20.0f/60.0f;		// Enable this for the static version corresponding to
        //the debug render code
        if (mQueryShapeType == PINT_SHAPE_BOX) UpdateBoxSweeps(*this, mCurrentTime);
        //		else if(mQueryShapeType==PINT_SHAPE_SPHERE)
        //			UpdateSphereSweeps(*this, mCurrentTime);
        else if (mQueryShapeType == PINT_SHAPE_CAPSULE)
    ...
    virtual udword Update(Pint& pint, float dt) {
        ...
        if (mQueryShapeType == PINT_SHAPE_SPHERE) return DoBatchSphereSweeps(*this, pint);
        if (mQueryShapeType == PINT_SHAPE_CAPSULE) return DoBatchCapsuleSweeps(*this, pint);
        if (mQueryShapeType == PINT_SHAPE_CONVEX) return DoBatchConvexSweeps(*this, pint);
        return 0;
    }
    ...
    virtual void CommonDebugRender(PintRender& renderer) {
        ...
        if (mDoubleSided)
            //			renderer.DrawTriangle(mTriangle.mVerts[0], mTriangle.mVerts[1], mTriangle.mVerts[2],
            // Color);
...
static const char* gDesc_SceneBoxSweepVsStaticMeshes_Archipelago =
        "32*32 box-sweeps against the Archipelago mesh level.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportBoxSweeps) return false;
...
virtual udword Update(Pint& pint, float dt) { return DoBatchBoxSweeps(*this, pint); }
...
static bool CommonSetup_SceneBoxSweepVsStaticMeshes_TessBunny(TestBase& test,
                                                              const Point& extents,
                                                              udword nb_sweeps,
                                                              const Matrix3x3& rot) {
...
static bool Setup_SceneBoxSweepVsStaticMeshes_TessBunny(
        TestBase& test,
        Pint& pint,
        const PintCaps& caps /*, const Point& extents, udword nb_sweeps, const Matrix3x3& rot*/) {
    if (!caps.mSupportBoxSweeps) return false;

    return test.CreateMeshesFromRegisteredSurfaces(pint, caps);
}

static const char* gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test1 =
        "64 radial box-sweeps against the tessellated bunny. Each box is a (1, 1, 1) cube. Box orientation is "
        "identity.";

START_SQ_TEST(SceneBoxSweepVsStaticMeshes_TessBunny_Test1,
              CATEGORY_SWEEP,
              gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test1)
...
static const char* gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test1b =
        "4096 radial box-sweeps against the tessellated bunny. Each box is a (1, 1, 1) cube. Box orientation is "
        "identity.";

START_SQ_TEST(SceneBoxSweepVsStaticMeshes_TessBunny_Test1b,
              CATEGORY_SWEEP,
              gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test1b)
...
static const char* gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test2 =
        "256 radial box-sweeps against the tessellated bunny. Each box is a (1, 1, 1) cube. Box orientation is "
        "non-identity.";

START_SQ_TEST(SceneBoxSweepVsStaticMeshes_TessBunny_Test2,
              CATEGORY_SWEEP,
              gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test2)
...
static const char* gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test3 =
        "A single radial box-sweep against the tessellated bunny. Box is a (1, 1, 1) cube. Box orientation is "
        "non-identity.";

START_SQ_TEST(SceneBoxSweepVsStaticMeshes_TessBunny_Test3,
              CATEGORY_SWEEP,
              gDesc_SceneBoxSweepVsStaticMeshes_TessBunny_Test3)
...
static const char* gDesc_SceneBoxSweepVsStaticMeshes_TessBunny2_Test4 =
        "256 radial box-sweeps against the tessellated bunny. Each box is a (0.1, 0.1, 0.1) cube. Box orientation is "
        "identity.";

START_SQ_TEST(SceneBoxSweepVsStaticMeshes_TessBunny_Test4,
              CATEGORY_SWEEP,
              gDesc_SceneBoxSweepVsStaticMeshes_TessBunny2_Test4)
...
static const char* gDesc_SceneSphereSweepVsStaticMeshes_Archipelago =
        "32*32 sphere-sweeps against the Archipelago mesh level.";

START_SQ_TEST(SceneSphereSweepVsStaticMeshes_Archipelago,
              CATEGORY_SWEEP,
              gDesc_SceneSphereSweepVsStaticMeshes_Archipelago)
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!caps.mSupportSphereSweeps) return false;
...
virtual udword Update(Pint& pint, float dt) { return DoBatchSphereSweeps(*this, pint); }
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) override {
...

        {
            ...
            mEditBox_SweepStartDistance = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "20", &UIElems,
                                                               EDITBOX_FLOAT_POSITIVE, null, null);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Sphere radius:", &UIElems);
            mEditBox_SphereRadius =
                    helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, _F("%.2f", mCurrentSphereRadius),
    ...
    void UpdateSphereSweeps() {
        ...
        if (StartDistance == mCurrentStartDistance && SphereRadius == mCurrentSphereRadius) return;
        mCurrentStartDistance = StartDistance;
...
static const char* gDesc_SceneSphereSweepVsStaticMeshes_TessBunny =
        "64 radial sphere-sweeps against the tessellated bunny. Radius of the spheres is 1.0.";
...
public:
    ...
    virtual bool CommonSetup() override { return CommonSetupBase(1.0f); }
    ...
    static const char* gDesc_SceneSphereSweepVsStaticMeshes_TessBunny2 =
            "64 radial sphere-sweeps against the tessellated bunny. Radius of the spheres is 0.1.";
    ...
    public:
        ...
        virtual bool CommonSetup() override { return CommonSetupBase(0.1f); }
        ...
        static const char* gDesc_SceneCapsuleSweepVsStaticMeshes_Archipelago =
                "32*32 capsule-sweeps against the Archipelago mesh level.";

        START_SQ_TEST(SceneCapsuleSweepVsStaticMeshes_Archipelago,
                      CATEGORY_SWEEP,
                      gDesc_SceneCapsuleSweepVsStaticMeshes_Archipelago)
        ...
        virtual bool CommonSetup() {
            ...
            const float CapsuleRadius = 0.4f;
            const float HalfHeight = 1.8f;
            RegisterArrayOfCapsuleSweeps(*this, NbX, NbY, Altitude, ScaleX, ScaleY, Dir, CapsuleRadius, HalfHeight,
                                         Offset, gSQMaxDist);
        ...
        START_SQ_TEST(SceneCapsuleSweepVsStaticMeshes_Archipelago2,
                      CATEGORY_SWEEP,
                      gDesc_SceneCapsuleSweepVsStaticMeshes_Archipelago2)
        ...
        static const char* gDesc_SceneCapsuleSweepVsStaticMeshes_KP =
                "32*32 capsule-sweeps against the Konoko Payne mesh level.";
        ...
        static const char* gDesc_SceneCapsuleSweepVsStaticMeshes_TessBunny =
                "64 radial capsule-sweeps against the tessellated bunny.";

        START_SQ_TEST(SceneCapsuleSweepVsStaticMeshes_TessBunny,
                      CATEGORY_SWEEP,
                      gDesc_SceneCapsuleSweepVsStaticMeshes_TessBunny)
        ...
        virtual bool CommonSetup() {
            ...
            //			if(i==0)	// Very interesting one, gives very different hit results for each
            engine
            //			if(i==1)	// Clear issue with bv4, impact is way too early
            //			if(i==2)	// Gives a hit in bv4 (probably wrong), no hit for other engines... clear bug
            => now fixed RegisterCapsuleSweep(Capsule, -Dir, 5000.0f);
                            }*/
        ...
        static const char* gDesc_SceneCapsuleSweepVsStaticMeshes_TessBunny2 =
                "64 radial capsule-sweeps against the tessellated bunny.";

        START_SQ_TEST(SceneCapsuleSweepVsStaticMeshes_TessBunny2,
                      CATEGORY_SWEEP,
                      gDesc_SceneCapsuleSweepVsStaticMeshes_TessBunny2)
        ...
        static const char* gDesc_SceneCapsuleSweepVsStaticMeshes_TessBunny3 =
                "64 radial capsule-sweeps against the tessellated bunny.";

        START_SQ_TEST(SceneCapsuleSweepVsStaticMeshes_TessBunny3,
                      CATEGORY_SWEEP,
                      gDesc_SceneCapsuleSweepVsStaticMeshes_TessBunny3)
        ...
        static PintShapeRenderer* CommonSetup_SceneConvexSweepVsStaticMeshes_TessBunny(MyConvex& convex,
                                                                                       TestBase& test) {
            GLRenderStates::SetDefaultCullMode(CULL_NONE);
        ...
        static bool Setup_SceneConvexSweepVsStaticMeshes_TessBunny(
                PintShapeRenderer* /*renderer*/, MyConvex& convex, TestBase& test, Pint& pint, const PintCaps& caps) {
            if (!caps.mSupportMeshes || !caps.mSupportConvexSweeps) return false;
        ...
        static const char* gDesc_SceneConvexSweepVsStaticMeshes_TessBunny =
                "64 radial convex-sweeps against the tessellated bunny.";

        START_SQ_TEST(SceneConvexSweepVsStaticMeshes_TessBunny,
                      CATEGORY_SWEEP,
                      gDesc_SceneConvexSweepVsStaticMeshes_TessBunny)
        ...
        virtual bool CommonSetup() {
            ...
            if (1) {
                ...
                /*			for(udword i=0;i<1024;i++)
                                        {
                                                ...
                                                Q.Identity();
                                                RegisterBoxSweep(OBB(Point(x, 50.0f, z), Point(1.0f, 1.0f, 1.0f), Q), D,
                1000.0f);
                //				RegisterBoxSweep(OBB(Point(x, 50.0f, z), Point(1.0f, 1.0f, 1.0f), Q),
                Point(0.0f, -1.0f, 0.0f), 1000.0f);
                                        }*/
                ...
                for (udword i = 0; i < Nb; i++) {
                    ...
                    //				UnitRandomQuat(Q, Rnd);
                    Q.Identity();
                    //				RegisterBoxSweep(OBB(Point(Center.x, 2.0f, Center.z), Point(0.1f, 0.1f, 0.1f), Q),
                    //D, 1000.0f); 				RegisterBoxSweep(OBB(Point(8.36f, -4.22f, -28.18f),
                    // Point(0.1f, 0.1f, 0.1f), Q), D, 1000.0f);
                    RegisterBoxSweep(OBB(Point(8.36f, -4.22f, -28.18f), Point(0.5f, 0.5f, 0.5f), Q), D, 1000.0f);
                }
            } else {
                if (!GetNbRegisteredSurfaces()) return false;
                ...
                for (udword i = 0; i < IS->GetNbFaces(); i++) {
                    ...
                    //			RegisterBoxSweep(OBB(Center+Normal*2.0f, Point(0.1f, 0.1f, 0.1f), Q),
                    //-Normal, 10.0f);
                }
            }
            return true;
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            if (!caps.mSupportRaycasts || !caps.mSupportMeshes || !caps.mSupportBoxSweeps) return false;
        ...
        virtual bool Setup(Pint& pint, const PintCaps& caps) {
            if (!caps.mSupportMeshes || !caps.mSupportBoxSweeps) return false;
            return AddToPint(pint, mRepX);
        }
...
#include "TestScenes_Heightfield.h"
        ...
        public:
            SceneSweepVsStaticHeightfield(udword scenario)
                : HeightfieldTest(64, 32), mRenderer(null), mScenario(scenario) {}
            ...
            virtual bool CommonSetup() {
                ...
                if (mScenario == 0) {
                    const Point BoxExtents(1.2f, 0.5f, 0.5f);
                    RegisterArrayOfBoxSweeps(*this, NbX, NbY, Altitude, ScaleX, ScaleY, Dir, BoxExtents, Offset,
                                             gSQMaxDist);
                } else if (mScenario == 1) {
                    RegisterArrayOfSphereSweeps(*this, NbX, NbY, Altitude, ScaleX, ScaleY, Dir, 0.5f, Offset,
                                                gSQMaxDist);
                } else if (mScenario == 2) {
                    const float CapsuleRadius = 0.4f;
                    const float HalfHeight = 1.8f;
                    RegisterArrayOfCapsuleSweeps(*this, NbX, NbY, Altitude, ScaleX, ScaleY, Dir, CapsuleRadius,
                                                 HalfHeight, Offset, gSQMaxDist);
                } else if (mScenario == 3) {
                    BasicRandom Rnd(42);
            ...
            virtual void CommonUpdate(float dt) {
                TestBase::CommonUpdate(dt);
                if (mScenario == 0) UpdateBoxSweeps(*this, mCurrentTime);
                //		else if(mScenario==1)
                //			UpdateSphereSweeps(*this, mCurrentTime);
                else if (mScenario == 2)
                    UpdateCapsuleSweeps(*this, mCurrentTime);
        ...
        public:
            ...
            public:
                ...
                public:
                    ...
                    static const char* gDesc_SceneConvexSweepVsStaticHeightfield =
                            "convex-sweeps against a heightfield.";
