...
bool DeformManager::Update(const PintVehicleData& vehicle,
                           TerrainRegionManager& region_manager,
                           float wheel_radius,
                           float dt) {
    SPY_ZONE("DeformManager::Update")

    Pint_Shape* ShapeAPI = mOwner.GetShapeAPI();
    if (!ShapeAPI) return false;
    ...
    const udword Nb = mOwner.BatchSphereOverlapObjects(mOwner.mSQHelper->GetThreadContext(), 4, Dest,
                                                       mOwner.mSQHelper->GetSphereOverlapObjectsStream(), Overlaps);
    // printf("Nb: %d\n", Nb);
    if (gDebugDrawTouchedShapes)  // Abuse selection code to draw touched actors
    {
        ...
        const PintOverlapHit* AllHits =
                (const PintOverlapHit*)mOwner.mSQHelper->GetSphereOverlapObjectsStream().GetEntries();  // ####
        for (udword i = 0; i < Nb; i++) DCI.mSelMan.AddToSelection(&mOwner, AllHits[i].mTouchedActor, 0);
    }
    ...
    if (1) {
        ...
        // Iterate over touched objects, fetch region for each of them, then store them in local hash. This is clumsy
        // because:
        // - the actor/shape/region are already stored in a hashmap, in region_manager, so we could just access that one
        // directly.
        // - but the map in region_manager contains all regions and we don't want to further iterate over all of them,
        // we want to iterate over touched ones.
        // - we want to store touched regions only once, i.e. when crossing a region boundary two wheel overlaps could
        // return the same (actor/shape) and we don't want to see them appear twice in the array we're going to iterate
        // over for deformation. Hence the local hashmap.
        ...
        for (udword i = 0; i < Nb; i++) {
            ...
            if (RD) {
                // We only want to keep the terrain tiles that can be deformed. In another context where the vehicle
                // would have to push away dynamic obstacles we could keep more actors here.
        ...
        for (udword i = 0; i < 4; i++) {
            ...
            for (ps::HashMap<ActorShape, void*>::Iterator iter = mObjects.getIterator(); !iter.done(); ++iter) {
                ...
                if (ShapeAPI->GetTriangleMeshData(SI, TerrainShape, true)) {
                    ...
                    if (ShapeAPI->FindTouchedTriangles(TouchedTriangleIndices, mOwner.mSQHelper->GetThreadContext(),
                                                       TerrainShape, TerrainPose, Overlaps[i])) {
                        ...
                        {
                            TerrainRegionManager::RegionData* RD =
                                    reinterpret_cast<TerrainRegionManager::RegionData*>(iter->second);
                            ...
                            while (NbVerts--) {
                                ...
                                if (D < MaxDeform) {
                                    // D += 0.02f;
                                    D += 0.04f;
                                    if (1) V[Index].y = RD->mVertices[Index].y - D;
