...
DefaultControlInterface& GetDefaultControlInterface() { return *gDefaultControlInterface; }

void SetDefaultControlInterface() { SetControlInterface(gDefaultControlInterface); }
...
HWND GetWindowHandle() { return gWindowHandle; }
...
// static	float				gRTDistance = MAX_FLOAT;	// Test how different engines react to "infinite"
// rays
static float gRTDistance = 5000.0f;
float GetPickingDistance() { return gRTDistance; }
...
static bool SelectRenderModel(udword index) {
    if (index >= RENDER_MODEL_COUNT) return false;
    gRenderModelIndex = RenderModelType(index);
    for (udword i = 0; i < RENDER_MODEL_COUNT; i++) {
        if (gRenderModelOptions[i]) gRenderModelOptions[i]->SetVisible(i == index);
    }

    if (gCurrentRenderModel) gCurrentRenderModel->Close();
    gCurrentRenderModel = gRenderModel[index];
    const bool status = gCurrentRenderModel->Init();
    // printf("Render model status: %d\n", status);

    if (gRunningTest && !gMenuIsVisible) gRunningTest->mMustResetTest = true;
    return status;
}
...
static void CloseRenderModels() {
    for (udword i = 0; i < RENDER_MODEL_COUNT; i++) DELETESINGLE(gRenderModel[i]);
}
...
static inline_ ScreenQuadMode* GetCurrentScreenQuadMode() { return gScreenQuadModes[gScreenQuadModeIndex]; }

static void SelectScreenQuadMode(udword index) {
    if (index >= SCREEN_QUAD_COUNT) return;
    gScreenQuadModeIndex = ScreenQuadType(index);
    for (udword i = 0; i < SCREEN_QUAD_COUNT; i++) {
        if (gScreenQuadOptions[i]) gScreenQuadOptions[i]->SetVisible(i == index);
    }
}

static void InitScreenQuadModes() {
    gScreenQuadModes[SCREEN_QUAD_BACK_COLOR] = ICE_NEW(ScreenQuadBackColor)(gMenuClearColor);
    gScreenQuadModes[SCREEN_QUAD_GRADIENT] =
            ICE_NEW(ScreenQuadColorGradient)(gScreenQuadTab_TopColor, gScreenQuadTab_BottomColor);
...
static void CloseScreenQuadModes() {
    for (udword i = 0; i < SCREEN_QUAD_COUNT; i++) DELETESINGLE(gScreenQuadModes[i]);
}
...
#ifdef PEEL_USE_MSAA
    if (g_msaaSamples) glBindFramebuffer(GL_DRAW_FRAMEBUFFER_EXT, g_msaaFbo);
#endif
}
...
static void EndFrame_MSAA() {
    ...
    if (g_msaaFbo && g_msaaSamples) {
        ...
        glBlitFramebuffer(0, 0, gScreenWidth, gScreenHeight, 0, 0, gScreenWidth, gScreenHeight, GL_COLOR_BUFFER_BIT,
                          GL_LINEAR);
    }
#endif
}
...
const char* GetFilenameForExport(const char* extension, const char* target) {
    ...
    if (!gRunningTest) return _F("%sexported.%s", Path.Get(), extension);
...
struct CursorKeysState {
    ...
    void Reset() { mUp = mDown = mLeft = mRight = false; }
    bool mUp;
    bool mDown;
    bool mLeft;
    bool mRight;
...
const char* GetRoot() { return gRoot ? gRoot->Get() : null; }

static CameraData gCamera;

CameraData& GetCamera() { return gCamera; }
...
static bool InitAll(PhysicsTest* test) {
    ...
    if (test) {
        ...
        if (test != PreviousTest) {
            if (PreviousTest) {
                for (udword i = 0; i < gNbPlugIns; i++) gPlugIns[i]->CloseTestGUI();
            ...
            if (TestTabControl) {
                ...
                for (udword i = 0; i < gNbPlugIns; i++) {
                    ...
                    if (Main) TestTabControl->Add(Main, gPlugIns[i]->GetTestGUIName());
                }
            }
        ...
        if (GravityFromUI != Desc.mGravity) UI_SetGravity(Desc.mGravity);
        ...
        if (!MustResetCamera) {
            CameraData Tmp;
            Tmp.Init(Desc);
            if (!(Tmp == gCamera)) MustResetCamera = true;
        }
    ...
    for (udword i = 0; i < gNbPlugIns; i++) gPlugIns[i]->Init(Desc);
...
static void ResetToolAndInterfaces() {
    ...
    if (CurrentTool) CurrentTool->Reset(INVALID_ID);
    gState.Reset();
...
static void CloseAll() {
    ...
    for (udword i = 0; i < gNbPlugIns; i++) gPlugIns[i]->Close();
...
static void RecordCameraPose() {
    if (!gCameraData) gCameraData = ICE_NEW(Vertices);
...
static void CopyCameraPoseToClipboard() {
    const Point CamPos = GetCameraPos();
    const Point CamDir = GetCameraDir();
    const char* Text = _F("desc.mCamera[0] = PintCameraPose(Point(%.2ff, %.2ff, %.2ff), Point(%.2ff, %.2ff, %.2ff));",
...
static void KeyboardCallback(unsigned char key, int x, int y, bool down) {
    ...
    if (!gMenuIsVisible) KeyHasBeenHandled = gRunningTest->KeyboardCallback(key, x, y, down);

    ToolInterface* CurrentTool = GetCurrentTool();
    if (CurrentTool) {
        FOR_EACH_ENGINE(i) {
            if (!gEngines[i].mEnabled || !gEngines[i].mSupportsCurrentTest) continue;

            CurrentTool->KeyboardCallback(*gEngines[i].mEngine, i, key, down);
        }
    }

    if (!down || KeyHasBeenHandled) return;
    ...
    switch (key) {
        ...
        case 'U':
            ...
            if (!gAutoCameraMove) SaveCameraData();
            break;
            /*		case 'u':
                            case 'U':
                                    RecordCameraPose();
...
static void SpecialKeyCallback(int key, int x, int y, bool down) {
    ...
    if (down) {
        ...
        FOR_EACH_ENGINE(i) {
            if (key == i + 1) {
                ...
                if (CurrentTool) CurrentTool->Reset(i);
            }
        }
    }
    ...
    if (!gMenuIsVisible) {
        ...
        if (down) gCurrentTest = TestSelectionKeyboardCallback(key, gCurrentTest);
    }
}
...
static void PrintTimings() {
            ...
            if (gEngines[i].mEnabled) {
                if (gEngines[i].mSupportsCurrentTest) {
                    ...
                    if (Timing.mCurrentTestResult == INVALID_ID) {
                        gTexter.print(0.0f, y, TextScale,
                                      _F("%s: %d (Avg: %d)(Worst: %d)(%d Kb)\n", Engine->GetName(), Timing.mCurrentTime,
                                         Timing.GetAvgTime(), Timing.mWorstTime, Timing.mCurrentMemory / 1024));
                    } else {
                        gTexter.print(0.0f, y, TextScale,
                                      _F("%s: %d (Avg: %d)(Worst: %d)(%d Kb)(Val: %d)\n", Engine->GetName(),
                                         Timing.mCurrentTime, Timing.GetAvgTime(), Timing.mWorstTime,
                                         Timing.mCurrentMemory / 1024, Timing.mCurrentTestResult));
                    }
                } else {
    ...
    gTexter.setColor(1.0f, 1.0f, 1.0f, 1.0f);
    if (gShowInfos) {
        if (gPaused) gTexter.print(0.0f, y, TextScale, "(Paused)");
        y -= TextScale;

        if (gShowFPS) gTexter.print(0.0f, y, TextScale, _F("Frame: %d", gFrameNb));
        y -= TextScale;

        if (gShowFPS) gTexter.print(0.0f, y, TextScale, _F("FPS: %.02f", gFPS.GetFPS()));
        y -= TextScale * 2.0f;
    }
...
void SetFocusActor(PintActorHandle h) { gPintRender->mFocusActor = h ? Editor_GetNativeHandle(h) : null; }
...
void DrawScene(PintRenderPass render_pass) {
    ...
    FOR_EACH_ENGINE(i) {
        if (gEngines[i].mEnabled && gEngines[i].mSupportsCurrentTest) {
            ...
            if (gRunningTest) gRunningTest->CommonRender(*gPintRender, render_pass);
...
float ComputeConstantScale(const Point& pos, const Matrix4x4& view, const Matrix4x4& proj) {
    ...
    const float l1 =
            1.0f / (ppcam0.x * proj.m[0][3] + ppcam0.y * proj.m[1][3] + ppcam0.z * proj.m[2][3] + proj.m[3][3]);
    const float c1 = (ppcam0.x * proj.m[0][0] + ppcam0.y * proj.m[1][0] + ppcam0.z * proj.m[2][0] + proj.m[3][0]) * l1;
    const float l2 =
            1.0f / (ppcam1.x * proj.m[0][3] + ppcam1.y * proj.m[1][3] + ppcam1.z * proj.m[2][3] + proj.m[3][3]);
...
/*static*/ void RenderWireframeOverlay(/*PintRenderPass render_pass*/) {
    ...
    if (!gWireframeOverlayEnabled) return;
    ...
    FOR_EACH_ENGINE(i) {
        if (gEngines[i].mEnabled && gEngines[i].mSupportsCurrentTest) {
            ASSERT(gEngines[i].mEngine);
            Pint* Engine = gEngines[i].mEngine;
            gPintRender->StartRender(gEngines[i].mEngine, &gDefaultControlInterface->mSelMan,
                                     PINT_RENDER_PASS_WIREFRAME_OVERLAY);
    ...
    /*	FOR_EACH_ENGINE(i)
            {
                    if(gEngines[i].mEnabled && gEngines[i].mSupportsCurrentTest)
                    {
                            ...
                            Pint* Engine = gEngines[i].mEngine;
                            gPintRender->StartRender(gEngines[i].mEngine, &gDefaultControlInterface.mSelMan,
       PINT_RENDER_PASS_WIREFRAME_OVERLAY); Engine->Render(*gPintRender, PINT_RENDER_PASS_WIREFRAME_OVERLAY);
                            gPintRender->EndRender();
                    }
            }*/
            ...
            if (gEngines[i].mEnabled) {
                ...
                if (gRaytracingWindows[i]) {
                    ...
                    if (gRaytracingTest == RAYTRACING_TEST_ST)
                        NbHits = RaytracingTest(Target, *Engine, TotalTime, gScreenWidth, gScreenHeight, NbRays,
                                                gRTDistance);
                    else  // if(gRaytracingTest==RAYTRACING_TEST_MT)
                        NbHits = RaytracingTestMT(Target, *Engine, TotalTime, gScreenWidth, gScreenHeight, NbRays,
                                                  gRTDistance);
                ...
                /*				else
                                                {
                                                        udword TotalTime;
                                                        udword NbHits = RaytracingTest(*Engine, TotalTime, gScreenWidth,
                   gScreenHeight); printf("%s: %d hits, time = %d\n", Engine->GetName(), NbHits, TotalTime/1024);
                                                }*/
            }
        }
        {
...
static void RenderCallback() {
    ...
    if (!gScreenWidth || !gScreenHeight) return;
...
#ifdef PEEL_USE_SPY
    {
        ...
        if (gDisplayMessageDelay > 0.0f) {
            ...
            if (gDisplayMessageDelay < 0.0f) gDisplayMessageDelay = 0.0f;
        }
        ...
        if (!gMenuIsVisible) {
            ...
            for (udword i = 0; i < gNbSimulateCallsPerFrame; i++)
                Simulate(gTimestep, gProfilingUnits, gUserProfilingMode, gRunningTest, gNbEngines, gEngines,
                         gRandomizeOrder, gPaused, gTrashCache);
        }
...

        {
            ...
            if (gState.mUp) MoveCameraForward(Delta);
            if (gState.mDown) MoveCameraBackward(Delta);
            if (gState.mLeft) MoveCameraRight(Delta);
            if (gState.mRight) MoveCameraLeft(Delta);
        }
        ...
        if (gMenuIsVisible) {
            ...
            if (SQM) SQM->Apply(gScreenWidth, gScreenHeight);
        }
        ...
        if (gCurrentRenderModel) gCurrentRenderModel->SetupCamera();
            ...
            if (gCurrentRenderModel) {
                {
                    ...
                    if (gRunningTest) RenderSize = gRunningTest->GetRenderData(RenderCenter);
...

                {
                    ...
                    gCurrentRenderModel->Render(gScreenWidth, gScreenHeight, GetCameraPos(), GetCameraDir(),
                                                GetCameraFOV());
                    //				SimpleViewer::Render(gCallbacks, gScreenWidth, gScreenHeight, GetCameraPos(),
                    //GetCameraDir(), 60.0f);
                }
            }
        ...
        if (gFreezeFrustum && !gMenuIsVisible) DrawCapturedFrustum();
        ...
        if (!gMenuIsVisible && 0) {
            ...
            GLRenderHelpers::DrawRectangle(0.0f, 1.0f, 0.0f, 1.0f, color, color, Alpha, gScreenWidth, gScreenHeight,
                                           false, GLRenderHelpers::SQT_TEXTURING);
        ...
        if (!gMenuIsVisible && gRoundCorners) DrawRoundCorners(gScreenWidth, gScreenHeight, gRoundCornersSize);

        {
            ToolInterface* CurrentTool = GetCurrentTool();
            if (CurrentTool) CurrentTool->FinalRenderCallback();
        }

        if (0) DisplayTexture(gScreenWidth, gScreenHeight);
                ...
                if (gEngines[i].mEnabled) {
                    ...
                    if (gRaytracingWindows[i]) {
                        ...
                        if (Target.GetPixels()) {
                            ...
                            if (!gTexID)
                                gTexID = GLTexture::CreateTexture(CurrentWidth, CurrentHeight, Target.GetPixels(),
                                                                  false);
                            else
                                GLTexture::UpdateTexture(gTexID, CurrentWidth, CurrentHeight, Target.GetPixels(),
                                                         false);
        ...
        if (gMenuIsVisible) {
            ...
            if (gHelpIsVisible) {
                gTexter.print(x, y, TextScale, "Press H to Hide help");
                y -= TextScale;
                y -= TextScale;
                ...
                gTexter.print(x, y, HelpTextScale, "In menu mode (this screen):");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale,
                              "Press arrow up/down to select a test, left/right to skip a category");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press Return to run selected test (and enter 'simulation mode')");
                y -= HelpTextScale;

                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "In simulation mode:");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press Return to go back to the menu mode");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press arrow keys to move the camera");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press PAGEUP/PAGEDOWN or +/- (on the keypad) to switch cameras");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press C to reset the Camera");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press O to step One frame");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press R to disable Rendering");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press W to enable Wireframe mode");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press X to enable Wireframe Overlay mode");
                y -= HelpTextScale;
                //			gTexter.print(x, y, HelpTextScale, "Use the mouse and left mouse button to move the
                //camera");				y -= HelpTextScale; 			gTexter.print(x, y, HelpTextScale, "Use the
                //mouse and right mouse button for picking");						y -=
                //HelpTextScale;

                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "At any time:");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press F1->Fx to enable/disable engines");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press P to enable/disable Pause");
                y -= HelpTextScale;
                gTexter.print(x, y, HelpTextScale, "Press S to Save results as an Excel file");
                y -= HelpTextScale;
            } else {
            ...
            if (gCandidateTest) {
                ...
                GLRenderHelpers::DrawRectangle(XOffset, 1.0f - XOffset, ystart, ystart - PanelSize, BackColor,
                                               Point(0.0f, 0.0f, 0.0f), 0.5f, gScreenWidth, gScreenHeight, true,
                                               GLRenderHelpers::SQT_DISABLED);
            ...
            if (gRectSelX0 != gRectSelX1 && gRectSelY0 != gRectSelY1 && gScreenWidth != 0.0f && gScreenHeight != 0.0f) {
                ...
                GLRenderHelpers::DrawRectangle(x0, x1, y0, y1, Color, Color, Alpha, gScreenWidth, gScreenHeight, true,
                                               GLRenderHelpers::SQT_DISABLED);
            }
        }
        ...
        if (!gBlitToScreen) RunRaytractingTest();

        if (gOneFrame) gPaused = true;
...
void MoveTestSelection(udword key, udword nb) {
    for (udword i = 0; i < nb; i++) gCurrentTest = TestSelectionKeyboardCallback(key, gCurrentTest);
}
...
static void gCleanup() {
    ...
    timeBeginPeriod(1);
    printf("Exiting...\n");
    if (gRunningTest) gRunningTest->CloseUI();
    CloseAll();
...
static char* gFolders[] = {"../Media/%s", "../Media/#Private/Customers/%s", "../Media/#Private/Debug/%s",
                           "../Media/#Scripts/%s", "../Media/#Shaders/%s", "../Media/#Sounds/%s",
                           "../Media/#VehicleData/%s", "../Media/#Vehicles/%s", "../Media/NutsAndBolts/%s",
                           "../Media/Zcb/%s", "../Excel/%s",
                           //	"../build/%s",
                           //	"../build/Customers/%s",
                           //	"../build/Data/%s",
                           ...
                           //	"./Data/%s",
                           //	"../Data/%s",
                           "%s", "../Private/Bruneton/%s", null};

static String gDropFile;

void SetDropFile(const char* name) { gDropFile = name; }
...
const char* FindPEELFile(const char* filename) {
    ...
    do {
        ...
        if (FileExists(gCurrentFile)) return gCurrentFile;
        i++;
    } while (Candidate);
...
/*
    ...
    RI_MOUSE_WHEEL
    wheelDelta, applicable when RI_MOUSE_WHEEL is set, gives positive or negative delta of mouse wheel. On my system
it's always -120 or +120. x, y is relative or absolute, depending on the (mouseData.usFlags & MOUSE_MOVE_ABSOLUTE). But
in my tests it's always relative, like (-3, 0), on every USB mouse, as well as touchpad on my laptop. There is also
mouseData.ulRawButtons available with bits corresponding to current state of all mouse buttons, but works on my PC and
not on my laptop (it is always 0).
...
int PEEL_main(int argc, char** argv) {
    ...
    InitTests();
    {
        ...
        if (Load("PEEL", "Autosaved", "CurrentTest", Value)) {
            if (Value < GetNbTests()) gCurrentTest = Value;
        }
    }
    ...
    if (0) {
        ...
        for (UINT i = 0; i < numDevices; ++i) {
            ...
            if (device.dwType == RIM_TYPEMOUSE) {
                ...
                UINT dataSize;
                GetRawInputDeviceInfo(device.hDevice, RIDI_DEVICENAME, null, &dataSize);
                if (dataSize) {
                ...
                if (result != UINT_MAX) {
                    ...
                    wprintf(L"  Id=%u, Buttons=%u, SampleRate=%u, HorizontalWheel=%s\n", deviceInfo.mouse.dwId,
                            deviceInfo.mouse.dwNumberOfButtons, deviceInfo.mouse.dwSampleRate,
    ...
    struct Exit {
        static void Function(int) { exit(0); }
    };
...

    {
        ...
        // ### compiler screw up right here, for some reason using GetPath crashes on x64 even though it's the same
        // implementation
    ...
    if (TestEditor && gNbPlugIns) {
        if (gNbPlugIns != 1) {
            IceCore::MessageBoxA(null, "Editor behavior is undefined with more than 1 plugin selected!", "WARNING",
                                 MB_OK);
        }
        gPlugIns[gNbPlugIns++] = GetEditorPlugin();
    ...
    if (1) SetBestRenderMode();

    if (gCandidateTest) ActivateTest();

    // Render one frame now to force shader compilation before running the scripts
    RenderCallback();

    if (gCmdLineScript[0]) RunScript(FindPEELFile(gCmdLineScript));
...
/*static*/ void EnableRendering(bool b) {
    if (gRenderEnabled != b) {
        gRenderEnabled = b;
        if (gCheckBox_Render) gCheckBox_Render->SetChecked(b);
    }
}
...
static void EnableWireframe(bool b) {
    if (gWireframeEnabled != b) {
        gWireframeEnabled = b;
        if (gCheckBox_Wireframe) gCheckBox_Wireframe->SetChecked(b);
    }
}
...
void EnableWireframeOverlay(bool b) {
    if (gWireframeOverlayEnabled != b) {
        ...
        if (gCheckBox_WireframeOverlay) gCheckBox_WireframeOverlay->SetChecked(b);
    }
}
...
static const char* gTooltip_RaytracingTest =
        "Raytrace current scene from current camera pose, using scene-level raycast calls. Results are dumped to "
        "console.";
...
static const char* gTooltip_RaycastMode =
        "Desired mode for SQ raycast tests. 'Closest' returns one closest hit, 'Any' returns the first hit and early "
        "exits, 'All' collects all hits touched by the ray.";
...
static void gPEEL_GetOptionsFromGUI() {
    ...
    if (gComboBox_CurrentTool) {
        ...
        if (SelectedIndex != GetCurrentToolIndex()) SelectTool(SelectedIndex);
    }
    ...
    if (gComboBox_RenderModel) {
        ...
        if (SelectedIndex != gRenderModelIndex) SelectRenderModel(SelectedIndex);
    }

    if (gComboBox_ScreenQuad) {
        const udword SelectedIndex = gComboBox_ScreenQuad->GetSelectedIndex();
        if (SelectedIndex != gScreenQuadModeIndex) SelectScreenQuadMode(SelectedIndex);
    }
...
public:
    ...
    virtual void OnComboBoxEvent(ComboBoxEvent event) {
        if (event == CBE_SELECTION_CHANGED) {
            ...
            if (SelectedTool != GetCurrentToolIndex()) SelectTool(SelectedTool);
...
static void CreateMainTab(IceWindow* tab) {
    sdword y = 4;
    {
        struct RandomizeOrder {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
                gRandomizeOrder = checked;
            }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Randomize physics engines order", gMainGUI, gRandomizeOrder,
                                  RandomizeOrder::Lambda, gTooltip_RandomizeOrder);
        y += YStep;

        struct TrashCache {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gTrashCache = checked; }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Trash cache", gMainGUI, gTrashCache, TrashCache::Lambda,
                                  gTooltip_TrashCache);
        y += YStep;
        ...
        struct VSync {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
                ...
                GL_SelectVSYNC(checked);
            }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "VSYNC", gMainGUI, gEnableVSync, VSync::Lambda,
                                  gTooltip_VSYNC);
        y += YStep;
        ...
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Use comma separator", gMainGUI, gCommaSeparator,
                                  CommaSeparator::Lambda, gTooltip_CommaSeparator);
        y += YStep;

        struct InfoMessages {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
                gDisplayInfoMessages = checked;
            }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Print info messages to console", gMainGUI, gCommaSeparator,
                                  InfoMessages::Lambda, gTooltip_InfoMessages);
        y += YStep;

        struct RoundCorners {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gRoundCorners = checked; }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Round corners", gMainGUI, gRoundCorners, RoundCorners::Lambda,
                                  null /*gTooltip_*/);
        y += YStep;

        gGUIHelper.CreateLabel(tab, 0, y + LabelOffsetY, 90, 20, "Corners size:", gMainGUI);
        gEditBox_RoundCornersSize = gGUIHelper.CreateEditBox(
                tab, MAIN_GUI_ROUND_CORNERS_SIZE, OffsetX, y, EditBoxWidth, 20, _F("%d", gRoundCornersSize), gMainGUI,
                EDITBOX_INTEGER_POSITIVE, gEBCallback, null /*gTooltip_PickingForce*/);
        y += YStep;

        gGUIHelper.CreateLabel(tab, 0, y + LabelOffsetY, 90, 20, "Frame size:", gMainGUI);
        gEditBox_FrameSize =
                gGUIHelper.CreateEditBox(tab, MAIN_GUI_FRAME_SIZE, OffsetX, y, EditBoxWidth, 20, _F("%f", gFrameSize),
                                         gMainGUI, EDITBOX_FLOAT_POSITIVE, gEBCallback, gTooltip_FrameSize);
        y += YStep;

        struct SymmetricFrames {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
                gSymmetricFrames = checked;
            }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Symmetric frames", gMainGUI, gSymmetricFrames,
                                  SymmetricFrames::Lambda, null /*gTooltip_*/);
        y += YStep;

        struct FatFrames {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gUseFatFrames = checked; }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Use fat frames", gMainGUI, gUseFatFrames, FatFrames::Lambda,
                                  null /*gTooltip_*/);
        y += YStep;
    }

    {
        ...
        gComboBox_ProfilingUnits = CreateComboBox<IceComboBox>(tab, MAIN_GUI_PROFILING_UNITS, 4 + OffsetX, y, 150, 20,
                                                               "Profiling units", gMainGUI, null);
    ...
    }
    {
        ...
        gComboBox_UserProfilingMode = CreateComboBox<IceComboBox>(tab, MAIN_GUI_USER_PROFILING_MODE, 4 + OffsetX, y,
                                                                  150, 20, "Profiling mode", gMainGUI, null);
    ...
    }
    {
        ...
        gComboBox_SQRaycastMode = CreateComboBox<MultiComboBox>(tab, MAIN_GUI_SQ_RAYCAST_MODE, 4 + OffsetX, y, 150, 20,
                                                                "SQ raycast mode", gMainGUI, gTooltip_RaycastMode);
        ...
        struct SolidImpactShapes {
            static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
                gRenderSolidImpactShapes = checked;
            }
        };
        gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Draw solid impact shapes", gMainGUI, gRenderSolidImpactShapes,
                                  SolidImpactShapes::Lambda, null /*gTooltip_*/);
        y += YStep;
    }
}

Point UI_GetGravity() { return gEditBox_Gravity ? gEditBox_Gravity->GetPos() : gDefaultGravity; }

void UI_SetGravity(const Point& gravity) {
    if (gEditBox_Gravity) gEditBox_Gravity->SetPos(gravity);
}

bool UI_GetSceneDesc(String& text) {
    if (!gEditBox_SceneDesc) return false;
...
static bool IsCarriageReturn(char c, udword i, udword length, const char* text) {
    if (c == ASCII_CARRIAGE_RETURN && (i + 1) < length && text[i + 1] == ASCII_NEXT_LINE) return true;
    return false;
}
...
static bool HasCarriageReturn(const char* text, udword length) {
    for (udword i = 0; i < length; i++) {
        const char c = text[i];
        if (IsCarriageReturn(c, i, length, text)) return true;
    }
    return false;
}
...
static void CreateSceneTab(IceWindow* tab) {
    ...
    struct AddDefaultEnv {
        static void Lambda(IceButton& button, void* user_data) {
            for (udword i = 0; i < gNbPlugIns; i++) {
                Pint* Engine = gPlugIns[i]->GetPint();
                if (Engine) SetupDefaultEnvironment(*Engine, true);
            }
        }
    };
    gGUIHelper.CreateButton(tab, 0, 200, 8, 130, 20, "Add default env", gMainGUI, AddDefaultEnv::Lambda, null);

    struct RemoveDefaultEnv {
        static void Lambda(IceButton& button, void* user_data) {
            for (udword i = 0; i < gNbPlugIns; i++) {
                Pint* Engine = gPlugIns[i]->GetPint();
                if (Engine) ReleaseDefaultEnvironment(*Engine);
            }
        }
    };
    gGUIHelper.CreateButton(tab, 0, 200, 28, 130, 20, "Remove default env", gMainGUI, RemoveDefaultEnv::Lambda, null);

    gGUIHelper.CreateButton(tab, COLOR_PICKER_PLANE_COLOR, 200, 48, 130, 20, "Change plane color", gMainGUI,
                            gColorPickerButtonCallback, null, "Change reflective plane color");

    // gGUIHelper.CreateEditBox(tab, 0, 4, y, EditBoxWidth, 20, "Scene description", gMainGUI, EDITBOX_TEXT, null,
    // null);
    {
        struct Local {
            static void gEBCallback(const IceEditBox& edit_box, udword param, void* user_data) {
...
void UpdateToolComboBox() { gComboBox_CurrentTool->Select(GetCurrentToolIndex()); }

static void CreateToolTab(IceWindow* tab) {
    sdword y = 4;
    {
        gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Current tool:", gMainGUI);
        gComboBox_CurrentTool = CreateComboBox<ToolComboBox>(tab, MAIN_GUI_CURRENT_TOOL, 4 + OffsetX, y, 150, 200,
                                                             "Current tool", gMainGUI, gTooltip_CurrentTool);
        const udword NbTools = GetNbTools();
        for (udword i = 0; i < NbTools; i++) gComboBox_CurrentTool->Add(GetToolName(i));
    ...
    for (udword i = 0; i < TOOL_COUNT; i++) {
        ...
        if (T) T->CreateUI(gGUIHelper, OptionWindow, *gMainGUI);
    }
    if (1) {
        IceEditBox* tmp =
                gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, 2, y, ToolSpecificAreaWidth, ToolSpecificAreaHeight,
                                         "=== Tool-specific settings ===", gMainGUI, EDITBOX_TEXT, null);
...
static void CreateCameraTab(IceWindow* tab) {
    ...
    gEditBox_CameraSpeed = gGUIHelper.CreateEditBox(tab, MAIN_GUI_CAMERA_SPEED, 4 + OffsetX, y, EditBoxWidth, 20,
                                                    gGUIHelper.Convert(gCameraSpeedEdit), gMainGUI,
                                                    EDITBOX_FLOAT_POSITIVE, gEBCallback, gTooltip_CameraSpeed);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Camera near clip:", gMainGUI);
    gEditBox_CameraNearClip = gGUIHelper.CreateEditBox(tab, MAIN_GUI_CAMERA_NEAR_CLIP, 4 + OffsetX, y, EditBoxWidth, 20,
                                                       gGUIHelper.Convert(gCameraNearClipEdit), gMainGUI,
                                                       EDITBOX_FLOAT_POSITIVE, gEBCallback, gTooltip_CameraNearClip);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Camera far clip:", gMainGUI);
    gEditBox_CameraFarClip = gGUIHelper.CreateEditBox(tab, MAIN_GUI_CAMERA_FAR_CLIP, 4 + OffsetX, y, EditBoxWidth, 20,
                                                      gGUIHelper.Convert(gCameraFarClipEdit), gMainGUI,
                                                      EDITBOX_FLOAT_POSITIVE, gEBCallback, gTooltip_CameraFarClip);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Camera FOV:", gMainGUI);
    gEditBox_CameraFOV = gGUIHelper.CreateEditBox(tab, MAIN_GUI_CAMERA_FOV, 4 + OffsetX, y, EditBoxWidth, 20,
                                                  gGUIHelper.Convert(gCameraFOVEdit), gMainGUI, EDITBOX_FLOAT_POSITIVE,
                                                  gEBCallback, gTooltip_CameraFOV);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Camera sensitivity:", gMainGUI);
    gEditBox_CameraSensitivity =
            gGUIHelper.CreateEditBox(tab, MAIN_GUI_CAMERA_SENSITIVITY, 4 + OffsetX, y, EditBoxWidth, 20,
                                     gGUIHelper.Convert(gCameraSensitivityEdit), gMainGUI, EDITBOX_FLOAT_POSITIVE,
                                     gEBCallback, gTooltip_CameraSensitivity);
    y += YStep;

    struct FreezeFrustum {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
            gCameraFreezeFrustum = checked;
            gFreezeFrustum = checked;
            if (gFreezeFrustum) CaptureFrustum();
        }
    };
    gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "Freeze frustum", gMainGUI, gCameraFreezeFrustum,
                              FreezeFrustum::Lambda, null);
    y += YStep;

    struct VFC {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gCameraVFC = checked; }
    };
    gGUIHelper.CreateCheckBox(tab, 0, 4, y, 200, 20, "VFC", gMainGUI, gCameraVFC, VFC::Lambda, null);
    y += YStep;

    struct ResetCamera {
        static void Lambda(IceButton& button, void* user_data) { gCamera.Reset(); }
    };
    gGUIHelper.CreateButton(tab, 0, 4, y, 130, 20, "Reset view", gMainGUI, ResetCamera::Lambda, null);
    y += YStep;

    struct NextCamera {
        static void Lambda(IceButton& button, void* user_data) { gCamera.SelectNextCamera(); }
    };
    gGUIHelper.CreateButton(tab, 0, 4, y, 130, 20, "Select next camera", gMainGUI, NextCamera::Lambda, null);
    y += YStep;

    struct PreviousCamera {
        static void Lambda(IceButton& button, void* user_data) { gCamera.SelectPreviousCamera(); }
    };
    gGUIHelper.CreateButton(tab, 0, 4, y, 130, 20, "Select previous camera", gMainGUI, PreviousCamera::Lambda, null);
    y += YStep;

    struct CamPoseToClipboard {
        static void Lambda(IceButton& button, void* user_data) { CopyCameraPoseToClipboard(); }
    };
...
static void CreateRaytraceTab(IceWindow* tab) {
    ...
    if (1) {
...

        {
            struct Blit {
                static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
                    gBlitToScreen = checked;
                }
            };
            gCheckBox_RT_Blit = gGUIHelper.CreateCheckBox(tab, 0, x, y, 80, 20, "Blit to screen", gMainGUI,
                                                          gBlitToScreen, Blit::Lambda, null);
            y += YStep;
        }
...

        {
            ...
            gEditBox_RaytracingDistance = gGUIHelper.CreateEditBox(
                    tab, MAIN_GUI_RAYTRACING_DISTANCE, x + 50, y, EditBoxWidth, 20, gGUIHelper.Convert(gRTDistance),
                    gMainGUI, EDITBOX_FLOAT_POSITIVE, gEBCallback /*, gTooltip_CameraSpeed*/);
        }

        {
            y += YStep;
            // gGUIHelper.CreateLabel(MainOptions, 4, y+LabelOffsetY, 90, 20, "RT resolution:", gMainGUI);
            gComboBox_RaytracingResolution = CreateComboBox<MultiComboBox>(tab, MAIN_GUI_RAYTRACING_RESOLUTION, x, y,
                                                                           150, 20, "RT resolution", gMainGUI, null);
        ...
        //		IceEditBox* tmp = gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, 4, saved, 120, 100, "===
        //Raytracing test ===", gMainGUI, EDITBOX_TEXT, null); 		IceEditBox* tmp = gGUIHelper.CreateEditBox(tab,
        //MAIN_GUI_DUMMY, 296, saved, 180, 140, "=== Raytracing test ===", gMainGUI, EDITBOX_TEXT, null);
        IceEditBox* tmp = gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, x + 296 - 310, saved, 180, 160,
                                                   "=== Raytracing test ===", gMainGUI, EDITBOX_TEXT, null,
                                                   gTooltip_RaytracingTest);
        //		tmp->AddToolTip(gTooltip_RaytracingTest);
        tmp->SetReadOnly(true);
    }
    tab->SetVisible(false);
...
static void CreateRenderTab(IceWindow* tab) {
    ...
    struct Render {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gRenderEnabled = checked; }
    };
    gCheckBox_Render =
            gGUIHelper.CreateCheckBox(tab, 0, 4, y, 80, 20, "Render", gMainGUI, gRenderEnabled, Render::Lambda, null);
    //	y += YStep;

    struct Wireframe {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gWireframeEnabled = checked; }
    };
    gCheckBox_Wireframe = gGUIHelper.CreateCheckBox(tab, 0, 4 + 100, y, 80, 20, "Wireframe", gMainGUI,
                                                    gWireframeEnabled, Wireframe::Lambda, null);
    //	y += YStep;

    struct WireframeOverlay {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) {
            gWireframeOverlayEnabled = checked;
        }
    };
    gCheckBox_WireframeOverlay = gGUIHelper.CreateCheckBox(tab, 0, 4 + 200, y, 80, 20, "Overlay", gMainGUI,
                                                           gWireframeOverlayEnabled, WireframeOverlay::Lambda, null);

    struct GLFinish {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gGLFinish = checked; }
    };
    gCheckBox_GLFinish = gGUIHelper.CreateCheckBox(tab, 0, 4 + 300, y, 80, 20, "GLFinish", gMainGUI, gGLFinish,
                                                   GLFinish::Lambda, null);
    y += YStep;

    {
        const udword OffsetX2 = 0;
        gGUIHelper.CreateLabel(tab, 4 + OffsetX2, y + LabelOffsetY, 90, 20, "Wireframe overlay:", gMainGUI);
        gEditBox_WireframeOverlayCoeff =
                gGUIHelper.CreateEditBox(tab, MAIN_GUI_WIREFRAME_OVERLAY_COEFF, 4 + OffsetX + OffsetX2, y, EditBoxWidth,
                                         20, gGUIHelper.Convert(gWireframeOverlayCoeff), gMainGUI,
                                         EDITBOX_FLOAT_POSITIVE, gEBCallback, gTooltip_WireframeOverlayCoeff);
    }

    {
        const udword LocalLabelWidth = 130;
        const udword OffsetX2 = 200;
        gGUIHelper.CreateLabel(tab, 4 + OffsetX2, y + LabelOffsetY, LocalLabelWidth, 20,
                               "Convex edges threshold:", gMainGUI);
        gEditBox_ConvexEdgesThreshold =
                gGUIHelper.CreateEditBox(tab, MAIN_GUI_CONVEX_EDGES_THRESHOLD, 4 + LocalLabelWidth + OffsetX2, y,
                                         EditBoxWidth, 20, gGUIHelper.Convert(gConvexEdgesThreshold), gMainGUI,
                                         EDITBOX_FLOAT_POSITIVE, gEBCallback, null /*gTooltip_WireframeOverlayCoeff*/);
    }
...

    {
        ...
        gEditBox_BatchSize = gGUIHelper.CreateEditBox(tab, MAIN_GUI_BATCH_SIZE, 4 + OffsetX + OffsetX2, y, EditBoxWidth,
                                                      20, _F("%d", gBatchSize), gMainGUI, EDITBOX_INTEGER_POSITIVE,
                                                      gEBCallback, gTooltip_BatchSize);
    }
    y += YStep;

    {
        sdword y2 = 10;
        gGUIHelper.CreateButton(tab, COLOR_PICKER_WIRE_COLOR, 300, 50, 130, 20, "Change wire color", gMainGUI,
                                gColorPickerButtonCallback, null, "Change wireframe color");
        y2 += 30;
    }

    /*	gGUIHelper.CreateLabel(tab, 4, y+LabelOffsetY, 90, 20, "Wireframe overlay:", gMainGUI);
            gEditBox_WireframeOverlayCoeff = gGUIHelper.CreateEditBox(tab, MAIN_GUI_WIREFRAME_OVERLAY_COEFF, 4+OffsetX,
       y, EditBoxWidth, 20, gGUIHelper.Convert(gWireframeOverlayCoeff), gMainGUI, EDITBOX_FLOAT_POSITIVE, gEBCallback,
       gTooltip_WireframeOverlayCoeff); y += YStep;

            gGUIHelper.CreateLabel(tab, 4, y+LabelOffsetY, 90, 20, "Batch size:", gMainGUI);
            gEditBox_BatchSize = gGUIHelper.CreateEditBox(tab, MAIN_GUI_BATCH_SIZE, 4+OffsetX, y, EditBoxWidth, 20,
       _F("%d", gBatchSize), gMainGUI, EDITBOX_INTEGER_POSITIVE, gEBCallback, gTooltip_BatchSize); y += YStep;*/

    /*	{
                    y += YStep;
                    gGUIHelper.CreateLabel(tab, 4, y+LabelOffsetY, 90, 20, "Fog type:", gMainGUI);
                    gComboBox_FogType = CreateComboBox<MultiComboBox>(tab, MAIN_GUI_FOG_TYPE, 94, y, 150, 20, "Fog
    type", gMainGUI, gTooltip_FogType); gComboBox_FogType->Add("Disabled"); gComboBox_FogType->Add("Linear");
...
#ifdef PEEL_USE_MSAA
    {
        ...
        gComboBox_MSAA =
                CreateComboBox<MultiComboBox>(tab, MAIN_GUI_MSAA, 94, y, 150, 20, "MSAA", gMainGUI, gTooltip_MSAA);
...

    {
        ...
        gComboBox_RenderModel = CreateComboBox<MultiComboBox>(tab, MAIN_GUI_RENDER_MODEL, 94, y, 150, 20,
                                                              "Render model", gMainGUI, gTooltip_RenderModel);
        for (udword i = 0; i < RENDER_MODEL_COUNT; i++) gComboBox_RenderModel->Add(gRenderModel[i]->GetUIName());
    ...
    {
        IceEditBox* tmp = gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, 4, y, 350, 230,
                                                   "===== Render model options =====", gMainGUI, EDITBOX_TEXT, null);
        tmp->SetReadOnly(true);
    }
...
static void CreateScreenQuadTab(IceWindow* tab) {
    ...
    gComboBox_ScreenQuad = CreateComboBox<MultiComboBox>(tab, MAIN_GUI_SCREEN_QUAD, 94, y, 200, 20, "Screen quad",
                                                         gMainGUI, gTooltip_ScreenQuad);
    for (udword i = 0; i < SCREEN_QUAD_COUNT; i++) gComboBox_ScreenQuad->Add(gScreenQuadModes[i]->GetUIName());
    ...
    {
        IceEditBox* tmp = gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, 4, 4 + 30, 350, 300,
                                                   "===== Screen quad options =====", gMainGUI, EDITBOX_TEXT, null);
        tmp->SetReadOnly(true);
    }
...
static void CreateSunTab(IceWindow* tab) {
    ...
    struct RenderSun {
        static void Lambda(const IceCheckBox& check_box, bool checked, void* user_data) { gRenderSunEnabled = checked; }
    };
    gCheckBox_RenderSun = gGUIHelper.CreateCheckBox(tab, 0, 4, y, 80, 20, "Render sun", gMainGUI, gRenderSunEnabled,
                                                    RenderSun::Lambda, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Phi:", gMainGUI);
    gEditBox_SunPhi = gGUIHelper.CreateEditBox(tab, MAIN_GUI_SUN_PHI, 4 + OffsetX, y, EditBoxWidth, 20,
                                               gGUIHelper.Convert(gSunPhi), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Theta:", gMainGUI);
    gEditBox_SunTheta =
            gGUIHelper.CreateEditBox(tab, MAIN_GUI_SUN_THETA, 4 + OffsetX, y, EditBoxWidth, 20,
                                     gGUIHelper.Convert(gSunTheta), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Intensity:", gMainGUI);
    gEditBox_SunIntensity =
            gGUIHelper.CreateEditBox(tab, MAIN_GUI_SUN_INTENSITY, 4 + OffsetX, y, EditBoxWidth, 20,
                                     gGUIHelper.Convert(gSunIntensity), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Rayleigh multiplier:", gMainGUI);
    gEditBox_RayleighMultiplier = gGUIHelper.CreateEditBox(tab, MAIN_GUI_RAYLEIGH_MULTIPLIER, 4 + OffsetX, y,
                                                           EditBoxWidth, 20, gGUIHelper.Convert(gRayleighMultiplier),
                                                           gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Mie multiplier:", gMainGUI);
    gEditBox_MieMultiplier =
            gGUIHelper.CreateEditBox(tab, MAIN_GUI_MIE_MULTIPLIER, 4 + OffsetX, y, EditBoxWidth, 20,
                                     gGUIHelper.Convert(gMieMultiplier), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Scattering multiplier:", gMainGUI);
    gEditBox_ScatteringMultiplier = gGUIHelper.CreateEditBox(
            tab, MAIN_GUI_SCATTERING_MULTIPLIER, 4 + OffsetX, y, EditBoxWidth, 20,
            gGUIHelper.Convert(gScatteringMultiplier), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "Extinction multiplier:", gMainGUI);
    gEditBox_ExtinctionMultiplier = gGUIHelper.CreateEditBox(
            tab, MAIN_GUI_EXTINCTION_MULTIPLIER, 4 + OffsetX, y, EditBoxWidth, 20,
            gGUIHelper.Convert(gExtinctionMultiplier), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;

    gGUIHelper.CreateLabel(tab, 4, y + LabelOffsetY, 90, 20, "HGg:", gMainGUI);
    gEditBox_HGg = gGUIHelper.CreateEditBox(tab, MAIN_GUI_HG, 4 + OffsetX, y, EditBoxWidth, 20,
                                            gGUIHelper.Convert(gHGg), gMainGUI, EDITBOX_FLOAT, gEBCallback, null);
    y += YStep;
...
static void gColorPickerButtonCallback(IceButton& button, void* user_data) {
    ...
    if (user_data) gColorPicker->SetLabel((const char*)user_data);
...
public:
    ...
    virtual void MenuEvent(udword menu_item_id) { ProcessMenuEvent(menu_item_id); }
    ...
    virtual int handleEvent(IceGUIEvent* evt) {
        ...
        if (evt->mType == EVENT_MOUSE_WHEEL) {
            //				MouseWheelCallback(0, 0, event->mHeight);
        }
    ...
    if (1) {
        ...
        IceWindow* MainOptions;
        {
...

            {
                ...
                gGUIHelper.CreateLabel(MainOptions, 4, y, 300, 20, _F("Version %s - Build: %s", gVersion, Date),
                                       gMainGUI);
                y += YStep;
                //				y += YStep;
            }

            {
                ...
                //				IceButton* gButton_ExecuteScript = gGUIHelper.CreateButton(MainOptions, 0, 20,
                //y, 450, 20, "Execute script", gMainGUI, gButtonCallbackScript, null); 				y += YStep; 				y += YStep; 				y += 30;

                //				gGUIHelper.CreateButton(MainOptions, 0, 20, y, 450, 20, "Import scene",
                //gMainGUI, gButtonCallbackScene, null); 				y += 30;
            }
        }
        {
    ...
    for (udword i = 0; i < gNbPlugIns; i++) {
        ...
        if (Main) {
            ...
            if (Flags & PINT_HAS_RAYTRACING_WINDOW) {
                ...
                gRaytracingWindows[i] = CreateRaytracingWindow(*gMainGUI, Main, 512 - RaytracingWindowWidth - 20,
                                                               768 - RaytracingWindowHeight - 50, RaytracingWindowWidth,
                                                               RaytracingWindowHeight);
            }
            if (TabControl) TabControl->Add(Main, Engine->GetUIName());
        }
    }
}
...
static void SetBestRenderMode() {
    ...
    Select_MSAA(3);
    if (gComboBox_RenderModel) gComboBox_RenderModel->Select(index);
    if (gComboBox_ScreenQuad) gComboBox_ScreenQuad->Select(SCREEN_QUAD_COLOR_SPHERE);
    if (gComboBox_MSAA) gComboBox_MSAA->Select(3);
}

void TestCSVExport() {
    if (!gRunningTest) return;
    ...
    for (udword b = 0; b < gNbEngines; b++) {
        if (!gEngines[b].mEnabled || !gEngines[b].mSupportsCurrentTest ||
            !(gEngines[b].mEngine->GetFlags() & PINT_IS_ACTIVE))
            continue;
