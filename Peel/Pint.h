...
struct PintCameraPose {
    ...
    bool operator==(const PintCameraPose& other) const { return mPos == other.mPos && mDir == other.mDir; }
...
/*	class PintFilterCallback
        {
                public:
                virtual	bool	PairFound(Pint& pint, PintActorHandle actor0, PintShapeHandle shape0, PintActorHandle
   actor1, PintShapeHandle shape1)	= 0;
        };*/
...
public:
    ...
    virtual bool PrepContactModify(Pint& pint,
                                   udword nb_contacts,
                                   PintActorHandle h0,
                                   PintActorHandle h1,
                                   PintShapeHandle s0,
                                   PintShapeHandle s1) = 0;
    ...
    virtual ContactModif ModifyContact(Pint& pint,
                                       const PR& pose0,
                                       const PR& pose1,
                                       Point& p,
                                       Point& n,
                                       ...
                                       udword feature0,
                                       udword feature1) = 0;
    virtual ContactModif ModifyDelayedContact(Pint& pint,
                                              const PR& pose0,
                                              const PR& pose1,
                                              Point& p,
                                              Point& n,
...
public:
    ...
    virtual PintContactModifyPairHandle GetContactPairData(udword pair_index,
                                                           ContactPairData& data,
                                                           PR* pose0,
                                                           PR* pose1) = 0;
...
struct PINT_SHAPE_CREATE : public Allocateable {
    PINT_SHAPE_CREATE(PintShape type)
        : mName(null), mType(type), mMaterial(null), mRenderer(null), mSharing(SHAPE_SHARING_UNDEFINED), mNext(null) {
        mLocalPos.Zero();
        mLocalRot.Identity();
    }
    ...
    inline_ bool CanShare(bool defaultValue) const {
        if (mSharing == SHAPE_SHARING_YES) return true;
        if (mSharing == SHAPE_SHARING_NO) return false;
        return defaultValue;
    }
...
struct PINT_SPHERE_CREATE : PINT_SHAPE_CREATE {
    PINT_SPHERE_CREATE(float radius = 0.0f) : PINT_SHAPE_CREATE(PINT_SHAPE_SPHERE), mRadius(radius) {}

    PINT_SPHERE_CREATE(const PINT_SPHERE_CREATE& desc) : PINT_SHAPE_CREATE(desc), mRadius(desc.mRadius) {}
...
struct PINT_CAPSULE_CREATE : PINT_SHAPE_CREATE {
    PINT_CAPSULE_CREATE(float radius = 0.0f, float half_height = 0.0f)
        : PINT_SHAPE_CREATE(PINT_SHAPE_CAPSULE), mRadius(radius), mHalfHeight(half_height) {}

    PINT_CAPSULE_CREATE(const PINT_CAPSULE_CREATE& desc)
        : PINT_SHAPE_CREATE(desc), mRadius(desc.mRadius), mHalfHeight(desc.mHalfHeight) {}
...
struct PINT_CYLINDER_CREATE : PINT_SHAPE_CREATE {
    PINT_CYLINDER_CREATE(float radius = 0.0f, float half_height = 0.0f)
        : PINT_SHAPE_CREATE(PINT_SHAPE_CYLINDER), mRadius(radius), mHalfHeight(half_height) {}
...
struct PINT_BOX_CREATE : PINT_SHAPE_CREATE {
    PINT_BOX_CREATE(float x = 0.0f, float y = 0.0f, float z = 0.0f)
        : PINT_SHAPE_CREATE(PINT_SHAPE_BOX), mExtents(x, y, z) {}

    PINT_BOX_CREATE(const Point& extents) : PINT_SHAPE_CREATE(PINT_SHAPE_BOX), mExtents(extents) {}

    PINT_BOX_CREATE(const PINT_BOX_CREATE& desc) : PINT_SHAPE_CREATE(desc), mExtents(desc.mExtents) {}
...
{
    PINT_CONVEX_DATA_CREATE(udword nb_verts = 0, const Point* verts = null) : mNbVerts(nb_verts), mVerts(verts) {}

    PINT_CONVEX_DATA_CREATE(const PINT_CONVEX_DATA_CREATE& desc) : mNbVerts(desc.mNbVerts), mVerts(desc.mVerts) {}
...
struct PINT_CONVEX_CREATE : PINT_SHAPE_CREATE, PINT_CONVEX_DATA_CREATE {
    PINT_CONVEX_CREATE(udword nb_verts = 0, const Point* verts = null)
        : PINT_SHAPE_CREATE(PINT_SHAPE_CONVEX), PINT_CONVEX_DATA_CREATE(nb_verts, verts) {}

    PINT_CONVEX_CREATE(const PINT_CONVEX_CREATE& desc)
        : PINT_SHAPE_CREATE(desc), PINT_CONVEX_DATA_CREATE(desc.mNbVerts, desc.mVerts) {}
};
...
struct PINT_MESH_DATA_CREATE  // : public Allocateable
{
    ...
    PINT_MESH_DATA_CREATE(const PINT_MESH_DATA_CREATE& desc)
        : mSurface(desc.mSurface), mDynamic(desc.mDynamic), mDeformable(desc.mDeformable) {}

    inline_ const PintSurfaceInterface& GetSurface() const { return mSurface; }

    inline_ void SetSurfaceData(
            udword nb_verts, const Point* verts, udword nb_faces, const udword* dfaces, const uword* wfaces) {
    ...
    inline_ void SetSurfaceData(const SurfaceInterface& surface) { mSurface.Init(surface); }
...
struct PINT_MESH_CREATE : PINT_SHAPE_CREATE, PINT_MESH_DATA_CREATE {
    ...
    PINT_MESH_CREATE(const PINT_MESH_CREATE& desc) : PINT_SHAPE_CREATE(desc), PINT_MESH_DATA_CREATE(desc) {}
};
...
struct PINT_MESH_CREATE2 : PINT_SHAPE_CREATE {
    ...
    PINT_MESH_CREATE2(const PINT_MESH_CREATE2& desc) : PINT_SHAPE_CREATE(desc), mTriangleMesh(desc.mTriangleMesh) {}
...
struct PINT_HEIGHTFIELD_DATA_CREATE  // : public Allocateable
{
    PINT_HEIGHTFIELD_DATA_CREATE(udword nbu = 0, udword nbv = 0, const float* heights = null, float v = 0.0f)
        : mNbU(nbu), mNbV(nbv), mHeights(heights), mUniqueValue(v) {}
...
struct PINT_HEIGHTFIELD_CREATE : PINT_SHAPE_CREATE {
    PINT_HEIGHTFIELD_CREATE(PintHeightfieldHandle h = null, float scale_u = 1.0f, float scale_v = 1.0f)
        : PINT_SHAPE_CREATE(PINT_SHAPE_HEIGHTFIELD), mHeightfield(h), mScaleU(scale_u), mScaleV(scale_v) {}

    PINT_HEIGHTFIELD_CREATE(const PINT_HEIGHTFIELD_CREATE& desc)
        : PINT_SHAPE_CREATE(desc), mHeightfield(desc.mHeightfield), mScaleU(desc.mScaleU), mScaleV(desc.mScaleV) {}
...
struct PINT_OBJECT_CREATE : public Allocateable {
    ...
    ~PINT_OBJECT_CREATE() { DELETESINGLE(mExtraShapes); }
...
public:
    ...
    void AddShape(const PINT_SHAPE_CREATE* shape) {
        if (!mShape) {
            ...
            if (!mExtraShapes) mExtraShapes = ICE_NEW(PtrContainer);
    ...
    udword GetNbShapes(PintShapeEnumerateCallback* callback = null, void* user_data = null) const {
        ...
        while (CurrentShape) {
            ...
            if (callback) callback->ReportShape(*CurrentShape, NbShapes, user_data);
        ...
        if (mExtraShapes) {
            ...
            if (callback) {
                for (udword i = 0; i < NbExtraShapes; i++)
                    callback->ReportShape(*reinterpret_cast<const PINT_SHAPE_CREATE*>(mExtraShapes->GetEntry(i)),
                                          NbShapes + i, user_data);
            }
    ...
    inline_ const PINT_SHAPE_CREATE* GetFirstShape() const { return mShape; };
};

///////////////////////////////////////////////////////////////////////////

struct PINT_AGGREGATE_CREATE : public Allocateable {
    PINT_AGGREGATE_CREATE(udword max_size, bool self_collision) : mMaxSize(max_size), mSelfCollision(self_collision) {}

    PINT_AGGREGATE_CREATE(const PINT_AGGREGATE_CREATE& desc)
        : mMaxSize(desc.mMaxSize), mSelfCollision(desc.mSelfCollision) {}
...
inline_ bool IsSphericalLimitEnabled(const PintLimits& limits) {
    return limits.mMinValue > 0.0f && limits.mMaxValue > 0.0f;
}
...
struct PINT_JOINT_CREATE : public Allocateable {
    PINT_JOINT_CREATE(PintJoint type) : mName(null), mType(type), mObject0(null), mObject1(null) {}

    PINT_JOINT_CREATE(const PINT_JOINT_CREATE& create)
        : mName(create.mName), mType(create.mType), mObject0(create.mObject0), mObject1(create.mObject1) {}
...
struct PINT_SPHERICAL_JOINT_CREATE : PINT_JOINT_CREATE {
    PINT_SPHERICAL_JOINT_CREATE()
        : PINT_JOINT_CREATE(PINT_JOINT_SPHERICAL), mLocalPivot0(Idt), mLocalPivot1(Idt), mLimits(-1.0f, -1.0f) {}

    PINT_SPHERICAL_JOINT_CREATE(PintActorHandle object0, PintActorHandle object1, const PR& p0, const PR& p1)
        : PINT_JOINT_CREATE(PINT_JOINT_SPHERICAL), mLocalPivot0(p0), mLocalPivot1(p1), mLimits(-1.0f, -1.0f) {
        mObject0 = object0;
        mObject1 = object1;
    }

    PINT_SPHERICAL_JOINT_CREATE(PintActorHandle object0, PintActorHandle object1, const Point& p0, const Point& p1)
        : PINT_JOINT_CREATE(PINT_JOINT_SPHERICAL), mLimits(-1.0f, -1.0f) {
...
struct PINT_FIXED_JOINT_CREATE : PINT_JOINT_CREATE {
    ...
    PINT_FIXED_JOINT_CREATE(const PINT_FIXED_JOINT_CREATE& create)
        : PINT_JOINT_CREATE(create), mLocalPivot0(create.mLocalPivot0), mLocalPivot1(create.mLocalPivot1) {}
...
struct PINT_ARTICULATED_MOTOR_CREATE : public Allocateable {
    PINT_ARTICULATED_MOTOR_CREATE()
        : mExternalCompliance(0.0f), mInternalCompliance(0.0f), mStiffness(0.0f), mDamping(0.0f) {
...
struct PINT_RC_ARTICULATION_CREATE : public Allocateable {
    PINT_RC_ARTICULATION_CREATE(bool fix_base = false) : mFixBase(fix_base) {}
...
struct PINT_RC_ARTICULATED_MOTOR_CREATE : public Allocateable {
    PINT_RC_ARTICULATED_MOTOR_CREATE()
        : mStiffness(0.0f), mDamping(0.0f), mMaxForce(FLT_MAX), mAccelerationDrive(false) {}
...
struct PintDisabledGroups : public Allocateable {
    PintDisabledGroups(PintCollisionGroup group0 = 0xffff, PintCollisionGroup group1 = 0xffff)
        : mGroup0(group0), mGroup1(group1) {}
...
struct PintBooleanHit : public Allocateable {
    ...
    inline_ void SetNoHit() { mHit = false; }
};
...
struct PintMultipleHits : public Allocateable {
    ...
    inline_ void SetNoHit() { mNbHits = mOffset = 0; }
};
...
struct PintConvexOverlapData : public Allocateable {
    ...
    // So there is a design issue here. For simpler shapes we don't need per-plugin data in the sweeps (they can all
    // share the same data) but for convexes we need to create per-plugin convex objects, and the convex sweep data
    // becomes engine dependent. PintConvexIndex tries to make this work with the existing infrastructure.
...
public:
    ...
    virtual void SetDisabledGroups(udword nb_groups, const PintDisabledGroups* groups) {
        NotImplemented("SetDisabledGroups");
    }

    virtual PintConvexHandle CreateConvexObject(const PINT_CONVEX_DATA_CREATE& desc, PintConvexIndex* index = null) {
        NotImplemented("CreateConvexObject");
        return null;
    }
    virtual bool DeleteConvexObject(PintConvexHandle handle = null, const PintConvexIndex* index = null) {
        NotImplemented("DeleteConvexObject");
        return false;
    }

    virtual PintMeshHandle CreateMeshObject(const PINT_MESH_DATA_CREATE& desc, PintMeshIndex* index = null) {
        NotImplemented("CreateMeshObject");
        return null;
    }
    virtual bool DeleteMeshObject(PintMeshHandle handle = null, const PintMeshIndex* index = null) {
        NotImplemented("DeleteMeshObject");
        return false;
    }

    virtual PintHeightfieldHandle CreateHeightfieldObject(const PINT_HEIGHTFIELD_DATA_CREATE& desc,
                                                          PintHeightfieldData& data,
                                                          PintHeightfieldIndex* index = null) {
        NotImplemented("CreateHeightfieldObject");
        return null;
    }
    virtual bool DeleteHeightfieldObject(PintHeightfieldHandle handle = null,
                                         const PintHeightfieldIndex* index = null) {
        NotImplemented("DeleteHeightfieldObject");
        return false;
    }
...
public:
    ...
    // SQ filtering - disable scene queries for hidden/invisible objects (mainly used by the editor). This is a bit
    // tricky:
    ...
    // - we cannot use a second SQ system within PEEL, because we (usually) don't keep copies of geom data / bounds
    // there.
    //   Also it would defeat the purpose of doing SQ in native engines.
    // => so for now we use a generic API capturing the desired intent, and we let each engine deal with it the way they
    // want.
    ...
    virtual bool SetSQFlag(PintActorHandle actor, PintShapeHandle shape, bool flag) {
        return NotImplemented("SetSQFlag");
    }
    virtual bool ResetSQFilters() { return NotImplemented("ResetSQFilters"); }

    // Raycasts
    virtual udword BatchRaycasts(PintSQThreadContext context,
                                 udword nb,
                                 PintRaycastHit* dest,
                                 const PintRaycastData* raycasts) {
        return NotImplemented("BatchRaycasts");
    }
    virtual udword BatchRaycastAny(PintSQThreadContext context,
                                   udword nb,
                                   PintBooleanHit* dest,
                                   const PintRaycastData* raycasts) {
        return NotImplemented("BatchRaycastAny");
    }
    virtual udword BatchRaycastAll(PintSQThreadContext context,
                                   udword nb,
                                   PintMultipleHits* dest,
                                   Container& stream,
                                   const PintRaycastData* raycasts) {
        return NotImplemented("BatchRaycastAll");
    }
    //		virtual	bool				Picking						(PintSQThreadContext context, PintRaycastHit* dest, const
    //PintRaycastData& raycast, PintQueryFilterCallback* cb)		{ return NotImplemented("Picking");
    //}
    // Sweeps
    virtual udword BatchBoxSweeps(PintSQThreadContext context,
                                  udword nb,
                                  PintRaycastHit* dest,
                                  const PintBoxSweepData* sweeps) {
        NotImplemented("BatchBoxSweeps");
        return 0;
    }
    virtual udword BatchSphereSweeps(PintSQThreadContext context,
                                     udword nb,
                                     PintRaycastHit* dest,
                                     const PintSphereSweepData* sweeps) {
        NotImplemented("BatchSphereSweeps");
        return 0;
    }
    virtual udword BatchCapsuleSweeps(PintSQThreadContext context,
                                      udword nb,
                                      PintRaycastHit* dest,
                                      const PintCapsuleSweepData* sweeps) {
        NotImplemented("BatchCapsuleSweeps");
        return 0;
    }
    virtual udword BatchConvexSweeps(PintSQThreadContext context,
                                     udword nb,
                                     PintRaycastHit* dest,
                                     const PintConvexSweepData* sweeps) {
        NotImplemented("BatchConvexSweeps");
        return 0;
    }
    // Overlaps
    virtual udword BatchSphereOverlapAny(PintSQThreadContext context,
                                         udword nb,
                                         PintBooleanHit* dest,
                                         const PintSphereOverlapData* overlaps) {
        NotImplemented("BatchSphereOverlapAny");
        return 0;
    }
    virtual udword BatchBoxOverlapAny(PintSQThreadContext context,
                                      udword nb,
                                      PintBooleanHit* dest,
                                      const PintBoxOverlapData* overlaps) {
        NotImplemented("BatchBoxOverlapAny");
        return 0;
    }
    virtual udword BatchCapsuleOverlapAny(PintSQThreadContext context,
                                          udword nb,
                                          PintBooleanHit* dest,
                                          const PintCapsuleOverlapData* overlaps) {
        NotImplemented("BatchCapsuleOverlapAny");
        return 0;
    }
    virtual udword BatchConvexOverlapAny(PintSQThreadContext context,
                                         udword nb,
                                         PintBooleanHit* dest,
                                         const PintConvexOverlapData* overlaps) {
        NotImplemented("BatchConvexOverlapAny");
        return 0;
    }
    virtual udword BatchSphereOverlapObjects(PintSQThreadContext context,
                                             udword nb,
                                             PintMultipleHits* dest,
                                             Container& stream,
                                             const PintSphereOverlapData* overlaps) {
        NotImplemented("BatchSphereOverlapObjects");
        return 0;
    }
    virtual udword BatchBoxOverlapObjects(PintSQThreadContext context,
                                          udword nb,
                                          PintMultipleHits* dest,
                                          Container& stream,
                                          const PintBoxOverlapData* overlaps) {
        NotImplemented("BatchBoxOverlapObjects");
        return 0;
    }
    virtual udword BatchCapsuleOverlapObjects(PintSQThreadContext context,
                                              udword nb,
                                              PintMultipleHits* dest,
                                              Container& stream,
                                              const PintCapsuleOverlapData* overlaps) {
        NotImplemented("BatchCapsuleOverlapObjects");
        return 0;
    }
    virtual udword BatchConvexOverlapObjects(PintSQThreadContext context,
                                             udword nb,
                                             PintMultipleHits* dest,
                                             Container& stream,
                                             const PintConvexOverlapData* overlaps) {
        NotImplemented("BatchConvexOverlapObjects");
        return 0;
    }
    // Midphase
    virtual udword FindTriangles_MeshSphereOverlap(PintSQThreadContext context,
                                                   PintActorHandle handle,
                                                   udword nb,
                                                   PintMultipleHits* dest,
                                                   Container& stream,
                                                   const PintSphereOverlapData* overlaps) {
        NotImplemented("FindTriangles_MeshSphereOverlap");
        return 0;
    }
    virtual udword FindTriangles_MeshBoxOverlap(PintSQThreadContext context,
                                                PintActorHandle handle,
                                                udword nb,
                                                PintMultipleHits* dest,
                                                Container& stream,
                                                const PintBoxOverlapData* overlaps) {
        NotImplemented("FindTriangles_MeshBoxOverlap");
        return 0;
    }
    virtual udword FindTriangles_MeshCapsuleOverlap(PintSQThreadContext context,
                                                    PintActorHandle handle,
                                                    udword nb,
                                                    PintMultipleHits* dest,
                                                    Container& stream,
                                                    const PintCapsuleOverlapData* overlaps) {
        NotImplemented("FindTriangles_MeshCapsuleOverlap");
        return 0;
    }
    virtual udword FindTriangles_MeshMeshOverlap(PintSQThreadContext context,
                                                 PintActorHandle handle0,
                                                 PintActorHandle handle1,
                                                 Container& results) {
        NotImplemented("FindTriangles_MeshMeshOverlap");
        return 0;
    }

    // TODO: move to Pint_Actor API
    virtual PR GetWorldTransform(PintActorHandle handle) {
        NotImplemented("GetWorldTransform");
        PR Idt;
        Idt.Identity();
        return Idt;
    }
    ...
    // Deprecated
    // TODO: move to Pint_Actor API
    //		virtual	void				ApplyActionAtPoint(PintActorHandle handle, PintActionType action_type, const Point&
    //action, const Point& pos)	{ NotImplemented("ApplyActionAtPoint");	}
    virtual void AddWorldImpulseAtWorldPos(PintActorHandle handle, const Point& world_impulse, const Point& world_pos) {
        NotImplemented("AddWorldImpulseAtWorldPos");
    }
    virtual void AddLocalTorque(PintActorHandle handle, const Point& local_torque) { NotImplemented("AddLocalTorque"); }

    // TODO: move to Pint_Actor API
    virtual Point GetLinearVelocity(PintActorHandle handle) {
        NotImplemented("GetLinearVelocity");
        return Point(0.0f, 0.0f, 0.0f);
    }
    virtual void SetLinearVelocity(PintActorHandle handle, const Point& linear_velocity) {
        NotImplemented("SetLinearVelocity");
    }

    // TODO: move to Pint_Actor API
    virtual Point GetAngularVelocity(PintActorHandle handle) {
        NotImplemented("GetAngularVelocity");
        return Point(0.0f, 0.0f, 0.0f);
    }
    virtual void SetAngularVelocity(PintActorHandle handle, const Point& angular_velocity) {
        NotImplemented("SetAngularVelocity");
    }

#ifdef DEPRECATED
    virtual Point GetWorldLinearVelocity(PintActorHandle handle) {
        NotImplemented("GetWorldLinearVelocity");
        return Point(0.0f, 0.0f, 0.0f);
    }
    virtual void SetWorldLinearVelocity(PintActorHandle handle, const Point& linear_velocity) {
        NotImplemented("SetWorldLinearVelocity");
    }

    virtual Point GetWorldAngularVelocity(PintActorHandle handle) {
        NotImplemented("GetWorldAngularVelocity");
        return Point(0.0f, 0.0f, 0.0f);
    }
    virtual void SetWorldAngularVelocity(PintActorHandle handle, const Point& angular_velocity) {
        NotImplemented("SetWorldAngularVelocity");
    }

    virtual float GetMass(PintActorHandle handle) {
        NotImplemented("GetMass");
        return 0.0f;
    }
    ...
    virtual Point GetLocalInertia(PintActorHandle handle) {
        NotImplemented("GetLocalInertia");
        return Point(0.0f, 0.0f, 0.0f);
    }
    ...
    // TODO: move to Pint_Actor API
    virtual bool SetKinematicPose(PintActorHandle handle, const Point& pos) {
        return NotImplemented("SetKinematicPose");
    }
    ...
    virtual PintAggregateHandle CreateAggregate(udword max_size, bool enable_self_collision) {
        NotImplemented("CreateAggregate");
        return null;
    }
    inline_ PintAggregateHandle CreateAggregate(const PINT_AGGREGATE_CREATE& desc) {
        return CreateAggregate(desc.mMaxSize, desc.mSelfCollision);
    }
    virtual bool AddToAggregate(PintActorHandle object, PintAggregateHandle aggregate) {
        return NotImplemented("AddToAggregate");
    }
    ...
    virtual PintArticHandle CreateArticulation(const PINT_ARTICULATION_CREATE&) {
        NotImplemented("CreateArticulation");
        return null;
    }
    virtual PintActorHandle CreateArticulatedObject(const PINT_OBJECT_CREATE&,
                                                    const PINT_ARTICULATED_BODY_CREATE&,
                                                    PintArticHandle articulation) {
        NotImplemented("CreateArticulatedObject");
        return null;
    }
    virtual bool AddArticulationToScene(PintArticHandle articulation) {
        return NotImplemented("AddArticulationToScene");
    }
    virtual void SetArticulatedMotor(PintActorHandle object, const PINT_ARTICULATED_MOTOR_CREATE& motor) {
        NotImplemented("SetArticulatedMotor");
    }
    ...
    virtual PintArticHandle CreateRCArticulation(const PINT_RC_ARTICULATION_CREATE&) {
        NotImplemented("CreateRCArticulation");
        return null;
    }
    virtual PintActorHandle CreateRCArticulatedObject(const PINT_OBJECT_CREATE&,
                                                      const PINT_RC_ARTICULATED_BODY_CREATE&,
                                                      PintArticHandle articulation) {
        NotImplemented("CreateRCArticulatedObject");
        return null;
    }
    virtual bool AddRCArticulationToScene(PintArticHandle articulation) {
        return NotImplemented("AddRCArticulationToScene");
    }
    //		virtual	void				SetRCArticulatedMotor(PintActorHandle object, const PINT_ARTICULATED_MOTOR_CREATE& motor)
    //{ NotImplemented("SetRCArticulatedMotor");						}
    virtual bool AddRCArticulationToAggregate(PintArticHandle articulation, PintAggregateHandle aggregate) {
        return NotImplemented("AddRCArticulationToAggregate");
    }
    ...
    virtual bool SetRCADriveVelocity(PintActorHandle handle, float velocity) {
        return NotImplemented("SetRCADriveVelocity");
    }
    ...
    virtual bool SetDriveVelocity(PintJointHandle handle, const Point& linear, const Point& angular) {
        return NotImplemented("SetDriveVelocity");
    }
    virtual PR GetDrivePosition(PintJointHandle handle) {
        NotImplemented("GetDrivePosition");
        return PR(Idt);
    }
    //		virtual	void				SetGearJointError(PintJointHandle handle, float error)								{
    //NotImplemented("SetGearJointError");					}
    virtual void SetPortalJointRelativePose(PintJointHandle handle, const PR& pose) {
        NotImplemented("SetPortalJointRelativePose");
    }

    // New APIs - WIP - eventually everything should move to these domain-specific APIs
    virtual Pint_Scene* GetSceneAPI() {
        NotImplemented("GetSceneAPI");
        return null;
    }
    virtual Pint_Actor* GetActorAPI() {
        NotImplemented("GetActorAPI");
        return null;
    }
    virtual Pint_Shape* GetShapeAPI() {
        NotImplemented("GetShapeAPI");
        return null;
    }
    virtual Pint_Joint* GetJointAPI() {
        NotImplemented("GetJointAPI");
        return null;
    }
    //		virtual	Pint_Mesh*			GetMeshAPI()		{ NotImplemented("GetMeshAPI");			return null;
    //}
    virtual Pint_Vehicle* GetVehicleAPI() {
        NotImplemented("GetVehicleAPI");
        return null;
    }
    virtual Pint_Character* GetCharacterAPI() {
        NotImplemented("GetCharacterAPI");
        return null;
    }
...
public:
    ...
    //		virtual	bool	SetGravity(/*PintSceneHandle handle,*/ const Point& gravity)	{ return
    //mPint.NotImplemented("Pint_Scene::SetGravity");							} 		virtual
    //Point	GetGravity(/*PintSceneHandle handle*/)	const					{ mPint.NotImplemented("Pint_Scene::GetGravity");
    //return Point(0.0f, 0.0f, 0.0f);	}

    virtual bool AddActors(udword nb_actors, const PintActorHandle* actors) {
        return mPint.NotImplemented("Pint_Scene::AddActors");
    }

    virtual void GetActors(Reporter& reporter) const { mPint.NotImplemented("Pint_Scene::GetActors"); }
    virtual void Cull(udword nb_planes, const Plane* planes, Reporter& reporter) const {
        mPint.NotImplemented("Pint_Scene::Cull");
    }
...
public:
    ...
    virtual bool GetLocalInertia(PintActorHandle handle, Point& inertia) const {
        return mPint.NotImplemented("Pint_Actor::GetLocalInertia");
    }
    virtual bool SetLocalInertia(PintActorHandle handle, const Point& inertia) {
        return mPint.NotImplemented("Pint_Actor::SetLocalInertia");
    }

    virtual bool GetCMassLocalPose(PintActorHandle handle, PR& pose) const {
        return mPint.NotImplemented("Pint_Actor::GetCMassLocalPose");
    }
    virtual bool SetCMassLocalPose(PintActorHandle handle, const PR& pose) {
        return mPint.NotImplemented("Pint_Actor::SetCMassLocalPose");
    }
...
public:
    ...
    virtual const char* GetName(PintShapeHandle handle) const {
        mPint.NotImplemented("Pint_Shape::GetName");
        return null;
    }
    virtual bool SetName(PintShapeHandle handle, const char* name) {
        return mPint.NotImplemented("Pint_Shape::SetName");
    }

    virtual PintShape GetType(PintShapeHandle handle) const {
        mPint.NotImplemented("Pint_Shape::GetType");
        return PINT_SHAPE_UNDEFINED;
    }

    virtual PR GetWorldTransform(PintActorHandle actor, PintShapeHandle shape) const {
        mPint.NotImplemented("Pint_Shape::GetWorldTransform");
        return PR(Idt);
    }

    virtual bool GetWorldBounds(PintActorHandle actor, PintShapeHandle shape, AABB& bounds) const {
        return mPint.NotImplemented("Pint_Shape::GetWorldBounds");
    }

    // Mesh-related, temp design/location
    virtual bool GetTriangleMeshData(SurfaceInterface& surface, PintShapeHandle handle, bool) const {
        return mPint.NotImplemented("Pint_Shape::GetTriangleMeshData");
    }
    virtual bool GetTriangle(Triangle& tri, PintShapeHandle handle, udword index) const {
        return mPint.NotImplemented("Pint_Shape::GetTriangle");
    }
    virtual bool GetIndexedTriangle(IndexedTriangle& tri, PintShapeHandle handle, udword index) const {
        return mPint.NotImplemented("Pint_Shape::GetIndexedTriangle");
    }
    virtual bool FindTouchedTriangles(Container& indices,
                                      PintSQThreadContext context,
                                      PintShapeHandle handle,
                                      const PR& pose,
                                      const PintSphereOverlapData& sphere) const {
        return mPint.NotImplemented("Pint_Shape::FindTouchedTriangles");
    }
    virtual bool Refit(PintShapeHandle shape, PintActorHandle actor) {
        return mPint.NotImplemented("Pint_Shape::Refit");
    }
...
public:
    ...
    virtual const char* GetName(PintJointHandle handle) const {
        mPint.NotImplemented("Pint_Joint::GetName");
        return null;
    }
    virtual bool SetName(PintJointHandle handle, const char* name) {
        return mPint.NotImplemented("Pint_Joint::SetName");
    }

    virtual PintJoint GetType(PintJointHandle handle) const {
        mPint.NotImplemented("Pint_Joint::GetType");
    ...
    virtual bool GetActors(PintJointHandle handle, PintActorHandle& actor0, PintActorHandle& actor1) const {
        return mPint.NotImplemented("Pint_Joint::GetActors");
    }
    virtual bool SetActors(PintJointHandle handle, PintActorHandle actor0, PintActorHandle actor1) {
        return mPint.NotImplemented("Pint_Joint::SetActors");
    }

    virtual bool GetFrames(PintJointHandle handle, PR* frame0 = null, PR* frame1 = null) const {
        return mPint.NotImplemented("Pint_Joint::GetFrames");
    }
    virtual bool SetFrames(PintJointHandle handle, const PR* frame0 = null, const PR* frame1 = null) {
        return mPint.NotImplemented("Pint_Joint::SetFrames");
    }

    // ### purely experimental design
    virtual bool GetLimits(PintJointHandle handle, PintLimits& limits, udword index) const {
        return mPint.NotImplemented("Pint_Joint::GetLimits");
    }
    virtual bool SetLimits(PintJointHandle handle, const PintLimits& limits, udword index) {
        return mPint.NotImplemented("Pint_Joint::SetLimits");
    }

    virtual bool GetSpring(PintJointHandle handle, PintSpring& spring) const {
        return mPint.NotImplemented("Pint_Joint::GetSpring");
    }
    virtual bool SetSpring(PintJointHandle handle, const PintSpring& spring) {
        return mPint.NotImplemented("Pint_Joint::SetSpring");
    }

    virtual bool GetGearRatio(PintJointHandle handle, float& ratio) const {
        return mPint.NotImplemented("Pint_Joint::GetGearRatio");
    }
    virtual bool SetGearRatio(PintJointHandle handle, float ratio) {
        return mPint.NotImplemented("Pint_Joint::SetGearRatio");
    }

    // For hinge & D6 joints
    virtual bool GetHingeDynamicData(PintJointHandle handle, PintHingeDynamicData& data) const {
        return mPint.NotImplemented("GetHingeDynamicData");
    }
    // For D6
    virtual bool GetD6DynamicData(PintJointHandle handle, PintD6DynamicData& data) const {
        return mPint.NotImplemented("GetD6DynamicData");
    }
...
/*	class Pint_Mesh : public Allocateable
        {
                ...
                virtual	bool	FindTouchedTriangles(Container& indices, PintSQThreadContext context, PintMeshHandle
   handle, const PintSphereOverlapData& sphere)	const	{ return
   mPint.NotImplemented("Pint_Shape::FindTouchedTriangles");	}
...
struct PINT_VEHICLE_CREATE : public Allocateable {
    PINT_VEHICLE_CREATE()
        ...
        const float DefaultSteerVsForwardSpeedData[8] = {
                0.0f * 3.6f, 1.0f, 5.0f * 3.6f, 0.75f, 30.0f * 3.6f, 0.125f, 120.0f * 3.6f, 0.1f,
        };
        CopyMemory(mSteerVsForwardSpeedData, DefaultSteerVsForwardSpeedData, sizeof(float) * 8);

        const float DefaultSmoothingData[] = {
                //				Rise		Fall
                // Keyboard
                /*Accel*/ 3.0f,
                5.0f,
    ...
    // "A bit of tweaking here.  The car will have more responsive turning if we reduce the y-component of the chassis
    // moment of inertia."
    ...
    // Wheels
    const PINT_SHAPE_CREATE*
            mWheels[4];     // Unique for shared shapes, or define 4 different shapes. Must be convex shapes for now.
...
struct PintVehicleData {
    inline_ PintVehicleData() : mChassisActor(null), mChassisShape(null) {
        for (udword i = 0; i < 4; i++) mWheelShapes[i] = null;
    }
...
public:
    ...
    virtual PintVehicleHandle CreateVehicle(PintVehicleData& data, const PINT_VEHICLE_CREATE& vehicle) {
        mPint.NotImplemented("Pint_Vehicle::CreateVehicle");
        return null;
    }
    virtual bool SetVehicleInput(PintVehicleHandle vehicle, const PINT_VEHICLE_INPUT& input) {
        return mPint.NotImplemented("Pint_Vehicle::SetVehicleInput");
    }
    virtual PintActorHandle GetVehicleActor(PintVehicleHandle vehicle) const {
        mPint.NotImplemented("Pint_Vehicle::GetVehicleActor");
        return null;
    }
    virtual bool GetVehicleInfo(PintVehicleHandle vehicle, PintVehicleInfo& info) const {
        return mPint.NotImplemented("Pint_Vehicle::GetVehicleInfo");
    }
    virtual bool ResetVehicleData(PintVehicleHandle vehicle) {
        return mPint.NotImplemented("Pint_Vehicle::ResetVehicleData");
    }
    virtual bool AddActor(PintVehicleHandle vehicle, PintActorHandle actor) {
        return mPint.NotImplemented("Pint_Vehicle::AddActor");
    }
    virtual bool AddShape(PintVehicleHandle vehicle, const PINT_SHAPE_CREATE& create) {
        return mPint.NotImplemented("Pint_Vehicle::AddShape");
    }
...
struct PINT_CHARACTER_CREATE : public Allocateable {
    PINT_CHARACTER_CREATE() { mPosition = Point(0.0f, 0.0f, 0.0f); }
...
public:
    ...
    virtual PintCharacterHandle CreateCharacter(const PINT_CHARACTER_CREATE& create) {
        mPint.NotImplemented("CreateCharacter");
        return null;
    }
    virtual PintActorHandle GetCharacterActor(PintCharacterHandle h) {
        mPint.NotImplemented("GetCharacterActor");
        return null;
    }
    virtual udword MoveCharacter(PintCharacterHandle h, const Point& disp) {
        mPint.NotImplemented("MoveCharacter");
        return 0;
    }
