...
struct Thread : public ThreadT<UtilAllocator> {
    Thread(ThreadEntryPoint entryPoint, void* data) : ThreadT<UtilAllocator>(), mEntryPoint(entryPoint), mData(data) {}

    virtual void execute(void) { mEntryPoint(mData); }
...
struct PEEL_Thread {
    inline_ PEEL_Thread()
        : mWorkReadySyncHandle(null),
          mThreadHandle(null),
          mWorkDoneSyncHandle(null),
          mCallback(null),
          mUserData(null) {}
...
static void threadExecute(void* data) {
    ...
    for (;;) {
        ...
        if (::threadQuitIsSignalled(testThread->mThreadHandle)) break;
...
void PEEL_ReleaseThreads() {
    ...
    for (PxU32 i = 0; i < gNumThreads; i++) ::syncRelease(gThreads[i].mWorkDoneSyncHandle);
}
...
void PEEL_StartThreadWork(udword nb_threads) {
    ...
    for (udword i = 0; i < nb_threads; i++) ::syncSet(gThreads[i].mWorkReadySyncHandle);
}
        ...
        if(gUseMultipleThreads)
        {
                ...
                for(PxU32 j=0;j<4;j++)
                {
                        for(PxU32 i=0;i<4;i++)
                        {
                                ...
                                gThreads[index++].Setup(this, &rp, buffer + offset, camPos, fScreenWidth, fScreenHeight,
   i, j);
                        }
                }
