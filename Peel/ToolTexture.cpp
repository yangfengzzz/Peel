...
void ToolTexture::Reset(udword pint_index) {
    ...
    if (mTextureComboBox) mTextureComboBox->RemoveAll();
    if (mTextureComboBox2) mTextureComboBox2->RemoveAll();
    mMinX = mMinY = 10;
    mMaxX = mMaxY = 10;
    mOffsetX = mOffsetY = 0;
    mCapture = false;
...
void ToolTexture::RightUpCallback(Pint& pint, udword pint_index) { mCapture = false; }
...
void ToolTexture::UpdateUI() {
    if (mTextureComboBox) {
        ...
        for (udword i = 0; i < NbTextures; i++) {
            ...
            if (!UIName) UIName = _F("(Unnamed texture %d)", i);
...
void ToolTexture::FinalRenderCallback() {
    if (!mTextureComboBox && !mTextureComboBox2) return;
...

    {
        ...
        if (mMinX > LimitX) mMinX = LimitX;
        if (mMinY > LimitY) mMinY = LimitY;
        if (mMaxX < BorderSize) {
            mMaxX = BorderSize;
    ...
    if (GLID) {
        ...
        GLRenderHelpers::DrawRectangle(x_start, x_end, y_start, y_end, color, color, Alpha, screen_width, screen_height,
...
void ToolTexture::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
    ...
    mEditBox_Width = helper.CreateEditBox(parent, 0, x + OffsetX, y, EditBoxWidth, 20, "-", &owner,
    ...
    mEditBox_Height = helper.CreateEditBox(parent, 1, x + OffsetX, y, EditBoxWidth, 20, "-", &owner,
                                           EDITBOX_INTEGER_POSITIVE, null, null);
    mEditBox_Height->SetEnabled(false);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 100, 20, "Scale:", &owner);
    mEditBox_Scale = helper.CreateEditBox(parent, 1, x + OffsetX, y, EditBoxWidth, 20, "1.0", &owner,
