...
ToolTranslate::ToolTranslate()
    : mCachedOrigin(Point(0.0f, 0.0f, 0.0f)), mCachedDir(Point(0.0f, 0.0f, 0.0f)), mDrag(false) {}
...
void ToolTranslate::Reset(udword pint_index) {
    ...
    if (pint_index == INVALID_ID) {
        for (udword i = 0; i < MAX_NB_ENGINES; i++) mData[i].Reset();
    } else {
...
void ToolTranslate::RightDownCallback(Pint& pint, udword pint_index) {
    ...
    if (Raycast(pint, Hit, mOrigin, mDir) && !IsDefaultEnv(pint, Hit.mTouchedActor)) {
        ...
        TD.Reset();
#ifdef SUPPORT_TRANSLATE_TOOL_UI
        if (mEditPos) mEditPos->Reset();
#endif
    }
}

void ToolTranslate::RightDragCallback(Pint& pint, udword pint_index) { mDrag = true; }
...
void ToolTranslate::RenderCallback(PintRender& render, Pint& pint, udword pint_index) {
    ...
    // Recompute camera vectors to avoid stuttering when the camera moves (say because of key presses) while one object
    // is manipulated.
    ...
    if (TD.mObject) {
...
#ifdef SUPPORT_TRANSLATE_TOOL_UI
            if (mEditPos) mEditPos->SetPos(Pose.mPos);
#endif
        }
        ...
        if (mEditPos) mEditPos->SetEnabled(false);
    }
#endif
}
...
void ToolTranslate::PostRenderCallback() {
    mDrag = false;
#ifdef SUPPORT_TRANSLATE_TOOL_UI
    if (mEditPos) mEditPos->mHasEdit = false;
#endif
}
...
void ToolTranslate::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
    ...
    /*	helper.CreateLabel(parent, x, y+LabelOffsetY, 90, 20, "X:", &owner);
            mEditBoxX = helper.CreateEditBox(parent, X_, x+OffsetX, y, EditBoxWidth, 20, helper.Convert(gEdited.x),
       &owner, EDITBOX_FLOAT, Local::gEBCallback, null); y += YStep;

            helper.CreateLabel(parent, x, y+LabelOffsetY, 90, 20, "Y:", &owner);
            mEditBoxY = helper.CreateEditBox(parent, Y_, x+OffsetX, y, EditBoxWidth, 20, helper.Convert(gEdited.y),
       &owner, EDITBOX_FLOAT, Local::gEBCallback, null); y += YStep;

            helper.CreateLabel(parent, x, y+LabelOffsetY, 90, 20, "Z:", &owner);
            mEditBoxZ = helper.CreateEditBox(parent, Z_, x+OffsetX, y, EditBoxWidth, 20, helper.Convert(gEdited.z),
       &owner, EDITBOX_FLOAT, Local::gEBCallback, null); y += YStep;*/
