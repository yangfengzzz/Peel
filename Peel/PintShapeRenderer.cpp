...
udword GetNbRenderSources() { return gRenderSources ? gRenderSources->GetNbEntries() : 0; }

RenderDataChunk* GetRenderSource(udword i) {
    if (!gRenderSources) return null;
...
static void RegisterRenderSource(PintShapeRenderer* renderer) {
    ...
    if (renderer->mRenderSource) {
        if (!gRenderSources) gRenderSources = ICE_NEW(PtrContainer);
...
udword GetNbShapeRenderers() {
    udword Nb = 0;
    if (gShapeRenderers) Nb += gShapeRenderers->GetNbEntries();
...
PintShapeRenderer* GetShapeRenderer(udword i) {
    if (!gShapeRenderers) return null;
...
bool ReleaseShapeRenderer(PintShapeRenderer* renderer) {
    ...
    if (gShapeRenderers && gShapeRenderers->Delete(renderer)) Found = true;
    ...
    if (Found) ICE_SAFE_RELEASE(renderer);
    return Found;
}
...
static PintShapeRenderer* _RegisterShapeRenderer(PtrContainer*& renderers, PintShapeRenderer* renderer) {
    if (renderer) {
        ...
        if (!renderers) renderers = ICE_NEW(PtrContainer);
        ASSERT(renderers);
...
PintShapeRenderer* CreateNullRenderer() { return &gNullRenderer; }

///////////////////////////////////////////////////////////////////////////////

// New management of shape renderers:
// 1) make private ctor/dtor for each shape renderer class you want to track.  Now you cannot allocate them all over the
// place. 2) add a factory function (e.g. CreatePintDLSphereShapeRenderer), make it a friend of the shape renderer
// class. 3) redirect non-compiling shape renderer creation calls to factory function. 4) move the render source
// creation to new factory function. We'll have to skip that step for shared shape renderers. 5) call
// _RegisterNewShapeRenderer() instead of _RegisterShapeRenderer(). 6) add a container for newly created shape
// renderers, store new ptr in it (KeepTrackOfRendererForSharing). 7) delete that container in
// ReleaseAllShapeRenderers() 8) iterate that container at the start of factory function, looking for existing shareable
// renderer. Might need to store previously unstored creation param in the renderer there. 9) add a customized
// GetOwnerContainer() function that returns the new container.
...
PintShapeRenderer* CreateSphereRenderer(float radius, bool geo_sphere) {
    if (gUseNullRenderer) return &gNullRenderer;
...
PintDLSphereShapeRenderer* CreatePintDLSphereShapeRenderer(float radius, bool uses_vertex_normals, bool geo_sphere) {
    PintDLSphereShapeRenderer* Shared = FindRenderer<PintDLSphereShapeRenderer>(
            gPintDLSphereShapeRenderers, PintDLSphereShapeRenderer::Data(radius, uses_vertex_normals, geo_sphere));
    if (Shared) return Shared;

    PintDLSphereShapeRenderer* NewRenderer =
            ICE_NEW(PintDLSphereShapeRenderer)(radius, uses_vertex_normals, geo_sphere);
    return KeepTrackOfRendererForSharing(gPintDLSphereShapeRenderers, NewRenderer);
}

PtrContainer* PintDLSphereShapeRenderer::GetOwnerContainer() const { return gPintDLSphereShapeRenderers; }
...
PintDLCapsuleShapeRenderer* CreatePintDLCapsuleShapeRenderer(float radius, float height, bool uses_vertex_normals) {
    PintDLCapsuleShapeRenderer* Shared = FindRenderer<PintDLCapsuleShapeRenderer>(
            gPintDLCapsuleShapeRenderers, PintDLCapsuleShapeRenderer::Data(radius, height, uses_vertex_normals));
    if (Shared) return Shared;
...
PtrContainer* PintDLCapsuleShapeRenderer::GetOwnerContainer() const { return gPintDLCapsuleShapeRenderers; }
...
PintDLCylinderShapeRenderer* CreatePintDLCylinderShapeRenderer(float radius, float height) {
    PintDLCylinderShapeRenderer* Shared = FindRenderer<PintDLCylinderShapeRenderer>(
            gPintDLCylinderShapeRenderers, PintDLCylinderShapeRenderer::Data(radius, height));
    if (Shared) return Shared;
...
PtrContainer* PintDLCylinderShapeRenderer::GetOwnerContainer() const { return gPintDLCapsuleShapeRenderers; }
...
PintShapeRenderer* CreateBoxRenderer(const Point& extents) {
    ...
    /*#else
            if(1)
            {
                    return RegisterShapeRenderer(ICE_NEW(PintDLBoxShapeRenderer)(extents,
    gCurrentRenderModel->NeedsVertexNormals()));
            }
            else
            {
                    AABB box;
...
PintDLBoxShapeRenderer* CreatePintDLBoxShapeRenderer(const Point& extents, bool uses_vertex_normals) {
    PintDLBoxShapeRenderer* Shared = FindRenderer<PintDLBoxShapeRenderer>(
            gPintDLBoxShapeRenderers, PintDLBoxShapeRenderer::Data(extents, uses_vertex_normals));
    if (Shared) return Shared;
...
PtrContainer* PintDLBoxShapeRenderer::GetOwnerContainer() const { return gPintDLBoxShapeRenderers; }
...
PintShapeRenderer* CreateMeshRenderer(const PintSurfaceInterface& surface,
                                      const Point* normals,
                                      bool active_edges,
                                      bool direct_data) {
    if (gUseNullRenderer) return &gNullRenderer;

    PintShapeRenderer* NewRenderer =
            gCurrentRenderModel->_CreateMeshRenderer(surface, normals, active_edges, direct_data);

    if (gUseEditor && gExportRenderSources && NewRenderer &&
        !NewRenderer->mRenderSource)  // mRenderSource will already exist for shared renderers
...
PintShapeRenderer* CreateMeshRenderer(const MultiSurface& multi_surface, bool active_edges, bool direct_data) {
    ...
    if (gDiscardUVs)
        return CreateMeshRenderer(PintSurfaceInterface(multi_surface.GetSurfaceInterface()), null, active_edges,
                                  direct_data);
...
PintDLMeshShapeRenderer* CreatePintDLMeshShapeRenderer(const PintSurfaceInterface& surface, udword flags) {
    PintDLMeshShapeRenderer* Shared = FindRenderer<PintDLMeshShapeRenderer>(
            gPintDLMeshShapeRenderers, PintDLMeshShapeRenderer::Data(surface, flags));
    if (Shared) return Shared;
...
PintDLMeshShapeRenderer* CreatePintDLMeshShapeRenderer(const MultiSurface& multi_surface, udword flags) {
    //	PintDLMeshShapeRenderer* Shared = FindRenderer<PintDLMeshShapeRenderer>(gPintDLMeshShapeRenderers,
    //PintDLMeshShapeRenderer::Data(flags)); 	if(Shared) 		return Shared;

    PintDLMeshShapeRenderer* NewRenderer = ICE_NEW(PintDLMeshShapeRenderer)(multi_surface, flags);
    return KeepTrackOfRendererForSharing(gPintDLMeshShapeRenderers, NewRenderer);
}

PtrContainer* PintDLMeshShapeRenderer::GetOwnerContainer() const { return gPintDLMeshShapeRenderers; }

///////////////////////////////////////////////////////////////////////////////

PintShapeRenderer* CreateColorShapeRenderer(PintShapeRenderer* source_renderer,
                                            const RGBAColor& color,
                                            const ManagedTexture* texture) {
    if (gUseNullRenderer) return &gNullRenderer;
        ...
        !NewRenderer->mRenderSource)  // mRenderSource will already exist for shared renderers
    {
        ...
        //		if(AdjustedColor)
        //			NewRenderer->mRenderSource = ICE_NEW(ColorRenderDataChunk)(source_renderer->mRenderSource,
        //*AdjustedColor, texture); 		else
...
PintColorShapeRenderer* CreatePintColorShapeRenderer(PintShapeRenderer* source_renderer,
                                                     const RGBAColor& color,
                                                     const ManagedTexture* texture) {
    PintColorShapeRenderer* Shared = FindRenderer<PintColorShapeRenderer>(
            gPintColorShapeRenderers, PintColorShapeRenderer::Data(source_renderer, color, texture));
    if (Shared) return Shared;
...
PtrContainer* PintColorShapeRenderer::GetOwnerContainer() const { return gPintColorShapeRenderers; }
...
PintShapeRenderer* CreateRendererCollection() {
    ...
    if (gUseEditor && renderer && gExportRenderSources) renderer->mRenderSource = ICE_NEW(RenderDataChunkCollection);
