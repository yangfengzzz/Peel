...
static const char* gLayerNames[] = {
        "Bitmap",   "Cellular", "Clamp",  "Constant", "Crater", "Erosion",  "fbm",        "Filter",
        "Midpoint", "Power",    "Ridged", "Scale",    "Sinus",  "Spectral", "Turbulence", "Multiply-Add",
};
...
static LayerData gLayers[] = {
        {"Multiply-Add", null, HEIGHT_LAYER_TYPE_MADD},     {"Power", null, HEIGHT_LAYER_TYPE_POWER},
        {"Clamp", null, HEIGHT_LAYER_TYPE_CLAMP},
#ifdef OBSOLETE
        {"Constant", null, HEIGHT_LAYER_TYPE_CONSTANT},     {"Scale", null, HEIGHT_LAYER_TYPE_SCALE},
#endif
        {"Sinus", null, HEIGHT_LAYER_TYPE_SINUS},           {"fbm", null, HEIGHT_LAYER_TYPE_FBM},
        {"Turbulence", null, HEIGHT_LAYER_TYPE_TURBULENCE}, {"Ridged", null, HEIGHT_LAYER_TYPE_RIDGED},
        {"Cellular", null, HEIGHT_LAYER_TYPE_CELLULAR},     {"Midpoint", null, HEIGHT_LAYER_TYPE_MIDPOINT},
...
void ToolTerrainEditor::RightDownCallback(Pint& pint, udword pint_index) {
    ...
    if (PreviouslySelected != mSelectedTerrain) UpdateUI();
}

void ToolTerrainEditor::RightDragCallback(Pint& pint, udword pint_index) {}

void ToolTerrainEditor::RightUpCallback(Pint& pint, udword pint_index) { mModifyTerrain = false; }

static CustomLayer* gTEST = null;

void ToolTerrainEditor::RenderCallback(PintRender& render, Pint& pint, udword pint_index) {
    if (mPendingAction == CREATE_GRID) CreateGridMesh(pint);
    ...
    PintRaycastHit Hit;
    if (mDir.IsZero() || !Raycast(pint, Hit, mOrigin, mDir)) return;
    ...
    GLRenderHelpers::DrawSphereWireframe(Overlap.mSphere.mRadius, PR(Overlap.mSphere.mCenter, Quat(Idt)),
    ...
    if (Type != PINT_SHAPE_MESH && Type != PINT_SHAPE_MESH2) return;
    ...
    if (!ShapeAPI->FindTouchedTriangles(TouchedTriangleIndices, pint.mSQHelper->GetThreadContext(), TerrainShape,
                                        TerrainPose, Overlap))
        return;

    SurfaceInterface SI;
    if (!ShapeAPI->GetTriangleMeshData(SI, TerrainShape, true)) return;
    ...
    for (udword i = 0; i < NbToucheddTris; i++) {
        if (0) {
            ...
            if (ShapeAPI->GetIndexedTriangle(T, TerrainShape, TouchedTriangleIndices[i])) {
                ...
                //				GLRenderHelpers::DrawTriangle(SI.mVerts[T.mRef[0]], SI.mVerts[T.mRef[1]],
                //SI.mVerts[T.mRef[2]], Point(1.0f, 0.0f, 0.0f));
    ...
    /*	static bool init=false;
            if(!init)
            {
                    ...
                    {
                            const Point& wp = V[i];
                            float w = Amplitude * 5.0f * fbm.Compute(wp*0.02f) * fbm.Compute(wp*0.01f);	// This one is
       pretty cool V[i].y += w;
                    }
            }*/
...
void ToolTerrainEditor::PostRenderCallback() { mPendingAction = NONE; }
...
bool ToolTerrainEditor::CreateGridMesh(Pint& pint) {
    {
        ...
        if (!caps.mSupportMeshes || !caps.mSupportDeformableMeshes) return false;
    }
...
void ToolTerrainEditor::OnListBoxLayerSelected() {
    ...
    for (udword i = 0; i < gNbLayers; i++) {
        ...
        if (gLayers[i].mParams) gLayers[i].mParams->SetVisible(IsSelected);
...
void ToolTerrainEditor::OnComboBoxLayerSelected() {
    ...
    for (udword i = 0; i < gNbLayers; i++) {
        if (gLayers[i].mParams) gLayers[i].mParams->SetVisible(i == Selected);
    }
}
...
void ToolTerrainEditor::UpdateUI() {
    ...
    if (mAddLayer) mAddLayer->SetEnabled(IsTerrainSelected);

    if (mRemoveLayer) mRemoveLayer->SetEnabled(IsTerrainSelected);
    ...
    for (udword i = 0; i < gNbLayers; i++) {
        if (gLayers[i].mParams) gLayers[i].mParams->SetVisible(false);
    }
}

void ToolTerrainEditor::AddLayer() {
    ASSERT(mSelectedTerrain);
    if (!mSelectedTerrain) return;
...
void ToolTerrainEditor::RemoveLayer() {
    ...
    if (!mSelectedTerrain || !mLayerListBox) return;
...
void ToolTerrainEditor::UpdateTerrain() {
    ...
    if (ShapeAPI->GetTriangleMeshData(SI, mSelectedTerrainShape, true)) {
        ...
        if (gTexture_AutoUpdate) UpdateTexture();
    }
}
...
/*	else
        {
                const ManagedTexture* MT = CreateManagedTexture(TT.GetWidth(), TT.GetHeight(), TT.GetPixels(),
   "TerrainTexture");
        }*/
#endif
}
...
HeightLayer* ToolTerrainEditor::GetSelectedLayer() const {
    ...
    if (Selected == INVALID_ID) return null;
...
static void CreateTilesTab(PintGUIHelper& helper, IceWindow* tab, Widgets& owner, ToolTerrainEditor* tte) {
    ...
    helper.CreateEditBox(tab, EDIT_BOX_TILE_NB, x + OffsetX, y, EditBoxWidth, 20, _F("%d", gTileNb), &owner,
                         EDITBOX_INTEGER_POSITIVE, Callbacks::gEditBox, null);
    y += YStep;

    helper.CreateLabel(tab, x, y + LabelOffsetY, 50, 20, "Tile size:", &owner);
    helper.CreateEditBox(tab, EDIT_BOX_TILE_SIZE, x + OffsetX, y, EditBoxWidth, 20, _F("%d", gTileSize), &owner,
                         EDITBOX_INTEGER_POSITIVE, Callbacks::gEditBox, null);
    y += YStep;

    helper.CreateLabel(tab, x, y + LabelOffsetY, 50, 20, "Tile scale:", &owner);
    helper.CreateEditBox(tab, EDIT_BOX_TILE_SCALE, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gTileScale), &owner,
                         EDITBOX_FLOAT_POSITIVE, Callbacks::gEditBox, null);
    y += YStep;
...
static void FillUI(const HeightLayer& layer) {
    ASSERT(gCheckBox_EnableLayer);
    gCheckBox_EnableLayer->SetChecked(layer.IsLayerEnabled());

    switch (layer.GetType()) {
        case HEIGHT_LAYER_TYPE_MADD: {
            const MaddLayer& L = static_cast<const MaddLayer&>(layer);
            ASSERT(gEditBox_MaddLayer_Amplitude);
            gEditBox_MaddLayer_Amplitude->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gEditBox_MaddLayer_Altitude);
            gEditBox_MaddLayer_Altitude->SetText(_F("%f", L.GetAltitude()));
        } break;

        case HEIGHT_LAYER_TYPE_POWER: {
            const PowerLayer& L = static_cast<const PowerLayer&>(layer);
            // ASSERT(gEditBox_PowerLayer_Amplitude);	gEditBox_PowerLayer_Amplitude->SetText(_F("%f",
            // L.GetAmplitude()));
            ASSERT(gEditBox_PowerLayer_Factor);
            gEditBox_PowerLayer_Factor->SetText(_F("%f", L.GetPowerFactor()));
        } break;

        case HEIGHT_LAYER_TYPE_CLAMP: {
            const ClampLayer& L = static_cast<const ClampLayer&>(layer);
            ASSERT(gEditBox_ClampLayer_Height);
            gEditBox_ClampLayer_Height->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gCheckBox_ClampLayer_Bool);
            gCheckBox_ClampLayer_Bool->SetChecked(L.GetCutGreaterThan());
        } break;

#ifdef OBSOLETE
        case HEIGHT_LAYER_TYPE_CONSTANT: {
            const ConstantLayer& L = static_cast<const ConstantLayer&>(layer);
            ASSERT(gEditBox_ConstantLayer_Height);
            gEditBox_ConstantLayer_Height->SetText(_F("%f", L.GetConstantHeight()));
        } break;

        case HEIGHT_LAYER_TYPE_SCALE: {
            const ScaleLayer& L = static_cast<const ScaleLayer&>(layer);
            ASSERT(gEditBox_ScaleLayer_Scale);
            gEditBox_ScaleLayer_Scale->SetText(_F("%f", L.GetScaleFactor()));
        } break;
#endif

        case HEIGHT_LAYER_TYPE_SINUS: {
            const SinusLayer& L = static_cast<const SinusLayer&>(layer);
            ASSERT(gEditBox_SinusLayer_Amplitude);
            gEditBox_SinusLayer_Amplitude->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gEditBox_SinusLayer_XFreq);
            gEditBox_SinusLayer_XFreq->SetText(_F("%f", L.GetXFreq()));
            ASSERT(gEditBox_SinusLayer_YFreq);
            gEditBox_SinusLayer_YFreq->SetText(_F("%f", L.GetYFreq()));
            ASSERT(gEditBox_SinusLayer_PhaseX);
            gEditBox_SinusLayer_PhaseX->SetText(_F("%f", L.GetPhaseX()));
            ASSERT(gEditBox_SinusLayer_PhaseY);
            gEditBox_SinusLayer_PhaseY->SetText(_F("%f", L.GetPhaseY()));
        } break;

        case HEIGHT_LAYER_TYPE_FBM: {
            const FBMLayer& L = static_cast<const FBMLayer&>(layer);
            ASSERT(gEditBox_fbmLayer_Amplitude);
            gEditBox_fbmLayer_Amplitude->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gEditBox_fbmLayer_PhaseX);
            gEditBox_fbmLayer_PhaseX->SetText(_F("%f", L.GetOffsetX()));
            ASSERT(gEditBox_fbmLayer_PhaseY);
            gEditBox_fbmLayer_PhaseY->SetText(_F("%f", L.GetOffsetY()));
            ASSERT(gEditBox_fbmLayer_XFreq);
            gEditBox_fbmLayer_XFreq->SetText(_F("%f", L.GetXFreq()));
            ASSERT(gEditBox_fbmLayer_YFreq);
            gEditBox_fbmLayer_YFreq->SetText(_F("%f", L.GetYFreq()));
            ASSERT(gEditBox_fbmLayer_Z);
            gEditBox_fbmLayer_Z->SetText(_F("%f", L.GetZ()));
            ASSERT(gEditBox_fbmLayer_FractalInc);
            gEditBox_fbmLayer_FractalInc->SetText(_F("%f", L.GetFBM().GetFractalIncrement()));
            ASSERT(gEditBox_fbmLayer_Lacunarity);
            gEditBox_fbmLayer_Lacunarity->SetText(_F("%f", L.GetFBM().GetLacunarity()));
            ASSERT(gEditBox_fbmLayer_Octaves);
            gEditBox_fbmLayer_Octaves->SetText(_F("%f", L.GetFBM().GetOctaves()));
        } break;

        case HEIGHT_LAYER_TYPE_TURBULENCE: {
            const TurbulenceLayer& L = static_cast<const TurbulenceLayer&>(layer);
            ASSERT(gEditBox_TurbLayer_Amplitude);
            gEditBox_TurbLayer_Amplitude->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gEditBox_TurbLayer_PhaseX);
            gEditBox_TurbLayer_PhaseX->SetText(_F("%f", L.GetOffsetX()));
            ASSERT(gEditBox_TurbLayer_PhaseY);
            gEditBox_TurbLayer_PhaseY->SetText(_F("%f", L.GetOffsetY()));
            ASSERT(gEditBox_TurbLayer_XFreq);
            gEditBox_TurbLayer_XFreq->SetText(_F("%f", L.GetXFreq()));
            ASSERT(gEditBox_TurbLayer_YFreq);
            gEditBox_TurbLayer_YFreq->SetText(_F("%f", L.GetYFreq()));
            ASSERT(gEditBox_TurbLayer_Z);
            gEditBox_TurbLayer_Z->SetText(_F("%f", L.GetZ()));
            ASSERT(gEditBox_TurbLayer_Octaves);
            gEditBox_TurbLayer_Octaves->SetText(_F("%f", L.GetOctaves()));
        } break;

        case HEIGHT_LAYER_TYPE_RIDGED: {
            const RidgedFractalLayer& L = static_cast<const RidgedFractalLayer&>(layer);
            ASSERT(gEditBox_RidgedLayer_Amplitude);
            gEditBox_RidgedLayer_Amplitude->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gEditBox_RidgedLayer_PhaseX);
            gEditBox_RidgedLayer_PhaseX->SetText(_F("%f", L.GetOffsetX()));
            ASSERT(gEditBox_RidgedLayer_PhaseY);
            gEditBox_RidgedLayer_PhaseY->SetText(_F("%f", L.GetOffsetY()));
            ASSERT(gEditBox_RidgedLayer_XFreq);
            gEditBox_RidgedLayer_XFreq->SetText(_F("%f", L.GetXFreq()));
            ASSERT(gEditBox_RidgedLayer_YFreq);
            gEditBox_RidgedLayer_YFreq->SetText(_F("%f", L.GetYFreq()));
            ASSERT(gEditBox_RidgedLayer_Z);
            gEditBox_RidgedLayer_Z->SetText(_F("%f", L.GetZ()));
            ASSERT(gEditBox_RidgedLayer_FractalInc);
            gEditBox_RidgedLayer_FractalInc->SetText(_F("%f", L.GetRidgedFractal().GetFractalIncrement()));
            ASSERT(gEditBox_RidgedLayer_Lacunarity);
            gEditBox_RidgedLayer_Lacunarity->SetText(_F("%f", L.GetRidgedFractal().GetLacunarity()));
            ASSERT(gEditBox_RidgedLayer_Octaves);
            gEditBox_RidgedLayer_Octaves->SetText(_F("%f", L.GetRidgedFractal().GetOctaves()));
            ASSERT(gEditBox_RidgedLayer_Offset);
            gEditBox_RidgedLayer_Offset->SetText(_F("%f", L.GetRidgedFractal().GetOffset()));
            ASSERT(gEditBox_RidgedLayer_Gain);
            gEditBox_RidgedLayer_Gain->SetText(_F("%f", L.GetRidgedFractal().GetGain()));
        } break;

        case HEIGHT_LAYER_TYPE_CELLULAR: {
            const CellularLayer& L = static_cast<const CellularLayer&>(layer);
            ASSERT(gEditBox_CellularLayer_Amplitude);
            gEditBox_CellularLayer_Amplitude->SetText(_F("%f", L.GetAmplitude()));
            ASSERT(gEditBox_CellularLayer_PhaseX);
            gEditBox_CellularLayer_PhaseX->SetText(_F("%f", L.GetOffsetX()));
            ASSERT(gEditBox_CellularLayer_PhaseY);
            gEditBox_CellularLayer_PhaseY->SetText(_F("%f", L.GetOffsetY()));
            ASSERT(gEditBox_CellularLayer_ScaleX);
            gEditBox_CellularLayer_ScaleX->SetText(_F("%f", L.GetScaleX()));
            ASSERT(gEditBox_CellularLayer_ScaleY);
            gEditBox_CellularLayer_ScaleY->SetText(_F("%f", L.GetScaleY()));
            ASSERT(gEditBox_CellularLayer_Z);
            gEditBox_CellularLayer_Z->SetText(_F("%f", L.GetZ()));
        } break;

        case HEIGHT_LAYER_TYPE_MIDPOINT: {
            const MidpointLayer& L = static_cast<const MidpointLayer&>(layer);
            //			ASSERT(gEditBox_MidpointLayer_Amplitude);	gEditBox_MidpointLayer_Amplitude->SetText(_F("%f",
            //L.GetAmplitude()));
            ASSERT(gEditBox_MidpointLayer_Disp);
            gEditBox_MidpointLayer_Disp->SetText(_F("%f", L.GetDisplacement()));
            ASSERT(gEditBox_MidpointLayer_Smoothness);
            gEditBox_MidpointLayer_Smoothness->SetText(_F("%f", L.GetSmoothness()));
        } break;

        case HEIGHT_LAYER_TYPE_CRATER: {
            const CraterLayer& L = static_cast<const CraterLayer&>(layer);
            //			ASSERT(gEditBox_CraterLayer_Amplitude);		gEditBox_CraterLayer_Amplitude->SetText(_F("%f",
            //L.GetAmplitude()));
            ASSERT(gEditBox_CraterLayer_Nb);
            gEditBox_CraterLayer_Nb->SetText(_F("%d", L.GetNbCraters()));
        } break;
    ...
    if (SelectedLayer) {
        ...
        switch (ID) {
            ...
            case CHECK_BOX_CLAMP_LAYER_BOOL: {
                //	UPDATE_BOOL_VALUE(gDefault_ClampLayer_Bool, HEIGHT_LAYER_TYPE_CLAMP, ClampLayer,
                //SetCutGreaterThan)
...
static void gLayers_EditBox_CB(const IceEditBox& edit_box, udword param, void* user_data) {
    ...
    const udword ID = edit_box.GetID();
    switch (ID) {
        case EDIT_BOX_MADD_LAYER_AMPLITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_MaddLayer_Amplitude, HEIGHT_LAYER_TYPE_MADD, MaddLayer, SetAmplitude)
        } break;
        case EDIT_BOX_MADD_LAYER_ALTITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_MaddLayer_Altitude, HEIGHT_LAYER_TYPE_MADD, MaddLayer, SetAltitude)
        } break;

        // case EDIT_BOX_POWER_LAYER_AMPLITUDE:	{ UPDATE_FLOAT_VALUE(gDefault_PowerLayer_Amplitude,
        // HEIGHT_LAYER_TYPE_POWER, PowerLayer, SetAmplitude)				}	break;
        case EDIT_BOX_POWER_LAYER_FACTOR: {
            UPDATE_FLOAT_VALUE(gDefault_PowerLayer_Factor, HEIGHT_LAYER_TYPE_POWER, PowerLayer, SetPowerFactor)
        } break;

        case EDIT_BOX_CLAMP_LAYER_HEIGHT: {
            UPDATE_FLOAT_VALUE(gDefault_ClampLayer_Height, HEIGHT_LAYER_TYPE_CLAMP, ClampLayer, SetAmplitude)
        } break;
        //		case CHECK_BOX_CLAMP_LAYER_BOOL:		{ UPDATE_BOOL_VALUE(gDefault_ClampLayer_Bool,
        //HEIGHT_LAYER_TYPE_CLAMP, ClampLayer, SetCutGreaterThan)				}	break;

#ifdef OBSOLETE
        case EDIT_BOX_CONSTANT_LAYER_HEIGHT: {
            UPDATE_FLOAT_VALUE(gDefault_ConstantLayer_Height, HEIGHT_LAYER_TYPE_CONSTANT, ConstantLayer,
                               SetConstantHeight)
        } break;
        case EDIT_BOX_SCALE_LAYER_SCALE: {
            UPDATE_FLOAT_VALUE(gDefault_ScaleLayer_Scale, HEIGHT_LAYER_TYPE_SCALE, ScaleLayer, SetScaleFactor)
        } break;
#endif

        case EDIT_BOX_SINUS_LAYER_AMPLITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_SinusLayer_Amplitude, HEIGHT_LAYER_TYPE_SINUS, SinusLayer, SetAmplitude)
        } break;
        case EDIT_BOX_SINUS_LAYER_XFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_SinusLayer_XFreq, HEIGHT_LAYER_TYPE_SINUS, SinusLayer, SetXFreq)
        } break;
        case EDIT_BOX_SINUS_LAYER_YFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_SinusLayer_YFreq, HEIGHT_LAYER_TYPE_SINUS, SinusLayer, SetYFreq)
        } break;
        case EDIT_BOX_SINUS_LAYER_PHASEX: {
            UPDATE_FLOAT_VALUE(gDefault_SinusLayer_PhaseX, HEIGHT_LAYER_TYPE_SINUS, SinusLayer, SetPhaseX)
        } break;
        case EDIT_BOX_SINUS_LAYER_PHASEY: {
            UPDATE_FLOAT_VALUE(gDefault_SinusLayer_PhaseY, HEIGHT_LAYER_TYPE_SINUS, SinusLayer, SetPhaseY)
        } break;

        case EDIT_BOX_FBM_LAYER_AMPLITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_Amplitude, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetAmplitude)
        } break;
        case EDIT_BOX_FBM_LAYER_XFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_XFreq, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetXFreq)
        } break;
        case EDIT_BOX_FBM_LAYER_YFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_YFreq, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetYFreq)
        } break;
        case EDIT_BOX_FBM_LAYER_PHASEX: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_PhaseX, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetOffsetX)
        } break;
        case EDIT_BOX_FBM_LAYER_PHASEY: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_PhaseY, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetOffsetY)
        } break;
        case EDIT_BOX_FBM_LAYER_Z: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_Z, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetZ)
        } break;
        case EDIT_BOX_FBM_LAYER_FRACTAL_INC: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_FractalInc, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetFractalIncrement)
        } break;
        case EDIT_BOX_FBM_LAYER_LACUNARITY: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_Lacunarity, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetLacunarity)
        } break;
        case EDIT_BOX_FBM_LAYER_OCTAVES: {
            UPDATE_FLOAT_VALUE(gDefault_fbmLayer_Octaves, HEIGHT_LAYER_TYPE_FBM, FBMLayer, SetOctaves)
        } break;

        case EDIT_BOX_TURB_LAYER_AMPLITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_Amplitude, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer,
                               SetAmplitude)
        } break;
        case EDIT_BOX_TURB_LAYER_XFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_XFreq, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer, SetXFreq)
        } break;
        case EDIT_BOX_TURB_LAYER_YFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_YFreq, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer, SetYFreq)
        } break;
        case EDIT_BOX_TURB_LAYER_PHASEX: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_PhaseX, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer, SetOffsetX)
        } break;
        case EDIT_BOX_TURB_LAYER_PHASEY: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_PhaseY, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer, SetOffsetY)
        } break;
        case EDIT_BOX_TURB_LAYER_Z: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_Z, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer, SetZ)
        } break;
        case EDIT_BOX_TURB_LAYER_OCTAVES: {
            UPDATE_FLOAT_VALUE(gDefault_TurbLayer_Octaves, HEIGHT_LAYER_TYPE_TURBULENCE, TurbulenceLayer, SetOctaves)
        } break;

        case EDIT_BOX_RIDGED_LAYER_AMPLITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_Amplitude, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer,
                               SetAmplitude)
        } break;
        case EDIT_BOX_RIDGED_LAYER_XFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_XFreq, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetXFreq)
        } break;
        case EDIT_BOX_RIDGED_LAYER_YFREQ: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_YFreq, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetYFreq)
        } break;
        case EDIT_BOX_RIDGED_LAYER_PHASEX: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_PhaseX, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetOffsetX)
        } break;
        case EDIT_BOX_RIDGED_LAYER_PHASEY: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_PhaseY, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetOffsetY)
        } break;
        case EDIT_BOX_RIDGED_LAYER_Z: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_Z, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetZ)
        } break;
        case EDIT_BOX_RIDGED_LAYER_FRACTAL_INC: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_FractalInc, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer,
                               SetFractalIncrement)
        } break;
        case EDIT_BOX_RIDGED_LAYER_LACUNARITY: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_Lacunarity, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer,
                               SetLacunarity)
        } break;
        case EDIT_BOX_RIDGED_LAYER_OCTAVES: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_Octaves, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetOctaves)
        } break;
        case EDIT_BOX_RIDGED_LAYER_OFFSET: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_Offset, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetOffset)
        } break;
        case EDIT_BOX_RIDGED_LAYER_GAIN: {
            UPDATE_FLOAT_VALUE(gDefault_RidgedLayer_Gain, HEIGHT_LAYER_TYPE_RIDGED, RidgedFractalLayer, SetGain)
        } break;

        case EDIT_BOX_CELLULAR_LAYER_AMPLITUDE: {
            UPDATE_FLOAT_VALUE(gDefault_CellularLayer_Amplitude, HEIGHT_LAYER_TYPE_CELLULAR, CellularLayer,
                               SetAmplitude)
        } break;
        case EDIT_BOX_CELLULAR_LAYER_PHASEX: {
            UPDATE_FLOAT_VALUE(gDefault_CellularLayer_PhaseX, HEIGHT_LAYER_TYPE_CELLULAR, CellularLayer, SetOffsetX)
        } break;
        case EDIT_BOX_CELLULAR_LAYER_PHASEY: {
            UPDATE_FLOAT_VALUE(gDefault_CellularLayer_PhaseY, HEIGHT_LAYER_TYPE_CELLULAR, CellularLayer, SetOffsetY)
        } break;
        case EDIT_BOX_CELLULAR_LAYER_SCALEX: {
            UPDATE_FLOAT_VALUE(gDefault_CellularLayer_ScaleX, HEIGHT_LAYER_TYPE_CELLULAR, CellularLayer, SetScaleX)
        } break;
        case EDIT_BOX_CELLULAR_LAYER_SCALEY: {
            UPDATE_FLOAT_VALUE(gDefault_CellularLayer_ScaleY, HEIGHT_LAYER_TYPE_CELLULAR, CellularLayer, SetScaleY)
        } break;
        case EDIT_BOX_CELLULAR_LAYER_Z: {
            UPDATE_FLOAT_VALUE(gDefault_CellularLayer_Z, HEIGHT_LAYER_TYPE_CELLULAR, CellularLayer, SetZ)
        } break;

            //		case EDIT_BOX_MIDPOINT_LAYER_AMPLITUDE:	{ UPDATE_FLOAT_VALUE(gDefault_MidpointLayer_Amplitude,
            //HEIGHT_LAYER_TYPE_MIDPOINT, MidpointLayer, SetAmplitude)		}	break;
        case EDIT_BOX_MIDPOINT_LAYER_DISP: {
            UPDATE_FLOAT_VALUE(gDefault_MidpointLayer_Disp, HEIGHT_LAYER_TYPE_MIDPOINT, MidpointLayer, SetDisplacement)
        } break;
        case EDIT_BOX_MIDPOINT_LAYER_SMOOTHNESS: {
            UPDATE_FLOAT_VALUE(gDefault_MidpointLayer_Smoothness, HEIGHT_LAYER_TYPE_MIDPOINT, MidpointLayer,
                               SetSmoothness)
        } break;

            //		case EDIT_BOX_CRATER_LAYER_AMPLITUDE:	{ UPDATE_FLOAT_VALUE(gDefault_CraterLayer_Amplitude,
            //HEIGHT_LAYER_TYPE_CRATER, CraterLayer, SetAmplitude)	}	break;
        case EDIT_BOX_CRATER_LAYER_NB: {
            UPDATE_INT_VALUE(gDefault_CraterLayer_Nb, HEIGHT_LAYER_TYPE_CRATER, CraterLayer, SetNbCraters)
        } break;
...
static void CreateLayersTab(PintGUIHelper& helper, IceWindow* tab, Widgets& owner, ToolTerrainEditor* tte) {
...

    {
        ...
        tte->mAddLayer = helper.CreateButton(tab, 0, x + 280, y + LBD.mHeight + 8, 130, 20, "Add layer", &owner,
                                             gAddLayerCB, tte);
        tte->mRemoveLayer = helper.CreateButton(tab, 1, x + 280, y + LBD.mHeight + 8 + 20, 130, 20, "Remove layer",
                                                &owner, gAddLayerCB, tte);
    }
    ...
    {
        y += YStep;
        gCheckBox_EnableLayer = helper.CreateCheckBox(tab, CHECK_BOX_ENABLE_LAYER, x, y, EditBoxWidth, 20,
                                                      "Enable:", &owner, true, gLayers_CheckBox_CB, null);
    ...
    IceEditBox* LayerBox;
    {
        ...
        LayerBox = helper.CreateEditBox(tab, 0, 4, y, ToolSpecificAreaWidth, ToolSpecificAreaHeight,
                                        "=== Layer-specific settings ===", &owner, EDITBOX_TEXT, null);
...

    {
        ...
        gEditBox_MaddLayer_Amplitude = helper.CreateEditBox(
                Params, EDIT_BOX_MADD_LAYER_AMPLITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_MaddLayer_Amplitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_MaddLayer_Amplitude->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Altitude:", &owner);
        gEditBox_MaddLayer_Altitude = helper.CreateEditBox(
                Params, EDIT_BOX_MADD_LAYER_ALTITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_MaddLayer_Altitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        //		helper.CreateLabel(Params, x2, y2+LabelOffsetY, 100, 20, "Amplitude:", &owner);
        //		gEditBox_PowerLayer_Amplitude = helper.CreateEditBox(Params, EDIT_BOX_POWER_LAYER_AMPLITUDE,
        //x2+OffsetX, y2, EditBoxWidth, 20, helper.Convert(gDefault_PowerLayer_Amplitude), &owner, EDITBOX_FLOAT,
        //gLayers_EditBox_CB, null); 		gEditBox_PowerLayer_Amplitude->SetUserData(tte); 		y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Factor:", &owner);
        gEditBox_PowerLayer_Factor = helper.CreateEditBox(Params, EDIT_BOX_POWER_LAYER_FACTOR, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_PowerLayer_Factor),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        gEditBox_ClampLayer_Height = helper.CreateEditBox(Params, EDIT_BOX_CLAMP_LAYER_HEIGHT, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_ClampLayer_Height),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_ClampLayer_Height->SetUserData(tte);
        y2 += YStep;

        gCheckBox_ClampLayer_Bool =
                helper.CreateCheckBox(Params, CHECK_BOX_CLAMP_LAYER_BOOL, x2 + OffsetX, y2, EditBoxWidth, 20,
                                      "Bool:", &owner, gDefault_ClampLayer_Bool, gLayers_CheckBox_CB, null);
...
#ifdef OBSOLETE
    {
        ...
        sdword y2 = 4;
        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Height:", &owner);
        gEditBox_ConstantLayer_Height = helper.CreateEditBox(
                Params, EDIT_BOX_CONSTANT_LAYER_HEIGHT, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_ConstantLayer_Height), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        gEditBox_ScaleLayer_Scale = helper.CreateEditBox(Params, EDIT_BOX_SCALE_LAYER_SCALE, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_ScaleLayer_Scale),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Amplitude:", &owner);
        gEditBox_SinusLayer_Amplitude = helper.CreateEditBox(
                Params, EDIT_BOX_SINUS_LAYER_AMPLITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_SinusLayer_Amplitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_SinusLayer_Amplitude->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "X freq:", &owner);
        gEditBox_SinusLayer_XFreq = helper.CreateEditBox(Params, EDIT_BOX_SINUS_LAYER_XFREQ, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_SinusLayer_XFreq),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_SinusLayer_XFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Y freq:", &owner);
        gEditBox_SinusLayer_YFreq = helper.CreateEditBox(Params, EDIT_BOX_SINUS_LAYER_YFREQ, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_SinusLayer_YFreq),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_SinusLayer_YFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase X:", &owner);
        gEditBox_SinusLayer_PhaseX = helper.CreateEditBox(Params, EDIT_BOX_SINUS_LAYER_PHASEX, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_SinusLayer_PhaseX),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_SinusLayer_PhaseX->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase Y:", &owner);
        gEditBox_SinusLayer_PhaseY = helper.CreateEditBox(Params, EDIT_BOX_SINUS_LAYER_PHASEY, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_SinusLayer_PhaseY),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Amplitude:", &owner);
        gEditBox_fbmLayer_Amplitude = helper.CreateEditBox(
                Params, EDIT_BOX_FBM_LAYER_AMPLITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_fbmLayer_Amplitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_Amplitude->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "X freq:", &owner);
        gEditBox_fbmLayer_XFreq = helper.CreateEditBox(Params, EDIT_BOX_FBM_LAYER_XFREQ, x2 + OffsetX, y2, EditBoxWidth,
                                                       20, helper.Convert(gDefault_fbmLayer_XFreq), &owner,
                                                       EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_XFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Y freq:", &owner);
        gEditBox_fbmLayer_YFreq = helper.CreateEditBox(Params, EDIT_BOX_FBM_LAYER_YFREQ, x2 + OffsetX, y2, EditBoxWidth,
                                                       20, helper.Convert(gDefault_fbmLayer_YFreq), &owner,
                                                       EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_YFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase X:", &owner);
        gEditBox_fbmLayer_PhaseX = helper.CreateEditBox(Params, EDIT_BOX_FBM_LAYER_PHASEX, x2 + OffsetX, y2,
                                                        EditBoxWidth, 20, helper.Convert(gDefault_fbmLayer_PhaseX),
                                                        &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_PhaseX->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase Y:", &owner);
        gEditBox_fbmLayer_PhaseY = helper.CreateEditBox(Params, EDIT_BOX_FBM_LAYER_PHASEY, x2 + OffsetX, y2,
                                                        EditBoxWidth, 20, helper.Convert(gDefault_fbmLayer_PhaseY),
                                                        &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_PhaseY->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Z:", &owner);
        gEditBox_fbmLayer_Z = helper.CreateEditBox(Params, EDIT_BOX_FBM_LAYER_Z, x2 + OffsetX, y2, EditBoxWidth, 20,
                                                   helper.Convert(gDefault_fbmLayer_Z), &owner, EDITBOX_FLOAT,
                                                   gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_Z->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Fractal inc:", &owner);
        gEditBox_fbmLayer_FractalInc = helper.CreateEditBox(
                Params, EDIT_BOX_FBM_LAYER_FRACTAL_INC, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_fbmLayer_FractalInc), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_FractalInc->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Lacunarity:", &owner);
        gEditBox_fbmLayer_Lacunarity = helper.CreateEditBox(
                Params, EDIT_BOX_FBM_LAYER_LACUNARITY, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_fbmLayer_Lacunarity), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_fbmLayer_Lacunarity->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Octaves:", &owner);
        gEditBox_fbmLayer_Octaves = helper.CreateEditBox(Params, EDIT_BOX_FBM_LAYER_OCTAVES, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_fbmLayer_Octaves),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Amplitude:", &owner);
        gEditBox_TurbLayer_Amplitude = helper.CreateEditBox(
                Params, EDIT_BOX_TURB_LAYER_AMPLITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_TurbLayer_Amplitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_TurbLayer_Amplitude->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "X freq:", &owner);
        gEditBox_TurbLayer_XFreq = helper.CreateEditBox(Params, EDIT_BOX_TURB_LAYER_XFREQ, x2 + OffsetX, y2,
                                                        EditBoxWidth, 20, helper.Convert(gDefault_TurbLayer_XFreq),
                                                        &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_TurbLayer_XFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Y freq:", &owner);
        gEditBox_TurbLayer_YFreq = helper.CreateEditBox(Params, EDIT_BOX_TURB_LAYER_YFREQ, x2 + OffsetX, y2,
                                                        EditBoxWidth, 20, helper.Convert(gDefault_TurbLayer_YFreq),
                                                        &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_TurbLayer_YFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase X:", &owner);
        gEditBox_TurbLayer_PhaseX = helper.CreateEditBox(Params, EDIT_BOX_TURB_LAYER_PHASEX, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_TurbLayer_PhaseX),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_TurbLayer_PhaseX->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase Y:", &owner);
        gEditBox_TurbLayer_PhaseY = helper.CreateEditBox(Params, EDIT_BOX_TURB_LAYER_PHASEY, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_TurbLayer_PhaseY),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_TurbLayer_PhaseY->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Z:", &owner);
        gEditBox_TurbLayer_Z = helper.CreateEditBox(Params, EDIT_BOX_TURB_LAYER_Z, x2 + OffsetX, y2, EditBoxWidth, 20,
                                                    helper.Convert(gDefault_TurbLayer_Z), &owner, EDITBOX_FLOAT,
                                                    gLayers_EditBox_CB, null);
        gEditBox_TurbLayer_Z->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Octaves:", &owner);
        gEditBox_TurbLayer_Octaves = helper.CreateEditBox(Params, EDIT_BOX_TURB_LAYER_OCTAVES, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_TurbLayer_Octaves),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Amplitude:", &owner);
        gEditBox_RidgedLayer_Amplitude = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_AMPLITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_Amplitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_Amplitude->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "X freq:", &owner);
        gEditBox_RidgedLayer_XFreq = helper.CreateEditBox(Params, EDIT_BOX_RIDGED_LAYER_XFREQ, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_RidgedLayer_XFreq),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_XFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Y freq:", &owner);
        gEditBox_RidgedLayer_YFreq = helper.CreateEditBox(Params, EDIT_BOX_RIDGED_LAYER_YFREQ, x2 + OffsetX, y2,
                                                          EditBoxWidth, 20, helper.Convert(gDefault_RidgedLayer_YFreq),
                                                          &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_YFreq->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase X:", &owner);
        gEditBox_RidgedLayer_PhaseX = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_PHASEX, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_PhaseX), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_PhaseX->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase Y:", &owner);
        gEditBox_RidgedLayer_PhaseY = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_PHASEY, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_PhaseY), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_PhaseY->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Z:", &owner);
        gEditBox_RidgedLayer_Z = helper.CreateEditBox(Params, EDIT_BOX_RIDGED_LAYER_Z, x2 + OffsetX, y2, EditBoxWidth,
                                                      20, helper.Convert(gDefault_RidgedLayer_Z), &owner, EDITBOX_FLOAT,
                                                      gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_Z->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Fractal inc:", &owner);
        gEditBox_RidgedLayer_FractalInc = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_FRACTAL_INC, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_FractalInc), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_FractalInc->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Lacunarity:", &owner);
        gEditBox_RidgedLayer_Lacunarity = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_LACUNARITY, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_Lacunarity), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_Lacunarity->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Octaves:", &owner);
        gEditBox_RidgedLayer_Octaves = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_OCTAVES, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_Octaves), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_Octaves->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Offset:", &owner);
        gEditBox_RidgedLayer_Offset = helper.CreateEditBox(
                Params, EDIT_BOX_RIDGED_LAYER_OFFSET, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_RidgedLayer_Offset), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_RidgedLayer_Offset->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Gain:", &owner);
        gEditBox_RidgedLayer_Gain = helper.CreateEditBox(Params, EDIT_BOX_RIDGED_LAYER_GAIN, x2 + OffsetX, y2,
                                                         EditBoxWidth, 20, helper.Convert(gDefault_RidgedLayer_Gain),
                                                         &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Amplitude:", &owner);
        gEditBox_CellularLayer_Amplitude = helper.CreateEditBox(
                Params, EDIT_BOX_CELLULAR_LAYER_AMPLITUDE, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_CellularLayer_Amplitude), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_CellularLayer_Amplitude->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase X:", &owner);
        gEditBox_CellularLayer_PhaseX = helper.CreateEditBox(
                Params, EDIT_BOX_CELLULAR_LAYER_PHASEX, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_CellularLayer_PhaseX), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_CellularLayer_PhaseX->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Phase Y:", &owner);
        gEditBox_CellularLayer_PhaseY = helper.CreateEditBox(
                Params, EDIT_BOX_CELLULAR_LAYER_PHASEY, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_CellularLayer_PhaseY), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_CellularLayer_PhaseY->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Scale X:", &owner);
        gEditBox_CellularLayer_ScaleX = helper.CreateEditBox(
                Params, EDIT_BOX_CELLULAR_LAYER_SCALEX, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_CellularLayer_ScaleX), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_CellularLayer_ScaleX->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Scale Y:", &owner);
        gEditBox_CellularLayer_ScaleY = helper.CreateEditBox(
                Params, EDIT_BOX_CELLULAR_LAYER_SCALEY, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_CellularLayer_ScaleY), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_CellularLayer_ScaleY->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Z:", &owner);
        gEditBox_CellularLayer_Z = helper.CreateEditBox(Params, EDIT_BOX_CELLULAR_LAYER_Z, x2 + OffsetX, y2,
                                                        EditBoxWidth, 20, helper.Convert(gDefault_CellularLayer_Z),
                                                        &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        //		helper.CreateLabel(Params, x2, y2+LabelOffsetY, 100, 20, "Amplitude:", &owner);
        //		gEditBox_MidpointLayer_Amplitude = helper.CreateEditBox(Params, EDIT_BOX_MIDPOINT_LAYER_AMPLITUDE,
        //x2+OffsetX, y2, EditBoxWidth, 20, helper.Convert(gDefault_MidpointLayer_Amplitude), &owner, EDITBOX_FLOAT,
        //gLayers_EditBox_CB, null); 		gEditBox_MidpointLayer_Amplitude->SetUserData(tte); 		y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Displacement:", &owner);
        gEditBox_MidpointLayer_Disp = helper.CreateEditBox(
                Params, EDIT_BOX_MIDPOINT_LAYER_DISP, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_MidpointLayer_Disp), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
        gEditBox_MidpointLayer_Disp->SetUserData(tte);
        y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Smoothness:", &owner);
        gEditBox_MidpointLayer_Smoothness = helper.CreateEditBox(
                Params, EDIT_BOX_MIDPOINT_LAYER_SMOOTHNESS, x2 + OffsetX, y2, EditBoxWidth, 20,
                helper.Convert(gDefault_MidpointLayer_Smoothness), &owner, EDITBOX_FLOAT, gLayers_EditBox_CB, null);
...

    {
        ...
        //		helper.CreateLabel(Params, x2, y2+LabelOffsetY, 100, 20, "Amplitude:", &owner);
        //		gEditBox_CraterLayer_Amplitude = helper.CreateEditBox(Params, EDIT_BOX_CRATER_LAYER_AMPLITUDE,
        //x2+OffsetX, y2, EditBoxWidth, 20, helper.Convert(gDefault_CraterLayer_Amplitude), &owner, EDITBOX_FLOAT,
        //gLayers_EditBox_CB, null); 		gEditBox_CraterLayer_Amplitude->SetUserData(tte); 		y2 += YStep;

        helper.CreateLabel(Params, x2, y2 + LabelOffsetY, 100, 20, "Nb craters:", &owner);
        gEditBox_CraterLayer_Nb = helper.CreateEditBox(Params, EDIT_BOX_CRATER_LAYER_NB, x2 + OffsetX, y2, EditBoxWidth,
                                                       20, _F("%d", gDefault_CraterLayer_Nb), &owner,
                                                       EDITBOX_INTEGER_POSITIVE, gLayers_EditBox_CB, null);
...
static void CreateTextureTab(PintGUIHelper& helper, IceWindow* tab, Widgets& owner, ToolTerrainEditor* tte) {
...
#endif
                {
                    const ManagedTexture* MT =
                            CreateManagedTexture(TT.GetWidth(), TT.GetHeight(), TT.GetPixels(), "TerrainTexture");
                }
    ...
    /*	helper.CreateLabel(tab, x, y+LabelOffsetY, 100, 20, "Tile nb:", &owner);
            helper.CreateEditBox(tab, EDIT_BOX_TILE_NB, x+OffsetX, y, EditBoxWidth, 20, _F("%d", gTileNb), &owner,
       EDITBOX_INTEGER_POSITIVE, Callbacks::gEditBox, null); y += YStep;
       ...
       EDITBOX_INTEGER_POSITIVE, Callbacks::gEditBox, null); y += YStep;
            ...
            helper.CreateEditBox(tab, EDIT_BOX_TILE_SCALE, x+OffsetX, y, EditBoxWidth, 20, helper.Convert(gTileScale),
       &owner, EDITBOX_FLOAT_POSITIVE, Callbacks::gEditBox, null); y += YStep;*/
    ...
    if (0) {
        gCheckBox_EnableTexture =
                helper.CreateCheckBox(tab, CHECK_BOX_ENABLE_TEXTURE, x, y, EditBoxWidth, 20, "Enable:", &owner,
                                      gTexture_Enabled, Callbacks::gCheckBox_CB, null);
        gCheckBox_EnableTexture->SetUserData(tte);
        y += YStep;

        gCheckBox_AutoTextureUpdate =
                helper.CreateCheckBox(tab, CHECK_BOX_AUTO_TEXTURE_UPDATE, x, y, EditBoxWidth, 20,
                                      "Auto update:", &owner, gTexture_AutoUpdate, Callbacks::gCheckBox_CB, null);
