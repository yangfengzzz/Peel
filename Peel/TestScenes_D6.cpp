...
static bool CheckD6Caps(const PintCaps& caps) {
    if (!caps.mSupportRigidBodySimulation || !caps.mSupportD6Joints) return false;
    return true;
}
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    if (!CheckD6Caps(caps)) return false;
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        sdword y = 0;
        {
            mCheckBox_UsePositionDrive = helper.CreateCheckBox(
                    UI, 0, 4, y, 400, 20, "Use position drive (else velocity drive)", &UIElems, false, null, null);
            ...
            mEditBox_Stiffness = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
            ...
            mEditBox_Damping = helper.CreateEditBox(UI, 2, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
                                                    EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_Damping->SetEnabled(InitialEnabled);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Target velocity:", &UIElems);
            mEditBox_TargetVelocity = helper.CreateEditBox(UI, 2, 4 + OffsetX, y, EditBoxWidth, 20, "2.0", &UIElems,
            ...
            mEditBox_ObstacleMass = helper.CreateEditBox(UI, 2, 4 + OffsetX, y, EditBoxWidth, 20, "0.0", &UIElems,
                                                         EDITBOX_FLOAT, null, null);
        ...
        /*		{
                                ...
                                class MyComboBox : public IceComboBox
                                {
                                        ...
                                        public:
                                                                        MyComboBox(const ComboBoxDesc& desc,
           D6Joint_PrismaticDrive& test) : IceComboBox(desc), mTest(test)			{} virtual
           ~MyComboBox()			{} virtual	void	OnComboBoxEvent(ComboBoxEvent event)
                                        {
                                                if(event==CBE_SELECTION_CHANGED)
                                                {
                                                        ...
                                                        {
                                                                mTest.mEditBox_Size->SetText(_F("%d",
           gPreset_GridSize[SelectedIndex])); mTest.mEditBox_Multiplier->SetText(_F("%d",
           gPreset_Multiplier[SelectedIndex]));
    ...
    virtual udword Update(Pint& pint, float dt) {
        ...
        if (UserData->mDynamicObject) {
            ...
            if (mUsePosDrive) {
                ...
                //			pint.SetLinearVelocity(UserData->mDynamicObject, Point(mTargetVelocity, 0.0f,
                // 0.0f));
            }
        }
    ...
    virtual float DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale) {
        ...
        if (Handle) {
            ...
            renderer.print(0.0f, y, text_scale,
                           _F("Linear velocity: %.3f | %.3f | %.3f\n", LinVel.x, LinVel.y, LinVel.z));
            y -= text_scale;
        }
        return y;
    }
...
public:
    ...
    virtual IceTabControl* InitUI(PintGUIHelper& helper) {
        ...
        {
            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Stiffness:", &UIElems);
            mEditBox_Stiffness = helper.CreateEditBox(UI, 1, 4 + OffsetX, y, EditBoxWidth, 20, "1000.0", &UIElems,
                                                      EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_Stiffness->SetEnabled(InitialEnabled);
            y += YStep;

            helper.CreateLabel(UI, 4, y + LabelOffsetY, LabelWidth, 20, "Damping:", &UIElems);
            mEditBox_Damping = helper.CreateEditBox(UI, 2, 4 + OffsetX, y, EditBoxWidth, 20, "100.0", &UIElems,
                                                    EDITBOX_FLOAT_POSITIVE, null, null);
            mEditBox_Damping->SetEnabled(InitialEnabled);
            y += YStep;
...
static PintJointHandle CreateD6Hinge(Pint& pint,
                                     const PINT_BOX_CREATE& box_desc,
                                     const Point& pos,
                                     const Matrix3x3& rot,
                                     float box_size,
                                     float min_angle,
...
virtual float DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale) {
    ...
    if (!GetD6DynamicData(pint, Handle, Data)) return y;
...
static const char* gDesc_D6Joint_Hinge3 =
        "D6 joint - hinge joint with PI limits around the twist axis, and a rotated joint frame.";
...
virtual bool Setup(Pint& pint, const PintCaps& caps) {
    ...
    Matrix3x3 RotY;
    RotY.RotY(APIRotY * DEGTORAD);
    Matrix3x3 RotZ;
    RotZ.RotZ(APIRotZ * DEGTORAD);
...
virtual float DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale) {
    ...
    renderer.print(0.0f, y, text_scale, _F("Swing Y angle: %.5f\n", Data.mSwingYAngle * RADTODEG + APIRotY));
    y -= text_scale;
    renderer.print(0.0f, y, text_scale, _F("Swing Z angle: %.5f\n", Data.mSwingZAngle * RADTODEG + APIRotZ));
    y -= text_scale;
    return y;
}
