...
ShadowShader::ShadowShader() {
    for (int i = 0; i < mNumLights; i++) mShadowMaps[i] = NULL;
    mReflectionTexId = 0;
    mReflectionWidth = 0;
    mReflectionHeight = 0;
...
void ShadowShader::Activate(const ShaderProps& mat) {
    ...
    if (mReflectionWidth && mReflectionHeight)
        GLShader::SetUniform2f(mShaderProgram, "reflectionSize", 1.0f / float(mReflectionWidth),
                               1.0f / float(mReflectionHeight));
    ...
    eyeDir = camTrans.rotate(SpotLightDir0);
    eyeDir.normalize();
    ...
    eyeDir = camTrans.rotate(SpotLightDir1);
    eyeDir.normalize();
    ...
    eyeDir = camTrans.rotate(SpotLightDir2);
    eyeDir.normalize();
    ...
    eyeDir = camTrans.rotate((const PxVec3&)mBackLightDir);
    eyeDir.normalize();
    GLShader::SetUniform3f(mShaderProgram, "parallelLightDir", eyeDir.x, eyeDir.y, eyeDir.z);

    if (mShadowMaps[0]) {
        GLShader::SetUniform4f(mShaderProgram, "far_d", mShadowMaps[0]->getFarBound(0), mShadowMaps[0]->getFarBound(1),
                               mShadowMaps[0]->getFarBound(2), mShadowMaps[0]->getFarBound(3));
    }

    for (int i = 0; i < mNumLights; i++) {
        if (mShadowMaps[i]) mShadowMaps[i]->prepareForRender(mCamModelView, mCamProj);
    }
...
void ShadowShader::Deactivate() {
    ...
    for (int i = 0; i < mNumLights; i++) {
        if (mShadowMaps[i]) mShadowMaps[i]->doneRender();
    }
