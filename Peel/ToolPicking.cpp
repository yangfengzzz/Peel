...
void ToolPicking::RightDownCallback(Pint& pint, udword pint_index) {
    ...
    if (Raycast(pint, Hit, mOrigin, mDir) && !IsDefaultEnv(pint, Hit.mTouchedActor)) {
        ...
        if (gDrawTouchedTriangle) {
            ...
            Triangle Tri;
            if (FindTouchedTriangle(Tri, pint, Hit)) PD.mTouchedTriangle = Tri;
        }
        ...
        if (gDebugRadius > 0.0f) {
            ...
            if (FindTouchedTriangle(Tri, pint, Hit)) {
                ...
                mHits.SetNoHit();
                mStream.Reset();
                pint.FindTriangles_MeshSphereOverlap(pint.mSQHelper->GetThreadContext(), Hit.mTouchedActor, 1, &mHits,
                                                     mStream, &Data);
            }
        }
    } else {
...
void ToolPicking::PreRenderCallback() { mIsControllingCamera = false; }
...
void ToolPicking::RenderCallback(PintRender& render, Pint& pint, udword pint_index) {
    ...
    if (PD.mPickedObject) {
...

        {
            ...
            if (gDrawTouchedTriangle) {
                GLRenderHelpers::DrawTriangle(PD.mTouchedTriangle.mVerts[0], PD.mTouchedTriangle.mVerts[1],
                                              PD.mTouchedTriangle.mVerts[2], Point(1.0f, 1.0f, 0.0f));
                GLRenderHelpers::DrawTriangle(PD.mTouchedTriangle.mVerts[0], PD.mTouchedTriangle.mVerts[2],
                                              PD.mTouchedTriangle.mVerts[1], Point(1.0f, 1.0f, 0.0f));
            }

            float Mass = 1.0f;
            Pint_Actor* API = pint.GetActorAPI();
            if (API) Mass = API->GetMass(PD.mPickedObject);
...
bool ToolPicking::IsControllingCamera() const { return mIsControllingCamera; }
...
void ToolPicking::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner) {
    ...
    struct Local {
        ...
        static void CheckBoxCallback(const IceCheckBox& check_box, bool checked, void* user_data) {
            if (check_box.GetID() == 0) gDrawTouchedTriangle = checked;
            //			else if(check_box.GetID()==1)
            //				gDebugMode = checked;
        }

        static void ButtonCallback(IceButton& button, void* user_data) { ((ToolPicking*)user_data)->CreateDebugMesh(); }
    };

    sdword y = 0;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 90, 20, "Picking force:", &owner);
    helper.CreateEditBox(parent, 0, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gPickingForce), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::EditBoxCallback, gTooltip_PickingForce);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 90, 20, "Linear damping:", &owner);
    helper.CreateEditBox(parent, 1, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gLinearDamping), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::EditBoxCallback, gTooltip_LinearDamping);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 90, 20, "Angular damping:", &owner);
    helper.CreateEditBox(parent, 2, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gAngularDamping), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::EditBoxCallback, gTooltip_AngularDamping);
    y += YStep;

    helper.CreateLabel(parent, x, y + LabelOffsetY, 90, 20, "Debug radius:", &owner);
    helper.CreateEditBox(parent, 3, x + OffsetX, y, EditBoxWidth, 20, helper.Convert(gDebugRadius), &owner,
                         EDITBOX_FLOAT_POSITIVE, Local::EditBoxCallback /*, gTooltip_AngularDamping*/);
    y += YStep;

    helper.CreateCheckBox(parent, 0, x, y, CheckBoxWidth, 20, "Draw touched triangle", &owner, gDrawTouchedTriangle,
                          Local::CheckBoxCallback /*, gTooltip_AngularDamping*/);
    y += YStep;

    //	helper.CreateCheckBox(parent, 1, x, y, CheckBoxWidth, 20, "Debug mode", &owner, gDebugMode,
    //Local::CheckBoxCallback/*, gTooltip_AngularDamping*/); 	y += YStep;

    IceButton* b = helper.CreateButton(parent, 0, x, y, ButtonWidth, 20, "Create debug mesh", &owner,
                                       Local::ButtonCallback, this /*, const char* tooltip*/);
    y += YStep;
}
