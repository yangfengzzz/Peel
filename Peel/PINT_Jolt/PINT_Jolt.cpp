...
- shapes with non-idt local poses or with a non-zero COM are a special case (same design as Bullet/Havok), while it's
the default in PhysX.
...
"The higher the frequency, the quicker the body will move to its target. If you set a frequency of 1 then it will
oscillate 1 time per second around the target position, if you set it to 20 it will oscillate 20 times per second around
the target position. You should never go higher than half of your simulation frequency (otherwise: instability).

Damping prevents overshoot. A damping of 0 means that the body will oscillate forever around its target position (in
theory that is, with the integrator that Jolt uses that won't happen). A damping of 1 will barely overshoot the target
and have almost no oscillation, but it will also take slightly longer to reach the target. I would say sensible ranges
for frequency are 0.1-20 and for damping 0-1."
*/
...
static udword gMaxBodies =
        65536;  // "This is the max amount of rigid bodies that you can add to the physics system. If you try to add
                // more you'll get an error. For a real project use something in the order of 65536."
static udword gMaxBodyPairs =
        65536;  // "This is the max amount of body pairs that can be queued at any time (the broad phase will detect
                ...
                // start to do narrow phase work. This is slightly less efficient. For a real project use something in
                // the order of 65536."
static udword gMaxContactConstraints =
...
static udword gNbBodyMutexes = 0;  // "This determines how many mutexes to allocate to protect rigid bodies from
                                   // concurrent access. Set it to 0 for the default settings."
static udword gNbPosIter = 2;      // Default value in Jolt = 2
static udword gNbVelIter =
        10;  // Default value in Jolt = 10. "Note that this needs to be >= 2 in order for friction to work (friction is
             // applied using the non-penetration impulse from the previous iteration)"
...
public:
    MyGroupFilterTable() {
        for (int i = 0; i < 32; i++)
            for (int j = 0; j < 32; j++) mMyTable[j][i] = true;
    }

    ~MyGroupFilterTable() { DELETESINGLE(mPM); }
    ...
    virtual bool CanCollide(const CollisionGroup& inGroup1, const CollisionGroup& inGroup2) const override {
        ...
        if (mPM && mPM->FindPair(inGroup1.GetGroupID(), inGroup2.GetGroupID())) return false;
...
public:
    ...
    explicit MyTempAllocatorImpl(uint inSize) : mBase(static_cast<uint8*>(malloc(inSize))), mSize(inSize) {}
    ...
    virtual void* Allocate(uint inSize) override {
        if (inSize == 0) {
            ...
            if (new_top > mSize) JPH_CRASH;  // Out of memory
    ...
    virtual void Free(void* inAddress, uint inSize) override {
        if (inAddress == nullptr) {
            ...
            if (mBase + mTop != inAddress) JPH_CRASH;  // Freeing in the wrong order
        }
    }
...
static void* AllocateHook(size_t inSize) { return TagAllocation(malloc(inSize + 16), 16, inSize); }

static void FreeHook(void* inBlock) { free(UntagAllocation(inBlock, 16)); }
...
static void AlignedFreeHook(void* inBlock) { _aligned_free(UntagAllocation(inBlock, 64)); }
...
bool Jolt_SceneAPI::AddActors(udword nb_actors, const PintActorHandle* actors) {
    ...
    while (nb_actors--) {
        ...
        body_interface.AddBody(CurrentActor->GetID(),
                               CurrentActor->IsStatic() ? EActivation::DontActivate : EActivation::Activate);
    }
    return true;
}
...
const char* JoltPint::GetUIName() const { return "Jolt"; }
...
void JoltPint::Init(const PINT_WORLD_CREATE& desc) {
    ...
    gPhysicsSystem->Init(gMaxBodies, gNbBodyMutexes, gMaxBodyPairs, gMaxContactConstraints, gBroadPhaseLayerInterface,
...
void JoltPint::SetGravity(const Point& gravity) {
    if (gPhysicsSystem) gPhysicsSystem->SetGravity(ToVec3(gravity));
}
...
udword JoltPint::Update(float dt) {
    ...
    if (gPhysicsSystem) {
        ...
        // If you want more accurate step results you can do multiple sub steps within a collision step. Usually you
        // would set this to 1.
...
Point JoltPint::GetMainColor() { return Point(0.5f, 0.9f, 0.8f); }
...
void JoltPint::Render(PintRender& renderer, PintRenderPass render_pass) {
    ...
    for (BodyID ID : BodyIDs) {
        ...
        if (Lock.SucceededAndIsInBroadPhase()) {
            ...
            if (!renderer.SetCurrentActor(PintActorHandle(Current))) continue;

            // Collect all leaf shapes
            Collector.Reset();
            Current->GetShape()->CollectTransformedShapes(
                    AABox(Vec3::sReplicate(-1.0e6f), Vec3::sReplicate(1.0e6f)), Current->GetCenterOfMassPosition(),
                    Current->GetRotation(), JPH::Vec3::sReplicate(1.0f), JPH::SubShapeIDCreator(), Collector, {});
            ...
            for (const TransformedShape& TS : Collector.mHits) {
                ...
                if (Renderer) {
                    const PR Pose(ToPoint(TS.mShapePositionCOM - TS.mShapeRotation * TS.mShape->GetCenterOfMass()),
                                  ToIQuat(TS.mShapeRotation));
...
static void SetupDynamicActorSettings(BodyCreationSettings& settings,
...
static void SetupOffsetShape(Ref<JPH::Shape>& shape, const PR& local_pose) {
    if (!local_pose.IsIdentity()) {
        const RotatedTranslatedShapeSettings OffsetShapeSettings(ToVec3(local_pose.mPos), ToJQuat(local_pose.mRot),
                                                                 shape);
...
Ref<JPH::Shape> JoltPint::CreateShape(const PINT_SHAPE_CREATE* shape_create) {
    ...
    switch (shape_create->mType) {
        ...
        case PINT_SHAPE_CYLINDER: {
            ...
            shape = new CylinderShape(Create->mHalfHeight, Create->mRadius,
                                      TMin(cDefaultConvexRadius, Create->mHalfHeight));
        } break;
        ...
        case PINT_SHAPE_CONVEX: {
            ...
            for (udword i = 0; i < Create->mNbVerts; i++) pts.push_back(ToVec3(Create->mVerts[i]));
            ...
            if (r.IsValid())
                ...
                Vec3 Avg = Vec3::sZero();
                for (Vec3 P : pts) Avg += P;
                Avg = Avg / float(pts.size());

                // Determine radius
                float RadiusSq = 0.0f;
                for (Vec3 P : pts) RadiusSq = max(RadiusSq, (P - Avg).LengthSq());
    ...
    if (AllowSharing) mCachedShapes.push_back({Renderer, LocalPose, shape});
...
PintActorHandle JoltPint::CreateObject(const PINT_OBJECT_CREATE& desc) {
    ...
    if (NewBody) {
        ...
        // TODO: keeping the sequence number for now because I didn't bother flushing the map when objects are deleted.
        // A proper implementation would revisit this.
        NewBody->SetCollisionGroup(CollisionGroup(gGroupFilter, NewBody->GetID().GetIndexAndSequenceNumber(),
                                                  CollisionGroup::SubGroupID(desc.mCollisionGroup)));
    }
...
bool JoltPint::ReleaseObject(PintActorHandle handle) {
    ...
    // Remove the body from the physics system. Note that the body itself keeps all of its state and can be re-added at
    // any time.
...
PintJointHandle JoltPint::CreateJoint(const PINT_JOINT_CREATE& desc) {
    ...
    if (!Actor0) Actor0 = &Body::sFixedToWorld;
    if (!Actor1) Actor1 = &Body::sFixedToWorld;
    ...
    switch (desc.mType) {
        ...
        case PINT_JOINT_HINGE: {
            ...
            settings.mNormalAxis2 = (Actor1->GetInverseCenterOfMassTransform() * Actor0->GetCenterOfMassTransform())
                                            .Multiply3x3(settings.mNormalAxis1)
                                            .Normalized();
        ...
        case PINT_JOINT_D6: {
            ...
            // We don't know how this joint is going to be driven yet, tentatively set it to velocity until the call to
            // SetDriveVelocity/SetDrivePosition
        ...
        case PINT_JOINT_RACK_AND_PINION: {
            ...
            settings.mSpace = EConstraintSpace::LocalToBodyCOM;
            // TODO: This is very fragile but I have no idea how I'm supposed to get the constraint axis from the
            // creation settings as mLocalPivot0/1 is usually not filled in
            ...
            RackAndPinionConstraint* NewJoint =
                    static_cast<RackAndPinionConstraint*>(settings.Create(*Actor0, *Actor1));
    ...
    if (J) {
        ...
        if (gGroupFilter)
            gGroupFilter->DisableJointedBodies(Actor0->GetID().GetIndexAndSequenceNumber(),
                                               Actor1->GetID().GetIndexAndSequenceNumber());
    }
...
void JoltPint::SetDisabledGroups(udword nb_groups, const PintDisabledGroups* groups) {
    if (!gGroupFilter) return;
    for (udword i = 0; i < nb_groups; i++) gGroupFilter->DisableCollision(groups[i].mGroup0, groups[i].mGroup1);
}
...
PintHeightfieldHandle JoltPint::CreateHeightfieldObject(const PINT_HEIGHTFIELD_DATA_CREATE& desc,
    ...
    Settings->mScale = Vec3(
            float(desc.mNbV) / Settings->mSampleCount, 1.0f,
            float(desc.mNbU) / Settings->mSampleCount);  // U and V seem to be Z and X so we have to flip everything
...
udword JoltPint::BatchRaycasts(PintSQThreadContext context,
                               udword nb,
                               PintRaycastHit* dest,
                               const PintRaycastData* raycasts) {
    if (!gPhysicsSystem) return 0;
...
udword JoltPint::BatchRaycastAny(PintSQThreadContext context,
                                 udword nb,
                                 PintBooleanHit* dest,
                                 const PintRaycastData* raycasts) {
    if (!gPhysicsSystem) return 0;
    ...
    RayCastSettings inRayCastSettings;
    inRayCastSettings.mBackFaceMode =
            gBackfaceCulling ? EBackFaceMode::IgnoreBackFaces : EBackFaceMode::CollideWithBackFaces;
...
udword JoltPint::BatchBoxSweeps(PintSQThreadContext context,
                                udword nb,
                                PintRaycastHit* dest,
    ...
    while (nb--) {
        ...
        const ShapeCast shape_cast{&QueryShape, Vec3::sReplicate(1.0f), Mat44(v0, v1, v2, v3),
                                   ToVec3(sweeps->mDir * MaxDist)};
...
udword JoltPint::BatchSphereSweeps(PintSQThreadContext context,
                                   udword nb,
                                   PintRaycastHit* dest,
    ...
    while (nb--) {
        ...
        const ShapeCast shape_cast{&QueryShape, Vec3::sReplicate(1.0f),
                                   Mat44::sTranslation(ToVec3(sweeps->mSphere.mCenter)),
                                   ToVec3(sweeps->mDir * MaxDist)};
...
static IQuat _ShortestRotation(const Point& v0, const Point& v1) {
    ...
    IQuat q = d > -1.0f            ? IQuat(1.0f + d, cross.x, cross.y, cross.z)
              : fabsf(v0.x) < 0.1f ? IQuat(0.0f, 0.0f, v0.z, -v0.y)
                                   : IQuat(0.0f, v0.y, -v0.x, 0.0f);
...
udword JoltPint::BatchCapsuleSweeps(PintSQThreadContext context,
                                    udword nb,
                                    PintRaycastHit* dest,
    ...
    while (nb--) {
        ...
        const ShapeCast shape_cast{&QueryShape, Vec3::sReplicate(1.0f),
                                   Mat44::sRotationTranslation(ToJQuat(q), ToVec3(Center)),
                                   ToVec3(sweeps->mDir * MaxDist)};
...
/*udword JoltPint::BatchConvexSweeps(PintSQThreadContext context, udword nb, PintRaycastHit* dest, const
PintConvexSweepData* sweeps)
{
        const NarrowPhaseQuery& NPQ = gPhysicsSystem->GetNarrowPhaseQuery();
        ...
        while(nb--)
        {
                ...
                const ShapeCast shape_cast { &QueryShape, Vec3::sReplicate(1.0f),
Mat44::sRotationTranslation(ToJQuat(q), ToVec3(Center)), ToVec3(sweeps->mDir * MaxDist) };
...
//			dest->mDistance = collector.mHit.mFraction * MaxDist;
                                ...
                                //const Vec3 outPosition = ToVec3(sweeps->mBox.mCenter) + collector.mHit.mFraction *
shape_cast.mDirection; const Vec3 outPosition = collector.mHit.mContactPointOn2;

                                dest->mImpact	= ToPoint(outPosition);
                                dest->mDistance = collector.mHit.mFraction * MaxDist;

                                //BodyLockRead lock(gPhysicsSystem->GetBodyLockInterface(), collector.mHit.mBodyID2);
                                BodyLockRead lock(gPhysicsSystem->GetBodyLockInterfaceNoLock(),
collector.mHit.mBodyID2); if (lock.Succeeded())
                                {
                                        const Body& hit_body = lock.GetBody();

                                        const Vec3 normal =
hit_body.GetWorldSpaceSurfaceNormal(collector.mHit.mSubShapeID2, outPosition); dest->mNormal			=
ToPoint(normal);
...
udword JoltPint::BatchSphereOverlapAny(PintSQThreadContext context,
                                       udword nb,
                                       PintBooleanHit* dest,
    ...
    while (nb--) {
        ...
        NPQ.CollideShape(&QueryShape, Vec3::sReplicate(1.0f), Mat44::sTranslation(ToVec3(overlaps->mSphere.mCenter)),
                         settings, collector);
...
udword JoltPint::BatchSphereOverlapObjects(PintSQThreadContext context,
                                           udword nb,
    ...
    while (nb--) {
        ...
        if (Nb) {
            PintOverlapHit* buffer =
                    reinterpret_cast<PintOverlapHit*>(stream.Reserve(Nb * (sizeof(PintOverlapHit) / sizeof(udword))));
...
/*udword JoltPint::BatchBoxOverlapAny(PintSQThreadContext context, udword nb, PintBooleanHit* dest, const
PintBoxOverlapData* overlaps)
{
        return 0;
}
...
udword JoltPint::BatchBoxOverlapObjects(PintSQThreadContext context, udword nb, PintOverlapObjectHit* dest, const
PintBoxOverlapData* overlaps)
{
        return 0;
}
...
udword JoltPint::BatchCapsuleOverlapAny(PintSQThreadContext context, udword nb, PintBooleanHit* dest, const
PintCapsuleOverlapData* overlaps)
{
        return 0;
}

udword JoltPint::BatchCapsuleOverlapObjects(PintSQThreadContext context, udword nb, PintOverlapObjectHit* dest, const
PintCapsuleOverlapData* overlaps)
{
        return 0;
}
...
udword JoltPint::FindTriangles_MeshSphereOverlap(PintSQThreadContext context, PintActorHandle handle, udword nb, const
PintSphereOverlapData* overlaps)
{
        return 0;
}
...
udword JoltPint::FindTriangles_MeshBoxOverlap(PintSQThreadContext context, PintActorHandle handle, udword nb, const
PintBoxOverlapData* overlaps)
{
        return 0;
}
...
udword JoltPint::FindTriangles_MeshCapsuleOverlap(PintSQThreadContext context, PintActorHandle handle, udword nb, const
PintCapsuleOverlapData* overlaps)
{
        return 0;
}*/
...
Point JoltPint::GetLinearVelocity(PintActorHandle handle) {
    const MotionProperties* MP = GetDynamicData(handle);
    if (!MP) return Point(0.0f, 0.0f, 0.0f);
...
static void gJolt_GetOptionsFromGUI(const char* test_name) {
    if (gCheckBox_AllowSleeping) gAllowSleeping = gCheckBox_AllowSleeping->IsChecked();
    if (gCheckBox_AllowShapeSharing) gAllowShapeSharing = gCheckBox_AllowShapeSharing->IsChecked();
    if (gCheckBox_CCD) gEnableCCD = gCheckBox_CCD->IsChecked();
    if (gCheckBox_BackfaceCulling) gBackfaceCulling = gCheckBox_BackfaceCulling->IsChecked();
...
static IceEditBox* CreateEditBox(PintGUIHelper& helper,
    ...
    IceEditBox* EB =
            helper.CreateEditBox(parent, INVALID_ID, 4 + EditBoxX2, y, EditBoxWidth, 20, value, gJoltGUI, filter, null);
    y += YStep;
    return EB;
}
...
IceWindow* Jolt_InitGUI(IceWidget* parent, PintGUIHelper& helper) {
    ...
    gCheckBox_AllowSleeping = helper.CreateCheckBox(Main, JOLT_GUI_ALLOW_SLEEPING, 4, y, CheckBoxWidth, 20,
                                                    "Allow sleeping", gJoltGUI, gAllowSleeping, gCheckBoxCallback);
    y += YStepCB;

    gCheckBox_AllowShapeSharing =
            helper.CreateCheckBox(Main, JOLT_GUI_ALLOW_SHAPE_SHARING, 4, y, CheckBoxWidth, 20, "Allow shape sharing",
                                  gJoltGUI, gAllowShapeSharing, gCheckBoxCallback);
    y += YStepCB;

    gCheckBox_CCD = helper.CreateCheckBox(Main, JOLT_GUI_ENABLE_CCD, 4, y, CheckBoxWidth, 20, "Enable CCD", gJoltGUI,
                                          gEnableCCD, gCheckBoxCallback);
    y += YStepCB;

    gCheckBox_BackfaceCulling =
            helper.CreateCheckBox(Main, JOLT_GUI_BACKFACE_CULLING, 4, y, CheckBoxWidth, 20,
                                  "Backface culling (scene queries)", gJoltGUI, gBackfaceCulling, gCheckBoxCallback);
    y += YStep;

    gEditBox_NbThreads = CreateEditBox(helper, Main, y, "Nb threads (0==automatic):", _F("%d", gNbThreads),
                                       EDITBOX_INTEGER_POSITIVE);
    gEditBox_NbSubsteps =
            CreateEditBox(helper, Main, y, "Nb substeps:", _F("%d", gNbSubsteps), EDITBOX_INTEGER_POSITIVE);
    gEditBox_TempAllocSize =
            CreateEditBox(helper, Main, y, "Tmp alloc size (Mb):", _F("%d", gTempAllocSize), EDITBOX_INTEGER_POSITIVE);
    gEditBox_MaxBodies = CreateEditBox(helper, Main, y, "Max bodies:", _F("%d", gMaxBodies), EDITBOX_INTEGER_POSITIVE);
    gEditBox_MaxBodyPairs =
            CreateEditBox(helper, Main, y, "Max body pairs:", _F("%d", gMaxBodyPairs), EDITBOX_INTEGER_POSITIVE);
    gEditBox_MaxContactConstraints = CreateEditBox(
            helper, Main, y, "Max contact constraints:", _F("%d", gMaxContactConstraints), EDITBOX_INTEGER_POSITIVE);
    gEditBox_NbBodyMutexes =
            CreateEditBox(helper, Main, y, "Nb body mutexes:", _F("%d", gNbBodyMutexes), EDITBOX_INTEGER_POSITIVE);
    ...
    gEditBox_LinearDamping =
            CreateEditBox(helper, Main, y, "Linear damping:", helper.Convert(gLinearDamping), EDITBOX_FLOAT_POSITIVE);
    gEditBox_AngularDamping =
            CreateEditBox(helper, Main, y, "Angular damping:", helper.Convert(gAngularDamping), EDITBOX_FLOAT_POSITIVE);
    gEditBox_SpeculativeContactDistance =
            CreateEditBox(helper, Main, y, "Speculative contact distance:", helper.Convert(gSpeculativeContactDistance),
                          EDITBOX_FLOAT_POSITIVE);
    gEditBox_PenetrationSlop = CreateEditBox(helper, Main, y, "Max penetration slop:", helper.Convert(gPenetrationSlop),
                                             EDITBOX_FLOAT_POSITIVE);
    gEditBox_Baumgarte =
            CreateEditBox(helper, Main, y, "Baumgarte:", helper.Convert(gBaumgarte), EDITBOX_FLOAT_POSITIVE);
    gEditBox_Friction = CreateEditBox(helper, Main, y, "Default friction:", helper.Convert(gDefaultFriction),
                                      EDITBOX_FLOAT_POSITIVE);
    gEditBox_Restitution = CreateEditBox(helper, Main, y, "Default restitution:", helper.Convert(gDefaultRestitution),
                                         EDITBOX_FLOAT_POSITIVE);
...
public:
    virtual IceWindow* InitGUI(IceWidget* parent, PintGUIHelper& helper) override {
        return Jolt_InitGUI(parent, helper);
    }
...
static IceWindow* CreateUI_CCD(IceWidget* parent, PintGUIHelper& helper, Widgets& owner, const char* test_name) {
    ...
    struct Override {
        static void CCD(const IceCheckBox& check_box, bool checked, void* user_data) {
            gCheckBox_Generic0->SetEnabled(checked);
        }
    };
    ASSERT(!gCheckBox_Override);
    gCheckBox_Override = helper.CreateCheckBox(TabWindow, 0, x, y, 200, 20, "Override main panel settings", &owner,
                                               true, Override::CCD, null);
    y += YStep;
    ...
    const sdword CheckBoxWidth2 = 220;
    {
        ...
        gCheckBox_Generic0 = helper.CreateCheckBox(TabWindow, 0, x2, y, CheckBoxWidth2, 20, "Enable Jolt CCD", &owner,
                                                   EnableCCD, null, null);
        y += YStep;
...
static bool IsCCDTest(const char* test_name) { return strncmp(test_name, "CCDTest_", 8) == 0; }

IceWindow* JoltPlugIn::InitTestGUI(const char* test_name, IceWidget* parent, PintGUIHelper& helper, Widgets& owner) {
    if (IsCCDTest(test_name)) return CreateUI_CCD(parent, helper, owner, test_name);
