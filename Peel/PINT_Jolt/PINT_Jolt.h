...
public:
    ...
    /*		virtual	udword			GetNbShapes(PintActorHandle handle)				const
       override; virtual	PintShapeHandle	GetShape(PintActorHandle handle, udword index)	const	override;

                    virtual	udword			GetNbJoints(PintActorHandle handle)				const
       override; virtual	PintJointHandle	GetJoint(PintActorHandle handle, udword index)	const	override;*/
    ...
    /*		virtual	void			WakeUp(PintActorHandle handle)	override;
       ...
       override; virtual	bool			SetDebugVizFlag(PintActorHandle handle, bool flag)
       override; virtual	bool			SetSimulationFlag(PintActorHandle handle, bool flag)
       override;*/
    ...
    /*		virtual	bool			SetLocalInertia(PintActorHandle handle, const Point& inertia)
       ...
       override; virtual	bool			SetCMassLocalPose(PintActorHandle handle, const PR& pose)
       override;*/
};
...
public:
    ...
    //		virtual	udword					BatchConvexSweeps			(PintSQThreadContext context, udword nb, PintRaycastHit* dest,
    ...
    virtual udword BatchSphereOverlapObjects(PintSQThreadContext context,
                                             ...
                                             Container& stream,
                                             const PintSphereOverlapData* overlaps) override;
    /*		virtual	udword					BatchBoxOverlapAny			(PintSQThreadContext context, udword nb, PintBooleanHit*
       dest, const PintBoxOverlapData* overlaps); virtual	udword
       BatchBoxOverlapObjects		(PintSQThreadContext context, udword nb, PintOverlapObjectHit* dest, const
       PintBoxOverlapData* overlaps);
                    virtual	udword					BatchCapsuleOverlapAny		(PintSQThreadContext context,
       udword nb, PintBooleanHit* dest, const PintCapsuleOverlapData* overlaps); virtual	udword
       BatchCapsuleOverlapObjects	(PintSQThreadContext context, udword nb, PintOverlapObjectHit* dest, const
       PintCapsuleOverlapData* overlaps);
                    //
                    virtual	udword					FindTriangles_MeshSphereOverlap	(PintSQThreadContext
       context, PintActorHandle handle, udword nb, const PintSphereOverlapData* overlaps); virtual	udword
       FindTriangles_MeshBoxOverlap	(PintSQThreadContext context, PintActorHandle handle, udword nb, const
       PintBoxOverlapData* overlaps);
                    virtual	udword					FindTriangles_MeshCapsuleOverlap(PintSQThreadContext context,
       PintActorHandle handle, udword nb, const PintCapsuleOverlapData* overlaps);*/
