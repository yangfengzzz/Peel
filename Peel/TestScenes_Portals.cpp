...
inline_ const PR ToPR(const PxTransform& pose) { return PR(ToPoint(pose.p), ToQuat(pose.q)); }
}
...
public:
    ...
    PintActorHandle CreateBackPortalGeom(Pint& pint) const {
        if (gDynamicPortals) return null;
    ...
    PintActorHandle CreatePortalGeom(Pint& pint) const {
        ...
        if (1) {
            ...
            /*					const Point BackExtents = Box.mExtents + Point(PortalBorderSize,
               PortalBorderSize, PortalBorderSize); PINT_BOX_CREATE BoxDescBack(BackExtents); BoxDescBack.mLocalPos.z
               = PortalBorderSize*2.0f * (mPositive ? -1.0f : 1.0f); BoxDescBack.mRenderer	=
               CreateBoxRenderer(BackExtents); BoxDescBack.mMaterial	= &LowFrictionMaterial; BoxDescB.mNext
               = &BoxDescBack;*/
    ...
    void Render(PintRender& renderer) {
        ...
        if (0) {
            ...
            //					renderer.DrawLine(Box.mCenter, Box.mCenter + Box.mRot[0]*Box.mExtents.x, Point(1.0f,
            //0.0f,
            // 0.0f)); 					renderer.DrawLine(Box.mCenter, Box.mCenter + Box.mRot[1]*Box.mExtents.y, Point(0.0f, 1.0f,
            // 0.0f)); 					renderer.DrawLine(Box.mCenter, Box.mCenter + Box.mRot[2]*Box.mExtents.z, Point(0.0f,
            // 0.0f, 1.0f));
    ...
    OBB GetOBB() const { return OBB(mPose.mPos, mExtents, mPose.mRot); }
...
public:
    ...
    virtual ContactModif ModifyContact(Pint& pint,
                                       ...
                                       udword feature0,
                                       udword feature1) {
        if (mH0 == mPortalGeomHandle0 || mH0 == mPortalGeomHandle1 || mH1 == mPortalGeomHandle0 ||
            mH1 == mPortalGeomHandle1)
            return CONTACT_AS_IS;

        if (!mCanTeleport0 && !mCanTeleport1) return CONTACT_AS_IS;

        PhysicsPortal* Portal0 = &mPortal0;
        PhysicsPortal* Portal1 = &mPortal1;
        if (mCanTeleport1) TSwap(Portal0, Portal1);
        ...
        if (mH0 == mBackPortalGeomHandle0 || mH0 == mBackPortalGeomHandle1 || mH1 == mBackPortalGeomHandle0 ||
            mH1 == mBackPortalGeomHandle1)
            KeepContact = false;

        if (KeepContact) {
            return CONTACT_AS_IS;
        } else {
            if (gUseContactIgnore) return CONTACT_IGNORE;
    ...
    void CreateShadowObject(
            Pint& pint, const PxTransform& pose, const PxTransform& delta, const Point& linVel, const Point& angVel) {
        if (!mHandle1) {
    ...
    virtual udword Update(Pint& pint, float dt) {
        struct Local {
            static bool UpdateRenderer(Pint& pint,
                                       PintActorHandle handle,
                                       const OBB& ObjectBox,
                                       const PhysicsPortal& portal,
                                       PintDLMeshShapeRenderer2* renderer,
                                       bool clip) {
                Point Pts[8];
                ...
                if (!clip) {
                    if (renderer) renderer->Init(TS);
                    //					pint.EnableGravity(handle, true);
                    Pint_Actor* API = pint.GetActorAPI();
                    if (API) API->SetGravityFlag(handle, true);
                    return true;
                }
                ...
                if (renderer) renderer->Init(Surface);
                ...
                if (NbFaces == 12) {
                    ...
                    //					const float d = PL.Distance(ObjectBox.mCenter) * portal.mPositive ?
                    //-1.0f
                    //: 1.0f; 					const float d = PL.Distance(ObjectBox.mCenter) *
                    //: portal.mPositive ? 1.0f : -1.0f;
                ...
                Pint_Actor* API = pint.GetActorAPI();
                if (API) API->SetGravityFlag(handle, gravity);
                return gravity;
            }
        };

        if (gDynamicPortals) {
            if (mPortalGeomHandle0) mPortal0.mPose = pint.GetWorldTransform(mPortalGeomHandle0);

            if (mPortalGeomHandle1) mPortal1.mPose = pint.GetWorldTransform(mPortalGeomHandle1);
        ...
        if (mHandle0) {
            ...
            if (mPortal0.mPositive) mBehindPortal0 = !mBehindPortal0;
            mBehindPortal1 = PortalPlane1.Distance(ObjectBox.mCenter) > 0.0f;
            if (mPortal1.mPositive) mBehindPortal1 = !mBehindPortal1;
            ...
            if (mOverlap0 && !mBehindPortal0) mCanTeleport0 = true;
            if (mOverlap1 && !mBehindPortal1) mCanTeleport1 = true;
            ...
            if (mCanTeleport0 && !mHandle1) {
                ...
                CreateShadowObject(pint, mExpectedPose0to1, mDelta0to1, ToPoint(ExpectedLinVel),
                                   ToPoint(ExpectedAngVel));
            }
            ...
            if (mCanTeleport1 && !mHandle1) {
                ...
                CreateShadowObject(pint, mExpectedPose1to0, mDelta1to0, ToPoint(ExpectedLinVel),
                                   ToPoint(ExpectedAngVel));
            }
            ...
            /*			if(mHandle0)
                                    {
                                            ...
                                            Local::UpdateRenderer(pint, mHandle0, ObjectBox, mPortal0, mRenderer0,
               true);
                                    }
                                    if(mHandle1)
                                    {
                                            ...
                                            const OBB ObjectBox(Pose.mPos, mExtents, Pose.mRot);
                                            Local::UpdateRenderer(pint, mHandle1, ObjectBox, mPortal1, mRenderer1,
               true);
                                    }*/
            ...
            if (mCanTeleport0 || mCanTeleport1) {
                ...
                TriSurface TS;
                if (mRenderer1) mRenderer1->Init(TS);
            }
        }
    ...
    virtual void CommonDebugRender(PintRender& renderer) {
        if (0) {
            renderer.DrawWireframeOBB(OBB(ToPoint(mExpectedPose0to1.p), mExtents, ToQuat(mExpectedPose0to1.q)),
                                      Point(1.0f, 0.75f, 0.0f));
            renderer.DrawWireframeOBB(OBB(ToPoint(mExpectedPose1to0.p), mExtents, ToQuat(mExpectedPose1to0.q)),
                                      Point(0.0f, 0.0f, 0.75f));
        }

        //		renderer.DrawWirefameOBB(OBB(ToPoint(mLastFreePose.p), mExtents, ToQuat(mLastFreePose.q)),
