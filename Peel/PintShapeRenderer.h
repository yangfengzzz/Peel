...
enum ShapeRendererFlags {
    ...
    //		SHAPE_RENDERER_USES_BATCHING				= (1<<4),	// Skips per-shape SetupMaterial / SetupGeometry
    //calls
};
...
protected:
    PintShapeRenderer(/*const ShapeRendererType type,*/ udword flags = SHAPE_RENDERER_CAN_BE_SHARED)
        : mRenderSource(null),
          ...
          mRefCount(1)
    //											mID
    //(INVALID_ID), 											mExportable		(false) mType			(type)
...
public:
    ...
    inline_ udword _UseExplicitVertexNormals() const {
        return mBaseFlags & SHAPE_RENDERER_USES_EXPLICIT_VERTEX_NORMALS;
    }
...
PintShapeRenderer* CreateMeshRenderer(const PintSurfaceInterface& surface,
                                      const Point* normals = null,
                                      bool active_edges = false,
                                      bool direct_data = false);
PintShapeRenderer* CreateMeshRenderer(const MultiSurface& multi_surface,
                                      bool active_edges = false,
                                      bool direct_data = false);

// Purely experimental
PintShapeRenderer* CreateColorShapeRenderer(PintShapeRenderer* renderer,
...
public:
    ...
    virtual udword GetNbRenderers() const override {
        return mRenderData.GetNbEntries() / (sizeof(RenderData) / sizeof(udword));
    }
    virtual const RenderData* GetRenderers() const override {
        return (const PintRendererCollection::RenderData*)mRenderData.GetEntries();
    }
